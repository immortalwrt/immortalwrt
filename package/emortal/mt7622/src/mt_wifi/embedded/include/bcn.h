...
#define MAX_BEACON_LENGTH                                                      \
	(sizeof(HEADER_802_11) + TIMESTAMP_FIELD_LEN +                         \
	 BEACON_INTERVAL_FIELD_LEN + CAP_INFO_FIELD_LEN + MAX_IE_LENGTH)

enum BCN_UPDATE_REASON {
	BCN_UPDATE_INIT = 0, /* beacon resource initial. */
	BCN_UPDATE_IF_STATE_CHG =
		1, /* there is interface up or down, check related TXD handle. */
	BCN_UPDATE_IE_CHG =
		2, /* simple IE change, just update the corresponding interface content. */
	BCN_UPDATE_AP_RENEW =
		3, /* prepared All beacon for All active interface. */
	BCN_UPDATE_PRETBTT =
		4, /* update function routine, source could from INT isr or timer or event notify. */
