...
INT CFG80211_OpsChanWithSet(
	...
	if (NULL == wiphy || NULL == dev || NULL == chandef) {
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			("[%s](%d): ERROR, Invalid parameter: pWiphy = %p;net_dev = %p; chandef = %p\n",
		__func__, __LINE__, wiphy, dev, chandef));
		return -EINVAL;
	}
	...
	CFG80211DBG(DBG_LVL_TRACE, ("[%s](%d): Channel = %d, CenterChanId = %d\n",
	...
	CFG80211DBG(DBG_LVL_OFF, ("[%s](%d): ChanType = %d\n", __func__, __LINE__, chaninfo.ChanType));
	...
	IN INT32							 timeout)
{
	...
	if (!inf_wdev) {
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			("[%s](%d): ERROR, Invalid data: inf_wdev = %p\n",
	...
	struct station_info *pstainfo)
{
	...
	if (NULL == pstainfo) {
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			("[%s](%d): ERROR, Invalid data bw = NULL;\n",
		__func__, __LINE__));
		return FALSE;
	}
	...
	struct station_info *pstainfo)
{
	...
	if (NULL == pbss_info || NULL == pstainfo) {
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			("[%s](%d): ERROR, pbss_info = %p, pstainfo = %p;\n",
		__func__, __LINE__, pbss_info, pstainfo));
		return FALSE;
	}
	...
	struct station_info *pstainfo)
{
	...
	if (NULL == pdrv_stainfo || NULL == pstainfo) {
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			("[%s](%d): ERROR, pdrv_stainfo = %p, pstainfo = %p;\n",
		__func__, __LINE__, pdrv_stainfo, pstainfo));
		return FALSE;
	}
	...
	MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			("[%s](%d): rx_beacon = %llu, rx_loss = %u\n",
		__func__, __LINE__, pstainfo->rx_beacon, pstainfo->beacon_loss_count));
...
static INT CFG80211_FILL_BW(CHAR *bw)
{
	...
	default:
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			("[%s](%d): default ERROR, Invalid data bw = %d;\n",
	...
	IN struct station_info				*pSinfo)
{
	...
	if (NULL == pWiphy || NULL == pNdev || NULL == pSinfo) {
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			("[%s](%d): ERROR, Invalid data: pWiphy = %p; pNdev = %p; pSinfo = %p\n",
		__func__, __LINE__, pWiphy, pNdev, pSinfo));
		return -ENOENT;
	}
	...
	if (StaInfo.tx_packets > 0) {
		...
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			...
			__func__, __LINE__, pSinfo->txrate.legacy/CFG_LEGACY_RATE,
	...
	if (StaInfo.sta_flags.mask & BIT(NL80211_STA_FLAG_ASSOCIATED)) {
		...
		if (FALSE == CFG80211_FILL_BSS_PARAM(&StaInfo.bss_param, pSinfo)) {
			MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			("[%s](%d):ERROR, CFG80211_FILL_BSS_PARAM Fail!\n",
			__func__, __LINE__));
		}
	}
...
/*beacon statistics*/
	...
	if (TRUE != CFG80211_FILL_BCN_PARAM(&StaInfo, pSinfo)) {
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			("[%s](%d):ERROR, CFG80211_FILL_BCN_PARAM Fail!\n",
			__func__, __LINE__));
	}
#endif/*WIFI_IAP_BCN_STAT_FEATURE*/
#endif/*KERNEL_VERSION(4, 0, 0)*/
	...
	IN UINT32							Changed)
{
	...
	CFG80211DBG(DBG_LVL_TRACE, ("80211> %s ==>distance or COVERAGE_threshold(%d), ack_time=%d us\n",
	...
	CFG80211DBG(DBG_LVL_TRACE, ("[%s](%d): SET ACK TIMEOUT SUCCESS!\n",
		__func__, __LINE__));
	return 0;
}
	...
	INT datalen)
{
	...
	CFG80211DBG(DBG_LVL_TRACE, ("80211> %s datalen:%d==> ", __func__, datalen));

	hex_dump_with_lvl("testmode_buffer", (unsigned char *) data, datalen, DBG_LVL_TRACE);
		...
		IN struct cfg80211_qos_map *qos_map)
{
	...
	if (qos_map == NULL) {
		MTWF_LOG(DBG_CAT_INIT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Qos Map NULL\n"));
		return 0;
	}

	apidx = CFG80211_FindMbssApIdxByNetDevice(pAd, dev);
	if (apidx != WDEV_NOT_FOUND) {
		CFG80211DBG(DBG_LVL_TRACE, ("Setting Qos Param for apidx %d \n", apidx));
		RTMP_DRIVER_80211_QOS_PARAM_SET(pAd, qos_map, apidx);
	} else
		MTWF_LOG(DBG_CAT_INIT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("AP Index for setting qos map not found\n"));
	...
	u32 rx_ant)
{
	...
	if (tx_ant_count == 0 || rx_ant_count == 0) {
		ret = -1;
		CFG80211DBG(DBG_LVL_TRACE, ("80211> Wrong input parameter!\n"));
	} else {
		CFG80211DBG(DBG_LVL_TRACE, ("80211> TX ant count = %x, RX ant count = %x\n", tx_ant_count, rx_ant_count));
...
#endif /* LINUX_VERSION_CODE: 3.6.0 */
{
	...
	if (pWiphy->flags && WIPHY_FLAG_4ADDR_STATION)
		CFG80211DBG(DBG_LVL_OFF, ("Support WIPHY_FLAG_4ADDR_STATION\n"));
	...
	if (pDev == NULL)
		CFG80211DBG(DBG_LVL_OFF, ("CFG80211_OpsVirtualInfAdd pDev Null\n"));
