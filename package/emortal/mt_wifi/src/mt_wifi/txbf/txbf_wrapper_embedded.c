	...
	if (wdev->wdev_type == WDEV_TYPE_STA) {
		...
		if (pStaCfg) {
			if (pStaCfg->MlmeAux.vht_cap.vht_cap.bfer_cap_su) {
				...
				MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: %s num_snd_dimension=%d, bfee_sts_cap=%d, bfer=%d, bfee=%d\n",
	...
	struct he_bf_info *he_bf_struct)
{
	...
	UINT8 tx_nss = wlan_config_get_tx_stream(wdev);/* tx_stream: 1 for 1T, 2 for 2T, etc. */
	VOID *hdev_ctrl = hc_get_hdev_ctrl(wdev);
	struct _RTMP_CHIP_OP *ops = hc_get_chip_ops(hdev_ctrl);

	if (he_bw > HE_BW_80) {
		if (tx_nss > (mcs_nss->bw160_max_nss))/* bw160_max_nss: 1 for 1T, 2 for 2T, etc. */
			tx_nss = mcs_nss->bw160_max_nss;
	}

	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_INFO,
		("%s: mcs_nss->bw160_max_nss=%u, tx_nss=%u\n", __func__, mcs_nss->bw160_max_nss, tx_nss));

	txbf_status.he_bf_info = he_bf_struct;
	txbf_status.ucTxPathNum = tx_nss;
