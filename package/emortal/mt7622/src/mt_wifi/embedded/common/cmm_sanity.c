...
/*
    ...
    ==========================================================================
 */
BOOLEAN MlmeAddBAReqSanity(IN PRTMP_ADAPTER pAd, IN VOID *Msg, IN ULONG MsgLen,
			   OUT PUCHAR pAddr2)
{
...
BOOLEAN MlmeDelBAReqSanity(IN PRTMP_ADAPTER pAd, IN VOID *Msg, IN ULONG MsgLen)
{
	...
	if (NdisEqualMemory(pAd->MacTab.Content[pInfo->Wcid].Addr, pInfo->Addr,
			    MAC_ADDR_LEN) == 0) {
		MTWF_LOG(
			DBG_CAT_MLME, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
...
BOOLEAN PeerAddBAReqActionSanity(IN PRTMP_ADAPTER pAd, IN VOID *pMsg,
...
BOOLEAN PeerAddBARspActionSanity(IN PRTMP_ADAPTER pAd, IN VOID *pMsg,
				 IN ULONG MsgLen)
{
...
BOOLEAN PeerDelBAActionSanity(IN PRTMP_ADAPTER pAd, IN UCHAR Wcid,
...
static inline void copy_to_vie(UCHAR *ptr, USHORT *len_vie, UCHAR *ptr_eid,
			       EID_STRUCT *eid)
{
 ...
 */
BOOLEAN
PeerBeaconAndProbeRspSanity(IN PRTMP_ADAPTER pAd, IN struct wifi_dev *wdev,
			    IN VOID *Msg, IN ULONG MsgLen, IN UCHAR MsgChannel,
			    OUT BCN_IE_LIST *ie_list, OUT USHORT *LengthVIE,
			    OUT PNDIS_802_11_VARIABLE_IEs pVIE,
			    IN BOOLEAN bGetDtim, IN BOOLEAN bFromBeaconReport)
{
	UCHAR *Ptr;
	...
	while ((Length + 2 + pEid->Len) <= MsgLen) {
		...
		case IE_SUPP_RATES:
			if (pEid->Len <= MAX_LEN_OF_SUPPORTED_RATES) {
				...
				parse_support_rate_ie(
					(struct dev_rate_info *)&ie_list
						->SupRate[0],
					pEid);
				/*
		...
		case IE_CHANNEL_SWITCH_ANNOUNCEMENT:
			if (pEid->Len == 3) {
				ie_list->NewChannel =
					pEid->Octet[1]; /*extract new channel number*/
#ifdef CONFIG_RCSA_SUPPORT
				NdisMoveMemory(&CsaInfo->ChSwAnnIE,
					       &pEid->Octet[0], pEid->Len);
#endif
			}
			break;
		...
		case IE_EXT_CHANNEL_SWITCH_ANNOUNCEMENT:
			if (pEid->Len == 4) {
				if (ie_list->NewChannel == 0)
					ie_list->NewChannel =
						pEid->Octet[2]; /*extract new channel number*/
				NdisMoveMemory(&CsaInfo->ExtChSwAnnIE,
					       &pEid->Octet[0], pEid->Len);
			}
			break;
		...
		case IE_WIDE_BW_CH_SWITCH:
			if (pEid->Len == 3)
				NdisMoveMemory(&CsaInfo->wb_info,
					       &pEid->Octet[0], pEid->Len);
			break;
#endif
		...
		*/
		...
		case IE_VENDOR_SPECIFIC:
			...
			check_vendor_ie(pAd, (UCHAR *)pEid,
					&(ie_list->vendor_ie));
			...
			if (NdisEqualMemory(pEid->Octet, OWE_TRANS_OUI, 4)) {
				...
				Ptr = (PUCHAR)pVIE;
				NdisMoveMemory(Ptr + *LengthVIE, ptr_eid,
					       pEid->Len + 2);
				*LengthVIE += (pEid->Len + 2);
				...
				if (pair_ch != 0) {
					UINT32 idx = 0;
					UINT32 max_con_bands =
						HcGetAmountOfBand(pAd);
				...
				if (NdisEqualMemory(pEid->Octet, WPA_OUI, 4)) {
					...
					NdisMoveMemory(Ptr + *LengthVIE,
						       ptr_eid, pEid->Len + 2);
					*LengthVIE += (pEid->Len + 2);
				} else if (NdisEqualMemory(pEid->Octet,
							   WME_PARM_ELEM, 6) &&
					   (pEid->Len == 24)) {
					PUCHAR ptr;
					int i;
					...
					ie_list->EdcaParm.bQAck =
						FALSE; /* pEid->Octet[0] & 0x10;*/
					ie_list->EdcaParm.bQueueRequest =
						FALSE; /* pEid->Octet[0] & 0x20;*/
					ie_list->EdcaParm.bTxopRequest =
					...
					for (i = 0; i < 4; i++) {
						...
						ie_list->EdcaParm.bACM[aci] =
							(((*ptr) & 0x10) ==
							 0x10); /* b5 is ACM*/
						ie_list->EdcaParm.Aifsn[aci] =
							(*ptr) &
							0x0f; /* b0~3 is AIFSN*/
						ie_list->EdcaParm.Cwmin[aci] =
							*(ptr + 1) &
							0x0f; /* b0~4 is Cwmin*/
						ie_list->EdcaParm.Cwmax[aci] =
							*(ptr + 1) >>
							4; /* b5~8 is Cwmax*/
					...
					ie_list->EdcaParm.Cwmax[QID_AC_VI] =
						pAd->wmm_cw_max;
					ie_list->EdcaParm.Txop[QID_AC_VI] =
					...
					ie_list->EdcaParm.Cwmax[QID_AC_VO] =
						pAd->wmm_cw_max - 1;
					ie_list->EdcaParm.Txop[QID_AC_VO] =
						48; /* AC_VO: 48*32us ~= 1.5ms*/
				} else if (NdisEqualMemory(pEid->Octet, WPS_OUI,
							   4)) {
					/*
			...
			if (IS_OCE_ENABLE(wdev) &&
			    !pOceCtrl->Scan11bOceAPTimerRunning) {
				if (NdisEqualMemory(pEid->Octet,
						    MBO_OCE_OUIBYTE, 4) &&
				    (pEid->Len >= 5))
					bHasOceIE = OceCheckOceCap(pAd, wdev,
								   pEid->Octet,
								   pEid->Len);
			}
#endif
		...
		case IE_EXT_SUPP_RATES:
			parse_support_ext_rate_ie(
				(struct dev_rate_info *)&ie_list->ExtRate[0],
				pEid);
			break;
		...
		case IE_RSN:
			...
			if (RTMPEqualMemory(pEid->Octet + 2, RSN_OUI, 3)) {
				...
				copy_to_vie((UCHAR *)pVIE, LengthVIE, ptr_eid,
					    pEid);
			}
		...
		case IE_EXT_CAPABILITY:
			if (pEid->Len >= 1) {
				...
				(*(UINT32 *)(&(ie_list->ExtCapInfo))) =
					le2cpu32(*(UINT32 *)(&(
						ie_list->ExtCapInfo)));
				(*(UINT32 *)((UCHAR *)&(ie_list->ExtCapInfo) +
					     4)) =
					le2cpu32(*(
						UINT32 *)((UCHAR *)&(
								  ie_list->ExtCapInfo) +
		...
		case IE_VHT_CAP:
			if (pEid->Len == sizeof(VHT_CAP_IE)) {
				...
				NdisCopyMemory(&tmp_1,
					       &ie_list->vht_cap_ie.vht_cap, 4);
				tmp_1 = le2cpu32(tmp_1);
				NdisCopyMemory(&ie_list->vht_cap_ie.vht_cap,
					       &tmp_1, 4);

				NdisCopyMemory(&tmp_2,
					       &(ie_list->vht_cap_ie.mcs_set),
					       8);
				tmp_2 = le2cpu64(tmp_2);
				NdisCopyMemory(&(ie_list->vht_cap_ie.mcs_set),
					       &tmp_2, 8);
#endif
			}
...
#ifdef RT_BIG_ENDIAN
				NdisCopyMemory(
					&tmp, &ie_list->vht_op_ie.basic_mcs_set,
					sizeof(VHT_MCS_MAP));
				tmp = le2cpu16(tmp);
				NdisCopyMemory(
					&ie_list->vht_op_ie.basic_mcs_set, &tmp,
					sizeof(VHT_MCS_MAP));
#endif
			}
		...
		case IE_OPERATING_MODE_NOTIFY:
			if (pEid->Len == sizeof(OPERATING_MODE) &&
			    (bFromBeaconReport == FALSE)) {
#ifdef CONFIG_AP_SUPPORT
				MAC_TABLE_ENTRY *pEntry =
					MacTableLookup(pAd, pFrame->Hdr.Addr2);
#endif
				OPERATING_MODE op_mode;

				NdisMoveMemory(&op_mode, &pEid->Octet[0],
		...
		case IE_CH_SWITCH_WRAPPER: {
			...
			while (ch_sw_wrp_len > 0) {
				...
				if (*subelement == IE_WIDE_BW_CH_SWITCH) {
					NdisMoveMemory(&ie_list->wb_info,
						       subelement,
						       len_subelement);
					break;
				} else {
		...
		Length = Length + 2 +
			 pEid->Len; /* Eid[1] + Len[1]+ content[Len]*/
...
SanityCheck:
	...
	if ((Sanity != 0x7) || (bWscCheck == FALSE)) {
		if (((bFromBeaconReport == FALSE) &&
		     (Sanity != 0x7)) /* case 1: */
		    || ((bFromBeaconReport == TRUE) &&
			(!(Sanity & 0x1))) /* case 2: */
		    || (bWscCheck == FALSE) /* case 3 */
		) {
			MTWF_LOG(
				DBG_CAT_MLME, DBG_SUBCAT_ALL, DBG_LVL_LOUD,
				("%s() - missing field, Sanity=0x%02x, bWscCheck=%d\n",
	...
	if (IS_OCE_ENABLE(wdev) && !pOceCtrl->Scan11bOceAPTimerRunning) {
		if (!bHasOceIE) {
			OceNonOcePresentOldValue = OCE_GET_CONTROL_FIELD(
				pOceCtrl->OceCapIndication,
				OCE_NONOCE_PRESENT_MASK,
				OCE_NONOCE_PRESENT_OFFSET);
			OCE_SET_CONTROL_FIELD(pOceCtrl->OceCapIndication, 1,
					      OCE_NONOCE_PRESENT_MASK,
					      OCE_NONOCE_PRESENT_OFFSET);
...
/*
	...
	==========================================================================
 */
BOOLEAN PeerBeaconAndProbeRspSanity2(IN PRTMP_ADAPTER pAd, IN VOID *Msg,
				     IN ULONG MsgLen,
...
/*
    ...
    ==========================================================================
 */
BOOLEAN MlmeScanReqSanity(IN PRTMP_ADAPTER pAd, IN VOID *Msg, IN ULONG MsgLen,
			  OUT UCHAR *pBssType, OUT CHAR Ssid[],
...
#ifdef DBDC_MODE
#ifdef WSC_INCLUDED
UCHAR CheckWPSTriggeredPerBand(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem)
{
	UCHAR bss_index = 0;
	UCHAR current_band;
	...
	for (bss_index = 0; bss_index < pAd->ApCfg.BssidNum; bss_index++) {
		...
		*/
		if ((current_band ==
		     HcGetBandByWdev(&pAd->ApCfg.MBSSID[bss_index].wdev)) &&
		    pAd->ApCfg.MBSSID[bss_index].wdev.WscControl.WscConfMode !=
			    WSC_DISABLE &&
		    pAd->ApCfg.MBSSID[bss_index].wdev.WscControl.bWscTrigger ==
			    TRUE)
			break;
	}
...
/*
    ...
    ==========================================================================
 */
BOOLEAN PeerDeauthSanity(IN PRTMP_ADAPTER pAd, IN VOID *Msg, IN ULONG MsgLen,
...
/*
    ...
    ==========================================================================
 */
BOOLEAN PeerAuthSanity(IN PRTMP_ADAPTER pAd, IN VOID *Msg, IN ULONG MsgLen,
...
BOOLEAN MlmeAuthReqSanity(IN PRTMP_ADAPTER pAd, IN VOID *Msg, IN ULONG MsgLen,
			  OUT PUCHAR pAddr, OUT ULONG *pTimeout,
			  OUT USHORT *pAlg)
{
...
/*
    ...
    ==========================================================================
 */
BOOLEAN PeerDisassocSanity(IN PRTMP_ADAPTER pAd, IN VOID *Msg, IN ULONG MsgLen,
...
/*
    ...
    ==========================================================================
 */
BOOLEAN PeerProbeReqSanity(IN PRTMP_ADAPTER pAd, IN VOID *Msg, IN ULONG MsgLen,
			   OUT PEER_PROBE_REQ_PARAM *ProbeReqParam)
{
	...
	NdisMoveMemory(ProbeReqParam->Ssid, &Fr->Octet[2],
	...
	while ((eid_data + eid_len) <= ((UCHAR *)Fr + MsgLen)) {
		...
		case IE_VENDOR_SPECIFIC:
			...
			if ((pAd->ApCfg.ap_customer_oui.pointer != NULL) &&
			    ...
			    NdisEqualMemory(eid_data,
					    pAd->ApCfg.ap_customer_oui.pointer,
					    pAd->ApCfg.ap_customer_oui.length)) {
				ProbeReqParam->report_param.vendor_ie
					.element_id = IE_VENDOR_SPECIFIC;
				ProbeReqParam->report_param.vendor_ie.len =
					eid_len;
				NdisMoveMemory(ProbeReqParam->report_param
						       .vendor_ie.custom_ie,
					       eid_data, eid_len);
				break;
			}
			...
			if (NdisEqualMemory(eid_data, WPS_OUI, 4)) {
...
#endif /* CONFIG_AP_SUPPORT */
#ifdef WSC_INCLUDED
				Elem = CONTAINER_OF(Msg, MLME_QUEUE_ELEM,
						    Msg[0]);
				current_band =
					HcGetBandByChannel(pAd, Elem->Channel);
				...
				if (pPeerWscIe) {
					...
					if (*(eid_data + 4) ==
					    0x10) { /* First WPS IE will have version 0x10 */
...
#ifdef OCE_SUPPORT
#ifdef CONFIG_AP_SUPPORT
			if (NdisEqualMemory(eid_data, MBO_OCE_OUIBYTE, 4) &&
			    (eid_len >= 5))
				OceParseStaOceIE(pAd, eid_data, eid_len,
						 ProbeReqParam);
#endif
#endif
			break;
			...
			if (ProbeReqParam->IsOceCapability &&
			    *(eid_data) == FILS_REQ_ID_EXTENSION &&
				...
				if (!pOceCtrl->MaxChannelTimerRunning) {
					RTMPSetTimer(&pOceCtrl->MaxChannelTimer,
						     MaxChannelTime);
		...
		case IE_INTERWORKING:
			...
			if (eid_len > 3) {
				if (eid_len == 7)
					NdisMoveMemory(ProbeReqParam->Hessid,
						       eid_data + 1,
						       MAC_ADDR_LEN);
				else
					NdisMoveMemory(ProbeReqParam->Hessid,
...
/* WPS_BandSteering Support */
		...
		*/
		...
		if ((pAd->ApCfg.MBSSID[apidx].wdev.WscControl.WscConfMode &
		     WSC_PROXY) != WSC_DISABLE) {
			int bufLen = 0;
			PUCHAR pBuf = NULL;
			...
			*/
			...
			if (pBuf) {
				...
				if (ProbeReqParam->SsidLen <=
				    32) { /*Well, I think that it must be TRUE!*/
					NdisMoveMemory(
						pprobreq->ssid,
						ProbeReqParam->Ssid,
						ProbeReqParam->SsidLen); /* SSID*/
					NdisMoveMemory(pprobreq->macAddr,
						       Fr->Hdr.Addr2,
						       6); /* Mac address*/
					pprobreq->data[0] = PeerWscIeLen >>
							    8; /* element ID*/
					pprobreq->data[1] =
						PeerWscIeLen &
						0xff; /* element Length					*/
					NdisMoveMemory(
						(pBuf +
						 sizeof(WSC_IE_PROBREQ_DATA)),
						pPeerWscIe,
						PeerWscIeLen); /* (WscProbeReqData)*/
					WscSendUPnPMessage(
						pAd, apidx,
						WSC_OPCODE_UPNP_MGMT,
						WSC_UPNP_MGMT_SUB_PROBE_REQ,
						pBuf, bufLen, 0, 0,
						&Fr->Hdr.Addr2[0], AP_MODE);
				}
...
*/
BOOLEAN
IsPublicActionFrame(IN PRTMP_ADAPTER pAd, IN VOID *pbuf)
{
