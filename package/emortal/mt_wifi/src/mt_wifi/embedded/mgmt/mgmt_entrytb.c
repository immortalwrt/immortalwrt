...
static void aid_dump(struct _aid_info *aid_info)
{
	...
	if (aid_info->aid_bitmap == NULL) {
		MTWF_LOG(DBG_CAT_ALL, CATMLME_WTBL, DBG_LVL_ERROR,
			("no aid_bitmap\n"));
		return;
	}
...
VOID TRTableEntryDump(RTMP_ADAPTER *pAd, INT tr_idx, const RTMP_STRING *caller, INT line)
{
	...
	MTWF_LOG(DBG_CAT_MLME, CATMLME_WTBL, DBG_LVL_OFF, ("Dump TR_ENTRY called by function %s(%d)\n", caller, line));
	tr_entry = &tr_ctl->tr_entry[tr_idx];
	MTWF_LOG(DBG_CAT_MLME, CATMLME_WTBL, DBG_LVL_OFF, ("TR_ENTRY[%d]\n", tr_idx));
	MTWF_LOG(DBG_CAT_MLME, CATMLME_WTBL, DBG_LVL_OFF, ("\tEntryType=%x\n", tr_entry->EntryType));
	...
	MTWF_LOG(DBG_CAT_MLME, CATMLME_WTBL, DBG_LVL_OFF, ("\tTxRx Characters\n"));
	MTWF_LOG(DBG_CAT_MLME, CATMLME_WTBL, DBG_LVL_OFF, ("\t\tNonQosDataSeq=%d\n", tr_entry->NonQosDataSeq));
	MTWF_LOG(DBG_CAT_MLME, CATMLME_WTBL, DBG_LVL_OFF, ("\t\tTxSeq[0]=%d, TxSeq[1]=%d, TxSeq[2]=%d, TxSeq[3]=%d\n",
			 tr_entry->TxSeq[0], tr_entry->TxSeq[1], tr_entry->TxSeq[2], tr_entry->TxSeq[3]));
	MTWF_LOG(DBG_CAT_MLME, CATMLME_WTBL, DBG_LVL_OFF, ("\tCurrTxRate=%x\n", tr_entry->CurrTxRate));
	MTWF_LOG(DBG_CAT_MLME, CATMLME_WTBL, DBG_LVL_OFF, ("\tQueuing Info\n"));
	MTWF_LOG(DBG_CAT_MLME, CATMLME_WTBL, DBG_LVL_OFF, ("\t\tenq_cap=%d, deq_cap=%d\n", tr_entry->enq_cap, tr_entry->deq_cap));
	MTWF_LOG(DBG_CAT_MLME, CATMLME_WTBL, DBG_LVL_OFF, ("\t\tQueuedPkt: TxQ[0]=%d, TxQ[1]=%d, TxQ[2]=%d, TxQ[3]=%d, PSQ=%d\n",
	...
	MTWF_LOG(DBG_CAT_MLME, CATMLME_WTBL, DBG_LVL_OFF, ("\t\tdeq_cnt=%d, deq_bytes=%d\n", tr_entry->deq_cnt, tr_entry->deq_bytes));
	...
	if (IS_TR_WCID_VALID(pAd, tr_tb_idx)) {
		...
		if (IS_HIF_TYPE(pAd, HIF_MT)) {
			if (tr_entry->wdev)
				...
				MTWF_LOG(DBG_CAT_MLME, CATMLME_WTBL, DBG_LVL_ERROR, ("%s: wdev == NULL\n",
						 __func__));
		}
#endif /* MT_MAC */
...
VOID TRTableInsertMcastEntry(RTMP_ADAPTER *pAd, UINT16 tr_tb_idx, struct wifi_dev *wdev)
{
	...
	*/
	...
	if (pAd->vow_cfg.en_bw_ctrl)
		pAd->bss_group.group_idx[wdev->func_idx] = wdev->func_idx % (pAd->max_bssgroup_num >> 1) +
			((band == DBDC_BAND0) ? 0 : (pAd->max_bssgroup_num>>1));
				else
#endif
		pAd->bss_group.group_idx[wdev->func_idx] = wdev->func_idx % pAd->max_bssgroup_num;

	MTWF_LOG(DBG_CAT_MLME, CATMLME_WTBL, DBG_LVL_ERROR, ("%s:band%u group_idx[%d]=%d\n", __func__,
	...
	IN USHORT cap_info)
{
	...
	if ((pEntry->HTC_ICVErrCnt)
		...
		MTWF_LOG(DBG_CAT_RX, CATMLME_WTBL, DBG_LVL_ERROR, ("@@@ %s(): (wcid=%u), HTC_ICVErrCnt(%u), HTC_AAD_OM_Freeze(%u)\n",
				 __func__, pEntry->wcid, pEntry->HTC_ICVErrCnt, pEntry->HTC_AAD_OM_Force));
		MTWF_LOG(DBG_CAT_RX, CATMLME_WTBL, DBG_LVL_ERROR, (", HTC_AAD_OM_CountDown(%u),  HTC_AAD_OM_Freeze(%u) is in Asso. stage!\n",
	...
	IN BOOLEAN CleanAll)
{
	...
	if (IS_ENTRY_NONE(&pAd->MacTab.Content[i])) {
		...
		do {
			...
			if (ent_type == ENTRY_AP || ent_type == ENTRY_ADHOC) {
				...
				if (VOW_IS_ENABLED(pAd)) {
					...
					if (pAd->vow_cfg.en_bw_ctrl)
						pAd->bss_group.group_idx[pEntry->func_tb_idx] =
							pAd->max_bssgroup_num - pRootApEntry->func_tb_idx - 1;
		...
		if (IS_ENTRY_CLIENT(pEntry)
			...
			MTWF_LOG(DBG_CAT_MLME, CATMLME_WTBL, DBG_LVL_TRACE, ("%s(): INIT UAPSD MR ENTRY", __func__));
...
#endif /* CONFIG_AP_SUPPORT */
		MTWF_LOG(DBG_CAT_MLME, CATMLME_WTBL, DBG_LVL_TRACE, ("%s(): alloc entry #%d, Total= %d\n",
...
static VOID mac_entry_disconn_act(struct _RTMP_ADAPTER *pAd, struct _MAC_TABLE_ENTRY *pEntry)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
		...
		if (wdev_do_disconn_act(pEntry->wdev, pEntry) != TRUE)
			MTWF_LOG(DBG_CAT_MLME, CATMLME_WTBL, DBG_LVL_OFF, ("%s(): AP disconnection fail!\n", __func__));
	}
#endif /* CONFIG_STA_SUPPORT */
}
...
BOOLEAN MacTableDeleteEntry(RTMP_ADAPTER *pAd, USHORT wcid, UCHAR *pAddr)
{
	...
	if (pEntry &&
		...
		pEntry->rx_state.CurrentState = WHC_STA_STATE_IDLE;
			...
			if (P2P_GO_ON(pAd)) {
				...
				MTWF_LOG(DBG_CAT_MLME, CATMLME_WTBL, DBG_LVL_ERROR, ("MacTableDeleteEntry1 - Total= %d. p2pEntry = %d.\n", pAd->MacTab.Size, p2pEntryCnt));
			}

#endif /* P2P_SUPPORT */
			MTWF_LOG(DBG_CAT_MLME, CATMLME_WTBL, DBG_LVL_TRACE, ("MacTableDeleteEntry1 - Total= %d\n", pAd->MacTab.Size));
		} else {
			MTWF_LOG(DBG_CAT_MLME, CATMLME_WTBL, DBG_LVL_OFF, ("\n%s: Impossible Wcid = %d !!!!!\n", __func__, wcid));
#ifdef CONFIG_AP_SUPPORT
#if defined(RT_CFG80211_SUPPORT) || defined(MBO_SUPPORT)
	...
	if (bDeleteEntry) {
		...
		*/
		...
		twt_resource_release_at_link_down(wdev, wcid);
...
VOID MacTableResetWdev(RTMP_ADAPTER *pAd, struct wifi_dev *wdev)
{
	...
	MTWF_LOG(DBG_CAT_MLME, CATMLME_WTBL, DBG_LVL_TRACE, ("MacTableResetWdev\n"));
			...
			IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
				...
				if (pMacEntry->Sst == SST_ASSOC) {
					...
					if (NStatus != NDIS_STATUS_SUCCESS) {
						MTWF_LOG(DBG_CAT_MLME, CATMLME_WTBL, DBG_LVL_TRACE, (" MlmeAllocateMemory fail  ..\n"));
						/*NdisReleaseSpinLock(&pAd->MacTabLock);*/
						return;
					}
...
VOID MacTableReset(RTMP_ADAPTER *pAd)
{
	...
	*/
	MTWF_LOG(DBG_CAT_MLME, CATMLME_WTBL, DBG_LVL_TRACE, ("MacTableReset\n"));
	/*NdisAcquireSpinLock(&pAd->MacTabLock);*/
...
#endif /* RTMP_MAC_PCI */
		MTWF_LOG(DBG_CAT_MLME, CATMLME_WTBL, DBG_LVL_TRACE, ("McastPsQueue.Number %d...\n", pAd->MacTab.McastPsQueue.Number));

		if (pAd->MacTab.McastPsQueue.Number > 0)
			APCleanupPsQueue(pAd, &pAd->MacTab.McastPsQueue);

		MTWF_LOG(DBG_CAT_MLME, CATMLME_WTBL, DBG_LVL_TRACE, ("2McastPsQueue.Number %d...\n", pAd->MacTab.McastPsQueue.Number));
...
INT	SetCommonHtVht(RTMP_ADAPTER *pAd, struct wifi_dev *wdev)
{
	if (wdev)
		...
		MTWF_LOG(DBG_CAT_MLME, CATMLME_WTBL, DBG_LVL_ERROR, ("%s(): Can't update HT/VHT due to wdev is null!\n", __func__));
...
void entrytb_aid_bitmap_init(struct _RTMP_CHIP_CAP *cap, struct _aid_info *aid_info)
{
	...
	if (aid_bitmap == NULL) {
		dump_stack();
		MTWF_LOG(DBG_CAT_ALL, CATMLME_WTBL, DBG_LVL_ERROR,
				 ("%s(): Allocate memory size:%d for aid_bitmap failed!\n",
				  __func__, map_size));
		return;
	}
	...
	if (aid_info->aid_bitmap == NULL) {
		dump_stack();
		MTWF_LOG(DBG_CAT_ALL, CATMLME_WTBL, DBG_LVL_ERROR,
				 ("%s(): Allocate memory size:%d for aid_info->aid_bitmap failed!\n",
				  __func__, map_size));
		return;
	}
...
INT show_aid_info(struct _RTMP_ADAPTER *ad, RTMP_STRING *arg)
{
	...
	if (arg && strlen(arg)) {
		...
		if ((aid == 0) || (aid >= INVALID_AID)) {
			MTWF_LOG(DBG_CAT_CFG, CATMLME_WTBL, 	DBG_LVL_OFF,
				("wrong AID input\n"));
			return FALSE;
		}
	}
...
UINT32 traversal_func_dump_entry_associated_to_bss(struct _MAC_TABLE_ENTRY *entry, void *cookie)
{
	...
	if (check_bss->need_print_field_name) {
		MTWF_LOG(DBG_CAT_AP, CATMLME_WTBL, DBG_LVL_OFF,
			("Dump the entries associated to BssIDX:%d\n", check_bss->bss_idx));
	...
	if (check_bss->bss_idx == entry->func_tb_idx) {
		...
		{
			MTWF_LOG(DBG_CAT_AP, CATMLME_WTBL, DBG_LVL_OFF,
				("%-12s", get_phymode_str(entry->HTPhyMode.field.MODE)));
			MTWF_LOG(DBG_CAT_AP, CATMLME_WTBL, DBG_LVL_OFF,
			...
			MTWF_LOG(DBG_CAT_AP, CATMLME_WTBL, DBG_LVL_OFF,
				("%-12s", tmp_str));
			MTWF_LOG(DBG_CAT_AP, CATMLME_WTBL, DBG_LVL_OFF,
				("%-9d", entry->HTPhyMode.field.ShortGI));
			MTWF_LOG(DBG_CAT_AP, CATMLME_WTBL, DBG_LVL_OFF,
...
UINT32 traversal_func_dump_entry_psm_by_aid(struct _MAC_TABLE_ENTRY *entry, void *cookie)
{
	...
	if (*aid == entry->Aid) {
		...
		MTWF_LOG(DBG_CAT_AP, CATMLME_WTBL, DBG_LVL_OFF,
			("\t%d", (int)entry->wcid));
		MTWF_LOG(DBG_CAT_AP, CATMLME_WTBL, DBG_LVL_OFF,
			("\t%d", (int)entry->func_tb_idx));
		MTWF_LOG(DBG_CAT_AP, CATMLME_WTBL, DBG_LVL_OFF,
			("\t%d", (int)entry->PsMode));
		MTWF_LOG(DBG_CAT_AP, CATMLME_WTBL, DBG_LVL_OFF,
			("\t%-9d", (int)entry->NoDataIdleCount));
		MTWF_LOG(DBG_CAT_AP, CATMLME_WTBL, DBG_LVL_OFF,
