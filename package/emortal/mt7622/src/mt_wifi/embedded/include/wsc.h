...
#define WDEV_WSC_AP(__wdev)                                                    \
	((__wdev->wdev_type == WDEV_TYPE_AP) ||                                \
...
#define WSC_HDR_BTN_CHECK_PERIOD                                               \
	MLME_TASK_EXEC_INTV /* unit: ms, check pin every 100ms */
...
#endif
#define WSC_HDR_BTN_CONT_TIMES                                                 \
	(WSC_HDR_BTN_PRESS_TIME / WSC_HDR_BTN_CHECK_PERIOD)
...
#define WSC_HDR_BTN_MR_IS_HDR_SUPPORT(__pAd) ((__pAd)->WscHdrPshBtnFlag)
...
#define WSC_TIMER_INIT(_pAd, _pData, _time_var, _time_flg, _time_fn)           \
	do {                                                                   \
		RTMPInitTimer((_pAd), (_time_var),                             \
			      GET_TIMER_FUNCTION(_time_fn), (_pData), FALSE);  \
		(_time_flg) = FALSE;                                           \
	} while (0)
...
#define STATUS_WSC_EAP_RSP_WRONG_SMI                                           \
	7 /* Receive EAP-Req with wrong WSC SMI Vendor Id */
#define STATUS_WSC_EAP_RSP_WRONG_VENDOR_TYPE                                   \
...
#define STATUS_WSC_EAP_INVALID_DATA                                            \
	0x0105 /* Receive EAP without integrity (Hmac mismatch) */
#define STATUS_WSC_PASSWORD_MISMATCH                                           \
	0x0106 /* Error PIN Code (R-Hash mismatch) */
#define STATUS_WSC_EAP_REQ_WRONG_SMI                                           \
	0x0107 /* Receive EAP-Req with wrong WPS SMI Vendor Id */
#define STATUS_WSC_EAP_REQ_WRONG_VENDOR_TYPE                                   \
	0x0108 /* Receive EAPReq with wrong WPS Vendor Type */
#define STATUS_WSC_PBC_SESSION_OVERLAP 0x0109 /* AP PBC session overlap */
#define STATUS_WSC_SMPBC_TOO_MANY_REGISTRAR                                    \
	0x010a /* Too many SMPBC Registrars avaliable */
#define STATUS_WSC_EMPTY_IPV4_SUBMASK_LIST                                     \
	0x010b /* Empty available IPv4 Submask list */
...
#define WSC_PROFILE_RETRY_TIME_OUT 15000
...
typedef struct _WSC_DEV_INFO {
	...
	UCHAR SecDevTypList
		[MAX_2ND_DEV_TYPE_LIST_BUFFER]; /* 2nd Device Type List, ref. P2P Spec. v1.1 Table 29*/
	...
	UCHAR Ssid[32];
	UCHAR NewKey[64 +
		     1]; /* not sure sprintf would add '\0' or not, add one byte for \0' */
	INT NewKeyLen;
	UCHAR NewKeyIndex;
...
typedef struct _WSC_CTRL {
	...
	NDIS_802_11_SSID
	WscSsid; /* select a desired ssid to connect for PIN mode */
	...
	WSC_UPNP_NODE_INFO
	WscUPnPNodeInfo; /*Use to save UPnP node related info. */
...
#ifdef CONFIG_AP_SUPPORT
	NDIS_802_11_SSID
	WscDefaultSsid; /* Default WPS SSID after WPS process complete with Enrollee when AP is un-configured Registrar. */
	BOOLEAN bWCNTest;
#endif /* CONFIG_AP_SUPPORT */
...
#define RTMP_WSC_NLMSG_HDR_LEN                                                 \
	30 /*signature(8) + envID(4) + ackID(4) + msgLen(4) + Flag(2) + segLen(2) + devAddr(6) */
typedef struct GNU_PACKED _RTMP_WSC_NLMSG_HDR {
	UCHAR signature
		[8]; /* Signature used to identify that this's a Ralink specific NETLINK message.
...
*/
#define RTMP_WSC_MSG_HDR_LEN                                                   \
	12 /*msgType(2) + msgSubType(2) + ipAddr(4) + len(4) */
...
#define IWEVCUSTOM_MSG_MAX_LEN                                                 \
	255 /*refer to kernel definition. <linux/wireless.h> */
#define IWEVCUSTOM_PAYLOD_MAX_LEN                                              \
	(IWEVCUSTOM_MSG_MAX_LEN - RTMP_WSC_NLMSG_HDR_LEN)
