	...
	CMD_RTPRIV_IOCTL_80211_CHAN *pchan_info)
{
	...
	if (!pAd || !wdev || !pchan_info) {
		CFG80211DBG(DBG_LVL_ERROR, ("[%s](%d): ERROR! invalid parameter. pAd=%p,wdev=%p, pchan_info=%p\n",
				__func__, __LINE__, pAd, wdev, pchan_info));
		return FALSE;
	}
	...
	if (MTK_NL80211_CHAN_WIDTH_20_NOHT == pchan_info->ChanType) {
		...
		if (HT_BW_20 == bw && 1 == pAd->CommonCfg.HT_Disable) {
			CFG80211DBG(DBG_LVL_TRACE, ("[%s](%d): bw 20 is already set.\n",
				__func__, __LINE__));
			goto set_channel;
		}
		...
		if (WLAN_OPER_OK == wlan_operate_set_ht_bw(wdev, HT_BW_20, EXTCHA_NONE)) {
			...
			CFG80211DBG(DBG_LVL_ERROR, ("[%s](%d):ERROR! wlan_operate_set_ht_bw 20 FAIL\n",
				__func__, __LINE__));
			ret = FALSE;
		}
		...
		if (HT_BW_40 == bw && ext_cha == wlan_operate_get_ext_cha(wdev)
			...
			CFG80211DBG(DBG_LVL_TRACE, ("[%s](%d): bw 40 ext_bw(%s) is already set.\n",
		...
		if (WLAN_OPER_OK == wlan_operate_set_ht_bw(wdev, HT_BW_40, ext_cha)) {
			...
			CFG80211DBG(DBG_LVL_ERROR, ("[%s](%d):ERROR! wlan_operate_set_ht_bw 40 FAIL\n",
				__func__, __LINE__));
			ret = FALSE;
		}
		...
		if (VHT_BW_80 == bw && 0 == pAd->CommonCfg.HT_Disable) {
			CFG80211DBG(DBG_LVL_TRACE, ("[%s](%d): bw 80 is already set.\n",
				__func__, __LINE__));
			goto set_channel;
		}
		CFG8021DRV_AP_PHYINIT(pAd, wdev, pchan_info->ChanId);
		...
		if (WLAN_OPER_OK == wlan_operate_set_vht_bw(wdev, VHT_BW_80)) {
			...
			CFG80211DBG(DBG_LVL_ERROR, ("[%s](%d):ERROR! wlan_operate_set_ht_bw 80 FAIL\n",
				__func__, __LINE__));
			ret = FALSE;
		}
		...
		if (VHT_BW_8080 == bw && 0 == pAd->CommonCfg.HT_Disable) {
			CFG80211DBG(DBG_LVL_TRACE, ("[%s](%d): bw 8080 is already set.\n",
				__func__, __LINE__));
			goto set_channel;
		}
		CFG8021DRV_AP_PHYINIT(pAd, wdev, pchan_info->ChanId);
		...
		if (WLAN_OPER_OK == wlan_operate_set_vht_bw(wdev, VHT_BW_8080)) {
			...
			CFG80211DBG(DBG_LVL_ERROR, ("[%s](%d):ERROR! wlan_operate_set_ht_bw 8080 FAIL\n",
				__func__, __LINE__));
			ret = FALSE;
		}
		...
		if (VHT_BW_160 == bw && 0 == pAd->CommonCfg.HT_Disable) {
			CFG80211DBG(DBG_LVL_TRACE, ("[%s](%d): bw 160 is already set.\n",
				__func__, __LINE__));
			goto set_channel;
		}
		CFG8021DRV_AP_PHYINIT(pAd, wdev, pchan_info->ChanId);
		...
		if (WLAN_OPER_OK == wlan_operate_set_vht_bw(wdev, VHT_BW_160)) {
			...
			CFG80211DBG(DBG_LVL_ERROR, ("[%s](%d):ERROR! wlan_operate_set_ht_bw 160 FAIL\n",
				__func__, __LINE__));
			ret = FALSE;
		}
	} else {
...
set_channel:
	...
	CFG80211DBG(DBG_LVL_TRACE, ("[%s](%d):ret:%d (FALSE:%d), wdev->channel:%d,pchan_info->ChanId: %d\n",
	...
	if (wdev->channel != pchan_info->ChanId) {
		if (FALSE == rtmp_set_channel(pAd, wdev, pchan_info->ChanId)) {
			CFG80211DBG(DBG_LVL_ERROR, ("[%s](%d):ERROR! channel(%d) set fail\n",
					__func__, __LINE__, pchan_info->ChanId));
			return FALSE;
		}
	} else {
		CFG80211DBG(DBG_LVL_TRACE, ("[%s](%d): channel(%d) is already set!\n",
				__func__, __LINE__, pchan_info->ChanId));
	}
	return TRUE;
...
BOOLEAN CFG80211DRV_AP_OpsSetChannel(RTMP_ADAPTER *pAd, VOID *pData)
{
	...
	if (!pData || !pAd) {
		CFG80211DBG(DBG_LVL_ERROR, ("[%s](%d): ERROR! invalid parametr: pAd = %p, pData=%p\n",
			__func__, __LINE__, pAd, pData));
		return FALSE;
	}
	...
	if (TRUE != CFG80211DRV_AP_SetChanBw_byWdev(pAd, pwdev, pchan_info)) {
		CFG80211DBG(DBG_LVL_ERROR, ("[%s](%d): ERROR! CFG80211DRV_AP_SetChanBw_byWdev FAIL!\n",
			__func__, __LINE__));
		return FALSE;
	}
	...
	BOOLEAN bReptCli)
{
	...
	if (NULL == pEntry || NULL == pEntry->wdev || NULL == pApStaInfo || NULL == tr_ctl) {
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			("[%s](%d):ERROR! invalid pareamter! pEntry = %p, wdev=%p, pApStaInfo=%p, tr_ctl = %p!\n",
			__func__, __LINE__, pEntry, pEntry->wdev, pApStaInfo, tr_ctl));
		return FALSE;
	}
...
/*RX MCS*/
		...
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			...
			__func__, __LINE__, prate_info->legacy,
		...
		if (IS_ENTRY_PEER_AP(pEntry) && pstacfg && pstacfg->ApcliInfStat.Valid) {
			...
			MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
					("\n[%s](%d):pApStaInfo->signal_avg:%d\n",
					__func__, __LINE__, pApStaInfo->signal_avg));
			...
			MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
					("\n[%s](%d):Rssi_temp:%d, rx_stream:%d, pApStaInfo->Signal:%d\n",
					__func__, __LINE__, Rssi_temp, rssi_idx, pApStaInfo->Signal));
			/*InactiveTime*/
			...
			MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
				("\n[%s](%d):beacon_loss_count  = %u, rx_beacon = %lu;\n",
				__func__, __LINE__, pstacfg->beacon_loss_count, pstacfg->rx_beacon));
...
/*apcli or mbss infos fill*/
	...
	MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			("\n[%s](%d):BSS_PARAM_FLAGS_CTS_PROT(%d)\n",
			__func__, __LINE__, wlan_config_get_ht_protect_en(wdev)));
	/*short preamble*/
	...
	MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			("\n[%s](%d):BSS_PARAM_FLAGS_SHORT_PREAMBLE(%d)\n",
			__func__, __LINE__, CAP_IS_SHORT_PREAMBLE_ON(bss_capability)));
	...
	MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			("\n[%s](%d):BSS_PARAM_FLAGS_SHORT_SLOT_TIME(%d)\n",
			__func__, __LINE__, bss_shorttime_en));
	...
	MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			("\n[%s](%d):NL80211_STA_BSS_PARAM_DTIM_PERIOD(%d)\n",
			__func__, __LINE__, pApStaInfo->bss_param.dtim_period));
	...
	MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("\n[%s](%d):NL80211_STA_BSS_PARAM_BEACON_INTERVAL:%d\n",
					__func__, __LINE__, pApStaInfo->bss_param.beacon_interval));
	return TRUE;
}
	...
	VOID						*pData)
{
	...
	if (FALSE == CFG80211DRV_FILL_STAInfo(pAd, pEntry, pApStaInfo, FALSE)) {
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				("[%s](%d):ERROR! CFG80211DRV_FILL_STAInfo Fail.\n",
				__func__, __LINE__));
		return FALSE;
	}
	return TRUE;
	...
	UINT enable)
{
	if (NULL == pstacfg || NULL == pAd) {
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				("[%s](%d):ERROR! pstacfg = %p or pAd=%p\n",
				__func__, __LINE__, pstacfg, pAd));
		return FALSE;
	}
	...
	/*off*/
		...
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			("[%s](%d):WARN! invalid parameter pstacfg->BssType=%d;\n",
			__func__, __LINE__, pstacfg->BssType));
	}
	...
	UINT enable)
{
	...
	if (NULL == wdev || NULL == pAd) {
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				("[%s](%d):ERROR! wdev = %p or pAd=%p\n",
				__func__, __LINE__, wdev, pAd));
		return FALSE;
	}
	...
	UINT enable)
{
	...
	if (!pAd || !wdev) {
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				("[%s](%d):ERROR! pAd = %p, wdev=%p\n",
				__func__, __LINE__, pAd, wdev));
		return FALSE;
	}
	...
	/*set apcli interface ps_mode*/
		...
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				("[%s](%d):ERROR! wdev->wdev_type = %d not suuport.\n",
				__func__, __LINE__, wdev->wdev_type));
		return FALSE;
	}
...
#ifdef DBDC_MODE
	if (pAd->CommonCfg.dbdc_mode) {
		BandInfo.RFICType = (RFIC_24GHZ | RFIC_5GHZ);
	}
#endif
