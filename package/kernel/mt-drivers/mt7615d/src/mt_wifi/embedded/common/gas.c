...
static VOID SendGASIndication(IN PRTMP_ADAPTER pAd, GAS_EVENT_DATA *Event);
...
#ifdef CONFIG_AP_SUPPORT
void wext_send_locationIE_to_daemon_event(PNET_DEV net_dev,
					  const char *location_buf,
					  UINT16 location_buf_len,
					  UINT16 info_id)
{
	...
	RtmpOSWrielessEventSend(net_dev, RT_WLAN_EVENT_CUSTOM,
				OID_802_11_HS_LOCATION_DRV_INFORM_IE, NULL,
				(PUCHAR)buf, buflen);
	os_free_mem(buf);
}
...
{
	wext_send_locationIE_to_daemon_event(net_dev, location_buf,
					     location_buf_len, info_id);
}
			      ...
			      const char *anqp_req, UINT16 anqp_req_len)
{
	...
	RtmpOSWrielessEventSend(net_dev, RT_WLAN_EVENT_CUSTOM,
				OID_802_11_HS_ANQP_REQ, NULL, (PUCHAR)buf,
				buflen);
	os_free_mem(buf);
}
...
void SendAnqpReqEvent(PNET_DEV net_dev, const char *peer_mac_addr,
		      const char *anqp_req, UINT16 anqp_req_len)
{
	wext_send_anqp_req_event(net_dev, peer_mac_addr, anqp_req,
				 anqp_req_len);
}
...
VOID SendGASRsp(IN PRTMP_ADAPTER pAd, GAS_EVENT_DATA *Event)
{
	...
	UCHAR WildcardBssid[MAC_ADDR_LEN] = {
	...
	PHOTSPOT_CTRL pHSCtrl =
		&pAd->ApCfg.MBSSID[Event->ControlIndex].HotSpotCtrl;
#endif
	...
	if (Event->u.GAS_RSP_DATA.AdvertisementProID ==
	    ACCESS_NETWORK_QUERY_PROTOCOL) {
	...
	if (!Buf) {
		...
		if (GASPeerEntry->GASRspBufferingTimerRunning) {
			RTMPCancelTimer(&GASPeerEntry->GASRspBufferingTimer,
					&Cancelled);
			GASPeerEntry->GASRspBufferingTimerRunning = FALSE;
		}

		RTMPReleaseTimer(&GASPeerEntry->GASRspBufferingTimer,
				 &Cancelled);
	...
	if ((pEntry)
		...
		ActHeaderInit(
			pAd, &GASFrame->Hdr, Event->PeerMACAddr,
		...
		ActHeaderInit(pAd, &GASFrame->Hdr, Event->PeerMACAddr,
			      pAd->ApCfg.MBSSID[Event->ControlIndex].wdev.bssid,
			      WildcardBssid);
	}
	...
	if (Event->EventType == GAS_RSP) {
		GASFrame->u.GAS_INIT_RSP.DialogToken =
			Event->u.GAS_RSP_DATA.DialogToken;
		FrameLen += 1;
		GASFrame->u.GAS_INIT_RSP.StatusCode =
			cpu2le16(Event->u.GAS_RSP_DATA.StatusCode);
		FrameLen += 2;
		GASFrame->u.GAS_INIT_RSP.GASComebackDelay =
			Event->u.GAS_RSP_DATA.GASComebackDelay;
		FrameLen += 2;
		...
		*Pos++ =
			Event->u.GAS_RSP_DATA
				.AdvertisementProID; /* Advertisement Protocol ID field */
		...
		if (Event->u.GAS_RSP_DATA.QueryRspLen > 0)
			NdisMoveMemory(Pos, Event->u.GAS_RSP_DATA.QueryRsp,
				       Event->u.GAS_RSP_DATA.QueryRspLen);
		...
		FrameLen += 1;
		GASFrame->u.GAS_INIT_RSP.StatusCode =
			cpu2le16(Event->u.GAS_RSP_MORE_DATA.StatusCode);
		FrameLen += 2;
		GASFrame->u.GAS_INIT_RSP.GASComebackDelay =
		...
		*Pos++ =
		 ...
		 */
		if (!GASPeerEntry->GASRspBufferingTimerRunning) {
			RTMPSetTimer(&GASPeerEntry->GASRspBufferingTimer,
				     dotGASResponseBufferingTime +
...
VOID DPP_ReceiveGASInitRsp(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem)
{
	...
	if ((GASFrame->u.GAS_INIT_RSP.Variable[GAS_WFA_DPP_Length_Index] >
	     GAS_WFA_DPP_Min_Length) &&
	    NdisEqualMemory(&GASFrame->u.GAS_INIT_RSP.Variable[GAS_OUI_Index],
			    DPP_OUI, OUI_LEN) &&
	    GASFrame->u.GAS_INIT_RSP.Variable[GAS_WFA_DPP_Subtype_Index] ==
		    WFA_DPP_SUBTYPE) {
		if (pAd->bDppEnable) {
			wext_send_dpp_action_frame(pAd, Elem->wdev,
						   &Elem->Msg[10],
						   Elem->Channel, Elem->Msg,
						   Elem->MsgLen, 1);
			MTWF_LOG(
...
VOID ReceiveGASInitReq(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem)
{
	...
	MTWF_LOG(DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_TRACE,
		 ("%s\n", __func__));
#ifdef DPP_SUPPORT
	if ((GASFrame->u.GAS_INIT_REQ.Variable[GAS_WFA_DPP_Length_Index] >
	     GAS_WFA_DPP_Min_Length) &&
	    NdisEqualMemory(&GASFrame->u.GAS_INIT_REQ.Variable[GAS_OUI_Index],
			    DPP_OUI, OUI_LEN) &&
	    GASFrame->u.GAS_INIT_REQ.Variable[GAS_WFA_DPP_Subtype_Index] ==
		    WFA_DPP_SUBTYPE) {
		if (pAd->bDppEnable) {
			wext_send_dpp_action_frame(pAd, Elem->wdev,
						   &Elem->Msg[10],
	...
	for (APIndex = 0; APIndex < MAX_MBSSID_NUM(pAd); APIndex++) {
		...
		*/
		if (MAC_ADDR_EQUAL(GASFrame->Hdr.Addr1,
		...
		/* if (GASPeerEntry->QueryNum++ >= 15) */
		{
			...
			MTWF_LOG(
				...
				("%s Q %d, old 0x%x, 0x%x, %02x:%02x:%02x:%02x:%02x:%02x remove peer\n",
				 __func__, GASPeerEntry->QueryNum,
				 GASPeerEntry->DialogToken,
				 GASFrame->u.GAS_INIT_REQ.DialogToken,
				 GASPeerEntry->PeerMACAddr[0],
				 GASPeerEntry->PeerMACAddr[1],
				 GASPeerEntry->PeerMACAddr[2],
				 GASPeerEntry->PeerMACAddr[3],
				 GASPeerEntry->PeerMACAddr[4],
				 GASPeerEntry->PeerMACAddr[5]));
			RTMP_SEM_LOCK(&pGASCtrl->GASPeerListLock);
			DlListForEachSafe(GASPeerEntry, GASPeerEntryTmp,
					  &pGASCtrl->GASPeerList,
					  GAS_PEER_ENTRY, List)
			{
				if (MAC_ADDR_EQUAL(GASPeerEntry->PeerMACAddr,
						   GASFrame->Hdr.Addr2)) {
					...
					if (GASPeerEntry->PostReplyTimerRunning) {
						RTMPCancelTimer(
							...
							&Cancelled);
						GASPeerEntry
							->PostReplyTimerRunning =
							FALSE;
					}

					RTMPReleaseTimer(
						&GASPeerEntry->PostReplyTimer,
						&Cancelled);

					if (GASPeerEntry
						    ->GASRspBufferingTimerRunning) {
						RTMPCancelTimer(
							&GASPeerEntry
								 ->GASRspBufferingTimer,
							&Cancelled);
						GASPeerEntry
							->GASRspBufferingTimerRunning =
							FALSE;
	...
	GASPeerEntry->QueryNum++;
	NdisMoveMemory(GASPeerEntry->PeerMACAddr, GASFrame->Hdr.Addr2,
		       MAC_ADDR_LEN);
	...
	RTMPInitTimer(pAd, &GASPeerEntry->PostReplyTimer,
		      GET_TIMER_FUNCTION(PostReplyTimeout), GASPeerEntry,
		      FALSE);
	GASPeerEntry->AllocResource++;
	RTMPInitTimer(pAd, &GASPeerEntry->GASRspBufferingTimer,
		      GET_TIMER_FUNCTION(GASRspBufferingTimeout), GASPeerEntry,
		      FALSE);
	GASPeerEntry->AllocResource++;
	DlListInit(&GASPeerEntry->GASQueryRspFragList);
	RTMP_SEM_LOCK(&pGASCtrl->GASPeerListLock);
	...
	Len += 2;
	Event->u.PEER_GAS_REQ_DATA.DialogToken =
		GASFrame->u.GAS_INIT_REQ.DialogToken;
	Len += 1;
	...
	Event->u.PEER_GAS_REQ_DATA.QueryReqLen =
		le2cpu16(Event->u.PEER_GAS_REQ_DATA.QueryReqLen);
	Len += 2;
	Pos += 2;
	if (Event->u.PEER_GAS_REQ_DATA.AdvertisementProID ==
		    ACCESS_NETWORK_QUERY_PROTOCOL &&
	    !pGASCtrl->ExternalANQPServerTest) {
		/* Send anqp request indication to daemon */
		SendAnqpReqEvent(NetDev, Event->PeerMACAddr,
				 &GASFrame->Category,
				 Event->u.PEER_GAS_REQ_DATA.QueryReqLen + 9);
	}
	NdisMoveMemory(Event->u.PEER_GAS_REQ_DATA.QueryReq, Pos,
		       Event->u.PEER_GAS_REQ_DATA.QueryReqLen);
...
static VOID SendGASCBRsp(IN PRTMP_ADAPTER pAd,
			 /* IN MLME_QUEUE_ELEM  *Elem) */
			 GAS_EVENT_DATA *Event)
{
	/* GAS_EVENT_DATA *Event = (GAS_EVENT_DATA *)Elem->Msg; */
	...
	os_alloc_mem(NULL, (UCHAR **)&Buf,
		     sizeof(*GASFrame) + 6 + pGASCtrl->MMPDUSize);
		      ...
		      List)
	{
		if (MAC_ADDR_EQUAL(GASPeerEntry->PeerMACAddr,
				   Event->PeerMACAddr)) {
			if (GASPeerEntry->AdvertisementProID ==
			    ACCESS_NETWORK_QUERY_PROTOCOL) {
				/* Advertisement protocol element + Query response length field */
				VarLen += 6;
			}

			DlListForEach(GASQueryRspFrag,
				      &GASPeerEntry->GASQueryRspFragList,
				      GAS_QUERY_RSP_FRAGMENT, List)
			{
	...
	if (Event->EventType == GAS_CB_REQ) {
		GASFrame->u.GAS_CB_RSP.DialogToken =
			Event->u.GAS_CB_REQ_DATA.DialogToken;
		FrameLen += 1;
		GASFrame->u.GAS_CB_RSP.StatusCode =
			cpu2le16(Event->u.GAS_CB_REQ_DATA.StatusCode);
		FrameLen += 2;

		if (bGASQueryRspFragFound && GASQueryRspFrag)
			GASFrame->u.GAS_CB_RSP.GASRspFragID =
				(GASQueryRspFrag->GASRspFragID & 0x7F);
		else
		...
		*Pos++ =
		...
		GASFrame->u.GAS_CB_RSP.DialogToken =
			Event->u.GAS_CB_REQ_MORE_DATA.DialogToken;
		FrameLen += 1;
		GASFrame->u.GAS_CB_RSP.StatusCode =
			cpu2le16(Event->u.GAS_CB_REQ_MORE_DATA.StatusCode);
		FrameLen += 2;
		if (GASQueryRspFrag)
			GASFrame->u.GAS_CB_RSP.GASRspFragID =
				(0x80 | (GASQueryRspFrag->GASRspFragID & 0x7F));
		MTWF_LOG(DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_OFF,
			 ("GASRspFragID = %d\n",
			  GASFrame->u.GAS_CB_RSP.GASRspFragID));
		FrameLen += 1;
...
VOID ReceiveGASCBReq(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (!IsGASCBReqMore) {
		if (IsFound) {
			...
			Len += 1;
			Event->u.GAS_CB_REQ_DATA.AdvertisementProID =
				GASPeerEntry->AdvertisementProID;
			Len += 1;

			if (pGASCtrl->ExternalANQPServerTest == 2)
				Event->u.GAS_CB_REQ_DATA.StatusCode =
					SERVER_UNREACHABLE;
			else
			...
			os_alloc_mem(NULL, (UCHAR **)&GASPeerEntry,
				     sizeof(*GASPeerEntry));
			...
			NdisMoveMemory(GASPeerEntry->PeerMACAddr,
				       GASFrame->Hdr.Addr2, MAC_ADDR_LEN);
			GASPeerEntry->DialogToken =
				GASFrame->u.GAS_CB_REQ.DialogToken;
			GASPeerEntry->AdvertisementProID =
				ACCESS_NETWORK_QUERY_PROTOCOL; /* FIXME */
			GASPeerEntry->Priv = pAd;
			DlListInit(&GASPeerEntry->GASQueryRspFragList);
			RTMP_SEM_LOCK(&pGASCtrl->GASPeerListLock);
			DlListAddTail(&pGASCtrl->GASPeerList,
				      &GASPeerEntry->List);
			RTMP_SEM_UNLOCK(&pGASCtrl->GASPeerListLock);
			Event->EventType = GAS_CB_REQ;
			Len += 2;
		...
		Event->u.GAS_CB_REQ_MORE_DATA.DialogToken =
			GASFrame->u.GAS_CB_REQ.DialogToken;
		Len += 1;
		Event->u.GAS_CB_REQ_MORE_DATA.AdvertisementProID =
			GASPeerEntry->AdvertisementProID;
		Len += 1;
	...
	IN enum DOT11U_ADVERTISMENT_PROTOCOL_ID AdvertisementProtoID)
{
	...
	for (i = 0; i < sizeof(dot11GASAdvertisementID) /
				sizeof(enum DOT11U_ADVERTISMENT_PROTOCOL_ID);
	     i++) {
...
void PostReplyTimeout(IN PVOID SystemSpecific1, IN PVOID FunctionContext,
...
VOID GASRspBufferingTimeout(IN PVOID SystemSpecific1, IN PVOID FunctionContext,
			    IN PVOID SystemSpecific2, IN PVOID SystemSpecific3)
{
	GAS_PEER_ENTRY *GASPeerEntry = (GAS_PEER_ENTRY *)FunctionContext;
	PRTMP_ADAPTER pAd;
...
static VOID SendGASIndication(IN PRTMP_ADAPTER pAd,
			      /* IN MLME_QUEUE_ELEM  *Elem) */
			      GAS_EVENT_DATA *Event)
{
	PGAS_PEER_ENTRY GASPeerEntry;
	 ...
	 */
	if (IsAdvertisementProIDValid(
		    pAd, Event->u.PEER_GAS_REQ_DATA.AdvertisementProID) &&
	    !pGASCtrl->ExternalANQPServerTest) {
		RTMP_SEM_LOCK(&pGASCtrl->GASPeerListLock);
			      ...
			      GAS_PEER_ENTRY, List)
		{
			if (MAC_ADDR_EQUAL(GASPeerEntry->PeerMACAddr,
				...
				if (!GASPeerEntry->PostReplyTimerRunning) {
					RTMPSetTimer(
						&GASPeerEntry->PostReplyTimer,
						dot11GASResponseTimeout);
					GASPeerEntry->PostReplyTimerRunning =
						TRUE;
				}
	...
	} else if (IsAdvertisementProIDValid(
			   pAd,
			   Event->u.PEER_GAS_REQ_DATA.AdvertisementProID) &&
		   pGASCtrl->ExternalANQPServerTest ==
		...
		Len += 1;
		NdisMoveMemory(GASRspEvent->PeerMACAddr, Event->PeerMACAddr,
			       MAC_ADDR_LEN);
		Len += MAC_ADDR_LEN;
			      ...
			      GAS_PEER_ENTRY, List)
		{
			if (MAC_ADDR_EQUAL(GASPeerEntry->PeerMACAddr,
					   Event->PeerMACAddr)) {
				GASRspEvent->u.GAS_RSP_DATA.DialogToken =
					GASPeerEntry->DialogToken;
				Len += 1;
				break;
			}
		...
		Len += 2;
		GASRspEvent->u.GAS_RSP_DATA.AdvertisementProID =
			GASPeerEntry->AdvertisementProID;
		Len += 1;
			      ...
			      GAS_PEER_ENTRY, List)
		{
			if (MAC_ADDR_EQUAL(GASPeerEntry->PeerMACAddr,
					   Event->PeerMACAddr)) {
				GASRspEvent->u.GAS_RSP_MORE_DATA.DialogToken =
					GASPeerEntry->DialogToken;
				Len += 1;
				break;
			}
		...
		Len += 2;
		GASRspEvent->u.GAS_RSP_MORE_DATA.AdvertisementProID =
			GASPeerEntry->AdvertisementProID;
		Len += 1;
		/* GASSetPeerCurrentState(pAd, Elem, WAIT_GAS_RSP); */
		GASSetPeerCurrentState(pAd, Event, WAIT_GAS_RSP);
		...
		RTMP_SEM_UNLOCK(&pGASCtrl->GASPeerListLock);
		GASRspEvent->u.GAS_RSP_DATA.StatusCode =
...
VOID GASCtrlExit(IN PRTMP_ADAPTER pAd)
{
	...
	for (APIndex = 0; APIndex < MAX_MBSSID_NUM(pAd); APIndex++) {
				  ...
				  &pGASCtrl->GASPeerList, GAS_PEER_ENTRY, List)
		{
			...
			if (GASPeerEntry->GASRspBufferingTimerRunning) {
				RTMPCancelTimer(
					&GASPeerEntry->GASRspBufferingTimer,
					&Cancelled);
				GASPeerEntry->GASRspBufferingTimerRunning =
					FALSE;
			}

			RTMPReleaseTimer(&GASPeerEntry->PostReplyTimer,
					 &Cancelled);
			RTMPReleaseTimer(&GASPeerEntry->GASRspBufferingTimer,
					 &Cancelled);
			os_free_mem(GASPeerEntry);
			 ...
			 OUT STATE_MACHINE_FUNC Trans[])
{
	MTWF_LOG(DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_TRACE,
		 ("%s\n", __func__));
	GASCtrlInit(pAd);
	StateMachineInit(S, (STATE_MACHINE_FUNC *)Trans, MAX_GAS_STATE,
			 MAX_GAS_MSG, (STATE_MACHINE_FUNC)Drop, GAS_UNKNOWN,
			 GAS_MACHINE_BASE);
#ifdef CONFIG_AP_SUPPORT
	StateMachineSetAction(S, WAIT_PEER_GAS_REQ, PEER_GAS_REQ,
			      (STATE_MACHINE_FUNC)SendGASIndication);
	StateMachineSetAction(S, WAIT_GAS_RSP, GAS_RSP,
			      (STATE_MACHINE_FUNC)SendGASRsp);
	StateMachineSetAction(S, WAIT_GAS_RSP, GAS_RSP_MORE,
			      (STATE_MACHINE_FUNC)SendGASRsp);
	StateMachineSetAction(S, WAIT_GAS_RSP, GAS_CB_REQ,
			      (STATE_MACHINE_FUNC)SendGASCBRsp);
	StateMachineSetAction(S, WAIT_GAS_CB_REQ, GAS_CB_REQ,
			      (STATE_MACHINE_FUNC)SendGASCBRsp);
	StateMachineSetAction(S, WAIT_GAS_CB_REQ, GAS_CB_REQ_MORE,
			      (STATE_MACHINE_FUNC)SendGASCBRsp);
#endif /* CONFIG_AP_SUPPORT */
}
