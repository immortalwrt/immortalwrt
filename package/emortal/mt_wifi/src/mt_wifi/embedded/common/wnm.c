						  ...
						  IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (pWNMCtrl->WNMBTMEnable == 0) {
		MTWF_LOG(DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_ERROR,
				(" %s BTM Not Supported Drop!! WNMBTMEnable %d\n", __func__, pWNMCtrl->WNMBTMEnable));
		return;
	}
		...
		case IE_VENDOR_SPECIFIC:
			if (NdisEqualMemory(wfa_oui, &WNMFrame->u.WNM_NOTIFY_REQ.Variable[pos], 3)) {
				...
				if (pWdev && IS_MBO_ENABLE(pWdev)) {
					...
					default:
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
							("\033[1;32m %s, %u vendor specific,unknown OUI typeJ,pls check \033[0m\n"
							, __func__, __LINE__));
					}
				}
#endif /* MBO_SUPPORT */
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (!WNMNotifyPeerEntry) {
		MTWF_LOG(DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_ERROR, ("%s Warning:WNMNotifyPeerEntry is NULL\n", __func__));
		return;
	}
	RTMP_SEM_EVENT_UP(&pWNMCtrl->WNMNotifyPeerListLock);
