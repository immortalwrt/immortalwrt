 ...
 */
#if (KERNEL_VERSION(3, 6, 0) <= LINUX_VERSION_CODE)
static int CFG80211_OpsScan(IN struct wiphy *pWiphy,
...
#endif /* LINUX_VERSION_CODE: 3.6.0 */
{
	...
	if (RTMP_DRIVER_80211_SCAN(pAd, pNdev->ieee80211_ptr->iftype) !=
	    NDIS_STATUS_SUCCESS) {
		CFG80211DBG(DBG_LVL_ERROR,
			    ("\n\n\n\n\n80211> BUSY - SCANING\n\n\n\n\n"));
		CFG80211OS_ScanEnd(pCfg80211_CB, TRUE);
	...
	if (Flag && (strcmp(pNdev->name, "p2p0") == 0)) {
		CFG80211DBG(DBG_LVL_ERROR,
			    ("MCC Protect DHCP - Aborting Scan\n"));
		return 0;
	}
	...
	if (pRequest->n_channels > 0) {
		...
		os_alloc_mem(NULL, (UCHAR **)&pChanList,
			     sizeof(UINT32 *) * pRequest->n_channels);
		...
		for (idx = 0; idx < pRequest->n_channels; idx++) {
			pChanList[idx] = ieee80211_frequency_to_channel(
				pRequest->channels[idx]->center_freq);
			CFG80211DBG(DBG_LVL_INFO, ("%d,", pChanList[idx]));
		}

		CFG80211DBG(DBG_LVL_INFO, ("\n"));
		RTMP_DRIVER_80211_SCAN_CHANNEL_LIST_SET(pAd, pChanList,
							pRequest->n_channels);
...
static int CFG80211_OpsPwrMgmt(IN struct wiphy *pWiphy,
			       IN struct net_device *pNdev, IN bool enabled,
			       IN INT32 timeout)
{
	VOID *pAd;
 ...
 * ========================================================================
 */
static int CFG80211_OpsStaGet(IN struct wiphy *pWiphy,
			      IN struct net_device *pNdev,
...
static int CFG80211_OpsStaDump(IN struct wiphy *pWiphy,
			       IN struct net_device *pNdev, IN int Idx,
			       IN UINT8 *pMac, IN struct station_info *pSinfo)
{
	VOID *pAd;
 ...
 * ========================================================================
 */
static int CFG80211_OpsWiphyParamsSet(IN struct wiphy *pWiphy,
				      IN UINT32 Changed)
{
	VOID *pAd;
	...
	if (Changed & WIPHY_PARAM_RTS_THRESHOLD) {
		...
		RTMP_DRIVER_80211_FRAG_THRESHOLD_ADD(pAd,
...
static int CFG80211_OpsKeyDefaultSet(IN struct wiphy *pWiphy,
				     IN struct net_device *pNdev,
				     IN UINT8 KeyIdx, IN bool Unicast,
				     IN bool Multicast)
#else
 ...
 * ========================================================================
 */
static int CFG80211_OpsConnect(IN struct wiphy *pWiphy,
			       IN struct net_device *pNdev,
			       IN struct cfg80211_connect_params *pSme)
{
	...
	RTMP_DRIVER_80211_STA_ASSSOC_IE_SET(pAd, &AssocIe,
					    pNdev->ieee80211_ptr->iftype);

	if ((pSme->ie_len > 6) /* EID(1) + LEN(1) + OUI(4) */ &&
	    (pSme->ie[0] == WLAN_EID_VENDOR_SPECIFIC && pSme->ie[1] >= 4 &&
	     pSme->ie[2] == 0x00 && pSme->ie[3] == 0x50 &&
	     pSme->ie[4] == 0xf2 && pSme->ie[5] == 0x04))
 ...
 * ========================================================================
 */
static int CFG80211_OpsPmksaSet(IN struct wiphy *pWiphy,
				IN struct net_device *pNdev,
 ...
 * ========================================================================
 */
static int CFG80211_OpsPmksaDel(IN struct wiphy *pWiphy,
				IN struct net_device *pNdev,
				IN struct cfg80211_pmksa *pPmksa)
{
	return 0;
 ...
 * ========================================================================
 */
static int CFG80211_OpsPmksaFlush(IN struct wiphy *pWiphy,
				  IN struct net_device *pNdev)
{
	return 0;
...
#endif /* LINUX_VERSION_CODE < 3.6.0 */
{
	...
	CFG80211DBG(DBG_LVL_TRACE,
		    ("%s: CH = %d, Type = %d, duration = %d, cookie=%d\n",
		     __func__, ChanId, ChannelType, duration, rndCookie));
	/* init */
	*cookie = rndCookie;
	memset(&ChanInfo, 0, sizeof(ChanInfo));
	ChanInfo.ChanId = ChanId;
...
static int CFG80211_OpsTdlsMgmt(IN struct wiphy *pWiphy,
				IN struct net_device *pDev, IN u8 *peer,
				IN u8 action_code, IN u8 dialog_token,
				IN u16 status_code, IN const u8 *extra_ies,
				IN size_t extra_ies_len)
{
	...
	case WLAN_PUB_ACTION_TDLS_DISCOVER_RES:
		cfg_tdls_build_frame(pAd, peer, dialog_token, action_code,
				     status_code, extra_ies, extra_ies_len,
				     FALSE, 0, 0);
		break;

	case TDLS_ACTION_CODE_WFD_TUNNELED_PROBE_REQ:
		cfg_tdls_TunneledProbeRequest(pAd, peer, extra_ies,
					      extra_ies_len);
		break;

	case TDLS_ACTION_CODE_WFD_TUNNELED_PROBE_RSP:
		cfg_tdls_TunneledProbeResponse(pAd, peer, extra_ies,
					       extra_ies_len);
		break;
				...
				IN enum nl80211_tdls_operation oper)
{
	...
	case NL80211_TDLS_ENABLE_LINK:
		RTMP_DRIVER_80211_STA_TDLS_INSERT_DELETE_PENTRY(
			pAd, peer, tdls_insert_entry);
		break;

	case NL80211_TDLS_DISABLE_LINK:
		RTMP_DRIVER_80211_STA_TDLS_INSERT_DELETE_PENTRY(
			pAd, peer, tdls_delete_entry);
		break;
...
static int CFG80211_OpsMgmtTx(IN struct wiphy *pWiphy,
			      IN struct wireless_dev *wdev,
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 14, 0))
			      IN struct cfg80211_mgmt_tx_params *params,
#else
			      IN struct ieee80211_channel *pChan,
			      IN bool Offchan, IN unsigned int Wait,
			      IN const u8 *pBuf, IN size_t Len, IN bool no_cck,
...
static int CFG80211_OpsMgmtTx(IN struct wiphy *pWiphy,
			      ...
			      IN bool Offchan,
			      IN enum nl80211_channel_type ChannelType,
			      IN bool ChannelTypeValid, IN unsigned int Wait,
			      IN const u8 *pBuf, IN size_t Len,
...
static int CFG80211_OpsDelBeacon(struct wiphy *pWiphy,
				 struct net_device *netdev)
{
	VOID *pAd;
			       ...
			       struct cfg80211_ap_settings *settings)
{
	...
	if (settings->beacon.head_len > 0) {
		...
		NdisCopyMemory(beacon_head_buf, settings->beacon.head,
	...
	if (settings->ssid && (settings->ssid_len <= 32))
		NdisCopyMemory(&bcn.ssid[0], settings->ssid,
			       settings->ssid_len);
	...
	if (settings->chandef.chan) {
		...
		case NL80211_CHAN_WIDTH_40:
			if (settings->chandef.center_freq1 >
			    settings->chandef.chan->center_freq)
				ChanInfo.ChanType =
					RT_CMD_80211_CHANTYPE_HT40PLUS;
			else
				ChanInfo.ChanType =
...
VOID CFG80211_UpdateAssocRespExtraIe(VOID *pAdOrg, UINT32 apidx,
				     UCHAR *assocresp_ies,
				     UINT32 assocresp_ies_len)
{
	PRTMP_ADAPTER pAd = (PRTMP_ADAPTER)pAdOrg;
	PUCHAR pAssocRespBuf =
		(PUCHAR)pAd->ApCfg.MBSSID[apidx].AssocRespExtraIe;

	CFG80211DBG(DBG_LVL_TRACE,
		    ("%s: IE len = %d\n", __FUNCTION__, assocresp_ies_len));
	if (assocresp_ies_len >
	    sizeof(pAd->ApCfg.MBSSID[apidx].AssocRespExtraIe)) {
		CFG80211DBG(DBG_LVL_TRACE,
			    ("%s: AssocResp buf size not enough\n",
			     __FUNCTION__));
		return;
	}
				    ...
				    struct cfg80211_beacon_data *info)
{
	...
	ssid_ie = cfg80211_find_ie(WLAN_EID_SSID, bcn.beacon_head + 36,
				   bcn.beacon_head_len - 36);
	bcn.ssid_len = *(ssid_ie + 1);
	/* Update assoc resp extra ie */
	if (info->assocresp_ies_len && info->assocresp_ies) {
		CFG80211_UpdateAssocRespExtraIe(pAd, bcn.apidx,
...
#endif
{
...
#endif
	CMD_RTPRIV_IOCTL_AP_STA_DEL rApStaDel = { .pSta_MAC = NULL,
						  .pWdev = NULL };
	...
	if (dev) {
		...
		CFG80211DBG(
			DBG_LVL_OFF,
			("80211> %s ==> for bssid (%02X:%02X:%02X:%02X:%02X:%02X)\n",
	...
	RTMP_DRIVER_80211_AP_STA_DEL(pAd, (VOID *)&rApStaDel,
				     params->reason_code);
#else
		...
		if (params->sta_flags_set & BIT(NL80211_STA_FLAG_AUTHORIZED)) {
			CFG80211DBG(
				...
				("80211> STA(%02X:%02X:%02X:%02X:%02X:%02X) ==> PortSecured\n",
				 PRINT_MAC(pMacAddr)));
			RTMP_DRIVER_80211_AP_MLME_PORT_SECURED(
				pAd, (VOID *)pMacAddr, 1);
		} else {
			CFG80211DBG(
				DBG_LVL_TRACE,
					       ...
					       IN struct device *pDev)
{
	...
	*/
	...
	pWdev->wiphy->n_iface_combinations =
 ...
 * ========================================================================
 */
BOOLEAN CFG80211_Register(IN VOID *pAd, IN struct device *pDev,
