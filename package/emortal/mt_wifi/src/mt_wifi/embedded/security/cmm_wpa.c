...
static BOOLEAN wpa_check_rsn_cap(
	...
	IN UCHAR rsnie_len)
{
	...
	if (end_field >= RSN_FIELD_RSN_CAP
		...
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("[ERROR]%s : invalid rsn cap !!!\n", __func__));
		return MLME_UNSPECIFY_FAIL;
	}
	return res;
}
	...
	OUT ULONG *offset)
{
	...
	*offset += LEN_KDE_HDR + len;
	...
	hex_dump_with_cat_and_lvl("oci kde", buf,
		LEN_KDE_HDR + len, DBG_CAT_SEC, CATSEC_OCV, DBG_LVL_OFF); /* todo: change to trace*/
}
	...
	OUT PEAPOL_PACKET pMsg)
{
	...
	if ((MsgType == EAPOL_PAIR_MSG_3 && bWPA2) || (MsgType == EAPOL_GROUP_MSG_1)) {
		...
		if (pSecPairwise->ocv_support) {
			MTWF_LOG(DBG_CAT_SEC, CATSEC_OCV, DBG_LVL_WARN, ("%s - Insert OCV\n", __func__));
			insert_oci_kde(pEntry->pAd, pEntry->wdev, &Key_Data[data_offset], &data_offset);
		}
#ifdef BCN_PROTECTION_SUPPORT
		if (pSecGroup->bcn_prot_cfg.bcn_prot_en) {
			MTWF_LOG(DBG_CAT_SEC, CATSEC_BCNPROT, DBG_LVL_WARN, ("%s - Insert BIGTK\n", __func__));
	...
	OUT struct key_data_element_ptr *ele_ptr)
{
	...
	while (len > 0) {
		...
		if (eid_ptr->Len + 2 > len) { /* it would be a normal behavior if need to padding */
			...
			hex_dump_with_cat_and_lvl("key data", data, data_len, DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_INFO);
			break;
		}
	...
	IN MAC_TABLE_ENTRY * pEntry)
{
	...
	if (MsgType == EAPOL_PAIR_MSG_2 || MsgType == EAPOL_PAIR_MSG_3) {
		...
		if (!RTMPCheckRSNIE(&ele_ptr, bWPA2, pEntry)) {
			...
			hex_dump("Receive RSN_IE ", ele_ptr.rsne_ptr, eid_ptr->Len + 2);
		...
		if (bWPA2 && ele_ptr.rsnxe_ptr) {
			...
			MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: peer rsnxe is presented\n", __func__));
		...
		if (IS_AKM_SAE(pEntry->SecConfig.AKMMap) &&
			...
			MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				("%s: should include rsnxe in 4-way msg2/3, rsnxe_len = %d, sae_conn_type = %d\n",
				__func__, pEntry->SecConfig.rsnxe_len, pEntry->SecConfig.sae_conn_type));
			return FALSE;
		}
#endif
	}
	...
	if (bWPA2 &&
		...
		if (pEntry->SecConfig.ocv_support && !ele_ptr.oci_kde_ptr) {
				MTWF_LOG(DBG_CAT_SEC, CATSEC_OCV, DBG_LVL_OFF, ("ERROR: oci kde isn't existed\n"));
				return FALSE;
		}
	}
	if (!bWPA2 && MsgType == EAPOL_GROUP_MSG_1) {
	...
	IN MAC_TABLE_ENTRY * pEntry)
{
	...
	MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			("===> %s, STA(:%02x:%02x:%02x:%02x:%02x:%02x), WpaState(%d)\n",
			__func__, PRINT_MAC(pEntry->Addr), pHandshake4Way->WpaState));
	...
	if ((GroupMsg1Len < (MIN_LEN_OF_EAPOL_KEY_MSG - LEN_KEY_DESC_MIC))
		...
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("<=== %s : error!pair Message 1 length = %d\n",
			__func__, GroupMsg1Len));
		os_free_mem(mpool);
		return;
	}
	...
	IN MAC_TABLE_ENTRY * pEntry)
{
	...
	if ((GroupMsg1Len < (MIN_LEN_OF_EAPOL_KEY_MSG - LEN_KEY_DESC_MIC))
			|| (GroupMsg1Len > TX_EAPOL_BUFFER)) {
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("<=== %s : error!pair Message 2 length = %d\n",
			__func__, GroupMsg1Len));
		os_free_mem(mpool);
		return;
	}
	...
	IN MAC_TABLE_ENTRY * pEntry)
{
	...
	if (IS_AKM_FILS_Entry(pEntry)) {
		...
		if (!filsInfo->is_pending_encrypt) {
			...
			if ((filsInfo->pending_ie_len < (MIN_LEN_OF_EAPOL_KEY_MSG - LEN_KEY_DESC_MIC))
				...
				MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("<=== %s : error!pair Message 3 length = %d£¬pending_ie_len\n",
					__func__, filsInfo->pending_ie_len));
				os_free_mem(mpool);
				return;
			}
	...
	if ((GroupMsg1Len < (MIN_LEN_OF_EAPOL_KEY_MSG))
		...
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("<=== %s : error!pair Message 3 length = %d\n",
			__func__, GroupMsg1Len));
		os_free_mem(mpool);
		return;
	}
	...
	IN MAC_TABLE_ENTRY * pEntry)
{
	...
	if (pEntry == NULL) {/*ALPS05330180*/
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s:pEntry is NULL!\n", __func__));
		return;
	}
	...
	tr_entry = &tr_ctl->tr_entry[pEntry->wcid];
	if (tr_entry == NULL) {
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s:tr_entry is NULL!\n", __func__));
		return;
	}
	...
	if ((GroupMsg1Len < (MIN_LEN_OF_EAPOL_KEY_MSG - LEN_KEY_DESC_MIC))
		|| (GroupMsg1Len > TX_EAPOL_BUFFER)) {
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("<=== %s : error!pair Message 4 length = %d\n",
			__func__, GroupMsg1Len));
		os_free_mem(mpool);
		return;
	}
	...
	IN MAC_TABLE_ENTRY * pEntry)
{
	...
	if ((GroupMsg1Len < (MIN_LEN_OF_EAPOL_KEY_MSG - LEN_KEY_DESC_MIC))
		|| (GroupMsg1Len > TX_EAPOL_BUFFER)) {
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("<=== %s :error! Group Message 1 length = %d\n",
			__func__, GroupMsg1Len));
		os_free_mem(mpool);
		return;
	}
	...
	IN MAC_TABLE_ENTRY * pEntry)
{
	...
	MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("===> %s\n", __func__));
	if (pEntry == NULL) {
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("!!!%s :pEntry NULL!!!\n", __func__));
		return;
	}
	/* Allocate memory for output*/
	os_alloc_mem(NULL, (PUCHAR *)&mpool, TX_EAPOL_BUFFER);
	...
	if ((GroupMsg1Len < (MIN_LEN_OF_EAPOL_KEY_MSG - LEN_KEY_DESC_MIC))
		|| (GroupMsg1Len > TX_EAPOL_BUFFER)) {
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("<=== %s : error!group Message 2 length = %d\n",
			__func__, GroupMsg1Len));
		os_free_mem(mpool);
		return;
	}
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (pEntry == NULL) {
		...
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			("%s, find entry:%02x:%02x:%02x:%02x:%02x:%02x\n",
				__func__, PRINT_MAC(pEntry->Addr)));
	...
	tr_entry = &tr_ctl->tr_entry[pEntry->wcid];
	MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s: PortSecured(%d), WpaState(%d),AKM(0x%x),is_cache(%d)\n",
		__func__, tr_entry->PortSecured, pSecConfig->Handshake.WpaState,
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	if ((pEntry) && IS_ENTRY_CLIENT(pEntry)) {
		...
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
				("%s===> STA(%02x:%02x:%02x:%02x:%02x:%02x), WpaState=%d,  MsgType=%d\n",
				__func__, PRINT_MAC(Elem->Msg), pHandshake->WpaState, pHandshake->MsgType));
	...
	IN RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			("%s::td_value_fixed_en = %d, td_value = %d\n",
