...
static const UINT32 aes_rcon[] = { 0x00000000, 0x01000000, 0x02000000,
...
static UINT8 Const_Zero[16] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
				0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
static UINT8 Const_Rb[16] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
========================================================================
*/
VOID RT_AES_KeyExpansion(IN UINT8 Key[], IN UINT KeyLength,
...
========================================================================
*/
VOID RT_AES_Encrypt(IN UINT8 PlainBlock[], IN UINT PlainBlockSize,
		    IN UINT8 Key[], IN UINT KeyLength, OUT UINT8 CipherBlock[],
	...
	*/
	...
	for (RoundIndex = 1; RoundIndex < NumberOfRound; RoundIndex++) {
		...
		for (RowIndex = 0; RowIndex < AES_STATE_ROWS; RowIndex++)
			for (ColumnIndex = 0; ColumnIndex < AES_STATE_COLUMNS;
			     ColumnIndex++)
				paes_ctx->State[RowIndex]
					       [ColumnIndex] = aes_sbox_enc
					[paes_ctx->State[RowIndex][ColumnIndex]];
...
========================================================================
*/
VOID RT_AES_Decrypt(IN UINT8 CipherBlock[], IN UINT CipherBlockSize,
		    IN UINT8 Key[], IN UINT KeyLength, OUT UINT8 PlainBlock[],
	...
	*/
	...
	for (RoundIndex = (NumberOfRound - 1); RoundIndex > 0; RoundIndex--) {
		...
		for (ColumnIndex = 0; ColumnIndex < AES_STATE_COLUMNS;
			...
			Row3 = paes_ctx->State[3][ColumnIndex];
			paes_ctx->State[0][ColumnIndex] =
				aes_mul_e[Row0] ^ aes_mul_b[Row1] ^
				aes_mul_d[Row2] ^ aes_mul_9[Row3];
			paes_ctx->State[1][ColumnIndex] =
				aes_mul_9[Row0] ^ aes_mul_e[Row1] ^
				aes_mul_b[Row2] ^ aes_mul_d[Row3];
			paes_ctx->State[2][ColumnIndex] =
				aes_mul_d[Row0] ^ aes_mul_9[Row1] ^
				aes_mul_e[Row2] ^ aes_mul_b[Row3];
			paes_ctx->State[3][ColumnIndex] =
				aes_mul_b[Row0] ^ aes_mul_d[Row1] ^
				aes_mul_9[Row2] ^ aes_mul_e[Row3];
		}
	}
...
========================================================================
*/
VOID AES_CCM_MAC(IN UINT8 Payload[], IN UINT PayloadLength, IN UINT8 Key[],
		 IN UINT KeyLength, IN UINT8 Nonce[], IN UINT NonceLength,
		 IN UINT8 AAD[], IN UINT AADLength, IN UINT MACLength,
		 OUT UINT8 MACText[])
{
	...
	Block[0] |= Temp_Value;
	Temp_Value = (15 - NonceLength) -
		     1; /* Set bit 0-2 to (q-1), q = 15 - Nonce Length */
	Block[0] |= Temp_Value;
...
========================================================================
*/
INT AES_CCM_Encrypt(IN UINT8 PlainText[], IN UINT PlainTextLength,
		    IN UINT8 Key[], IN UINT KeyLength, IN UINT8 Nonce[],
		    IN UINT NonceLength, IN UINT8 AAD[], IN UINT AADLength,
		    IN UINT MACLength, OUT UINT8 CipherText[],
		    INOUT UINT *CipherTextLength)
{
	...
	if ((MACLength != 4) && (MACLength != 6) && (MACLength != 8) &&
	    (MACLength != 10) && (MACLength != 12) && (MACLength != 14) &&
	    (MACLength != 16)) {
		MTWF_LOG(
			DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
...
INT AES_CCM_Decrypt(IN UINT8 CipherText[], IN UINT CipherTextLength,
		    IN UINT8 Key[], IN UINT KeyLength, IN UINT8 Nonce[],
		    IN UINT NonceLength, IN UINT8 AAD[], IN UINT AADLength,
		    IN UINT MACLength, OUT UINT8 PlainText[],
...
========================================================================
*/
VOID AES_CMAC_GenerateSubKey(IN UINT8 Key[], IN UINT KeyLength,
...
========================================================================
*/
VOID AES_CMAC(IN UINT8 PlainText[], IN UINT PlainTextLength, IN UINT8 Key[],
	      IN UINT KeyLength, OUT UINT8 MACText[], INOUT UINT *MACTextLength)
{
	...
	if ((PlainTextLength - PlainBlockStart) == AES_BLOCK_SIZES) {
		for (Index = 0; Index < AES_BLOCK_SIZES; Index++)
			Y[Index] = PlainText[PlainBlockStart + Index] ^
				   X[Index] ^ SubKey1[Index];
	} else {
...
static UINT8 Default_IV[8] = { 0xa6, 0xa6, 0xa6, 0xa6, 0xa6, 0xa6, 0xa6, 0xa6 };
...
========================================================================
*/
VOID AES_CBC_Encrypt(IN UINT8 PlainText[], IN UINT PlainTextLength,
		     IN UINT8 Key[], IN UINT KeyLength, IN UINT8 IV[],
		     IN UINT IVLength, OUT UINT8 CipherText[],
		     INOUT UINT *CipherTextLength)
{
	 ...
	 */
	PaddingSize = ((UINT)AES_BLOCK_SIZES) -
...
VOID AES_CBC_Decrypt(IN UINT8 CipherText[], IN UINT CipherTextLength,
		     IN UINT8 Key[], IN UINT KeyLength, IN UINT8 IV[],
		     IN UINT IVLength, OUT UINT8 PlainText[],
		     INOUT UINT *PlainTextLength)
{
...
========================================================================
*/
INT AES_Key_Wrap(IN UINT8 PlainText[], IN UINT PlainTextLength, IN UINT8 Key[],
		 IN UINT KeyLength, OUT UINT8 CipherText[],
		 OUT UINT *CipherTextLength)
{
	 ...
	 */
	Number_Of_Block = PlainTextLength /
			  AES_KEY_WRAP_BLOCK_SIZE; /* 64 bits each block */
	...
	for (Index_j = 0; Index_j < 6; Index_j++) {
		for (Index_i = 0; Index_i < Number_Of_Block; Index_i++) {
			...
			IV[7] = Block_B[7] ^
				((Number_Of_Block * Index_j) + Index_i + 1);
...
INT AES_Key_Unwrap(IN UINT8 CipherText[], IN UINT CipherTextLength,
		   IN UINT8 Key[], IN UINT KeyLength, OUT UINT8 PlainText[],
		   OUT UINT *PlainTextLength)
{
	 ...
	 */
	Number_Of_Block =
		PlainLength / AES_KEY_WRAP_BLOCK_SIZE; /* 64 bits each block */
