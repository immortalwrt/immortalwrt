...
static VOID ApCliWpaDisassocApAndBlockAssoc(IN PVOID SystemSpecific1,
					    IN PVOID FunctionContext,
					    IN PVOID SystemSpecific2,
					    IN PVOID SystemSpecific3);
#endif /* APCLI_CERT_SUPPORT */

static VOID ApCliMlmeAssocReqAction(IN PRTMP_ADAPTER pAd,
				    IN MLME_QUEUE_ELEM *Elem);

static VOID ApCliMlmeDisassocReqAction(IN PRTMP_ADAPTER pAd,
				       IN MLME_QUEUE_ELEM *Elem);

static VOID ApCliPeerAssocRspAction(IN PRTMP_ADAPTER pAd,
				    IN MLME_QUEUE_ELEM *Elem);

static VOID ApCliPeerDisassocAction(IN PRTMP_ADAPTER pAd,
				    IN MLME_QUEUE_ELEM *Elem);

static VOID ApCliAssocTimeoutAction(IN PRTMP_ADAPTER pAd,
				    IN MLME_QUEUE_ELEM *Elem);

static VOID ApCliInvalidStateWhenAssoc(IN PRTMP_ADAPTER pAd,
				       IN MLME_QUEUE_ELEM *Elem);

static VOID ApCliInvalidStateWhenDisassociate(IN PRTMP_ADAPTER pAd,
					      IN MLME_QUEUE_ELEM *Elem);

static VOID ApCliAssocPostProc(IN PRTMP_ADAPTER pAd, IN PUCHAR pAddr2,
			       IN USHORT CapabilityInfo, IN USHORT IfIndex,
...
static VOID set_mlme_rsn_ie(PRTMP_ADAPTER pAd, struct wifi_dev *wdev,
...
static VOID ApCliAssocDeleteMacEntry(IN PRTMP_ADAPTER pAd, IN UCHAR ifIndex,
				     IN UCHAR CliIdx)
{
...
/*
    ...
    ==========================================================================
 */
VOID ApCliAssocStateMachineInit(IN PRTMP_ADAPTER pAd, IN STATE_MACHINE *S,
				OUT STATE_MACHINE_FUNC Trans[])
{
	UCHAR i;
	PAPCLI_STRUCT pApCliEntry;

	StateMachineInit(S, (STATE_MACHINE_FUNC *)Trans, APCLI_MAX_ASSOC_STATE,
			 APCLI_MAX_ASSOC_MSG, (STATE_MACHINE_FUNC)Drop,
			 APCLI_ASSOC_IDLE, APCLI_ASSOC_MACHINE_BASE);
	/* first column */
	...
	StateMachineSetAction(
		S, APCLI_ASSOC_WAIT_RSP, APCLI_MT2_MLME_DISASSOC_REQ,
		(STATE_MACHINE_FUNC)ApCliInvalidStateWhenDisassociate);
	StateMachineSetAction(S, APCLI_ASSOC_WAIT_RSP,
			      APCLI_MT2_PEER_DISASSOC_REQ,
			      (STATE_MACHINE_FUNC)ApCliPeerDisassocAction);
	...
	for (i = 0; i < MAX_APCLI_NUM; i++) {
		...
		RTMPInitTimer(
			...
			&pAd->ApCfg.ApCliTab[i]
				 .MlmeAux.WpaDisassocAndBlockAssocTimer,
			      ...
			      IN PVOID SystemSpecific3)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_TRACE,
		 ("APCLI_ASSOC - enqueue APCLI_MT2_ASSOC_TIMEOUT\n"));
					    ...
					    IN PVOID SystemSpecific3)
{
	...
	MTWF_LOG(
		...
		("(%s) disassociate with current AP after sending second continuous EAPOL frame.\n",
		 __func__));
	DisassocParmFill(pAd, &DisassocReq, pApCliEntry->MlmeAux.Bssid,
			 REASON_MIC_FAILURE);
				    ...
				    IN MLME_QUEUE_ELEM *Elem)
{
	...
	UCHAR WmeIe[9] = { IE_VENDOR_SPECIFIC,
			   0x07,
			   0x00,
			   0x50,
			   0xf2,
	...
	defined(RT_CFG80211_P2P_CONCURRENT_DEVICE) ||                          \
	...
	if (apcli_entry->bBlockAssoc == TRUE) {
		...
		*pCurrState = APCLI_ASSOC_IDLE;
		...
		MlmeEnqueue(pAd, APCLI_CTRL_STATE_MACHINE, APCLI_CTRL_ASSOC_RSP,
			    ...
			    ifIndex);
	} else if (MlmeAssocReqSanity(pAd, Elem->Msg, Elem->MsgLen, ApAddr,
				      &CapabilityInfo, &Timeout, &ListenIntv)) {
		/* RTMPCancelTimer(&apcli_entry->MlmeAux.ApCliAssocTimer, &Cancelled); */
#ifdef MAC_REPEATER_SUPPORT
		if (CliIdx != 0xFF)
			RTMPCancelTimer(&pReptEntry->ApCliAssocTimer,
					&Cancelled);
		else
#endif /* MAC_REPEATER_SUPPORT */
			RTMPCancelTimer(&apcli_entry->MlmeAux.ApCliAssocTimer,
					&Cancelled);
		...
		ApCliMgtMacHeaderInit(pAd, &AssocHdr, SUBTYPE_ASSOC_REQ, 0,
				      ApAddr, ApAddr, ifIndex);
#ifdef MAC_REPEATER_SUPPORT
		...
		MakeOutgoingFrame(pOutBuffer, &FrameLen, sizeof(HEADER_802_11),
				  &AssocHdr, 2, &CapabilityInfo, 2, &ListenIntv,
				  1, &SsidIe, 1, &apcli_entry->MlmeAux.SsidLen,
				  apcli_entry->MlmeAux.SsidLen,
				  apcli_entry->MlmeAux.Ssid, 1, &SupRateIe, 1,
				  &apcli_entry->MlmeAux.SupRateLen,
				  apcli_entry->MlmeAux.SupRateLen,
				  apcli_entry->MlmeAux.SupRate, END_OF_ARGS);

		if (apcli_entry->MlmeAux.ExtRateLen != 0) {
			MakeOutgoingFrame(pOutBuffer + FrameLen, &tmp, 1,
					  &ExtRateIe, 1,
					  &apcli_entry->MlmeAux.ExtRateLen,
					  apcli_entry->MlmeAux.ExtRateLen,
					  apcli_entry->MlmeAux.ExtRate,
					  END_OF_ARGS);
			FrameLen += tmp;
		}
		...
		*/
		if (pAd->CommonCfg.HT_DisallowTKIP &&
		    IS_INVALID_HT_SECURITY(
			    apcli_entry->MlmeAux.PairwiseCipher)) {
...
#endif /* DOT11_VHT_AC */
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_TRACE,
				 ("%s : Force AP-client as Non-HT mode\n",
				  __func__));
		}
		...
		if ((apcli_entry->MlmeAux.HtCapabilityLen > 0) &&
			...
			NdisMoveMemory(&HtCapabilityTmp,
				       &apcli_entry->MlmeAux.HtCapability,
			...
			if (HcIsBfCapSupport(wdev) == FALSE) {
				...
				mt_WrapSetETxBFCap(pAd, wdev,
						   &HtCapabilityTmp.TxBFCap);
				...
				if (cap->FlgHwTxBfCap & TXBF_AID_HW_LIMIT) {
					...
					if ((pReptEntry != NULL) &&
					    (pAd->fgClonedStaWithBfeeSelected ==
					     FALSE)) {
						MAC_TABLE_ENTRY *pEntry =
							(MAC_TABLE_ENTRY *)NULL;
				...
				if (cap->FlgHwTxBfCap & TXBF_AID_HW_LIMIT) {
					...
					if ((pReptEntry != NULL) &&
						...
						MTWF_LOG(
							...
							DBG_SUBCAT_ALL,
							DBG_LVL_OFF,
							("%s : OriginalAddress[0~5] = %x:%x:%x:%x:%x:%x\n",
							 __func__,
		...
		if (apcli_entry->MlmeAux.HtCapabilityLen > 0) {
			...
			if (bNeedAppendExtIE == TRUE) {
				for (infoPos = (extInfoLen - 1);
				     infoPos >= EXT_CAP_MIN_SAFE_LENGTH;
				     infoPos--) {
					if (pInfo[infoPos] == 0)
						extInfoLen--;
					else
		...
		if ((apcli_entry->wpa_supplicant_info.WpaSupplicantUP & 0x7F) ==
			...
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_TRACE,
				 ("%s:: APCLI WPA_ASSOC_IE FROM SUPPLICANT\n",
				  __func__));
			MakeOutgoingFrame(
				pOutBuffer + FrameLen, &TmpWpaAssocIeLen,
			...
			VarIesOffset = 0;
			NdisMoveMemory(
				apcli_entry->ReqVarIEs + VarIesOffset,
				apcli_entry->wpa_supplicant_info.pWpaAssocIe,
				apcli_entry->wpa_supplicant_info.WpaAssocIeLen);
			VarIesOffset +=
				apcli_entry->wpa_supplicant_info.WpaAssocIeLen;
			...
			if (IS_AKM_PSK(apcli_entry->MlmeAux.AKMMap)
			    ...
			    && ((apcli_entry->wdev.WscControl.WscConfMode ==
				 WSC_DISABLE) ||
				((apcli_entry->wdev.WscControl.WscConfMode !=
				  WSC_DISABLE) &&
     ...
     (defined(APCLI_SAE_SUPPORT) || defined(APCLI_OWE_SUPPORT)))
				{
					...
					for (rsne_idx = 0;
						...
						MakeOutgoingFrame(
							pOutBuffer + FrameLen,
							&TempLen, 1,
							&pentry->SecConfig.RSNE_EID
								 [rsne_idx][0],
							1,
							&pentry->SecConfig.RSNE_Len
								 [rsne_idx],
							pentry->SecConfig.RSNE_Len
								[rsne_idx],
							&pentry->SecConfig.RSNE_Content
								 [rsne_idx][0],
							END_OF_ARGS);
						FrameLen += TempLen;
					}
...
#else
			{
				...
				{ /* Todo by Eddy: It's not good code */
					...
					pSecConfig->AKMMap =
						apcli_entry->MlmeAux.AKMMap;
					pSecConfig->PairwiseCipher =
						apcli_entry->MlmeAux
							.PairwiseCipher;
					pSecConfig->GroupCipher =
						apcli_entry->MlmeAux.GroupCipher;
				...
				for (rsne_idx = 0; rsne_idx < SEC_RSNIE_NUM;
				     rsne_idx++) {
					if (apcli_entry->wdev.SecConfig
						    .RSNE_Type[rsne_idx] ==
					    SEC_RSNIE_NONE)
						continue;

					MakeOutgoingFrame(
						pOutBuffer + FrameLen, &TempLen,
						1,
						&apcli_entry->wdev.SecConfig
							 .RSNE_EID[rsne_idx][0],
						1,
		...
		if ((pAd->ApCfg.ApCliTab[ifIndex].wdev.WscControl.WscConfMode !=
			...
			if (pWscBuf != NULL) {
				...
				WscBuildAssocReqIE(&pAd->ApCfg.ApCliTab[ifIndex]
							    .wdev.WscControl,
						   pWscBuf, &WscIeLen);
				MakeOutgoingFrame(pOutBuffer + FrameLen,
		 ...
		 */
		{
			...
			if ((pAd->ApCfg.bMACRepeaterEn) &&
				...
				if (pReptEntry->pre_entry_alloc == TRUE)
					ApCliAssocDeleteMacEntry(pAd, ifIndex,
								 CliIdx);
...
#if (defined(FAST_EAPOL_WAR) && defined(APCLI_OWE_SUPPORT))
		{
			...
			if ((pAd->ApCfg.bMACRepeaterEn) && (CliIdx != 0xFF)) {
				if (pReptEntry->pre_entry_alloc == TRUE)
					pentry =
						&pAd->MacTab.Content
							 [pReptEntry->MacTabWCID];
...
#endif /* MAC_REPEATER_SUPPORT */
			RTMPSetTimer(&apcli_entry->MlmeAux.ApCliAssocTimer,
				     Timeout);
				       ...
				       IN MLME_QUEUE_ELEM *Elem)
{
	...
	ApCliMgtMacHeaderInit(pAd, &DisassocHdr, SUBTYPE_DISASSOC, 0,
			      pDisassocReq->Addr, pDisassocReq->Addr, ifIndex);
#ifdef MAC_REPEATER_SUPPORT
...
#endif /* MAC_REPEATER_SUPPORT */
	MakeOutgoingFrame(pOutBuffer, &FrameLen, sizeof(HEADER_802_11),
			  &DisassocHdr, 2, &pDisassocReq->Reason, END_OF_ARGS);
	MiniportMMRequest(pAd, QID_AC_BE, pOutBuffer, FrameLen);
	MlmeFreeMemory(pOutBuffer);
	*pCurrState = APCLI_ASSOC_IDLE;
				    ...
				    IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (ApCliPeerAssocRspSanity(
		    pAd, Elem->Msg, Elem->MsgLen, Addr2, &CapabilityInfo,
		    &Status, &Aid, SupRate, &SupRateLen, ExtRate, &ExtRateLen,
		    &HtCapability, &AddHtInfo, &HtCapabilityLen, &AddHtInfoLen,
		    &NewExtChannelOffset, &EdcaParm, &CkipFlag, ie_list)) {
		/* The frame is for me ? */
		if (MAC_ADDR_EQUAL(Addr2,
				   pAd->ApCfg.ApCliTab[ifIndex].MlmeAux.Bssid)) {
#if defined(RT_CFG80211_P2P_CONCURRENT_DEVICE) ||                              \
	defined(CFG80211_MULTI_STA) || defined(APCLI_CFG80211_SUPPORT)
			PFRAME_802_11 pFrame = (PFRAME_802_11)(Elem->Msg);
			/* Store the AssocRsp Frame to wpa_supplicant via CFG80211 */
			NdisZeroMemory(pAd->ApCfg.ApCliTab[ifIndex].ResVarIEs,
				       MAX_VIE_LEN);
			pAd->ApCfg.ApCliTab[ifIndex].ResVarIELen = 0;
			pAd->ApCfg.ApCliTab[ifIndex].ResVarIELen =
				Elem->MsgLen - 6 - sizeof(HEADER_802_11);
			NdisCopyMemory(
				pAd->ApCfg.ApCliTab[ifIndex].ResVarIEs,
				&pFrame->Octet[6],
				pAd->ApCfg.ApCliTab[ifIndex].ResVarIELen);
			...
			if (Status == MLME_SUCCESS) {
...
#ifdef MAP_R2
					pEntry->profile =
						ie_list->MAP_ProfileValue;
					pApCliEntry->wdev.MAPCfg.primary_vid =
						ie_list->MAP_default_vid;
					MTWF_LOG(DBG_CAT_CLIENT,
						 CATCLIENT_APCLI, DBG_LVL_ERROR,
						 ("pEntry=%p, profile=%02x\n",
				...
				{
					ApCliAssocPostProc(
						pAd, Addr2, CapabilityInfo,
						ifIndex, SupRate, SupRateLen,
						ExtRate, ExtRateLen, &EdcaParm,
				...
				if (CliIdx != 0xFF)
					pEntry =
						&pAd->MacTab.Content
							 [pReptEntry->MacTabWCID];
#endif /* MAC_REPEATER_SUPPORT */
				set_mlme_rsn_ie(pAd, &pApCliEntry->wdev,
						pEntry);
				...
				if (IS_AKM_OWE(pEntry->SecConfig.AKMMap)) {
					...
					pmkid = WPA_ExtractSuiteFromRSNIE(
						ie_list->RSN_IE,
						ie_list->RSNIE_Len, PMKID_LIST,
						&pmkid_count);

					if (pmkid != NULL) {
						idx = apcli_search_pmkid_cache(
							pAd, Addr2, ifIndex,
							CliIdx);
					...
					if ((pEntry->need_process_ecdh_ie ==
					     TRUE) &&
					    (ie_list->ecdh_ie.length > 0))
						NdisMoveMemory(
							&pEntry->ecdh_ie,
							&ie_list->ecdh_ie,
				...
				pApCliEntry->MlmeAux.APEdcaParm.bValid =
					EdcaParm.bValid;
				ApCliCtrlMsg.Status = MLME_SUCCESS;
				    ...
				    IN MLME_QUEUE_ELEM *Elem)
{
	...
	{
		pCurrState = &pAd->ApCfg.ApCliTab[ifIndex].AssocCurrState;
		pDisconnect_Sub_Reason =
			&pAd->ApCfg.ApCliTab[ifIndex].Disconnect_Sub_Reason;
	}
	...
	if (PeerDisassocSanity(pAd, Elem->Msg, Elem->MsgLen, Addr2, &Reason)) {
		...
		if (MAC_ADDR_EQUAL(pAd->ApCfg.ApCliTab[ifIndex].MlmeAux.Bssid,
				   Addr2)) {
			*pCurrState = APCLI_ASSOC_IDLE;
#ifdef MAC_REPEATER_SUPPORT
			ifIndex = (USHORT)(Elem->Priv);
				    ...
				    IN MLME_QUEUE_ELEM *Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_ERROR,
		 ("APCLI_ASSOC - ApCliAssocTimeoutAction\n"));
	...
	*pCurrState = APCLI_ASSOC_IDLE;
	...
	ifIndex = (USHORT)(Elem->Priv);
#endif /* MAC_REPEATER_SUPPORT */
	MlmeEnqueue(pAd, APCLI_CTRL_STATE_MACHINE, APCLI_CTRL_ASSOC_REQ_TIMEOUT,
		    0, NULL, ifIndex);
}
				       ...
				       IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (ifIndex >= REPT_MLME_START_IDX) {
		...
		ifIndex = pReptEntry->wdev->func_idx;
		pCurrState =
			&pAd->ApCfg.pRepeaterCliPool[CliIdx].AssocCurrState;
	} else
#endif /* MAC_REPEATER_SUPPORT */
		pCurrState = &pAd->ApCfg.ApCliTab[ifIndex].AssocCurrState;
	...
	if (Idx == BSS_NOT_FOUND) {
		...
		if (IS_AKM_WPA_CAPABILITY(wdev->SecConfig.AKMMap) &&
			...
			while (len > 0) {
				...
				/* For WPA/WPAPSK */
				if ((pEid->Eid == IE_WPA) &&
				    (NdisEqualMemory(pEid->Octet, WPA_OUI,
						     4)) &&
			       ...
			       IN ADD_HT_INFO_IE *pAddHtInfo)
{
	...
	NdisMoveMemory(&pApCliEntry->MlmeAux.APEdcaParm, pEdcaParm,
		       sizeof(EDCA_PARM));
