...
UCHAR FirstSafeChannel(RTMP_ADAPTER *pAd, struct wifi_dev *wdev)
{
	...
	for (i = 0; i < pChCtrl->ChListNum; i++) {
		if (IsChannelSafe(pAd, pChCtrl->ChList[i].Channel)) {
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s, caller:%pS: find the first safe channel: %d\n", __func__, OS_TRACE, safe_channel));
			break;
		}
	}

	if (i == pChCtrl->ChListNum) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s, caller:%pS: failed to find a safe channel and use the first default channel\n", __func__, OS_TRACE));
	...
	if (WMODE_CAP_AC(wdev->PhyMode) && (pStaCfg->StaActive.SupportedPhyInfo.bVhtEnable == TRUE)) {
		if (HAS_VHT_OP_EXIST(cmm_ies->ie_exists) && HAS_HT_OP_EXIST(cmm_ies->ie_exists)) {
			...
			if ((prev_operating_mode != current_operating_mode) ||
				(pwdev && (softap_prev_op_mode != current_operating_mode))) {
				MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_TRACE,
			...
			if ((prev_operating_mode > cmm_ies->vht_op.vht_op_info.ch_width) ||
			...
			if (pwdev && (current_operating_mode == 0 && (prev_operating_mode >= 1 || softap_prev_op_mode >= 1) && cmm_ies->ht_op.AddHtInfo.RecomWidth == 0)
				...
				bAdjustHTBW = TRUE;
			} else if (pwdev && (current_operating_mode >= 1 && (prev_operating_mode == 0 || softap_prev_op_mode == 0) && cmm_ies->ht_op.AddHtInfo.RecomWidth == 1)
