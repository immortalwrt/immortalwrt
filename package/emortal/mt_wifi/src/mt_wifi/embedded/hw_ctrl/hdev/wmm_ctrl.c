...
VOID wmm_ctrl_release_entry(struct hdev_obj *obj)
{
	...
	if (!obj || !obj->bWmmAcquired) {
		MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(): Can't find HdevObj or Edca not required\n", __func__));
		return;
	}
	...
	if (!edca || !edca->bValid) {
		MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(): Can't find Edca for rdev: %d, Obj: %d\n",
	...
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Release a WMM for ObjIdx: %d\n", obj->Idx));
}
...
struct wmm_entry *wmm_ctrl_acquire_entry(struct hdev_obj *obj, struct _EDCA_PARM *pEdcaParm)
{
	...
	if (!obj) {
		MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s(): Can't find HdevObj\n", __func__));
		goto end;
	}
...
VOID wmm_ctrl_show_entry(struct wmm_ctrl *ctrl)
{
	...
	for (i = 0; i < ctrl->num; i++) {
		...
		if (edca->bValid) {
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\tEdcaIdx: %d,BandIdx: %d, RfCnt: %d, TXMODE: %d\n",
