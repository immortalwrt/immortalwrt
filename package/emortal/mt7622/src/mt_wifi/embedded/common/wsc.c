...
UCHAR AP_Wsc_Pri_Dev_Type[8] = {
	0x00, 0x06, 0x00, 0x50, 0xf2, 0x04, 0x00, 0x01
};

#ifdef APCLI_SUPPORT

VOID WscApCliLinkDown(IN PRTMP_ADAPTER pAd, IN PWSC_CTRL pWscControl);
#endif /* APCLI_SUPPORT */
#endif /* CONFIG_AP_SUPPORT */

static BOOLEAN WscCheckNonce(IN PRTMP_ADAPTER pAdapter,
			     IN MLME_QUEUE_ELEM *Elem, IN BOOLEAN bFlag,
			     IN PWSC_CTRL pWscControl);

static VOID WscGetConfigErrFromNack(IN PRTMP_ADAPTER pAdapter,
				    IN MLME_QUEUE_ELEM *pElem,
				    OUT USHORT *pConfigError);

static INT WscSetAuthMode(IN PRTMP_ADAPTER pAd, IN UCHAR CurOpMode,
			  IN UCHAR apidx, IN RTMP_STRING *arg);

static INT WscSetEncrypType(IN PRTMP_ADAPTER pAd, IN UCHAR CurOpMode,
			    IN UCHAR apidx, IN RTMP_STRING *arg);

static VOID WscSendNACK(IN PRTMP_ADAPTER pAdapter, IN MAC_TABLE_ENTRY *pEntry,
			IN PWSC_CTRL pWscControl);

static INT wsc_write_dat_file_thread(IN ULONG data);

#ifdef CONFIG_AP_SUPPORT
static UINT32 WscGetAuthMode(IN USHORT authFlag);

static UINT32 WscGetWepStatus(IN USHORT encryFlag);
#endif /* CONFIG_AP_SUPPORT */

#ifdef CON_WPS
VOID ConWpsApCliMonitorTimeout(IN PVOID SystemSpecific1,
			       ...
			       IN PVOID SystemSpecific3)
{
...
#endif /* MULTI_INF_SUPPORT */
	{
		...
		if (pAd->ApCfg.ConWpsApCliMode == CON_WPS_APCLI_BAND_2G)
			WscStop(pAd, TRUE,
				&pAd->ApCfg.ApCliTab[BSS1].wdev.WscControl);
		else if (pAd->ApCfg.ConWpsApCliMode == CON_WPS_APCLI_BAND_5G)
			WscStop(pAd, TRUE,
				&pAd->ApCfg.ApCliTab[BSS0].wdev.WscControl);
	}
...
#endif /* MULTI_INF_SUPPORT */
	{
		for (index = 0; index < MAX_APCLI_NUM; index++)
			RTEnqueueInternalCmd(pActionAd, CMDTHREAD_APCLI_IF_DOWN,
					     (VOID *)&index, sizeof(UCHAR));
	}
	pActionAd->ApCfg.ApCliTab[BSS0].Enable = TRUE;

	for (index = 0; index < MAX_APCLI_NUM; index++)
		pAd->ApCfg.ApCliTab[index].wdev.WscControl.conWscStatus =
	...
	if (from_apcli == FALSE) {
		for (i = 0; i < MAX_NUM_OF_INF; i++) {
			if ((pAd != adapt_list[i]) && (adapt_list[i] != NULL)) {
				...
				if (pOpposAd->ApCfg.MBSSID[apIdx]
					    .wdev.if_up_down_state) {
					pWscControl =
						&pOpposAd->ApCfg.MBSSID[apIdx]
							 .wdev.WscControl;
					NdisCopyMemory(
						uuidHexStr,
...
#define FIXED_UUID
	...
	if (bUseCurrentTime) {
		...
		uuid_time =
			2860; /*xtime.tv_sec;	// Well, we fix this to make JumpStart  happy! */
	...
	clkSeq =
		(UINT16)(0x0601 /*jiffies*/ &
			 0xFFFF); /* Again, we fix this to make JumpStart happy! */
#ifdef RTMP_RBUS_SUPPORT
#ifdef FIXED_UUID
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	{
		...
		if (from_apcli)
			NdisMoveMemory(
				&uuid_t.node[0],
				&pAd->ApCfg.ApCliTab[apIdx].wdev.if_addr[0],
				sizeof(uuid_t.node));
		else
#endif /* APCLI_SUPPORT */
		{
			NdisMoveMemory(
				&uuid_t.node[0],
				&pAd->ApCfg.MBSSID[apIdx].wdev.if_addr[0],
				sizeof(uuid_t.node));
		}
	}
#endif /* CONFIG_AP_SUPPORT */
	...
	snprintf(uuidTmpStr, sizeof(uuidTmpStr),
		 "%08x-%04x-%04x-%02x%02x-%02x%02x%02x%02x%02x%02x",
		 (unsigned int)uuid_t.timeLow, uuid_t.timeMid,
		 uuid_t.timeHi_Version, uuid_t.clockSeqHi_Var,
		 uuid_t.clockSeqLow, uuid_t.node[0], uuid_t.node[1],
		 uuid_t.node[2], uuid_t.node[3], uuid_t.node[4],
		 uuid_t.node[5]);
...
VOID WscInitCommonTimers(IN PRTMP_ADAPTER pAdapter, IN PWSC_CTRL pWscControl)
{
	WSC_TIMER_INIT(pAdapter, pWscControl, &pWscControl->EapolTimer,
		       pWscControl->EapolTimerRunning, WscEAPOLTimeOutAction);
	WSC_TIMER_INIT(pAdapter, pWscControl, &pWscControl->Wsc2MinsTimer,
		       pWscControl->Wsc2MinsTimerRunning,
		       Wsc2MinsTimeOutAction);
	WSC_TIMER_INIT(pAdapter, pWscControl,
		       &pWscControl->WscUPnPNodeInfo.UPnPMsgTimer,
	...
	WSC_TIMER_INIT(pAdapter, pWscControl, &pWscControl->M2DTimer,
		       pWscControl->bM2DTimerRunning, WscM2DTimeOutAction);
#ifdef WSC_LED_SUPPORT
	WSC_TIMER_INIT(pAdapter, pWscControl, &pWscControl->WscLEDTimer,
		       pWscControl->WscLEDTimerRunning, WscLEDTimer);
	WSC_TIMER_INIT(pAdapter, pWscControl,
		       &pWscControl->WscSkipTurnOffLEDTimer,
		       pWscControl->WscSkipTurnOffLEDTimerRunning,
		       WscSkipTurnOffLEDTimer);
#endif /* WSC_LED_SUPPORT */
}

VOID WscInitClientTimers(IN PRTMP_ADAPTER pAdapter, IN PWSC_CTRL pWScControl)
{
	WSC_TIMER_INIT(pAdapter, pWScControl, &pWScControl->WscPBCTimer,
		       pWScControl->WscPBCTimerRunning, WscPBCTimeOutAction);
#ifdef WSC_STA_SUPPORT
	WSC_TIMER_INIT(pAdapter, pWScControl, &pWScControl->WscPINTimer,
		       pWScControl->WscPINTimerRunning, WscPINTimeOutAction);
#endif
	WSC_TIMER_INIT(pAdapter, pWScControl, &pWScControl->WscScanTimer,
		       pWScControl->WscScanTimerRunning, WscScanTimeOutAction);
	WSC_TIMER_INIT(
		pAdapter, pWScControl, &pWScControl->WscProfileRetryTimer,
		pWScControl->WscProfileRetryTimerRunning,
		WscProfileRetryTimeout); /* add by johnli, fix WPS test plan 5.1.1 */
#ifdef CON_WPS
	WSC_TIMER_INIT(pAdapter, pWScControl,
		       &pWScControl->ConWscApcliScanDoneCheckTimer,
		       pWScControl->ConWscApcliScanDoneCheckTimerRunning,
		       WscScanDoneCheckTimeOutAction);
#endif /*CON_WPS*/
}
...
*	==========================================================================
*/
VOID WscStateMachineInit(IN PRTMP_ADAPTER pAd, IN STATE_MACHINE *S,
			 OUT STATE_MACHINE_FUNC Trans[])
{
	PWSC_CTRL pWScControl;

	StateMachineInit(S, (STATE_MACHINE_FUNC *)Trans, MAX_WSC_STATE,
			 MAX_WSC_MSG, (STATE_MACHINE_FUNC)Drop, WSC_IDLE,
			 WSC_MACHINE_BASE);
	StateMachineSetAction(S, WSC_IDLE, WSC_EAPOL_START_MSG,
			      (STATE_MACHINE_FUNC)WscEAPOLStartAction);
	StateMachineSetAction(S, WSC_IDLE, WSC_EAPOL_PACKET_MSG,
			      (STATE_MACHINE_FUNC)WscEAPAction);
	StateMachineSetAction(S, WSC_IDLE, WSC_EAPOL_UPNP_MSG,
			      (STATE_MACHINE_FUNC)WscEAPAction);
#ifdef CONFIG_AP_SUPPORT
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	{
		...
		for (apidx = 0; apidx < MAX_MBSSID_NUM(pAd); apidx++) {
			...
			pWScControl->EntryIfIdx =
				(MIN_NET_DEVICE_FOR_MBSSID | apidx);
			WscInitCommonTimers(pAd, pWScControl);
			pWScControl->WscUpdatePortCfgTimerRunning = FALSE;
			WSC_TIMER_INIT(
				pAd, pWScControl,
				&pWScControl->WscUpdatePortCfgTimer,
				pWScControl->WscUpdatePortCfgTimerRunning,
				WscUpdatePortCfgTimeout);
#ifdef WSC_V2_SUPPORT
			WSC_TIMER_INIT(pAd, pWScControl,
				       &pWScControl->WscSetupLockTimer,
				       pWScControl->WscSetupLockTimerRunning,
				       WscSetupLockTimeout);
		...
		for (apidx = 0; apidx < MAX_APCLI_NUM; apidx++) {
			...
			pWScControl->EntryIfIdx =
				(MIN_NET_DEVICE_FOR_APCLI | apidx);
			WscInitCommonTimers(pAd, pWScControl);
...
#endif /* APCLI_SUPPORT */
#ifdef CON_WPS
		RTMPInitTimer(pAd, &pAd->ApCfg.ConWpsApCliBandMonitorTimer,
			      GET_TIMER_FUNCTION(ConWpsApCliMonitorTimeout),
			      pAd, FALSE);
			 ...
			 IN PVOID SystemSpecific2, IN PVOID SystemSpecific3)
{
	...
	*/
	...
	if (
#ifdef CONFIG_AP_SUPPORT
		(((pEntry == NULL) || (pWscNodeInfo->registrarID != 0)) &&
		 (CurOpMode == AP_MODE)) ||
#endif /* CONFIG_AP_SUPPORT */
		(0)) {
		MTWF_LOG(
	...
	if (pWscControl->M2DACKBalance != 0) {
		...
		if (CurOpMode == AP_MODE) {
			MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
				 ("%s(): send EAP-Fail to wireless Station!\n",
				  __func__));
...
#define WSC_UPNP_TIMER_PENDIND_WAIT 2000
		RTMPModTimer(&pWscNodeInfo->UPnPMsgTimer,
			     WSC_UPNP_TIMER_PENDIND_WAIT);
		MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
		...
		if (pWscData != NULL) {
			...
			WscSendUPnPMessage(pAd,
					   (pWscControl->EntryIfIdx & 0x0F),
					   WSC_OPCODE_UPNP_DATA,
					   WSC_UPNP_DATA_SUB_NORMAL, pWscData,
					   dataLen, 0, 0,
					   &pAd->CurrentAddress[0], AP_MODE);
		...
		RTMPSendWirelessEvent(pAd, IW_WSC_STATUS_FAIL, NULL,
				      (pWscControl->EntryIfIdx & 0x0F), 0);
	}
...
*	==========================================================================
*/
VOID WscEAPOLStartAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem)
{
...
#ifdef CON_WPS
	{
		...
		if (CurOpMode == AP_MODE) {
			if (pEntry->func_tb_idx < MAX_APCLI_NUM)
				...
				pApCliWpsCtrl = &pAd->ApCfg.ApCliTab[BSS0]
							 .wdev.WscControl;
		...
		if (pApCliWpsCtrl &&
		    (pApCliWpsCtrl->conWscStatus != CON_WPS_STATUS_DISABLED) &&
	...
	*/
	if (pWpsCtrl->WscPeerList.size != 0) {
		...
		if (NdisEqualMemory(pEntry->Addr, pWscPeer->mac_addr,
				    MAC_ADDR_LEN) == FALSE) {
			MTWF_LOG(
				DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
	...
	if ((pEntry->Receive_EapolStart_EapRspId == 0) ||
		...
		if (!pWpsCtrl->EapolTimerRunning) {
			...
			RTMPSetTimer(&pWpsCtrl->EapolTimer,
				     WSC_EAP_ID_TIME_OUT);
		}
	} else
		MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
...
#ifdef APCLI_SUPPORT
#ifdef CON_WPS
static VOID WscConWPSChecking(IN PRTMP_ADAPTER pAd, IN PRTMP_ADAPTER pOpposAd,
			      IN UCHAR if_idx, IN PWSC_CTRL pWscControl)
{
#ifdef MULTI_INF_SUPPORT
	if (pOpposAd) {
		if (pOpposAd->ApCfg.ConWpsApCliStatus == TRUE) {
			...
			if (pOpposAd->ApCfg.ConWpsMonitorTimerRunning) {
				RTMPCancelTimer(
					&pOpposAd->ApCfg
						 .ConWpsApCliBandMonitorTimer,
					&Cancelled);
				pOpposAd->ApCfg.ConWpsMonitorTimerRunning =
					FALSE;

				/* BandSelection */
				if (pAd->ApCfg.ConWpsApCliMode ==
				    CON_WPS_APCLI_BAND_2G)
					pActionAd =
						(PRTMP_ADAPTER)adapt_list[0];
				else
					pActionAd =
						(PRTMP_ADAPTER)adapt_list[1];

				pActionAd->ApCfg.ApCliTab[if_idx].Enable =
					FALSE;
			...
			ApCliIfDown(pAd);
			RTMPSetTimer(&pAd->ApCfg.ConWpsApCliBandMonitorTimer,
				     CON_WPS_APCLIENT_MONITOR_TIME);
...
*	==========================================================================
*/
VOID WscEAPAction(IN PRTMP_ADAPTER pAdapter, IN MLME_QUEUE_ELEM *Elem)
{
	UCHAR MsgType;
		...
		if (!bUPnPMsg) {
			if (pEntry) {
				if (IS_ENTRY_CLIENT(pEntry) &&
				    pEntry->func_tb_idx >=
					    pAdapter->ApCfg.BssidNum) {
					MTWF_LOG(
						DBG_CAT_SEC, CATSEC_WPS,
						DBG_LVL_TRACE,
						("WscEAPAction: Unknow apidex(=%d).\n",
						 pEntry->func_tb_idx));
					MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS,
						 DBG_LVL_TRACE,
						 ("<----- WscEAPAction\n"));
					return;
				}
	...
	if (pEntry && IS_ENTRY_CLIENT(pEntry)) {
		if ((MsgType == WSC_MSG_EAP_REG_RSP_ID) ||
			...
			if ((pEntry->Receive_EapolStart_EapRspId &
	...
	if (MsgType == WSC_MSG_EAP_REG_RSP_ID) {
		...
		if (bUPnPMsg) {
			...
			if ((pWscControl->WscMode == WSC_PBC_MODE)) {
				...
				*/
				current_band = HcGetBandByChannel(
					pAdapter, Elem->Channel);

				WscPBC_DPID_FromSTA(pAdapter,
						    pWscControl->EntryAddr,
						    current_band);
				WscPBCSessionOverlapCheck(pAdapter,
							  current_band);

				if ((pAdapter->CommonCfg.WscStaPbcProbeInfo
					     .WscPBCStaProbeCount[current_band] ==
				     1) &&
				    !NdisEqualMemory(
					    pAdapter->CommonCfg
						    .WscStaPbcProbeInfo
						    .StaMacAddr[current_band][0],
					    &ZERO_MAC_ADDR[0], MAC_ADDR_LEN) &&
				    (NdisEqualMemory(
					     pAdapter->CommonCfg
						     .WscStaPbcProbeInfo
						     .StaMacAddr[current_band]
								[0],
						...
						if ((current_band ==
						     HcGetBandByWdev(
							     &pAdapter->ApCfg
								      .MBSSID[bss_index]
								      .wdev)) &&
						    pAdapter->ApCfg.MBSSID[bss_index]
								    .wdev
								    .WscControl
								    .WscConfMode !=
							    WSC_DISABLE &&
			...
			if ((pWscControl->WscMode == WSC_PBC_MODE) &&
				...
				pWscControl->WscStatus =
					STATUS_WSC_PBC_SESSION_OVERLAP;
				RTMPSendWirelessEvent(
					pAdapter, IW_WSC_PBC_SESSION_OVERLAP,
					NULL, (pWscControl->EntryIfIdx & 0x0F),
					0);

				MTWF_LOG(
					DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
					("WscEAPAction: PBC Session Overlap!\n"));
#if defined(VENDOR_FEATURE6_SUPPORT) || defined(VENDOR_FEATURE7_SUPPORT)
				WscSendUPnPMessage(
					pAdapter,
					(pWscControl->EntryIfIdx & 0x0F),
					WSC_OPCODE_UPNP_CTRL,
					WSC_UPNP_DATA_SUB_PBC_OVERLAP,
					&pAdapter->ApCfg
...
#endif /* CONFIG_AP_SUPPORT */
				if ((pWscControl->WscConfMode &
				     WSC_PROXY_REGISTRAR) != 0) {
		...
		if (bUPnPMsg && (pWscControl->WscConfMode == WSC_ENROLLEE)) {
			...
			WscEapEnrolleeAction(pAdapter, Elem,
					     WSC_MSG_EAP_REQ_START, pEntry,
					     pWscControl);
		} else {
		...
		if (pWscControl->WscConfMode == WSC_ENROLLEE) {
			...
			if (!pWscControl->EapolTimerRunning) {
				...
				RTMPSetTimer(&pWscControl->EapolTimer,
					     WSC_EAP_ID_TIME_OUT);
			}
		} else
			MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
		...
		if (pWscControl->WscState >= WSC_STATE_WAIT_EAPFAIL) {
			...
			if ((CurOpMode == STA_MODE) &&
				...
				if (pWscControl->conWscStatus !=
				    CON_WPS_STATUS_DISABLED) {
					if (pAdapter->ApCfg.ConWpsApCliMode ==
					    CON_WPS_APCLI_BAND_AUTO) {
						WscConWpsStop(pAdapter, TRUE,
							      pWscControl);
						...
						nowBandIdx = multi_inf_get_idx(
							pAdapter);
						opposBandIdx = !nowBandIdx;
						pOpposAd = (PRTMP_ADAPTER)
							adapt_list[opposBandIdx];
#endif /* MULTI_INF_SUPPORT */
						pAdapter->ApCfg
							.ConWpsApCliStatus =
							TRUE;
						WscConWPSChecking(pAdapter,
								  pOpposAd,
								  if_idx,
								  pWscControl);
					}
				...
				if (pEntry && IS_ENTRY_APCLI(pEntry) &&
				    pAdapter->ApCfg.ApCliTab[if_idx].Enable ==
					    TRUE
#ifdef CON_WPS
				    && (pAdapter->ApCfg.ConWpsApCliMode ==
					CON_WPS_APCLI_BAND_AUTO)
#endif /* CON_WPS */
				) {
					pAdapter->ApCfg.ApCliTab[if_idx].Enable =
...
#endif
						pAdapter->ApCfg.ApCliTab[if_idx]
							.Enable = TRUE;
				}
			}
#endif /* APCLI_SUPPORT */
			...
			pWscControl->WscStatus = STATUS_WSC_FAIL;
			RTMPSendWirelessEvent(pAdapter, IW_WSC_STATUS_FAIL,
					      NULL,
					      (pWscControl->EntryIfIdx & 0x0F),
					      0);
			pWscControl->WscConfMode = WSC_DISABLE;
		...
		*/
		if ((pWscControl->WscConfMode & WSC_REGISTRAR) != 0) {
			...
			if (!bUPnPMsg &&
				...
				if (pEntry)
					WscEapRegistrarAction(pAdapter, Elem,
							      MsgType, pEntry,
							      pWscControl);
			}
		...
		if (((pWscControl->WscConfMode & WSC_PROXY) != 0) &&
		    (!bUPnPMsg) && (CurOpMode == AP_MODE)) {
			if ((pWscControl->bWscTrigger) &&
			    (pWscControl->WscState >= WSC_STATE_WAIT_M3))
				;
			else {
				pWscControl->WscActionMode = WSC_PROXY;
				WscEapApProxyAction(pAdapter, Elem, MsgType,
						    pEntry, pWscControl);
			}
		} else if ((!pWscControl->bWscTrigger) &&
			   ((pWscControl->WscConfMode & WSC_PROXY) == 0) &&
			   (pAdapter->OpMode == OPMODE_AP)) {
			MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
				 ("WscTrigger is FALSE, ignore EAP M1.\n"));
			goto out;
		}
#endif /* CONFIG_AP_SUPPORT */
	} else if (MsgType == WSC_MSG_M3 || MsgType == WSC_MSG_M5 ||
		   MsgType == WSC_MSG_M7 || MsgType == WSC_MSG_WSC_DONE) {
		BOOLEAN bNonceMatch =
			WscCheckNonce(pAdapter, Elem, TRUE, pWscControl);

		if (((pWscControl->WscConfMode & WSC_REGISTRAR) != 0) &&
		    (pWscControl->bWscTrigger) && bNonceMatch) {
...
#endif /* CONFIG_AP_SUPPORT */
	} else if (MsgType == WSC_MSG_M2 || MsgType == WSC_MSG_M2D ||
		   MsgType == WSC_MSG_M4 || MsgType == WSC_MSG_M6 ||
		   MsgType == WSC_MSG_M8) {
		BOOLEAN bNonceMatch =
		...
		if ((CurOpMode == AP_MODE) ||
		    ((CurOpMode == STA_MODE) && (pWscControl->bWscTrigger)))
			bGoWPS = TRUE;

#ifdef CONFIG_AP_SUPPORT
#ifdef WSC_V2_SUPPORT
		if ((CurOpMode == AP_MODE) &&
		    ((pWscControl->WscV2Info.bWpsEnable == FALSE) &&
		...
		if (((pWscControl->WscConfMode & WSC_ENROLLEE) != 0) &&
				...
				if (CurOpMode == AP_MODE) {
					if (pWscControl->bWscTrigger == FALSE) {
						...
						WscGetConfWithoutTrigger(
							pAdapter, pWscControl,
							FALSE);
					} else if (!(pWscControl->EntryIfIdx &
...
#endif /* CONFIG_AP_SUPPORT */
				if (bReadOwnPIN) {
					pWscControl->WscPinCodeLen =
						pWscControl
							->WscEnrolleePinCodeLen;
					WscGetRegDataPIN(
						pAdapter,
			...
			if (!bUPnPMsg &&
				...
				WscEapEnrolleeAction(pAdapter, Elem, MsgType,
						     pEntry, pWscControl);
		}
#ifdef CONFIG_AP_SUPPORT
		else if (((pWscControl->WscConfMode & WSC_PROXY) != 0) &&
...
#ifdef CONFIG_AP_SUPPORT
			else if (((pWscControl->WscConfMode & WSC_PROXY) !=
				  0) &&
				 (CurOpMode == AP_MODE)) {
				pWscControl->WscActionMode = WSC_PROXY;
				WscEapApProxyAction(pAdapter, Elem, MsgType,
		...
		if (bUPnPMsg) {
			...
			}
			{
				...
				os_alloc_mem(NULL, (UCHAR **)&pWscData,
					     WSC_MAX_DATA_LEN);

				if (pWscData != NULL) {
					memset(pWscData, 0, WSC_MAX_DATA_LEN);
					dataLen = BuildMessageNACK(pAdapter,
								   pWscControl,
								   pWscData);
					bUPnPStatus = WscSendUPnPMessage(
						pAdapter,
						(pWscControl->EntryIfIdx &
						 0x0F),
						WSC_OPCODE_UPNP_DATA,
						WSC_UPNP_DATA_SUB_NORMAL,
						pWscData, dataLen,
						Elem->TimeStamp.u.LowPart,
						Elem->TimeStamp.u.HighPart,
				...
				if (pWscUPnPNodeInfo->bUPnPMsgTimerRunning ==
				    TRUE) {
					RTMPCancelTimer(
						&pWscUPnPNodeInfo->UPnPMsgTimer,
						&Cancelled);
					pWscUPnPNodeInfo->bUPnPMsgTimerRunning =
						FALSE;
				}

				pWscUPnPNodeInfo->bUPnPInProgress = FALSE;
				RTMPSendWirelessEvent(
					pAdapter, IW_WSC_STATUS_FAIL, NULL,
					(pWscControl->EntryIfIdx & 0x0F), 0);
			}
		}
		if (!bUPnPMsg &&
			...
			*/
			if ((pWscControl->WscState >= WSC_STATE_WAIT_M5) &&
			    (config_error != WSC_ERROR_SETUP_LOCKED)) {
				pWscControl->WscRejectSamePinFromEnrollee =
					TRUE;
			...
			if ((pWscControl->WscState == WSC_STATE_WAIT_M8) &&
				...
				pWscControl->WscRejectSamePinFromEnrollee =
					FALSE;
				RTMPSendWirelessEvent(
					pAdapter, IW_WSC_STATUS_SUCCESS, NULL,
					(pWscControl->EntryIfIdx & 0x0F), 0);
			}
#ifdef CONFIG_AP_SUPPORT
				 ...
				 (pWscControl->WscConfStatus ==
				  WSC_SCSTATE_CONFIGURED) &&
				 (IS_CIPHER_WEP(pAdapter->ApCfg.MBSSID[apidx]
							.wdev.SecConfig
							.PairwiseCipher))) {
				bReSetWscIE = TRUE;
				pWscControl->WscStatus = STATUS_WSC_FAIL;
			...
			if ((CurOpMode == AP_MODE) && bReSetWscIE &&
				...
				struct wifi_dev *wdev =
					&pAdapter->ApCfg.MBSSID[apidx].wdev;

				WscBuildBeaconIE(
					pAdapter, pWscControl->WscConfStatus,
					FALSE, 0, 0,
					(pWscControl->EntryIfIdx & 0x0F), NULL,
					0, CurOpMode);
				WscBuildProbeRespIE(pAdapter,
						    ...
						    pWscControl->EntryIfIdx,
						    NULL, 0, CurOpMode);
				UpdateBeaconHandler(pAdapter, wdev,
						    BCN_UPDATE_IE_CHG);

				if (pWscControl->Wsc2MinsTimerRunning) {
					RTMPCancelTimer(
						&pWscControl->Wsc2MinsTimer,
						&Cancelled);
					pWscControl->Wsc2MinsTimerRunning =
						FALSE;
				}
		...
		RTMPSendWirelessEvent(pAdapter, IW_WSC_STATUS_FAIL, NULL, BSS0,
				      0);
		goto out;
	}
	...
	if (bUPnPMsg) {
		/* Messages from UPnP */
		if (pWscUPnPNodeInfo->bUPnPMsgTimerRunning)
			RTMPModTimer(&pWscUPnPNodeInfo->UPnPMsgTimer,
				     WSC_UPNP_MSG_TIME_OUT);
	} else {
		if ((pWscControl->EapMsgRunning == TRUE) &&
			...
			/* Messages from EAP */
			RTMPModTimer(&pWscControl->EapolTimer,
				     WSC_EAP_MSG_TIME_OUT);
			pWscControl->EapolTimerRunning = TRUE;
		}
	}
...
*	============================================================================
*/
VOID WscEapEnrolleeAction(IN PRTMP_ADAPTER pAdapter, IN MLME_QUEUE_ELEM *Elem,
			  ...
			  IN PWSC_CTRL pWscControl)
{
		...
		if ((MsgType == WSC_MSG_EAP_RSP_ID) && (CurOpMode == AP_MODE)) {
			...
			if (MsgType == WSC_MSG_M2 &&
				...
				if (CurOpMode == AP_MODE) {
					...
					tempEntry = MacTableLookup(
						pAdapter,
						&pWscControl->EntryAddr[0]);
					if (tempEntry) {
						if ((tempEntry->Receive_EapolStart_EapRspId &
						     WSC_ENTRY_GET_EAP_RSP_ID) ==
						    WSC_ENTRY_GET_EAP_RSP_ID)
							goto Done;
					}
				...
				RTMPSetTimer(&pWscUPnPInfo->UPnPMsgTimer,
					     WSC_UPNP_MSG_TIME_OUT);
...
#ifdef WSC_V2_SUPPORT
	MaxWscDataLen =
		MaxWscDataLen + (UINT)pWscControl->WscV2Info.ExtraTlv.TlvLen;
#endif /* WSC_V2_SUPPORT */
	...
	case WSC_MSG_EAP_REQ_START:
...

		{
			...
			*/
			IF_DEV_CONFIG_OPMODE_ON_AP(pAdapter)
			{
				if ((pWscControl->bWscAutoTriggerDisable ==
				     TRUE) &&
				    (pWscControl->bWscTrigger == FALSE)) {
					if (bUPnPMsg == TRUE) {
						MTWF_LOG(
			...
			if (pWscControl->RegData.ReComputePke == 1) {
				...
				NdisZeroMemory(
					pWscControl->RegData.Pke,
					sizeof(pWscControl->RegData.Pke));
				RT_DH_PublicKey_Generate(
					WPS_DH_G_VALUE, sizeof(WPS_DH_G_VALUE),
					WPS_DH_P_VALUE, sizeof(WPS_DH_P_VALUE),
					pWscControl->RegData.EnrolleeRandom,
					sizeof(pWscControl->RegData
						       .EnrolleeRandom),
					pWscControl->RegData.Pke,
					(UINT *)&DH_Len);
				...
				if ((DH_Len !=
					...
					DiffCnt = sizeof(pWscControl->RegData
								 .Pke) -
						  DH_Len;
					NdisFillMemory(&TempKey, DiffCnt, 0);
					NdisCopyMemory(&TempKey[DiffCnt],
						       pWscControl->RegData.Pke,
						       DH_Len);
					NdisCopyMemory(pWscControl->RegData.Pke,
						       TempKey,
						       sizeof(TempKey));
					DH_Len += DiffCnt;
		...
		RTMPSendWirelessEvent(pAdapter, IW_WSC_SEND_M1, NULL,
				      (pWscControl->EntryIfIdx & 0x0F), 0);
		break;
	...
	case WSC_MSG_M2:
		...
		if (pWscControl->WscState == WSC_STATE_SENT_M1 ||
			...
			NdisMoveMemory(pWscControl->RegData.PeerInfo.MacAddr,
				       pWscControl->EntryAddr, 6);
			rv = ProcessMessageM2(pAdapter, pWscControl, Elem->Msg,
					      Elem->MsgLen,
					      (pWscControl->EntryIfIdx & 0x0F),
					      &pWscControl->RegData);
			if (rv)
				...
				{
					OpCode |= WSC_OPCODE_MSG;
					DataLen = BuildMessageM3(
						pAdapter, pWscControl, WscData);
					pWscControl->WscStatus =
						STATUS_WSC_EAP_M3_SENT;
					/* Change the state to next one */
					pWscControl->WscState =
						WSC_STATE_WAIT_M4;
					RTMPSendWirelessEvent(
						pAdapter, IW_WSC_SEND_M3, NULL,
						(pWscControl->EntryIfIdx &
						 0x0F),
						0);
				}
	...
	case WSC_MSG_M2D:
		...
		if (pWscControl->WscState == WSC_STATE_SENT_M1 ||
			...
			rv = ProcessMessageM2D(pAdapter, Elem->Msg,
					       Elem->MsgLen,
					       &pWscControl->RegData);
			if (rv)
				goto Fail;
			...
			if (bReplyNack) {
				...
				DataLen = BuildMessageNACK(
					pAdapter, pWscControl, WscData);
				RTMPSendWirelessEvent(
					pAdapter, IW_WSC_SEND_NACK, NULL,
					(pWscControl->EntryIfIdx & 0x0F), 0);
			} else {
				...
				DataLen = BuildMessageACK(pAdapter, pWscControl,
							  WscData);
				RTMPSendWirelessEvent(
					pAdapter, IW_WSC_SEND_ACK, NULL,
					(pWscControl->EntryIfIdx & 0x0F), 0);
			}
	...
	case WSC_MSG_M4:
		...
		if (pWscControl->WscState == WSC_STATE_WAIT_M4) {
			...
			rv = ProcessMessageM4(pAdapter, pWscControl, Elem->Msg,
					      Elem->MsgLen,
					      &pWscControl->RegData);
			if ((rv)) {
#ifdef CONFIG_AP_SUPPORT
				...
				if (CurOpMode == AP_MODE)
					WscCheckPinAttackCount(pAdapter,
							       pWscControl);
	...
	case WSC_MSG_M6:
		...
		if (pWscControl->WscState == WSC_STATE_WAIT_M6) {
			...
			rv = ProcessMessageM6(pAdapter, pWscControl, Elem->Msg,
					      Elem->MsgLen,
					      &pWscControl->RegData);
			if (rv) {
#ifdef CONFIG_AP_SUPPORT
				...
				*/
				if (pEntry) {
					RTMP_SEM_LOCK(
						&pWscControl
							 ->WscPeerListSemLock);
					WscDelListEntryByMAC(
						&pWscControl->WscPeerList,
						pEntry->Addr);
					RTMP_SEM_UNLOCK(
						&pWscControl
							 ->WscPeerListSemLock);
	...
	case WSC_MSG_M8:
		...
		if (pWscControl->WscState == WSC_STATE_WAIT_M8) {
			...
			rv = ProcessMessageM8(pAdapter, Elem->Msg, Elem->MsgLen,
					      pWscControl);
			if (rv)
				goto Fail;
			else {
				OpCode |= WSC_OPCODE_DONE;
				DataLen = BuildMessageDONE(
					pAdapter, pWscControl, WscData);
#ifdef CONFIG_AP_SUPPORT
				...
				RTMPSendWirelessEvent(
					pAdapter, IW_WSC_SEND_DONE, NULL,
					(pWscControl->EntryIfIdx & 0x0F), 0);
#ifdef VENDOR_FEATURE7_SUPPORT
				{
					...
					*/
					ARRISMOD_CALL(arris_event_send_hook,
						      ATOM_HOST, WLAN_EVENT,
						      WPS_AP_CONFIGURED,
						      newApConfig,
						      strlen(newApConfig));
				}
	...
	if (OpCode > WSC_OPCODE_UPNP_MASK)
		bUPnPStatus = WscSendUPnPMessage(
			pAdapter, (pWscControl->EntryIfIdx & 0x0F),
			WSC_OPCODE_UPNP_DATA, WSC_UPNP_DATA_SUB_NORMAL, WscData,
			DataLen, Elem->TimeStamp.u.LowPart,
			Elem->TimeStamp.u.HighPart,
		...
		if (pWscControl->WscState != WSC_STATE_CONFIGURED) {
			...
			if (CurOpMode == AP_MODE) {
				if (pEntry && IS_ENTRY_APCLI(pEntry))
					WscSendMessage(pAdapter, OpCode,
						       WscData, DataLen,
						       pWscControl,
						       AP_CLIENT_MODE,
						       EAP_CODE_RSP);
...
Fail:
	...
	if (rv) {
		...
		else if ((rv == WSC_ERROR_HASH_FAIL) ||
			 (rv == WSC_ERROR_HMAC_FAIL))
			pWscControl->RegData.SelfInfo.ConfigError =
				WSC_ERROR_DECRYPT_CRC_FAIL;

		switch (rv) {
		case WSC_ERROR_DEV_PWD_AUTH_FAIL:
			pWscControl->WscStatus =
				STATUS_WSC_ERROR_DEV_PWD_AUTH_FAIL;
			break;
		...
		RTMPSendWirelessEvent(pAdapter, IW_WSC_STATUS_FAIL, NULL,
				      (pWscControl->EntryIfIdx & 0x0F), 0);
		if (bUPnPMsg) {
			if (pWscUPnPInfo->bUPnPMsgTimerRunning == TRUE) {
				RTMPCancelTimer(&pWscUPnPInfo->UPnPMsgTimer,
		...
		if (pEntry && IS_ENTRY_APCLI(pEntry)) {
			...
			MlmeEnqueue(pAdapter, APCLI_CTRL_STATE_MACHINE,
				    APCLI_CTRL_PEER_DISCONNECT_REQ, 0, NULL,
				    ifIndex);
		}
#endif
...
Done:
	...
	if (bUPnPMsg && (bUPnPStatus == FALSE))
		WscUPnPErrHandle(pAdapter, pWscControl,
				 Elem->TimeStamp.u.LowPart);
	...
	if (CurOpMode == AP_MODE) {
		if (((bUPnPMsg || (pEntry && IS_ENTRY_CLIENT(pEntry))) &&
		     (pWscControl->WscState == WSC_STATE_CONFIGURED ||
		      pWscControl->WscState == WSC_STATE_WAIT_ACK))
#ifdef APCLI_SUPPORT
		    || ((!bUPnPMsg && pEntry && IS_ENTRY_APCLI(pEntry)) &&
	...
	if (rv == 1) {
		...
		if ((pWscControl->WscConfStatus == WSC_SCSTATE_UNCONFIGURED)
			...
			pWscControl->WscMode = 1;
			RTMPSendWirelessEvent(pAdapter, IW_WSC_STATUS_SUCCESS,
					      NULL,
					      (pWscControl->EntryIfIdx & 0x0F),
					      0);
#ifdef CONFIG_AP_SUPPORT
			...
			if (CurOpMode == AP_MODE) {
				pWscControl->RegData.SelfInfo.ScState =
					pWscControl->WscConfStatus;
#ifdef APCLI_SUPPORT

				if (!bUPnPMsg && pEntry &&
				    IS_ENTRY_APCLI(pEntry)) {
					WscWriteConfToApCliCfg(
						pAdapter, pWscControl,
						&pWscControl->WscProfile
							 .Profile[0],
						TRUE);
#ifdef CONFIG_MAP_SUPPORT
					wsc_send_config_event_to_wapp(
						pAdapter, pWscControl,
						&pWscControl->WscProfile,
						pEntry);
#endif
				...
				if (bUPnPMsg ||
					...
					UCHAR apidx =
						pWscControl->EntryIfIdx & 0x0F;
					struct wifi_dev *wdev =
						&pAdapter->ApCfg.MBSSID[apidx]
							 .wdev;

					WscBuildBeaconIE(
						pAdapter,
						WSC_SCSTATE_CONFIGURED, FALSE,
						0, 0,
						(pWscControl->EntryIfIdx &
						 0x0F),
						NULL, 0, CurOpMode);
...
*	============================================================================
*/
VOID WscEapApProxyAction(IN PRTMP_ADAPTER pAdapter, IN MLME_QUEUE_ELEM *Elem,
			 IN UCHAR MsgType, IN MAC_TABLE_ENTRY *pEntry,
			 IN PWSC_CTRL pWscControl)
{
	PUCHAR WscData = NULL;
	...
	MTWF_LOG(
		DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
		("WscEapApProxyAction():pEntry=%p, ElemMsgType=%ld, MsgType=%d!\n",
	...
	if (Elem->MsgType == WSC_EAPOL_UPNP_MSG) {
		...
		case WSC_MSG_M8:
			MTWF_LOG(
				DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
				("WscEapApProxyAction: Rx WscMsg(%d) from UPnP, eventID=0x%x!\n",
				 MsgType, reqID));
			WscSendMessage(pAdapter, WSC_OPCODE_MSG, Elem->Msg,
				       Elem->MsgLen, pWscControl, AP_MODE,
				       EAP_CODE_REQ);
			...
			if (MsgType == WSC_MSG_M2) {
				...
				bUPnPStatus = WscSendUPnPMessage(
					...
					WSC_UPNP_MGMT_SUB_REG_SELECT,
					(PUCHAR)(&pWscUPnPInfo->registrarID),
					sizeof(UINT), 0, 0, NULL, AP_MODE);

				/*Reset the UPnP timer and status. */
				if (pWscControl->bM2DTimerRunning == TRUE) {
					RTMPCancelTimer(&pWscControl->M2DTimer,
							&Cancelled);
					pWscControl->bM2DTimerRunning = FALSE;
				}
			...
			if (MsgType == WSC_MSG_M8) {
				...
				WscBuildBeaconIE(
					pAdapter, WSC_SCSTATE_CONFIGURED, FALSE,
					0, 0, (pWscControl->EntryIfIdx & 0x0F),
					NULL, 0, AP_MODE);
				WscBuildProbeRespIE(pAdapter,
						    WSC_MSGTYPE_AP_WLAN_MGR,
						    WSC_SCSTATE_CONFIGURED,
		...
		case WSC_MSG_M2D:
			MTWF_LOG(
				DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
				("WscEapApProxyAction: Rx WscMsg M2D(%d) from UPnP, eventID=0x%x!\n",
				 MsgType, reqID));
			...
			if (reqID > 0)
				bUPnPStatus = WscSendUPnPMessage(
					...
					(pWscControl->EntryIfIdx & 0x0F),
					WSC_OPCODE_UPNP_DATA,
					WSC_UPNP_DATA_SUB_ACK, 0, 0, reqID, 0,
					NULL, AP_MODE);
			...
			if ((pWscUPnPInfo->registrarID == 0) &&
			    (pWscControl->bM2DTimerRunning == FALSE)) {
				/* Add M2D timer used to trigger the EAPFail Packet! */
				RTMPSetTimer(&pWscControl->M2DTimer,
					     WSC_UPNP_M2D_TIME_OUT);
		...
		default:
			MTWF_LOG(
				DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
				("Recv WscMsg(%d) from UPnP, request EventID=%d! drop it!\n",
				 MsgType, reqID));
			break;
		}
	} else {
		...
		case WSC_MSG_M7:
			MTWF_LOG(
				DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
				("WscEapApProxyAction: Rx WscMsg(%d) from EAP\n",
				 MsgType));
			...
			if (MsgType == WSC_MSG_M1) {
				bUPnPStatus = WscSendUPnPMessage(
					...
					(pWscControl->EntryIfIdx & 0x0F),
					WSC_OPCODE_UPNP_DATA,
					WSC_UPNP_DATA_SUB_TO_ALL, Elem->Msg,
					Elem->MsgLen, 0, 0,
		...
		case WSC_MSG_WSC_ACK:
			MTWF_LOG(
				DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
				("WscEapApProxyAction: Rx WSC_ACK from EAP\n"));
		...
		case WSC_MSG_WSC_DONE:
			MTWF_LOG(
				DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
				("WscEapApProxyAction: Rx WSC_DONE from EAP\n"));
			MTWF_LOG(
				DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
				("WscEapApProxyAction: send WSC_DONE to UPnP Registrar!\n"));
		...
		default:
			MTWF_LOG(
				DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
				("Recv WSC Msg(%d) from EAP , it's impossible, drop it!\n",
				 MsgType));
			break;
		}
	}
...
Fail:
	...
	if (sendToUPnP && (bUPnPStatus == FALSE)) {
		MTWF_LOG(
			DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
			("Need to send UPnP but bUPnPStatus is false!MsgType=%d, regID=0x%x!\n",
			 MsgType, reqID));
...
*	============================================================================
*/
VOID WscEapRegistrarAction(IN PRTMP_ADAPTER pAdapter, IN MLME_QUEUE_ELEM *Elem,
			   IN UCHAR MsgType, IN MAC_TABLE_ENTRY *pEntry,
			   IN PWSC_CTRL pWscControl)
{
	INT DataLen = 0, rv = 0;
	...
	case WSC_MSG_M1:
		...
		if (pWscControl->WscState == WSC_STATE_WAIT_M1) {
			OpCode |= WSC_OPCODE_MSG;
			/* Process M1 */
			rv = ProcessMessageM1(pAdapter, pWscControl, Elem->Msg,
					      Elem->MsgLen,
					      &pWscControl->RegData);
			if (rv)
				goto Fail;
				...
				if (IS_MAP_ENABLE(pAdapter))
					pEntry->DevPeerRole =
						pWscControl->RegData.PeerInfo
							.map_DevPeerRole;
#endif /* CONFIG_MAP_SUPPORT */

				if (pWscControl->bWscTrigger &&
				    (!pWscControl->bWscAutoTigeer)) {
					if (((pWscControl->WscMode ==
					      WSC_PBC_MODE) ||
					     (pWscControl->WscMode ==
					      WSC_SMPBC_MODE)) ||
					    (pWscControl->WscMode ==
				...
				if (bSendM2D) {
					...
					if (pWscControl->bM2DTimerRunning ==
						...
						/* Add M2D timer used to trigger the EAPFail Packet! */
						RTMPSetTimer(
							&pWscControl->M2DTimer,
							WSC_UPNP_M2D_TIME_OUT);
						pWscControl->bM2DTimerRunning =
							TRUE;
					}
				} else {
					pWscControl->WscStatus =
						STATUS_WSC_EAP_M2_SENT;
					DataLen = BuildMessageM2(
						pAdapter, pWscControl, WscData);
					/* Change the state to next one */
					pWscControl->WscState =
						WSC_STATE_WAIT_M3;
	...
	case WSC_MSG_M3:
		...
		if (pWscControl->WscState == WSC_STATE_WAIT_M3) {
			...
			rv = ProcessMessageM3(pAdapter, Elem->Msg, Elem->MsgLen,
					      &pWscControl->RegData);
			if (rv)
				goto Fail;
			else {
				OpCode |= WSC_OPCODE_MSG;
				DataLen = BuildMessageM4(pAdapter, pWscControl,
							 WscData);
				pWscControl->WscStatus = STATUS_WSC_EAP_M4_SENT;
				/* Change the state to next one */
				pWscControl->WscState = WSC_STATE_WAIT_M5;
				RTMPSendWirelessEvent(
					pAdapter, IW_WSC_SEND_M4, NULL,
					(pWscControl->EntryIfIdx & 0x0F), 0);
			}
		}
	...
	case WSC_MSG_M5:
		...
		if (pWscControl->WscState == WSC_STATE_WAIT_M5) {
			...
			rv = ProcessMessageM5(pAdapter, pWscControl, Elem->Msg,
					      Elem->MsgLen,
					      &pWscControl->RegData);
			if (rv)
				goto Fail;
	...
	case WSC_MSG_M7:
		...
		if (pWscControl->WscState == WSC_STATE_WAIT_M7) {
			...
			rv = ProcessMessageM7(pAdapter, pWscControl, Elem->Msg,
					      Elem->MsgLen,
					      &pWscControl->RegData);
			if (rv)
				goto Fail;
				...
				if ((CurOpMode == AP_MODE)) {
					OpCode |= WSC_OPCODE_MSG;
					DataLen = BuildMessageM8(
						pAdapter, pWscControl, WscData);
					pWscControl->WscStatus =
						STATUS_WSC_EAP_M8_SENT;
					/* Change the state to next one */
					pWscControl->WscState =
						WSC_STATE_WAIT_DONE;
#if defined(VENDOR_FEATURE6_SUPPORT) || defined(VENDOR_FEATURE7_SUPPORT)
					...
					if (pWscControl->WscV2Info.bEnableWpsV2 &&
					    (CurOpMode == AP_MODE)) {
						WscAddEntryToAclList(
							pAdapter,
							pEntry->func_tb_idx,
							pEntry->Addr);
						WscDeleteEntryFromAclList(
							pAdapter,
							pEntry->func_tb_idx,
							pEntry->Addr);
					}
	...
	case WSC_MSG_WSC_DONE:
		...
		if (pWscControl->WscState == WSC_STATE_WAIT_DONE) {
			...
			if (CurOpMode == AP_MODE) {
				pWscControl->WscStatus =
					STATUS_WSC_EAP_RAP_RSP_DONE_SENT;
				/* Send EAP-Fail */
				WscSendEapFail(pAdapter, pWscControl, TRUE);
				pWscControl->WscStatus = STATUS_WSC_CONFIGURED;
#ifdef CON_WPS
...
#ifdef VENDOR_FEATURE7_SUPPORT
			{
				...
				if (WMODE_CAP_5G(
					    pAdapter->ApCfg
						    .MBSSID[pWscControl
							    ...
							    0x0F]
						    .wdev.PhyMode))
					sprintf((char *)wsc_done_event_msg,
						"WSC Done for Bss(%d)",
						(pWscControl->EntryIfIdx &
						 0x0F) + WIFI_50_RADIO);
				else
				...
				ARRISMOD_CALL(arris_event_send_hook, ATOM_HOST,
					      WLAN_EVENT, WPS_AP_CONFIGURED,
					      wsc_done_event_msg,
					      strlen(wsc_done_event_msg));
			}
#endif
			pWscControl->EapMsgRunning = FALSE;
		}
	...
	if (OpCode > WSC_OPCODE_UPNP_MASK)
		bUPnPStatus = WscSendUPnPMessage(
			...
			WSC_OPCODE_UPNP_DATA, WSC_UPNP_DATA_SUB_NORMAL, WscData,
			DataLen, Elem->TimeStamp.u.LowPart,
			Elem->TimeStamp.u.HighPart, &pWscControl->EntryAddr[0],
			CurOpMode);
	else if (OpCode > 0 && OpCode < WSC_OPCODE_UPNP_MASK) {
#ifdef WSC_V2_SUPPORT
		pWscControl->WscTxBufLen = 0;
		...
		if (pWscControl->bWscFragment &&
			...
			pWscControl->pWscCurBufIdx = (pWscControl->pWscTxBuf +
						      pWscControl->WscFragSize);
		}
		...
		if (CurOpMode == AP_MODE) {
			if (pWscControl->WscState != WSC_STATE_CONFIGURED)
				WscSendMessage(pAdapter, OpCode, WscData,
					       DataLen, pWscControl, AP_MODE,
					       EAP_CODE_REQ);
		}
...
Fail:
	...
	if (rv) {
		...
		if (bUPnPMsg) {
			...
			DataLen = BuildMessageNACK(pAdapter, pWscControl,
						   WscData);
#ifdef CONFIG_AP_SUPPORT

			if (CurOpMode == AP_MODE) {
				WscSendMessage(pAdapter, WSC_OPCODE_NACK,
					       WscData, DataLen, pWscControl,
...
/* WPS_BandSteering Support */
		...
		if (pAdapter->ApCfg.BandSteering) {
			...
			if (wdev) {
				PBND_STRG_CLI_TABLE table = Get_BndStrgTable(
					pAdapter, wdev->func_idx);

				if (table && table->bEnabled) {
					NdisAcquireSpinLock(
						&table->WpsWhiteListLock);
		...
		NdisZeroMemory(&pAdapter->CommonCfg.WscStaPbcProbeInfo,
...
VOID WscTimeOutProcess(IN PRTMP_ADAPTER pAd, IN PMAC_TABLE_ENTRY pEntry,
		       IN INT nWscState, IN PWSC_CTRL pWscControl)
{
	INT WscMode;
	UCHAR CurOpMode = 0xFF;
#ifdef CONFIG_AP_SUPPORT
	...
	if (CurOpMode == AP_MODE) {
		if ((pWscControl->WscConfStatus == WSC_SCSTATE_UNCONFIGURED) &&
		    ((nWscState == WSC_STATE_WAIT_DONE) ||
		     (nWscState == WSC_STATE_WAIT_ACK))) {
			pWscControl->bWscTrigger = FALSE;
			pWscControl->WscConfStatus = WSC_SCSTATE_CONFIGURED;
			WscBuildBeaconIE(pAd, pWscControl->WscConfStatus, FALSE,
					 WscMode, pWscControl->WscConfigMethods,
					 (pWscControl->EntryIfIdx & 0x0F), NULL,
					 0, CurOpMode);
			WscBuildProbeRespIE(pAd, WSC_MSGTYPE_AP_WLAN_MGR,
					    pWscControl->WscConfStatus, FALSE,
					    WscMode,
					    pWscControl->WscConfigMethods,
					    pWscControl->EntryIfIdx, NULL, 0,
					    CurOpMode);
			pAd->WriteWscCfgToDatFile = pWscControl->EntryIfIdx;
			WscWriteConfToPortCfg(
				pAd, pWscControl,
				&pWscControl->WscProfile.Profile[0], FALSE);
			{
				UCHAR apidx = pWscControl->EntryIfIdx & 0x0F;
				/*
				...
				*	@20160321
				*/
				RTEnqueueInternalCmd(pAd, CMDTHREAD_AP_RESTART,
						     (VOID *)&apidx,
						     sizeof(UCHAR));
			}
			   ...
			   IN PVOID SystemSpecific2, IN PVOID SystemSpecific3)
{
	...
	ASSERT(wdev);
	wdev->wdev_ops->mac_entry_lookup(pAd, pWscControl->EntryAddr, wdev,
					 &pEntry);
	...
	if (CurOpMode == AP_MODE) {
		if (pEntry && IS_ENTRY_CLIENT(pEntry) &&
			...
			WscTimeOutProcess(pWscControl->pAd, pEntry,
					  WSC_STATE_WAIT_RESP_ID, pWscControl);
	...
	case WSC_STATE_WAIT_REQ_ID:
		...
		if ((pWscControl->WscRetryCount >= 2))
			WscTimeOutProcess(pWscControl->pAd, pEntry,
					  WSC_STATE_WAIT_REQ_ID, pWscControl);
		else {
	...
	case WSC_STATE_WAIT_M1:
		if (pWscControl->WscRetryCount >= 2)
			WscTimeOutProcess(pWscControl->pAd, pEntry,
					  WSC_STATE_WAIT_M1, pWscControl);
		else {
#ifdef CONFIG_AP_SUPPORT

			if (CurOpMode == AP_MODE)
				WscSendMessage(pWscControl->pAd,
					       WSC_OPCODE_START, NULL, 0,
					       pWscControl, AP_MODE,
					       EAP_CODE_REQ);
	...
	case WSC_STATE_WAIT_PIN:
		if (pWscControl->WscRetryCount >= 2)
			WscTimeOutProcess(pWscControl->pAd, pEntry,
					  WSC_STATE_WAIT_PIN, pWscControl);
		else {
			pWscControl->WscRetryCount++;
			MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
	...
	case WSC_STATE_WAIT_DONE:
		if (pWscControl->WscRetryCount >= 2)
			WscTimeOutProcess(pWscControl->pAd, pEntry,
					  WSC_STATE_WAIT_DONE, pWscControl);
		else {
			if (pWscControl->WscActionMode == WSC_REGISTRAR) {
#ifdef CONFIG_AP_SUPPORT
				if (CurOpMode == AP_MODE)
	...
	case WSC_STATE_WAIT_ACK:
		WscTimeOutProcess(pWscControl->pAd, pEntry, WSC_STATE_WAIT_ACK,
				  pWscControl);
		break;
#endif /* CONFIG_AP_SUPPORT */
	...
	case WSC_STATE_WAIT_EAPFAIL:
		...
		if (pWscControl->WscRetryCount >= 1)
			WscTimeOutProcess(pWscControl->pAd, pEntry,
					  WSC_STATE_WAIT_EAPFAIL, pWscControl);
		else {
			RTMPModTimer(&pWscControl->EapolTimer,
				     WSC_EAP_EAP_FAIL_TIME_OUT);
			pWscControl->WscRetryCount++;
		}
		break;
			   ...
			   IN PVOID SystemSpecific2, IN PVOID SystemSpecific3)
{
	...
	if (pWscControl != NULL) {
		...
		if (pWscControl->conWscStatus != CON_WPS_STATUS_DISABLED) {
			MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_OFF,
				 ("CON_WPS: Reset the status to default.\n"));
			pWscControl->conWscStatus = CON_WPS_STATUS_DISABLED;
		}
		...
		if (pWscControl->Wsc2MinsTimerRunning) {
			...
			if (CurOpMode == AP_MODE) {
				...
				if ((pWscControl->EntryIfIdx & 0x0F) <
				    pAd->ApCfg.BssidNum) {
					UCHAR apidx =
						pWscControl->EntryIfIdx & 0x0F;
					struct wifi_dev *wdev =
...
/* WPS_BandSteering Support */
					...
					WscBuildProbeRespIE(
						...
						WSC_MSGTYPE_AP_WLAN_MGR,
						IsAPConfigured, FALSE, 0, 0,
						pWscControl->EntryIfIdx, NULL,
						0, CurOpMode);
					UpdateBeaconHandler(pWscControl->pAd,
							    wdev,
							    BCN_UPDATE_IE_CHG);
				}

				if ((pWscControl->WscConfMode & WSC_PROXY) ==
				    0) {
			...
			pWscControl->WscStatus = STATUS_WSC_IDLE;
			RTMPSendWirelessEvent(pAd, IW_WSC_2MINS_TIMEOUT, NULL,
					      (pWscControl->EntryIfIdx & 0x0F),
					      0);
			...
			WscSendUPnPMessage(
				...
				WSC_OPCODE_UPNP_CTRL,
				WSC_UPNP_DATA_SUB_WSC_TIMEOUT,
				&pAd->ApCfg
					 .MBSSID[pWscControl->EntryIfIdx & 0x0F]
					 .wdev.bssid[0],
				MAC_ADDR_LEN, 0, 0,
				&pAd->ApCfg
					 .MBSSID[pWscControl->EntryIfIdx & 0x0F]
			...
			if (pWscControl->WscScanTimerRunning) {
				...
				RTMPCancelTimer(&pWscControl->WscScanTimer,
						&Cancelled);
			}

			if (pWscControl->WscPBCTimerRunning) {
				pWscControl->WscPBCTimerRunning = FALSE;
				RTMPCancelTimer(&pWscControl->WscPBCTimer,
						&Cancelled);
			}
		}
		...
		if ((pWscControl->WscState == WSC_STATE_OFF) &&
		    (pWscControl->WscStatus == STATUS_WSC_CONFIGURED))
#else
		if ((STA_STATUS_TEST_FLAG(pStaCfg,
					  fSTA_STATUS_MEDIA_STATE_CONNECTED)) &&
		    (pWscControl->WscState == WSC_STATE_OFF) &&
		    (pWscControl->WscStatus == STATUS_WSC_CONFIGURED))
#endif
			MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
				 ("WscConnectTimeout --> Connection OK\n"));
		else {
			UCHAR WPSLEDStatus;
			...
			if ((LED_MODE(pAd) == WPS_LED_MODE_7) ||
				...
				RTMPSetLED(pAd, WPSLEDStatus);
				    ...
				    WPS_LED_MODE_9) /* WPS LED mode 9. */
			) {
				if (pWscControl->WscMode ==
				    WSC_PIN_MODE) { /* PIN method. */
					...
					RTMPSetTimer(
						...
						WSC_WPS_FAIL_LED_PATTERN_TIMEOUT);
					...
					RTMPSetTimer(
						&pWscControl
							 ->WscSkipTurnOffLEDTimer,
						WSC_WPS_SKIP_TURN_OFF_LED_TIMEOUT);
					MTWF_LOG(
						DBG_CAT_SEC, CATSEC_WPS,
						DBG_LVL_TRACE,
						("%s: The NIC using PIN method fails to finish the WPS handshaking within 120 seconds.\n",
						 __func__));
				} else if (pWscControl->WscMode ==
					   WSC_PBC_MODE) { /* PBC method. */
					switch (pWscControl
							->WscLastWarningLEDMode) { /* Based on last WPS warning LED mode. */
					case 0:
					case LED_WPS_ERROR:
					case LED_WPS_SESSION_OVERLAP_DETECTED:
						...
						/* The Ralink UI would make RT_OID_DISCONNECT_REQUEST request while it receive STATUS_WSC_EAP_FAILED. */
						/* Allow the NIC to turn off the WPS LED after WSC_WPS_SKIP_TURN_OFF_LED_TIMEOUT seconds. */
						pWscControl->bSkipWPSTurnOffLED =
							TRUE;
						RTMPSetTimer(
							&pWscControl
								 ->WscSkipTurnOffLEDTimer,
						...
						MTWF_LOG(
							DBG_CAT_SEC, CATSEC_WPS,
							DBG_LVL_TRACE,
							("%s: Last WPS LED status is LED_WPS_ERROR.\n",
							 __func__));
						break;
			...
			MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
				 ("WscConnectTimeout --> Fail to connect\n"));
		}
...
*	========================================================================
*/
UCHAR WscRxMsgType(IN PRTMP_ADAPTER pAdapter, IN PMLME_QUEUE_ELEM pElem)
{
	USHORT Length;
	PUCHAR pData;
	...
	if (pElem->Msg[0] == 'W' && pElem->Msg[1] == 'F' &&
	    pElem->Msg[2] == 'A') {
		/* Eap-Rsp(Identity) */
		if (memcmp(regIdentity, pElem->Msg, strlen(regIdentity)) == 0)
			return WSC_MSG_EAP_REG_RSP_ID;
		else if (memcmp(enrIdentity, pElem->Msg, strlen(enrIdentity)) ==
			 0)
...
VOID WscInitRegistrarPair(RTMP_ADAPTER *pAd, WSC_CTRL *pWscControl, UCHAR apidx)
{
	...
	NdisMoveMemory(&pWscControl->RegData.SelfInfo.Uuid[0],
	...
	if (CurOpMode == AP_MODE) {
		...
		if (apidx < HW_BEACON_MAX_NUM) {
			NdisMoveMemory(pWscControl->RegData.SelfInfo.MacAddr,
				       pAd->ApCfg.MBSSID[apidx].wdev.bssid, 6);
	...
	if (CurOpMode == AP_MODE) {
		if (!RTMP_TEST_FLAG(pWscControl, 0x04)) {
			...
			if (pWscControl->WscConfStatus ==
				...
				NdisMoveMemory(&pWscControl->RegData.SelfInfo
							.DeviceName,
					       AP_WSC_DEVICE_NAME_OOB,
					       sizeof(AP_WSC_DEVICE_NAME_OOB));
			/* ARRIS MODIFY */
			else if (WMODE_CAP_5G(wdev->PhyMode))
				NdisMoveMemory(&pWscControl->RegData.SelfInfo
							.DeviceName,
					       AP_WSC_DEVICE_NAME_5G,
					       sizeof(AP_WSC_DEVICE_NAME_5G));
			/* ARRIS MODIFY */
			else if (WMODE_CAP_2G(wdev->PhyMode))
				NdisMoveMemory(&pWscControl->RegData.SelfInfo
							.DeviceName,
					       AP_WSC_DEVICE_NAME_2G,
					       sizeof(AP_WSC_DEVICE_NAME_2G));
				/* ARRIS MODIFY */
#else
			NdisZeroMemory(
				&pWscControl->RegData.SelfInfo.DeviceName[0],
				sizeof(pWscControl->RegData.SelfInfo
					       .DeviceName));
			snprintf(
				&pWscControl->RegData.SelfInfo.DeviceName[0],
				sizeof(pWscControl->RegData.SelfInfo.DeviceName),
				"%s_%d", AP_WSC_DEVICE_NAME, apidx);
#endif
		}
	}
	...
	if (!RTMP_TEST_FLAG(pWscControl, 0x08))
		NdisMoveMemory(&pWscControl->RegData.SelfInfo.ModelNumber,
			       WSC_MODEL_NUMBER, sizeof(WSC_MODEL_NUMBER));

	/* 8. Serial Number */
	if (!RTMP_TEST_FLAG(pWscControl, 0x10))
		NdisMoveMemory(&pWscControl->RegData.SelfInfo.SerialNumber,
			       WSC_MODEL_SERIAL, sizeof(WSC_MODEL_SERIAL));
	...
	/* 12. Associate state */
	pWscControl->RegData.SelfInfo.AssocState =
		cpu2be16(0x0000); /* Not associated */
	/* 13. Configure Error */
	pWscControl->RegData.SelfInfo.ConfigError =
		cpu2be16(0x0000); /* No error */
	/* 14. OS Version */
	pWscControl->RegData.SelfInfo.OsVersion =
		cpu2be32(0x80000000); /* first bit must be 1 */
	/* 15. RF Band */
	/* Some WPS AP would check RfBand value in M1, ex. D-Link DIR-628 */
	pWscControl->RegData.SelfInfo.RfBand = 0x00;

	if (WMODE_CAP_5G(wdev->PhyMode))
		pWscControl->RegData.SelfInfo.RfBand |=
			WSC_RFBAND_50GHZ; /* 5.0G */

	if (WMODE_CAP_2G(wdev->PhyMode))
		pWscControl->RegData.SelfInfo.RfBand |=
			WSC_RFBAND_24GHZ; /* 2.4G */

	/* 16. Config Method */
	pWscControl->RegData.SelfInfo.ConfigMethods =
		cpu2be16(pWscControl->WscConfigMethods);
	...
	if (pWscControl->WscMode == WSC_PBC_MODE)
		pWscControl->RegData.SelfInfo.DevPwdId =
			cpu2be16(DEV_PASS_ID_PBC); /* PBC */
	else {
		/* Let PIN be default DPID.  */
		pWscControl->RegData.SelfInfo.DevPwdId =
			cpu2be16(DEV_PASS_ID_PIN); /* PIN mode */
	}
	...
	if (CurOpMode == AP_MODE) {
		if (apidx < HW_BEACON_MAX_NUM)
			NdisMoveMemory(pWscControl->RegData.SelfInfo.Ssid,
	...
	if (CurOpMode == AP_MODE) {
		if (pWscControl->EntryIfIdx & MIN_NET_DEVICE_FOR_APCLI)
			NdisMoveMemory(
				&pWscControl->RegData.SelfInfo.PriDeviceType,
				&STA_Wsc_Pri_Dev_Type[0], 8);
		else
			NdisMoveMemory(
				&pWscControl->RegData.SelfInfo.PriDeviceType,
				&AP_Wsc_Pri_Dev_Type[0], 8);
	}
...
VOID WscSendEapReqId(IN PRTMP_ADAPTER pAd, IN PMAC_TABLE_ENTRY pEntry,
		     IN UCHAR CurOpMode)
{
	...
	if (CurOpMode == AP_MODE) {
		if (pEntry->func_tb_idx < HW_BEACON_MAX_NUM) {
			...
			MAKE_802_3_HEADER(Header802_3, &pEntry->Addr[0],
					  &pAd->ApCfg
						   .MBSSID[pEntry->func_tb_idx]
						   .wdev.bssid[0],
					  EAPOL);
	...
	MakeOutgoingFrame(pOutBuffer, &FrameLen, sizeof(IEEE8021X_FRAME),
			  &Ieee_8021x, sizeof(EapFrame), &EapFrame,
			  (sizeof(Data) - 1), Data, END_OF_ARGS);
	/* Copy frame to Tx ring */
	RTMPToWirelessSta(pAd, pEntry, Header802_3, sizeof(Header802_3),
			  (PUCHAR)pOutBuffer, FrameLen, TRUE);
...
*	========================================================================
*/
VOID WscSendEapolStart(IN PRTMP_ADAPTER pAdapter, IN PUCHAR pBssid,
		       IN UCHAR CurOpMode, IN VOID *wdev_obj)
{
	...
	if (pEntry)
		RTMPToWirelessSta(pAdapter, pEntry, Header802_3,
				  sizeof(Header802_3), (PUCHAR)&Packet, 4,
				  TRUE);
...
VOID WscSendEapRspId(IN PRTMP_ADAPTER pAdapter, IN PMAC_TABLE_ENTRY pEntry,
		     IN PWSC_CTRL pWscControl)
{
	UCHAR Header802_3[14];
	USHORT Length = 0;
	...
	MAKE_802_3_HEADER(Header802_3, &pEntry->Addr[0], &wdev->if_addr[0],
			  EAPOL);
	...
	/* Copy frame to Tx ring */
	RTMPToWirelessSta((PRTMP_ADAPTER)pWscControl->pAd, pEntry, Header802_3,
			  LENGTH_802_3, (PUCHAR)pOutBuffer, FrameLen, TRUE);
	pWscControl->WscRetryCount = 0;
...
VOID WscUPnPErrHandle(IN PRTMP_ADAPTER pAd, IN PWSC_CTRL pWscControl,
		      IN UINT eventID)
{
	int dataLen;
	UCHAR *pWscData;
	...
	if (pWscData != NULL) {
		...
		WscSendUPnPMessage(pAd, (pWscControl->EntryIfIdx & 0x0F),
				   ...
				   eventID, 0, NULL, CurOpMode);
		os_free_mem(pWscData);
	} else {
		WscSendUPnPMessage(pAd, (pWscControl->EntryIfIdx & 0x0F),
				   WSC_OPCODE_UPNP_CTRL, 0, NULL, 0, eventID, 0,
				   NULL, CurOpMode);
	}
}
...
*/
int WscSendUPnPConfReqMsg(IN PRTMP_ADAPTER pAd, IN UCHAR apIdx,
...
#define WSC_JOIN_MSG_LEN (MAX_LEN_OF_SSID + MAC_ADDR_LEN + 1)
	...
	WscSendUPnPMessage(pAd, apIdx, WSC_OPCODE_UPNP_MGMT,
			   WSC_UPNP_MGMT_SUB_CONFIG_REQ, &pData[0],
			   WSC_JOIN_MSG_LEN, eventID, 0, NULL, CurOpMode);
	return 0;
}
...
*
*/
int WscSendUPnPMessage(IN PRTMP_ADAPTER pAd, IN UCHAR devIfIdx,
		       IN USHORT msgType, IN USHORT msgSubType, IN PUCHAR pData,
		       IN INT dataLen, IN UINT eventID, IN UINT toIPAddr,
		       IN PUCHAR pMACAddr, IN UCHAR CurOpMode)
{
	...
	if (CurOpMode == AP_MODE) {
		...
		if (devIfIdx & MIN_NET_DEVICE_FOR_APCLI) {
			...
			if (bssIdx < MAX_APCLI_NUM)
				pDevAddr = &pAd->ApCfg.ApCliTab[bssIdx]
						    .wdev.if_addr[0];
		} else
#endif /* APCLI_SUPPORT */
	...
	*/
	...
	if ((pWscMsgHdr->msgType == WSC_OPCODE_UPNP_DATA) && (eventID == 0) &&
	...
	if (pBuf != NULL) {
		...
		while (leftLen) {
			...
			pNLMsgHdr->segLen =
				(leftLen > IWEVCUSTOM_PAYLOD_MAX_LEN ?
					 IWEVCUSTOM_PAYLOD_MAX_LEN :
					       leftLen);
			...
			if (firstSeg) {
				memcpy(pBufPtr, pWscMsgHdr,
				       RTMP_WSC_MSG_HDR_LEN);
				pBufPtr += RTMP_WSC_MSG_HDR_LEN;
				copyLen = (pNLMsgHdr->segLen -
					   RTMP_WSC_MSG_HDR_LEN);

				if ((pWscMsgHdr->msgSubType &
				     WSC_UPNP_DATA_SUB_INCLUDE_MAC) ==
				    WSC_UPNP_DATA_SUB_INCLUDE_MAC) {
					NdisMoveMemory(pBufPtr, pMACAddr,
						       MAC_ADDR_LEN);
			...
			RtmpOSWrielessEventSend(
				pAd->net_dev, RT_WLAN_EVENT_CUSTOM,
				RT_WSC_UPNP_EVENT_FLAG, NULL, pBuf,
				pNLMsgHdr->segLen + sizeof(RTMP_WSC_NLMSG_HDR));
		}
...
VOID WscSendMessage(IN PRTMP_ADAPTER pAdapter, IN UCHAR OpCode, IN PUCHAR pData,
		    IN INT Len, IN PWSC_CTRL pWscControl, IN UCHAR OpMode,
		    IN UCHAR EapType)
{
	/* Inb-EAP Message */
	UCHAR Header802_3[14];
	...
	if ((Len <= 0) && (OpCode != WSC_OPCODE_START) &&
	    (OpCode != WSC_OPCODE_FRAG_ACK))
		return;
	...
	if (CurOpMode == AP_MODE) {
		if (OpMode == AP_MODE) {
			if (bssIdx < HW_BEACON_MAX_NUM)
				MAKE_802_3_HEADER(
					Header802_3, &pWscControl->EntryAddr[0],
					&pAdapter->ApCfg.MBSSID[bssIdx]
						 .wdev.bssid[0],
					EAPOL);
		}
#ifdef APCLI_SUPPORT
			...
			if (bssIdx < MAX_APCLI_NUM)
				MAKE_802_3_HEADER(
					Header802_3, &pWscControl->EntryAddr[0],
					&pAdapter->ApCfg.ApCliTab[bssIdx]
						 .wdev.if_addr[0],
					EAPOL);
		}
	...
	if (pData && (Len > 0)) {
		if (pWscControl->bWscFragment && (pWscControl->bWscFirstOne)) {
			...
			MakeOutgoingFrame(pOutBuffer, &TmpLen,
					  sizeof(IEEE8021X_FRAME), &Ieee_8021x,
					  sizeof(EapFrame), &EapFrame,
					  sizeof(WscFrame), &WscFrame, LF_Len,
					  &MsgLen, END_OF_ARGS);
			FrameLen += TmpLen;
			MakeOutgoingFrame(pOutBuffer + FrameLen, &TmpLen, Len,
					  pData, END_OF_ARGS);
			FrameLen += TmpLen;
		} else {
			MakeOutgoingFrame(pOutBuffer, &FrameLen,
					  sizeof(IEEE8021X_FRAME), &Ieee_8021x,
					  sizeof(EapFrame), &EapFrame,
					  sizeof(WscFrame), &WscFrame, Len,
					  pData, END_OF_ARGS);
		}
	} else
		MakeOutgoingFrame(pOutBuffer, &FrameLen,
				  sizeof(IEEE8021X_FRAME), &Ieee_8021x,
				  sizeof(EapFrame), &EapFrame, sizeof(WscFrame),
				  &WscFrame, END_OF_ARGS);
	...
	if (pEntry)
		RTMPToWirelessSta(pAdapter, pEntry, Header802_3,
				  sizeof(Header802_3), (PUCHAR)pOutBuffer,
				  FrameLen, TRUE);
	else
...
VOID WscBuildBeaconIE(IN PRTMP_ADAPTER pAd, IN UCHAR b_configured,
		      ...
		      IN UCHAR CurOpMode)
{
	...
	/* 1. Version */
	templen =
		AppendWSCTLV(WSC_ID_VERSION, pData, &pReg->SelfInfo.Version, 0);
	pData += templen;
	Len += templen;
	/* 2. Simple Config State */
	templen =
		AppendWSCTLV(WSC_ID_SC_STATE, pData, (UINT8 *)&b_configured, 0);
	pData += templen;
	Len += templen;
#ifdef CONFIG_AP_SUPPORT
#ifdef WSC_V2_SUPPORT
	...
	if ((CurOpMode == AP_MODE) && pWpsCtrl->bSetupLock) {
		/* AP Setup Lock */
		templen = AppendWSCTLV(WSC_ID_AP_SETUP_LOCKED, pData,
				       (UINT8 *)&pWpsCtrl->bSetupLock, 0);
		pData += templen;
		Len += templen;
	}
	...
	if (b_selRegistrar) {
		...
		templen = AppendWSCTLV(WSC_ID_SEL_REGISTRAR, pData,
				       (UINT8 *)&b_selRegistrar, 0);
		pData += templen;
		Len += templen;
		/*4. Device Password ID */
		tempVal = htons(devPwdId);
		templen = AppendWSCTLV(WSC_ID_DEVICE_PWD_ID, pData,
				       (UINT8 *)&tempVal, 0);
		pData += templen;
		Len += templen;
		...
		templen = AppendWSCTLV(WSC_ID_SEL_REG_CFG_METHODS, pData,
				       (UINT8 *)&tempVal, 0);
		pData += templen;
		Len += templen;
	}
	...
	templen =
		AppendWSCTLV(WSC_ID_UUID_E, pData, &pWpsCtrl->Wsc_Uuid_E[0], 0);
	pData += templen;
	Len += templen;
	...
	if (pWpsCtrl->WscV2Info.bEnableWpsV2) {
		...
		WscGenV2Msg(pWpsCtrl, b_selRegistrar, pAuthorizedMACs,
			    AuthorizedMACsLen, &pData, &Len);

		/* Extra attribute that is not defined in WSC Sepc. */
		if (pWscTLV->pTlvData && pWscTLV->TlvLen) {
			templen = AppendWSCTLV(pWscTLV->TlvTag, pData,
					       (UINT8 *)pWscTLV->pTlvData,
					       pWscTLV->TlvLen);
			pData += templen;
			Len += templen;
		}
	...
	NdisCopyMemory(wdev->WscIEBeacon.Value + sizeof(WSC_IE_HEADER), Data,
		       Len);
...
VOID WscBuildProbeRespIE(IN PRTMP_ADAPTER pAd, IN UCHAR respType,
			 ...
			 IN INT AuthorizedMACsLen, IN UCHAR CurOpMode)
{
	...
	templen = AppendWSCTLV(
		WSC_ID_MANUFACTURER, pData, pReg->SelfInfo.Manufacturer,
		strlen((RTMP_STRING *)pReg->SelfInfo.Manufacturer));
	pData += templen;
	Len += templen;
	/* 9. Model Name */
	NdisZeroMemory(pData, 32 + 4);
	templen =
		AppendWSCTLV(WSC_ID_MODEL_NAME, pData, pReg->SelfInfo.ModelName,
			     strlen((RTMP_STRING *)pReg->SelfInfo.ModelName));
	pData += templen;
	Len += templen;
	/* 10. Model Number */
	NdisZeroMemory(pData, 32 + 4);
	templen =
		AppendWSCTLV(WSC_ID_MODEL_NUMBER, pData,
			     pReg->SelfInfo.ModelNumber,
			     strlen((RTMP_STRING *)pReg->SelfInfo.ModelNumber));
	pData += templen;
	Len += templen;
	/* 11. Serial Number */
	NdisZeroMemory(pData, 32 + 4);
	templen = AppendWSCTLV(
		WSC_ID_SERIAL_NUM, pData, pReg->SelfInfo.SerialNumber,
		strlen((RTMP_STRING *)pReg->SelfInfo.SerialNumber));
	pData += templen;
	Len += templen;
	/* 12. Primary Device Type */
	templen = AppendWSCTLV(WSC_ID_PRIM_DEV_TYPE, pData,
			       pReg->SelfInfo.PriDeviceType, 0);
	pData += templen;
	Len += templen;
	/* 13. Device Name */
	NdisZeroMemory(pData, 32 + 4);
	templen =
		AppendWSCTLV(WSC_ID_DEVICE_NAME, pData,
			     pReg->SelfInfo.DeviceName,
			     strlen((RTMP_STRING *)pReg->SelfInfo.DeviceName));
	pData += templen;
	Len += templen;
	/* 14. Config Methods */
	...
	tempVal = htons(tempVal);
	templen = AppendWSCTLV(WSC_ID_CONFIG_METHODS, pData, (UINT8 *)&tempVal,
			       0);
	pData += templen;
	Len += templen;
	...
	ieHdr.length = ieHdr.length + Len;
	NdisCopyMemory(wdev->WscIEProbeResp.Value, &ieHdr,
		       sizeof(WSC_IE_HEADER));
	NdisCopyMemory(wdev->WscIEProbeResp.Value + sizeof(WSC_IE_HEADER), Data,
		       Len);
...
*	========================================================================
*/
VOID WscSendEapFail(IN PRTMP_ADAPTER pAd, IN PWSC_CTRL pWscControl,
		    IN BOOLEAN bSendDeAuth)
{
	...
	if (CurOpMode == AP_MODE) {
		MAKE_802_3_HEADER(Header802_3, &pWscControl->EntryAddr[0],
	...
	/* Make	 Transmitting frame */
	MakeOutgoingFrame(pOutBuffer, &FrameLen, sizeof(IEEE8021X_FRAME),
			  &Ieee_8021x, sizeof(EapFrame) - 1, &EapFrame,
			  END_OF_ARGS);
#ifdef CONFIG_AP_SUPPORT
...
#ifdef CONFIG_AP_SUPPORT
VOID WscBuildAssocRespIE(IN PRTMP_ADAPTER pAd, IN UCHAR ApIdx, IN UCHAR Reason,
			 OUT PUCHAR pOutBuf, OUT PUCHAR pIeLen)
{
	...
	if (wdev->WscControl.WscV2Info.bEnableWpsV2) {
		WscGenV2Msg(&wdev->WscControl, FALSE, NULL, 0, &pData, &Len);
	}
...
VOID WscSelectedRegistrar(IN PRTMP_ADAPTER pAd, IN PUCHAR pReginfo,
			  IN UINT Length, IN UCHAR apidx)
{
	...
	if (Length < 4) {
		MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
			 ("WscSelectedRegistrar --> Unknown IE\n"));
		return;
	}
	...
	while (Length > 4) {
		...
		case WSC_ID_SEL_REG_CFG_METHODS:
			NdisMoveMemory(&wsc_sel_reg_conf_mthd, pData,
				       sizeof(USHORT));
		...
		case WSC_ID_VENDOR_EXT:
			...
			if (pWscCtrl->WscV2Info.bEnableWpsV2 && (WscLen > 0)) {
				...
				*/
				if (!pAuthorizedMACs)
					os_alloc_mem(NULL, &pAuthorizedMACs,
						     WscLen);
				...
				WscParseV2SubItem(WFA_EXT_ID_AUTHORIZEDMACS,
						  pData, WscLen,
						  pAuthorizedMACs,
						  &AuthorizedMACsLen);
			}
		...
		default:
			MTWF_LOG(
				DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
				("WscSelectedRegistrar --> Unknown IE 0x%04x\n",
				 WscType));
			break;
		}
	...
	if (wsc_sel_reg == 0x01) {
		...
		WscBuildBeaconIE(pAd, WSC_SCSTATE_CONFIGURED, TRUE,
				 wsc_dev_pass_id, wsc_sel_reg_conf_mthd, apidx,
				 pAuthorizedMACs, AuthorizedMACsLen, AP_MODE);
		WscBuildProbeRespIE(pAd, WSC_MSGTYPE_AP_WLAN_MGR,
				    WSC_SCSTATE_CONFIGURED, TRUE,
				    wsc_dev_pass_id, wsc_sel_reg_conf_mthd,
				    pWscCtrl->EntryIfIdx, pAuthorizedMACs,
				    AuthorizedMACsLen, AP_MODE);
#ifdef WSC_V2_SUPPORT
		hex_dump("WscSelectedRegistrar - AuthorizedMACs::",
			 pAuthorizedMACs, AuthorizedMACsLen);

		if ((AuthorizedMACsLen == 6) &&
		    (NdisEqualMemory(pAuthorizedMACs, BROADCAST_ADDR,
				     MAC_ADDR_LEN) == FALSE) &&
		    (NdisEqualMemory(pAuthorizedMACs, ZERO_MAC_ADDR,
				     MAC_ADDR_LEN) == FALSE) &&
		    (pWscCtrl->WscState <= WSC_STATE_WAIT_M3)) {
			PWSC_PEER_ENTRY pWscPeer = NULL;

			NdisMoveMemory(pWscCtrl->EntryAddr, pAuthorizedMACs,
				       MAC_ADDR_LEN);
			RTMP_SEM_LOCK(&pWscCtrl->WscPeerListSemLock);
			WscClearPeerList(&pWscCtrl->WscPeerList);
			os_alloc_mem(pAd, (UCHAR **)&pWscPeer,
				     sizeof(WSC_PEER_ENTRY));

			if (pWscPeer) {
				NdisZeroMemory(pWscPeer,
					       sizeof(WSC_PEER_ENTRY));
				NdisMoveMemory(pWscPeer->mac_addr,
					       pAuthorizedMACs, MAC_ADDR_LEN);
		...
		WscBuildProbeRespIE(pAd, WSC_MSGTYPE_AP_WLAN_MGR,
				    WSC_SCSTATE_CONFIGURED, FALSE, 0, 0,
				    pWscCtrl->EntryIfIdx, NULL, 0, AP_MODE);
	}
			...
			OUT PUCHAR pOutBuf, OUT PUCHAR pIeLen)
{
	...
	MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_INFO,
		 ("-----> WscBuildProbeReqIE\n"));
	/* allocate memory */
	...
	templen =
		AppendWSCTLV(WSC_ID_RF_BAND, pData, &pReg->SelfInfo.RfBand, 0);
	pData += templen;
	Len += templen;
	/* 7. Associate state */
	...
	templen =
		AppendWSCTLV(WSC_ID_CONFIG_ERROR, pData, (UINT8 *)&tempVal, 0);
	pData += templen;
	Len += templen;
	/* 9. Device password ID */
	tempVal = pReg->SelfInfo.DevPwdId;
	templen =
		AppendWSCTLV(WSC_ID_DEVICE_PWD_ID, pData, (UINT8 *)&tempVal, 0);
	pData += templen;
	Len += templen;
#ifdef WSC_V2_SUPPORT
	...
	if (wsc_ctrl->WscV2Info.bEnableWpsV2) {
		...
		NdisZeroMemory(pData, 32 + 4);
		templen = AppendWSCTLV(
			WSC_ID_MODEL_NAME, pData, pReg->SelfInfo.ModelName,
			strlen((RTMP_STRING *)pReg->SelfInfo.ModelName));
		pData += templen;
		Len += templen;
		/* 12. Model Number */
		NdisZeroMemory(pData, 32 + 4);
		templen = AppendWSCTLV(
			WSC_ID_MODEL_NUMBER, pData, pReg->SelfInfo.ModelNumber,
			strlen((RTMP_STRING *)pReg->SelfInfo.ModelNumber));
		pData += templen;
		Len += templen;
		/* 13. Device Name */
		NdisZeroMemory(pData, 32 + 4);
		templen = AppendWSCTLV(
			WSC_ID_DEVICE_NAME, pData, pReg->SelfInfo.DeviceName,
			strlen((RTMP_STRING *)pReg->SelfInfo.DeviceName));
		pData += templen;
		Len += templen;
		/* Version2 */
		WscGenV2Msg(wsc_ctrl, FALSE, NULL, 0, &pData, &Len);
	}
			    ...
			    IN PVOID SystemSpecific2, IN PVOID SystemSpecific3)
{
	...
	if (pAdapter != NULL) {
		...
		if ((CurOpMode == AP_MODE) &&
		    (pAdapter->ApCfg.ApCliTab[if_idx].CtrlCurrState ==
			...
			for (i = 0; VALID_UCAST_ENTRY_WCID(pAdapter, i); i++) {
				PMAC_TABLE_ENTRY pEntry =
					&pAdapter->MacTab.Content[i];
				STA_TR_ENTRY *tr_entry =
					&pAdapter->MacTab.tr_entry[i];
...
#endif /* CONFIG_AP_SUPPORT */
		if (bReConnect) {
			if (pWscControl->WscProfile.ApplyProfileIdx <
			...
			if (CurOpMode == AP_MODE) {
				WscWriteConfToApCliCfg(
					pAdapter, pWscControl,
					&pWscControl->WscProfile.Profile
						 [pWscControl->WscProfile
							  .ApplyProfileIdx],
					TRUE);
				{
...
#ifdef MULTI_INF_SUPPORT
	MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_OFF,
		 ("[IfaceIdx = %d] WscPBCTimeOutAction !!!\n",
	...
	if (pAd != NULL) {
		...
		if (pWscControl->WscMode == WSC_PBC_MODE) {
			...
			if (pWscControl->WscState != WSC_STATE_OFF) {
				...
				{
#ifdef APCLI_SUPPORT
					if (pWscControl->WscApCliScanMode !=
...
#ifdef MULTI_INF_SUPPORT
	MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_OFF,
		 ("[IfaceIdx = %d] WscScanDoneCheckTimeOutAction !!!\n",
		  multi_inf_get_idx(pAd)));
#endif /* MULTI_INF_SUPPORT */

	if (pAd != NULL) {
		pWscControl->ConWscApcliScanDoneCheckTimerRunning = FALSE;
		RTMPCancelTimer(&pWscControl->ConWscApcliScanDoneCheckTimer,
				&Cancelled);
		...
		if (if_idx < MAX_APCLI_NUM) {
			...
			MlmeEnqueue(pAd, AP_SYNC_STATE_MACHINE,
				    APMT2_MLME_SCAN_COMPLETE, 0, NULL, if_idx);
			RTMP_MLME_HANDLER(pAd);
		} else {
...
BOOLEAN ValidateChecksum(IN UINT PIN)
{
	UINT accum = 0;
...
*/
UINT WscRandomGen4digitPinCode(IN PRTMP_ADAPTER pAd)
{
	UINT iPin;

	iPin = RandomByte2(pAd) * 256 * 256 + RandomByte2(pAd) * 256 +
	       RandomByte2(pAd);
	iPin = iPin % 10000;
	return iPin;
}

UINT WscRandomGeneratePinCode(IN PRTMP_ADAPTER pAd, IN UCHAR apidx)
{
	UINT iPin;
	UINT checksum;

	iPin = RandomByte(pAd) * 256 * 256 + RandomByte(pAd) * 256 +
	       RandomByte(pAd);
	iPin = iPin % 10000000;
...
VOID WscStop(IN PRTMP_ADAPTER pAd,
#ifdef CONFIG_AP_SUPPORT
	     ...
	     IN PWSC_CTRL pWscControl)
{
	...
	if (pWscControl->WscUpdatePortCfgTimerRunning) {
		pWscControl->WscUpdatePortCfgTimerRunning = FALSE;
		RTMPCancelTimer(&pWscControl->WscUpdatePortCfgTimer,
				&Cancelled);
	}
	if (pWscControl->EapolTimerRunning) {
		pWscControl->EapolTimerRunning = FALSE;
	...
	if ((pWscControl->EntryIfIdx & 0x0F) < pAd->ApCfg.BssidNum) {
		pEntry = MacTableLookup2(pAd, pWscControl->EntryAddr,
					 (struct wifi_dev *)pWscControl->wdev);
...
/* WPS_BandSteering Support */
	...
	if (!bFromApCli && (pAd->ApCfg.BandSteering)) {
		struct wifi_dev *wdev =
			&pAd->ApCfg.MBSSID[pWscControl->EntryIfIdx & 0x0F].wdev;
		PBND_STRG_CLI_TABLE table =
	...
	if (bFromApCli && (pWscControl->EntryIfIdx & 0x0F) < MAX_APCLI_NUM) {
		pAd->ApCfg.ApCliTab[(pWscControl->EntryIfIdx & 0x0F)]
			.ConWpsApCliModeScanDoneStatus =
	...
	if (pWscControl->WscSkipTurnOffLEDTimerRunning) {
		...
		RTMPCancelTimer(&pWscControl->WscSkipTurnOffLEDTimer,
				&Cancelled);
	}
		   ...
		   IN PWSC_CTRL pWscControl)
{
	...
	if (bFromApCli) {
		if (pOpposAd) {
			for (loop = 0; loop < pOpposAd->ApCfg.ApCliNum;
			     loop++) {
				pWpsCtrl = &pOpposAd->ApCfg.ApCliTab[loop]
						    .wdev.WscControl;
				MTWF_LOG(
					DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_OFF,
					("CON_WPS: Stop the Band[%d] %s ApCli WPS, state [%d]\n",
					 opsBandIdx, pWpsCtrl->IfName,
					 pWpsCtrl->WscState));
		...
		for (loop = 0; loop < pAd->ApCfg.ApCliNum; loop++) {
			...
			if (pWpsCtrl->WscState != WSC_STATE_OFF) {
				MTWF_LOG(
					DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_OFF,
					("CON_WPS: Stop the current pAd apcli [%d]\n",
					 loop));
		...
		if (pOpposAd != NULL) {
			for (loop = 0; loop < HW_BEACON_MAX_NUM; loop++) {
				struct wifi_dev *wdev =
					&pOpposAd->ApCfg.MBSSID[loop].wdev;
				...
				if ((wdev) &&
					...
					MTWF_LOG(
						DBG_CAT_SEC, CATSEC_WPS,
						DBG_LVL_OFF,
						("CON_WPS[%d]: Stop the %s AP Wsc Machine\n",
						 opsBandIdx,
						 pOpposAd->net_dev->name));
					WscBuildBeaconIE(pOpposAd,
					...
					WscBuildProbeRespIE(
						...
						IsAPConfigured, FALSE, 0, 0,
						loop, NULL, 0, AP_MODE);
					UpdateBeaconHandler(pOpposAd, wdev,
							    BCN_UPDATE_IE_CHG);
			...
			for (loop = 0; loop < HW_BEACON_MAX_NUM; loop++) {
				...
				if ((wdev) &&
					...
					MTWF_LOG(
						DBG_CAT_SEC, CATSEC_WPS,
						DBG_LVL_OFF,
						("CON_WPS: Stop the %s AP Wsc Machine\n",
						 wdev->if_dev->name));
...
VOID WscInit(IN PRTMP_ADAPTER pAd, IN BOOLEAN bFromApCli, IN UCHAR BssIndex)
{
	PWSC_CTRL pWscControl = NULL;
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	{
		...
		if (bFromApCli) {
			if ((BssIndex & 0x0F) < MAX_APCLI_NUM)
				wdev = &pAd->ApCfg.ApCliTab[BssIndex & 0x0F]
						.wdev;
		} else
#endif /* APCLI_SUPPORT */
	...
	if (pWscControl->WscEnrolleePinCode == 0) {
		if (pWscControl->WscEnrollee4digitPinCode) {
			...
			pWscControl->WscEnrolleePinCode =
				GenerateWpsPinCode(pAd, bFromApCli, BssIndex);
...
#endif /* CONFIG_AP_SUPPORT */
	{
		...
		if (CurOpMode == AP_MODE) {
...
#endif /* APCLI_SUPPORT */
			{
				...
				IsAPConfigured = pWscControl->WscConfStatus;
				WscBuildBeaconIE(pAd, IsAPConfigured, FALSE, 0,
						 0, (BssIndex & 0x0F), NULL, 0,
						 AP_MODE);
				WscBuildProbeRespIE(pAd,
						    WSC_MSGTYPE_AP_WLAN_MGR,
						    IsAPConfigured, FALSE, 0, 0,
						    BssIndex, NULL, 0, AP_MODE);
				UpdateBeaconHandler(pAd, wdev,
						    BCN_UPDATE_IE_CHG);
			}
		}
...
RTMP_STRING *WscGetAuthTypeStr(IN USHORT authFlag)
{
	switch (authFlag) {
...
RTMP_STRING *WscGetEncryTypeStr(IN USHORT encryFlag)
{
	switch (encryFlag) {
...
static VOID WscSetWepCipher(IN struct _SECURITY_CONFIG *pSecConfig,
			    IN PWSC_CREDENTIAL pCredential, IN UCHAR WepKeyId,
			    IN USHORT WepKeyLen)
{
	if (WepKeyLen == 5 || WepKeyLen == 13) {
		pSecConfig->WepKey[WepKeyId].KeyLen = (UCHAR)WepKeyLen;
		os_move_mem(pSecConfig->WepKey[WepKeyId].Key, pCredential->Key,
			    WepKeyLen);
		...
		AtoH((RTMP_STRING *)pCredential->Key,
		     pSecConfig->WepKey[WepKeyId].Key, WepKeyLen / 2);
...
void WscWriteConfToPortCfg(IN PRTMP_ADAPTER pAd, IN PWSC_CTRL pWscControl,
			   IN PWSC_CREDENTIAL pCredential, IN BOOLEAN bEnrollee)
{
	...
	if (bEnrollee || (CurOpMode == AP_MODE)) {
		if (CurOpMode == AP_MODE) {
			NdisZeroMemory(pAd->ApCfg.MBSSID[CurApIdx].Ssid,
				       MAX_LEN_OF_SSID);
			NdisMoveMemory(pAd->ApCfg.MBSSID[CurApIdx].Ssid,
		...
		if (pCredential->AuthType &
			...
			if (!(pCredential->EncrType &
			      (WSC_ENCRTYPE_TKIP | WSC_ENCRTYPE_AES))) {
				MTWF_LOG(
					DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
		...
		WscSetAuthMode(pAd, CurOpMode, CurApIdx,
			       WscGetAuthTypeStr(pCredential->AuthType));
		WscSetEncrypType(pAd, CurOpMode, CurApIdx,
		...
		if (pCredential->EncrType != WSC_ENCRTYPE_NONE) {
			if (pCredential->EncrType &
				...
				if (CurOpMode == AP_MODE) {
					p_wdev = &pAd->ApCfg.MBSSID[CurApIdx]
							  .wdev;
				...
				if (pCredential->KeyLength >= 8 &&
					...
					RTMPMoveMemory(pWscControl->WpaPsk,
						       pCredential->Key,
						       pWscControl->WpaPskLen);
					RTMPMoveMemory(PassphraseStr,
						       pCredential->Key,
						       pWscControl->WpaPskLen);
					RTMPZeroMemory(p_wdev->SecConfig.PSK,
						       LEN_PSK + 1);
					RTMPMoveMemory(p_wdev->SecConfig.PSK,
						       pCredential->Key,
						       pWscControl->WpaPskLen);
					MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS,
						 DBG_LVL_TRACE,
			...
			} else if (pCredential->EncrType ==
				   WSC_ENCRTYPE_WEP) { /* Only for WPS 2.0 */
				UCHAR WepKeyId = 0;
				USHORT WepKeyLen = pCredential->KeyLength;

				if ((pCredential->KeyIndex >= 1) &&
				    (pCredential->KeyIndex <= 4)) {
					struct _SECURITY_CONFIG *pSecConfig =
						NULL;

					WepKeyId = (pCredential->KeyIndex -
						    1); /* KeyIndex = 1 ~ 4 */
#ifdef CONFIG_AP_SUPPORT
					...
					if (WepKeyLen == 5 || WepKeyLen == 13 ||
					    WepKeyLen == 10 || WepKeyLen == 26)
						WscSetWepCipher(pSecConfig,
								pCredential,
								WepKeyId,
								WepKeyLen);
					else
...
VOID WscWriteSsidToDatFile(IN PRTMP_ADAPTER pAd, IN RTMP_STRING *pTempStr,
			   IN BOOLEAN bNewFormat, IN UCHAR CurOpMode)
{
#ifdef CONFIG_AP_SUPPORT
	UCHAR apidx;
#endif /* CONFIG_AP_SUPPORT */
	...
	if (bNewFormat == FALSE) {
		...
		for (apidx = 0; apidx < pAd->ApCfg.BssidNum; apidx++) {
			snprintf(item_str, sizeof(item_str), "SSID%d",
				 (apidx + 1));

			if (rtstrstr(pTempStr, item_str)) {
				NdisZeroMemory(pTempStr, 512);
				NdisMoveMemory(pTempStr, item_str,
					       strlen(item_str));
				offset = strlen(pTempStr);
...
VOID WscWriteWpaPskToDatFile(IN PRTMP_ADAPTER pAd, IN RTMP_STRING *pTempStr,
	...
	if (bNewFormat == FALSE) {
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		{
			for (apidx = 0; apidx < pAd->ApCfg.BssidNum; apidx++) {
				...
				if (apidx == 0) {
					NdisMoveMemory(pTempStr, "WPAPSK=",
						       strlen("WPAPSK="));
					offset = strlen(pTempStr);
				} else {
					offset = strlen(pTempStr);
		...
		for (apidx = 0; apidx < pAd->ApCfg.BssidNum; apidx++) {
			snprintf(item_str, sizeof(item_str), "WPAPSK%d",
				 (apidx + 1));
...
static BOOLEAN WscCheckNonce(IN PRTMP_ADAPTER pAdapter,
			     IN MLME_QUEUE_ELEM *pElem, IN BOOLEAN bFlag,
			     IN PWSC_CTRL pWscControl)
{
	USHORT Length;
	PUCHAR pData;
...
VOID WscGetRegDataPIN(IN PRTMP_ADAPTER pAdapter, IN UINT PinCode,
		      IN PWSC_CTRL pWscControl)
{
	UCHAR tempPIN[9] = { 0 };
	...
	if (pWscControl->WscPinCode == 0) {
		...
		if (pWscControl->WscPinCodeLen == 4) {
			...
			snprintf((RTMP_STRING *)temp4PIN, sizeof(temp4PIN),
				 "%04u", pWscControl->WscPinCode);
			memcpy(pWscControl->RegData.PIN, temp4PIN, 4);
			pWscControl->RegData.PinCodeLen = 4;
		} else {
			snprintf((RTMP_STRING *)tempPIN, sizeof(tempPIN),
				 "%08u", pWscControl->WscPinCode);
			memcpy(pWscControl->RegData.PIN, tempPIN, 8);
			pWscControl->RegData.PinCodeLen = 8;
		}
	}
...
static VOID WscGetConfigErrFromNack(IN RTMP_ADAPTER *pAdapter,
				    IN MLME_QUEUE_ELEM *pElem,
				    OUT USHORT *pConfigError)
{
	USHORT Length = 0;
	...
	if (CurOpMode == AP_MODE) {
		...
		for (i = 0; VALID_UCAST_ENTRY_WCID(pAd, i); i++) {
			if (IS_ENTRY_CLIENT(&pAd->MacTab.Content[i]))
				pAd->MacTab.tr_entry[i].PortSecured =
					WPA_802_1X_PORT_NOT_SECURED;
		}

		pAd->ApCfg.MBSSID[apidx].wdev.PortSecured =
			WPA_802_1X_PORT_NOT_SECURED;
		MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
			 ("IF(ra%d) %s::(AuthMode=0x%x)\n", apidx, __func__,
...
VOID WscPushPBCAction(IN PRTMP_ADAPTER pAd, IN PWSC_CTRL pWscControl)
{
	BOOLEAN Cancelled;
...
VOID WscPINAction(IN PRTMP_ADAPTER pAd, IN PWSC_CTRL pWscControl)
{
	BOOLEAN Cancelled;
...
*	========================================================================
*/
VOID WscScanExec(IN PRTMP_ADAPTER pAd, IN PWSC_CTRL pWscControl)
{
#ifdef WSC_LED_SUPPORT
	UCHAR WPSLEDStatus;
#endif /* WSC_LED_SUPPORT */
...
#endif /*CON_WPS*/
			ApSiteSurvey_by_wdev(pAd, NULL, SCAN_WSC_ACTIVE, FALSE,
...
*	========================================================================
*/
BOOLEAN WscPBCExec(IN PRTMP_ADAPTER pAd, IN BOOLEAN bFromM2,
		   IN PWSC_CTRL pWscControl)
{
#ifdef WSC_LED_SUPPORT
	UCHAR WPSLEDStatus;
	...
	if (pWscControl->WscPBCBssCount != 1) {
		...
		if (pWscControl->WscPBCBssCount == 0) {
			...
			if (LED_MODE(pAd) == WPS_LED_MODE_SHARE)
				RTMPSetTimer(&pWscControl->WscLEDTimer,
					     WSC_WPS_FAIL_WIFI_LED_TIMEOUT);
			...
			pWscControl->WscStatus = STATUS_WSC_PBC_TOO_MANY_AP;
			RTMPSendWirelessEvent(pAd, IW_WSC_PBC_SESSION_OVERLAP,
					      NULL, BSS0, 0);
			...
			if (LED_MODE(pAd) ==
				...
				MTWF_LOG(
					DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
					("%s: Skip the WPS session overlap detected LED indication.\n",
					 __func__));
			} else { /* Other LED mode. */
	...
	if (CurOpMode == AP_MODE) {
		...
		rtmp_set_channel(
			pAd, wdev,
			pAd->ApCfg.ApCliTab[apcli_idx].MlmeAux.Channel);
#else
		RTEnqueueInternalCmd(pAd, CMDTHREAD_APCLI_PBC_TIMEOUT,
				     (VOID *)&pAd->ApCfg.ApCliTab[apcli_idx],
				     sizeof(APCLI_STRUCT));
#endif
		/* bring apcli interface down first */
		MTWF_LOG(
			...
			("!!! WscPBCExec --> Goto CMDTHREAD_APCLI_IF_DOWN!, pAd->ApCfg.ApCliTab[%d].Enable=%d  !!!\n",
			 apcli_idx, pAd->ApCfg.ApCliTab[apcli_idx].Enable));
		{
...
*	========================================================================
*/
BOOLEAN WscPINExec(IN PRTMP_ADAPTER pAd, IN BOOLEAN bFromM2,
		   IN PWSC_CTRL pWscControl)
{
#ifdef WSC_LED_SUPPORT
	UCHAR WPSLEDStatus;
	...
	if (pWscControl->WscPINBssCount != 1) {
		...
		if (pWscControl->WscPINBssCount == 0) {
			pWscControl->WscStatus =
				STATUS_WSC_PBC_NO_AP; /*No error code regarding PIN, so use PBC error code*/
#ifdef WSC_LED_SUPPORT
			/* Failed to find any partner. */
			WPSLEDStatus = LED_WPS_ERROR;
			RTMPSetLED(pAd, WPSLEDStatus);
			...
			MTWF_LOG(
				DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_OFF,
				("WscPINExec --> AP list is %d, wait for next time\n",
			...
			*/
			MTWF_LOG(
				DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
				("WscPINExec --> AP list is %d, stop WPS process!\n",
				 pWscControl->WscPINBssCount));
			WscStop(pAd,
#ifdef CONFIG_AP_SUPPORT
				FALSE,
...
static VOID WscCheckIsSameAP(IN UUID_BSSID_CH_INFO *ApUuidBssid,
			     IN UUID_BSSID_CH_INFO *TmpInfo, IN UCHAR *zeros16,
			     OUT BOOLEAN *bSameAP)
{
	if (RTMPCompareMemory(&ApUuidBssid->Uuid[0], &TmpInfo->Uuid[0], 16) ==
	    0) {
		if (RTMPCompareMemory(&TmpInfo->Uuid[0], zeros16, 16) != 0) {
			...
			*/
			*bSameAP = TRUE;
		} else if (RTMPCompareMemory(&TmpInfo->Uuid[0], zeros16, 16) ==
			   0) {
			if (ApUuidBssid->Band != TmpInfo->Band) {
				if (RTMPCompareMemory(&ApUuidBssid->Bssid[0],
						      &TmpInfo->Bssid[0],
						      5) == 0) {
					/*
		   ...
		   (RTMPCompareMemory(&ApUuidBssid->Uuid[0], zeros16, 16) ==
		    0)) {
		if ((RTMPCompareMemory(&ApUuidBssid->Bssid[0],
				       &TmpInfo->Bssid[0], 5) == 0) &&
		    (ApUuidBssid->Band != TmpInfo->Band)) {
			INT tmpDiff = (INT)(ApUuidBssid->Bssid[5] -
					    TmpInfo->Bssid[5]);
...
BOOLEAN WscBssWpsIESearchForPBC(RTMP_ADAPTER *pAd, WSC_CTRL *pWscControl,
				BSS_ENTRY *pInBss,
				UUID_BSSID_CH_INFO ApUuidBssid[], INT VarIeLen,
				PUCHAR pVar)
{
	INT j = 0, Len = 0, idx = 0;
	...
	while ((Len > 0) && (bFound == FALSE)) {
		...
		while (Len > 0) {
			...
			if (be2cpu16(pWscIE->Type) == WSC_ID_DEVICE_PWD_ID) {
				...
				/*be2cpu16<-- */
				DevicePasswordID =
					WINBON_GET16((PUCHAR)&pWscIE->Data[0]);
#else
				DevicePasswordID = be2cpu16(get_unaligned(
					(USHORT *)&pWscIE->Data[0]));
				...
				if (((pWscControl->WscMode == WSC_PBC_MODE) &&
				     (DevicePasswordID == DEV_PASS_ID_PBC)) ||
				    ((pWscControl->WscMode == WSC_SMPBC_MODE) &&
				     (DevicePasswordID == DEV_PASS_ID_SMPBC))) {
					...
					RTMPMoveMemory(&TmpInfo.Bssid[0],
						       &pInBss->Bssid[0],
						       MAC_ADDR_LEN);
					TmpInfo.Channel = pInBss->Channel;
					RTMPZeroMemory(&TmpInfo.Ssid[0],
						       MAX_LEN_OF_SSID);
					RTMPMoveMemory(&TmpInfo.Ssid[0],
						       &pInBss->Ssid[0],
						       pInBss->SsidLen);
		...
		if (Reqwdev && IS_MAP_TURNKEY_ENABLE(pAd) &&
		    (MAPRcGetBandIdxByChannelCheck(pAd->hdev_ctrl,
						   pInBss->Channel) !=
		...
		if (pAd->ApCfg.ConWpsApCliDisabled == FALSE) {
			if ((pAd->ApCfg.ApCliTab[apcli_idx]
				     .wdev.WscControl.conWscStatus &
			     CON_WPS_STATUS_APCLI_RUNNING) &&
			    (pAd->ApCfg.ApCliTab[apcli_idx ^ 1]
				     .wdev.WscControl.conWscStatus &
			     CON_WPS_STATUS_APCLI_RUNNING) &&
		...
		if ((bFound == TRUE) && (bSelReg == TRUE)) {
			...
			if (pWscControl->WscPBCBssCount > 0) {
				for (j = 0; j < pWscControl->WscPBCBssCount;
				     j++) {
					WscCheckIsSameAP(&ApUuidBssid[j],
							 &TmpInfo, zeros16,
							 &bSameAP);
					if (bSameAP)
						break;
			...
			if (bSameAP) {
				if ((pWscControl->WpsApBand ==
				     PREFERRED_WPS_AP_PHY_TYPE_2DOT4_G_FIRST) &&
				    (TmpInfo.Band == WSC_RFBAND_24GHZ) &&
				    (ApUuidBssid[j].Band != TmpInfo.Band)) {
					RTMPMoveMemory(
						&(ApUuidBssid[j].Bssid[0]),
						&TmpInfo.Bssid[0],
						MAC_ADDR_LEN);
					RTMPZeroMemory(
						&(ApUuidBssid[j].Ssid[0]),
						MAX_LEN_OF_SSID);
					RTMPMoveMemory(
						&(ApUuidBssid[j].Ssid[0]),
						&TmpInfo.Ssid[0],
						TmpInfo.SsidLen);
					ApUuidBssid[j].SsidLen =
						TmpInfo.SsidLen;
					ApUuidBssid[j].Channel =
						TmpInfo.Channel;
				} else if ((pWscControl->WpsApBand ==
			...
			if (bSameAP == FALSE) {
				...
				/* Store UUID */
				RTMPMoveMemory(&(ApUuidBssid[index].Uuid[0]),
					       &TmpInfo.Uuid[0], 16);
				RTMPMoveMemory(&(ApUuidBssid[index].Bssid[0]),
					       &pInBss->Bssid[0], MAC_ADDR_LEN);
				RTMPZeroMemory(&(ApUuidBssid[index].Ssid[0]),
					       MAX_LEN_OF_SSID);
				RTMPMoveMemory(&(ApUuidBssid[index].Ssid[0]),
					       &pInBss->Ssid[0],
					       pInBss->SsidLen);
				...
				for (idx = 0; idx < 16; idx++)
					MTWF_LOG(
						...
						DBG_LVL_ERROR,
						("%02x  ",
						 ApUuidBssid[index].Uuid[idx]));

				MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_ERROR,
					 (" SSID: %s, CH: %d ",
					  ApUuidBssid[index].Ssid,
					  ApUuidBssid[index].Channel));
...
#ifdef WSC_STA_SUPPORT
BOOLEAN WscBssWpsIESearchForPIN(RTMP_ADAPTER *pAd, WSC_CTRL *pWscControl,
				BSS_ENTRY *pInBss,
				UUID_BSSID_CH_INFO ApUuidBssid[], INT VarIeLen,
				PUCHAR pVar)
{
	...
	while ((Len > 0) && (bFound == FALSE)) {
		...
		while (Len > 0) {
			...
			if (be2cpu16(pWscIE->Type) == WSC_ID_DEVICE_PWD_ID) {
				...
				MTWF_LOG(
					DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
					("WscPINBssTableSort : DevicePasswordID = 0x%04x\n",
					 DevicePasswordID));

				if ((pWscControl->WscMode == WSC_PIN_MODE) &&
				    (DevicePasswordID == DEV_PASS_ID_PIN)) {
		...
		if ((bFound == TRUE) && (bSelReg == TRUE)) {
			...
			if (pWscControl->WscPINBssCount > 0) {
				for (j = 0; j < pWscControl->WscPINBssCount;
				     j++) {
					WscCheckIsSameAP(&ApUuidBssid[j],
							 &TmpInfo, zeros16,
							 &bSameAP);
...
*	========================================================================
*/
VOID WscPBCBssTableSort(IN RTMP_ADAPTER *pAd, IN WSC_CTRL *pWscControl)
{
	INT i;
	BSS_ENTRY *pInBss;
	...
	/* allocate memory */
	os_alloc_mem(NULL, (UCHAR **)&ApUuidBssid,
		     sizeof(UUID_BSSID_CH_INFO) * 8);
	...
	for (i = 0; i < pAd->ScanTab.BssNr; i++) {
		...
		if (wdevBand != MAPRcGetBandIdxByChannelCheck(pAd->hdev_ctrl,
							      pInBss->Channel))
			continue;
#endif

		/* 2. Search for WSC IE - 0xdd xx 00 50 f2 04 */
		rv = WscBssWpsIESearchForPBC(pAd, pWscControl, pInBss,
					     ApUuidBssid, pInBss->VarIELen,
					     pInBss->VarIEs);

		if (rv == FALSE) {
			WscBssWpsIESearchForPBC(pAd, pWscControl, pInBss,
						ApUuidBssid,
	...
	if (pWscControl->WscPBCBssCount == 1) {
		...
		RTMPMoveMemory(pWscControl->WscSsid.Ssid, ApUuidBssid[0].Ssid,
			       ApUuidBssid[0].SsidLen);
		pWscControl->WscSsid.SsidLength = ApUuidBssid[0].SsidLen;
		RTMPZeroMemory(pWscControl->WscBssid, MAC_ADDR_LEN);
		RTMPMoveMemory(pWscControl->WscBssid, ApUuidBssid[0].Bssid,
			       MAC_ADDR_LEN);
		RTMPMoveMemory(pWscControl->WscPeerUuid, ApUuidBssid[0].Uuid,
			       sizeof(ApUuidBssid[0].Uuid));
#ifdef APCLI_SUPPORT
		if (CurOpMode == AP_MODE) {
			apcli->MlmeAux.Channel = ApUuidBssid[0].Channel;
			COPY_MAC_ADDR(apcli->CfgApCliBssid,
				      pWscControl->WscBssid);
		}
		printk("will connect %s (%02X:%02X:%02X:%02X:%02X:%02X) on %d\n",
		       pWscControl->WscSsid.Ssid,
		       PRINT_MAC(pWscControl->WscBssid),
		       ApUuidBssid[0].Channel);
...
*	========================================================================
*/
VOID WscPINBssTableSort(IN RTMP_ADAPTER *pAd, IN WSC_CTRL *pWscControl)
{
	INT i;
	BSS_ENTRY *pInBss;
	UUID_BSSID_CH_INFO *ApUuidBssid = NULL;
	...
	for (i = 0; i < pAd->ScanTab.BssNr; i++) {
		...
		MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
			 ("#  wsc Bssid %02x:%02x:%02x:%02x:%02x:%02x\n",
			  pWscControl->WscBssid[0], pWscControl->WscBssid[1],
			  pWscControl->WscBssid[2], pWscControl->WscBssid[3],
			  pWscControl->WscBssid[4], pWscControl->WscBssid[5]));
		MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
			 ("# pIn Bssid %02x:%02x:%02x:%02x:%02x:%02x\n",
			  pInBss->Bssid[0], pInBss->Bssid[1], pInBss->Bssid[2],
			  pInBss->Bssid[3], pInBss->Bssid[4],
			  pInBss->Bssid[5]));
		...
		/* 2. Search for WSC IE - 0xdd xx 00 50 f2 04 */
		rv = WscBssWpsIESearchForPIN(pAd, pWscControl, pInBss,
					     ApUuidBssid, pInBss->VarIELen,
					     pInBss->VarIEs);

		if (rv == FALSE) {
			WscBssWpsIESearchForPIN(pAd, pWscControl, pInBss,
						ApUuidBssid,
						pInBss->VarIeFromProbeRspLen,
						pInBss->pVarIeFromProbRsp);
		}
	...
	MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_OFF,
		 ("WscPINBssTableSort : Total %d PIN Registrar Found\n",
		  pWscControl->WscPINBssCount));
}
#endif

VOID WscGenRandomKey(IN PRTMP_ADAPTER pAd, IN PWSC_CTRL pWscControl,
	...
	*/
	if (pWscControl->WscKeyASCII == 0) {
		...
		for (idx = 0; idx < 32; idx++) {
			...
			snprintf((RTMP_STRING *)&tmpStrB[0], 3, "%02x",
				 tempRandomByte);
...
VOID WscCreateProfileFromCfg(IN PRTMP_ADAPTER pAd, IN UCHAR OpMode,
			     IN PWSC_CTRL pWscControl,
			     OUT PWSC_PROFILE pWscProfile)
{
	...
	if (CurOpMode == AP_MODE) {
		if ((OpMode & 0x0F) == AP_MODE) {
			...
			*/
			if (((pWscControl->WscConfStatus ==
			      WSC_SCSTATE_UNCONFIGURED) ||
			     (IS_AKM_FT_WPAPSKWPA2PSK_Entry(
				     &pAd->ApCfg.MBSSID[apidx].wdev))) &&
			    (OpMode & REGISTRAR_ACTION)) {
				switch (pAd->ApCfg.MBSSID[apidx]
						.wdev.WscSecurityMode) {
				case WPAPSKTKIP:
				...
				if (pWscControl->WscConfStatus ==
					...
					*/
					if (IS_CIPHER_TKIP_Entry(
						    &pAd->ApCfg.MBSSID[apidx]
							     .wdev))
						encyType = WSC_ENCRTYPE_TKIP;

					if (IS_CIPHER_AES_Entry(
						    &pAd->ApCfg.MBSSID[apidx]
							     .wdev))
						encyType |=
							WSC_ENCRTYPE_AES; /*mixmode case, bitmap*/
				}
			} else {
				authType = WscGetAuthType(
					pAd->ApCfg.MBSSID[apidx]
						.wdev.SecConfig.AKMMap);
				encyType = WscGetEncryType(
					pAd->ApCfg.MBSSID[apidx]
						.wdev.SecConfig.PairwiseCipher);
			}

			WepKeyId = pAd->ApCfg.MBSSID[apidx]
			...
			if (apidx < MAX_APCLI_NUM) {
				...
				encyType = WscGetEncryType(
					pAd->ApCfg.ApCliTab[apidx]
						.wdev.SecConfig.PairwiseCipher);
				WepKeyId = pAd->ApCfg.ApCliTab[apidx]
						   .wdev.SecConfig.GroupKeyId;
			}
		}
	...
	pCredential =
		&pWscProfile->Profile
			 [0]; /*Only support one credential now. 20070515 */
	...
	if ((pWscControl->WscConfStatus == WSC_SCSTATE_UNCONFIGURED) &&
	    ((((OpMode & 0x0F) == AP_MODE)) && (OpMode & REGISTRAR_ACTION))) {
		pCredential->KeyIndex = 1;

		if ((OpMode & 0x0F) == STA_MODE) {
			{
				WscGenRandomKey(pAd, pWscControl,
						pCredential->Key,
						&pCredential->KeyLength);
				authType = WSC_AUTHTYPE_WPA2PSK;
				encyType = WSC_ENCRTYPE_AES;
			...
			WscGenRandomKey(pAd, pWscControl, pCredential->Key,
					&pCredential->KeyLength);
	} else {
		struct _SECURITY_CONFIG *pSecConfig = NULL;
		...
		case WSC_ENCRTYPE_WEP:
			...
			if (CurOpMode == AP_MODE)
				pSecConfig =
					&pAd->ApCfg.MBSSID[apidx].wdev.SecConfig;

#ifdef APCLI_SUPPORT

			if (OpMode == AP_CLIENT_MODE) {
				if (apidx < MAX_APCLI_NUM)
					pSecConfig = &pAd->ApCfg.ApCliTab[apidx]
							      .wdev.SecConfig;
			}
			...
			if (pSecConfig && pSecConfig->WepKey[WepKeyId].KeyLen) {
				...
				for (i = 0;
				     i < pSecConfig->WepKey[WepKeyId].KeyLen;
				     i++)
					snprintf(
						(RTMP_STRING *)pCredential->Key,
						64, "%s%02x", pCredential->Key,
						pSecConfig->WepKey[WepKeyId]
							.Key[i]);

				pCredential->KeyLength =
					pSecConfig->WepKey[WepKeyId].KeyLen * 2;
			}
		...
		case (WSC_ENCRTYPE_AES | WSC_ENCRTYPE_TKIP):
			...
			memcpy(pCredential->Key, pWscControl->WpaPsk,
			       pWscControl->WpaPskLen);
			break;
		}
	}
	...
	if (CurOpMode == AP_MODE) {
		if ((OpMode & 0x0F) == AP_MODE) {
			NdisMoveMemory(pCredential->MacAddr,
				       pAd->ApCfg.MBSSID[apidx].wdev.bssid, 6);
			...
			if ((pWscControl->WscConfStatus ==
				...
				NdisMoveMemory(
					pCredential->SSID.Ssid,
					pWscControl->WscDefaultSsid.Ssid,
					pWscControl->WscDefaultSsid.SsidLength);
				pCredential->SSID.SsidLength =
					pWscControl->WscDefaultSsid.SsidLength;
			} else {
				NdisMoveMemory(
			...
			if (apidx < MAX_APCLI_NUM) {
				NdisMoveMemory(
					pCredential->MacAddr,
					APCLI_ROOT_BSSID_GET(
						pAd, pAd->ApCfg.ApCliTab[apidx]
							     .MacTabWCID),
					6);
				NdisMoveMemory(
					pCredential->SSID.Ssid,
					pAd->ApCfg.ApCliTab[apidx].Ssid,
					pAd->ApCfg.ApCliTab[apidx].SsidLen);
				pCredential->SSID.SsidLength =
					pAd->ApCfg.ApCliTab[apidx].SsidLen;
			}
		}
...
#ifdef CONFIG_AP_SUPPORT
#ifdef APCLI_SUPPORT
void WscWriteConfToApCliCfg(IN PRTMP_ADAPTER pAd, IN PWSC_CTRL pWscControl,
			    ...
			    IN BOOLEAN bEnrollee)
{
		 ...
		 ("-----> WscWriteConfToApCliCfg (apcli%d)\n", CurApIdx));
	{
		...
		Set_SecAuthMode_Proc(pAd,
				     WscGetAuthTypeStr(pCredential->AuthType));
		Set_SecEncrypType_Proc(
			pAd, WscGetEncryTypeStr(pCredential->EncrType));
		...
		if (pCredential->EncrType != WSC_ENCRTYPE_NONE) {
			if (pCredential->EncrType &
				...
				if (pCredential->KeyLength >= 8 &&
					...
					NdisMoveMemory(pWscControl->WpaPsk,
						       pCredential->Key,
						       pWscControl->WpaPskLen);
					os_zero_mem(
						pApCliTab->wdev.SecConfig.PSK,
						LEN_PSK + 1);
					os_move_mem(
						pApCliTab->wdev.SecConfig.PSK,
						pCredential->Key,
				...
				if ((WepKeyId >= 0) && (WepKeyId <= 3)) {
					...
					MTWF_LOG(
						...
						("Unsupport default key index (%d), use key Index 1.\n",
						 WepKeyId));
					pApCliTab->wdev.SecConfig.PairwiseKeyId =
						0;
				}
			}
		}
	...
	if (pWscControl->WscProfile.ProfileCnt > 1) {
		...
		RTMPSetTimer(&pWscControl->WscProfileRetryTimer,
...
VOID WscApCliLinkDownById(IN PRTMP_ADAPTER pAd, IN UCHAR apidx)
{
	...
	MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
		 ("-----> WscApCliLinkDownById, apidx=%u, apcliEn=%d\n", apidx,
		  apcliEn));
...
VOID WscApCliLinkDown(IN PRTMP_ADAPTER pAd, IN PWSC_CTRL pWscControl)
{
	...
	MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
		 ("-----> WscApCliLinkDown, apidx=%u\n", apidx));

	if (apidx >= MAX_APCLI_NUM)
		return;

	RTEnqueueInternalCmd(pAd, CMDTHREAD_WSC_APCLI_LINK_DOWN, (VOID *)&apidx,
			     sizeof(UCHAR));
}
...
VOID WpsSmProcess(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem)
{
	...
	HeaderLen +=
		LENGTH_802_1_H + sizeof(IEEE8021X_FRAME) + sizeof(EAP_FRAME);
	...
	if ((Elem->MsgType == WSC_EAPOL_UPNP_MSG) &&
		...
		pData = (RTMP_STRING *)&Elem
				->Msg[LENGTH_802_11 + LENGTH_802_1_H];
#ifdef A4_CONN
		if (pHeader->FC.FrDs == 1 && pHeader->FC.ToDs == 1)
			pData = (RTMP_STRING *)&Elem
					->Msg[LENGTH_802_11_WITH_ADDR4 +
					      LENGTH_802_1_H];
#endif
		...
		if (pEapFrame->Code == EAP_CODE_FAIL) {
			...
			NdisMoveMemory(Elem->Msg + MAC_ADDR_LEN, fail_data,
				       strlen(fail_data));
			Elem->MsgLen = strlen(fail_data);
			StateMachinePerformAction(
				pAd, &pAd->Mlme.WscMachine, Elem,
				pAd->Mlme.WscMachine.CurrState);
			return;
		} else if ((pEapFrame->Code == EAP_CODE_REQ) &&
			...
			NdisMoveMemory(Elem->Msg, pHeader->Addr2, MAC_ADDR_LEN);
			NdisMoveMemory(Elem->Msg + MAC_ADDR_LEN, id_data,
				       strlen(id_data));
			...
			if (WscCheckWSCHeader((PUCHAR)pData)) {
				...
				NdisMoveMemory(Elem->Msg, pHeader->Addr2,
					       MAC_ADDR_LEN);
				...
				if (pWsc->OpCode == WSC_OPCODE_FRAG_ACK) {
					...
					*/
					RTMP_STRING wsc_frag_ack[] =
						"WSC_FRAG_ACK";

					NdisMoveMemory(Elem->Msg + MAC_ADDR_LEN,
						       wsc_frag_ack,
						       strlen(wsc_frag_ack));
					...
					if ((pWpsCtrl->WscRxBufLen +
					     Elem->MsgLen) <
					    (MAX_MGMT_PKT_LEN - 6)) {
						NdisMoveMemory(
							(pWpsCtrl->pWscRxBuf +
							 pWpsCtrl->WscRxBufLen),
							pData, Elem->MsgLen);
						pWpsCtrl->WscRxBufLen +=
							Elem->MsgLen;
					}
					...
					if (pWsc->Flags & WSC_MSG_FLAG_MF)
						WscSendEapFragAck(pAd, pWpsCtrl,
								  pEntry);
					else
#endif /* WSC_V2_SUPPORT */
					{
		...
		if (IS_ENTRY_CLIENT(pEntry) &&
			...
			if (strstr(pData, "SimpleConfig")) {
				/* EAP-Rsp (Identity) */
				NdisMoveMemory(Elem->Msg + 6, pData,
					       Elem->MsgLen);
				StateMachinePerformAction(
					pAd, &pAd->Mlme.WscMachine, Elem,
					pAd->Mlme.WscMachine.CurrState);
		...
		if (WscCheckWSCHeader((PUCHAR)pData)) {
			...
			if (pWsc->OpCode == WSC_OPCODE_FRAG_ACK) {
				...
				*/
				...
				if ((pWpsCtrl->WscRxBufLen + Elem->MsgLen) <
				    (MAX_MGMT_PKT_LEN - 6)) {
					NdisMoveMemory((pWpsCtrl->pWscRxBuf +
							pWpsCtrl->WscRxBufLen),
						       pData, Elem->MsgLen);
...
#endif /* WSC_V2_SUPPORT */
				{
					...
					StateMachinePerformAction(
						...
						pAd->Mlme.WscMachine.CurrState);
					pWpsCtrl->WscRxBufLen = 0;
					NdisZeroMemory(pWpsCtrl->pWscRxBuf,
						       MAX_MGMT_PKT_LEN);
				}
			}
...
INT WscGetConfWithoutTrigger(IN PRTMP_ADAPTER pAd, IN PWSC_CTRL pWscControl,
			     IN BOOLEAN bFromUPnP)
{
	...
	if (wdev != NULL) {
		if (pAd->dev_idx == 0)
			RtmpOSWrielessEventSend(wdev->if_dev,
						RT_WLAN_EVENT_CUSTOM, SIGXFSZ,
						NULL, NULL, 0); /* ra0 */
		else
			RtmpOSWrielessEventSend(wdev->if_dev,
						RT_WLAN_EVENT_CUSTOM, SIGWINCH,
						NULL, NULL, 0); /* rai0 */
	}
	...
	if (pWscControl->WscConfMode == WSC_DISABLE) {
		pWscControl->bWscTrigger = FALSE;
		MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
			 ("WscGetConfForUpnp:: WPS is disabled.\n"));
		return FALSE;
	}
	...
	WscBuildBeaconIE(pAd, IsAPConfigured, TRUE, WscMode,
			 pWscControl->WscConfigMethods,
			 (pWscControl->EntryIfIdx & 0x0F), NULL, 0, AP_MODE);
	WscBuildProbeRespIE(pAd, WSC_MSGTYPE_AP_WLAN_MGR, IsAPConfigured, TRUE,
			    WscMode, pWscControl->WscConfigMethods,
			    pWscControl->EntryIfIdx, NULL, 0, AP_MODE);
	if (wdev != NULL)
		UpdateBeaconHandler(pAd, wdev, BCN_UPDATE_IE_CHG);
			...
			IN PWSC_CTRL pWscControl)
{
	...
	if (CurOpMode == AP_MODE) {
		if (pEntry && (IS_ENTRY_APCLI(pEntry)))
			WscSendMessage(pAdapter, WSC_OPCODE_NACK, pWscData,
				       DataLen, pWscControl, AP_CLIENT_MODE,
				       EAP_CODE_RSP);
		else
			WscSendMessage(pAdapter, WSC_OPCODE_NACK, pWscData,
				       DataLen, pWscControl, AP_MODE,
...
VOID WscCheckWpsIeFromWpsAP(IN PRTMP_ADAPTER pAd, IN PEID_STRUCT pEid,
			    OUT PUSHORT pDPIDFromAP)
{
	...
	if (NdisEqualMemory(pEid->Octet, WPS_OUI, 4)) {
		...
		while (Len > 0) {
			...
			if (pDPIDFromAP &&
			    be2cpu16(pWscIE->Type) == WSC_ID_DEVICE_PWD_ID) {
				/* Found device password ID */
				NdisMoveMemory(&DevicePasswordID, pData + 4,
					       sizeof(DevicePasswordID));
...
VOID WscPBCSessionOverlapCheck(IN PRTMP_ADAPTER pAd, IN UCHAR current_band)
{
	ULONG now;
	PWSC_STA_PBC_PROBE_INFO pWscStaPbcProbeInfo =
		&pAd->CommonCfg.WscStaPbcProbeInfo;
#ifdef CONFIG_AP_SUPPORT
	UCHAR bss_index = 0;
#endif
	...
	if (pWscStaPbcProbeInfo->WscPBCStaProbeCount[current_band] > 1) {
		...
		for (i = 0; i < MAX_PBC_STA_TABLE_SIZE; i++) {
			...
			if (pWscStaPbcProbeInfo->Valid[current_band][i] &&
			    RTMP_TIME_AFTER(
				    now,
				    pWscStaPbcProbeInfo->ReciveTime[current_band]
								   [i] +
					    120 * OS_HZ)) {
				NdisZeroMemory(
					&(pWscStaPbcProbeInfo
						  ->StaMacAddr[current_band][i]
							      [0]),
					MAC_ADDR_LEN);
				pWscStaPbcProbeInfo
					->ReciveTime[current_band][i] = 0;
				pWscStaPbcProbeInfo->Valid[current_band][i] =
					FALSE;
				pWscStaPbcProbeInfo
					->WscPBCStaProbeCount[current_band]--;
			}
		...
		if (pWscStaPbcProbeInfo->WscPBCStaProbeCount[current_band] >
		    1) {
#ifdef CONFIG_AP_SUPPORT
			...
			*/
			for (bss_index = 0; bss_index < pAd->ApCfg.BssidNum;
			     bss_index++) {
				if ((current_band ==
				     HcGetBandByWdev(
					     &pAd->ApCfg.MBSSID[bss_index]
						      .wdev)) &&
				    pAd->ApCfg.MBSSID[bss_index]
						    .wdev.WscControl
						    .WscConfMode !=
	...
	MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
		 ("WscPBCSessionOverlapCheck : WscPBCStaProbeCount[%d] = %d\n",
		  current_band,
		  pWscStaPbcProbeInfo->WscPBCStaProbeCount[current_band]));
}

VOID WscPBC_DPID_FromSTA(IN PRTMP_ADAPTER pAd, IN PUCHAR pMacAddr,
			 IN UCHAR current_band)
{
	...
	if (pWscStaPbcProbeInfo->WscPBCStaProbeCount[current_band] == 0)
		...
		for (tab_idx = 0; tab_idx < MAX_PBC_STA_TABLE_SIZE; tab_idx++) {
			if (NdisEqualMemory(
				    pMacAddr,
				    pWscStaPbcProbeInfo
					    ->StaMacAddr[current_band][tab_idx],
				    MAC_ADDR_LEN)) {
				pWscStaPbcProbeInfo
					->ReciveTime[current_band][tab_idx] =
					now;
				return;
			}
		}
			...
			if (RTMP_TIME_AFTER(
			    ...
			    NdisEqualMemory(
				    pWscStaPbcProbeInfo
					    ->StaMacAddr[current_band][tab_idx],
				    &ZERO_MAC_ADDR[0], MAC_ADDR_LEN)) {
				if (pWscStaPbcProbeInfo
					    ->Valid[current_band][tab_idx] ==
				    FALSE) {
					Index = tab_idx;
					bAddEntry = TRUE;
					break;
	...
	if (bAddEntry) {
		...
		NdisMoveMemory(
			pWscStaPbcProbeInfo->StaMacAddr[current_band][Index],
			pMacAddr, MAC_ADDR_LEN);
	}

	MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
		 ("%s(): STA_MAC = %02x:%02x:%02x:%02x:%02x:%02x\n", __func__,
		  PRINT_MAC(pMacAddr)));
	MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
		 ("%s(): WscPBCStaProbeCount[%d] = %d\n", __func__,
		  current_band,
		  pWscStaPbcProbeInfo->WscPBCStaProbeCount[current_band]));
}

static VOID WscWriteAuthToDAT(IN RTMP_ADAPTER *pAd, IN UCHAR CurOpMode,
	...
	if (CurOpMode == AP_MODE) {
		...
		for (index = 0; index < pAd->ApCfg.BssidNum; index++) {
			if (pAd->ApCfg.MBSSID[index].SsidLen) {
				if (index == 0)
					snprintf(
						pTempStr, 512, "%s%s", pTempStr,
						GetAuthModeStr(
							pAd->ApCfg.MBSSID[index]
								.wdev.SecConfig
								.AKMMap));
				else
					snprintf(
...
static VOID WscWriteEncrToDAT(IN RTMP_ADAPTER *pAd, IN UCHAR CurOpMode,
			      IN RTMP_STRING *pTempStr)
{
#ifdef CONFIG_AP_SUPPORT
	if (CurOpMode == AP_MODE) {
...
static VOID WscWriteWscConfModeToDAT(IN RTMP_ADAPTER *pAd, IN UCHAR CurOpMode,
				     IN RTMP_STRING *pTempStr)
{
	WSC_CTRL *wsc_ctrl;
	...
	if (CurOpMode == AP_MODE) {
		...
		for (index = 0; index < pAd->ApCfg.BssidNum; index++) {
			...
			if (index == 0)
				snprintf(pTempStr, 512, "%s%d", pTempStr,
					 wsc_ctrl->WscConfMode);
			else
				snprintf(pTempStr, 512, "%s;%d", pTempStr,
					 wsc_ctrl->WscConfMode);
		}
...
static VOID WscWriteWscConfStatusToDAT(IN RTMP_ADAPTER *pAd, IN UCHAR CurOpMode,
				       IN RTMP_STRING *pTempStr)
{
	WSC_CTRL *wsc_ctrl;
...
static VOID WscWriteDefaultKeyIdToDAT(IN RTMP_ADAPTER *pAd, IN UCHAR CurOpMode,
				      IN RTMP_STRING *pTempStr)
{
#ifdef CONFIG_AP_SUPPORT
	if (CurOpMode == AP_MODE) {
...
#define WSC_PRINT_WEP_KEY(__idx, __key_len, __key, __out_buf)                  \
	{                                                                      \
		for (__idx = 0; __idx < __key_len; (__idx)++)                  \
			snprintf(__out_buf, 512, "%s%02x", __out_buf,          \
				 __key[__idx]);                                \
	}

static BOOLEAN WscWriteWEPKeyToDAT(IN RTMP_ADAPTER *pAd, IN UCHAR CurOpMode,
				   ...
				   IN RTMP_STRING *pTempStr)
{
	...
	if (CurOpMode == AP_MODE) {
		...
		if ((strncmp(pTempStr, WepKeyFormatName,
			     strlen(WepKeyFormatName)) == 0)) {
			pCredentail =
				&pAd->ApCfg.MBSSID[apidx]
					 .wdev.WscControl.WscProfile.Profile[0];

			if (IS_CIPHER_WEP_Entry(
				    &pAd->ApCfg.MBSSID[apidx].wdev)) {
				...
				pTempStr = temp_ptr;
				NdisZeroMemory(pTempStr, 512);
				NdisMoveMemory(pTempStr, WepKeyFormatName,
					       strlen(WepKeyFormatName));

				for (idx = 0; idx < pAd->ApCfg.BssidNum;
				     idx++) {
					if (idx == apidx)
						snprintf(pTempStr, 512, "%s0",
							 pTempStr);
					else
						snprintf(pTempStr, 512, "%s%d",
							 pTempStr,
							 KeyType[idx]);
		...
		} else if ((strncmp(pTempStr, WepKeyName, strlen(WepKeyName)) ==
			    0)) {
			pCredentail =
				&pAd->ApCfg.MBSSID[apidx]
					 .wdev.WscControl.WscProfile.Profile[0];
			...
			if (IS_CIPHER_WEP_Entry(
				    &pAd->ApCfg.MBSSID[apidx].wdev)) {
				NdisZeroMemory(pTempStr, 512);
				NdisMoveMemory(pTempStr, WepKeyName,
					       strlen(WepKeyName));
...
VOID WscWriteConfToDatFile(RTMP_ADAPTER *pAd, UCHAR CurOpMode)
{
	...
	if (CurOpMode == AP_MODE) {
		...
		snprintf((RTMP_STRING *)WepKeyName, sizeof(WepKeyName),
			 "Key%dStr%d=",
			 pAd->ApCfg.MBSSID[apidx].wdev.SecConfig.PairwiseKeyId +
				 1,
			 apidx + 1);
		snprintf((RTMP_STRING *)WepKeyFormatName,
			 sizeof(WepKeyFormatName), "Key%dType=",
			 pAd->ApCfg.MBSSID[apidx].wdev.SecConfig.PairwiseKeyId +
				 1);
	}
	...
	if (IS_FILE_OPEN_ERR(file_w))
		...
		RtmpOSFileWrite(file_w, (RTMP_STRING *)cfgData,
				(int)(offset - cfgData));
		...
		for (;;) {
			...
			if ((size_t)(offset - cfgData) < fileLen) {
				...
				if ((strncmp(pTempStr,
					     "SSID=", strlen("SSID=")) == 0) ||
				    strncmp(pTempStr,
					    "SSID1=", strlen("SSID1=")) == 0 ||
				    strncmp(pTempStr,
					    "SSID2=", strlen("SSID2=")) == 0 ||
				    strncmp(pTempStr,
					    "SSID3=", strlen("SSID3=")) == 0 ||
				    strncmp(pTempStr,
					    "SSID4=", strlen("SSID4=")) == 0) {
					if (rtstrstr(pTempStr, "SSID="))
						bNewFormat = FALSE;

					WscWriteSsidToDatFile(pAd, pTempStr,
							      bNewFormat,
							      CurOpMode);
				}

				else if (strncmp(pTempStr, "AuthMode=",
						 strlen("AuthMode=")) == 0) {
					NdisZeroMemory(pTempStr, 512);
					snprintf(pTempStr, 512, "AuthMode=");
					WscWriteAuthToDAT(pAd, CurOpMode,
							  pTempStr);
				} else if (strncmp(pTempStr, "EncrypType=",
						   strlen("EncrypType=")) ==
					   0) {
					NdisZeroMemory(pTempStr, 512);
					snprintf(pTempStr, 512, "EncrypType=");
					WscWriteEncrToDAT(pAd, CurOpMode,
							  pTempStr);
				} else if ((strncmp(pTempStr, "WPAPSK=",
						    strlen("WPAPSK=")) == 0) ||
					   (strncmp(pTempStr, "WPAPSK1=",
						    strlen("WPAPSK1=")) == 0) ||
					   (strncmp(pTempStr, "WPAPSK2=",
						    strlen("WPAPSK2=")) == 0) ||
					   (strncmp(pTempStr, "WPAPSK3=",
						    strlen("WPAPSK3=")) == 0) ||
					   (strncmp(pTempStr, "WPAPSK4=",
						    strlen("WPAPSK4=")) == 0)) {
					bNewFormat = TRUE;
					...
					WscWriteWpaPskToDatFile(pAd, pTempStr,
								bNewFormat);
				} else if (strncmp(pTempStr, "WscConfMode=",
						   strlen("WscConfMode=")) ==
					   0) {
					snprintf(pTempStr, 512, "WscConfMode=");
					WscWriteWscConfModeToDAT(pAd, CurOpMode,
								 pTempStr);
				} else if (strncmp(pTempStr, "WscConfStatus=",
						   strlen("WscConfStatus=")) ==
					   0) {
					snprintf(pTempStr, 512,
						 "WscConfStatus=");
					WscWriteWscConfStatusToDAT(
						pAd, CurOpMode, pTempStr);
				} else if (strncmp(pTempStr, "DefaultKeyID=",
						   strlen("DefaultKeyID=")) ==
					   0) {
					NdisZeroMemory(pTempStr, 512);
					snprintf(pTempStr, 512,
						 "DefaultKeyID=");
					WscWriteDefaultKeyIdToDAT(
						pAd, CurOpMode, pTempStr);
				} else {
					if (WscWriteWEPKeyToDAT(
						    pAd, CurOpMode,
						    WepKeyFormatName,
						    WepKeyName,
						    pTempStr) == FALSE)
...
#ifdef CONFIG_AP_SUPPORT
static VOID WscWriteWepKeyToAR9File(IN RTMP_ADAPTER *pAd,
				    IN RTMP_STRING *pTempStr,
				    IN RTMP_STRING *pDatStr, OUT INT *datoffset)
{
	INT index;
	INT apidx;
	INT offset;
	...
	for (index = 1; index <= 4; index++) {
		snprintf(WepKeyFormatName, sizeof(WepKeyFormatName),
			 "Key%dType=", index);
		{
			NdisZeroMemory(pTempStr, 512);
			offset = 0;
			...
			for (apidx = 0; apidx < pAd->ApCfg.BssidNum; apidx++) {
				if (IS_CIPHER_WEP(pAd->ApCfg.MBSSID[MAIN_MBSSID]
							  .wdev.SecConfig
							  .PairwiseCipher)) {
					pCredentail =
						&pAd->ApCfg.MBSSID[apidx]
							 .wdev.WscControl
							 .WscProfile.Profile[0];

					if ((pCredentail->KeyLength == 5) ||
					    (pCredentail->KeyLength == 13))
						snprintf(pTempStr, 512, "%s1",
							 pTempStr); /* ASCII */
					else
						snprintf(pTempStr, 512, "%s0",
							 pTempStr); /* Hex */
			...
			snprintf(pTempStr, 512, "%s\n", pTempStr);
			offset = strlen(pTempStr);
			NdisMoveMemory(pDatStr + (*datoffset), pTempStr,
				       offset);
			(*datoffset) += offset;
		}
		...
		/*if (rtstrstr(pTempStr, WepKeyName)) */
		{
			...
			for (apidx = 0; apidx < pAd->ApCfg.BssidNum; apidx++) {
				pCredentail = &pAd->ApCfg.MBSSID[apidx]
						       .wdev.WscControl
						       .WscProfile.Profile[0];

				if (pCredentail->KeyLength) {
					NdisMoveMemory(pTempStr + tempStrLen,
						       pCredentail->Key,
						       pCredentail->KeyLength);
					tempStrLen = strlen(pTempStr);
				}
		...
		for (apidx = 0; apidx < pAd->ApCfg.BssidNum; apidx++) {
			snprintf(WepKeyName, sizeof(WepKeyName),
				 "Key%dStr%d=", index, (apidx + 1));
...
void WscWriteConfToAR9File(IN PRTMP_ADAPTER pAd, IN UCHAR CurOpMode)
{
	RTMP_STRING *fileName = NULL;
	RTMP_OS_FD file_w;
	RTMP_OS_FS_INFO osFSInfo;
	...
	if (IS_FILE_OPEN_ERR(file_w))
		...
		if (CurOpMode == AP_MODE) {
			for (index = 0; index < pAd->ApCfg.BssidNum; index++) {
				pWscControl = &pAd->ApCfg.MBSSID[index]
						       .wdev.WscControl;

				if (index == 0)
					snprintf(
						pTempStr, 512, "%s%d", pTempStr,
						pAd->ApCfg.MBSSID[apidx]
								.wdev.SecConfig
								.PairwiseKeyId +
							1);
...
#ifdef CONFIG_AP_SUPPORT
		if (CurOpMode == AP_MODE)
			WscWriteWepKeyToAR9File(pAd, pTempStr, pDatStr,
						&datoffset);
#endif /* CONFIG_AP_SUPPORT */
...
static INT wsc_write_dat_file_thread(IN ULONG Context)
{
	RTMP_OS_TASK *pTask;
	RTMP_ADAPTER *pAd;
	int Status = 0;
	...
	*/
	...
	if ((pAd->OpMode == OPMODE_AP)) {
		...
		for (ap_idx = 0; ap_idx < MaxBssidNum; ap_idx++) {
			PWSC_CTRL pWpsCtrl =
				&pAd->ApCfg.MBSSID[ap_idx].wdev.WscControl;

			WscStop(pAd, FALSE, pWpsCtrl);
			RTMPReleaseTimer(&pWpsCtrl->EapolTimer, &Cancelled);
			RTMPReleaseTimer(&pWpsCtrl->Wsc2MinsTimer, &Cancelled);
			RTMPReleaseTimer(
				&pWpsCtrl->WscUPnPNodeInfo.UPnPMsgTimer,
				&Cancelled);
			RTMPReleaseTimer(&pWpsCtrl->M2DTimer, &Cancelled);
#ifdef WSC_LED_SUPPORT
			RTMPReleaseTimer(&pWpsCtrl->WscLEDTimer, &Cancelled);
			RTMPReleaseTimer(&pWpsCtrl->WscSkipTurnOffLEDTimer,
					 &Cancelled);
#endif
			RTMPReleaseTimer(&pWpsCtrl->WscUpdatePortCfgTimer,
					 &Cancelled);
#ifdef WSC_V2_SUPPORT
			RTMPReleaseTimer(&pWpsCtrl->WscSetupLockTimer,
					 &Cancelled);
#endif
			pWpsCtrl->WscRxBufLen = 0;
			...
			if (pWpsCtrl->WscSetupLockTimerRunning) {
				...
				RTMPCancelTimer(&pWpsCtrl->WscSetupLockTimer,
						&Cancelled);
			}

			if (pWpsCtrl->WscV2Info.ExtraTlv.pTlvData) {
				os_free_mem(
					pWpsCtrl->WscV2Info.ExtraTlv.pTlvData);
...
#ifdef APCLI_SUPPORT
		{
			...
			for (index = 0; index < MAX_APCLI_NUM; index++) {
				PWSC_CTRL pWpsCtrl = &pAd->ApCfg.ApCliTab[index]
							      .wdev.WscControl;

				WscStop(pAd, TRUE, pWpsCtrl);
				RTMPReleaseTimer(&pWpsCtrl->EapolTimer,
						 &Cancelled);
				RTMPReleaseTimer(&pWpsCtrl->Wsc2MinsTimer,
						 &Cancelled);
				RTMPReleaseTimer(
					&pWpsCtrl->WscUPnPNodeInfo.UPnPMsgTimer,
					&Cancelled);
				RTMPReleaseTimer(&pWpsCtrl->M2DTimer,
						 &Cancelled);
#ifdef WSC_LED_SUPPORT
				RTMPReleaseTimer(&pWpsCtrl->WscLEDTimer,
						 &Cancelled);
				RTMPReleaseTimer(
					&pWpsCtrl->WscSkipTurnOffLEDTimer,
					&Cancelled);
#endif
				RTMPReleaseTimer(&pWpsCtrl->WscPBCTimer,
						 &Cancelled);
				RTMPReleaseTimer(&pWpsCtrl->WscScanTimer,
						 &Cancelled);
				RTMPReleaseTimer(
					&pWpsCtrl->WscProfileRetryTimer,
					&Cancelled);
#ifdef CON_WPS
				RTMPReleaseTimer(
					&pWpsCtrl->ConWscApcliScanDoneCheckTimer,
					&Cancelled);
#endif /* CON_WPS */
				pWpsCtrl->WscTxBufLen = 0;
...
#endif /* APCLI_SUPPORT */
#ifdef CON_WPS
		RTMPReleaseTimer(&pAd->ApCfg.ConWpsApCliBandMonitorTimer,
				 &Cancelled);
#endif /* CON_WPS */
	}
...
NDIS_STATUS WscThreadInit(RTMP_ADAPTER *pAd)
{
	...
	status = RtmpOSTaskAttach(pTask, wsc_write_dat_file_thread,
				  (ULONG)&pAd->wscTask);
	...
	if (status == NDIS_STATUS_SUCCESS) {
		...
		os_alloc_mem(NULL, (UCHAR **)&pAd->pWscElme,
			     sizeof(MLME_QUEUE_ELEM));
	}
...
*========================================================================
*/
VOID WSC_HDR_BTN_Init(IN PRTMP_ADAPTER pAd)
{
...
*========================================================================
*/
VOID WSC_HDR_BTN_Stop(IN PRTMP_ADAPTER pAd)
{
	pAd->CommonCfg.WscHdrPshBtnCheckCount = 0;
...
*========================================================================
*/
VOID WSC_HDR_BTN_CheckHandler(IN PRTMP_ADAPTER pAd)
{
	POS_COOKIE pObj = (POS_COOKIE)pAd->OS_Cookie;
	...
	if (flg_pressed) {
		...
		if (pAd->CommonCfg.WscHdrPshBtnCheckCount ==
		    WSC_HDR_BTN_CONT_TIMES) {
			...
			{
				pObj->ioctl_if = 0;
				Set_AP_WscMode_Proc(pAd,
						    (PUCHAR) "2"); /* 2: PBC */
				Set_AP_WscGetConf_Proc(
					pAd, (PUCHAR) "1"); /* 1: Trigger */
			}
#endif /* CONFIG_AP_SUPPORT */
			return;
		}
...
/* */
BOOLEAN WscSupportWPSLEDMode(IN PRTMP_ADAPTER pAd)
{
	if ((LED_MODE(pAd) == WPS_LED_MODE_7) ||
	    (LED_MODE(pAd) == WPS_LED_MODE_8) ||
		...
		MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
			 ("%s: Support WPS LED mode (The WPS LED mode = %d).\n",
			  __func__, LED_MODE(pAd)));
		return TRUE; /* Support WPS LED mode. */
	}
	MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
		 ("%s: Not support WPS LED mode (The WPS LED mode = %d).\n",
		  __func__, LED_MODE(pAd)));
	return FALSE; /* Not support WPS LED mode. */
}

BOOLEAN WscSupportWPSLEDMode10(IN PRTMP_ADAPTER pAd)
{
	UCHAR led_mode;
...
/* */
BOOLEAN WscAPHasSecuritySetting(IN PRTMP_ADAPTER pAdapter,
				IN PWSC_CTRL pWscControl)
{
	...
	default: {
		MTWF_LOG(
			DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
			("%s: Incorrect encryption types (%d)\n", __func__,
		 ...
		 IN PVOID SystemSpecific2, IN PVOID SystemSpecific3)
{
	...
	if ((LED_MODE(pAd) == WPS_LED_MODE_7) ||
		...
		WPSLEDStatus = LED_WPS_TURN_LED_OFF;
		MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
			 ("%s: Turn off the WPS successful LED pattern.\n",
			  __func__));
		...
		case LED_WPS_SUCCESS:
			...
			RTMPSetTimer(&pWscControl->WscLEDTimer,
				     WSC_WPS_TURN_OFF_LED_TIMEOUT);
			pWscControl->WscLEDTimerRunning = TRUE;
			MTWF_LOG(
				DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
				("%s: LED_WPS_SUCCESS => LED_WPS_TURN_LED_OFF\n",
				 __func__));
			break;
		...
		case LED_WPS_TURN_LED_OFF:
			if ((pWscControl->WscState == WSC_STATE_OFF) &&
			    (pWscControl->WscStatus == STATUS_WSC_CONFIGURED)) {
				if (WscAPHasSecuritySetting(pAd, pWscControl) ==
				    TRUE) { /* The NIC connects with an AP using an encryption algorithm. */
					/* Turn WPS success LED. */
					WPSLEDStatus = LED_WPS_TURN_ON_BLUE_LED;
					MTWF_LOG(
						DBG_CAT_SEC, CATSEC_WPS,
						DBG_LVL_TRACE,
						("%s: LED_WPS_TURN_LED_OFF => LED_WPS_TURN_ON_BLUE_LED\n",
						 __func__));
					...
					MTWF_LOG(
						DBG_CAT_SEC, CATSEC_WPS,
						DBG_LVL_TRACE,
						("%s: LED_WPS_TURN_LED_OFF => LED_WPS_TURN_LED_OFF\n",
						 __func__));
				}
			}
		...
		case LED_WPS_ERROR:
			WPSLEDStatus =
				LED_WPS_TURN_LED_OFF; /* Turn off the WPS LED. */
			MTWF_LOG(
				DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
				("%s: LED_WPS_ERROR/LED_WPS_SESSION_OVERLAP_DETECTED => LED_WPS_TURN_LED_OFF\n",
				 __func__));
			break;
		...
		case LED_WPS_SESSION_OVERLAP_DETECTED:
			...
			MTWF_LOG(
				DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
				("%s: LED_WPS_SESSION_OVERLAP_DETECTED => LED_WPS_TURN_LED_OFF\n",
				 __func__));
			break;
			    ...
			    IN PVOID SystemSpecific2, IN PVOID SystemSpecific3)
{
	...
	pWscControl->bSkipWPSTurnOffLED = FALSE;
	MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
		 ("%s: Allow the NIC to turn off the WPS LED again.\n",
		  __func__));
}
			     ...
			     IN PVOID SystemSpecific2, IN PVOID SystemSpecific3)
{
	...
	if (WscGetAuthMode(pCredential->AuthType) ==
		    pMbss->wdev.SecConfig.AKMMap &&
	    WscGetWepStatus(pCredential->EncrType) ==
		    pMbss->wdev.SecConfig.PairwiseCipher &&
	    NdisEqualMemory(pMbss->Ssid, pCredential->SSID.Ssid,
			    pCredential->SSID.SsidLength) &&
	    NdisEqualMemory(pWscControl->WpaPsk, pCredential->Key,
			    pCredential->KeyLength))
		return;
	...
	WscWriteConfToPortCfg(pAd, pWscControl,
			      &pWscControl->WscProfile.Profile[0], bEnrollee);
	pWscControl->WscProfile.ApplyProfileIdx &= 0x7FFF;
	{
		...
		*	@20150710
		*/
		enq_rv = RTEnqueueInternalCmd(pAd, CMDTHREAD_AP_RESTART,
					      (VOID *)&apidx, sizeof(UCHAR));
		MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
...
VOID WscCheckPeerDPID(IN PRTMP_ADAPTER pAd, IN PFRAME_802_11 Fr,
		      IN PUCHAR eid_data, IN INT eid_len, IN UCHAR current_band)
{
	WSC_IE *pWscIE;
	PUCHAR pData = NULL;
	INT Len = 0;
	...
	if ((pAd->OpMode == OPMODE_AP)) {
		...
		for (ap_idx = 0; ap_idx < pAd->ApCfg.BssidNum; ap_idx++) {
			if (NdisEqualMemory(Fr->Hdr.Addr1,
					    pAd->ApCfg.MBSSID[ap_idx].wdev.bssid,
					    MAC_ADDR_LEN)) {
				pWscCtrl = &pAd->ApCfg.MBSSID[ap_idx]
						    .wdev.WscControl;
				break;
			}
		}
	...
	while (Len > 0) {
		...
		if (be2cpu16(pWscIE->Type) == WSC_ID_DEVICE_PWD_ID) {
			...
			if (DevicePasswordID ==
			    DEV_PASS_ID_PBC) { /* Check for PBC value*/
				WscPBC_DPID_FromSTA(pAd, Fr->Hdr.Addr2,
						    current_band);
				hex_dump("PBC STA:", Fr->Hdr.Addr2,
					 MAC_ADDR_LEN);
				MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
					 ("\n"));
				...
				*/
				if (pWscCtrl &&
				    (!pWscCtrl->bWscTrigger ||
				     (pWscCtrl->WscMode != WSC_PBC_MODE))) {
					RTMP_SEM_LOCK(
						&pWscCtrl->WscPeerListSemLock);
					WscInsertPeerEntryByMAC(
						&pWscCtrl->WscPeerList,
						Fr->Hdr.Addr2);
					RTMP_SEM_UNLOCK(
						&pWscCtrl->WscPeerListSemLock);
				}
...
VOID WscClearPeerList(IN PLIST_HEADER pWscEnList)
{
...
PWSC_PEER_ENTRY WscFindPeerEntry(IN PLIST_HEADER pWscEnList, IN PUCHAR pMacAddr)
{
	...
	while (pPeerEntry != NULL) {
		if (NdisEqualMemory(pPeerEntry->mac_addr, pMacAddr,
				    MAC_ADDR_LEN))
			return pPeerEntry;
...
VOID WscInsertPeerEntryByMAC(IN PLIST_HEADER pWscEnList, IN PUCHAR pMacAddr)
{
	PWSC_PEER_ENTRY pWscPeer = NULL;
	...
	if (pWscPeer)
		...
		if (pWscPeer) {
			...
			pWscPeer->pNext = NULL;
			NdisMoveMemory(pWscPeer->mac_addr, pMacAddr,
				       MAC_ADDR_LEN);
			NdisGetSystemUpTime(&pWscPeer->receive_time);
...
INT WscApShowPin(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (pObj->ioctl_if_type == INT_APCLI) {
		...
		if (pWscControl->WscEnrolleePinCodeLen == 8)
			MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_OFF,
				 ("Enrollee PinCode(ApCli%d)        %08u\n",
				  inf_idx, pWscControl->WscEnrolleePinCode));
		else
			MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_OFF,
		...
		for (idx_2 = 0; idx_2 < pWscControl->WscProfile.ProfileCnt;
		     idx_2++) {
			PWSC_CREDENTIAL pCredential =
				&pWscControl->WscProfile.Profile[idx_2];
...
#endif /* APCLI_SUPPORT */
	{
		...
		MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
			 ("WPS Wsc2MinsTimerRunning(ra%d)        = %d\n",
			  inf_idx,
			  pAd->ApCfg.MBSSID[inf_idx]
				  .wdev.WscControl.Wsc2MinsTimerRunning));
#else
		/* display pin code */
		if (pAd->ApCfg.MBSSID[inf_idx]
...
VOID WscMaintainPeerList(IN PRTMP_ADAPTER pAd, IN PWSC_CTRL pWpsCtrl)
{
	PWSC_PEER_ENTRY pPeerEntry = NULL;
	...
	while (pPeerEntry != NULL) {
		if (RTMP_TIME_AFTER(now_time,
				    pPeerEntry->receive_time + (30 * OS_HZ))) {
...
VOID WscDelListEntryByMAC(IN PLIST_HEADER pWscEnList, IN PUCHAR pMacAddr)
{
	RT_LIST_ENTRY *pListEntry = NULL;
...
VOID WscAssignEntryMAC(IN PRTMP_ADAPTER pAd, IN PWSC_CTRL pWpsCtrl)
{
	PWSC_PEER_ENTRY pPeerEntry = NULL;
	...
	if (pPeerEntry)
		NdisMoveMemory(pWpsCtrl->EntryAddr, pPeerEntry->mac_addr,
			       MAC_ADDR_LEN);
...
*/
BOOLEAN WscGetDataFromPeerByTag(IN PRTMP_ADAPTER pAd, IN PUCHAR pIeData,
...
VOID WscUUIDInit(IN PRTMP_ADAPTER pAd, IN INT inf_idx, IN UCHAR from_apcli)
{
	...
	if (from_apcli) {
		PWSC_CTRL pWpsCtrl =
			&pAd->ApCfg.ApCliTab[inf_idx].wdev.WscControl;
#ifdef WSC_V2_SUPPORT
		...
		WscGenerateUUID(pAd, &pWpsCtrl->Wsc_Uuid_E[0],
				&pWpsCtrl->Wsc_Uuid_Str[0], 0, FALSE,
				from_apcli);
...
#endif /* APCLI_SUPPORT */
	{
		...
		if (pAd->ApCfg.MBSSID[inf_idx].Hostapd == Hostapd_EXT)
			MTWF_LOG(DBG_CAT_INIT, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
				 ("[ra%d] WPS is control by hostapd now.\n",
				  inf_idx));
		else
#endif /*HOSTAPD_SUPPORT*/
		{
			...
			pWscControl =
				&pAd->ApCfg.MBSSID[inf_idx].wdev.WscControl;
			MTWF_LOG(DBG_CAT_INIT, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				 ("Generate UUID for apidx(%d)\n", inf_idx));

			if (NdisEqualMemory(&pWscControl->Wsc_Uuid_E[0],
					    zeros16, UUID_LEN_HEX))
				WscGenerateUUID(pAd,
						&pWscControl->Wsc_Uuid_E[0],
						&pWscControl->Wsc_Uuid_Str[0],
						inf_idx, FALSE, from_apcli);
