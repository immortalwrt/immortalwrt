...
#ifndef __TDLS_UAPSD_H
#define __TDLS_UAPSD_H
...
#define TDLS_UAPSD_DIALOG_GET(__pAd, __Token)					\
	do {\
		__pAd->StaCfg[0].TdlsInfo.TdlsDialogToken++;			\
		if (__pAd->StaCfg[0].TdlsInfo.TdlsDialogToken == 0)		\
			__pAd->StaCfg[0].TdlsInfo.TdlsDialogToken++;		\
		__Token = __pAd->StaCfg[0].TdlsInfo.TdlsDialogToken;		\
	} while (0)
...
#define TDLS_UAPSD_ARE_WE_IN_PS(__pAd)										\
	(__pAd->StaCfg[0].PwrMgmt.Psm == PWR_SAVE)
...
#define TDLS_UAPSD_ENTRY_GET(__pAd, __LinkId)								\
	(&(__pAd)->StaCfg[0].TdlsInfo.TDLSEntry[__LinkId])
