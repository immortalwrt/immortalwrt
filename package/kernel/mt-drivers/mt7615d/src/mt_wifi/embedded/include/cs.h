...
#define CARRIER_DETECT_CRITIRIA(_pAd)                                          \
	((IS_RBUS_INF(_pAd)) ? CARRIER_DETECT_CRITIRIA_RBUS :                  \
				     CARRIER_DETECT_CRITIRIA_COMMON)
#else
...
typedef enum CD_STATE_n { CD_NORMAL, CD_SILENCE, CD_MAX_STATE } CD_STATE;
...
VOID ToneRadarProgram_v3(IN PRTMP_ADAPTER pAd, IN ULONG threshold);
