...
BOOLEAN FtmEntryDump(IN PRTMP_ADAPTER pAd, IN PFTM_PEER_ENTRY pEntry,
		     IN UINT32 DbgLvl);

VOID FtmPidPendingQDump(IN PRTMP_ADAPTER pAd, IN UINT32 DbgLvl);

UINT32 FtmMinDeltaToMS(IN UINT32 MinDelta);

UINT32 FtmBurstDurationToMS(IN UINT8 BurstDuration);

BOOLEAN FtmConvertTodToa(IN PRTMP_ADAPTER pAd, IN UINT64 *pTOD,
			 IN UINT64 *pTOA);
...
PFTM_PEER_ENTRY FtmGetPidPendingNode(IN PRTMP_ADAPTER pAd, IN UINT8 PID);
...
PFTM_PEER_ENTRY FtmEntrySearch(IN PRTMP_ADAPTER pAd, IN UINT8 *Addr);

PFTM_PEER_ENTRY FtmEntryGet(IN PRTMP_ADAPTER pAd, IN UINT8 *Addr);

VOID FtmEntryNegoDoneAction(IN PRTMP_ADAPTER pAd, IN PFTM_PEER_ENTRY pEntry);

BOOLEAN FtmEntryCntDownAction(IN PRTMP_ADAPTER pAd, IN PFTM_PEER_ENTRY pEntry);

BOOLEAN FtmEntryTerminate(IN PRTMP_ADAPTER pAd, IN PFTM_PEER_ENTRY pEntry,
			  IN BOOLEAN bTxFTM);
...
PFTM_MAPPING FtmSearchRxMappingNodeBySN(IN PRTMP_ADAPTER pAd,
					IN PFTM_PEER_ENTRY pEntry,
					IN UINT16 sn);

PFTM_MAPPING FtmGetRxMappingNodeByToken(IN PRTMP_ADAPTER pAd,
					IN PFTM_PEER_ENTRY pEntry,
					IN UINT8 DialogToken);

INT32 FtmRTTCalculate(IN UINT64 *pT1, IN UINT64 *pT2, IN UINT64 *pT3,
		      IN UINT64 *pT4);
...
INT Set_FtmReqTx_Proc(IN RTMP_ADAPTER *pAd, IN RTMP_STRING *arg);
...
VOID FtmMapSigmaCmdToLocLCI(IN RTMP_ADAPTER *pAd, IN RTMP_STRING *tmpbuf,
			    IN PLCI_FIELD pLci);

VOID FtmProfileNeighborApParse(IN RTMP_ADAPTER *pAd, IN UINT NeighborIdx,
