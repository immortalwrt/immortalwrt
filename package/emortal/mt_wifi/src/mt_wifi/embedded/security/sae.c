...
int SAE_PK_DEBUG_LEVEL = DBG_LVL_TRACE;
...
INT show_sae_info_proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_OFF, ("k iter varieble: %d, anti clogging th: %d, wpa3_test_ctrl: %d/%d\n",
	...
	for (i = 0; i < wtbl_max_num; i++) {
		...
		MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_OFF,
			 ("\tconnect_type=%d, is_pwd_id_only = %d, removable = %d, pwd_id = %s\n",
	...
	IN UCHAR sae_conn_type)
{
	...
	if (pSaeIns->pt != NULL)
		...
		MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_ERROR,
			 ("%s(): pSaeIns->pt is null, change to looping\n", __func__));
	...
	IN SAE_INSTANCE *pSaeIns)
{
	...
	if (!NdisEqualMemory(finger_print, sae_pk->finger_print, fp_len)) {
		MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_OFF, ("%s: public key verify fail\n", __func__));
	...
	IN SAE_INSTANCE *pSaeIns)
{
	...
	if (point)
		...
		res = SAE_SILENTLY_DISCARDED;
		MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_ERROR,
			("%s(): point is NULL, unexpected error !!\n", __func__));
		goto error;
	}
	MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, SAE_PK_DEBUG_LEVEL, ("r\n"));
	SAE_BN_PRINT_W_CAT_AND_LVL(r, DBG_CAT_SEC, CATSEC_SAE, SAE_PK_DEBUG_LEVEL);
	...
	IN UCHAR *end)
{
	...
	if (FALSE == aes_siv_decrypt(pSaeIns->kek, pSaeIns->kck_kek_len, cur_pos,
		...
		MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_ERROR, ("%s: aes_siv_decrypt fail\n", __func__));
		return SAE_SILENTLY_DISCARDED;
	}
	...
	IN UCHAR *buf)
{
	...
	if (2 + sae_pk->asn1_sub_pub_key_info_len > 255) {
		MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_ERROR,
			("%s: the length(%d) is larger than 255, it should do element fragmentation\n", __func__, 2 + sae_pk->asn1_sub_pub_key_info_len));
	}
	...
	IN UCHAR *buf)
{
	...
	struct sae_pk_cfg *sae_pk = pSaeIns->sae_pk_ptr;
	UCHAR encrypt_modifier[AES_BLOCK_SIZES + SAE_PK_MODIFIER_BYTES_LEN];

	if (sae_pk == NULL) {
		MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_OFF,
			("%s: unexpected error due to sae_pk_ptr is null\n", __func__));
		return len;
	}
	...
	OUT UCHAR *psk)
{
	...
	if (is_sae_group_ecc(sae_pk->group_id))
		...
		MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_ERROR,
			("%s(): sae_pk_init due to unknown group\n", __func__));
		return FALSE;
	}
}
	...
	INOUT UCHAR *psk)
{
	...
	if (sae_pk->fixed_pri_key) {
		...
		MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_ERROR, ("fix_pri_key\n"));
		...
		MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_ERROR, ("public key x\n"));
		SAE_BN_PRINT(pub->x);
		MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_ERROR, ("public key y\n"));
		SAE_BN_PRINT(pub->y);
		sae_pk->pub_key = pub;
	} else if (ecc_gen_key(ec_group_bi, &sae_pk->pri_key, &sae_pk->pub_key) == 0) {
		MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_ERROR,
	...
	if (sae_pk->sae_pk_test_ctrl & SAE_PK_CFG_FORCE_REDERIVE_PW)
		sae_pk_deri_finger_print(sae_pk, sae_pk->ssid, sae_pk->ssid_len, TRUE, finger_print);
	else
		sae_pk_deri_finger_print(sae_pk, sae_pk->ssid, sae_pk->ssid_len, FALSE, finger_print);

	if (sae_pk->sae_pk_test_ctrl & SAE_PK_CFG_FORCE_REDERIVE_PW) {
		MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_OFF, ("%s: the password is wrong, rederive the password\n", __func__));
		...
		MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_OFF, ("%s: the password is wrong\n", __func__));
		hex_dump_with_lvl("finger_print", finger_print, fp_len, DBG_LVL_OFF);
		hex_dump_with_lvl("expected finger_print (derived from pwd)", sae_pk->finger_print, fp_len, DBG_LVL_OFF);

		if (!(sae_pk->sae_pk_test_ctrl & SAE_PK_CFG_NO_REDERIVE_PW))
			return FALSE;
	}

	MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_OFF, ("\x1b[32m%s: password = %s\x1b[m\n", __func__, psk));
