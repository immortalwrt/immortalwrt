...
{
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IWSC - InvalidState (state=%ld, msg_type = %ld)\n",
...
VOID IWSC_Stop(RTMP_ADAPTER *pAd, BOOLEAN bSendNotification)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<----- IWSC_Stop\n"));
	...
	IN  PVOID SystemSpecific3)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("!!! IWSC_T1TimerAction !!!\n"));
	...
	if (pAd) {
		...
		if (pWpsCtrl) {
			...
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("!!! IWSC_T1TimerAction - pWpsCtrl is null !!!\n"));
	} else
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("!!! IWSC_T1TimerAction - pAd is null !!!\n"));
}
	...
	IN  PVOID SystemSpecific3)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("!!! IWSC_T2TimerAction - Become Enrollee !!!\n"));
	...
	if (pAd) {
		...
		if (pWpsCtrl) {
			...
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("!!! IWSC_T2TimerAction - pWpsCtrl is null !!!\n"));
	} else
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("!!! IWSC_T2TimerAction - pAd is null !!!\n"));
}
	...
	IN  PVOID SystemSpecific3)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("!!! IWSC_EntryTimerAction !!!\n"));
	...
	if (pAd) {
		...
		if (pIWscInfo) {
			...
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("!!! IWSC_EntryTimerAction - pIWscInfo is null !!!\n"));
	} else
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("!!! IWSC_EntryTimerAction - pAd is null !!!\n"));
}
	...
	IN  PVOID SystemSpecific3)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("!!! IWSC_DevQueryAction !!!\n"));
	...
	if (pAd) {
		...
		if (pIWscInfo) {
			...
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("!!! IWSC_DevQueryAction - pIWscInfo is null !!!\n"));
	} else
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("!!! IWSC_DevQueryAction - pAd is null !!!\n"));
}
...
#endif /* IWSC_TEST_SUPPORT // */
		{
			...
			if (STA_STATUS_TEST_FLAG(pStaCfg, fSTA_STATUS_MEDIA_STATE_CONNECTED)) {
				...
				if ((pAd->StaCfg[0].IWscInfo.bSinglePIN == FALSE)
					...
					MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RandomTime = %d\n", RandomTime));
...
VOID IWSC_MlmeStopAction(RTMP_ADAPTER *pAd, PMLME_QUEUE_ELEM pElem)
{
	...
	if (pAd->StaCfg[0].IWscInfo.bSinglePIN &&
		...
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("SinglePIN registrar now, keep going!!\n"));
...
VOID IWSC_ScanDoneAction(RTMP_ADAPTER *pAd, PMLME_QUEUE_ELEM pElem)
{
	...
	if (pAd->Mlme.IWscMachine.CurrState != IWSC_SCAN) {
		if (pAd->Mlme.IWscMachine.CurrState != IWSC_IDLE)
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IWSC_ScanDoneAction:: CurrState = %ld\n", pAd->Mlme.IWscMachine.CurrState));

		return;
	}

	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("-----> IWSC_ScanDoneAction\n"));
...
VOID IWSC_BuildSelRegStartNotification(RTMP_ADAPTER *pAd, UCHAR *pOutBuf, USHORT *pIeLen)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("-----> IWSC_BuildSelRegStartNotification\n"));
	/* WSC IE HEader */
	ieHdr.elemId = 221;
	ieHdr.length = 4;
	ieHdr.oui[0] = 0x00;
	...
	OUT	PUSHORT			pIeLen)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("-----> IWSC_BuildSelRegFinishNotification\n"));
	/* WSC IE HEader */
	ieHdr.elemId = 221;
	ieHdr.length = 4;
	ieHdr.oui[0] = 0x00;
...
VOID IWSC_SendActionFrame(RTMP_ADAPTER *pAd, UCHAR FrameType)
{
	...
	if (NStatus != NDIS_STATUS_SUCCESS) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IWSC_SendStartNotification:: allocate memory failed\n"));
		return;
	}
	...
	if (pWscBuf != NULL) {
		...
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s:: WscBuf Allocate failed!\n", __func__));
...
VOID IWSC_PeerAction(RTMP_ADAPTER *pAd, PMLME_QUEUE_ELEM pElem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("-----> IWSC_PeerAction\n"));
	/*
	 ...
	 */
	{
		...
		for (;;) {
			...
			if (TotalLen <= 0) {
				MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("No IWSC IE!\n<----- IWSC_PeerAction\n"));
				return;
			}
		...
		if (FrameType == IWSC_SEL_REG_START_NOTITY) {
			...
			if (pEntry) {
				...
				if ((pAd->StaCfg[0].wdev.AuthMode == Ndis802_11AuthModeWPA2PSK)
					...
					if (pEntry->bPeerHigherMAC == FALSE) {
						 ...
						 */
						MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s - EnqueueStartForPSKTimer.\n", __func__));
			...
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Unknow Frame Type!\n"));
	}
...
VOID IWSC_PeerProbeRequest(RTMP_ADAPTER *pAd, PMLME_QUEUE_ELEM pElem)
{
	...
	if (pWscCtrl->WscConfMode != WSC_REGISTRAR) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("IWSC_PeerProbeRequest:: Not Registrar now(WscConfMode = %d)\n"
				 , pWscCtrl->WscConfMode));
		return;
	}

	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<----- IWSC_PeerProbeRequest\n"));
	...
	if (pWscCtrl->WscMode == WSC_PIN_MODE) {
		if (PeerConfigMethod & WPS_CONFIG_METHODS_KEYPAD) {
			...
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IWSC_PeerProbeRequest:: WscPinCode = %08d\n", pWscCtrl->WscPinCode));
		} else {
			if (pAd->StaCfg[0].IWscInfo.bLimitedUI) {
				...
				MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IWSC_PeerProbeRequest:: Please input Peer's PIN\n"));
			}
		}
...
VOID IWSC_PeerProbeResponse(RTMP_ADAPTER *pAd, PMLME_QUEUE_ELEM pElem)
{
	...
	if (pWscCtrl->WscConfMode != WSC_ENROLLEE) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("IWSC_PeerProbeResponse:: Not Enrollee now(WscConfMode = %d)\n"
				 , pWscCtrl->WscConfMode));
		return;
	}

	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<----- IWSC_PeerProbeResponse\n"));
	...
	if (RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_BSS_SCAN_IN_PROGRESS)) {
		...
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Cancel Scan Timer\n"));
	}
	...
	if (pAd->StaCfg[0].IWscInfo.bLimitedUI) {
		if (PeerConfigMethod & WPS_CONFIG_METHODS_KEYPAD) {
			if (pIWscInfo->bSinglePIN == FALSE) {
				...
				pWscCtrl->WscEnrolleePinCodeLen = 8;
				pWscCtrl->WscPinCodeLen = 8;
				MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IWSC_PeerProbeResponse:: WscPinCode = %d\n", pWscCtrl->WscPinCode));
				pWscCtrl->WscStatus = STATUS_WSC_IBSS_NEW_RANDOM_PIN;
			} else
				pWscCtrl->WscStatus = STATUS_WSC_IBSS_FIXED_PIN;
		...
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IWSC_PeerProbeResponse:: Please input Peer's PIN\n"));
...
VOID IWSC_PeerPIN(RTMP_ADAPTER *pAd, PMLME_QUEUE_ELEM pElem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("-----> IWSC_PeerPIN\n"));
...
BOOLEAN	IWSC_PeerEapolStart(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry, MLME_QUEUE_ELEM *Elem)
{
	...
	if (pWpsCtrl->WscConfMode != WSC_REGISTRAR) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IWSC_PeerEapolStart:: Not Registrar now(WscConfMode = %d)\n", pWpsCtrl->WscConfMode));
		return FALSE;
	}

	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<----- IWSC_PeerEapolStart\n"));
	...
	if (pWpsCtrl->WscMode == WSC_PBC_MODE) {
		...
		if (pAd->CommonCfg.WscPBCOverlap == TRUE) {
			...
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IWSC_PeerEapolStart: PBC Session Overlap!\n"));
			IWSC_MlmeStopAction(pAd, NULL);
			return FALSE;
		}
	} else if (pWpsCtrl->WscMode == WSC_SMPBC_MODE) {
		if (pEntry->bIWscSmpbcAccept == FALSE) {
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IWSC_PeerEapolStart:: pEntry->bIWscSmpbcAccept == FALSE\n"));
			return FALSE;
		}

		if (pWpsCtrl->EapMsgRunning == TRUE) {
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IWSC_PeerEapolStart:: Busy now\n"));
			return FALSE;
		}
...
VOID IWSC_AddSmpbcEnrollee(RTMP_ADAPTER *pAd, UCHAR *pPeerAddr)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<----- IWSC_AddSmpbcEnrollee (WscConfMode = %d, WscStatus = %d)\n",
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("-----> IWSC_AddSmpbcEnrollee\n"));
}
...
VOID IWSC_BuildDevQueryFrame(RTMP_ADAPTER *pAd, UCHAR *pOutBuf, USHORT *pIeLen)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("-----> IWSC_BuildDevQueryFrame\n"));
	/* WSC IE HEader */
	ieHdr.elemId = 221;
	ieHdr.length = 4;
	ieHdr.oui[0] = 0x00;
...
BOOLEAN IWSC_IpContentForCredential(RTMP_ADAPTER *pAd)
{
	...
	if (pIWscInfo->IpDevCount >= IWSC_MAX_IP_DEV_COUNT) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("pIWscInfo->IpDevCount = %d\n",
				 pIWscInfo->IpDevCount));
		return FALSE;
	}

	if (pIWscInfo->bAssignWscIPv4 == FALSE) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("pIWscInfo->bAssignWscIPv4 = %d\n",
	...
	if (pIWscInfo->RegDepth == 0) {
		...
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("1) SelfIpv4Addr = 0x%08x, PeerIpv4Addr = 0x%08x\n",
		...
		if (pIWscInfo->SelfIpv4Addr == 0) {
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IMPOSSIBLE!! pIWscInfo->SelfIpv4Addr = %d\n", pIWscInfo->SelfIpv4Addr));
			return FALSE;
		}
		...
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("2) SelfIpv4Addr = 0x%08x, PeerIpv4Addr = 0x%08x\n",
				 pIWscInfo->SelfIpv4Addr,
				 pIWscInfo->PeerIpv4Addr));
		return TRUE;
	}
