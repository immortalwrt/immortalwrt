...
#define MBO_ATTR_MAX_LEN                                                       \
	252 /* spec 0.0.23 - IE LEN is 256 = OUI 4 + ATTR 252 */
#define MBO_NEIGHBOR_REPORT_MAX_LEN                                            \
	2304 /* 11v spec define BTM candidate list max len = 2304 */
...
#define MBO_ATTR_STA_NOT_PREFER_CH_REP_2ND                                     \
	99 /* enable sta to insert 2nd NPC attr in assoc, need to mark attr id valid check for test */
...
#define MBO_AP_DISALLOW_REASON_RESERVED                                        \
	0x0 /* allow assoc , no disallow attr in MBO IE */
...
#define MBO_AP_CDCP_PREFER_STA_USE_CDC                                         \
	0xFF /* 255 - prefer cellular data connection */
...
#define MBO_AP_TRANS_REASON_TOO_MANY_RETRY                                     \
	0x6 /* Received excessive number of retransmissions */
...
#define IS_MBO_ENABLE(_wdev) ((_wdev)->MboCtrl.bMboEnable == TRUE)

#define VALID_MBO_ATTR_ID(_I) (_I <= MBO_WDEV_ATTR_MAX_NUM)

#define MBO_AP_ALLOW_ASSOC(_wdev)                                              \
	((_wdev)->MboCtrl.AssocDisallowReason ==                               \
	 MBO_AP_DISALLOW_REASON_RESERVED)

VOID MakeMboOceIE(PRTMP_ADAPTER pAd, struct wifi_dev *wdev,
		  struct _MAC_TABLE_ENTRY *pEntry, PUINT8 pFrameBuf,
		  PULONG pFrameLen, UINT8 FrameType);
...
VOID MboBssTermCountDownStart(UINT8 validity_intvl);
