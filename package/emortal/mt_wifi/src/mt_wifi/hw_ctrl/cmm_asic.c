...
VOID AsicSwitchChannel(RTMP_ADAPTER *pAd, UCHAR band_idx, struct freq_oper *oper, BOOLEAN bScan)
{
	...
	if (IS_HIF_TYPE(pAd, HIF_MT)) {
		...
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			("\x1b[41m%s(): RxStream[%d], RxPath[0x%x]\x1b[m\n", __func__, SwChCfg.RxStream,
		...
		if (pAd->CommonCfg.dbdc_mode) {
			...
			MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			("\x1b[41m%s(): DBDC RxStream[%d]\x1b[m\n", __func__, SwChCfg.RxStream));
...
VOID asic_write_tmac_info_beacon(struct _RTMP_ADAPTER *pAd,
	struct wifi_dev *wdev, UCHAR *tmac_buf, HTTRANSMIT_SETTING *beaconTransmit, ULONG frmLen)
{
	struct _RTMP_ARCH_OP *arch_ops = hc_get_arch_ops(pAd->hdev_ctrl);

	if (arch_ops->write_tmac_info_beacon)
		arch_ops->write_tmac_info_beacon(pAd, wdev, tmac_buf, beaconTransmit, frmLen);
	else
		AsicNotSupportFunc(pAd, __func__);
}
	...
	UINT8 ucPmState)
{
	...
	if (!wdev) {
		MTWF_LOG(DBG_CAT_PS, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			("%s:wdev(NULL) for PM=%d, State=%d\n", __func__, ucPmNumber, ucPmState));
		return 0;
	}

	if (wdev->wdev_type != WDEV_TYPE_STA) {
		MTWF_LOG(DBG_CAT_PS, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			("%s:wdev_type(0x%x)!=STA,return!\n", __func__, wdev->wdev_type));
		return 0;
	}
