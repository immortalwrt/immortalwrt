			    ...
			    OUT STATE_MACHINE_FUNC Trans[])
{
	...
	StateMachineSetAction(S, ACT_IDLE, FT_CATEGORY_BSS_TRANSITION,
			      (STATE_MACHINE_FUNC)FT_FtAction);
	...
	StateMachineSetAction(S, ACT_IDLE, CATEGORY_WNM,
			      (STATE_MACHINE_FUNC)PeerWNMAction);
#endif /* CONFIG_DOT11V_WNM */
	StateMachineSetAction(S, ACT_IDLE, CATEGORY_PD,
			      (STATE_MACHINE_FUNC)PeerProtectedDualAction);
}
...
VOID MlmeADDBAAction(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	if ((MlmeAddBAReqSanity(pAd, Elem->Msg, Elem->MsgLen, Addr)) &&
		...
		if (IS_ENTRY_REPEATER(pEntry) && (pEntry->bReptCli == TRUE)) {
			ActHeaderInit(
				pAd, &Frame.Hdr, pInfo->pAddr,
				pAd->ApCfg
					.pRepeaterCliPool[pEntry->MatchReptCliIdx]
					.CurrentAddress,
				pInfo->pAddr);
		} else
		...
		Frame.BaStartSeq.field.StartSeq =
			AsicGetTidSn(pAd, pInfo->Wcid, pInfo->TID);
		...
		*(USHORT *)(&(Frame.BaParm)) =
		...
		MiniportMMRequest(
			pAd, (MGMT_USE_QUEUE_FLAG | WMM_UP2AC_MAP[pInfo->TID]),
			pOutBuffer, FrameLen);
		MlmeFreeMemory(pOutBuffer);
		MTWF_LOG(
			DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
...
VOID MlmeDELBAAction(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	if ((MlmeDelBAReqSanity(pAd, Elem->Msg, Elem->MsgLen)) &&
	    (VALID_UCAST_ENTRY_WCID(pAd, pInfo->Wcid))) {
		if (MlmeAllocateMemory(pAd, &pOutBuffer) !=
		    NDIS_STATUS_SUCCESS) {
			MTWF_LOG(
				DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
		...
		if (MlmeAllocateMemory(pAd, &pOutBuffer2) !=
		    NDIS_STATUS_SUCCESS) {
			MlmeFreeMemory(pOutBuffer);
			MTWF_LOG(
				DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
		...
		/* SEND DELBA FRAME*/
		FrameLen = 0;
		ActHeaderInit(pAd, &Frame.Hdr, pEntry->Addr, src_addr,
			      pEntry->wdev->bssid);
		Frame.Category = CATEGORY_BA;
		Frame.Action = DELBA;
...
VOID MlmeQOSAction(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	if (NStatus != NDIS_STATUS_SUCCESS) {
		MTWF_LOG(DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			 ("ACT - MlmeQOSAction() allocate memory failed\n"));
		return;
	}
	...
	if (pInfo->ActionField == ACTION_QOSMAP_CONFIG) {
		...
		UCHAR ielen = 0;
		ActHeaderInit(pAd, &Frame.Hdr, pInfo->Addr, mbss->wdev.bssid,
			      mbss->wdev.bssid);
		Frame.Category = CATEGORY_QOS;
		Frame.Action = ACTION_QOSMAP_CONFIG;
		ielen = pQosMapBuf->DSCP_Field_Len;
		MakeOutgoingFrame(pOutBuffer, &FrameLen,
				  sizeof(FRAME_ACTION_HDR), &Frame, 1,
				  &OosMapIE, 1, &ielen, ielen,
				  pQosMapBuf->DSCP_Field, END_OF_ARGS);
		MiniportMMRequest(pAd, QID_AC_BE, pOutBuffer, FrameLen);
	}
...
extern UCHAR get_regulatory_class(IN PRTMP_ADAPTER pAd, UCHAR Channel,
				  UCHAR PhyMode, struct wifi_dev *wdev);
				  ...
				  IN UCHAR apidx, IN UCHAR InfoReq)
{
	...
	MTWF_LOG(
		...
		("SendBSS2040CoexistMgmtAction(): Wcid=%d, apidx=%d, InfoReq=%d!\n",
		 Wcid, apidx, InfoReq));
	NdisZeroMemory((PUCHAR)&BssCoexistInfo,
		       sizeof(BSS_2040_COEXIST_ELEMENT));
	NdisZeroMemory((PUCHAR)&BssIntolerantInfo,
	...
	BssCoexistInfo.BssCoexistIe.word =
		pAd->CommonCfg.LastBSSCoexist2040.word;
	...
	BssIntolerantInfo.RegulatoryClass =
	...
	MakeOutgoingFrame(pOutBuffer, &FrameLen, sizeof(FRAME_ACTION_HDR),
			  &Frame, sizeof(BSS_2040_COEXIST_ELEMENT),
			  &BssCoexistInfo,
			  sizeof(BSS_2040_INTOLERANT_CH_REPORT),
			  &BssIntolerantInfo, END_OF_ARGS);
	MiniportMMRequest(pAd, QID_AC_BE, pOutBuffer, FrameLen);
	MlmeFreeMemory(pOutBuffer);
	MTWF_LOG(DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
	...
	for (k = 0; k < 7; k++) {
		...
		for (i = 0; i < pAd->CommonCfg.TriggerEventTab.EventANo; i++) {
			if (pAd->CommonCfg.TriggerEventTab.EventA[i].bValid ==
			    TRUE) {
				if (pAd->CommonCfg.TriggerEventTab.EventA[i]
					    .RegClass == TmpRegClass) {
					for (j = 0; j < idx; j++) {
						if (ChannelList[j] ==
						    (UCHAR)pAd->CommonCfg
							    .TriggerEventTab
					...
					pAd->CommonCfg.TriggerEventTab.EventA[i]
						.bValid = FALSE;
				}
		...
		if (idx > 0) {
			...
			*(pDest + ReadOffset) =
				IE_2040_BSS_INTOLERANT_REPORT; /* IE*/
			*(pDest + ReadOffset +
			  1) = 1 + idx; /* Len = RegClass byte + channel byte.*/
			*(pDest + ReadOffset +
			  2) = TmpRegClass; /* Len = RegClass byte + channel byte.*/
...
/*
	...
	==========================================================================
 */
VOID Update2040CoexistFrameAndNotify(IN PRTMP_ADAPTER pAd, IN UCHAR Wcid,
				     IN BOOLEAN bAddIntolerantCha)
{
	BSS_2040_COEXIST_IE OldValue;
	MTWF_LOG(DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
		 ("%s(): ACT -BSSCoexist2040 = %x. EventANo = %d.\n", __func__,
...
BOOLEAN ChannelSwitchSanityCheck(IN PRTMP_ADAPTER pAd, IN UCHAR Wcid,
...
VOID ChannelSwitchAction(IN PRTMP_ADAPTER pAd, IN UCHAR Wcid,
			 IN UCHAR NewChannel, IN UCHAR Secondary)
{
	struct wifi_dev *wdev = NULL;
...
VOID PeerPublicAction(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	case ACTION_BSS_2040_COEXIST: { /* Format defined in IEEE 7.4.7a.1 in 11n Draf3.03*/
		...
		pCoexistInfo = (BSS_2040_COEXIST_ELEMENT *)&Elem
				       ->Msg[LENGTH_802_11 + 2];
		...
		pBssCoexistIe =
			(BSS_2040_COEXIST_IE *)(&pCoexistInfo->BssCoexistIe);
#ifdef CONFIG_AP_SUPPORT
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		{
			...
			if (IS_ENTRY_CLIENT(&pAd->MacTab.Content[Elem->Wcid])) {
				...
				UCHAR BandIdx = BAND0;
				CHANNEL_CTRL *pChCtrl = hc_get_channel_ctrl(
					pAd->hdev_ctrl, BandIdx);
				if (wdev)
					BandIdx = HcGetBandByWdev(wdev);
				...
				if (bNeedFallBack) {
					...
					apidx = pAd->MacTab.Content[Elem->Wcid]
							.apidx;
...
VOID PeerHTAction(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	case NOTIFY_BW_ACTION:
		...
		if (Elem->Msg[LENGTH_802_11 + 2] ==
		    0) /* 7.4.8.2. if value is 1, keep the same as supported channel bandwidth. */
			pEntry->HTPhyMode.field.BW = HT_BW_20;
		else {
			pEntry->HTPhyMode.field.BW =
				(pEntry->wdev) ?
					(pEntry->MaxHTPhyMode.field.BW &
					 (wlan_config_get_ht_bw(pEntry->wdev))) :
					      (pEntry->MaxHTPhyMode.field.BW);
		}
...
VOID PeerVHTAction(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	case ACT_VHT_OPMODE_NOTIFY: {
		OPERATING_MODE *op_mode =
			(OPERATING_MODE *)&Elem->Msg[LENGTH_802_11 + 2];
...
VOID ORIBATimerTimeout(RTMP_ADAPTER *pAd)
{
	...
	for (i = 1; ((i < MAX_LEN_OF_BA_ORI_TABLE) && (total > 0)); i++) {
		if (pAd->BATable.BAOriEntry[i].ORI_BA_Status ==
		    Originator_Done) {
			pEntry = &pAd->MacTab.Content[pAd->BATable.BAOriEntry[i]
							      .Wcid];
...
VOID SendRefreshBAR(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry)
{
	...
	for (i = 0; i < NUM_OF_TID; i++) {
		...
		if (pBAEntry->ORI_BA_Status == Originator_Done) {
			...
			NStatus = MlmeAllocateMemory(
				pAd,
				&pOutBuffer); /*Get an unused nonpaged memory*/
			...
			FrameBar.StartingSeq.field.FragNum =
				0; /* make sure sequence not clear in DEL function.*/
			FrameBar.StartingSeq.field.StartSeq =
				Sequence; /* make sure sequence not clear in DEL funciton.*/
			FrameBar.BarControl.TID =
				TID; /* make sure sequence not clear in DEL funciton.*/
			MakeOutgoingFrame(pOutBuffer, &FrameLen,
					  sizeof(FRAME_BAR), &FrameBar,
					  END_OF_ARGS);
			MiniportMMRequest(
				pAd, (MGMT_USE_QUEUE_FLAG | WMM_UP2AC_MAP[TID]),
				pOutBuffer, FrameLen);
			MlmeFreeMemory(pOutBuffer);
		}
	}
...
VOID ActHeaderInit(IN RTMP_ADAPTER *pAd, IN OUT PHEADER_802_11 pHdr80211,
...
/*
	...
	==========================================================================
 */
VOID InsertActField(IN PRTMP_ADAPTER pAd, OUT PUCHAR pFrameBuf,
		    OUT PULONG pFrameLen, IN UINT8 Category, IN UINT8 ActCode)
{
	ULONG TempLen;
	MakeOutgoingFrame(pFrameBuf, &TempLen, 1, &Category, 1, &ActCode,
			  END_OF_ARGS);
...
VOID SendSMPSAction(RTMP_ADAPTER *pAd, UCHAR Wcid, UCHAR smps)
{
...
#endif /* APCLI_SUPPORT */
		ActHeaderInit(pAd, &Frame.Hdr, pEntry->Addr, wdev->if_addr,
			      wdev->bssid);
	...
	MakeOutgoingFrame(pOutBuffer, &FrameLen, sizeof(FRAME_SMPS_ACTION),
			  &Frame, END_OF_ARGS);
	MiniportMMRequest(pAd, QID_AC_BE, pOutBuffer, FrameLen);
	MlmeFreeMemory(pOutBuffer);
	MTWF_LOG(DBG_CAT_PROTO, CATPROTO_BA, DBG_LVL_ERROR,
...
VOID SendBeaconRequest(RTMP_ADAPTER *pAd, UCHAR Wcid)
{
	...
	MeasureReg.Length =
		sizeof(MEASUREMENT_REQ) + sizeof(BEACON_REQUEST) - 2;
	MakeOutgoingFrame(pOutBuffer, &FrameLen, sizeof(FRAME_RM_REQ_ACTION),
