...
static VOID TYPE_FUNC FT_RIC_CMD_StatusDisplay(FT_RIC_STATUS *pRspStatus)
{
	...
	if (pRspStatus->FlgHasBaResource == TRUE) {
		if (pRspStatus->FlgIsBaAccepted == TRUE)
			MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_OFF, ("\tBA Resource is accepted.\n"));
		else
			MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_OFF, ("\tBA Resource is not accepted.\n")); /* End of if */
	} /* End of if */

	if (pRspStatus->TspecNumberOfRequested > 0) {
		MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_OFF, ("\tNumber of requested TSPEC: %d\n",
				 pRspStatus->TspecNumberOfRequested));
		MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_OFF, ("\tNumber of accepted TSPEC: %d\n",
				 pRspStatus->TspecNumberOfAccepted));
		MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_OFF, ("\tTotal requested Time: %d us\n",
				 pRspStatus->TspecMediumTimeTotalCur));
		MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_OFF, ("\tTotal accepted Time: %d us\n",
				 pRspStatus->TspecMediumTimeTotalNew));
	} /* End of if */

	if (pRspStatus->UnknownRDIE > 0) {
		MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_OFF, ("\tNumber of unknown RDIE: %d\n",
...
INT TYPE_FUNC FT_Ioctl(RTMP_ADAPTER * pAd, RTMP_STRING *pArgvIn)
{
	...
	case FT_RIC_CMD_SM_REQ_START:
		MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_TRACE, ("ft_iocl> (sm) Resource Request Start!\n"));
		FT_RIC_CMD_SimRscReqStart(pAd, Argc, pArgv);
		break;

	case FT_RIC_CMD_SM_REQ:
		MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_TRACE, ("ft_iocl> (sm) Resource Request!\n"));
		FT_RIC_CMD_SimRscReq(pAd, Argc, pArgv);
		break;

	case FT_RIC_CMD_SM_REQ_END:
		MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_TRACE, ("ft_iocl> (sm) Resource Request End!\n"));
		FT_RIC_CMD_SimRscReqEnd(pAd, Argc, pArgv);
		break;

	case FT_RIC_CMD_SM_REQ_RSP_LIST:
		MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_TRACE, ("ft_iocl> (sm) Resource Req/Rsp List!\n"));
