...
static struct vht_ch_layout vht_ch_40M[] = {
	...
	{ 140, 144, 142 }, { 149, 153, 151 }, { 157, 161, 159 },
	{ 0, 0, 0 },
};
#endif
...
VOID trigger_vht_ndpa(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *entry)
{
	...
	MTWF_LOG(DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("Send VHT NDPA Frame to STA(%02x:%02x:%02x:%02x:%02x:%02x)\n",
...
INT vht_mode_adjust(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry, VHT_CAP_IE *cap,
		    VHT_OP_IE *op)
{
		...
		if (ht_phyinfo) {
			MTWF_LOG(
				...
				("%s: DesiredHtPhyInfo->vht_bw=%d, ch_width=%d\n",
				 __func__, ht_phyinfo->vht_bw,
				 cap->vht_cap.ch_width));

			if ((ht_phyinfo->vht_bw == VHT_BW_2040)) {
				pEntry->MaxHTPhyMode.field.ShortGI =
					(vht_sgi & (cap->vht_cap.sgi_80M));
				pEntry->MaxHTPhyMode.field.STBC =
					((wlan_config_get_vht_stbc(
						  pEntry->wdev) &
					  cap->vht_cap.rx_stbc) > 1 ?
						 1 :
						       0);
			} else if ((ht_phyinfo->vht_bw >= VHT_BW_80) &&
				   (cap->vht_cap.ch_width == 0)) {
				if (op != NULL) {
					if (op->vht_op_info.ch_width ==
					    0) /* peer support VHT20,40 */
				...
				pEntry->MaxHTPhyMode.field.STBC =
					((wlan_config_get_vht_stbc(
					  ...
					  cap->vht_cap.rx_stbc) > 1 ?
						 1 :
						       0);
			} else if (((ht_phyinfo->vht_bw == VHT_BW_160) ||
				    (ht_phyinfo->vht_bw == VHT_BW_8080)) &&
				   (cap->vht_cap.ch_width != 0)) {
				/* bw160 or bw80+80 */
				pEntry->MaxHTPhyMode.field.BW = BW_160;
				pEntry->MaxHTPhyMode.field.ShortGI =
					(vht_sgi & (cap->vht_cap.sgi_160M));
				pEntry->MaxHTPhyMode.field.STBC =
					((wlan_config_get_vht_stbc(
						  pEntry->wdev) &
					  cap->vht_cap.rx_stbc) > 1 ?
...
VOID set_vht_cap(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *entry,
		 VHT_CAP_IE *vht_cap_ie)
{
	struct _RTMP_CHIP_CAP *cap = hc_get_chip_cap(pAd->hdev_ctrl);

	if (wlan_config_get_vht_ldpc(entry->wdev) &&
	    (cap->phy_caps & fPHY_CAP_LDPC) && (vht_cap_ie->vht_cap.rx_ldpc))
	...
	if (wlan_config_get_vht_stbc(entry->wdev)) {
		if (vht_cap_ie->vht_cap.tx_stbc)
			CLIENT_STATUS_SET_FLAG(
				entry, fCLIENT_STATUS_VHT_TXSTBC_CAPABLE);

		if (vht_cap_ie->vht_cap.rx_stbc)
			CLIENT_STATUS_SET_FLAG(
				entry, fCLIENT_STATUS_VHT_RXSTBC_CAPABLE);
	}

	entry->MaxRAmpduFactor =
		(vht_cap_ie->vht_cap.max_ampdu_exp > entry->MaxRAmpduFactor) ?
			vht_cap_ie->vht_cap.max_ampdu_exp :
			      entry->MaxRAmpduFactor;
	entry->AMsduSize = vht_cap_ie->vht_cap.max_mpdu_len;

	if (entry->AMsduSize <
	    (sizeof(vht_max_mpdu_size) / sizeof(vht_max_mpdu_size[0])))
				  ...
				  HTTRANSMIT_SETTING *tx)
{
	...
	if ((vht_cap->mcs_set.rx_mcs_map.mcs_ss2 != VHT_MCS_CAP_NA) &&
	    (wlan_operate_get_tx_stream(wdev) >= 2)) {
		nss = 2;
		spec_cap = dot11_vht_mcs_bw_cap[cap_offset + (nss - 1)];
	...
	if ((vht_cap->mcs_set.rx_mcs_map.mcs_ss3 != VHT_MCS_CAP_NA) &&
	    (wlan_operate_get_tx_stream(wdev) >= 3)) {
		nss = 3;
		spec_cap = dot11_vht_mcs_bw_cap[cap_offset + (nss - 1)];
	...
	if ((vht_cap->mcs_set.rx_mcs_map.mcs_ss4 != VHT_MCS_CAP_NA) &&
	    (wlan_operate_get_tx_stream(wdev) >= 4)) {
		nss = 4;
		spec_cap = dot11_vht_mcs_bw_cap[cap_offset + (nss - 1)];
...
/*
	...
	Appeared in Beacon, ProbResp frames
*/
INT build_vht_txpwr_envelope(RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
			     UCHAR *buf)
{
	INT len = 0, pwr_cnt;
...
*/
INT build_vht_op_ie(RTMP_ADAPTER *pAd, UCHAR bw, UCHAR Channel,
		    struct wifi_dev *wdev, UCHAR *buf)
{
	...
	if (Channel > 14 && (pAd->CommonCfg.bIEEE80211H == 1) &&
	...
	if ((wlan_config_get_vht_bw(wdev) == VHT_BW_160 ||
	     wlan_config_get_vht_bw(wdev) == VHT_BW_8080) &&
...
INT build_vht_cap_ie(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, UCHAR *buf)
{
	...
	if ((mcs_cap <= VHT_MCS_CAP_9) && ((rx_nss > 0) && (rx_nss <= 4)) &&
	    ((tx_nss > 0) && (tx_nss <= 4))) {
		/* BW 80M is the default setting*/
		vht_cap_ie.mcs_set.rx_high_rate =
			VHT_HIGH_RATE_BW80[mcs_cap][rx_nss - 1];
		vht_cap_ie.mcs_set.tx_high_rate =
			VHT_HIGH_RATE_BW80[mcs_cap][tx_nss - 1];
		if (rx_nss >= 1)
		...
		if (wlan_config_get_vht_bw(wdev) == VHT_BW_2040 &&
			...
			vht_cap_ie.mcs_set.rx_high_rate =
				VHT_HIGH_RATE_BW20[mcs_cap][rx_nss - 1];
			vht_cap_ie.mcs_set.tx_high_rate =
				VHT_HIGH_RATE_BW20[mcs_cap][rx_nss - 1];
			...
			if (mcs_cap == VHT_MCS_CAP_9) {
				if (vht_cap_ie.mcs_set.rx_high_rate == 0)
					vht_cap_ie.mcs_set.rx_high_rate =
						VHT_HIGH_RATE_BW20[VHT_MCS_CAP_8]
								  [rx_nss - 1];
				...
				if (rx_nss >= 1)
					vht_cap_ie.mcs_set.rx_mcs_map.mcs_ss1 =
						VHT_MCS_CAP_8;

				if (rx_nss >= 2)
					vht_cap_ie.mcs_set.rx_mcs_map.mcs_ss2 =
						VHT_MCS_CAP_8;

				if (rx_nss >= 3)
					vht_cap_ie.mcs_set.rx_mcs_map.mcs_ss3 =
						VHT_MCS_CAP_9;

				if (rx_nss >= 4)
					vht_cap_ie.mcs_set.rx_mcs_map.mcs_ss4 =
						VHT_MCS_CAP_8;

				if (tx_nss >= 1)
					vht_cap_ie.mcs_set.tx_mcs_map.mcs_ss1 =
						VHT_MCS_CAP_8;

				if (tx_nss >= 2)
					vht_cap_ie.mcs_set.tx_mcs_map.mcs_ss2 =
						VHT_MCS_CAP_8;

				if (tx_nss >= 3)
					vht_cap_ie.mcs_set.tx_mcs_map.mcs_ss3 =
						VHT_MCS_CAP_9;

				if (tx_nss >= 4)
					vht_cap_ie.mcs_set.tx_mcs_map.mcs_ss4 =
						VHT_MCS_CAP_8;
			}
			...
			vht_cap_ie.mcs_set.rx_high_rate =
				VHT_HIGH_RATE_BW40[mcs_cap][rx_nss - 1];
			vht_cap_ie.mcs_set.tx_high_rate =
				VHT_HIGH_RATE_BW40[mcs_cap][rx_nss - 1];
			...
			vht_cap_ie.mcs_set.rx_high_rate =
				VHT_HIGH_RATE_BW160_BW8080[mcs_cap][rx_nss - 1];
			vht_cap_ie.mcs_set.tx_high_rate =
				VHT_HIGH_RATE_BW160_BW8080[mcs_cap][rx_nss - 1];
		...
		if (wdev->rate.Eap_VhtSupRate_En == TRUE) {
			if (rx_nss >= 1)
				vht_cap_ie.mcs_set.rx_mcs_map.mcs_ss1 =
					wdev->rate.rx_mcs_map.mcs_ss1;

			if (rx_nss >= 2)
				vht_cap_ie.mcs_set.rx_mcs_map.mcs_ss2 =
					wdev->rate.rx_mcs_map.mcs_ss2;

			if (rx_nss >= 3)
				vht_cap_ie.mcs_set.rx_mcs_map.mcs_ss3 =
					wdev->rate.rx_mcs_map.mcs_ss3;

			if (rx_nss >= 4)
				vht_cap_ie.mcs_set.rx_mcs_map.mcs_ss4 =
					wdev->rate.rx_mcs_map.mcs_ss4;
...
static INT build_vht_op_mode_ie(RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
				UCHAR *buf)
{
	INT len = 0;
	EID_STRUCT eid_hdr;
	...
	buf += len;
	NdisMoveMemory(buf, (UCHAR *)&operating_mode_ie,
...
#ifdef G_BAND_256QAM
static BOOLEAN g_band_256_qam_enable(struct _RTMP_ADAPTER *pAd,
				     struct _build_ie_info *info)
{
	struct _RTMP_CHIP_CAP *cap = hc_get_chip_cap(pAd->hdev_ctrl);

	if ((pAd->CommonCfg.g_band_256_qam) && (cap->g_band_256_qam) &&
	    (WMODE_CAP(info->phy_mode, WMODE_GN)) && (info->channel < 14))
...
INT build_vht_ies(RTMP_ADAPTER *pAd, struct _build_ie_info *info)
{
	...
	if (
		...
		if ((info->frame_subtype == SUBTYPE_BEACON) ||
			...
			NdisMoveMemory((UCHAR *)(info->frame_buf + len),
				       (UCHAR *)&eid_hdr, 2);
			len += 2;
#if defined(G_BAND_256QAM)
			vht_bw = (g_band_256_qam_enable(pAd, info)) ?
					 VHT_BW_2040 :
					       vht_bw;
#endif /* G_BAND_256QAM */
			len += build_vht_op_ie(
				pAd, vht_bw, info->channel, info->wdev,
				(UCHAR *)(info->frame_buf + len));
...
void print_vht_op_info(VHT_OP_INFO *vht_op)
{
	...
	MTWF_LOG(DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("VHT Operation Infomation: 0x%02X%02X%02X\n",
		  vht_op->ch_width, vht_op->center_freq_1,
		  vht_op->center_freq_2));
	MTWF_LOG(DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("     - Channel Width: %u (%s)\n", vht_op->ch_width,
		  ch_with[vht_op->ch_width]));
	MTWF_LOG(DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_OFF,
