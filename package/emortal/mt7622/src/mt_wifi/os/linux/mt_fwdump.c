...
static ssize_t fwdumpread(struct file *fp, char __user *buf, size_t cnt,
			  loff_t *pos)
{
	UINT32 len = cnt;
	struct _RTMP_ADAPTER *pAd = 0;
...
static ssize_t fwdumpwrite(struct file *fp, const char __user *buf, size_t cnt,
			   loff_t *pos)
{
	return 0;
}
...
BOOLEAN FWDumpProcCreate(VOID *ptr, PCHAR suffix)
{
	...
	if (!pCookie->proc_fwdump_dir) {
		pCookie->proc_fwdump_dir =
			proc_mkdir(pCookie->fwdump_dir_name, NULL);
	...
	if (!pCookie->proc_fwdump_file) {
		...
		pCookie->proc_fwdump_file =
			proc_create_data(fwdump_file_name, 0644,
					 pCookie->proc_fwdump_dir, &fops_dump,
					 (VOID *)pAd);

		if (pCookie->proc_fwdump_file)
			MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				 ("%s: Create file /proc/%s/%s successfully\n",
				  __func__, pCookie->fwdump_dir_name,
				  fwdump_file_name));
		else {
			MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				 ("%s: Create %s fail\n", __func__,
