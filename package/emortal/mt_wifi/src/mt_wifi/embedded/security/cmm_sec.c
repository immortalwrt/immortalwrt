	...
	IN RTMP_IOCTL_INPUT_STRUCT * wrq)
{
	...
	if (copy_from_user(pKey, wrq->u.data.pointer, wrq->u.data.length)) {
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: copy from user failed\n", __func__));
		os_free_mem(pKey);
		return;
	}
...
VOID insert_pwd_id(struct _SECURITY_CONFIG *sec_cfg, struct pwd_id_list *pwd_id_list_head, RTMP_STRING *arg)
{
	...
	if (pwd == NULL) {
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s() null pwd\n", __func__));
		return;
	}
	if (pwdid == NULL) {
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s() null pwdid\n", __func__));
	...
	IN RTMP_STRING *pBuffer)
{
	...
	if (RTMPGetKeyParameter("PweMethod", tmpbuf, MAX_PARAMETER_LEN, pBuffer, TRUE)) {
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
			for (i = 0, macptr = rstrtok(tmpbuf, ";"); (macptr && i < pAd->ApCfg.BssidNum); macptr = rstrtok(NULL, ";"), i++) {
				...
				MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_TRACE, ("I/F(%s%d) ==> ",
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
		...
		for (i = 0; i < pAd->ApCfg.BssidNum; i++) {
			...
			if (RTMPGetKeyParameter(tok_str, tmpbuf, MAX_PARAMETER_LEN, pBuffer, FALSE)) {
				...
				hex_dump_with_cat_and_lvl("SAEPKKey", pri_key_bin, pri_key_len, DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_OFF);
			}
		}
	}
#endif /* CONFIG_AP_SUPPORT */
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
		...
		for (i = 0; i < pAd->ApCfg.BssidNum; i++) {
			...
			if (RTMPGetKeyParameter(tok_str, tmpbuf, MAX_PARAMETER_LEN, pBuffer, FALSE)) {
				...
				hex_dump_with_cat_and_lvl("SAEPKStartM", modifier, modifier_len, DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_OFF);
			}
		}
	}
#endif /* CONFIG_AP_SUPPORT */
	...
	if (RTMPGetKeyParameter("SAEPKSec", tmpbuf, MAX_PARAMETER_LEN, pBuffer, TRUE)) {
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
			for (i = 0, macptr = rstrtok(tmpbuf, ";"); (macptr && i < pAd->ApCfg.BssidNum); macptr = rstrtok(NULL, ";"), i++) {
				sec_cfg = &pAd->ApCfg.MBSSID[PF_TO_BSS_IDX(pAd, i)].wdev.SecConfig;/*ALPS05331840*/
	...
	OUT CMD_WTBL_SECURITY_KEY_T * rWtblSecurityKey)
{
	if (IS_REMOVEKEY_OPERATION(pInfo)) {
		...
		if (pSecKey->KeyLen > sizeof(rWtblSecurityKey->aucKeyMaterial)) {
			MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				("%s: KeyLen is larger than the aucKeyMaterial\n", __func__));
	...
	if (IS_REMOVEKEY_OPERATION(pInfo)) {
		...
		if (pInfo->bigtk_key_len) {
			...
			hex_dump_with_cat_and_lvl("install bigtk key:", wtbl_bip->aucKeyMaterial, pInfo->bigtk_key_len,
							DBG_CAT_SEC, CATSEC_BCNPROT, DBG_LVL_TRACE);
			rWtblSecurityKey->ucEntryCount++;
		}
	}
	...
	IN UCHAR need_copy)
{
	...
	if (IS_AKM_SAE(sec_cfg->AKMMap) && sec_cfg->sae_conn_type == SAE_CONNECTION_TYPE_SAEPK) {
		if (rsnxe_ie[1] == 0 || !(rsnxe_ie[2] & (1 << IE_RSNXE_CAPAB_SAE_PK))) {
			MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				("%s: IE_RSNXE_CAPAB_SAE_PK should not be 0\n", __func__));
			return MLME_UNSPECIFY_FAIL;
		}
	}
	...
	OUT ULONG *buf_len)
{
	...
	*buf_len = len + 3;
	...
	hex_dump_with_cat_and_lvl("oci ie", buf,
		*buf_len, DBG_CAT_SEC, CATSEC_OCV, DBG_LVL_OFF); /* todo: change to trace*/
}
	...
	IN ULONG buf_len)
{
	...
	hex_dump_with_cat_and_lvl("peer oci", buf, buf_len, DBG_CAT_SEC, CATSEC_OCV, DBG_LVL_OFF);

	if (buf_len < 3) {
		MTWF_LOG(DBG_CAT_SEC, CATSEC_OCV, DBG_LVL_ERROR,
			("%s : invalid oci len(%lu)\n",
			__func__, buf_len));
		return FALSE;
	}
	...
	if (!is_channel_in_channelset_by_reg_class(ad, buf[0], wdev->PhyMode, wdev->channel)) {
		MTWF_LOG(DBG_CAT_SEC, CATSEC_OCV, DBG_LVL_ERROR,
			("%s : operating class(%d) check fail, chanel isn't in channel list\n",
			__func__, buf[0]));
		return FALSE;
	}
	...
	if (get_spacing_by_reg_class(ad, buf[0], wdev->PhyMode, &spacing) == FALSE) {
		MTWF_LOG(DBG_CAT_SEC, CATSEC_OCV, DBG_LVL_ERROR,
			("%s : get reg_class table fail(op class = %d)\n",
			__func__, buf[0]));
		return FALSE;
	}
	...
	if (bw > spacing) {
		MTWF_LOG(DBG_CAT_SEC, CATSEC_OCV, DBG_LVL_ERROR,
			("%s : bandwidth mismatch (peer: %d, our: %d)\n",
			__func__, spacing, bw));
		return FALSE;
	}
	...
	if (buf[1] != wdev->channel) {
		MTWF_LOG(DBG_CAT_SEC, CATSEC_OCV, DBG_LVL_ERROR,
			("%s : primary ch(%d) check fail, primary ch(%d)\n",
			__func__, buf[1], wdev->channel));
		return FALSE;
	}

	if (buf[2] != wlan_operate_get_cen_ch_2(wdev)) {
		MTWF_LOG(DBG_CAT_SEC, CATSEC_OCV, DBG_LVL_ERROR,
			("%s : second central ch(%d) check fail, second central ch(%d)\n",
			__func__, buf[2], wlan_operate_get_cen_ch_2(wdev)));
		return FALSE;
	}

	if (!oct_support && buf_len > 3) {
		MTWF_LOG(DBG_CAT_SEC, CATSEC_OCV, DBG_LVL_ERROR,
			("%s : oci field len is wrong\n", __func__));
		return FALSE;
	}
	...
	IN ULONG buf_len)
{
	...
	if (buf_len < MIN_OCI_LEN) {
		MTWF_LOG(DBG_CAT_SEC, CATSEC_OCV, DBG_LVL_ERROR,
				("%s: buf_len(%lu) is less than min oci len\n", __func__, buf_len));
		return FALSE;
	}
	...
	if (buf[0] != IE_WLAN_EXTENSION ||
		...
		hex_dump_with_cat_and_lvl("oci_ie fail, peer oci ie", buf,
			buf_len, DBG_CAT_SEC, CATSEC_OCV, DBG_LVL_OFF);
		return FALSE;
	}
