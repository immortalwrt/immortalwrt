...
enum ENUM_SUBF_CAP_T { SUBF_OFF, SUBF_ALL, SUBF_BFER, SUBF_BFEE };
...
VOID mt_TxBFInit(IN struct _RTMP_ADAPTER *pAd, IN TXBF_STATUS_INFO *pTxBfInfo,
		 IN TXBF_MAC_TABLE_ENTRY *pEntryTxBf, IN HT_BF_CAP *pTxBFCap,
		 IN BOOLEAN supportsETxBF);

VOID mt_TxBFFwInit(IN struct _RTMP_ADAPTER *pAd);

VOID mt_WrapTxBFInit(IN struct _RTMP_ADAPTER *pAd,
		     IN struct _MAC_TABLE_ENTRY *pEntry, IN IE_LISTS *ie_list,
		     IN BOOLEAN supportsETxBF);

BOOLEAN mt_clientSupportsETxBF(IN struct _RTMP_ADAPTER *pAd,
			       IN HT_BF_CAP *pTxBFCap,
			       IN BOOLEAN ETxBfNoncompress);

BOOLEAN mt_WrapClientSupportsETxBF(IN struct _RTMP_ADAPTER *pAd,
				   IN HT_BF_CAP *pTxBFCap);

void mt_WrapSetETxBFCap(IN struct _RTMP_ADAPTER *pAd, IN struct wifi_dev *wdev,
			IN HT_BF_CAP *pTxBFCap);

#ifdef VHT_TXBF_SUPPORT
BOOLEAN mt_clientSupportsVhtETxBF(IN struct _RTMP_ADAPTER *pAd,
				  IN VHT_CAP_INFO *pTxBFCap);

BOOLEAN mt_WrapClientSupportsVhtETxBF(IN struct _RTMP_ADAPTER *pAd,
				      IN VHT_CAP_INFO *pTxBFCap);

void mt_WrapSetVHTETxBFCap(IN struct _RTMP_ADAPTER *pAd,
...
VOID TxBfProfileTag_TxBfType(IN P_PFMU_PROFILE_TAG1 prPfmuTag1,
			     IN UCHAR ucTxBf);
...
INT TxBfProfileTagRead(IN struct _RTMP_ADAPTER *pAd, IN UCHAR PfmuIdx,
		       IN BOOLEAN fgBFer);

INT TxBfProfileTagWrite(IN struct _RTMP_ADAPTER *pAd,
			IN P_PFMU_PROFILE_TAG1 prPfmuTag1,
			IN P_PFMU_PROFILE_TAG2 prPfmuTag2, IN UCHAR profileIdx);

VOID TxBfProfileTagPrint(IN struct _RTMP_ADAPTER *pAd, IN BOOLEAN fgBFer,
			 IN PUCHAR pBuf);

INT TxBfProfilePnRead(IN struct _RTMP_ADAPTER *pAd, IN UCHAR profileIdx);

INT TxBfProfilePnWrite(IN struct _RTMP_ADAPTER *pAd, IN UCHAR PfmuIdx,
		       IN UCHAR ucBw, IN PUCHAR pProfileData);

VOID TxBfProfilePnPrint(IN UCHAR ucBw, IN PUCHAR pBuf);

INT TxBfProfileDataRead(IN struct _RTMP_ADAPTER *pAd, IN UCHAR profileIdx,
			IN BOOLEAN fgBFer, IN USHORT subCarrIdx);

INT TxBfProfileDataWrite(IN struct _RTMP_ADAPTER *pAd, IN UCHAR profileIdx,
			 IN USHORT subCarrIdx, IN PUCHAR pProfileData);

INT TxBfProfileDataWrite20MAll(IN struct _RTMP_ADAPTER *pAd,
			       IN UCHAR profileIdx, IN PUCHAR pProfileData);

VOID TxBfProfileDataPrint(IN struct _RTMP_ADAPTER *pAd, IN USHORT subCarrIdx,
			  IN PUCHAR pBuf);

INT TxBfQdRead(IN struct _RTMP_ADAPTER *pAd, IN INT8 subCarrIdx);

INT mt_Trigger_Sounding_Packet(IN struct _RTMP_ADAPTER *pAd, IN UCHAR SndgEn,
			       IN UINT32 u4SNDPeriod, IN UCHAR ucSu_Mu,
			       IN UCHAR ucMuNum, IN PUCHAR pWlanId);

VOID TxBfProfileMemAllocMap(IN PUCHAR pBuf);

BOOLEAN TxBfModuleEnCtrl(IN struct _RTMP_ADAPTER *pAd);

#ifdef TXBF_DYNAMIC_DISABLE
INT DynamicTxBfDisable(IN struct _RTMP_ADAPTER *pAd, IN BOOLEAN fgDisable);
#endif /* TXBF_DYNAMIC_DISABLE */

VOID StaRecBfUpdate(IN struct _MAC_TABLE_ENTRY *pEntry,
		    IN P_CMD_STAREC_BF pCmdStaRecBf);

VOID StaRecBfRead(IN struct _RTMP_ADAPTER *pAd, IN PUCHAR pBuf);

INT32 mt_AsicBfStaRecUpdate(IN struct _RTMP_ADAPTER *pAd, IN UCHAR ucPhyMode,
			    IN UCHAR ucBssIdx, IN UCHAR ucWlanIdx);

INT32 mt_AsicBfStaRecRelease(IN struct _RTMP_ADAPTER *pAd, IN UCHAR ucBssIdx,
			     IN UCHAR ucWlanIdx);

VOID mt_AsicClientBfCap(IN struct _RTMP_ADAPTER *pAd,
			IN struct _MAC_TABLE_ENTRY *pEntry);

UCHAR AsicTxBfEnCondProc(IN struct _RTMP_ADAPTER *pAd,
			 IN TXBF_STATUS_INFO *pTxBfInfo);

/* displayTagfield - display one tagfield */
void displayTagfield(IN struct _RTMP_ADAPTER *pAd, IN int profileNum,
