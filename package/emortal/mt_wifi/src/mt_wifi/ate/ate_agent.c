...
INT32 SetATETxPower0(RTMP_ADAPTER *pAd, RTMP_STRING *Arg)
{
	...
	return mt_agent_cli_set_ext("ATETXPOW0", &pAd->serv, Arg);
#else
	struct _ATE_CTRL *ATECtrl = &(pAd->ATECtrl);
	struct _ATE_OPERATION *ATEOp = ATECtrl->ATEOp;
	ATE_TXPOWER TxPower;
...
{
#if defined(CONFIG_WLAN_SERVICE)
	return mt_agent_cli_set_ext("ATETXANT", &pAd->serv, Arg);
#else
	struct _ATE_CTRL *ATECtrl = &(pAd->ATECtrl);
	struct _ATE_OPERATION *ATEOp = ATECtrl->ATEOp;
	INT32 Ret = 0;
...
{
#if defined(CONFIG_WLAN_SERVICE)
	return mt_agent_cli_set_ext("ATERXANT", &pAd->serv, Arg);
#else
	INT32 Ret = 0;
	struct _ATE_CTRL *ATECtrl = &(pAd->ATECtrl);
	struct _ATE_OPERATION *ATEOp = ATECtrl->ATEOp;
...
{
#if defined(CONFIG_WLAN_SERVICE)
	return mt_agent_cli_set_dw("ATETXMCS", &pAd->serv, Arg);
#else
	INT32 Ret = 0;
	UCHAR Mcs, tx_mode = 0;
...
{
#if defined(CONFIG_WLAN_SERVICE)
	return mt_agent_cli_set_dw("ATETXNSS", &pAd->serv, Arg);
#else
	UCHAR control_band_idx = TESTMODE_GET_BAND_IDX(pAd);
	UCHAR Nss = 0;
...
{
#if defined(CONFIG_WLAN_SERVICE)
	return mt_agent_cli_set_dw("ATETXSTBC", &pAd->serv, Arg);
#else
	UCHAR control_band_idx = TESTMODE_GET_BAND_IDX(pAd);
	UCHAR Stbc;
...
{
#if defined(CONFIG_WLAN_SERVICE)
	return mt_agent_cli_set_dw("ATETXMODE", &pAd->serv, Arg);
#else
	UCHAR control_band_idx = TESTMODE_GET_BAND_IDX(pAd);
	UCHAR tx_mode;
...
{
#if defined(CONFIG_WLAN_SERVICE)
	return mt_agent_cli_set_dw("ATETXGI", &pAd->serv, Arg);
#else
	INT32 ret = FALSE;
...
INT32 set_ate_max_pe(RTMP_ADAPTER *ad, RTMP_STRING *Arg)
{
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		("%s: Max Packet Extension = %s, control_band_idx = %d\n", __func__,
	...
	if (max_pe > 2) {
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				 ("%s : %d is invalid (0: 0us, 1:8 us, 2:16 us)\n", __func__, max_pe));
		return FALSE;
	}
...
{
#if defined(CONFIG_WLAN_SERVICE)
	return mt_agent_cli_set_ext("ATERUINFO", &ad->serv, Arg);
#else
	INT32 ret = FALSE;
	...
	if (strlen(Arg) > 0) {
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				("%s: RU info = %s, control_band_idx = %d\n", __func__, Arg, band_idx));

		if (ate_op->set_ru_info)
			ate_op->set_ru_info(ad, band_idx, Arg);
		else
			MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: handler not registered, dismissed!\n", __func__));
	} else {
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: Invalid string\n", __func__));
	}
...
INT32 set_ate_ru_rx_aid(RTMP_ADAPTER *ad, RTMP_STRING *Arg)
{
	...
	if (strlen(Arg) > 0) {
		...
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				("%s: RX MU PPDU Aid = %s, control_band_idx = %d\n", __func__, Arg, TESTMODE_GET_BAND_IDX(ad)));
...
INT32 set_ate_tx_policy(RTMP_ADAPTER *ad, RTMP_STRING *Arg)
{
	...
	if (strlen(Arg) > 0) {
		...
		if (tx_mode >= TX_MODE_MAX)
			MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
					("%s: [band%d] Unknown TX mode(%d).\n", __func__, TESTMODE_GET_BAND_IDX(ad), tx_mode));
		else if (tx_method > 1)
			MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
					("%s: [band%d] Unknown TX policy(%d, 0:TXD ; 1:TXC).\n", __func__, TESTMODE_GET_BAND_IDX(ad), tx_method));
		else {
			TESTMODE_SET_PARAM(ad, TESTMODE_GET_BAND_IDX(ad), tx_method[tx_mode], tx_method);

			MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
					("%s: [band%d] Set %s as %s mode.\n", __func__, TESTMODE_GET_BAND_IDX(ad), tx_mode_map[tx_mode],
...
INT32 set_ate_retry(RTMP_ADAPTER *ad, RTMP_STRING *Arg)
{
	...
	if (strlen(Arg) > 0) {
		...
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				("%s: TX PPDU Retry = %s, control_band_idx = %d\n", __func__, Arg, TESTMODE_GET_BAND_IDX(ad)));
...
{
#if defined(CONFIG_WLAN_SERVICE)
	return mt_agent_cli_set_dw("ATEPAYLOAD", &pAd->serv, Arg);
#else
	struct _ATE_CTRL *ATECtrl = &pAd->ATECtrl;
	RTMP_STRING *Value;
...
{
#if defined(CONFIG_WLAN_SERVICE)
	return mt_agent_cli_set_dw("ATEFIXEDPAYLOAD", &pAd->serv, Arg);
#else
	UINT32 Value;
	UINT8 control_band_idx = TESTMODE_GET_BAND_IDX(pAd);
...
INT32 SetATEShow(RTMP_ADAPTER *ad, RTMP_STRING *Arg)
{
...

	{
		...
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("ATE Tx Methods:\n"));
		for (loop_index = 0 ; loop_index < TX_MODE_MAX ; loop_index++) {
			...
			if (strlen(tx_mode_map[loop_index])) {
				if (tx_method[loop_index])
					sprintf(tx_method_str, "TXC");
				MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\t%s = %s\n", tx_mode_map[loop_index], tx_method_str));
			}
		}
		...
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("TxLength = %u\n",
				...
				MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Header Length = %d\n",
...
{
#if defined(CONFIG_WLAN_SERVICE)
	return mt_agent_cli_set_ext("ATECTRLBANDIDX", &pAd->serv, Arg);
#else
	struct _ATE_CTRL *ATECtrl = &pAd->ATECtrl;
	INT32 ret = 0;
...
error1:
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
		("%s: invalid argument (no argument)\n", __func__));
	return FALSE;
...
error3:
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
		("%s: error status(%d) for fw command.\n", __func__, ret));
	return FALSE;
}
...
INT SetATEAssocProc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (wdev == NULL) {
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("can't find wdev!!\n"));
		return FALSE;
	}
...
INT SetATETxBfDutInitProc(RTMP_ADAPTER *pAd, RTMP_STRING *Arg)
{
	...
	if (IS_MT7915_FW_VER_E1(pAd)) {
		snprintf(cmdStr, sizeof(cmdStr), "4-00:%.2x:%.2x:%.2x:%.2x:%.2x",
	...
	if (fgDbdc) {
		...
		if (IS_MT7915_FW_VER_E1(pAd) || IS_MT7915_FW_VER_E2(pAd)) {
...
INT32 SetATETxBfChanProfileUpdate(RTMP_ADAPTER *pAd, RTMP_STRING *Arg)
{
	...
	for (i = 0, value = rstrtok(Arg, ":"); value; value = rstrtok(NULL, ":")) {
		...
		if (strLen & 1) {
			snprintf(value_T, sizeof(value_T) - strlen(value_T), "%s%s", "0", value);
...
INT32 SetATEIBfProfileUpdate(RTMP_ADAPTER *pAd, RTMP_STRING *Arg)
{
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : band[%d]'s TxAntennaSel = 0x%x\n",
	...
	snprintf(cmdStr, sizeof(cmdStr), "01:%.2x:00:%.2x:%.2x:%.2x:%.2x:%.2x:%.2x",
	...
	snprintf(cmdStr, sizeof(cmdStr), "%.2x:00:%.2x:00:00:00:%.2x:00:02:%.2x:%.2x:00:00:00:00:%.2x:%.2x:%.2x:%.2x:%.2x:%.2x:%.2x:%.2x",
	...
	snprintf(cmdStr, sizeof(cmdStr), "%d", u2Wcid);
...
INT32 SetATEEBfProfileConfig(RTMP_ADAPTER *pAd, RTMP_STRING *Arg)
{
	...
	snprintf(cmdStr, sizeof(cmdStr), "%.2x:00:%.2x:00:01:00:%.2x:00:02:%.2x:%.2x:00:00:00:00:%.2x:%.2x:%.2x:%.2x:%.2x:%.2x:%.2x:%.2x",
					u2Wcid, PfmuIdx, NdpNss, Nc, Nr,
					aucPfmuMemRow[0], aucPfmuMemCol[0],
					aucPfmuMemRow[1], aucPfmuMemCol[1],
					aucPfmuMemRow[2], aucPfmuMemCol[2],
	...
	RTMP_STRING *Arg)
{
	...
	snprintf(cmdStr, sizeof(cmdStr), "%.2x:%.2x:%.2x:%.2x:%.2x:%.2x", PRINT_MAC(addr1));
	SetATEDa(pAd, cmdStr);
	...
	/* Device info Update */
	snprintf(cmdStr, sizeof(cmdStr), "%.2x:%.2x:%.2x:%.2x:%.2x:%.2x:%.2x:%.2x", OwnMacIdx, PRINT_MAC(addr2), control_band_idx);
	...
	if (IS_MT7915(pAd)) {
		if (TRxStream == 4) {
			SetATETxAntenna(pAd, "1:24");  /* 15 (0xF:  wifi[0], wifi[1], wifi[2], wifi[3] on) */
			SetATERxAntenna(pAd, "1:24");
		} else if (TRxStream == 3) {
			...
			if (control_band_idx == DBDC_BAND1) {
				SetATETxAntenna(pAd, "1:25");	/* 12 (0xC:  wifi[2], wifi[3] on) */
				SetATERxAntenna(pAd, "1:25");
			} else {
				SetATETxAntenna(pAd, "1:24");	 /* 3 (0x3:  wifi[0], wifi[1] on) */
				SetATERxAntenna(pAd, "1:24");
			}
		}
	} else {
	...
	if ((TxMode == 4 || TxMode == 8) && ((BW == 3) || (BW == 6))) {
		if (TRxStream == 4)
			Nr = 1;
		else
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
	...
	snprintf(cmdStr, sizeof(cmdStr), "%u", u1Spe);
	...
	snprintf(cmdStr, sizeof(cmdStr), "%.2x:%.2x:00:%.2x:%.2x:%.2x:%.2x:%.2x:%.2x", WlanIdx, BssIdx, PRINT_MAC(addr1));
	Set_StaRecCmmUpdate(pAd, cmdStr);
#endif
	/* Station Record BF Info Update */
	if (TxMode == 8)
		snprintf(cmdStr, sizeof(cmdStr), "%.2x:%.2x:%.2x:00:01:49:00:49:08:00:%.2x:%.2x:00:00:00:00:00:01:00:02:00:03:00", WlanIdx, BssIdx, PfmuId, Nr, BW);
	else if (TxMode == 4)
	...
	if (IS_MT7915(pAd)) {
		...
		cr_addr = BN0_WF_ARB_TOP_TQSAXM0_TXCMD_ALTX0_START0_ADDR+(0x10000*control_band_idx);

		RTMP_IO_READ32(pAd->hdev_ctrl, cr_addr, &cr_value);
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: Read [TXCMD ALTX Q CR] %x=0x%x\n", __func__, cr_addr, cr_value));

		cr_value |= (BN0_WF_ARB_TOP_TQSAXM0_TXCMD_ALTX0_START0_MASK | BN0_WF_ARB_TOP_TQSAXM0_TXCMD_ALTX0_START1_MASK | BN0_WF_ARB_TOP_TQSAXM0_TXCMD_ALTX0_START2_MASK | BN0_WF_ARB_TOP_TQSAXM0_TXCMD_ALTX0_START3_MASK | BN0_WF_ARB_TOP_TQSAXM0_TXCMD_ALTX0_START4_MASK);

		RTMP_IO_WRITE32(pAd->hdev_ctrl, cr_addr, cr_value);

		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: Write [TXCMD ALTX Q] %x=0x%x\n", __func__, cr_addr, cr_value));
	}
	/*----------------------------------------------------------------------------------------------------------------------------------------*/
	/* EBF TxBf Apply */
	/*----------------------------------------------------------------------------------------------------------------------------------------*/
	...
	RTMP_STRING *Arg)
{
	...
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: g_EBF_certification = %d !!!!!\n", __func__, g_EBF_certification));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: BF_ON_certification = %d !!!!!\n", __func__, BF_ON_certification));
	return status;
}
	...
	RTMP_STRING *Arg)
{
	...
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("=============================================================================================\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("                            ATE ETxBF Certification Procedure Guide\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("=============================================================================================\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("For HT20 mode\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 1)  iwpriv ra0 set ATEEBFCE=1\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 2)  iwpriv ra0 set ATEConTxETxBfGdProc=02:00:00:036:112:1 (Use in Golden Device)\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 3)  iwpriv ra0 set ATEConTxETxBfInitProc=02:00:00:01:04:18:036:112:1:04000\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 4)  iwpriv ra0 set ATETXEBF=1 (Tx packet apply BF On)\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 5)  iwpriv ra0 set ATE=TXFRAME\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 6)  iwpriv ra0 mac 820fa09c (check [15:0] eBF counter, if apply then nonzero)\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 7)  check IQxel waveform\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 8)  iwpriv ra0 set ATETXEBF=0 (Tx packet apply BF Off)\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 9)  iwpriv ra0 mac 820fa09c (check [15:0] eBF counter, if not apply then zero)\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("10)  check Iqxel waveformn"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("---------------------------------------------------------------------------------------------\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("For HT40 mode\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 1)  iwpriv ra0 set ATEEBFCE=1\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 2)  iwpriv ra0 set ATEConTxETxBfGdProc=02:00:01:036:112:1 (Use in Golden Device)\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 3)  iwpriv ra0 set ATEConTxETxBfInitProc=02:00:01:01:04:18:036:112:1:04000\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 4)  iwpriv ra0 set ATETXEBF=1 (Tx packet apply BF On)\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 5)  iwpriv ra0 set ATE=TXFRAMESKB\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 6)  iwpriv ra0 mac 820fa09c (check [15:0] eBF counter, if apply then nonzero)\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 7)  check IQxel waveform\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 8)  iwpriv ra0 set ATETXEBF=0 (Tx packet apply BF Off)\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 9)  iwpriv ra0 mac 820fa09c (check [15:0] eBF counter, if not apply then zero)\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("10)  check Iqxel waveform\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("---------------------------------------------------------------------------------------------\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("For VHT80 mode\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 1)  iwpriv ra0 set ATEEBFCE=1\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 2)  iwpriv ra0 set ATEConTxETxBfGdProc=04:00:02:036:112:1 (Use in Golden Device)\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 3)  iwpriv ra0 set ATEConTxETxBfInitProc=04:00:02:01:04:18:036:112:1:16000\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 4)  iwpriv ra0 set ATETXEBF=1 (Tx packet apply BF On)\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 5)  iwpriv ra0 set ATE=TXFRAMESKB\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 6)  iwpriv ra0 mac 820fa09c (check [15:0] eBF counter, if apply then nonzero)\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 7)  check IQxel waveform\n"));
	...
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("10)  check Iqxel waveform\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("---------------------------------------------------------------------------------------------\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("For VHT160C mode\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 1)  iwpriv ra0 set ATEEBFCE=1\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 2)  iwpriv ra0 set ATEConTxETxBfGdProc=04:00:03:036:112:1 (Use in Golden Device)\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 3)  iwpriv ra0 set ATEConTxETxBfInitProc=04:00:03:01:04:18:036:112:1:16000\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 4)  iwpriv ra0 set ATETXEBF=1 (Tx packet apply BF On)\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 5)  iwpriv ra0 set ATE=TXFRAMESKB\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 6)  iwpriv ra0 mac 820fa09c (check [15:0] eBF counter, if apply then nonzero)\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 7)  check IQxel waveform\n"));
	...
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("10)  check Iqxel waveform\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("---------------------------------------------------------------------------------------------\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("For VHT160NC mode\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 1)  iwpriv ra0 set ATEEBFCE=1\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 2)  iwpriv ra0 set ATEConTxETxBfGdProc=04:00:06:036:112:1 (Use in Golden Device)\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 3)  iwpriv ra0 set ATEConTxETxBfInitProc=04:00:06:01:04:18:036:112:1:16000\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 4)  iwpriv ra0 set ATETXEBF=1 (Tx packet apply BF On)\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 5)  iwpriv ra0 set ATE=TXFRAMESKB\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 6)  iwpriv ra0 mac 820fa09c (check [15:0] eBF counter, if apply then nonzero)\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 7)  check IQxel waveform\n"));
	...
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("10)  check Iqxel waveform\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("---------------------------------------------------------------------------------------------\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("For DBDC Band1 HT20 mode\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 1)  configure DBDC mode and Reboot system\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 2)  iwpriv ra1 set ATEEBFCE=1\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 3)  iwpriv ra1 set ATEConTxETxBfGdProc=02:00:00:36:112:1 (Use in Golden Device)\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 4)  iwpriv ra1 set ATEConTxETxBfInitProc=02:00:00:01:02:18:36:112:1:04000\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 5)  iwpriv ra1 set ATETXEBF=1 (Tx packet apply BF On)\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 6)  iwpriv ra1 set ATE=TXFRAMESKB\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 7)  iwpriv ra1 mac 820fa09c (check [15:0] eBF counter, if apply then nonzero)\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 8)  check IQxel waveform\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 9)  iwpriv ra1 set ATETXEBF=0 (Tx packet apply BF Off)\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("10)  iwpriv ra1 mac 820fa09c (check [15:0] eBF counter, if not apply then zero)\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("11)  check Iqxel waveform\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("=============================================================================================\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("                           Method for Dynamical Control Tx Power\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("=============================================================================================\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 1)  Follow ETxBF Certification Procedure to enable TxBf packet at first\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 2)  Use command \"iwpriv ra0 set ATE=TXSTOP\" to stop Tx\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 3)  Use command \"iwpriv ra0 set ATETXPOW0=XX\" to configure Tx Power DAC value for OFDM 54M\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" 4)  USe command \"ra0 set ATE=TXFRAMESKB\" to start continuous packet Tx\n"));
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("=============================================================================================\n"));
	return status;
}
...
#if defined(CONFIG_WLAN_SERVICE)
	return mt_agent_cli_act(Arg, &pAd->serv);
#else
	{
		struct _ATE_CTRL *ATECtrl = &(pAd->ATECtrl);
		struct _ATE_OPERATION *ATEOp = ATECtrl->ATEOp;
		...
		if (!strcmp(Arg, "ATESTART") && (mode != ATE_START)) { /* support restart w/o ATESTOP */
			...
			if (chip_dbg->check_txv && entry) {
				...
				Ret = chip_dbg->check_txv(pAd->hdev_ctrl, "spe idx", entry->phy_param.spe_idx);
				Ret = chip_dbg->check_txv(pAd->hdev_ctrl, "phy mode", entry->phy_param.phy_mode);
				Ret = chip_dbg->check_txv(pAd->hdev_ctrl, "dbw", entry->phy_param.bw);
				Ret = chip_dbg->check_txv(pAd->hdev_ctrl, "ER-106T", entry->phy_param.su_ext_tone);
				Ret = chip_dbg->check_txv(pAd->hdev_ctrl, "DCM", entry->phy_param.dcm);
				if (phy_mode == MODE_CCK) {
					if (entry->phy_param.rate > MCS_8)
						Ret = chip_dbg->check_txv(pAd->hdev_ctrl, "Rate", entry->phy_param.rate+5);
					else
						Ret = chip_dbg->check_txv(pAd->hdev_ctrl, "Rate", entry->phy_param.rate);
				} else if (phy_mode == MODE_OFDM) {
					Ret = chip_dbg->check_txv(pAd->hdev_ctrl, "Rate", ofdm_rate[entry->phy_param.rate]);
				} else
					Ret = chip_dbg->check_txv(pAd->hdev_ctrl, "Rate", entry->phy_param.rate);
				if (phy_mode < MODE_HTMIX)
					Ret = chip_dbg->check_txv(pAd->hdev_ctrl, "NSTS", 0);
				else if (phy_mode < MODE_VHT)
					Ret = chip_dbg->check_txv(pAd->hdev_ctrl, "NSTS", (entry->phy_param.rate >> 3));
				else
					Ret = chip_dbg->check_txv(pAd->hdev_ctrl, "NSTS", entry->phy_param.vht_nss-1);
				Ret = chip_dbg->check_txv(pAd->hdev_ctrl, "HE LTF", entry->phy_param.ltf_type);
				Ret = chip_dbg->check_txv(pAd->hdev_ctrl, "HE GI", entry->phy_param.gi_type);
				Ret = chip_dbg->check_txv(pAd->hdev_ctrl, "stbc", entry->phy_param.stbc);
				Ret = chip_dbg->check_txv(pAd->hdev_ctrl, "FEC Coding", entry->phy_param.ldpc);
				if (entry->phy_param.phy_mode > MODE_VHT)
					Ret = chip_dbg->check_txv(pAd->hdev_ctrl, "Max TPE", TESTMODE_GET_PARAM(pAd, TESTMODE_GET_BAND_IDX(pAd), max_pkt_ext));
				else
					Ret = chip_dbg->check_txv(pAd->hdev_ctrl, "Max TPE", 0);
				Ret = chip_dbg->check_txv(pAd->hdev_ctrl, "TX LEN",
										  TESTMODE_GET_PARAM(pAd, TESTMODE_GET_BAND_IDX(pAd), tx_len)+((phy_mode > MODE_OFDM) ? 8 : 4));	/* 4 bytes of FCS and 4 bytes of AMPDU delimiter*/
			}
		}
#if defined(MT7915)
		else if (!strcmp(Arg, "DNLKCLEAN") && (mode & ATE_START)) {
			Ret = ATEOp->DnlK7915(pAd, "0");
			Ret = 0;
		} else if (!strcmp(Arg, "DNLK2G") && (mode & ATE_START)) {
			Ret = ATEOp->DnlK7915(pAd, "1");
			Ret = 0;
		} else if (!strcmp(Arg, "DNLK5G") && (mode & ATE_START)) {
			Ret = ATEOp->DnlK7915(pAd, "2");
			Ret = 0;
		}
#endif
		else {
...
{
#if defined(CONFIG_WLAN_SERVICE)
	return mt_agent_cli_set_ext("ATECHANNEL", &pAd->serv, Arg);
#else
	INT32 Ret = 0;
	const INT idx_num = 4;
	struct _ATE_CTRL *ATECtrl = &(pAd->ATECtrl);
...
{
#if defined(CONFIG_WLAN_SERVICE)
	return mt_agent_cli_set_ext("ATETXBW", &pAd->serv, Arg);
#else
	struct _ATE_CTRL *ATECtrl = &(pAd->ATECtrl);
	struct _ATE_OPERATION *ATEOp = ATECtrl->ATEOp;
	INT32 Ret = 0;
