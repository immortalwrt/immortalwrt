...
static INT pse_pg_cnt[] = { 256,  512,	512,  1024, 768,  1536, 1024, 2048,
			    1280, 2560, 1536, 3072, 1792, 3584, 0,    0 };
...
static INT32 chip_dump_ps_table(struct hdev_ctrl *ctrl, UINT32 ent_type,
				BOOLEAN bReptCli)
{
...
static INT32 chip_show_tmac_info(struct hdev_ctrl *ctrl, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("EIFS Time, Band0 (unit: 1us) = %d\n",
		  GET_ICR_EIFS_TIME(Value)));
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("RIFS Time, Band0 (unit: 1us) = %d\n",
		  GET_ICR_RIFS_TIME(Value)));
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("SIFS Time, Band0 (unit: 1us) = %d\n",
		  GET_ICR_SIFS_TIME(Value)));
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("SLOT Time, Band0 (unit: 1us) = %d\n",
		  GET_ICR_SLOT_TIME(Value)));
	RTMP_IO_READ32(pAd, TMAC_ICR_BAND_1, &Value);
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("EIFS Time, Band1 (unit: 1us) = %d\n",
		  GET_ICR_EIFS_TIME(Value)));
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("RIFS Time, Band1 (unit: 1us) = %d\n",
		  GET_ICR_RIFS_TIME(Value)));
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("SIFS Time, Band1 (unit: 1us) = %d\n",
		  GET_ICR_SIFS_TIME(Value)));
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("SLOT Time, Band1 (unit: 1us) = %d\n",
		  GET_ICR_SLOT_TIME(Value)));
...
static INT32 chip_dump_mib_info(struct hdev_ctrl *ctrl, RTMP_STRING *arg)
{
	...
	UINT32 msdr6, msdr7, msdr8, msdr9, msdr10, msdr16, msdr17, msdr18,
		msdr19, msdr20, msdr21;
	UINT32 mbxsdr[bss_nums][4];
	UINT32 mbtcr[16], mbtbcr[16], mbrcr[16], mbrbcr[16];
	UINT32 btcr[bss_nums], btbcr[bss_nums], brcr[bss_nums], brbcr[bss_nums],
		btdcr[bss_nums], brdcr[bss_nums];
	UINT32 mu_cnt[5];
	...
	for (band_idx = 0; band_idx < pChipCap->band_cnt; band_idx++) {
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			 ("MIB Status Control=0x%x\n", mac_val));
		RTMP_IO_READ32(pAd, MIB_M0PBSCR + band_offset, &mac_val);
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			 ("MIB Per-BSS Status Control=0x%x\n", mac_val));
		...
		MTWF_LOG(
			...
			("\tCCK_MDRDY_TIME=0x%x, OFDM_MDRDY_TIME=0x%x, OFDM_GREEN_MDRDY_TIME=0x%x\n",
			 msdr19 & 0x3ffffff, msdr20 & 0x3ffffff,
			 msdr21 & 0x3ffffff));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			 ("\tPrim CCA Time=0x%x\n", msdr16 & 0xffffff));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		...
		pAd->RadioStatsCounter.TotalBeaconSentCount +=
			(mac_val & 0xffff);
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			 ("\tTx AMPDU_Pkt_Cnt=0x%x\n", ampdu_cnt[0]));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			 ("\tTx AMPDU_MPDU_Pkt_Cnt=0x%x\n",
			  ampdu_cnt[1] & 0xffffff));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			 ("\tAMPDU SuccessCnt=0x%x\n",
			  ampdu_cnt[2] & 0xffffff));
		MTWF_LOG(
			DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			("\tTx Agg Range: \t1 \t2~5 \t6~15 \t16~22 \t23~33 \t34~49 \t50~57 \t58~64\n"));
		MTWF_LOG(
			DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("===Per-BSS Related Tx/Rx Counters===\n"));
	MTWF_LOG(
		DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		("BSS Idx   TxCnt/DataCnt  TxByteCnt  RxCnt/DataCnt  RxByteCnt\n"));

	for (idx = 0; idx < bss_nums; idx++) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			 ("%d\t 0x%x/0x%x\t 0x%x \t 0x%x/0x%x \t 0x%x\n", idx,
			  btcr[idx], btdcr[idx], btbcr[idx], brcr[idx],
			  brdcr[idx], brbcr[idx]));
	}
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("===Per-MBSS Related Tx/Rx Counters===\n"));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("MBSSIdx   TxCnt  TxByteCnt  RxCnt  RxByteCnt\n"));

	for (idx = 0; idx < 16; idx++) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			 ("%d\t 0x%x\t 0x%x \t 0x%x \t 0x%x\n", idx, mbtcr[idx],
			  mbtbcr[idx], mbrcr[idx], mbrbcr[idx]));
	}
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("Range1 = %d\t Range2 = %d\t Range3 = %d\t Range4 = %d\n",
		  GET_TX_DDLMT_RNG1_CNT(mac_val),
		  GET_TX_DDLMT_RNG2_CNT(mac_val),
		  GET_TX_DDLMT_RNG3_CNT(mac_val1),
		  GET_TX_DDLMT_RNG4_CNT(mac_val1)));
#ifdef TRACELOG_TCP_PKT
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("TCP RxAck = %d\t TxData = %d", pAd->u4TcpRxAckCnt,
		  pAd->u4TcpTxDataCnt));
	pAd->u4TcpRxAckCnt = 0;
...
static INT32 chip_show_protect_info(struct hdev_ctrl *ctrl, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" -Proetction\n"));
...
#define CFG_DBG_L1_SEL_N_GET(_A, _S, _pV)                                      \
	...
	for (idx = 0; idx <= 18; idx++) {
		...
		if (idx <= 3) {
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				 ("BCN(BSSID%d) FSM Debug Flag0a=0x%08x\n", idx,
				  mac_val));
			CFG_DBG_L1_SEL_N_GET(pAd, 0x0b0b0b0b, &mac_val);
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				 ("BCN(BSSID%d) FSM Debug Flag0b=0x%08x\n", idx,
				  mac_val));
		} else {
			CFG_DBG_L1_SEL_N_GET(pAd, 0x0a0a0a0a, &mac_val);
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				 ("BCN(BSSID0_%d) FSM Debug Flag0a=0x%08x\n",
				  (idx - 3), mac_val));
			CFG_DBG_L1_SEL_N_GET(pAd, 0x0b0b0b0b, &mac_val);
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				 ("BCN(BSSID0_%d) FSM Debug Flag0b=0x%08x\n",
				  (idx - 3), mac_val));
		}
	}
}
...
static VOID chip_show_bcn_info(struct hdev_ctrl *ctrl, UCHAR bandidx)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("ARB_BFCR(0x820f3190)=0x%08x\n", mac_val));
	MAC_IO_READ32(pAd, ARB_SCR, &mac_val);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("ARB_SCR(0x820f3080)=0x%08x\n", mac_val));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("dump 0x820f3100~0x820f3154:\n"));
	...
	for (addr = ARB_TQSW0; addr <= ARB_TQPM1; addr = addr + 4) {
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			 ("addr 0x%05x=0x%08x ", addr, mac_val));
	}
...
#define CR4_HEART_BEAT_STS 0x80200
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("CR4 heart beat status (0x80200)=0x%08x\n", mac_val));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("dump 0x%08x~0x%08x:\n", 0x82070618 + band_offset,
		  0x8207065c + band_offset));

	for (addr = PHY_BAND0_PHYMUX_6; addr <= PHY_BAND0_PHYMUX_23;
	     addr = addr + 4) {
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("dump 0x%08x~0x%08x:\n", 0x8207227c + band_offset,
		  0x82072294 + band_offset));

	for (addr = RO_BAND0_RXTD_DEBUG0; addr <= RO_BAND0_RXTD_DEBUG6;
	     addr = addr + 4) {
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("dump 0x%08x~0x%08x:\n", 0x820721a0 + band_offset,
		  0x820721b8 + band_offset));

	for (addr = RO_BAND0_AGC_DEBUG_0; addr <= RO_BAND0_AGC_DEBUG_6;
	     addr = addr + 4) {
	...
	*/
	...
	for (idx = 0; idx < 10; idx++) {
		if (IS_MT7615(pAd)) {
			MAC_IO_READ32(pAd, RO_BAND0_PHYCTRL_STS0 + band_offset,
				      &mac_val);
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				 ("loop %d addr(0x%8x)=0x%08x ", idx,
				  0x8207020c + band_offset, mac_val));
			MAC_IO_READ32(pAd, RO_BAND0_PHYCTRL_STS1 + band_offset,
				      &mac_val);
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				 ("addr(0x%08x)=0x%08x ",
				  0x82070210 + band_offset, mac_val));
			MAC_IO_READ32(pAd, RO_BAND0_PHYCTRL_STS2 + band_offset,
				      &mac_val);
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				 ("addr(0x%08x)=0x%08x ",
				  0x82070214 + band_offset, mac_val));
			MAC_IO_READ32(pAd, RO_BAND0_PHYCTRL_STS4 + band_offset,
				      &mac_val);
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				 ("addr(0x%08x)=0x%08x ",
				  0x8207021c + band_offset, mac_val));
			MAC_IO_READ32(pAd, RO_BAND0_PHYCTRL_STS5 + band_offset,
				      &mac_val);
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				 ("addr(0x%08x)=0x%08x\n",
		...
		MAC_IO_READ32(pAd, ARB_BFCR, &mac_val);
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			 ("loop %d ARB_BFCR(0x820f3190)=0x%08x\n", idx,
			  mac_val));
	}
	...
	for (j = 0; j < 16; j++) { /* show AC Q info */
		for (i = 0; i < 32; i++) {
			if (((ple_stat[j + 1] & (0x1 << i)) >> i) == 0) {
				UINT32 hfid, tfid, pktcnt, ac_num = j / 4,
							   ctrlvalue = 0;
				UINT32 sta_num = i + (j % 4) * 32,
				       fl_que_ctrl[3] = { 0 };
				struct wifi_dev *wdev =
					wdev_search_by_wcid(pAd, sta_num);
				UINT32 wmmidx = 0;
...
VOID mt7615_chip_dbg_init(struct _RTMP_CHIP_DBG *dbg_ops)
{
	...
	dbg_ops->show_pse_data =
		NULL; /* read PSE data from host is not supported */
