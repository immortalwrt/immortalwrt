...
/* */
#define OS_NdisAllocateSpinLock(__lock) spin_lock_init((spinlock_t *)(__lock))

#define OS_NdisFreeSpinLock(lock)                                              \
	do {                                                                   \
	} while (0)

#define OS_SEM_LOCK(__lock) spin_lock_bh((spinlock_t *)(__lock))

#define OS_SEM_UNLOCK(__lock) spin_unlock_bh((spinlock_t *)(__lock))
...
#define OS_IRQ_UNLOCK(__lock, __irqflag) spin_unlock_bh((spinlock_t *)(__lock))
...
#define OS_INT_UNLOCK(__lock, __irqflag)                                       \
	spin_unlock_irqrestore((spinlock_t *)(__lock),                         \
			       ((unsigned long)__irqflag))
...
#define OS_SEM_EVENT_DESTORY(_pSema)                                           \
	do {                                                                   \
	} while (0)
...
#define OS_WAIT(_time)                                                         \
	{                                                                      \
		if (in_interrupt())                                            \
			...
			int _i;                                                \
			long _loop = ((_time) / (1000 / OS_HZ)) > 0 ?          \
					     ((_time) / (1000 / OS_HZ)) :      \
						   1;                                \
			wait_queue_head_t _wait;                               \
			init_waitqueue_head(&_wait);                           \
			for (_i = 0; _i < (_loop); _i++)                       \
				wait_event_interruptible_timeout(_wait, 0,     \
								 ONE_TICK);    \
		}                                                              \
	}
...
#define ASSERT(x)                                                              \
	{                                                                      \
		if (!(x)) {                                                    \
			printk(KERN_WARNING __FILE__ ":%d assert " #x          \
						     "failed\n",               \
			       __LINE__);                                      \
			dump_stack();                                          \
...
#define PCI_MAP_SINGLE_DEV(_pAd, _ptr, _size, _sd_idx, _dir)                   \
	linux_pci_map_single(((POS_COOKIE)(_pAd->OS_Cookie))->pDev, _ptr,      \
			     _size, _sd_idx, _dir)

#define DMA_MAPPING_ERROR_HANDLE(_handle, _ptr)                                \
	dma_mapping_error(&((struct pci_dev *)(_handle))->dev, _ptr)

#define PCI_UNMAP_SINGLE(_pAd, _ptr, _size, _dir)                              \
	linux_pci_unmap_single(((POS_COOKIE)(_pAd->OS_Cookie))->pDev, _ptr,    \
			       _size, _dir)
 ...
 **********************************************************************************/
#ifdef RTMP_MAC_PCI
#if defined(INF_TWINPASS) || defined(INF_DANUBE) || defined(INF_AR9) ||        \
	defined(IKANOS_VX_1X0)
#define RTMP_IO_FORCE_READ32(_A, _R, _pV)                                      \
	do {                                                                   \
		(*(_pV) =                                                      \
			 readl((void *)((_A)->PciHif.CSRBaseAddress + (_R)))); \
...
#define RTMP_IO_READ32(_A, _R, _pV)                                            \
	do {                                                                   \
		if ((_A)->bPCIclkOff == FALSE) {                               \
			(*(_pV) = readl((void *)((_A)->PciHif.CSRBaseAddress + \
						 (_R))));                      \
			(*(_pV) = SWAP32(*((UINT32 *)(_pV))));                 \
		}                                                              \
	} while (0)
...
#define _RTMP_IO_WRITE32(_A, _R, _V)                                           \
	do {                                                                   \
		if ((_A)->bPCIclkOff == FALSE) {                               \
			...
			_Val = SWAP32(_V);                                     \
			writel(_Val,                                           \
			       (void *)((_A)->PciHif.CSRBaseAddress + (_R)));  \
		}                                                              \
	} while (0)
...
#define RTMP_IO_WRITE8(_A, _R, _V)                                             \
	do {                                                                   \
		...
		_i = ((_R)&0x3);                                               \
		Val = readl(                                                   \
			(void *)((_A)->PciHif.CSRBaseAddress + ((_R)-_i)));    \
		Val = SWAP32(Val);                                             \
	...
	do {                                                                   \
		if ((_A)->bPCIclkOff == FALSE) {                               \
			(pci_read_config_dword(                                \
				((POS_COOKIE)(_A->OS_Cookie))->pci_dev,        \
				_R + 0x80000000, _pV));                        \
		} else                                                         \
			*(_pV) = 0;                                            \
	} while (0)
...
#define RTMP_IO_FORCE_READ32(_A, _R, _pV)                                      \
	...
	defined(RALINK_2883) || defined(RTMP_RBUS_SUPPORT) || defined(BB_SOC)
...
#define RTMP_SYS_IO_READ32(_R, _pV) (*_pV = readl((void *)(_R)))

#define RTMP_SYS_IO_WRITE32(_R, _V) writel(_V, (void *)(_R))

#if defined(BRCM_6358) || defined(RALINK_2880) || defined(RALINK_3052) ||      \
	defined(RALINK_2883) || defined(RTMP_RBUS_SUPPORT) ||                  \
	defined(MT76x2) || defined(BB_SOC)
#define RTMP_IO_WRITE8(_A, _R, _V)                                             \
	do {                                                                   \
		ULONG Val;                                                     \
		UCHAR _i;                                                      \
...
#ifdef MT_MAC
#define RTMP_MCU_IO_READ32(_A, _R, _pV) CmdIORead32((_A), (_R), (_pV))
#define RTMP_MCU_IO_WRITE32(_A, _R, _V) CmdIOWrite32((_A), (_R), (_V))
...
#define RTMP_OS_NETDEV_SET_TYPE_MONITOR(_pNetDev)                              \
	RTMP_OS_NETDEV_SET_TYPE(_pNetDev, ARPHRD_IEEE80211_RADIOTAP)
#else
#define RTMP_OS_NETDEV_SET_TYPE_MONITOR(_pNetDev)                              \
	RTMP_OS_NETDEV_SET_TYPE(_pNetDev, ARPHRD_IEEE80211_PRISM)
#endif
...
#define GET_OS_PKT_DATATAIL(_pkt) skb_tail_pointer(RTPKT_TO_OSPKT(_pkt))
...
#define SET_OS_PKT_DATATAIL(_pkt, _len)                                        \
	((RTPKT_TO_OSPKT(_pkt))->tail) =                                       \
		(PUCHAR)(((RTPKT_TO_OSPKT(_pkt))->data) + (_len))
#endif
...
#define OS_PKT_TAIL_BUF_EXTEND(_pkt, _Len) skb_put(RTPKT_TO_OSPKT(_pkt), _Len)

#define OS_PKT_RESERVE(_pkt, _Len) skb_reserve(RTPKT_TO_OSPKT(_pkt), _Len)
...
extern void (*wf_fwd_check_device_hook)(struct net_device *net_dev,
					signed int type, signed int mbss_idx,
					unsigned char channel,
					unsigned char link);

extern void (*wf_fwd_set_cb_num)(unsigned int band_cb_num,
				 unsigned int receive_cb_num);
...
extern void (*wf_fwd_feedback_map_table)(void *adapter, void *peer,
					 void *opp_peer, void *oth_peer);
...
extern void (*wf_fwd_insert_repeater_mapping_hook)(void *apcli_device,
						   void *lock,
						   void *cli_mapping,
						   void *map_mapping,
						   void *ifAddr_mapping);
extern int (*wf_fwd_search_mapping_table_hook)(
	struct sk_buff *skb,
	struct APCLI_BRIDGE_LEARNING_MAPPING_STRUCT **tbl_entry);
...
extern void (*wf_fwd_set_bridge_hook)(unsigned char *bridge_intf_name,
				      unsigned int len);
...
struct wifi_fwd_func_table {
	...
	unsigned char (*wf_fwd_entry_insert_hook)(struct net_device *src,
						  struct net_device *dest,
						  void *adapter);
	unsigned char (*wf_fwd_entry_delete_hook)(struct net_device *src,
						  struct net_device *dest,
						  unsigned char link_down);
	void (*wf_fwd_set_cb_num)(unsigned int band_cb_num,
				  unsigned int receive_cb_num);
	...
	void (*wf_fwd_set_bridge_hook)(unsigned char *bridge_intf_name,
				       unsigned int len);
};
...
#define RTUSB_FILL_BULK_URB(_urb, _dev, _pipe, _buffer, _buffer_len,           \
	...
	usb_fill_bulk_urb(_urb, _dev, _pipe, _buffer, _buffer_len,             \
			  _complete_fn, _context)
#else
...
#define RTUSBBulkCmdRspEventComplete(Status, pURB, pt_regs)                    \
	RTUSBBulkCmdRspEventComplete(pURB)
#ifdef MT_MAC
...
USBHST_STATUS RTUSBBulkCmdRspEventComplete(URBCompleteStatus Status,
					   purbb_t pURB, pregs *pt_regs);
#ifdef MT_MAC
USBHST_STATUS RTUSBBulkOutBCNPacketComplete(URBCompleteStatus Status,
					    purbb_t pURB, pregs *pt_regs);
#endif
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 5, 0)
#define RTUSB_FILL_TX_BULK_URB(pUrb, pUsb_Dev, uEndpointAddress, pTransferBuf, \
			       BufSize, Complete, pContext, TransferDma)       \
	do {                                                                   \
...
#define RTUSB_FILL_RX_BULK_URB(pUrb, pUsb_Dev, uEndpointAddress, pTransferBuf, \
			       BufSize, Complete, pContext, TransferDma)       \
	do {                                                                   \
		usb_fill_bulk_urb(pUrb, pUsb_Dev,                              \
...
#define RTUSB_CONTROL_MSG(pUsb_Dev, uEndpointAddress, Request, RequestType,    \
			  Value, Index, tmpBuf, TransferBufferLength, timeout, \
			  ret)                                                 \
	do {                                                                   \
		if ((RequestType == DEVICE_VENDOR_REQUEST_OUT) ||              \
			...
			ret = USB_CONTROL_MSG(                                 \
				pUsb_Dev,                                      \
				usb_sndctrlpipe(pUsb_Dev, uEndpointAddress),   \
				Request, RequestType, Value, Index, tmpBuf,    \
				TransferBufferLength, timeout);                \
		else if (RequestType == DEVICE_VENDOR_REQUEST_IN)              \
			ret = USB_CONTROL_MSG(                                 \
				pUsb_Dev,                                      \
				usb_rcvctrlpipe(pUsb_Dev, uEndpointAddress),   \
				Request, RequestType, Value, Index, tmpBuf,    \
				TransferBufferLength, timeout);                \
		else {                                                         \
...
extern void rausb_buffer_free(VOID *dev, size_t size, void *addr,
			      ra_dma_addr_t dma);
#endif /* LINUX_VERSION_CODE */
...
struct starv_dbg_block {
	...
	void (*timeout_fn)(struct starv_dbg *starv,
