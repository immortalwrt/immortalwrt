...
#define ERP_OMAC_ALL                                                           \
	((ERP_OMAC0) | (ERP_OMAC1) | (ERP_OMAC2) | (ERP_OMAC3) | (ERP_OMAC4))
typedef struct {
	/* for hdev */
	UINT8 band_idx;
	UINT8 long_nav;
...
typedef enum {
	...
	RX_BC_ALL = 1 << 5,
	RX_MC_TABLE = 1
		      << 6, /*drop mcast frame that is not in the mcast table*/
	RX_BC_MC_OWN_MAC_A3 =
		1
		<< 7, /*drop bc/mc packet matches the following condition: ToDS=0,FromDS=1,A3=OwnMAC0 or OwnMAC1*/
	RX_BC_MC_DIFF_BSSID_A3 =
		1
		<< 8, /*drop bc/mc packet matches the following condition: ToDS=0,FromDS=0,A3!=BSSID0 or BSSID1*/
	RX_BC_MC_DIFF_BSSID_A2 =
		1
		<< 9, /*drop bc/mc packet matches the following condition: ToDS=0,FromDS=1,A2!=BSSID0 or BSSID1*/
	RX_BCN_DIFF_BSSID = 1
			    << 10, /*drop bcn packet and A3!=BSSID0 or BSSID1*/
	RX_CTRL_RSV = 1 << 11, /*drop control packets with reserve type*/
	RX_CTS = 1 << 12,
	RX_RTS = 1 << 13,
	RX_DUPLICATE =
		1
		<< 14, /*drop duplicate frame, BA session not includign in this filter*/
	RX_NOT_OWN_BSSID = 1 << 15, /*drop not my BSSID0/1/2/3 if enabled*/
	RX_NOT_OWN_UCAST =
		1 << 16, /*drop uncast packet not to OWN MAC 0/1/2/3/4*/
...
} ENUM_CIPHER_SUIT_T256,
	*P_ENUM_CIPHER_SUIT_T;
...
#ifdef COMPOS_WIN
VOID MtAsicSwitchChannel(struct _RTMP_ADAPTER *pAd,
			 struct _EXT_CMD_CHAN_SWITCH_T CmdChSwitch);
#else
VOID MtAsicSwitchChannel(struct _RTMP_ADAPTER *pAd,
			 struct _MT_SWITCH_CHANNEL_CFG SwChCfg);
#endif /*COMPOS_WIN*/

VOID MtAsicUpdateProtect(struct _RTMP_ADAPTER *pAd,
			 MT_PROTECT_CTRL_T *ProtectCtrl);
VOID MtAsicUpdateRtsThld(struct _RTMP_ADAPTER *pAd,
			 MT_RTS_THRESHOLD_T *RtsThld);
...
INT32 MtAsicSetDevMac(struct _RTMP_ADAPTER *pAd, UINT8 OwnMacIdx,
...
VOID MtAsicInsertRepeaterEntry(struct _RTMP_ADAPTER *pAd, UCHAR CliIdx,
			       UCHAR *pAddr);
VOID MtAsicRemoveRepeaterEntry(struct _RTMP_ADAPTER *pAd, UCHAR CliIdx);
VOID MtAsicInsertRepeaterRootEntry(IN struct _RTMP_ADAPTER *pAd, IN UCHAR Wcid,
				   IN UCHAR *pAddr, IN UCHAR ReptCliIdx);
#endif /* MAC_REPEATER_SUPPORT */
#endif /* APCLI_SUPPORT */

#ifndef COMPOS_TESTMODE_WIN
VOID MtSetTmrCal(struct _RTMP_ADAPTER *pAd, UCHAR TmrType, UCHAR Channel,
		 UCHAR Bw);
...
INT MtAsicGetTsfTimeByDriver(struct _RTMP_ADAPTER *pAd, UINT32 *high_part,
...
#ifdef CONFIG_AP_SUPPORT
VOID APCheckBcnQHandler(struct _RTMP_ADAPTER *pAd, INT apidx,
			BOOLEAN *is_pretbtt_int);
#endif /* CONFIG_AP_SUPPORT */

VOID MtAsicDisableSyncByDriver(struct _RTMP_ADAPTER *pAd, UCHAR HWBssidIdx);
VOID MtAsicEnableBssSyncByDriver(struct _RTMP_ADAPTER *pAd, USHORT BeaconPeriod,
				 UCHAR HWBssidIdx, UCHAR OPMode);

INT MtAsicSetWmmParam(struct _RTMP_ADAPTER *pAd, UCHAR idx, UINT ac, UINT type,
		      UINT val);
...
VOID MtAsicSetSlotTime(struct _RTMP_ADAPTER *pAd, UINT32 SlotTime,
		       UINT32 SifsTime, UCHAR BandIdx);

INT MtAsicSetMacMaxLen(struct _RTMP_ADAPTER *pAd);

/* Function by Driver */
INT32 MtAsicSetDevMacByDriver(struct _RTMP_ADAPTER *pAd, UINT8 OwnMacIdx,
			      UINT8 *OwnMacAddr, UINT8 BandIdx, UINT8 Active,
			      UINT32 EnableFeature);

INT32 MtAsicSetBssidByDriver(struct _RTMP_ADAPTER *pAd,
			     struct _BSS_INFO_ARGUMENT_T bss_info_argument);

INT32 MtAsicSetStaRecByDriver(struct _RTMP_ADAPTER *pAd,
			      STA_REC_CFG_T StaRecCfg);

VOID MtAsicDelWcidTabByDriver(IN struct _RTMP_ADAPTER *pAd, IN UCHAR wcid_idx);
VOID MtAsicUpdateRxWCIDTableByDriver(IN struct _RTMP_ADAPTER *pAd,
				     IN MT_WCID_TABLE_INFO_T WtblInfo);
INT32 MtAsicUpdateBASessionByDriver(IN struct _RTMP_ADAPTER *pAd,
				    IN MT_BA_CTRL_T BaCtrl);

/* Generic fucntion */
VOID MtAsicUpdateRxWCIDTable(IN struct _RTMP_ADAPTER *pAd,
			     IN MT_WCID_TABLE_INFO_T WtblInfo);

VOID MtAsicGetTxTscByDriver(struct _RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
			    UCHAR *pTxTsc);
VOID MtAsicAddSharedKeyEntry(struct _RTMP_ADAPTER *pAd, UCHAR BssIdx,
			     UCHAR KeyIdx, struct _CIPHER_KEY *pKey);
VOID MtAsicRemoveSharedKeyEntry(struct _RTMP_ADAPTER *pAd, UCHAR BssIndex,
				UCHAR KeyIdx);
VOID MtAsicTxCntUpdate(struct _RTMP_ADAPTER *pAd, UCHAR wcid,
		       struct _MT_TX_COUNTER *pTxInfo);
...
VOID MtAsicTxCapAndRateTableUpdate(struct _RTMP_ADAPTER *pAd, UCHAR ucWcid,
				   struct _RA_PHY_CFG_T *prTxPhyCfg,
				   UINT32 *Rate, BOOLEAN fgSpeEn);

VOID MtAsicAddPairwiseKeyEntry(struct _RTMP_ADAPTER *pAd, UCHAR WCID,
			       struct _CIPHER_KEY *pKey);

INT MtAsicSendCommandToMcu(IN struct _RTMP_ADAPTER *pAd, IN UCHAR Command,
			   IN UCHAR Token, IN UCHAR Arg0, IN UCHAR Arg1,
			   IN BOOLEAN in_atomic);
BOOLEAN MtAsicSendCmdToMcuAndWait(IN struct _RTMP_ADAPTER *pAd,
				  IN UCHAR Command, IN UCHAR Token,
				  IN UCHAR Arg0, IN UCHAR Arg1,
				  IN BOOLEAN in_atomic);
BOOLEAN MtAsicSendCommandToMcuBBP(IN struct _RTMP_ADAPTER *pAd,
				  IN UCHAR Command, IN UCHAR Token,
				  IN UCHAR Arg0, IN UCHAR Arg1,
...
VOID MtAsicSetStreamMode(struct _RTMP_ADAPTER *pAd, UCHAR *mac, INT chainIdx,
			 BOOLEAN enable);
...
BOOLEAN MtAsicSetBmcQCR(IN struct _RTMP_ADAPTER *pAd, IN UCHAR Operation,
			IN UCHAR CrReadWrite, IN UCHAR HwBssidIdx,
			IN UINT32 apidx, IN OUT UINT32 *pcr_val);
...
VOID MtAsicSetDmaFQCR(IN struct _RTMP_ADAPTER *pAd, IN BOOLEAN FQ_MODE,
		      IN UCHAR DEST_QID, IN UCHAR DEST_PID, IN UCHAR TARG_QID,
		      IN UCHAR apidx, IN UCHAR widx);
...
INT MtAsicStartContinousTx(struct _RTMP_ADAPTER *pAd, UINT32 PhyMode, UINT32 BW,
			   UINT32 PriCh, UINT32 Mcs, UINT32 WFSel);
#ifdef COMPOS_TESTMODE_WIN
INT MTAsicTxSetFrequencyOffset(struct _RTMP_ADAPTER *pAd, UINT32 iOffset,
			       BOOLEAN HasBeenSet);
INT MTAsicTxConfigPowerRange(struct _RTMP_ADAPTER *pAd, UCHAR ucMaxPowerDbm,
			     UCHAR ucMinPowerDbm);
INT MTAsicSetTMR(struct _RTMP_ADAPTER *pAd, UCHAR enable);
#endif
#endif /* CONFIG_ATE */

VOID MtAsicSetRxGroup(struct _RTMP_ADAPTER *pAd, UINT32 Port, UINT32 Group,
		      BOOLEAN Enable);

#ifdef DMA_SCH_SUPPORT
INT32 MtAsicDMASchedulerInit(struct _RTMP_ADAPTER *pAd,
...
VOID MtAsicSetBARTxCntLimit(struct _RTMP_ADAPTER *pAd, BOOLEAN Enable,
			    UINT32 Count);
VOID MtAsicSetTxSClassifyFilter(struct _RTMP_ADAPTER *pAd, UINT32 Port,
				UINT8 DestQ, UINT32 AggNums, UINT32 Filter,
				UCHAR BandIdx);
VOID MtAsicInitMac(struct _RTMP_ADAPTER *pAd);
UINT32 MtAsicGetWmmParam(struct _RTMP_ADAPTER *pAd, UINT32 AcNum,
			 UINT32 EdcaType);
VOID MtAsicAddRemoveKey(struct _RTMP_ADAPTER *pAd,
			MT_SECURITY_CTRL SecurityCtrl);
#if defined(COMPOS_WIN) || defined(COMPOS_TESTMODE_WIN)
#else
VOID MtAsicAddRemoveKeyByDriver(struct _RTMP_ADAPTER *pAd,
				struct _ASIC_SEC_INFO *pInfo);
#endif
INT32 MtAsicGetMacInfo(struct _RTMP_ADAPTER *pAd, UINT32 *ChipId, UINT32 *HwVer,
		       UINT32 *FwVer);
...
#ifdef CONFIG_AP_SUPPORT
VOID MtDmacSetMbssHwCRSetting(struct _RTMP_ADAPTER *pAd, UCHAR mbss_idx,
			      BOOLEAN enable);
VOID MtDmacSetExtTTTTHwCRSetting(struct _RTMP_ADAPTER *pAd, UCHAR mbss_idx,
				 BOOLEAN enable);
VOID MtDmacSetExtMbssEnableCR(struct _RTMP_ADAPTER *pAd, UCHAR mbss_idx,
			      BOOLEAN enable);
#endif
...
INT32 MtAsicSetTxQ(struct _RTMP_ADAPTER *pAd, INT WmmSet, INT BandIdx,
		   BOOLEAN Enable);

VOID MtAsicSetMbssLPOffset(struct _RTMP_ADAPTER *pAd, UCHAR mbss_idx,
			   BOOLEAN enable);
VOID MTRestartFW(struct _RTMP_ADAPTER *pAd);
INT32 MtAsicRxHeaderTransCtl(struct _RTMP_ADAPTER *pAd, BOOLEAN En,
			     BOOLEAN ChkBssid, BOOLEAN InSVlan, BOOLEAN RmVlan,
			     BOOLEAN SwPcP);
INT32 MtAsicRxHeaderTaranBLCtl(struct _RTMP_ADAPTER *pAd, UINT32 Index,
			       BOOLEAN En, UINT32 EthType);

#ifdef DBDC_MODE
INT32 MtAsicGetDbdcCtrl(struct _RTMP_ADAPTER *pAd,
			struct _BCTRL_INFO_T *pbInfo);
INT32 MtAsicSetDbdcCtrl(struct _RTMP_ADAPTER *pAd,
			struct _BCTRL_INFO_T *pbInfo);
#endif /*DBDC_MODE*/
...
VOID MtSmacSetMbssHwCRSetting(struct _RTMP_ADAPTER *pAd, UCHAR mbss_idx,
			      BOOLEAN enable);
VOID MtSmacSetExtTTTTHwCRSetting(struct _RTMP_ADAPTER *pAd, UCHAR mbss_idx,
				 BOOLEAN enable);
VOID MtSmacSetExtMbssEnableCR(struct _RTMP_ADAPTER *pAd, UCHAR mbss_idx,
			      BOOLEAN enable);
/* SMAC End */

#ifdef DOT11_VHT_AC
INT MtAsicSetRtsSignalTA(struct _RTMP_ADAPTER *pAd, UINT8 BandIdx,
			 BOOLEAN Enable);
#endif /*DOT11_VHT_AC*/
