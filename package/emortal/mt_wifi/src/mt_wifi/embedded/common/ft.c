	...
	IN INT32 PktLen)
{
	...
	if ((ft_act_len + sizeof(HEADER_802_11)) < MAX_MGMT_PKT_LEN) {
		...
		MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_ERROR, ("%s(): len is overflow\n", __func__));
	if (pOutBuffer)
	...
	OUT PFT_INFO			pFtInfo_out)
{
...

	{
		...
		*/
		...
		hex_dump_with_cat_and_lvl("pPmkR0Name=", pPmkR0Name, LEN_PMK_NAME, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO);
		hex_dump_with_cat_and_lvl("pAd->ApCfg.MBSSID[pEntry->func_tb_idx].Bssid=",
				 wdev->bssid, 6, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO);
		hex_dump_with_cat_and_lvl("pEntry->Addr=", pEntry->Addr, 6, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO);
		hex_dump_with_cat_and_lvl("pEntry->FT_PMK_R1_NAME=", pEntry->FT_PMK_R1_NAME,
				 sizeof(pEntry->FT_PMK_R1_NAME), DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO);
...
*/
		...
		*/
		if ((pR1hkEntry == NULL) ||
			...
			hex_dump_with_cat_and_lvl("Peer PMKR0Name", pPmkR0Name, LEN_PMK_NAME, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_TRACE);
			if (pR1hkEntry)
					hex_dump_with_cat_and_lvl("Own PMKR0Name",
								pR1hkEntry->PmkR0Name, LEN_PMK_NAME, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_TRACE);
			return FT_STATUS_CODE_INVALID_PMKID;
		}
		/* YF_TIE */
	...
	hex_dump_with_cat_and_lvl("anonce", handshake->ANonce, 32, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO);
	hex_dump_with_cat_and_lvl("snonce", handshake->SNonce, 32, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO);
	...
	IN	PFT_INFO			pFtInfo_in)
{
	...
	if (!RTMPEqualMemory(ft_mic, pFtInfo_in->FtIeInfo.MIC, FT_MIC_LEN)) {
		MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_ERROR,
				 ("%s : MIC is different\n", __func__));
		hex_dump_with_cat_and_lvl("received MIC", pFtInfo_in->FtIeInfo.MIC, FT_MIC_LEN, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_TRACE);
		hex_dump_with_cat_and_lvl("desired  MIC", ft_mic, FT_MIC_LEN, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_TRACE);
		return 0xFFFF;
	}
	...
	*/
	...
	if ((pmkid_ptr == NULL) ||
		...
		hex_dump_with_cat_and_lvl("FT_PMK_R1_NAME", pEntry->FT_PMK_R1_NAME, LEN_PMK_NAME, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_TRACE);
		hex_dump_with_cat_and_lvl("peer pmkid", pmkid_ptr, LEN_PMK_NAME, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_TRACE);
		MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_ERROR,
				 ("%s : Invalid PMKID\n", __func__));
		return FT_STATUS_CODE_INVALID_PMKID;
	}
	...
	OUT PFT_INFO			pFtInfo_out)
{
	...
	if (!RTMPEqualMemory(ft_mic, pFtInfo_in->FtIeInfo.MIC, 16)) {
		MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_ERROR,
				 ("%s : MIC is different\n", __func__));
		hex_dump_with_cat_and_lvl("received MIC", pFtInfo_in->FtIeInfo.MIC, 16, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_TRACE);
		hex_dump_with_cat_and_lvl("desired  MIC", ft_mic, 16, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_TRACE);
		return 0xFFFF;
	}
	...
	if (pFtInfo_in->FtIeInfo.MICCtr.field.rsnxe_used
		...
		MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_ERROR, ("%s : Invalid FTIE: rsnxe_used is 1 while no rsnxe ie\n", __func__));
	...
	if (pEntry->SecConfig.ocv_support &&
		...
		MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_ERROR, ("%s : OCI verify fail(len=%d)\n", __func__, pFtInfo_in->FtIeInfo.OCILen));
		return FT_STATUS_CODE_INVALID_FTIE;
	}
	...
	OUT	PUINT8	pmkr0_name)
{
	...
	/* R0KH-ID(5~48 bytes) */
	/* S0KH-ID(6 bytes) */
	hex_dump_with_cat_and_lvl("xxkey", (PUCHAR)xxkey, xxkey_len, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO);
	hex_dump_with_cat_and_lvl("label", (PUCHAR)label_name, sizeof(label_name), DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO);
	hex_dump_with_cat_and_lvl("ssid", (PUCHAR)ssid, ssid_len, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO);
	hex_dump_with_cat_and_lvl("mdis", (PUCHAR)mdid, 2, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO);
	hex_dump_with_cat_and_lvl("r0khid", (PUCHAR)r0khid, r0khid_len, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO);
	hex_dump_with_cat_and_lvl("s0khid", (PUCHAR)s0khid, MAC_ADDR_LEN, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO);
	...
	hex_dump_with_cat_and_lvl("PMK-R0", (UCHAR *)pmkr0, LEN_PMK, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_TRACE);
	hex_dump_with_cat_and_lvl("PMK-R0-Name", (UCHAR *)pmkr0_name, LEN_PMK_NAME, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_TRACE);
}
	...
	OUT	PUINT8	pmkr1_name)
{
	...
	hex_dump_with_cat_and_lvl("PMK-R1-Name", (UCHAR *)pmkr1_name, LEN_PMK_NAME, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_TRACE);
}
	...
	OUT	PUINT8	pmkr1_name)
{
	...
	/* =========================== */
	MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_TRACE, ("%s:\n", __func__));
	hex_dump_with_cat_and_lvl("pmkr0", pmkr0, 32, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO);
	hex_dump_with_cat_and_lvl("pmkr0_name", pmkr0_name, LEN_PMK_NAME, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO);
	hex_dump_with_cat_and_lvl("r1khid", r1khid, MAC_ADDR_LEN, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO);
	hex_dump_with_cat_and_lvl("s1khid", s1khid, MAC_ADDR_LEN, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO);
	hex_dump_with_cat_and_lvl("pmkr1", pmkr1, 32, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO);
	hex_dump_with_cat_and_lvl("pmkr1_name", pmkr1_name, 16, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO);
	...
	FT_DerivePMKR1Name(pmkr0_name,
					   ...
					   s1khid,
					   pmkr1_name);
	hex_dump_with_cat_and_lvl("PMK-R1", (UCHAR *)pmkr1, LEN_PMK, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_TRACE);
	hex_dump_with_cat_and_lvl("PMK-R1-Name", (UCHAR *)pmkr1_name, LEN_PMK_NAME, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_TRACE);
}
	...
	OUT	PUINT8	ptk_name)
{
	...
	hex_dump_with_cat_and_lvl("PTK", (UCHAR *)ptk, key_len, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_TRACE);
	hex_dump_with_cat_and_lvl("PTK-Name", (UCHAR *)ptk_name, LEN_PMK_NAME, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_TRACE);
}
	...
	RTMP_STRING *pBuffer)
{
	...
	if (RTMPGetKeyParameter("FtOnly", tmpbuf, 32, pBuffer, TRUE)) {
		...
		for (Loop = 0, macptr = rstrtok(tmpbuf, ";"); macptr; macptr = rstrtok(NULL, ";"), Loop++) {
			...
			MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_TRACE, ("I/F(ra%d) ft_only=%d\n",
