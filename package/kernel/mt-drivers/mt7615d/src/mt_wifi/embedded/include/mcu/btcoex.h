...
#define IS_MT76XXBTCOMBO(_pAd)                                                 \
	(IS_MT7650(_pAd) || IS_MT7630(_pAd) || IS_MT7662(_pAd))
...
VOID MT76xxMLMEHook(IN struct _RTMP_ADAPTER *pAd, IN UCHAR WlanStatus,
		    IN ULONG BssHashID);
...
INT Set_CoexSkipFDDFix20MH_Proc(IN RTMP_ADAPTER *pAd, IN RTMP_STRING *arg);

INT Set_MT76xxCoex_Protection_Mode_Proc(struct _RTMP_ADAPTER *pAd,
					RTMP_STRING *arg);
INT Set_MT76xxCoex_Protection_Rate_Proc(struct _RTMP_ADAPTER *pAd,
					RTMP_STRING *arg);
#endif /* COEX_SUPPORT */
...
#define BTCOEX_BB_BITMASK_READ(_BbReg, _BitLocation) ((_BbReg) & (_BitLocation))

#define MAC_BITWISE_WRITE(_MacReg, _BitLocation, _BitValue)                    \
	(_MacReg = (((_MacReg) & ~(_BitLocation)) | (_BitValue)))

#define MAC_BITMASK_READ(_MacReg, _BitLocation) ((_MacReg) & (_BitLocation))
#endif
