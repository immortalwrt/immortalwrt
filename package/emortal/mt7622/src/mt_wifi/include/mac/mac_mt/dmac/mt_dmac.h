...
#define TMI_HDR_INFO_VAL(_fmt, _mrd, _eosp, _rmvl, _vlan, _etype, _len, _ams,  \
			 _val)                                                 \
	do {                                                                   \
		switch (_fmt) {                                                \
		case TMI_HDR_FT_NON_80211:                                     \
			(_val) = TMI_HDR_INFO_0_VAL((_mrd), (_eosp), (_rmvl),  \
						    (_vlan), (_etype));        \
			break;                                                 \
		case TMI_HDR_FT_CMD:                                           \
...
INT mt_wtbl_get_entry234(struct _RTMP_ADAPTER *pAd, UCHAR idx,
...
struct _TMAC_INFO;
INT32 dump_txp_info(struct _RTMP_ADAPTER *pAd,
		    struct _CR4_TXP_MSDU_INFO *txp_info);
INT32 mtd_write_txp_info_by_cr4(struct _RTMP_ADAPTER *pAd, UCHAR *buf,
				struct _TX_BLK *pTxBlk);
INT32 mtd_write_txp_info_by_host(struct _RTMP_ADAPTER *pAd, UCHAR *buf,
				 struct _TX_BLK *pTxBlk);
INT32 mtd_write_txp_info_by_host_v2(struct _RTMP_ADAPTER *pAd, UCHAR *buf,
				    struct _TX_BLK *pTxBlk);
VOID mtd_write_tmac_info(struct _RTMP_ADAPTER *pAd, UCHAR *buf,
			 struct _TX_BLK *pTxBlk);
VOID mtd_write_tmac_info_by_cr4(struct _RTMP_ADAPTER *pAd, UCHAR *buf,
				struct _TX_BLK *pTxBlk);
VOID mtd_write_tmac_info_by_host(struct _RTMP_ADAPTER *pAd, UCHAR *buf,
				 struct _TX_BLK *pTxBlk);
INT mtd_check_hw_resource(struct _RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
			  UCHAR resource_idx);
INT mtd_hw_tx(struct _RTMP_ADAPTER *pAd, struct _TX_BLK *tx_blk);
INT mtd_mlme_hw_tx(struct _RTMP_ADAPTER *pAd, UCHAR *tmac_info,
		   struct _MAC_TX_INFO *info,
		   union _HTTRANSMIT_SETTING *transmit, struct _TX_BLK *tx_blk);
INT32 mtd_ate_hw_tx(struct _RTMP_ADAPTER *pAd, struct _TMAC_INFO *info,
