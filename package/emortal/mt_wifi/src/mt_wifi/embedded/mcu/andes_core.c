...
struct cmd_msg *AndesAllocCmdMsgGe(RTMP_ADAPTER *ad, unsigned int length)
{
...
{
	return hif_mcu_alloc_msg(ad, length);
}
...
INT32 AndesSendCmdMsg(PRTMP_ADAPTER ad, struct cmd_msg *msg)
{
	...
	if (IsStopingPdma(&ad->ErrRecoveryCtl)) {
		MTWF_LOG(DBG_CAT_HW, CATHW_SER, DBG_LVL_WARN,
				 ("%s(SER Period): Command type = %x, Extension command type = %x\n",
				  __func__, msg->attr.type, msg->attr.ext_type));
		AndesForceFreeCmdMsg(msg);
		return NDIS_STATUS_FAILURE;
	}
	...
	if (mdvt_block_command(ad, msg) == TRUE) {
		MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
				 ("%s: MDVT Block,Command type = %x, Extension command type = %x\n",
				  __func__, msg->attr.type, msg->attr.ext_type));
		AndesForceFreeCmdMsg(msg);
		return NDIS_STATUS_FAILURE;
	}
