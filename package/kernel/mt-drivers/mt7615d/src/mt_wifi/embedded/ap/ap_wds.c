...
BOOLEAN ValidWdsEntry(IN PRTMP_ADAPTER pAd, IN UCHAR WdsIndex)
{
	...
	if ((WdsIndex < MAX_WDS_ENTRY) &&
		...
		if ((pAd->WdsTab.WdsEntry[WdsIndex].MacTabMatchWCID > 0) &&
		    (pAd->WdsTab.WdsEntry[WdsIndex].MacTabMatchWCID <
		     GET_MAX_UCAST_NUM(pAd))) {
			pMacEntry =
				&pAd->MacTab
					 .Content[pAd->WdsTab.WdsEntry[WdsIndex]
							  .MacTabMatchWCID];
...
INT wds_fp_tx_pkt_allowed(IN RTMP_ADAPTER *pAd, IN struct wifi_dev *wdev,
			  IN PNDIS_PACKET pkt)
{
	UCHAR idx;
	INT allowed = FALSE;
...
INT wds_tx_pkt_allowed(IN RTMP_ADAPTER *pAd, IN struct wifi_dev *wdev,
		       IN PNDIS_PACKET pkt)
{
	UCHAR idx;
	INT allowed = FALSE;
...
INT wds_rx_foward_handle(RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
...
VOID WdsEntryDel(RTMP_ADAPTER *pAd, UCHAR *pAddr)
{
	...
	for (i = 0; i < MAX_WDS_ENTRY; i++) {
		...
		if (MAC_ADDR_EQUAL(pAddr, wds_entry->PeerWdsAddr) &&
		    (wds_entry->Valid == TRUE)) {
...
/*
	...
	==========================================================================
 */
BOOLEAN MacTableDeleteWDSEntry(IN PRTMP_ADAPTER pAd, IN USHORT wcid)
{
...
BOOLEAN wds_bss_linkdown(IN PRTMP_ADAPTER pAd, IN USHORT wcid)
{
	MAC_TABLE_ENTRY *pEntry;
...
================================================================
*/
MAC_TABLE_ENTRY *MacTableInsertWDSEntry(IN RTMP_ADAPTER *pAd, IN UCHAR *pAddr,
					UINT WdsTabIdx)
{
	...
	MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("%s(): WdsTabIdx = %d, Addr %02x-%02x-%02x-%02x-%02x-%02x\n",
		  __func__, WdsTabIdx, PRINT_MAC(pAddr)));
	...
	do {
		...
		pEntry = MacTableInsertEntry(pAd, pAddr, wdev, ENTRY_WDS,
					     OPMODE_AP, TRUE);
		...
		if (pEntry) {
			...
			MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				 ("  OmacIdx = %d, wcid = %d, PhyOpMode = %d\n",
				  tr_entry->OmacIdx, pEntry->wcid,
				  wds_entry->PhyOpMode));
			...
			if (wds_entry->PhyOpMode >= MODE_HTMIX) {
				...
				if (HcIsBfCapSupport(wdev) == FALSE) {
					...
					mt_WrapSetETxBFCap(
						pAd, wdev,
						&pEntry->HTCapability.TxBFCap);
					wlan_config_set_etxbf(wdev, ucEBfCap);
				}

#endif /* TXBF_SUPPORT */
				set_sta_ht_cap(pAd, pEntry,
					       &pEntry->HTCapability);
				...
				if (WMODE_CAP_AC(wdev->PhyMode) &&
...
#endif /* TXBF_SUPPORT && VHT_TXBF_SUPPORT */
					build_vht_cap_ie(pAd, wdev,
							 (UCHAR *)&vht_cap);
#if defined(TXBF_SUPPORT) && defined(VHT_TXBF_SUPPORT)
					wlan_config_set_etxbf(wdev, ucETxBfCap);
#endif /* TXBF_SUPPORT && VHT_TXBF_SUPPORT */
					vht_mode_adjust(pAd, pEntry, &vht_cap,
							NULL);
					...
					MTWF_LOG(
						DBG_CAT_AP, DBG_SUBCAT_ALL,
						DBG_LVL_OFF,
						("%s(): Peer's PhyCap=>Mode:%s, BW:%s, MCS: 0x%x (Word = 0x%x)\n",
						 __func__,
						 get_phymode_str(
							 pEntry->MaxHTPhyMode
								 .field.MODE),
						 get_bw_str(pEntry->MaxHTPhyMode
								    .field.BW),
						 pEntry->MaxHTPhyMode.field.MCS,
						 pEntry->MaxHTPhyMode.word));
					NdisMoveMemory(&pEntry->vht_cap_ie,
						       &vht_cap,
						       sizeof(VHT_CAP_IE));
					assoc_vht_info_debugshow(
						pAd, pEntry, &vht_cap, NULL);
				} else
					NdisZeroMemory(&pEntry->vht_cap_ie,
						       sizeof(VHT_CAP_IE));
				...
				NdisZeroMemory(&pEntry->HTCapability,
					       sizeof(HT_CAPABILITY_IE));

			RTMPSetSupportMCS(pAd, OPMODE_AP, pEntry, rate->SupRate,
					  rate->SupRateLen, rate->ExtRate,
					  rate->ExtRateLen,
#ifdef DOT11_VHT_AC
					  sizeof(VHT_CAP_IE),
					  &pEntry->vht_cap_ie,
			...
			if (wdev->bAutoTxRateSwitch == FALSE) {
				...
				RTMPUpdateLegacyTxSetting(
					(UCHAR)wdev->DesiredTransmitSetting
						.field.FixedTxMode,
					pEntry);
			} else {
			...
			/* update per wdev bw */
			wlan_operate_set_ht_bw(wdev,
					       wdev->MaxHTPhyMode.field.BW,
			...
			MTWF_LOG(
				...
				("%s() - allocate entry #%d(link to WCID %d), Total= %d\n",
				 __func__, WdsTabIdx,
				 wds_entry->MacTabMatchWCID, pAd->WdsTab.Size));
			MTWF_LOG(
				DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			...
			MTWF_LOG(
				...
				("  DesiredHtPhyInfo HtEn = %d, PreHt = %d, VhtEn = %d, vht_bw = %d\n",
				 ...
				 wdev->DesiredHtPhyInfo.bPreNHt,
				 wdev->DesiredHtPhyInfo.bVhtEnable,
...
MAC_TABLE_ENTRY *FindWdsEntry(IN RTMP_ADAPTER *pAd, IN struct _RX_BLK *pRxBlk)
{
	MAC_TABLE_ENTRY *pEntry;
	...
	defined(NEIGHBORING_AP_STAT)
			...
			OPMODE_AP, wdev, pRxBlk->rx_rate.field.MODE);
...
VOID WdsTableMaintenance(RTMP_ADAPTER *pAd)
{
	...
	for (idx = 0; idx < pAd->WdsTab.Size; idx++) {
		...
		MTWF_LOG(
			...
			("%s(): Entry[%d], Type = %d, Wcid = %d, %02x:%02x:%02x:%02x:%02x:%02x\n",
			 __func__, idx, pEntry->EntryType, wcid,
		...
		if (pEntry->NoDataIdleCount >= MAC_TABLE_AGEOUT_TIME) {
			MTWF_LOG(
				DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
				("ageout %02x:%02x:%02x:%02x:%02x:%02x from WDS #%d after %d-sec silence\n",
				 PRINT_MAC(pEntry->Addr), idx,
...
VOID AsicUpdateWdsRxWCIDTable(RTMP_ADAPTER *pAd, UINT WdsTabIdx)
{
	...
	MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("%s(): WdsEntry[%d] = %p, WDS_Mode = %d, PhyOpMode = %d\n",
		  __func__, WdsTabIdx, wds_entry, pAd->WdsTab.Mode,
		  wds_entry->PhyOpMode));
	...
	if (pAd->CommonCfg.HT_DisallowTKIP &&
		...
		wds_entry->PhyOpMode = (wds_entry->PhyOpMode >= MODE_OFDM) ?
					       MODE_OFDM :
						     MODE_CCK;
		MTWF_LOG(
	...
	if (!pEntry) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			 ("%s(): can't insert a new WDS entry!", __func__));
		return;
	}
...
VOID AsicUpdateWdsEncryption(RTMP_ADAPTER *pAd, UCHAR wcid)
{
	...
	if (IS_CIPHER_WEP(pSecConfig->PairwiseCipher)) {
		os_move_mem(&Info.Key, &pSecConfig->WepKey[Info.KeyIdx],
			    sizeof(SEC_KEY_INFO));
		...
		SetWPAPSKKey(pAd, pSecConfig->PSK, strlen(pSecConfig->PSK),
			     ...
			     sizeof(RALINK_PASSPHRASE), pSecConfig->PMK);
		...
		if (IS_CIPHER_TKIP(pSecConfig->PairwiseCipher)) {
			...
			os_move_mem(&Info.Key.Key[LEN_TK + LEN_TKIP_MIC],
				    &Info.Key.Key[LEN_TK], LEN_TKIP_MIC);
		}
...
UCHAR WdsGetPeerSuppPhyModeLegacy(IN PUCHAR SupRate, IN UCHAR SupRateLen,
				  IN UCHAR Channel)
{
...
UCHAR WdsGetPeerSuppPhyMode(IN BCN_IE_LIST *ie_list)
{
	UCHAR PeerPhyMode = 0;
#ifdef DOT11_N_SUPPORT
#ifdef DOT11_VHT_AC
	...
	PeerPhyMode |= WdsGetPeerSuppPhyModeLegacy(
		ie_list->SupRate, ie_list->SupRateLen, ie_list->Channel);
	PeerPhyMode |= WdsGetPeerSuppPhyModeLegacy(
		ie_list->ExtRate, ie_list->ExtRateLen, ie_list->Channel);
	MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_INFO,
		 ("%s(): %02x-%02x-%02x-%02x-%02x-%02x, PeerPhyMode = %d\n",
		  __func__, PRINT_MAC(ie_list->Addr2), PeerPhyMode));

	if (((ie_list->Channel > 0) && (ie_list->Channel <= 14) &&
	     WMODE_CAP_5G(PeerPhyMode)) ||
	    ((ie_list->Channel > 14) && WMODE_CAP_2G(PeerPhyMode)))
		MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			 ("ERROR!! Wrong PeerPhyMode 0x%x, Channel %d\n",
			  PeerPhyMode, ie_list->Channel));

	return PeerPhyMode;
}

VOID WdsPeerBeaconProc(IN PRTMP_ADAPTER pAd, IN PMAC_TABLE_ENTRY pEntry,
		       IN UCHAR MaxSupportedRateIn500Kbps,
		       IN UCHAR MaxSupportedRateLen, IN BOOLEAN bWmmCapable,
		       IN BCN_IE_LIST *ie_list)
{
	...
	MTWF_LOG(
		DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s(): wcid=%d, MaxSupportedRate = %d, HtCapLen = %d, VhtCapLen = %d\n",
		 __func__, pEntry->wcid, MaxSupportedRate, HtCapLen,
#ifdef DOT11_VHT_AC
		 VhtCapLen));
#else
		 0));
	...
	if (pEntry && IS_ENTRY_WDS(pEntry)) {
		...
		pEntry->MaxSupportedRate =
			min(pEntry->wdev->rate.MaxTxRate, MaxSupportedRate);
		...
		if (((ApPhyMode & WMODE_G) && (PeerPhyMode & WMODE_G)) ||
			...
			pEntry->wdev->PhyMode =
				(PeerPhyMode & WMODE_G) ? WMODE_G : WMODE_A;
...
#ifdef DOT11_N_SUPPORT
		curr_ht_cap = (HT_CAPABILITY_IE *)wlan_operate_get_ht_cap(
			pEntry->wdev);
		...
		if ((HtCapLen != 0) && WMODE_CAP_N(ApPhyMode) &&
			...
			pWdsEntry->wdev.PhyMode =
				WdsPhyOpModeToSuppPhyMode(pAd, pWdsEntry) &
				PeerPhyMode;

			if (pEntry->wdev) {
				/* ap and wds_peer both support BW40 */
				if (wlan_operate_get_ht_bw(
					    &(pAd->ApCfg.MBSSID[MAIN_MBSSID]
						      .wdev)) == HT_BW_40 &&
				    pHtCap->HtCapInfo.ChannelWidth ==
					    HT_BW_40) {
					new_bw = HT_BW_40;
				}

				if (wlan_operate_get_ht_bw(pEntry->wdev) !=
				    new_bw) {
					MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL,
						 DBG_LVL_TRACE,
						 ("%s(): set new_bw = %d\n",
			...
			/* find max fixed rate */
			pEntry->MaxHTPhyMode.field.MCS = get_ht_max_mcs(
				pAd,
				&pWdsEntry->wdev.DesiredHtPhyInfo.MCSSet[0],
				&pHtCap->MCSSet[0]);

			if ((pEntry->MaxHTPhyMode.field.MCS >
			     pWdsEntry->wdev.HTPhyMode.field.MCS) &&
			    (pWdsEntry->wdev.HTPhyMode.field.MCS != MCS_AUTO))
				pEntry->MaxHTPhyMode.field.MCS =
					pWdsEntry->wdev.HTPhyMode.field.MCS;

			pEntry->MaxHTPhyMode.field.STBC =
				(pHtCap->HtCapInfo.RxSTBC &
				 (curr_ht_cap->HtCapInfo.RxSTBC));
			set_sta_ht_cap(pAd, pEntry, pHtCap);
			NdisMoveMemory(&pEntry->HTCapability, pHtCap,
				       sizeof(HT_CAPABILITY_IE));
			NdisMoveMemory(curr_ht_cap, pHtCap,
				       sizeof(HT_CAPABILITY_IE));
#ifdef DOT11_VHT_AC
			MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_INFO,
				 ("  vhtCapLen = %d\n", VhtCapLen));
			...
			if (WMODE_CAP_AC(ApPhyMode) &&
			    (pWdsEntry->wdev.channel > 14) && VhtCapLen) {
				RT_PHY_INFO *pDesired_ht_phy =
					&pEntry->wdev->DesiredHtPhyInfo;
				...
				if (pDesired_ht_phy &&
					...
					rtmp_set_vht(pAd, &pWdsEntry->wdev,
						     pDesired_ht_phy);
				}

				vht_mode_adjust(pAd, pEntry, pVhtCap, NULL);
				dot11_vht_mcs_to_internal_mcs(
					pAd, pEntry->wdev, pVhtCap,
					&pEntry->MaxHTPhyMode);
				...
				MTWF_LOG(
					...
					("%s(): Peer's PhyCap=>Mode:%s, BW:%s, MCS: 0x%x (Word = 0x%x)\n",
					 ...
					 pEntry->MaxHTPhyMode.field.MCS,
					 pEntry->MaxHTPhyMode.word));
				NdisMoveMemory(&pEntry->vht_cap_ie, pVhtCap,
					       sizeof(VHT_CAP_IE));
...
#endif /* DOT11_N_SUPPORT */
		pWdsEntry->wdev.PhyMode =
			WdsPhyOpModeToSuppPhyMode(pAd, pWdsEntry) & PeerPhyMode;
		rate = &pWdsEntry->wdev.rate;
		RTMPSetSupportMCS(pAd, OPMODE_AP, pEntry, rate->SupRate,
				  rate->SupRateLen, rate->ExtRate,
				  rate->ExtRateLen,
#ifdef DOT11_VHT_AC
				  sizeof(VHT_CAP_IE), &pEntry->vht_cap_ie,
#endif /* DOT11_VHT_AC */
				  curr_ht_cap, sizeof(HT_CAPABILITY_IE));
		...
		MTWF_LOG(
			DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_INFO,
			(" Mode %d, MCS %d, STBC %d SGI %d, BW %d / MAX Mode %d, MCS %d, STBC %d SGI %d, BW %d\n",
		...
		if ((pEntry->HTPhyMode.field.MODE !=
		     pEntry->MaxHTPhyMode.field.MODE) ||
		    (pEntry->HTPhyMode.field.STBC !=
		     pEntry->MaxHTPhyMode.field.STBC) ||
		    (pEntry->HTPhyMode.field.ShortGI !=
			...
			MTWF_LOG(
				DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				(" bRaReInit, New Mode %d, MCS %d, STBC %d SGI %d, BW %d\n",
				 pEntry->HTPhyMode.field.MODE,
				 pEntry->HTPhyMode.field.MCS,
				 pEntry->HTPhyMode.field.STBC,
				 pEntry->HTPhyMode.field.ShortGI,
		...
		pEntry->SupportRateMode =
			WdsPhyOpModeToSuppRateMode(pAd, pWdsEntry);
	}
...
INT Show_WdsTable_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	for (i = 0; i < MAX_WDS_ENTRY; i++) {
		MTWF_LOG(
			DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			("IF/WDS%d-%02x:%02x:%02x:%02x:%02x:%02x(%s), OpState=%d, Cipher=%s, KeyId=%d\n",
			 i, PRINT_MAC(pAd->WdsTab.WdsEntry[i].PeerWdsAddr),
			 pAd->WdsTab.WdsEntry[i].Valid == 1 ? " Valid " :
								    "Invalid",
			 wlan_operate_get_state(&pAd->WdsTab.WdsEntry[i].wdev),
			 GetEncryModeStr(pAd->WdsTab.WdsEntry[i]
						 .wdev.SecConfig.PairwiseCipher),
			 pAd->WdsTab.WdsEntry[i].wdev.SecConfig.PairwiseKeyId));

		if (pAd->WdsTab.WdsEntry[i].WdsKey.KeyLen > 0)
			hex_dump("Wds Key", pAd->WdsTab.WdsEntry[i].WdsKey.Key,
				 pAd->WdsTab.WdsEntry[i].WdsKey.KeyLen);
	}

	MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("\n%-19s%-4s%-4s%-4s%-7s%-7s%-7s%-10s%-6s%-6s%-6s%-6s\n",
		  "MAC", "IDX", "AID", "PSM", "RSSI0", "RSSI1", "RSSI2", "PhMd",
		  "BW", "MCS", "SGI", "STBC"));
	...
	for (i = 0; VALID_UCAST_ENTRY_WCID(pAd, i); i++) {
		...
		if (IS_ENTRY_WDS(pEntry)) {
			...
			MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				 ("%-6d", pEntry->MaxHTPhyMode.field.ShortGI));
			MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				 ("%-6d\n", pEntry->MaxHTPhyMode.field.STBC));
		}
	}
...
VOID rtmp_read_wds_from_file(RTMP_ADAPTER *pAd, RTMP_STRING *tmpbuf,
			     RTMP_STRING *buffer)
{
	RTMP_STRING *macptr;
	INT i = 0, j;
	...
	/*WdsPhyMode */
	if (RTMPGetKeyParameter("WdsPhyMode", tmpbuf, MAX_PARAM_BUFFER_SIZE,
				buffer, TRUE)) {
		for (i = 0, macptr = rstrtok(tmpbuf, ";");
			...
			wdev->PhyMode =
				WdsPhyOpModeToSuppPhyMode(pAd, pWdsEntry);
			MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				 ("If/wds%d - PeerPhyOpMode=%d\n", i,
				  pWdsEntry->PhyOpMode));
		}
	}

	/*WdsList */
	if (RTMPGetKeyParameter("WdsList", tmpbuf, MAX_PARAM_BUFFER_SIZE,
				buffer, TRUE)) {
		if (pAd->WdsTab.Mode != WDS_LAZY_MODE) {
			for (i = 0, macptr = rstrtok(tmpbuf, ";");
			     (macptr && i < MAX_WDS_ENTRY);
	...
	/* WdsVlan */
	if (RTMPGetKeyParameter("WDS_VLANID", tmpbuf, MAX_PARAM_BUFFER_SIZE,
				buffer, TRUE)) {
		for (i = 0, macptr = rstrtok(tmpbuf, ";");
		     (macptr && i < MAX_WDS_ENTRY);
		     macptr = rstrtok(NULL, ";"), i++) {
...
VOID wds_find_cipher_algorithm(RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
			       TX_BLK *pTxBlk)
{
...
VOID WDS_Init(RTMP_ADAPTER *pAd, RTMP_OS_NETDEV_OP_HOOK *pNetDevOps)
{
	...
	if (pAd->WdsTab.flg_wds_init != FALSE) {
		for (index = 0; index < MAX_WDS_ENTRY; index++) {
			...
			wdev = &wds_entry->wdev;
			wdev->PhyMode =
				WdsPhyOpModeToSuppPhyMode(pAd, wds_entry);
	...
	for (index = 0; index < MAX_WDS_ENTRY; index++) {
		...
		pWdsNetDev =
			RtmpOSNetDevCreate(MC_RowID, &IoctlIF, INT_WDS, index,
		...
		if (pWdsNetDev == NULL) {
			...
			MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				 ("Allocate network device fail (WDS)...\n"));
			break;
		}

		MTWF_LOG(
			DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			("The new WDS interface MAC = %02X:%02X:%02X:%02X:%02X:%02X, MacTabMatchWCID = %d\n",
			 PRINT_MAC(
				 pAd->MacTab.Content[wds_entry->MacTabMatchWCID]
					 .Addr),
			 wds_entry->MacTabMatchWCID));
		NdisZeroMemory(&wds_entry->WdsCounter, sizeof(WDS_COUNTER));
		Ret = wdev_init(pAd, wdev, WDEV_TYPE_WDS, pWdsNetDev, index,
				wds_entry, (VOID *)pAd);
		...
		NdisMoveMemory(&wdev->if_addr[0], &pNetDevOps->devAddr[0],
			       MAC_ADDR_LEN);
		os_move_mem(wdev->bss_info_argument.Bssid, wdev->if_addr,
			    MAC_ADDR_LEN);
	...
	MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("Total allocated %d WDS interfaces!\n", index));
}
...
BOOLEAN WDS_StatsGet(RTMP_ADAPTER *pAd, RT_CMD_STATS *pStats)
{
	...
	pStats->rx_packets =
		wds_entry->WdsCounter.ReceivedFragmentCount.QuadPart;
	pStats->tx_packets =
		wds_entry->WdsCounter.TransmittedFragmentCount.QuadPart;
	...
	pStats->rx_fifo_errors =
		wds_entry->WdsCounter.RxNoBuffer; /* recv'r fifo overrun */
	return TRUE;
}

UCHAR WdsPhyOpModeToSuppPhyMode(PRTMP_ADAPTER pAd,
				PRT_802_11_WDS_ENTRY pWdsEntry)
{
	UCHAR ApPhyMode = pAd->ApCfg.MBSSID[MAIN_MBSSID].wdev.PhyMode;
	...
	MTWF_LOG(
		DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_INFO,
		("%s(): PhyOpMode=%d, SupportPhyMode = 0x%x, ApPhyMode = 0x%x\n",
		 __func__, pWdsEntry->PhyOpMode, SupportPhyMode, ApPhyMode));
	...
	if (SupportPhyMode == 0) {
		...
		MTWF_LOG(
			DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			("!!! WARNING !!! No matched PhyMode can be found, align to default 0x%x\n",
			 ApPhyMode));
	}
...
UCHAR WdsPhyOpModeToSuppRateMode(PRTMP_ADAPTER pAd,
				 PRT_802_11_WDS_ENTRY pWdsEntry)
{
	UCHAR ApPhyMode = pAd->ApCfg.MBSSID[MAIN_MBSSID].wdev.PhyMode;
	...
	case MODE_VHT:
		SupportRateMode = (SUPPORT_VHT_MODE | SUPPORT_HT_MODE |
				   SUPPORT_OFDM_MODE);
		break;
#endif /* DOT11_VHT_AC */
	...
	MTWF_LOG(
		DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_INFO,
		("%s(): PhyOpMode=%d, SupportRateMode = 0x%x, ApPhyMode = 0x%x\n",
		 __func__, pWdsEntry->PhyOpMode, SupportRateMode, ApPhyMode));
 ...
 *  \post
 */
BOOLEAN WdsMsgTypeSubst(IN PRTMP_ADAPTER pAd, IN PFRAME_802_11 pFrame,
			OUT PINT Machine, OUT PINT MsgType)
{
	/* wds data packet */
...
VOID ap_wds_rcv_uc_data_action(IN struct _RTMP_ADAPTER *pAd,
	...
	if ((pEntry == NULL) && (pAd->WdsTab.Mode >= WDS_LAZY_MODE)) {
		...
		if (WdsIdx >= 0 && WdsIdx < MAX_WDS_ENTRY) {
			...
			if (wds_entry->PhyOpMode == 0xff) {
				UINT32 encrypt_mode =
					wds_entry->wdev.SecConfig.PairwiseCipher;
				...
				wds_entry->wdev.PhyMode =
					WdsPhyOpModeToSuppPhyMode(pAd,
								  wds_entry);
			}
			pEntry = MacTableInsertWDSEntry(pAd, pFrame->Hdr.Addr2,
							(UCHAR)WdsIdx);
			...
			pEntry->SupportRateMode =
				WdsPhyOpModeToSuppRateMode(pAd, wds_entry);
			RAInit(pAd, pEntry);
		}
	}
}

VOID ap_wds_bss_linkdown(IN struct _RTMP_ADAPTER *pAd,
			 IN struct _MLME_QUEUE_ELEM *Elem)
{
	struct wifi_dev *wdev;
	USHORT wcid;
...
VOID WdsStateMachineInit(IN RTMP_ADAPTER *pAd, IN STATE_MACHINE *Sm,
			 OUT STATE_MACHINE_FUNC Trans[])
{
	StateMachineInit(Sm, (STATE_MACHINE_FUNC *)Trans, WDS_MAX_STATE,
			 WDS_MAX_MSG, (STATE_MACHINE_FUNC)Drop, WDS_IDLE,
			 WDS_MACHINE_BASE);
