...
struct __EC_GROUP_INFO *get_ecc_group_info(IN INT32 group);
...
VOID ecc_point_3d_to_2d(IN struct __EC_GROUP_INFO_BI *ec_group_bi,
...
/* double and add */
VOID ecc_point_mul_dblandadd(IN BIG_INTEGER_EC_POINT *point, IN SAE_BN *scalar,
			     IN struct __EC_GROUP_INFO_BI *ec_group_bi,
			     OUT BIG_INTEGER_EC_POINT **ec_point_res);
...
UCHAR ecc_point_is_on_curve(IN struct __EC_GROUP_INFO_BI *ec_group_bi,
...
/* y^2 = x^3 + ax + b  */
UCHAR ecc_point_find_by_x(IN struct __EC_GROUP_INFO_BI *ec_group_bi,
			  IN SAE_BN *x, IN SAE_BN **res_y, IN UCHAR need_res_y);
