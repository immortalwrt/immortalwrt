...
static char *q_idx_lmac_str[] = { "AC0",  "AC1",  "AC2",  "AC3",  "AC4",
				  "AC5",  "AC6",  "BCN",  "BMC",  "AC10",
				  "AC11", "AC12", "AC13", "AC14", "Invalid" };
...
VOID dump_tmac_info(RTMP_ADAPTER *pAd, UCHAR *tmac_info)
{
	...
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("\t\tQueID=%d(%s %s)\n", txd_0->QIdx,
		  ...
		  pIdx == P_IDX_LMAC ? q_idx_lmac_str[q_idx] :
					     q_idx_mcu_str[q_idx]));
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("\t\tTxByteCnt=%d\n", txd_0->TxByteCount));
	...
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("\t\tFormatType=%d(%s format)\n", txd_1->TxDFmt,
		  txd_1->TxDFmt == TMI_FT_LONG ? "Long - 8 DWORD" :
						       "Short - 3 DWORD"));
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("\t\tHdrPad=%d\n", txd_1->HdrPad));
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
...
VOID dump_rmac_info_normal(RTMP_ADAPTER *pAd, UCHAR *rmac_info)
{
	...
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("\t\tHTC/UC2ME/MC/BC=%d/%d/%d/%d\n", rxd_base->RxD1.HTC,
		  rxd_base->RxD1.UcastToMe, rxd_base->RxD1.Mcast,
		  rxd_base->RxD1.Bcast));
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
	...
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("\t\tLLC-SNAP mismatch(LLC-MIS, for HdrTrans)=%d\n",
		  rxd_base->RxD2.LlcMis));
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("\t\tUndefined VLAN tagged Type(UDF_VLT)=%d\n",
...
VOID DumpTxSFormat(RTMP_ADAPTER *pAd, UINT8 Format, CHAR *Data)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("\t\t(TXSFM=%d, TXS2M=%d, TXS2H=%d)\n", txs_d0->TxSFmt,
		  txs_d0->TxS2M, txs_d0->TxS2H));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("\t\tFR=%d, TxRate=0x%x\n", txs_d0->TxS_FR, txs_d0->TxRate));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("\t\tME=%d, RE=%d, LE=%d, BE=%d, TxOPLimitErr=%d\n",
		  txs_d0->ME, txs_d0->RE, txs_d0->LE, txs_d0->BE,
		  txs_d0->TxOp));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("\t\tPS=%d, BA Fail=%d, tid=%d, Ant_Id=%d\n", txs_d0->PSBit,
		  txs_d0->BAFail, txs_d0->Tid, txs_d0->AntId));
	...
	MTWF_LOG(
		...
		("\t\tSN=0x%x, TxBW=0x%x, AMPDU=%d, Final MPDU=%d PID=0x%x, MPDU TxCnt=%d, MCS Idx=%d\n",
		 txs_d4->TxS_SN_TSSI, txs_d4->TxS_TxBW, txs_d4->TxS_AMPDU,
		 txs_d4->TxS_FianlMPDU, txs_d4->TxS_Pid, txs_d4->TxS_MpduTxCnt,
		 txs_d4->TxS_LastTxMcsIdx));
}
...
VOID NICUpdateRawCounters(RTMP_ADAPTER *pAd)
{
	...
	pAd->WlanCounters[0].AmpduFailCount.u.LowPart +=
...
#ifdef DBG_DIAGNOSE
	{
		...
		if (pDiag->inited == 0) {
			...
			if ((pDiag->wcid == 0) && (pAd->MacTab.Size > 0)) {
				...
				for (idx = 1; VALID_UCAST_ENTRY_WCID(pAd, idx);
				     idx++) {
					...
					if (IS_ENTRY_CLIENT(pEntry) &&
						...
						MTWF_LOG(
							...
							DBG_SUBCAT_ALL,
							DBG_LVL_TRACE,
							("%s(): DBG_DIAGNOSE Start to monitor the SwQ depth of WCID[%d], ArrayCurIdx=%d\n",
							 __func__, pDiag->wcid,
							 pDiag->ArrayCurIdx));
						break;
					}
			...
			diag_info->TxNonAggCnt =
				bss_tx_cnt -
				AmpduTxCount; /* only useful when only one BSS case */
			...
			ArrayCurIdx = pDiag->ArrayCurIdx;
			os_zero_mem(&pDiag->diag_info[ArrayCurIdx],
...
VOID MtWriteTMacInfo(IN RTMP_ADAPTER *pAd, IN UCHAR *buf, IN TMAC_INFO *TxInfo)
{
	...
	UCHAR stbc = 0, bw = BW_20, mcs = 0, nss = 1, sgi = 0, phy_mode = 0,
	      preamble = 1, ldpc = 0, expBf = 0, impBf = 0;
	UCHAR vht_nss = 1;
	...
	if (!TxInfo->NeedTrans) {
		...
		TMI_HDR_INFO_VAL(txd_1->HdrFmt, 0, 0, 0, 0, 0,
				 TxInfo->WifiHdrLen, 0, txd_1->HdrInfo);

		/* TODO: depends on QoS to decide if need to padding */
		if (TxInfo->HdrPad)
			txd_1->HdrPad = (TMI_HDR_PAD_MODE_TAIL
					 << TMI_HDR_PAD_BIT_MODE) |
					TxInfo->HdrPad;
	} else {
		txd_1->HdrFmt = TMI_HDR_FT_NON_80211;
		TMI_HDR_INFO_VAL(txd_1->HdrFmt, TxInfo->MoreData, TxInfo->Eosp,
				 1, TxInfo->VlanFrame, TxInfo->EtherFrame,
				 TxInfo->WifiHdrLen, 0, txd_1->HdrInfo);

		if (TxInfo->HdrPad)
			txd_1->HdrPad = (TMI_HDR_PAD_MODE_HEAD
					 << TMI_HDR_PAD_BIT_MODE) |
					TxInfo->HdrPad;
	}
	...
	if (TxInfo->LongFmt == FALSE) {
		...
		if (txd_2->FixRate) {
			...
			if (txd_6->FixedRateMode == TMI_FIX_RATE_BY_TXD) {
				...
				txd_6->RateToBeFixed = tx_rate_to_tmi_rate(
	...
	if (txd_0->IpChkSum || txd_0->UTChkSum) {
		...
		MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			 ("%s():TxInfo->WifiHdrLen is %x tmp length is %d\n",
...
#ifdef COMPOS_WIN
VOID XmitWriteTxD(IN PMP_ADAPTER pAd, IN UCHAR *buf,
		  IN HARD_TRANSMIT_INFO TxInfo)
{
	...
	if (TxInfo.TMacInfo.LongFmt == TRUE) {
		if (TxInfo.bMgmtPacket || TxInfo.bMcast || TxInfo.bDHCPFrame ||
		    TxInfo.bEAPOLFrame ||
		    ((TxInfo.MimoPs == MMPS_STATIC) &&
		     (pAd->RegistryCfg.APTGN == TRUE))) {
			...
			MTWF_LOG(
				...
				("Force to Fixed Rate, reason:(Mgmt:%d, Mcast:%d, DHCP:%d, EAPOL:%d, MMPS_S:%d\n",
				 TxInfo.bMgmtPacket, TxInfo.bMcast,
				 TxInfo.bDHCPFrame, TxInfo.bEAPOLFrame,
				 TxInfo.MimoPs));
		}

		if (TxInfo.TMacInfo.QueIdx == 0x7 ||
		    TxInfo.TMacInfo.QueIdx == 0x8)
			TxInfo.TMacInfo.RemainTxCnt = 0x1f;
		else
			TxInfo.TMacInfo.RemainTxCnt = 0xf;

		if (TxInfo.TMacInfo.Pid == PID_NOT_RESPONSE)
			TxInfo.TMacInfo.TxS2Host =
				0; /* 0 : not report status to host */
		else
			TxInfo.TMacInfo.TxS2Host =
				1; /* 1 : report status to host */
...
/*
	...
	========================================================================
*/
VOID mt_write_tmac_info(IN RTMP_ADAPTER *pAd, IN UCHAR *tmac_info,
			IN MAC_TX_INFO *info, IN HTTRANSMIT_SETTING *pTransmit)
{
	...
	txd_1->TxDFmt = TMI_FT_LONG;
	txd_1->HdrFmt = TMI_HDR_FT_NOR_80211;
	TMI_HDR_INFO_VAL(txd_1->HdrFmt, 0, 0, 0, 0, 0, info->hdr_len, 0,
			 txd_1->HdrInfo);

	if (info->hdr_pad) /* TODO: depends on QoS to decide if need to padding */
		txd_1->HdrPad =
			(TMI_HDR_PAD_MODE_TAIL << TMI_HDR_PAD_BIT_MODE) |
			info->hdr_pad;
	...
	if (txd_1->TxDFmt == TMI_FT_LONG) {
		...
		if (pAd->BtCoexMode == MT76xx_COEX_MODE_TDD) {
			...
			*/
			if ((txd_2->MaxTxTime != 0) && (txd_2->MaxTxTime < 50))
		...
		txd_2->SubType =
			(*((UINT16 *)(tmac_info + sizeof(TMAC_TXD_L))) &
			 (0xf << 4)) >>
			4;
		txd_2->FrmType =
			(*((UINT16 *)(tmac_info + sizeof(TMAC_TXD_L))) &
		...
		if (info->Type == FC_TYPE_MGMT &&
			...
			if (TxSCtl->TxS2HostStatusPerPkt & (1 << info->PID)) {
				txd_5->PktId =
					AddTxSStatus(pAd, TXS_TYPE0, info->PID,
						     0, 0, txd_6->RateToBeFixed,
						     info->TxSPriv);
				txd_5->TxS2Host = 1;
			} else
				txd_5->TxS2Host = 0;
			...
			if (txd_1->WlanIdx < 64)
				TxSStatusPerWlanIdx =
					TxSCtl->TxSStatusPerWlanIdx[0];
			else
				TxSStatusPerWlanIdx =
					TxSCtl->TxSStatusPerWlanIdx[1];

			if (TxSStatusPerWlanIdx & (1 << txd_1->WlanIdx)) {
				if (TxSCtl->TxSFormatPerPktType[txd_2->FrmType] &
				    (1 << txd_2->SubType))
					txd_5->TxSFmt = TXS_FORMAT1;
				else
					txd_5->TxSFmt = TXS_FORMAT0;

				if (TxSCtl->TxS2McUStatusPerPktType
					    [txd_2->FrmType] &
				    (1 << txd_2->SubType))
					txd_5->TxS2Mcu = 1;
				else
				...
				if (TxSCtl->TxS2HostStatusPerPktType
					    [txd_2->FrmType] &
				    (1 << txd_2->SubType)) {
					txd_5->PktId =
						AddTxSStatus(pAd, TXS_TYPE1, 0,
							     txd_2->FrmType,
							     txd_2->SubType, 0,
							     info->TxSPriv);
					txd_5->TxS2Host = 1;
	...
	if (txd_0->IpChkSum || txd_0->UTChkSum) {
		...
		tmp = (txd_size + info->hdr_len + 6) >>
		      1; /* 6 is shift to ethernet type */
		txd_0->EthTypeOffset = tmp;
		MTWF_LOG(
...
VOID mt_write_tmac_info(RTMP_ADAPTER *pAd, UCHAR *buf, TX_BLK *pTxBlk)
{
	...
	if (pMacEntry && IS_ENTRY_TDLS(pMacEntry) &&
	    pAd->StaCfg[0]
		    .wpa_supplicant_info.CFG_Tdls_info.bDoingPeriodChannelSwitch)
	...
	if (txd_long == TRUE) {
		...
		TxInfo.TxRadioSet.CurrentPerPktBW =
			(TxInfo.TxRadioSet.PhyMode <= MODE_OFDM) ?
				(BW_20) :
				      (pTransmit->field.BW);
		...
		if (pAd->BtCoexMode == MT76xx_COEX_MODE_TDD) {
			/*
				...
				TDD mode: remaining life time > 400ms. @20140827
			*/
			if ((TxInfo.MaxTxTime != 0) && (TxInfo.MaxTxTime < 50))
		...
		TxInfo.FrmType =
			(*((UINT16 *)(pTxBlk->wifi_hdr)) & (0x3 << 2)) >> 2;
		TxInfo.SubType =
			(*((UINT16 *)(pTxBlk->wifi_hdr)) & (0xf << 4)) >> 4;
		...
		if (pTxBlk->Pid) {
			...
			if (TxSCtl->TxS2HostStatusPerPkt & (1 << pTxBlk->Pid)) {
				TxInfo.Pid = AddTxSStatus(pAd, TXS_TYPE0,
							  pTxBlk->Pid, 0, 0, 0,
							  pTxBlk->TxSPriv);
				TxInfo.TxS2Host = 1;
			} else
			...
			if (TxSStatusPerWlanIdx & (1 << TxInfo.Wcid)) {
				if (TxSCtl->TxSFormatPerPktType[TxInfo.FrmType] &
				    (1 << TxInfo.SubType))
					TxInfo.TxSFmt = TXS_FORMAT1;
				else
					TxInfo.TxSFmt = TXS_FORMAT0;
	...
	TxInfo.PktLen =
		pTxBlk->SrcBufLen + pTxBlk->MpduHeaderLen + pTxBlk->HdrPadLen;
	/*fill TxInfo*/
...
VOID mt_asic_init_txrx_ring(RTMP_ADAPTER *pAd)
{
	...
	*/
...

	{
		...
		phy_addr =
			RTMP_GetPhysicalAddressLow(bmc_ring->Cell[0].AllocPa);
		mt_asic_cfg_hif_tx_ring(pAd, bmc_ring, offset, phy_addr,
					TX_RING_SIZE);
		MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		...
		phy_addr =
			RTMP_GetPhysicalAddressLow(mgmt_ring->Cell[0].AllocPa);
		...
		HIF_IO_WRITE32(pAd, mgmt_ring->hw_cidx_addr,
			       mgmt_ring->TxCpuIdx);
		...
		phy_addr =
			RTMP_GetPhysicalAddressLow(bcn_ring->Cell[0].AllocPa);
		bcn_ring->TxSwFreeIdx = 0;
		bcn_ring->TxCpuIdx = 0;
		bcn_ring->hw_desc_base = (MT_TX_RING_BASE +
					  MT_RINGREG_DIFF * MT_TX_RING_BCN_IDX);
...
#ifdef CONFIG_ANDES_SUPPORT
	{
		...
		phy_addr =
			RTMP_GetPhysicalAddressLow(ctrl_ring->Cell[0].AllocPa);
		...
		HIF_IO_WRITE32(pAd, ctrl_ring->hw_cidx_addr,
			       ctrl_ring->TxCpuIdx);
	...
	for (i = 0; i < GET_NUM_OF_RX_RING(cap); i++) {
		...
		MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("-->RX_RING%d[0x%x]: Base=0x%x, Cnt=%d\n", i,
			  rx_ring->hw_desc_base, phy_addr, RX_RING_SIZE));
	}
}
#endif /* RTMP_MAC_PCI */
...
VOID dump_wtbl_1_info(RTMP_ADAPTER *pAd, struct wtbl_1_struc *tb)
{
	...
	hex_dump("WTBL Segment 1 Raw Data", (UCHAR *)tb,
	...
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("\trc_a1:%d/rc_a2:%d/kid:%d/rkv:%d/rv:%d/rc_id:%d/sw:%d\n",
		  wtbl_1_d0->field.rc_a1, wtbl_1_d0->field.rc_a2,
		  wtbl_1_d0->field.kid, wtbl_1_d0->field.rkv,
		  wtbl_1_d0->field.rv, wtbl_1_d0->field.rc_id,
		  wtbl_1_d0->field.sw));
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
	...
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("\tht/vht/ldpc/dyn_bw:%d/%d/%d/%d\n", wtbl_1_d2->field.ht,
		  wtbl_1_d2->field.vht, wtbl_1_d2->field.ldpc,
		  wtbl_1_d2->field.dyn_bw));
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("\tTxBF(tibf/tebf):%d / %d\n", wtbl_1_d2->field.tibf,
		  wtbl_1_d2->field.tebf));
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
...
VOID dump_wtbl_2_info(RTMP_ADAPTER *pAd, struct wtbl_2_struc *tb)
{
	...
	hex_dump("WTBL Segment 2 Raw Data", (UCHAR *)tb,
		 sizeof(struct wtbl_2_struc));
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
	...
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("\tTxBwCnt(Current-Other):%d(%d) - %d(%d)\n",
		  dw_7->field.current_bw_tx_cnt,
		  dw_7->field.current_bw_fail_cnt, dw_8->field.other_bw_tx_cnt,
		  dw_8->field.other_bw_fail_cnt));
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("\tFreqCap:%d(%sMHz)\n", dw_9->field.fcap,
		  bw_str[dw_9->field.fcap]));
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("\tRateIdx/CBRN/CCBW_SEL/SPE_EN: %d/%d/%d/%d\n",
		  dw_9->field.rate_idx, dw_9->field.cbrn, dw_9->field.ccbw_sel,
		  dw_9->field.spe_en));
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("\tMpduCnt(Fail/OK):%d-%d\n", dw_9->field.mpdu_fail_cnt,
		  dw_9->field.mpdu_ok_cnt));
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("\tTxRate Info: G2/G4/G8/G16=%d/%d/%d/%d\n", dw_9->field.g2,
		  dw_9->field.g4, dw_9->field.g8, dw_9->field.g16));
	MTWF_LOG(
		DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		("\tTxRate Info: RateIdx/STBC/Nsts/PhyMode/TxRate (RawData)\n"));
	rate_info[0] = (dw_10->word & 0xfff);
	rate_info[1] = (dw_10->word & 0xfff000) >> 12;
	rate_info[2] =
		((dw_10->word & 0xff000000) >> 24) | ((dw_11->word & 0xf) << 8);
	rate_info[3] = ((dw_11->word & 0xfff0) >> 4);
	rate_info[4] = ((dw_11->word & 0xfff0000) >> 16);
	rate_info[5] = ((dw_11->word & 0xf0000000) >> 28) |
		       ((dw_12->word & 0xff) << 4);
	...
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("\tResp_RCPI0/Resp_RCPI1/Resp_RCPI2=0x%x/0x%x/0x%x\n",
		  dw_13->field.resp_rcpi_0, dw_13->field.resp_rcpi_1,
		  dw_13->field.resp_rcpi_2));
	MTWF_LOG(
		DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		("\t1CC(Noise)/2CC(Noise)/3CC(Noise)/CE_RMSD/CC_Sel/Ant_Sel=0x%x/0x%x/0x%x/0x%x/%d/%d\n",
		 dw_14->field.sts_1_ch_cap_noise,
		 dw_14->field.sts_2_ch_cap_noise,
		 dw_14->field.sts_3_ch_cap_noise, dw_14->field.ce_rmsd,
		 dw_14->field.cc_noise_sel, dw_14->field.ant_sel));
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("\tBA Info: TID/BA_En/BAWinSizeIdx(Range)\n"));

	for (idx = 0; idx <= 7; idx++) {
		ba_en = (dw_15->field.ba_en & (1 << idx)) ? 1 : 0;
		ba_size_idx =
			(dw_15->field.ba_win_size_tid & (0x7 << (idx * 3))) >>
			((idx * 3));
...
{
	hex_dump("WTBL Segment 3 Raw Data", (UCHAR *)tb,
		 sizeof(union wtbl_3_struc));
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("WTBL Segment 3 Fields:\n"));
}
...
VOID dump_wtbl_info_ByTlv(RTMP_ADAPTER *pAd, UINT wtbl_idx)
{
	...
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("(TLV)Dump WTBL info of WLAN_IDX:%d\n", wtbl_idx));
	CmdWtblDump.u2Tag = cpu_to_le16(WTBL_DUMP);
	CmdWtblDump.u2Length = cpu_to_le16(sizeof(CMD_WTBL_DUMP_T));
	CmdExtWtblUpdate(pAd, wtbl_idx, QUERY_WTBL, &CmdWtblDump,
			 sizeof(CMD_WTBL_DUMP_T));
}
#endif
...
VOID dump_wtbl_entry(RTMP_ADAPTER *pAd, struct wtbl_entry *ent)
{
	...
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("Dump WTBL SW Entry[%d] Cache info\n", ent->wtbl_idx));

	for (idx = 0; idx < 4; idx++) {
		MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			 ("\tWTBL %d info:\n", idx + 1));
		MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			 ("\t\tAddr=0x%x, FID=%d, EID=%d\n",
			  ent->wtbl_addr[idx], ent->wtbl_fid[idx],
			  ent->wtbl_eid[idx]));
...
INT mt_wtbl_init_ByFw(RTMP_ADAPTER *pAd)
{
	...
	pAd->mac_ctrl.page_size =
		MT_PSE_PAGE_SIZE; /* size in bytes of each PSE page */
	pAd->mac_ctrl.wtbl_base_addr[0] =
		WTBL_WTBL1DW0; /* Start address of WTBL2 in host view */
	...
	pAd->mac_ctrl.wtbl_base_addr[1] =
		MT_PCI_REMAP_ADDR_1; /* Start address of WTBL2 in host view */
	...
	page_cnt = (pAd->mac_ctrl.wtbl_entry_cnt[1] + ecnt_per_page - 1) /
		   ecnt_per_page;
	offset = page_cnt * pAd->mac_ctrl.page_size;
	pAd->mac_ctrl.wtbl_base_fid[2] =
		pAd->mac_ctrl.wtbl_base_fid[1] + page_cnt;
	pAd->mac_ctrl.wtbl_base_addr[2] =
		pAd->mac_ctrl.wtbl_base_addr[1] + offset;
	...
	page_cnt = (pAd->mac_ctrl.wtbl_entry_cnt[2] + ecnt_per_page - 1) /
		   ecnt_per_page;
	offset = page_cnt * pAd->mac_ctrl.page_size;
	pAd->mac_ctrl.wtbl_base_fid[3] =
		pAd->mac_ctrl.wtbl_base_fid[2] + page_cnt;
	pAd->mac_ctrl.wtbl_base_addr[3] =
		pAd->mac_ctrl.wtbl_base_addr[2] + offset;
...
INT mt_wtbl_init_ByDriver(RTMP_ADAPTER *pAd)
{
	...
	union WTBL_1_DW4 *dw4 =
		(union WTBL_1_DW4 *)&tb_entry.wtbl_1.wtbl_1_d4.word;
	struct _RTMP_CHIP_CAP *cap = hc_get_chip_cap(pAd->hdev_ctrl);
	UINT32 wtbl_num = cap->WtblHwNum;
	...
	MTWF_LOG(
		DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		("%s(%d): wtbl_base_addr[0]=0x%x, wtbl_entry_size[0]=%d, wtbl_entry_cnt[0]=%d, wtbl_base_fid[0]=%d\n",
		 __func__, __LINE__, pAd->mac_ctrl.wtbl_base_addr[0],
		 pAd->mac_ctrl.wtbl_entry_size[0],
		 pAd->mac_ctrl.wtbl_entry_cnt[0],
		 pAd->mac_ctrl.wtbl_base_fid[0]));
	MTWF_LOG(
		DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		("%s(%d): wtbl_base_addr[1]=0x%x, wtbl_entry_size[1]=%d, wtbl_entry_cnt[1]=%d, wtbl_base_fid[1]=%d\n",
		 __func__, __LINE__, pAd->mac_ctrl.wtbl_base_addr[1],
		 pAd->mac_ctrl.wtbl_entry_size[1],
		 pAd->mac_ctrl.wtbl_entry_cnt[1],
		 pAd->mac_ctrl.wtbl_base_fid[1]));
	MTWF_LOG(
		DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		("%s(%d): wtbl_base_addr[2]=0x%x, wtbl_entry_size[2]=%d, wtbl_entry_cnt[2]=%d, wtbl_base_fid[2]=%d\n",
		 __func__, __LINE__, pAd->mac_ctrl.wtbl_base_addr[2],
		 pAd->mac_ctrl.wtbl_entry_size[2],
		 pAd->mac_ctrl.wtbl_entry_cnt[2],
		 pAd->mac_ctrl.wtbl_base_fid[2]));
	MTWF_LOG(
		DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		("%s(%d): wtbl_base_addr[3]=0x%x, wtbl_entry_size[3]=%d, wtbl_entry_cnt[3]=%d, wtbl_base_fid[3]=%d\n",
		 __func__, __LINE__, pAd->mac_ctrl.wtbl_base_addr[3],
		 pAd->mac_ctrl.wtbl_entry_size[3],
		 pAd->mac_ctrl.wtbl_entry_cnt[3],
		 pAd->mac_ctrl.wtbl_base_fid[3]));
...
INT mt_mac_init(RTMP_ADAPTER *pAd)
{
	...
	if (pAd->archOps.archSetMacMaxLen)
		pAd->archOps.archSetMacMaxLen(
			pAd); /* for ATE build failure issue */
	else
		MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			 ("%s() archSetMacMaxLen is not supported\n",
			  __func__));
...
VOID mt_chip_info_show(RTMP_ADAPTER *pAd)
{
	...
	defined(MT7615_FPGA) || defined(MT7637_FPGA) ||                        \
	defined(MT7622_FPGA) || defined(P18_FPGA) || defined(MT7663_FPGA)
	...
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("##########################################\n"));
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("%s(%d): MT7603 Series FPGA Version:\n", __func__, __LINE__));
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("\tFGPA1: Code[0x700]:0x%x, [0x704]:0x%x, [0x708]:0x%x\n",
		  ver, rev, date_code));
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("\tFPGA2: Version[0x201f8]:0x%x\n", mac_val));
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("##########################################\n"));
...
INT mt_mlme_hw_tx(RTMP_ADAPTER *pAd, UCHAR *tmac_info, MAC_TX_INFO *info,
