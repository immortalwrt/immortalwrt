	...
	IN	BOOLEAN bConnected)
{
	...
	if (pTdlsPeer) {
		...
		if (pTdlsPeer) {
			...
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("!!!%s : no memory!!!\n", __func__));
			return FALSE;
		}
	}
	...
	IN	PUCHAR			pAddr)
{
	...
	if (idIdx == MAX_NUM_OF_TDLS_ENTRY) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("!!! TDLS_ValidIdLookup -  not found !!!\n"));
...
VOID TDLS_AutoSetupByRcvFrame(RTMP_ADAPTER *pAd, HEADER_802_11 *pHeader)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("TDLS ===> TDLS_AutoSetupByRcvFrame\n"));
	RTMP_SEM_LOCK(&pAd->StaCfg[0].TdlsInfo.TdlsBlackListSemLock);
	...
	if ((pTdlsBlackEntry == NULL) && (pTdlsDiscoveryEntry == NULL)) {
		...
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("AutoSetupByRcvFrame trigger discovery request !!!\n"));
		RTMP_SEM_LOCK(&pAd->StaCfg[0].TdlsInfo.TdlsDiscovPeerListSemLock);
		rv = TDLS_InsertDiscoveryPeerEntryByMAC(pTdlsDiscoveryEnList,
												pHeader->Addr3,
												FALSE);
		...
		if (LinkId < MAX_NUM_OF_TDLS_ENTRY) {
			...
			if (pTdlsPeer) {
				...
				MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("TDLS peer entry already on table !!!\n"));
			}
		} else if (rv) {
			MlmeEnqueue(pAd,
						TDLS_STATE_MACHINE,
	...
	IN PUCHAR pMacAddr)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("TDLS ===> TDLS_TunneledProbeRequest\n"));
	...
	IN PUCHAR pMacAddr)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("TDLS ===> TDLS_TunneledProbeResponse\n"));
	MAKE_802_3_HEADER(Header802_3, pMacAddr, pAd->CurrentAddress, TDLS_ETHERTYPE);
	/* Allocate buffer for transmitting message */
	NStatus = MlmeAllocateMemory(pAd, &pOutBuffer);
...
#endif /* WFD_SUPPORT */
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IF(ra%d) Set_TdlsCapableProc::(bTdlsCapable=%d)\n",
...
INT	Set_TdlsSetup_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (INFRA_ON(pAd)) {
		if (pAd->StaActive.ExtCapInfo.TDLSProhibited == TRUE) {
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("TDLS - Set_TdlsSetup_Proc() AP Prohibited TDLS !!!\n"));
			return FALSE;
		}
...
INT	Set_TdlsTearDown_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (INFRA_ON(pAd)) {
		...
		if (idx == -1 || idx == MAX_NUM_OF_TDLS_ENTRY) {
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("TDLS - Set_TdlsTearDown_Proc() can not find or full the LinkId!\n"));
			return FALSE;
		}
...
INT Set_TdlsAcceptWeakSecurityProc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IF(ra%d) Set_TdlsAcceptWeakSecurityProc::(bAcceptWeakSecurity=%d)\n",
...
INT	Set_TdlsTPKLifeTime_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("IF(ra%d) Set_TdlsTPKLifeTimeProc::(TdlsKeyLifeTime=%d)\n",
...
INT	Set_TdlsAutoLinkProc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("IF(ra%d) Set_TdlsAutoLinkProc::(TdlsAutoLink=%d)\n",
...
Set_TdlsRssiMeasurementPeriodProc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("IF(ra%d) Set_TdlsRssiMeasurementPeriodProc::(RssiMeasurementPeriod = %d secs)\n",
...
Set_TdlsAutoDiscoveryPeriodProc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("IF(ra%d) Set_TdlsAutoDiscoveryPeriodProc::(AutoDiscoveryPeriod = %d secs)\n",
...
Set_TdlsAutoSetupRssiThresholdProc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("IF(ra%d) Set_TdlsAutoSetupRssiThresholdProc::(AutoSetupRssiThreshold = %d dbm)\n",
...
Set_TdlsDisabledPeriodByTeardownProc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("IF(ra%d) Set_TdlsDisabledPeriodByTeardownProc::(DisabledPeriodByTeardown = %d secs)\n",
...
Set_TdlsAutoTeardownRssiThresholdProc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("IF(ra%d) Set_TdlsAutoTeardownRssiThresholdProc::(AutoTeardownRssiThreshold = %d dbm)\n",
...
INT	Set_TdlsChannelSwitchBW_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("IF(ra%d) Set_TdlsChannelSwitchBW_Proc::(TdlsChannelSwitchBW=%d)\n",
...
INT	Set_TdlsChannelSwitchDisable_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (INFRA_ON(pAd)) {
		...
		if (idx < MAX_NUM_OF_TDLS_ENTRY) {
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("IF(ra%d) Set_TdlsChannelSwitchDisable_Proc\n", pObj->ioctl_if));
			return TRUE;
		}
	...
	IN HT_CAPABILITY_IE * pHtCapability)
{
...
#endif /* DOT11_N_SUPPORT */
	{
		...
		CLIENT_STATUS_SET_FLAG(pEntry, fCLIENT_STATUS_WMM_CAPABLE);
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("TDLS - Receive Peer Legacy STA\n"));
	}
	...
	OUT	INT		*MsgType)
{
	...
	default:
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("TDLS_MsgTypeSubst : unsupported TDLS Action Type(%d);\n", TDLSActionType));
		return FALSE;
	}
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (!IS_TDLS_SUPPORT(pAd)) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("CNTL - TDLS Capable disable !!!\n"));
		return;
	}

	if (!INFRA_ON(pAd)) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("CNTL - STA do not connect to AP !!!\n"));
		return;
	}
	...
	if (Idx < MAX_P2P_GROUP_SIZE) {
		...
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::find entry in p2p table\n", __func__));
	...
	if (Idx == -1) { /* not found and the entry is not full */
			...
			for (i = 0; i < MAX_NUM_OF_TDLS_ENTRY; i++) {
				if (!pAd->StaCfg[0].TdlsInfo.TDLSEntry[i].Valid) {
					...
					MlmeEnqueue(pAd,
								...
								&MlmeTdlsReq,
								0);
					MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("CNTL - TDLS setup case\n"));
					break;
				}

				MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("CNTL - TDLS  do not find vaild entry !!!!\n"));
			}
		} else
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("CNTL - TDLS not changed in Idx = -1 (Valid=%d)\n", pTDLS->Valid));
			...
			for (i = 0; i < MAX_NUM_OF_TDLS_ENTRY; i++) {
				if ((pAd->StaCfg[0].TdlsInfo.TDLSEntry[i].Valid) && (pAd->StaCfg[0].TdlsInfo.TDLSEntry[i].Status < TDLS_MODE_CONNECTED)) {
					...
					MlmeEnqueue(pAd,
								...
								&MlmeTdlsReq,
								0);
					MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("CNTL - TDLS restart case\n"));
					break;
				}
			}
		} else
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("CNTL - TDLS not changed in Idx = MAX_NUM_OF_TDLS_ENTRY (Valid=%d)\n", pTDLS->Valid));
		...
		if ((pAd->StaCfg[0].WfdCfg.bWfdEnable) &&
			...
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("CNTL - Peer TDLS PC is P2P\n"));
		} else
#endif /* WFD_SUPPORT */
			if ((!pTDLS->Valid) && (pAd->StaCfg[0].TdlsInfo.TDLSEntry[Idx].Status >= TDLS_MODE_CONNECTED)) {
				...
				MlmeEnqueue(pAd,
							...
							&MlmeTdlsReq,
							0);
				MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("CNTL - start tear down procedure\n"));
				...
				MlmeEnqueue(pAd,
							...
							&MlmeTdlsReq,
							0);
				MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("CNTL - TDLS retry setup procedure\n"));
			} else {
#ifdef WFD_SUPPORT
...
#endif /* WFD_SUPPORT */
				MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("CNTL - TDLS not changed in entry - %d - Valid=%d, Status=%d\n",
	...
	IN	PUCHAR			pAddr)
{
	...
	if (i == MAX_NUM_OF_TDLS_ENTRY) {
		if (empty == 0) {
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("TDLS_SearchLinkId -  not found and full\n"));
			return MAX_NUM_OF_TDLS_ENTRY;
		} else {
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("TDLS_SearchLinkId -  not found\n"));
			return -1;
		}
	}
	...
	OUT UCHAR	TIIe[])
{
	...
	*pToken = *Ptr;
	...
	if (RemainLen < 2) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s() - Invaild packet length - (capability)\n", __func__));
		return FALSE;
	}
	...
	while ((Length + 2 + pEid->Len) <= RemainLen) {
		...
		default:
			...
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("PeerTdlsSetupReqSanity - unrecognized EID = %d\n", pEid->Eid));
			break;
		}
	...
	if (LinkIdLen == 0) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s() - Invaild packet - (link identifier)\n", __func__));
		return FALSE;
	} else {
	...
	OUT UCHAR	TIIe[])
{
	...
	*pTILen = 0;
	...
	if (RemainLen < 2) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s() - Invaild packet length - (status code)\n", __func__));
		return FALSE;
	}
	...
	*pToken = *Ptr;
	...
	while ((Length + 2 + pEid->Len) <= RemainLen) {
		...
		default:
			/* Unknown IE, we have to pass it as variable IEs */
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("PeerTdlsSetupRspSanity - unrecognized EID = %d\n", pEid->Eid));
			break;
		}
	...
	while ((Length + 2 + pEid->Len) <= RemainLen) {
		...
		case IE_ADD_HT2:
			if (pEid->Len >= sizeof(ADD_HT_INFO_IE)) {
				...
				MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("PeerTdlsSetupConfSanity - wrong IE_ADD_HT.\n"));
		...
		default:
			/* Unknown IE, we have to pass it as variable IEs */
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("PeerTdlsSetupConfSanity - unrecognized EID = %d\n", pEid->Eid));
			break;
		}
	...
	OUT UCHAR	FTIe[])
{
	...
	*pFTLen = 0;
	...
	/* Message contains 802.11 header (24 bytes), LLC_SNAP (8 bytes), TDLS Action header(3 bytes) and Payload (variable) */
	if (RemainLen < (LENGTH_802_11 + LENGTH_802_1_H + LENGTH_TDLS_PAYLOAD_H)) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s() - Invaild packet length - (cation header)\n", __func__));
		return FALSE;
	}
	...
	if (RemainLen < 2) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s() - Invaild packet length - (reason code)\n", __func__));
		return FALSE;
	}
	...
	while ((Length + 2 + pEid->Len) <= RemainLen) {
		...
		default:
			/* Unknown IE, we have to pass it as variable IEs */
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("PeerTdlsTearDownSanity - unrecognized EID = %d\n", pEid->Eid));
			break;
		}
	...
	if (LinkIdLen == 0) {
		...
		if (!MAC_ADDR_EQUAL(pAd->CurrentAddress, LinkIdInitiatorAddr)) {
			if (!MAC_ADDR_EQUAL(pAd->CurrentAddress, LinkIdResponderAddr)) {
				MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s() - It's not my Address\n", __func__));
				return FALSE;
			} else {
	...
	OUT UCHAR TIIe[])
{
	...
	*pToken = *Ptr;
	...
	while ((Length + 2 + pEid->Len) <= RemainLen) {
		...
		default:
			/* Unknown IE, we have to pass it as variable IEs */
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("PeerTdlsDiscovRspSanity - unrecognized EID = %d\n", pEid->Eid));
			break;
		}
	...
	OUT USHORT *pChSwitchTimeOut)
{
	...
	*pNewExtChannelOffset = 0;
	...
	/* Message contains 802.11 header (24 bytes), LLC_SNAP (8 bytes), TDLS Action header(3 bytes) and Payload (variable) */
	if (RemainLen < (LENGTH_802_11 + LENGTH_802_1_H + LENGTH_TDLS_PAYLOAD_H)) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s() - Invaild packet length - (ation header)\n", __func__));
		return FALSE;
	}
	...
	if (RemainLen < 1) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s() - Invaild packet length - (target channel)\n", __func__));
		return FALSE;
	}
	...
	*pTargetChannel = *Ptr;
	...
	if (RemainLen < 1) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s() - Invaild packet length - (regulatory class)\n", __func__));
		return FALSE;
	}

	*pRegulatoryClass = *Ptr;
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s() - Regulatory class = %d\n", __func__, *pRegulatoryClass));
	/* Offset to other elements */
	Ptr += 1;
	RemainLen -= 1;
	pEid = (PEID_STRUCT) Ptr;
	...
	while ((Length + 2 + pEid->Len) <= RemainLen) {
		...
		case IE_SECONDARY_CH_OFFSET:
			if (pEid->Len == 1)
				*pNewExtChannelOffset = pEid->Octet[0];
				...
				rv = FALSE;
				MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s() - wrong IE_SECONDARY_CH_OFFSET.\n", __func__));
			}
		...
		case IE_TDLS_LINK_IDENTIFIER:
			if (pEid->Len != TDLS_ELM_LEN_LINK_IDENTIFIER) {
				rv = FALSE;
				MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s() - wrong IE_TDLS_LINK_IDENTIFIER.\n", __func__));
			}
		...
		case IE_TDLS_CHANNEL_SWITCH_TIMING:
			if (pEid->Len == 4) {
				...
				rv = FALSE;
				MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s() - wrong IE_TDLS_CHANNEL_SWITCH_TIMING.\n", __func__));
			}
		...
		default:
			/* Unknown IE, we have to pass it as variable IEs */
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s() - unrecognized EID = %d\n", __func__, pEid->Eid));
			break;
		}
	...
	*Peerip = 0;
				...
				if (bFirstP2pOUI == TRUE) {
					...
					if (*P2PSubelementLen == 0) {
						...
						*P2PSubelementLen = (pEid->Len + 3);
						...
						MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("SYNC -1-1 P2PIeConLen  = %ld\n", P2PIeConLen));
					} else if (*P2PSubelementLen > 0) {
						if (((*P2PSubelementLen) + (pEid->Len + 3)) <= MAX_VIE_LEN) {
							...
							*P2PSubelementLen += (pEid->Len + 3);
							...
							MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_INFO, (" -1-2 P2PIeConLen  = %ld\n", P2PIeConLen));
						} else {
							MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: ERROR!! 222 Sum of P2PSubelementLen= %lu, > MAX_VIE_LEN !!\n",
									 __func__,
									 ((*P2PSubelementLen) + (pEid->Len + 3))));
							return FALSE;
						}
	...
	if ((P2PIeConLen != 0) && (pP2PIeConLen != NULL)) {
		...
		*(pP2PIeConLen + 1) = (UCHAR)(P2PIeConLen / 256);
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("  - 3 P2PIeConLen  = %ld. /256 = %ld. *P2PSubelementLen = %ld\n", P2PIeConLen, (P2PIeConLen / 256), *P2PSubelementLen));
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("  -  %x %x\n", *pP2PIeConLen,  *(pP2PIeConLen + 1)));

		for (idx = 0; idx < (*P2PSubelementLen);) {
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-",
	...
	IN PUCHAR arg)
{
	...
	for (i = 0; i < MAX_NUM_OF_TDLS_ENTRY; i++) {
		if ((pAd->StaCfg[0].TdlsInfo.TDLSEntry[i].Valid) && (pAd->StaCfg[0].TdlsInfo.TDLSEntry[i].Status == TDLS_MODE_CONNECTED)) {
			...
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%-4d", (int)CLIENT_STATUS_TEST_FLAG(pEntry, fCLIENT_STATUS_WMM_CAPABLE)));
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%-7d", pEntry->RssiSample.AvgRssi[0]));
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%-7d", pEntry->RssiSample.AvgRssi[1]));
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%-7d", pEntry->RssiSample.AvgRssi[2]));
#ifdef DOT11_N_SUPPORT
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%-8d", (int)pEntry->MmpsMode));
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%-10s", get_phymode_str(pEntry->HTPhyMode.field.MODE)));
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%-6s", get_bw_str(pEntry->HTPhyMode.field.BW)));
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%-6d", pEntry->HTPhyMode.field.MCS));
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%-6d", pEntry->HTPhyMode.field.ShortGI));
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%-6d", pEntry->HTPhyMode.field.STBC));
#endif /* DOT11_N_SUPPORT */
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%-10d, %d, %d%%\n", pEntry->DebugFIFOCount, pEntry->DebugTxCount,
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	if (MTxCycle == PKT_CHANNEL_MAX_WAIT_CYCLE)
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Polling MAC idle to max !!\n"));
}
...
VOID TDLS_DisablePktChannel(RTMP_ADAPTER *pAd, UCHAR QSel)
{
	...
	if (QSel == FIFO_HCCA) {
		...
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("unknow Out-Q\n"));
		return;
	}

	if (MTxCycle >= 10)
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Polling %s Out-Q max\n", (QSel == FIFO_EDCA ? "EDCA" : "HCCA")));
	...
	IN UCHAR TargetChannelBW)
{
	...
	if (TargetChannelBW == EXTCHA_ABOVE) {
		...
		if (pAd->MACVersion == 0x28600100) {
			...
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("!!!rt2860C !!!\n"));
		}

		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("A. %d. %ld @!\n", TargetChannel, (jiffies * 1000) / OS_HZ));
		...
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("B. %d. %ld @!\n", TargetChannel, (jiffies * 1000) / OS_HZ));
	} else
#endif /* DOT11_N_SUPPORT */
	{
		/* Must using 20MHz. */
		...
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%d... %ld @!\n", TargetChannel, (jiffies * 1000) / OS_HZ));
	}
	...
	IN UCHAR FrameType)
{
	...
	if ((!pEntry) || (!IS_ENTRY_TDLS(pEntry))) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR,  ("%s() --> unknow Entry !!!\n", __func__));
	...
	if (Status != NDIS_STATUS_SUCCESS) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s (error:: can't allocate NDIS PACKET)\n", __func__));
		return NDIS_STATUS_FAILURE;
	}
	...
	IN	BOOLEAN bWaitACK)
{
	...
	if (((pAd->StaCfg[0].wdev.AuthMode == Ndis802_11AuthModeWPA) ||
		...
		(pAd->StaCfg[0].wdev.PortSecured == WPA_802_1X_PORT_NOT_SECURED)) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("===> %s() - Port not Secured !!!\n", __func__));
		return;
	}
	...
	if ((FreeNum > 0)) {
		...
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("not enough space in MgmtRing, MgmtRingFullCount=%ld!\n",
				 pAd->RalinkCounters.MgmtRingFullCount));
	}
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	if (RTMP_TIME_AFTER(Now, pAd->StaCfg[0].LastBeaconRxTime + (3 * BPtoJiffies * pAd->StaCfg[0].DtimPeriod))) {
		...
		RTMP_CLEAR_MORE_FLAG(pAd, fRTMP_ADAPTER_DISABLE_DEQUEUE);
		RTMP_OS_NETDEV_WAKE_QUEUE(pAd->net_dev);
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("1000.\n"));
	}
}
