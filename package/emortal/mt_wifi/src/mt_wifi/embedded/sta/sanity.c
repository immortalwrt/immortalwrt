	...
	OUT UCHAR *pSsidLen)
{
	...
	if (Info->SsidLen > MAX_LEN_OF_SSID) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(): fail - wrong SSID length\n",
				 __func__));
		return FALSE;
	}
	...
	OUT IE_LISTS * ie_list)
{
	...
	*pAid = (*pAid) & 0x3fff;	/* AID is low 14-bit */
	...
	if ((IeType != IE_SUPP_RATES) || (sup_rate_len > MAX_LEN_OF_SUPPORTED_RATES)) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(): fail - wrong SupportedRates IE\n", __func__));
		return FALSE;
	} else
		parse_support_rate_ie(rate, (EID_STRUCT *)&pFrame->Octet[6]);
	...
	while ((Length + 2 + pEid->Len) <= MsgLen) {
		...
		case IE_FT_FTIE:
			...
			if (pStaCfg->Dot11RCommInfo.bFtSupport) {
				if (pEid->Len >= sizeof(FT_FTIE)) {
					...
					MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s():pEid->Len(%d) < sizeof(FT_FTIE)\n", __func__, pEid->Len));
			}
		...
		case IE_EXT_CAPABILITY:
			...
			if (pEid->Len >= 1) {
				...
				MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("PeerAssocReqSanity - IE_EXT_CAPABILITY!\n"));
			}
		...
		case IE_WLAN_EXTENSION:
		{
			...
			break;
		default:
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s():ignore unrecognized EID = %d\n", __func__, pEid->Eid));
			break;
		}
	...
	if (pStaCfg->Dot11RCommInfo.bFtSupport &&
		...
		if (RTMPEqualMemory(ft_mic, rcvd_mic, 16) == FALSE) {
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : MIC is different\n", __func__));
