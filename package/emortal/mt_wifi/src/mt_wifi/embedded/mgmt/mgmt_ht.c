	...
	struct common_ies *cmm_ies)
{
	...
	*/
	...
	MTWF_LOG(DBG_CAT_MLME, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("%s(): HtCapInfo.ChannelWidth=%d, RecomWidth=%d, Configure HT_BW=%d, BW40MAvailForA/G=%d/%d\n",
			  __func__, sta_cfg->MlmeAux.HtCapability.HtCapInfo.ChannelWidth, ht_op->AddHtInfo.RecomWidth,
...
static INT build_ht_cap_ie(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, UINT16 ReptMacTabWCID, UCHAR *buf)
{
	...
	if (wdev == NULL) {/*ALPS05330340*/
		MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s : wdev is null\n", __func__));
		return 0;
	}
