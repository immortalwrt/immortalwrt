	...
	u_char tx_mode, struct tx_mpdu_info *mpdu_info)
{
	...
	if (mpdu_info->tx_len > TEST_MAX_MSDU_LEN) {
		...
		if (mpdu_info->tx_len > datalen_limit[tx_mode].amsdu_limit)
	...
	void **virtual_wtbl)
{
	...
	if (stack->index == 0) {
		...
		for (sta_seq = 0 ; sta_seq < stack->index ; sta_seq++) {
	...
	u_int8 ctrl_band_idx)
{
	...
	if (configs->per_pkt_bw >= TEST_BW_160C)
		/* BW160 follows TXD option */
		tx_info.bw = TEST_TXD_BW160;
	else
	...
	u_int32 en_log)
{
	...
	band_num = (IS_TEST_DBDC(winfos)) ? 2 : 1;
	...
	u_char band_idx)
{
	...
	if (op_mode & OP_MODE_TXCARR) {
		...
		configs->rf_pwr = 0;
		configs->digi_pwr = 0;
		ret = ops->op_dbdc_tx_tone(winfos, band_idx, configs);
		if (ret)
			goto err;

		ret = ops->op_dbdc_tx_tone_pwr(winfos, band_idx, configs);
		if (ret)
			goto err;
	}
