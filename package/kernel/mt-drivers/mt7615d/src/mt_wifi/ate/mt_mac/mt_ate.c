...
static VOID MtATEWTBL2Update(RTMP_ADAPTER *pAd, UCHAR wcid)
{
	...
	stbc = ATECtrl->Stbc;
	nss = 1;
	rate[0] = tx_rate_to_tmi_rate(ATECtrl->PhyMode, ATECtrl->Mcs, nss, stbc,
				      preamble);
	rate[0] &= 0xfff;
...
static INT32 MT_ATERestoreInit(RTMP_ADAPTER *pAd)
{
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		 ("%s: Rx receive for 10000us workaround\n", __func__));
...
static INT32 MT_ATESetTxPower0(RTMP_ADAPTER *pAd, CHAR Value)
{
	...
	ATECtrl->TxPower1 =
		Value; /* pAd->EEPROMImage[TX1_G_BAND_TARGET_PWR]; */
...
static INT32 MT_ATESetTxPower1(RTMP_ADAPTER *pAd, CHAR Value)
{
	...
	if (ATECtrl->TxPower0 != ATECtrl->TxPower1) {
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			 ("%s: power1 do not same as power0\n", __func__));
		Ret = -1;
	}
...
static INT32 MT_ATEStart(RTMP_ADAPTER *pAd)
{
	...
	if (ATECtrl->en_man_set_freq == 1) {
		if (IS_MT76x6(pAd) || IS_MT7637(pAd)) {
			...
			RTMP_IO_WRITE32(pAd, FREQ_OFFSET_MANUAL_ENABLE,
					ATECtrl->en_man_freq_restore);
			RTMP_IO_WRITE32(pAd, FREQ_OFFSET_MANUAL_VALUE,
...
static INT32 MT_ATESetupFrame(RTMP_ADAPTER *pAd, UINT32 TxIdx)
{
	...
	pAd->archOps.write_tmac_info_fixed_rate(pAd, pDMAHeaderBufVA, &Info,
						&Transmit);
	NdisMoveMemory(pDMAHeaderBufVA + TXWISize, ATECtrl->TemplateFrame,
		       ATECtrl->HLen);
	NdisMoveMemory(pDMAHeaderBufVA + TXWISize + 4, ATECtrl->Addr1,
		       MAC_ADDR_LEN);
	NdisMoveMemory(pDMAHeaderBufVA + TXWISize + 10, ATECtrl->Addr2,
		       MAC_ADDR_LEN);
	NdisMoveMemory(pDMAHeaderBufVA + TXWISize + 16, ATECtrl->Addr3,
		       MAC_ADDR_LEN);
#ifdef RT_BIG_ENDIAN
	...
	pTxD->DMADONE = 0;
#ifdef RT_BIG_ENDIAN
	MTMacInfoEndianChange(pAd, pDMAHeaderBufVA, TYPE_TMACINFO,
			      sizeof(TMAC_TXD_L));
	RTMPFrameEndianChange(pAd, (((PUCHAR)pDMAHeaderBufVA) + TXWISize),
			      DIR_WRITE, FALSE);
...
static INT32 MT_ATEStartTx(RTMP_ADAPTER *pAd)
{
	...
	MtCmdChannelSwitch(pAd, ATECtrl->ControlChl, ATECtrl->Channel,
			   ATECtrl->BW, pAd->Antenna.field.TxPath,
			   pAd->Antenna.field.RxPath, FALSE);
	...
	if (if_ops->test_frame_tx) {
		...
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			 ("%s: no tx test frame callback function\n",
			  __func__));
...
static INT32 MT_ATESetTxAntenna(RTMP_ADAPTER *pAd, CHAR Ant)
{
	...
	if ((MTK_REV_GTE(pAd, MT7603, MT7603E1)) ||
		...
		if (pAd->E2pAccessMode == E2P_NONE) {
			...
			MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
				 ("E2P_NONE, EFUSE Value == 0x%x, ePA:0x%x\n",
				  v_efuse, ePA));
			...
			ePA = (v_efuse >> 8) & 0x00FF;
			MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
				 ("EFUSE_MODE, EFUSE Value == 0x%x, ePA:0x%x\n",
				  v_efuse, ePA));
		} else
#endif /* RTMP_EFUSE_SUPPORT */
		{
			MTWF_LOG(
				...
				("%s: BIN/FLASH/EEPROM_MODE(0x%x), get value in buffer\n",
				 __func__, pAd->E2pAccessMode));
			ePA = pAd->EEPROMImage[0x35];
		}

		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("%s: ePA:0x%x, E2pAccessMode:0x%x\n", __func__, ePA,
			  pAd->E2pAccessMode));
...
static INT32 MT_ATESampleRssi(RTMP_ADAPTER *pAd, RX_BLK *RxBlk)
{
	...
	if (RxBlk->rx_signal.raw_rssi[0] != 0) {
		ATECtrl->LastRssi0 = ConvertToRssi(
			pAd,
			(struct raw_rssi_info *)(&RxBlk->rx_signal.raw_rssi[0]),
			RSSI_IDX_0);
	...
	if (RxBlk->rx_signal.raw_rssi[1] != 0) {
		...
		ATECtrl->AvgRssi1X8 =
			(ATECtrl->AvgRssi1X8 - ATECtrl->AvgRssi1) +
			ATECtrl->LastRssi1;
...
static INT32 MT_ATEStartContinousTx(RTMP_ADAPTER *pAd, CHAR WFSel)
{
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		 ("%s\n", __func__));
	CmdTxContinous(pAd, ATECtrl->PhyMode, ATECtrl->BW, ATECtrl->ControlChl,
		       ATECtrl->Mcs, ATECtrl->TxAntennaSel, 1);
	return Ret;
}
...
static INT32 MT_ATEStopContinousTx(RTMP_ADAPTER *pAd)
{
	...
	CmdTxContinous(pAd, ATECtrl->PhyMode, ATECtrl->BW, ATECtrl->ControlChl,
		       ATECtrl->Mcs, ATECtrl->TxAntennaSel, 0);
	return Ret;
}
...
static INT32 MT_GetFWInfo(RTMP_ADAPTER *pAd, UCHAR *FWInfo)
{
	...
	memcpy(FWInfo,
	       ctrl->fw_profile[WM_CPU].source.img_ptr +
		       ctrl->fw_profile[WM_CPU].source.img_len - 36,
	       36);
	return 0;
}
...
pci_ate_init_err:
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
		 ("%s: Allocate test packet fail at pakcet%d\n", __func__,
		  Index));
...
static INT32 pci_setup_frame(RTMP_ADAPTER *pAd, UINT32 q_idx)
{
	...
	for (Index = 0; (Index < TX_RING_SIZE) && (Index < ATECtrl->TxCount);
	     Index++) {
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("Index = %d, ATECtrl->TxCount = %u\n", Index,
			  ATECtrl->TxCount));
...
static INT32 pci_clean_test_rx_frame(RTMP_ADAPTER *pAd)
{
	...
	for (RingNum = 0; RingNum < GET_NUM_OF_RX_RING(cap); RingNum++) {
		for (Index = 0; Index < RX_RING_SIZE; Index++) {
			...
			pRxD = (RXD_STRUC *)pAd->PciHif.RxRing[0]
				       .Cell[Index]
				       .AllocVa;
#endif
			pRxD->DDONE = 0;
...
static INT32 pci_ate_leave(RTMP_ADAPTER *pAd)
{
	...
	if (ATECtrl->allocated == 1) {
		...
		for (Index = 0; Index < TX_RING_SIZE; Index++) {
			PNDIS_PACKET pPacket;
#ifndef RT_BIG_ENDIAN
			pTxD = (TXD_STRUC *)pAd->PciHif.TxRing[QID_AC_BE]
				       .Cell[Index]
				       .AllocVa;
#else
			pDestTxD = (TXD_STRUC *)pAd->TxRing[QID_AC_BE]
					   .Cell[Index]
					   .AllocVa;
