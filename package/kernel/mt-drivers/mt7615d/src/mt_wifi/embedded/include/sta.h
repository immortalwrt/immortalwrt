...
#define STA_NO_SECURITY_ON(_p)                                                 \
	(IS_CIPHER_NONE(_p->StaCfg[0].wdev.SecConfig.PairwiseCipher))
#define STA_WEP_ON(_p)                                                         \
	(IS_CIPHER_WEP(_p->StaCfg[0].wdev.SecConfig.PairwiseCipher))
#define STA_TKIP_ON(_p)                                                        \
	(IS_CIPHER_TKIP(_p->StaCfg[0].wdev.SecConfig.PairwiseCipher))
#define STA_AES_ON(_p)                                                         \
	(IS_CIPHER_CCMP128(_p->StaCfg[0].wdev.SecConfig.PairwiseCipher))

#define STA_TGN_WIFI_ON(_p) (_p->StaCfg[0].bTGnWifiTest == TRUE)

#define CKIP_KP_ON(_p)                                                         \
	((((_p)->StaCfg[0].CkipFlag) & 0x10) &&                                \
	 ((_p)->StaCfg[0].bCkipCmicOn == TRUE))
#define CKIP_CMIC_ON(_p)                                                       \
	((((_p)->StaCfg[0].CkipFlag) & 0x08) &&                                \
	 ((_p)->StaCfg[0].bCkipCmicOn == TRUE))
...
#define STA_EXTRA_SETTING(_pAd)                                                \
	{                                                                      \
		if ((_pAd)->StaCfg[0].Dot11RCommInfo.bFtSupport &&             \
			...
			(_pAd)->StaCfg[0].Dot11RCommInfo.bInMobilityDomain =   \
				TRUE;                                          \
	}
...
#define STA_PORT_SECURED(_pAd)                                                 \
	{                                                                      \
		...
		(_pAd)->MacTab.tr_entry[pMEntry->wcid].PortSecured =           \
			pwdev->PortSecured;                                    \
...
VOID AdjustChannelRelatedValue(IN PRTMP_ADAPTER pAd, OUT UCHAR *pBwFallBack,
			       IN USHORT ifIndex, IN BOOLEAN BandWidth,
			       IN UCHAR PriCh, IN UCHAR ExtraCh,
			       IN struct wifi_dev *wdev);

VOID RTMPReportMicError(IN PRTMP_ADAPTER pAd, IN PSTA_ADMIN_CONFIG pStaCfg,
...
VOID sta_ieee_802_11_data_tx(RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
			     TX_BLK *pTxBlk);
VOID sta_ieee_802_3_data_tx(RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
			    TX_BLK *pTxBlk);
INT sta_ieee_802_3_data_rx(RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
			   RX_BLK *pRxBlk, MAC_TABLE_ENTRY *pEntry);
INT sta_ieee_802_11_data_rx(RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
			    RX_BLK *pRxBlk, MAC_TABLE_ENTRY *pEntry);
INT sta_tx_pkt_allowed(RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
		       PNDIS_PACKET pPacket);
VOID sta_find_cipher_algorithm(RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
			       TX_BLK *pTxBlk);
BOOLEAN sta_dev_rx_mgmt_frm(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk,
...
UINT32 bssinfo_sta_feature_decision(struct wifi_dev *wdev, UCHAR wcid,
				    UINT32 *feature);

INT StaAllowToSendPacket_new(IN RTMP_ADAPTER *pAd, IN struct wifi_dev *wdev,
			     IN PNDIS_PACKET pPacket, IN UCHAR *pWcid);
...
VOID RTMPStaCfgRadioCtrlFromEEPROM(RTMP_ADAPTER *pAd,
