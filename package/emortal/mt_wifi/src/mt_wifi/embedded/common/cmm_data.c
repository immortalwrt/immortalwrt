...
UINT32 mt_rx_pkt_process(RTMP_ADAPTER *pAd, UINT8 resource_idx, RX_BLK *rx_blk, PNDIS_PACKET rx_packet)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s, rx_pkt_type(%d)\n", __func__, rx_pkt_type));
	...
	default:
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s():Invalid PktType:%d (res_attr:%d)\n", __func__, rx_pkt_type, res_attr));
		if (res_attr != HIF_RX_DATA)
			hif_free_rx_buf(pAd->hdev_ctrl, resource_idx);
		else
	...
	IN struct wifi_dev *wdev)
{
	...
	if (pAd->CommonCfg.bEnableVlan && wdev->bVLAN_Tag && TypeLen != ETH_TYPE_VLAN
		...
		if (pPacket == NULL) {
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Insert VLAN Tag fail!\n"));
			return FALSE;
		}
...
extern BOOLEAN hotspot_osu_data_handler(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry, RX_BLK *pRxBlk);
#endif
	...
	IN UCHAR wdev_idx)
{
	...
	if (pEntry && IS_ENTRY_PEER_AP(pEntry)) {
		...
		if ((sup_info != NULL) && (sup_info->WpaSupplicantUP)) {
			...
			*/
			to_mlme = FALSE;
			MTWF_LOG(DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("eapcode=%d\n", eapcode));
			...
			if ((wdev->SecConfig.IEEE8021X == TRUE) && IS_CIPHER_WEP(wdev->SecConfig.PairwiseCipher) &&
				...
				MTWF_LOG(DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Receive EAP-SUCCESS Packet\n"));
#ifdef CONFIG_AP_SUPPORT
...
VOID dev_rx_ctrl_frm(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s - subtype:%d\n", __func__, FC->SubType));
	...
	case SUBTYPE_TRIGGER:
		MTWF_LOG(DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s():Recive TRIG frame . A1:%02x:%02x:%02x:%02x:%02x:%02x,A2:%02x:%02x:%02x:%02x:%02x:%02x\n\r", __func__,
		 PRINT_MAC(pRxBlk->Addr1), PRINT_MAC(pRxBlk->Addr2)));
#ifdef FW_LOG_DUMP
	...
	/* Spatial Reuse - HWITS00021718 */
	{
		if (IS_MT7915(pAd)) {
			SrRstNav(pAd, pRxBlk);
			MTWF_LOG(DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s():Recive CFEND or CFACK frame . A1:%02x:%02x:%02x:%02x:%02x:%02x,A2:%02x:%02x:%02x:%02x:%02x:%02x\n\r", __func__,
			PRINT_MAC(pRxBlk->Addr1), PRINT_MAC(pRxBlk->Addr2)));
		}
		break;
	}
...
INT rx_chk_duplicate_mgmt_frame(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk)
{
	...
	MTWF_LOG(DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s(): pFrameCtrl->Retry=%d, trEntry->cacheMgmtSn[%d]=%d, pkt->sn=%d\n",
		...
		if (pEntry) {
				...
				if (pAd->WdsTab.Mode[rx_blk->band] && IS_UCAST_MAC(rx_blk->Addr1)) {
					...
					if (MAC_ADDR_EQUAL(rx_blk->Addr1, main_bss_wdev->if_addr))
						...
						MTWF_LOG(DBG_CAT_AP, CATAP_WDS, DBG_LVL_ERROR,
							 ("%s(): WDS for TA(%02x-%02x-%02x-%02x-%02x-%02x) is not enabled\n",
							  __func__, PRINT_MAC(rx_blk->Addr1)));
				}
			}
			...
			if ((pFmeCtrl->FrDs == 0) && (pFmeCtrl->ToDs == 1))
				...
				for (i = 0; i < MAX_APCLI_NUM; i++) {
					...
					if (MAC_ADDR_EQUAL(rx_blk->Addr1, wdev->if_addr)) {
						ap_cls3_err_action(pAd, rx_blk);
						break;
					}
				}
			}
...
INT wdev_tx_pkts(NDIS_HANDLE dev_hnd, PPNDIS_PACKET pkt_list, UINT pkt_cnt, struct wifi_dev *wdev)
{
	...
	for (Index = 0; Index < pkt_cnt; Index++) {
        ...
        if (HcGetBandByWdev(wdev) < DBDC_BAND_NUM &&
           ...
           MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s(%d): get sta ip success\n", __FUNCTION__, __LINE__));
        }
        ...
        if (HcGetBandByWdev(wdev) < DBDC_BAND_NUM &&
           ...
           MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s(%d): get sta hostname success\n", __FUNCTION__, __LINE__));
        }
...
static void pktloss_chk_hex_dump(PUINT_8 pData, UINT_32 dump_len)
{
	...
	for (i = 0; i < dump_len; i++) {
		...
		if ((i%16 == 15) || (len >= (dump_len - 1))) {
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s", buf));
			len = 0;
		}
	}
}
...
void pktloss_chk_dump(PRTMP_ADAPTER pAd, BOOLEAN dump_all)
{
	...
	for (i = 0; i < MAX_PKTLOSS_CHK_PT; i++) {
		if (pktloss_chk->tot_cnt[i] > 0) {
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				("PKTLOSS[%d]times:%u,cnt:%u[tot_tx:%u,drop_tx:%u,seq:%u]\n",
