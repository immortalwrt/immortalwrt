...
static UINT32 LPON_TT0SBOR_CR_MAPPING_TABLE[] = {
	LPON_TT0TPCR,	LPON_TT0SBOR1,	LPON_TT0SBOR2,	LPON_TT0SBOR3,
	LPON_TT0SBOR4,	LPON_TT0SBOR5,	LPON_TT0SBOR6,	LPON_TT0SBOR7,
	LPON_TT0SBOR8,	LPON_TT0SBOR9,	LPON_TT0SBOR10, LPON_TT0SBOR11,
	LPON_TT0SBOR12, LPON_TT0SBOR13, LPON_TT0SBOR14, LPON_TT0SBOR15,
};
#endif
...
INT set_get_fid(RTMP_ADAPTER *pAd, char *arg)
{
	...
	UINT32 head_fid_addr = 0, next_fid_addr = 0, value = 0x00000000L,
	       dw_content;
	/* TODO: Star */
	q_idx = simple_strtol(arg, 0, 10);
	value = 0x00400000 | (q_idx << 16); /* port2. queue by input value. */
	MAC_IO_WRITE32(pAd, 0x8024, value);
	MAC_IO_READ32(pAd, 0x8024,
		      (UINT32 *)&head_fid_addr); /* get head FID. */
	...
	while (1) {
		for (dw_idx = 0; dw_idx < 8; dw_idx++) {
			MAC_IO_READ32(
				pAd,
				((MT_PCI_REMAP_ADDR_1 +
				  (((value & 0x0fff0000) >> 16) * 128)) +
				 (dw_idx * 4)),
				(UINT32 *)&dw_content); /* get head FID. */
			MTWF_LOG(
				DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				("pkt:%d, fid:%x, dw_idx = %d, dw_content = 0x%x\n",
				 loop, ((value & 0x0fff0000) >> 16), dw_idx,
				 dw_content));
		}

		MAC_IO_WRITE32(pAd, 0x8028, value);
		MAC_IO_READ32(pAd, 0x8028,
			      (UINT32 *)&next_fid_addr); /* get next FID. */
...
static VOID Wtbl2TxRateCounterGet(struct _RTMP_ADAPTER *pAd, UCHAR ucWcid,
...
VOID MtAsicSetBssid(RTMP_ADAPTER *pAd, UCHAR *pBssid, UCHAR BssidIndex)
{
	...
	ULONG curr_bssid_reg_base =
		RMAC_CB0R0; /* register for Current_Bssid from 0x60140004 */

	val = (UINT32)((pBssid[0]) | (UINT32)(pBssid[1] << 8) |
		       (UINT32)(pBssid[2] << 16) | (UINT32)(pBssid[3] << 24));
...
INT32 MtAsicSetDevMac(RTMP_ADAPTER *pAd, UINT8 OwnMacIdx, UINT8 *OwnMacAddr,
		      UINT8 BandIdx, UINT8 Active, UINT32 EnableFeature)
{
	UINT32 val;
	ULONG own_mac_reg_base =
		RMAC_OMA0R0; /* register for Own_Mac from 0x60140024 */

	if (Active) {
		val = (OwnMacAddr[0]) | (OwnMacAddr[1] << 8) |
		      (OwnMacAddr[2] << 16) | (OwnMacAddr[3] << 24);
		MAC_IO_WRITE32(pAd, own_mac_reg_base + (OwnMacIdx * 8), val);
		val = OwnMacAddr[4] | (OwnMacAddr[5] << 8) | (1 << 16);
		MAC_IO_WRITE32(pAd, (own_mac_reg_base + 4) + (OwnMacIdx * 8),
			       val);
	}

#if !defined(COMPOS_TESTMODE_WIN) && !defined(COMPOS_WIN)
	AsicDevInfoUpdate(pAd, OwnMacIdx, OwnMacAddr, BandIdx, Active,
...
INT MtAsicSetRDGByTLV(RTMP_ADAPTER *pAd, BOOLEAN bEnable, UINT8 Wcid)
{
	...
	return CmdExtWtblUpdate(pAd, Wcid, SET_WTBL, (PUCHAR)&CmdWtblRdg,
...
INT MtAsicWtblSetRDG(RTMP_ADAPTER *pAd, BOOLEAN bEnable, UINT8 Wcid)
{
	...
	union WTBL_1_DW2 *dw2 =
		(union WTBL_1_DW2 *)&tb_entry.wtbl_1.wtbl_1_d2.word;
		       ...
		       UCHAR BandIdx)
{
	...
	case ASIC_MAC_RXV:
		if (Enable) {
			Value &= ~MT_ARB_SCR_RXDIS;
			Value2 |= (ARB_RQCR_RXV_START | ARB_RQCR_RXV_R_EN |
				   ARB_RQCR_RXV_T_EN);
		} else {
			Value2 &= ~(ARB_RQCR_RXV_START | ARB_RQCR_RXV_R_EN |
				    ARB_RQCR_RXV_T_EN);
...
INT MtAsicSetRxStream(RTMP_ADAPTER *pAd, UINT32 StreamNums, UCHAR BandIdx)
{
	...
	Value &= ~(RMAC_RMCR_RX_STREAM_0 | RMAC_RMCR_RX_STREAM_1 |
...
INT MtAsicSetRxPath(RTMP_ADAPTER *pAd, UINT32 RxPathSel, UCHAR BandIdx)
{
	...
	switch (RxPathSel) {
	case 0: /* ALL */
		Mask = (RMAC_RMCR_RX_STREAM_0 | RMAC_RMCR_RX_STREAM_1 |
			RMAC_RMCR_RX_STREAM_2);
		break;
...
static VOID RxFilterCfg2Row(UINT32 FilterMask, UINT32 *RowFilterMask)
{
	...
	*RowFilterMask = FilterMask & RX_NOT_OWN_BTIM ?
				 (*RowFilterMask | DROP_DIFF_BSSID_BTIM) :
				       (*RowFilterMask & ~(DROP_DIFF_BSSID_BTIM));
	*RowFilterMask = FilterMask & RX_NDPA ? (*RowFilterMask | DROP_NDPA) :
						      (*RowFilterMask & ~(DROP_NDPA));
}
...
INT MtAsicSetRxFilter(RTMP_ADAPTER *pAd, MT_RX_FILTER_CTRL_T RxFilter)
{
	...
	if (RxFilter.bPromiscuous) {
		...
		if (IS_MT7615(pAd))
			Value = DROP_FCS_ERROR_FRAME | RM_FRAME_REPORT_EN |
				RX_UNWANTED_CTL_FRM;
		else
#endif
...
INT32 MtAsicRxHeaderTransCtl(RTMP_ADAPTER *pAd, BOOLEAN En, BOOLEAN ChkBssid,
...
INT32 MtAsicRxHeaderTaranBLCtl(RTMP_ADAPTER *pAd, UINT32 Index, BOOLEAN En,
			       UINT32 EthType)
{
	UINT32 Value, Value1;
...
INT32 MtAsicUpdateBASessionByDriver(RTMP_ADAPTER *pAd, MT_BA_CTRL_T BaCtrl)
{
	...
	if (BaCtrl.BaSessionType == BA_SESSION_RECP) {
		...
		if (BaCtrl.isAdd) {
			value = (BaCtrl.PeerAddr[0] |
				 (BaCtrl.PeerAddr[1] << 8) |
				 (BaCtrl.PeerAddr[2] << 16) |
				 (BaCtrl.PeerAddr[3] << 24));
			MAC_IO_WRITE32(pAd, BSCR0, value);
			MAC_IO_READ32(pAd, BSCR1, &value);
			value &= ~(BA_MAC_ADDR_47_32_MASK | RST_BA_TID_MASK |
				   RST_BA_SEL_MASK);
			value |= BA_MAC_ADDR_47_32((BaCtrl.PeerAddr[4] |
						    (BaCtrl.PeerAddr[5] << 8)));
			value |= (RST_BA_SEL(RST_BA_MAC_TID_MATCH) |
				  RST_BA_TID(BaCtrl.Tid) | START_RST_BA_SB);
		...
		if (BaCtrl.isAdd) {
			...
			case 2:
				reg = ent.wtbl_addr[1] +
				      (4 * 2); /* WTBL2.DW2 */
				HW_IO_READ32(pAd, reg, &wtbl_2->wtbl_2_d2.word);
				wtbl_2->wtbl_2_d2.field.tid_ac_2_sn_0 =
					(BaCtrl.Sn & 0xff);
				HW_IO_WRITE32(pAd, reg, wtbl_2->wtbl_2_d2.word);
				reg = ent.wtbl_addr[1] +
				      (4 * 3); /* WTBL2.DW3 */
				HW_IO_READ32(pAd, reg, &wtbl_2->wtbl_2_d3.word);
				wtbl_2->wtbl_2_d3.field.tid_ac_2_sn_9 =
					((BaCtrl.Sn & (0xf << 8)) >> 8);
			...
			case 5:
				reg = ent.wtbl_addr[1] +
				      (4 * 3); /* WTBL2.DW3 */
				HW_IO_READ32(pAd, reg, &wtbl_2->wtbl_2_d3.word);
				wtbl_2->wtbl_2_d3.field.tid_5_sn_0 =
					(BaCtrl.Sn & 0xf);
				HW_IO_WRITE32(pAd, reg, wtbl_2->wtbl_2_d3.word);
				reg = ent.wtbl_addr[1] +
				      (4 * 4); /* WTBL2.DW4 */
				HW_IO_READ32(pAd, reg, &wtbl_2->wtbl_2_d4.word);
				wtbl_2->wtbl_2_d4.field.tid_5_sn_5 =
					((BaCtrl.Sn & (0xff << 4)) >> 4);
			...
			wtbl_2->wtbl_2_d15.field.ba_win_size_tid &=
				(~range_mask);
			wtbl_2->wtbl_2_d15.field.ba_win_size_tid |=
				(idx << (BaCtrl.Tid * 3));
		...
		MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("%s(): ent->wtbl_addr[1]=0x%x, val=0x%08x\n",
...
VOID MtAsicSetRxGroup(RTMP_ADAPTER *pAd, UINT32 Port, UINT32 Group,
		      BOOLEAN Enable)
{
	UINT32 Value;
 ...
 */
	...
	VOID MtAsicUpdateRtsThld(PRTMP_ADAPTER pAd,
 ...
 * ========================================================================
 */
	VOID MtAsicUpdateProtect(IN PRTMP_ADAPTER pAd,
				 IN MT_PROTECT_CTRL_T * Protect)
	{
		...
		/* Other HT Protection */
		val &= ~(MM_PROTECTION | BW40_PROTECTION | BW80_PROTECTION |
			 BW160_PROTECTION);
...
#define INT_TIMER_EN_GP_TIMER 0x2
	static INT SetIntTimerEn(RTMP_ADAPTER * pAd, BOOLEAN enable,
				 UINT32 type, UINT32 timeout)
	{
 ...
 * @{
 */
	INT MtAsicSetPreTbtt(struct _RTMP_ADAPTER * pAd, BOOLEAN bEnable,
			     UCHAR HwBssidIdx)
	{
		 ...
		 */
			...
			MTWF_LOG(
				...
				("%s(): HwBssidIdx=%d, PreTBTT timeout = 0x%x\n",
 ...
 * ==========================================================================
 */
	VOID MtAsicSetApCliBssid(IN PRTMP_ADAPTER pAd, IN PUCHAR pBssid,
				 IN UCHAR index)
	{
		UINT32 val;

		MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("%s(): Set BSSID=%02x:%02x:%02x:%02x:%02x:%02x\n",
			  __func__, pBssid[0], pBssid[1], pBssid[2], pBssid[3],
			  pBssid[4], pBssid[5]));
		val = (UINT32)((pBssid[0]) | (UINT32)(pBssid[1] << 8) |
			       (UINT32)(pBssid[2] << 16) |
			       (UINT32)(pBssid[3] << 24));
		MAC_IO_WRITE32(pAd, RMAC_CB1R0, val);
		val = (UINT32)(pBssid[4]) | (UINT32)(pBssid[5] << 8) |
		      (1 << 16);
		...
		INT MtAsicSetReptFuncEnableByDriver(RTMP_ADAPTER * pAd,
						    BOOLEAN bEnable)
		{
		...
		VOID MtAsicInsertRepeaterEntry(IN PRTMP_ADAPTER pAd,
					       IN UCHAR CliIdx, IN PUCHAR pAddr)
		{
			...
			MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				 ("\n%s %02x:%02x:%02x:%02x:%02x:%02x-%02x\n",
				  __func__, tempMAC[0], tempMAC[1], tempMAC[2],
				  tempMAC[3], tempMAC[4], tempMAC[5], CliIdx));
			os_zero_mem(&rmac_mcbcs0, sizeof(RMAC_MAR0_STRUC));
			rmac_mcbcs0.addr_31_0 = tempMAC[0] + (tempMAC[1] << 8) +
						(tempMAC[2] << 16) +
						(tempMAC[3] << 24);
		...
		VOID MtAsicRemoveRepeaterEntry(RTMP_ADAPTER * pAd, UCHAR CliIdx)
		{
			...
			if (CliIdx <= 15)
				rmac_mcbcs1.field.multicast_addr_index =
					CliIdx; /* start from idx 0 */

			MAC_IO_WRITE32(
				pAd, RMAC_MAR1,
				rmac_mcbcs1.word); /* clear client entry first. */
			os_zero_mem(&rmac_mcbcs0, sizeof(RMAC_MAR0_STRUC));
			MAC_IO_WRITE32(pAd, RMAC_MAR0, rmac_mcbcs0.addr_31_0);
			os_zero_mem(&rmac_mcbcs1, sizeof(RMAC_MAR1_STRUC));
			rmac_mcbcs1.field.access_start = 1;
			...
			if (CliIdx <= 15)
				rmac_mcbcs1.field.multicast_addr_index =
					0x10 + CliIdx; /* start from idx 16 */

			MAC_IO_WRITE32(
				pAd, RMAC_MAR1,
				rmac_mcbcs1.word); /* clear rootap entry. */
		}

#ifdef CONFIG_WTBL_TLV_MODE
		VOID MtAsicInsertRepeaterRootEntryByWtblTlv(
			IN PRTMP_ADAPTER pAd, IN UCHAR Wcid, IN UCHAR * pAddr,
			IN UCHAR ReptCliIdx)
		{
			...
			RMAC_MAR1_STRUC rmac_mcbcs1;
			/* Allocate TLV msg */
			Status = os_alloc_mem(pAd, (UCHAR **)&pTlvBuffer,
			...
			pTempBuffer = pTlvAppend(pTempBuffer, WTBL_GENERIC,
						 sizeof(CMD_WTBL_GENERIC_T),
						 &rWtblGeneric, &u4TotalTlvLen,
						 &ucTotalTlvNumber);
			pTempBuffer =
				pTlvAppend(pTempBuffer, WTBL_RX,
					   sizeof(CMD_WTBL_RX_T), &rWtblRx,
					   &u4TotalTlvLen, &ucTotalTlvNumber);
			CmdExtWtblUpdate(pAd, Wcid, QUERY_WTBL, pTlvBuffer,
					 u4TotalTlvLen);
			pTempBuffer = pTlvBuffer;
			...
			prWtblRx->ucRca1 = 1;
			CmdExtWtblUpdate(pAd, Wcid, SET_WTBL, pTlvBuffer,
					 u4TotalTlvLen);
			...
			MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				 ("%s(%d):Write WTBL 1 DW0 Value:0x%x\n",
				  __func__, ReptCliIdx,
				  WtblDwQuery(pAd, Wcid, 1, 0)));
			MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				 ("%s(%d):Write WTBL 1 DW1 Value:0x%x\n",
				  __func__, ReptCliIdx,
				  WtblDwQuery(pAd, Wcid, 1, 1)));
			MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				 ("%s(%d):Write WTBL 1 DW2 Value:0x%x\n",
				  __func__, ReptCliIdx,
				  WtblDwQuery(pAd, Wcid, 1, 2)));
			/* Set RMAC_MAR0/1 */
			NdisZeroMemory(&rmac_mcbcs0, sizeof(RMAC_MAR0_STRUC));
			rmac_mcbcs0.addr_31_0 = pAddr[0] + (pAddr[1] << 8) +
						(pAddr[2] << 16) +
						(pAddr[3] << 24);
			MAC_IO_WRITE32(pAd, RMAC_MAR0, rmac_mcbcs0.addr_31_0);
...
#else
		VOID MtAsicInsertRepeaterRootEntryByDriver(IN PRTMP_ADAPTER pAd,
							   IN UCHAR Wcid,
							   IN UCHAR * pAddr,
							   IN UCHAR ReptCliIdx)
		{
			...
			struct wtbl_entry tb_entry;
			union WTBL_1_DW0 *dw0 = (union WTBL_1_DW0 *)&tb_entry
							.wtbl_1.wtbl_1_d0.word;
			union WTBL_1_DW1 *dw1 = (union WTBL_1_DW1 *)&tb_entry
							.wtbl_1.wtbl_1_d1.word;
			union WTBL_1_DW2 *dw2 = (union WTBL_1_DW2 *)&tb_entry
							.wtbl_1.wtbl_1_d2.word;
			RMAC_MAR0_STRUC rmac_mcbcs0;
			RMAC_MAR1_STRUC rmac_mcbcs1;
			STA_TR_ENTRY *tr_entry;

			tr_entry = &pAd->MacTab.tr_entry[Wcid];
			tb_entry.wtbl_addr[0] =
				wtbl_ctrl->wtbl_base_addr[0] +
				Wcid * wtbl_ctrl->wtbl_entry_size[0];
			HW_IO_READ32(pAd, tb_entry.wtbl_addr[0],
				     &tb_entry.wtbl_1.wtbl_1_d0.word);
			HW_IO_READ32(pAd, tb_entry.wtbl_addr[0] + 4,
				     &tb_entry.wtbl_1.wtbl_1_d1.word);
			HW_IO_READ32(pAd, tb_entry.wtbl_addr[0] + 8,
				     &tb_entry.wtbl_1.wtbl_1_d2.word);
			dw0->field.wm = 0;
			...
			MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				 ("%s(%d):Write WTBL 1 Addr:0x%x, Value:0x%x\n",
				  __func__, ReptCliIdx, tb_entry.wtbl_addr[0],
				  dw0->word));
			MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				 ("%s(%d):Write WTBL 1 Addr:0x%x, Value:0x%x\n",
				  __func__, ReptCliIdx,
		...
		VOID MtAsicInsertRepeaterRootEntry(IN PRTMP_ADAPTER pAd,
						   IN UCHAR Wcid,
						   IN UCHAR * pAddr,
						   IN UCHAR ReptCliIdx)
		{
	...
	MtAsicInsertRepeaterRootEntryByWtblTlv(pAd, Wcid, pAddr, ReptCliIdx));
#else
			MtAsicInsertRepeaterRootEntryByDriver(pAd, Wcid, pAddr,
							      ReptCliIdx);
#endif /* CONFIG_WTBL_TLV_MODE */
		}
		...
		BOOLEAN MtAsicSetBmcQCR(IN struct _RTMP_ADAPTER * pAd,
					IN UCHAR Operation,
					IN UCHAR CrReadWrite,
					IN UCHAR HwBssidIdx, IN UINT32 apidx,
		...
		BOOLEAN MtAsicSetBcnQCR(IN struct _RTMP_ADAPTER * pAd,
					IN UCHAR Operation, IN UCHAR HwBssidIdx,
					IN UINT32 apidx)
		{
			UINT32 cr_base = 0;
		...
		static SYNC_MODE_CR_TABLE_T
			g_arDisableSyncModeMapTable[HW_BSSID_MAX] = {
				/*WMM cr set band0 first, change it when is checked it link to Band1. */
				{ ARB_SCR, ARB_AIFSR0, ARB_ACCWIR1, LPON_MPTCR1,
				  LPON_T0TPCR, LPON_PISR, LPON_T0CR, HWIER3 },
				{ ARB_SCR, ARB_AIFSR0, ARB_ACCWIR1, LPON_MPTCR1,
				  LPON_T1TPCR, LPON_PISR, LPON_T1CR, HWIER0 },
				{ ARB_SCR, ARB_AIFSR0, ARB_ACCWIR1, LPON_MPTCR3,
				  LPON_T2TPCR, LPON_PISR, LPON_T2CR, HWIER0 },
				{ ARB_SCR, ARB_AIFSR0, ARB_ACCWIR1, LPON_MPTCR3,
				  LPON_T3TPCR, LPON_PISR, LPON_T3CR, HWIER0 },
			};
 ...
 * ==========================================================================
 */
		VOID MtAsicDisableSyncByDriver(RTMP_ADAPTER * pAd,
					       UCHAR HWBssidIdx)
		{
			...
			if (IS_PCI_INF(pAd) || IS_RBUS_INF(pAd)) {
				MAC_IO_READ32(pAd, cr_set.u4IntEnableCR,
					      &value);
				...
				MAC_IO_WRITE32(pAd, cr_set.u4IntEnableCR,
					       value);
			}
			...
			value = 0;
			value |= (TBTT_TIMEUP_EN | TBTT_PERIOD_TIMER_EN |
				  PRETBTT_TIMEUP_EN | PRETBTT_INT_EN);

			if ((HWBssidIdx == HW_BSSID_1) ||
			    (HWBssidIdx == HW_BSSID_3))
				value = (value << 8);
			...
			value &= ~(MT_ARB_SCR_OPMODE_MASK
				   << (HWBssidIdx * 2)); /* clean opmode */
			MAC_IO_WRITE32(pAd, cr_set.u4ArbOpModeCR, value);
		}

		static SYNC_MODE_CR_TABLE_T
			g_arEnableSyncModeMapTable[HW_BSSID_MAX] = {
				/*WMM cr set band0 first, change it when is checked it link to Band1. */
				{ ARB_SCR, ARB_AIFSR0, ARB_ACCWIR1, LPON_MPTCR0,
				  LPON_T0TPCR, LPON_PISR, LPON_T0CR, HWIER3 },
				{ ARB_SCR, ARB_AIFSR0, ARB_ACCWIR1, LPON_MPTCR0,
				  LPON_T1TPCR, LPON_PISR, LPON_T1CR, HWIER0 },
				{ ARB_SCR, ARB_AIFSR0, ARB_ACCWIR1, LPON_MPTCR2,
				  LPON_T2TPCR, LPON_PISR, LPON_T2CR, HWIER0 },
				{ ARB_SCR, ARB_AIFSR0, ARB_ACCWIR1, LPON_MPTCR2,
				  LPON_T3TPCR, LPON_PISR, LPON_T3CR, HWIER0 },
			};

		VOID MtAsicEnableBssSyncByDriver(RTMP_ADAPTER * pAd,
						 USHORT BeaconPeriod,
						 UCHAR HWBssidIdx, UCHAR OPMode)
		{
			...
			MAC_IO_WRITE32(pAd, cr_set.u4ArbOpModeCR, value);
			if (OPMode == OPMODE_AP) {
				MAC_IO_READ32(pAd, cr_set.u4LponSyncModeCR,
					      &value);
				value = value | TSF_TIMER_HW_MODE_TICK_ONLY;
				MAC_IO_WRITE32(pAd, cr_set.u4LponSyncModeCR,
					       value);
			}

			/*3. set Pretbtt time. */
			MAC_IO_READ32(pAd, cr_set.u4LponPreTbttTime, &value);
			value |= DEFAULT_PRETBTT_INTERVAL_IN_MS
				 << (HWBssidIdx * 8);
		...
		INT MtAsicSetGPTimer(RTMP_ADAPTER * pAd, BOOLEAN enable,
				     UINT32 timeout)
		{
			return SetIntTimerEn(pAd, enable, INT_TIMER_EN_GP_TIMER,
					     timeout);
		}
 ...
 * @{
 */
		UINT32 MtAsicGetWmmParam(RTMP_ADAPTER * pAd, UINT32 AcNum,
					 UINT32 EdcaType)
		{
		...
		INT MtAsicSetWmmParam(RTMP_ADAPTER * pAd, UCHAR idx,
				      UINT32 AcNum, UINT32 EdcaType,
				      UINT32 EdcaValue)
		{
		...
		VOID MtAsicSetEdcaParm(RTMP_ADAPTER * pAd, PEDCA_PARM pEdcaParm)
		{
			...
			if ((pEdcaParm != NULL) &&
				...
				for (ac = 0; ac < CMD_EDCA_AC_MAX; ac++) {
					...
					pAcParam->ucAifs =
						pEdcaParm->Aifsn[index];
					pAcParam->ucWinMin =
						pEdcaParm->Cwmin[index];
					pAcParam->u2WinMax =
						pEdcaParm->Cwmax[index];
					pAcParam->u2Txop =
						pEdcaParm->Txop[index];
				}
			}
		...
		INT MtAsicSetRetryLimit(RTMP_ADAPTER * pAd, UINT32 type,
					UINT32 limit)
		{
			/* TODO: shiang-7603 */
			MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
 ...
 */
		...
		VOID MtAsicTxCapAndRateTableUpdateByDriver(
			RTMP_ADAPTER * pAd, UCHAR ucWcid,
			RA_PHY_CFG_T * prTxPhyCfg, UINT32 * Rate, BOOL fgSpeEn)
		{
			...
			wtbl_2_d9.field.cbrn =
				7; /* change bw as (fcap/2) if rate_idx > 7, temporary code */
		...
		VOID MtAsicTxCapAndRateTableUpdate(RTMP_ADAPTER * pAd,
						   UCHAR ucWcid,
						   RA_PHY_CFG_T * prTxPhyCfg,
						   UINT32 * Rate, BOOL fgSpeEn)
		{
			/* #ifdef CONFIG_WTBL_TLV_MODE */
			MtAsicTxCapAndRateTableUpdateByDriver(
				pAd, ucWcid, prTxPhyCfg, Rate, fgSpeEn);
		}
 ...
 */
		...
		static VOID Wtbl2TxRateCounterGetByDriver(
			RTMP_ADAPTER * pAd, UCHAR ucWcid,
			TX_CNT_INFO * tx_cnt_info)
		{
			UINT32 u4RegVal;
			...
			if (wtbl_ctrl->wtbl_entry_cnt[0] > 0)
				wtbl_idx =
					(ucWcid < wtbl_ctrl->wtbl_entry_cnt[0] ?
						 ucWcid :
						       wtbl_ctrl->wtbl_entry_cnt[0] -
							 1);
			else {
				MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL,
					 DBG_LVL_ERROR,
			...
			addr = pAd->mac_ctrl.wtbl_base_addr[1] +
			       wtbl_idx * pAd->mac_ctrl.wtbl_entry_size[1];
			HW_IO_READ32(pAd, addr + 5 * 4,
				     &(tx_cnt_info->wtbl_2_d5.word));
			HW_IO_READ32(pAd, addr + 6 * 4,
				     &(tx_cnt_info->wtbl_2_d6.word));
			HW_IO_READ32(pAd, addr + 7 * 4,
				     &(tx_cnt_info->wtbl_2_d7.word));
			HW_IO_READ32(pAd, addr + 8 * 4,
				     &(tx_cnt_info->wtbl_2_d8.word));
			HW_IO_READ32(pAd, addr + 9 * 4,
				     &(tx_cnt_info->wtbl_2_d9.word));
		...
		static VOID Wtbl2TxRateCounterGet(RTMP_ADAPTER * pAd,
						  UCHAR ucWcid,
						  TX_CNT_INFO * tx_cnt_info)
		{
			/* #ifdef CONFIG_WTBL_TLV_MODE */
 ...
 */
		...
		VOID MtAsicTxCntUpdate(RTMP_ADAPTER * pAd, UCHAR Wcid,
				       MT_TX_COUNTER * pTxInfo)
		{
			...
			pTxInfo->TxCount =
				tx_cnt_info.wtbl_2_d7.field.current_bw_tx_cnt;
			pTxInfo->TxCount +=
				tx_cnt_info.wtbl_2_d8.field.other_bw_tx_cnt;
			pTxInfo->TxFailCount =
				tx_cnt_info.wtbl_2_d7.field.current_bw_fail_cnt;
			pTxInfo->TxFailCount +=
				tx_cnt_info.wtbl_2_d8.field.other_bw_fail_cnt;
			pTxInfo->Rate1TxCnt = (UINT16)tx_cnt_info.wtbl_2_d5
						      .field.rate_1_tx_cnt;
			pTxInfo->Rate1FailCnt = (UINT16)tx_cnt_info.wtbl_2_d5
							.field.rate_1_fail_cnt;
			pTxInfo->Rate2TxCnt =
				(CHAR)tx_cnt_info.wtbl_2_d6.field.rate_2_tx_cnt;
			pTxInfo->Rate3TxCnt =
				(CHAR)tx_cnt_info.wtbl_2_d6.field.rate_3_tx_cnt;
			pTxInfo->Rate4TxCnt =
				(CHAR)tx_cnt_info.wtbl_2_d6.field.rate_4_tx_cnt;
			pTxInfo->Rate5TxCnt =
				(CHAR)tx_cnt_info.wtbl_2_d6.field.rate_5_tx_cnt;
			pTxInfo->RateIndex =
				(CHAR)tx_cnt_info.wtbl_2_d9.field.rate_idx;
		}

		VOID MtAsicRssiGet(RTMP_ADAPTER * pAd, UCHAR Wcid,
				   CHAR * RssiSet)
		{
 ...
 */
		VOID MtAsicUpdateRxWCIDTableByDriver(
			RTMP_ADAPTER * pAd, MT_WCID_TABLE_INFO_T WtblInfo)
		{
			struct wtbl_entry tb_entry;
			union WTBL_1_DW0 *dw0 = (union WTBL_1_DW0 *)&tb_entry
							.wtbl_1.wtbl_1_d0.word;
			...
			union WTBL_1_DW2 *dw2 = (union WTBL_1_DW2 *)&tb_entry
							.wtbl_1.wtbl_1_d2.word;
			union WTBL_1_DW3 *dw3 = (union WTBL_1_DW3 *)&tb_entry
							.wtbl_1.wtbl_1_d3.word;
			union WTBL_1_DW4 *dw4 = (union WTBL_1_DW4 *)&tb_entry
							.wtbl_1.wtbl_1_d4.word;
			UINT32 Value, Index;

			os_zero_mem(&tb_entry, sizeof(tb_entry));

			if (mt_wtbl_get_entry234(pAd, (UCHAR)WtblInfo.Wcid,
						 &tb_entry) == FALSE) {
				MTWF_LOG(
					DBG_CAT_HW, DBG_SUBCAT_ALL,
					DBG_LVL_ERROR,
			...
			if (WtblInfo.WcidType == MT_WCID_TYPE_BMCAST) {
				...
				dw0->field.rkv = (WtblInfo.CipherSuit !=
						  WTBL_CIPHER_NONE) ?
							 1 :
							       0;
			...
			dw1->word = WtblInfo.Addr[0] | (WtblInfo.Addr[1] << 8) |
				    (WtblInfo.Addr[2] << 16) |
				    (WtblInfo.Addr[3] << 24);
			...
			HW_IO_WRITE32(pAd, tb_entry.wtbl_addr[0] + 8,
				      dw2->word);
			HW_IO_WRITE32(pAd, tb_entry.wtbl_addr[0] + 4,
				      dw1->word);
			HW_IO_WRITE32(pAd, tb_entry.wtbl_addr[0], dw0->word);
			HW_IO_WRITE32(pAd, tb_entry.wtbl_addr[0] + 8,
				      dw2->word);
			...
			HW_IO_WRITE32(pAd, tb_entry.wtbl_addr[0] + 12,
				      dw3->word);
			MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
				 ("%s(%d):Write WTBL 1 Addr:0x%x, Value:0x%x\n",
				  __func__, WtblInfo.Wcid,
				  tb_entry.wtbl_addr[0] + 12, dw3->word));
			HW_IO_WRITE32(pAd, tb_entry.wtbl_addr[0] + 16,
				      dw4->word);
			MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
				 ("%s(%d):Write WTBL 1 Addr:0x%x, Value:0x%x\n",
				  __func__, WtblInfo.Wcid,
				  tb_entry.wtbl_addr[0] + 16, dw4->word));
			MAC_IO_READ32(pAd, WTBL1OR, &Value);
			Value &= ~PSM_W_FLAG;
			MAC_IO_WRITE32(pAd, WTBL1OR, Value);
			/* Clear BA Information */
			HW_IO_WRITE32(pAd, tb_entry.wtbl_addr[1] + (15 * 4),
				      tb_entry.wtbl_2.wtbl_2_d15.word);
			/* Clear BA Information */
			HW_IO_WRITE32(pAd, tb_entry.wtbl_addr[1] + (15 * 4),
				      tb_entry.wtbl_2.wtbl_2_d15.word);
			...
			for (Index = 0; Index < 8; Index++)
				HW_IO_WRITE32(pAd,
					      tb_entry.wtbl_addr[2] +
						      (4 * Index),
					      0x0);
		}

		VOID MtAsicUpdateRxWCIDTable(RTMP_ADAPTER * pAd,
 ...
 * ==========================================================================
 */
		VOID MtAsicDelWcidTabByDriver(RTMP_ADAPTER * pAd,
					      UCHAR wcid_idx)
		{
			...
			for (cnt = cnt_s; cnt_s <= cnt_e; cnt_s++) {
				...
				if (mt_wtbl_get_entry234(pAd, cnt, &tb_entry) ==
				    FALSE) {
					MTWF_LOG(
						DBG_CAT_HW, DBG_SUBCAT_ALL,
						DBG_LVL_ERROR,
				...
				dw0->field.rc_a2 = 1;
				dw0->field.rv = 1;
				HW_IO_WRITE32(pAd, tb_entry.wtbl_addr[0],
					      tb_entry.wtbl_1.wtbl_1_d0.word);
				HW_IO_WRITE32(pAd, tb_entry.wtbl_addr[0] + 4,
					      tb_entry.wtbl_1.wtbl_1_d1.word);
				HW_IO_WRITE32(pAd, tb_entry.wtbl_addr[0] + 8,
...
#if defined(COMPOS_WIN) || defined(COMPOS_TESTMODE_WIN)
#else
	VOID MtAsicAddRemoveKeyByDriver(PRTMP_ADAPTER pAd,
					ASIC_SEC_INFO * pInfo)
	{
		struct wtbl_entry tb_entry;
		union WTBL_1_DW0 *dw0 =
			(union WTBL_1_DW0 *)&tb_entry.wtbl_1.wtbl_1_d0.word;
		union WTBL_1_DW1 *dw1 =
			(union WTBL_1_DW1 *)&tb_entry.wtbl_1.wtbl_1_d1.word;
		union WTBL_1_DW2 *dw2 =
			(union WTBL_1_DW2 *)&tb_entry.wtbl_1.wtbl_1_d2.word;
		...
		MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("%s Wcid[%d]. Operation=%d, Direction=%d\n", __func__,
			  pInfo->Wcid, pInfo->Operation, pInfo->Direction));
		os_zero_mem(&tb_entry, sizeof(tb_entry));

		if (mt_wtbl_get_entry234(pAd, pInfo->Wcid, &tb_entry) ==
		    FALSE) {
			MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				 ("%s():Cannot found WTBL2/3/4 for WCID(%d)\n",
				  __func__, pInfo->Wcid));
		...
		addr = pAd->mac_ctrl.wtbl_base_addr[2] +
		       pInfo->Wcid * pAd->mac_ctrl.wtbl_entry_size[2];
		...
		dw1->word = pInfo->PeerAddr[0] + (pInfo->PeerAddr[1] << 8) +
			    (pInfo->PeerAddr[2] << 16) +
			    (pInfo->PeerAddr[3] << 24);
		...
		if (IS_REMOVEKEY_OPERATION(pInfo)) {
			...
			if (IS_CIPHER_WEP(pInfo->Cipher)) {
				...
				os_move_mem(&Key[16], pSecKey->RxMic,
					    LEN_TKIP_MIC);
				os_move_mem(&Key[24], pSecKey->TxMic,
					    LEN_TKIP_MIC);
		...
		MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("%s(wcid =%d)\n", __func__, pInfo->Wcid));
		MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("\tWrite WTBL 1 Addr:0x%x, Value:0x%x\n",
			  tb_entry.wtbl_addr[0], dw0->word));
		MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("\tWrite WTBL 1 Addr:0x%x, Value:0x%x\n",
			  tb_entry.wtbl_addr[0] + 4, dw1->word));
 ...
 * ========================================================================
 */
		VOID MtAsicAddPairwiseKeyEntry(RTMP_ADAPTER * pAd, UCHAR WCID,
					       CIPHER_KEY * pKey)
		{
			/* TODO: shiang-7603 */
 ...
 * ========================================================================
 */
			VOID MtAsicAddSharedKeyEntry(IN PRTMP_ADAPTER pAd,
						     IN UCHAR BssIndex,
						     IN UCHAR KeyIdx,
			...
			/*	IRQL = DISPATCH_LEVEL*/
			VOID MtAsicRemoveSharedKeyEntry(IN PRTMP_ADAPTER pAd,
							IN UCHAR BssIndex,
							IN UCHAR KeyIdx)
			{
				/* TODO: shiang-7603 */
			...
			INT MtAsicSendCommandToMcu(IN RTMP_ADAPTER * pAd,
						   IN UCHAR Command,
						   IN UCHAR Token,
						   IN UCHAR Arg0, IN UCHAR Arg1,
			...
			BOOLEAN MtAsicSendCmdToMcuAndWait(
				IN RTMP_ADAPTER * pAd, IN UCHAR Command,
				IN UCHAR Token, IN UCHAR Arg0, IN UCHAR Arg1,
				IN BOOLEAN in_atomic)
			{
				BOOLEAN cmd_done = TRUE;

				MtAsicSendCommandToMcu(pAd, Command, Token,
						       Arg0, Arg1, in_atomic);
				return cmd_done;
			}
			...
			VOID MtAsicTurnOffRFClk(RTMP_ADAPTER * pAd,
						UCHAR Channel)
			{
				MTWF_LOG(
					DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
 ...
 * ========================================================================
 */
			VOID MtAsicSetStreamMode(IN RTMP_ADAPTER * pAd,
						 IN PUCHAR pMacAddr,
						 IN INT chainIdx,
						 IN BOOLEAN bEnabled)
			{
			...
			VOID MtAsicSetTxPreamble(RTMP_ADAPTER * pAd,
						 USHORT TxPreamble)
			{
...
#ifdef DOT11_N_SUPPORT
			INT MtAsicReadAggCnt(RTMP_ADAPTER * pAd, ULONG * aggCnt,
					     int cnt_len)
			{
			...
			INT MtAsicSetRalinkBurstMode(RTMP_ADAPTER * pAd,
						     BOOLEAN enable)
			{
				/* TODO: shiang-7603 */
				MTWF_LOG(
 ...
 */
			...
			INT MtAsicSetWPDMA(RTMP_ADAPTER * pAd, INT32 TxRx,
					   BOOLEAN enable, UINT8 WPDMABurstSIZE)
			{
			...
			BOOLEAN MtAsicWaitPDMAIdle(struct _RTMP_ADAPTER * pAd,
						   INT round, INT wait_us)
			{
				struct _RTMP_CHIP_OP *ops =
					hc_get_chip_ops(pAd->hdev_ctrl);

				if (ops->hif_wait_dma_idle)
					return ops->hif_wait_dma_idle(
						pAd, round, wait_us);
 ...
 */
			...
			VOID MtAsicSwitchChannel(RTMP_ADAPTER * pAd,
 ...
 */
			...
			UINT32 MtAsicGetRxStat(RTMP_ADAPTER * pAd, UINT type)
			{
				...
				case HQA_RX_STAT_PHY_MDRDYCNT:
					/* [31:16] OFDM [15:0] CCK */
					PHY_IO_READ32(pAd, RO_PHYCTRL_STS5,
						      &value);
					break;

				case HQA_RX_STAT_PHY_FCSERRCNT:
					/* [31:16] OFDM [15:0] CCK */
					PHY_IO_READ32(pAd, RO_PHYCTRL_STS4,
						      &value);
					break;

				case HQA_RX_STAT_PD:
					/* [31:16] OFDM [15:0] CCK */
					PHY_IO_READ32(pAd, RO_PHYCTRL_STS0,
						      &value);
					break;

				case HQA_RX_STAT_CCK_SIG_SFD:
					/* [31:16] SIG [15:0] SFD */
					PHY_IO_READ32(pAd, RO_PHYCTRL_STS1,
						      &value);
					break;

				case HQA_RX_STAT_OFDM_SIG_TAG:
					/* [31:16] SIG [15:0] TAG */
					PHY_IO_READ32(pAd, RO_PHYCTRL_STS2,
						      &value);
					break;
				...
				case HQA_RX_RESET_PHY_COUNT:
					if (IS_MT76x6(pAd) || IS_MT7637(pAd)) {
						PHY_IO_READ32(pAd, CR_PHYMUX_5,
							      &value);
						value &= 0xFF0FFFFF;
						PHY_IO_WRITE32(pAd, CR_PHYMUX_5,
							       value);
						MTWF_LOG(DBG_CAT_HW,
							 DBG_SUBCAT_ALL,
							 DBG_LVL_TRACE,
						...
						PHY_IO_READ32(pAd, CR_PHYCTRL_2,
							      &value);
						value |=
							(1
							 << 6); /* BIT6: CR_STSCNT_RST */
						PHY_IO_WRITE32(pAd,
							       CR_PHYCTRL_2,
							       value);
						value &= (~(1 << 6));
			...
			INT MtAsicSetTxTonePower(RTMP_ADAPTER * pAd, INT dec0,
						 INT dec1)
			{
				INT Ret = TRUE;
				MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL,
				...
				if (MTK_REV_GTE(pAd, MT7603, MT7603E2)) {
					...
					MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL,
						 DBG_LVL_TRACE,
						 ("%s RF 1db SetValue = 0x%x\n",
						  __func__, SetValue));
#if !defined(COMPOS_WIN) &&                                                    \
	!defined(COMPOS_TESTMODE_WIN) /* 1       todo fw Cmd */
					MtCmdRFRegAccessWrite(
						pAd, 0, 0x70,
						SetValue); /* bit 26(0x04000000) is enable */
					MtCmdRFRegAccessWrite(pAd, 1, 0x70,
							      SetValue);
#endif
					...
					SetValue =
						0x40000000 | (Tempdec1 << 20);
					MTWF_LOG(
						DBG_CAT_HW, DBG_SUBCAT_ALL,
						DBG_LVL_TRACE,
						("%s DC 0.25db SetValue = 0x%x\n",
						 __func__, SetValue));
					HW_IO_WRITE32(
						pAd, CR_TSSI_9,
						SetValue); /* 0x10D24, bit 30(0x40000000) isenable */
					HW_IO_WRITE32(
						pAd, CR_WF1_TSSI_9,
						SetValue); /* 0x11D24, bit 30(0x40000000) isenable */
				} else {
				}
			...
			INT MtAsicSetRfFreqOffset(RTMP_ADAPTER * pAd,
						  UINT32 FreqOffset)
			{
				UINT32 Value = 0;
				INT Ret = TRUE;
			...
			INT MtAsicSetTSSI(RTMP_ADAPTER * pAd, UINT32 bOnOff,
					  UCHAR WFSelect)
			{
				UINT32 CRValue = 0x0;
				UINT32 WF0Offset =
					0x10D04; /* WF_PHY_CR_FRONT CR_WF0_TSSI_1 */
				UINT32 WF1Offset =
					0x11D04; /* WF_PHY_CR_FRONT CR_WF1_TSSI_1 */
				INT Ret = TRUE;
			...
			INT MtAsicSetDPD(RTMP_ADAPTER * pAd, UINT32 bOnOff,
					 UCHAR WFSelect)
			{
				UINT32 CRValue = 0x0;
				...
				if (bOnOff == FALSE) {
					...
					if ((WFSelect == 0) ||
					    (WFSelect == 2)) {
						PHY_IO_READ32(pAd, WF0Offset,
							      &CRValue);
						CRValue |= 0xF0000000;
						PHY_IO_WRITE32(pAd, WF0Offset,
							       CRValue);
					}
					...
					if ((WFSelect == 1) ||
					    (WFSelect == 2)) {
						PHY_IO_READ32(pAd, WF1Offset,
							      &CRValue);
						CRValue |= 0xF0000000;
						PHY_IO_WRITE32(pAd, WF1Offset,
							       CRValue);
					}
			...
			INT MtAsicSetTxToneTest(RTMP_ADAPTER * pAd,
						UINT32 bOnOff, UCHAR Type)
			{
				MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL,
					 DBG_LVL_TRACE,
					 ("%s, bOnOff:%d Type:%d\n", __func__,
				...
				if (bOnOff == 0) { /* 0 = off 1 = on */
					...
					if (Type == WF0_TX_ONE_TONE_5M ||
					    Type == WF0_TX_TWO_TONE_5M ||
					    Type == WF0_TX_ONE_TONE_10M ||
					    Type == WF0_TX_ONE_TONE_DC) {
						/* 1. clock setup */
						PHY_IO_WRITE32(pAd,
							       CR_PHYCK_CTRL,
							       0x00000021);
						PHY_IO_WRITE32(pAd,
							       ...
							       0x00000021);
						/* 2. TX setup */
						PHY_IO_WRITE32(pAd, CR_TSSI_0,
							       0x00274027);
						PHY_IO_WRITE32(pAd, CR_TSSI_1,
							       0xC0000400);
						PHY_IO_WRITE32(pAd, CR_PHYMUX_3,
							       0x80000008);
						PHY_IO_WRITE32(pAd, CR_PHYMUX_5,
							       0x00000597);
						PHY_IO_WRITE32(pAd, CR_TXFD_1,
							       0x10000000);
						PHY_IO_WRITE32(pAd, CR_TSSI_9,
							       0x60000000);
						PHY_IO_WRITE32(pAd, CR_TXFE_3,
							       0xF0000000);
						...
						if (Type ==
							...
							MTWF_LOG(
								...
								("%s Type = %d error!!!\n",
								 __func__,
								 Type));
					} else if (Type == WF1_TX_ONE_TONE_5M ||
						   Type == WF1_TX_TWO_TONE_5M ||
						   Type == WF1_TX_ONE_TONE_10M ||
						   Type == WF1_TX_ONE_TONE_DC) {
						/* 1. clock setup */
						PHY_IO_WRITE32(pAd,
							       CR_PHYCK_CTRL,
							       0x00000021);
			...
			INT MtAsicStopContinousTx(RTMP_ADAPTER * pAd)
			{
				...
				PHY_IO_WRITE32(pAd, CR_TXFD_1,
					       0x00000000); /* 0x14704 */
				PHY_IO_WRITE32(pAd, CR_PHYCK_CTRL,
					       0x00000045); /* 0x10000 */
				PHY_IO_WRITE32(pAd, CR_FR_CKG_CTRL,
					       0x00000045); /* 0x10004 */
				PHY_IO_WRITE32(pAd, CR_FFT_MANU_CTRL,
					       0x00000000); /* 0x10704 */
				PHY_IO_WRITE32(pAd, CR_PHYCTRL_0,
					       0x00000000); /* 0x14100 */
				PHY_IO_WRITE32(pAd, CR_PHYCTRL_DBGCTRL,
					       0x00000000); /* 0x14140 */
				PHY_IO_WRITE32(pAd, CR_PHYMUX_3,
					       0x7C900408); /* 0x1420C */
				PHY_IO_WRITE32(pAd, CR_PHYMUX_5,
					       0x00000580); /* 0x14214 */
				PHY_IO_WRITE32(pAd, CR_PHYMUX_WF1_5,
					       0x00000000); /* 0x15214 */
				PHY_IO_WRITE32(pAd, CR_PHYMUX_10,
					       0x00000000); /* 0x14228 */
				PHY_IO_WRITE32(pAd, CR_PHYMUX_11,
					       0x00000000); /* 0x1422C */
				PHY_IO_WRITE32(pAd, CR_TXFE_4,
					       0x00000000); /* 0x10A0C */
				PHY_IO_WRITE32(pAd, CR_TXFE1_4,
					       0x00000000); /* 0x11A0C */
				PHY_IO_WRITE32(pAd, CR_TXFD_0,
					       0x00000000); /* 0x14700 */
				PHY_IO_WRITE32(pAd, CR_TXFD_3,
					       0x00000000); /* 0x1470C */
				MAC_IO_WRITE32(pAd, TMAC_PCTSR,
					       0x00000000); /* 0x21708 */
				PHY_IO_WRITE32(pAd, CR_FR_RST_CTRL,
					       0xFFFFFFFF); /* 0x10008 */
				PHY_IO_WRITE32(pAd, CR_BK_RST_CTRL,
					       0xFFFFFFFF); /* 0x14004 */
				ATEOp->RfRegRead(pAd, ATECtrl->TxAntennaSel,
						 0x0D4, &val);
				val &= 0x7fffffff;
				ATEOp->RfRegWrite(pAd, ATECtrl->TxAntennaSel,
						  0x0D4, val);
				ATEOp->RfRegRead(pAd, ATECtrl->TxAntennaSel,
						 0x0D4, &val);
				val &= 0x00000fff;
				ATEOp->RfRegWrite(pAd, ATECtrl->TxAntennaSel,
						  0x0D8, val);
				/* Restore TSSI CR */
				MtTestModeRestoreCr(pAd,
						    CR_TSSI_0); /* 0x0D00 */
				MtTestModeRestoreCr(pAd,
						    CR_WF1_TSSI_0); /* 0x1D00 */
				MtTestModeRestoreCr(pAd,
						    CR_TSSI_1); /* 0x0D04 */
				MtTestModeRestoreCr(pAd,
						    CR_WF1_TSSI_1); /* 0x1D04 */
				MtTestModeRestoreCr(pAd,
						    CR_PHYMUX_27); /* 0x1426C */
				...
				PHY_IO_WRITE32(pAd, CR_RFINTF_01,
					       0x00000000); /* 0x0204 */
				PHY_IO_WRITE32(pAd, CR_WF1_RFINTF_01,
					       0x00000000); /* 0x1204 */
				/* PHY_IO_WRITE32(pAd, CR_PHYMUX_27, ATECtrl->phy_mux_27); //0x1426C */
				return 0;
			}

			INT MtAsicStartContinousTx(RTMP_ADAPTER * pAd,
						   UINT32 PhyMode, UINT32 BW,
						   UINT32 PriCh, UINT32 Mcs,
						   UINT32 WFSel)
			{
				struct _ATE_CTRL *ATECtrl = &(pAd->ATECtrl);
				struct _ATE_OPERATION *ATEOp = ATECtrl->ATEOp;
				...
				/* Get Target Power*/
				PHY_IO_READ32(pAd, CR_TSSI_6,
					      &reg); /* 0x0D18 */
				PHY_IO_READ32(pAd, CR_TSSI_13,
					      &reg1); /* 0x1D18 */
				MTWF_LOG(
					DBG_CAT_ALL, DBG_SUBCAT_ALL,
					DBG_LVL_TRACE,
					("%s(): 0x0D18: 0x%x, tx_pwr(0x0D34):0x%x\n",
					 __func__, reg, reg1));
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL,
					 DBG_LVL_TRACE,
					 ("%s(): finish Tx TSSI Train\n",
					  __func__));
				ATEOp->RfRegRead(
					pAd, WFSel, 0x410,
					&rf_pa); /* wf?_pa_gc = 0x410[18:16], wf?_pa_bias = 0x410[15:0];*/
				...
				case 2:
					PHY_IO_READ32(pAd, CR_WF1_TSSI_13,
						      &tx_pwr);
					break;
				...
				PHY_IO_READ32(pAd, CR_PHYMUX_24,
					      &reg); /* 0x4260 */
				gc_thresh = (INT32)reg;
				pa_mode = (gc_thresh >> 24) &
					  0x0000007f; /* Half PA[30:24] */
				pa_mode = (pa_mode & 0x40) ? (pa_mode - 128) :
								   pa_mode;
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL,
					 DBG_LVL_TRACE,
					 ("pwr_dbm:%d, pa_mode:%d\n", pwr_dbm,
				...
				MTWF_LOG(
					DBG_CAT_HW, DBG_SUBCAT_ALL,
					DBG_LVL_TRACE,
					("%s(): Modulation = %d, BW=%d, pri_ch = %d, rate = %d, WFSelect = %d-->\n",
				...
				MtTestModeBkCr(pAd, CR_WF1_TSSI_0,
					       TEST_PHY_BKCR);
				MtTestModeBkCr(pAd, CR_TSSI_1, TEST_PHY_BKCR);
				MtTestModeBkCr(pAd, CR_WF1_TSSI_1,
					       TEST_PHY_BKCR);
				/* Contiuous Tx power patch */
				PHY_IO_WRITE32(pAd, CR_TSSI_0,
					       0x024041C0); /* 0x0D00 */
				PHY_IO_WRITE32(pAd, CR_WF1_TSSI_0,
					       0x024041C0); /* 0x1D00 */
				PHY_IO_WRITE32(pAd, CR_TSSI_1,
					       0x23F3F800); /* 0x0D04 */
				PHY_IO_WRITE32(pAd, CR_WF1_TSSI_1,
					       0x23F3F800); /* 0x1D04 */
				PHY_IO_WRITE32(pAd, CR_TSSI_6,
					       reg1); /* 0x0D18 */
				PHY_IO_WRITE32(pAd, CR_WF1_TSSI_6,
					       reg2); /* 0x1D18 */
				PHY_IO_WRITE32(pAd, CR_FFT_MANU_CTRL,
					       0x0AA00000); /* 0x10704 */
				...
				if (PhyMode == 0) { /* MODULATION_SYSTEM_CCK */
					...
					PHY_IO_WRITE32(pAd, CR_TXFD_3, value);
					   ...
					   1) { /* MODULATION_SYSTEM_OFDM */
					PHY_IO_READ32(pAd, CR_PHYMUX_26,
						      &cr_hilo); /* 0x4268 */
					value = 0;
					...
					PHY_IO_WRITE32(pAd, CR_TXFD_3,
						       value); /* 0x1470C */
					MtTestModeBkCr(pAd, CR_PHYMUX_27,
						       TEST_PHY_BKCR);
					PHY_IO_READ32(pAd, CR_PHYMUX_27,
						      &value); /* 0x1426C */
				...
				if (PhyMode == 0) /* PREAMBLE_CCK */
					...
					PHY_IO_WRITE32(
					 ...
					 3 == PhyMode) /* PREAMBLE_GREEN_FIELD */
					PHY_IO_WRITE32(
						pAd, CR_TXFD_1,
						0x320000F2); /* 0x14704 */
				else
 ...
 */
			INT MTAsicTxSetFrequencyOffset(RTMP_ADAPTER * pAd,
						       UINT32 iOffset,
						       BOOLEAN HasBeenSet)
			{
				UINT32 Value = 0;
				...
				if (HasBeenSet == FALSE) {
					if (IS_MT7603(pAd)) {
						...
						MAC_IO_READ32(pAd, RG_XO_C2,
							      &Value);
						Value = (Value & 0xFFFF80FF) |
							(0x3C << 8);
						MAC_IO_WRITE32(pAd, RG_XO_C2,
							       Value);
 ...
 * Otherwise, an error code is returned.
 */
			INT MTAsicTxConfigPowerRange(RTMP_ADAPTER * pAd,
						     IN UCHAR ucMaxPowerDbm,
						     IN UCHAR ucMinPowerDbm)
			{
				UINT32 u4RegValue;
				...
				u4RegValue &= ~(FPCR_FRAME_POWER_MAX_DBM |
						FPCR_FRAME_POWER_MIN_DBM);
				u4RegValue |=
					((ucMaxPowerDbm
					  << FPCR_FRAME_POWER_MAX_DBM_OFFSET) &
					 FPCR_FRAME_POWER_MAX_DBM);
				u4RegValue |=
					((ucMinPowerDbm
					  << FPCR_FRAME_POWER_MIN_DBM_OFFSET) &
					 FPCR_FRAME_POWER_MIN_DBM);
 ...
 */
			...
			INT32 MtAsicDMASchedulerInit(
				RTMP_ADAPTER * pAd, MT_DMASCH_CTRL_T DmaSchCtrl)
			{
				UINT32 mac_val;
...
#ifdef RTMP_MAC_PCI
				HW_IO_READ32(pAd, MCU_PCIE_REMAP_2,
					     &mac_restore_val);
				HW_IO_WRITE32(pAd, MCU_PCIE_REMAP_2,
					      MT_PSE_BASE_ADDR);
				...
				if (dma_mode == DMA_SCH_BYPASS) {
					MAC_IO_WRITE32(pAd, MT_SCH_REG_4,
						       1 << 5);
		  ...
		  */
				...
				if ((dma_mode == DMA_SCH_LMAC) ||
				    (dma_mode == DMA_SCH_HYBRID)) {
#ifdef CONFIG_AP_SUPPORT
					UINT32 max_beacon_page_count =
						MAX_BEACON_SIZE / page_size;
#endif
					UINT32 max_bmcast_page_count =
						MAX_BMCAST_SIZE / page_size;
					UINT32 max_mcucmd_page_count =
						MAX_MCUCMD_SIZE / page_size;
					UINT32 max_data_page_count =
						MAX_DATA_SIZE / page_size;
					...
					mac_val = 0x55555555;
					MAC_IO_WRITE32(pAd, MT_HIGH_PRIORITY_1,
						       mac_val);
					mac_val = 0x78555555;
					MAC_IO_WRITE32(pAd, MT_HIGH_PRIORITY_2,
						       mac_val);
					/* Queue Priority */
					mac_val = 0x2b1a096e;
					MAC_IO_WRITE32(pAd, MT_QUEUE_PRIORITY_1,
						       mac_val);
					mac_val = 0x785f4d3c;
					MAC_IO_WRITE32(pAd, MT_QUEUE_PRIORITY_2,
						       mac_val);
					MAC_IO_WRITE32(pAd, MT_PRIORITY_MASK,
						       0xffffffff);
					/* Schedule Priority, page size/FFA, FFA = (page_cnt * page_size) */
					mac_val = (2 << 28) | (page_cnt);
					MAC_IO_WRITE32(pAd, MT_SCH_REG_1,
						       mac_val);
					mac_val = MAX_DATA_SIZE / page_size;
					MAC_IO_WRITE32(pAd, MT_SCH_REG_2,
						       mac_val);
					/* Resvervation page */
#ifdef DOT11_N_SUPPORT

					if (pAd->CommonCfg.BACapability.field
						    .AmsduEnable == TRUE)
						max_data_page_count =
							MAX_AMSDU_DATA_SIZE /
							page_size;
...
#endif
					MAC_IO_WRITE32(pAd, MT_PAGE_CNT_0,
						       max_data_page_count);
					MAC_IO_WRITE32(pAd, MT_PAGE_CNT_1,
						       max_data_page_count);
					MAC_IO_WRITE32(pAd, MT_PAGE_CNT_2,
						       max_data_page_count);
					MAC_IO_WRITE32(pAd, MT_PAGE_CNT_3,
						       max_data_page_count);
					MAC_IO_WRITE32(pAd, MT_PAGE_CNT_4,
						       max_data_page_count);
					total_restore_val =
						max_data_page_count * 5;
					mcu_restore_val = max_mcucmd_page_count;
					MAC_IO_WRITE32(pAd, MT_PAGE_CNT_5,
						       mcu_restore_val);
					...
					MAC_IO_WRITE32(pAd, MT_PAGE_CNT_7,
						       bcn_restore_val);
#endif /* CONFIG_AP_SUPPORT */
					total_restore_val += bcn_restore_val;
					mbc_restore_val =
						max_bmcast_page_count *
							MAX_BMCAST_COUNT +
						max_data_page_count;
					MAC_IO_WRITE32(pAd, MT_PAGE_CNT_8,
						       mbc_restore_val);
					total_restore_val += mbc_restore_val;
					/* FFA1 max threshold */
					MAC_IO_WRITE32(
						pAd, MT_RSV_MAX_THD,
						(page_cnt - total_restore_val));
					/* Group Threshold */
					MAC_IO_WRITE32(pAd, MT_GROUP_THD_0,
						       page_cnt);
			 ...
			 */
						...
						HW_IO_WRITE32(pAd,
							      MCU_PCIE_REMAP_2,
							      0x80080000);
						HW_IO_READ32(pAd, 0xc006c,
							     &mac_val);
						mac_val &= (~(7 << 12));
						HW_IO_WRITE32(pAd, 0xc006c,
							      mac_val);
						/* After change the Tx Padding CR of PCI-E Client, we need to re-map for PSE region */
						HW_IO_WRITE32(pAd,
							      MCU_PCIE_REMAP_2,
...
#endif /* MT7603_FPGA */
						/* Disable TxD padding */
						MAC_IO_READ32(pAd, DMA_DCR1,
							      &mac_val);
						mac_val &= (~(0x7 << 8));
						MAC_IO_WRITE32(pAd, DMA_DCR1,
							       mac_val);
					}
				}
				...
				HW_IO_READ32(pAd, MCU_PCIE_REMAP_2,
					     &mac_restore_val);
				RemapBase =
					GET_REMAP_2_BASE(MT_FPGA_PSE_CLIENT_CNT)
					<< 19;
				RemapOffset = GET_REMAP_2_OFFSET(
					MT_FPGA_PSE_CLIENT_CNT);
				HW_IO_WRITE32(pAd, MCU_PCIE_REMAP_2, RemapBase);
				HW_IO_READ32(pAd, 0x80000 + RemapOffset,
					     &mac_val);
				HW_IO_WRITE32(pAd, MCU_PCIE_REMAP_2,
					      mac_restore_val);
#ifdef MT7636_FPGA
				...
				HW_IO_READ32(pAd, MCU_PCIE_REMAP_2,
					     &mac_restore_val);
				RemapBase = GET_REMAP_2_BASE(MT_FPGA_PSE_SET_0)
					    << 19;
				RemapOffset =
					GET_REMAP_2_OFFSET(MT_FPGA_PSE_SET_0);
				HW_IO_WRITE32(pAd, MCU_PCIE_REMAP_2, RemapBase);
				HW_IO_WRITE32(pAd, 0x80000 + RemapOffset,
					      0xffffffff);
				HW_IO_WRITE32(pAd, MCU_PCIE_REMAP_2,
					      mac_restore_val);
				...
				HW_IO_READ32(pAd, MCU_PCIE_REMAP_2,
					     &mac_restore_val);
				RemapBase = GET_REMAP_2_BASE(MT_FPGA_PSE_SET_1)
					    << 19;
				RemapOffset =
					GET_REMAP_2_OFFSET(MT_FPGA_PSE_SET_1);
				HW_IO_WRITE32(pAd, MCU_PCIE_REMAP_2, RemapBase);
				HW_IO_WRITE32(pAd, 0x80000 + RemapOffset,
					      0xffffffff);
				HW_IO_WRITE32(pAd, MCU_PCIE_REMAP_2,
			...
			INT32 MtAsicSetDmaByPassMode(RTMP_ADAPTER * pAd,
						     BOOLEAN isByPass)
			{
#ifdef DMA_SCH_SUPPORT
				UINT32 value;
 ...
 */
			VOID MtAsicInitMac(RTMP_ADAPTER * pAd)
			{
				...
				mac_val |=
					BIT13; /* TMR report send to HIF q1. */
				...
				mac_val |=
					0xc0200000; /* receive BA/CF_End/Ack/RTS/CTS/CTRL_RSVED */

				if (pAd->rx_pspoll_filter)
					mac_val |=
						0x00000008; /* Non-BAR Control frame to MCU */
				...
				MtAsicSetRxGroup(pAd, HIF_PORT,
						 RXS_GROUP1 | RXS_GROUP2 |
							 RXS_GROUP3,
						 TRUE);
				MtAsicSetRxGroup(pAd, MCU_PORT,
						 RXS_GROUP1 | RXS_GROUP2 |
							 RXS_GROUP3,
						 TRUE);
#else
				MtAsicSetRxGroup(pAd, HIF_PORT, RXS_GROUP3,
						 TRUE);
				MtAsicSetRxGroup(pAd, MCU_PORT, RXS_GROUP3,
						 TRUE);
#endif /* !COMPOS_WIN */
				/* AMPDU BAR setting */
...
/* Set RTS Retry from profile */
...
#endif
#if !defined(COMPOS_WIN) && !defined(COMPOS_TESTMODE_WIN)
				RTMP_UPDATE_RTS_THRESHOLD(
					pAd, pAd->CommonCfg.RtsPktThreshold,
					pAd->CommonCfg.RtsThreshold);
#endif
#endif /* MAC_INIT_OFFLOAD */
			}

			INT32 MtAsicGetMacInfo(RTMP_ADAPTER * pAd,
					       UINT32 * ChipId, UINT32 * HwVer,
					       UINT32 * FwVer)
			{
				UINT32 Value;
			...
			INT32 MtAsicGetAntMode(RTMP_ADAPTER * pAd,
					       UCHAR * AntMode)
			{
				UINT32 Value;
			...
			BOOLEAN MtAsicGetMcuStatus(RTMP_ADAPTER * pAd,
						   MCU_STAT State)
			{
				UINT32 value;

				HW_IO_READ32(pAd, TOP_MISC2, &value);
				MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL,
					 DBG_LVL_INFO,
					 ("TOP_MSIC = %x\n", value));
...
#define SW_SYN0 0x81021250
				...
				if (IS_PCI_INF(pAd)) {
					...
					RTMP_IO_READ32(pAd, MCU_PCIE_REMAP_2,
						       &RestoreValue);
					RemapBase = GET_REMAP_2_BASE(SW_SYN0)
						    << 19;
					RemapOffset =
						GET_REMAP_2_OFFSET(SW_SYN0);
...
#endif
#if defined(RTMP_RBUS_SUPPORT) || defined(RTMP_USB_SUPPORT) ||                 \
	defined(RTMP_SDIO_SUPPORT)

				if (IS_RBUS_INF(pAd) || IS_USB_INF(pAd) ||
				    IS_SDIO_INF(pAd)) {
			...
			VOID MtAsicSetSMPSByDriver(RTMP_ADAPTER * pAd,
						   UCHAR Wcid, UCHAR Smps)
			{
				struct wtbl_entry tb_entry;
				struct wtbl_1_struc wtbl_1;

				os_zero_mem((UCHAR *)&wtbl_1,
					    sizeof(struct wtbl_1_struc));
				os_zero_mem(&tb_entry, sizeof(tb_entry));

				if (mt_wtbl_get_entry234(pAd, Wcid,
							 &tb_entry) == FALSE) {
					MTWF_LOG(
						DBG_CAT_HW, DBG_SUBCAT_ALL,
						DBG_LVL_ERROR,
			...
			VOID MtAsicSetSMPS(RTMP_ADAPTER * pAd, UCHAR Wcid,
					   UCHAR Smps)
			{
			...
			INT32 MtAsicSetDevMacByDriver(
				RTMP_ADAPTER * pAd, UINT8 OwnMacIdx,
				...
				BSS_INFO_ARGUMENT_T bss_info_argument)
			{
				...
				if (OwnMacIdx < HW_BSSID_MAX) {
					...
					BssIdx = (OwnMacIdx &
						  ~cap->ExtMbssOmacStartIdx);
					MtSmacSetExtMbssEnableCR(
						pAd, BssIdx,
						Active); /* enable rmac 0_1~0_15 bit */
					MtSmacSetMbssHwCRSetting(
						pAd, BssIdx,
						Active); /* enable lp timing setting for 0_1~0_15 */
				}
			...
			INT32 MtAsicSetStaRecByDriver(RTMP_ADAPTER * pAd,
			...
			BOOLEAN MtSmacAsicSetDmaFqcr(
				RTMP_ADAPTER * pAd, UINT8 OmacIdx,
				UINT32 filter_mode, /* 0: use widx, 1: use OMAC */
				UCHAR dest_port, UCHAR dest_queue,
				UCHAR target_queue,
			...
			)
			{
				...
				val = (DMA_FQCR0_FQ_EN | filter_mode |
				...
				if (filter_mode == DMA_FQCR0_FQ_MODE_OMAC) {
					own_mac = OmacIdx;
					val = val |
					      DMA_FQCR0_FQ_TARG_OM(own_mac);
				} else if (filter_mode ==
					   DMA_FQCR0_FQ_MODE_WIDX)
					val = val |
					      DMA_FQCR0_FQ_TARG_WIDX(widx);
			...
			BOOLEAN MtSmacAsicSetArbBcnQCR(RTMP_ADAPTER * pAd,
						       UINT8 OmacIdx,
						       BOOLEAN enable)
			{
				UINT32 val = 0, temp = 0, own_mac = 0,
			...
			BOOLEAN MtSmacAsicEnableBeacon(RTMP_ADAPTER * pAd,
						       VOID * wdev_void)
			{
				struct wifi_dev *wdev =
					(struct wifi_dev *)wdev_void;
				BOOLEAN OperationResult = TRUE;
				BCN_BUF_STRUC *bcn_info = &wdev->bcn_buf;

				if (bcn_info->BcnUpdateMethod ==
				    BCN_GEN_BY_FW) {
					/* FW help to disable beacon. */
				} else if (bcn_info->BcnUpdateMethod ==
					   BCN_GEN_BY_HOST_IN_PRETBTT) {
					OperationResult =
						MtSmacAsicSetArbBcnQCR(
							pAd, wdev->OmacIdx,
							TRUE);
			...
			BOOLEAN MtSmacAsicDisableBeacon(RTMP_ADAPTER * pAd,
							VOID * wdev_void)
			{
				struct wifi_dev *wdev =
					(struct wifi_dev *)wdev_void;
			...
			UINT32 MtAsicGetChBusyCnt(RTMP_ADAPTER * pAd,
						  UCHAR ch_idx)
			{
				UINT32 msdr16;
			...
			INT MtAsicGetTsfTimeByDriver(struct _RTMP_ADAPTER * pAd,
						     UINT32 * high_part,
						     UINT32 * low_part,
						     UCHAR HwBssidIdx)
			{
				...
				Value = (Value & TSF_TIMER_HW_MODE_MASK) |
					TSF_TIMER_VALUE_READ; /* keep HW mode value. */
 ...
 * ==========================================================================
 */
			VOID MtAsicSetSlotTime(RTMP_ADAPTER * pAd,
					       UINT32 SlotTime, UINT32 SifsTime,
					       UCHAR BandIdx)
			{
				UINT32 RifsTime = RIFS_TIME;
				UINT32 EifsTime = EIFS_TIME;

				MtCmdSlotTimeSet(pAd, (UINT8)SlotTime,
						 (UINT8)SifsTime,
						 (UINT8)RifsTime,
						 (UINT16)EifsTime, BandIdx);
			}

#if defined(COMPOS_WIN) || defined(COMPOS_TESTMODE_WIN)
			VOID MtAsicGetTxTscByDriver(RTMP_ADAPTER * pAd,
						    struct wifi_dev * wdev,
						    UCHAR * pTxTsc)
			{
					    ...
					    UCHAR * pTxTsc)
		{
			...
			if (mt_wtbl_get_entry234(pAd, Wcid, &tb_entry) ==
			    FALSE) {
				MTWF_LOG(
					DBG_CAT_HW, DBG_SUBCAT_ALL,
					DBG_LVL_ERROR,
			...
			addr = pAd->mac_ctrl.wtbl_base_addr[1] +
			       Wcid * pAd->mac_ctrl.wtbl_entry_size[1];
			...
			*(pTxTsc + 5) = (val >> 8) & 0xff;
			MTWF_LOG(
				...
				("%s(): WCID(%d) TxTsc 0x%02x-0x%02x-0x%02x-0x%02x-0x%02x-0x%02x\n",
				 __func__, Wcid, *pTxTsc, *(pTxTsc + 1),
				 *(pTxTsc + 2), *(pTxTsc + 3), *(pTxTsc + 4),
				 *(pTxTsc + 5)));
		}
#endif
	...
	defined(MT7637_FPGA)
...
#endif /* MT7628_FPGA */
				MAC_IO_WRITE32(pAd, 0x24088,
					       0x900); /* Set 40MHz Clock */
				MAC_IO_WRITE32(
					pAd, 0x2d034,
					0x64180003); /* Set 32k clock, this clock is used for lower power. */
#endif /* defined(MT7603_FPGA) || defined(MT7628_FPGA) || defined(MT7636_FPGA) || defined(MT7637_FPGA)*/
				return TRUE;
			}

#ifdef CONFIG_AP_SUPPORT
			VOID MtAsicSetMbssLPOffset(struct _RTMP_ADAPTER * pAd,
						   UCHAR mbss_idx,
						   BOOLEAN enable)
			{
				UINT32 Value = 0;
				...
				if (enable == TRUE) {
					...
					MAC_IO_READ32(pAd, bssid_reg_base,
						      &Value);
					Value |= SBSS_TBTT0_TSF0_EN;
					MAC_IO_WRITE32(pAd, bssid_reg_base,
						       Value);
					MAC_IO_READ32(pAd,
						      (bssid_reg_base +
						       (mbss_idx - 1) * (0x4)),
					...
					Value |= SUB_BSSID0_TIME_OFFSET_n(
						mbss_idx *
						BCN_TRANSMIT_ESTIMATE_TIME);
					...
					MAC_IO_WRITE32(pAd,
							...
							(mbss_idx - 1) * (0x4)),
						       Value);
					pAd->ApCfg.ext_mbss_enable_bitmap |=
						(enable << mbss_idx);
				} else {
					pAd->ApCfg.ext_mbss_enable_bitmap &=
						~(enable << mbss_idx);
			...
			VOID MtSmacSetMbssHwCRSetting(RTMP_ADAPTER * pAd,
						      UCHAR mbss_idx,
						      BOOLEAN enable)
			{
			...
			VOID MtSmacSetExtMbssEnableCR(RTMP_ADAPTER * pAd,
						      UCHAR mbss_idx,
						      BOOLEAN enable)
			{
				UINT32 regValue;
				...
				if (enable) {
					MAC_IO_READ32(pAd, RMAC_ACBEN,
						      &regValue);
					regValue |= (1 << mbss_idx);
					MAC_IO_WRITE32(pAd, RMAC_ACBEN,
						       regValue);
				} else {
					MAC_IO_READ32(pAd, RMAC_ACBEN,
						      &regValue);
			...
			VOID MtAsicSetExtTTTTLPOffset(
				struct _RTMP_ADAPTER * pAd, UCHAR mbss_idx,
				BOOLEAN enable)
			{
				UINT32 Value = 0;
				/* where the register for sub Bssid start from */
				UINT32 bssid_reg_base =
					LPON_TT0SBOR_CR_MAPPING_TABLE[mbss_idx];
				UINT32 first_reg_base =
					LPON_TT0SBOR_CR_MAPPING_TABLE[1];
				...
				if (enable == TRUE) {
					/* if there is any sub bssid is enable. this bit in LPON_SBTOR1 shall be 1 always. */
					MAC_IO_READ32(pAd, first_reg_base,
						      &Value);
					Value |= SBSS_TTTT0_TSF0_EN;
					MAC_IO_WRITE32(pAd, first_reg_base,
						       Value);
					MAC_IO_READ32(pAd, bssid_reg_base,
						      &Value);
					...
					MAC_IO_WRITE32(pAd, bssid_reg_base,
						       Value);
					pAd->ApCfg.ext_mbss_tttt_enable_bitmap |=
						(enable << mbss_idx);
				} else {
					pAd->ApCfg.ext_mbss_tttt_enable_bitmap &=
						~(enable << mbss_idx);
					...
					if (pAd->ApCfg
						    .ext_mbss_tttt_enable_bitmap)
						any_other_mbss_tttt_enable =
							TRUE;
			...
			VOID MtSmacSetExtTTTTHwCRSetting(RTMP_ADAPTER * pAd,
							 UCHAR mbss_idx,
							 BOOLEAN enable)
			{
			...
			VOID MtSetTmrCal(IN PRTMP_ADAPTER pAd, IN UCHAR TmrType,
					 IN UCHAR Channel, IN UCHAR Bw)
			{
				...
				MtCmdTmrCal(pAd, TmrType,
					    (Channel > 14 ? _A_BAND : _G_BAND),
					    Bw, 0, /* Ant 0 at present */
					    TmrType);
			}

			VOID MtSetTmrCR(IN PRTMP_ADAPTER pAd, IN UCHAR TmrType)
			{
				UINT32 value = 0;
				...
				if (TmrType == TMR_DISABLE)
					...
					if (TmrType == TMR_RESPONDER) {
						...
						value &=
							0xffffffc0; /* clear type/sub_type field */
						value |= (SUBTYPE_ACTION << 2) |
							 FC_TYPE_MGMT;
					}
				}
			...
			INT MtSmacAsicArchOpsInit(RTMP_ADAPTER * pAd)
			{
				...
				arch_ops->archDisableBeacon =
					MtSmacAsicDisableBeacon;
				arch_ops->archEnableBeacon =
					MtSmacAsicEnableBeacon;
#ifdef CONFIG_AP_SUPPORT
				arch_ops->archSetMbssWdevIfAddr =
					MtAsicSetMbssWdevIfAddrGen1;
				arch_ops->archSetMbssHwCRSetting =
					MtSmacSetMbssHwCRSetting;
				arch_ops->archSetExtTTTTHwCRSetting =
					MtSmacSetExtTTTTHwCRSetting;
				arch_ops->archSetExtMbssEnableCR =
					MtSmacSetExtMbssEnableCR;
#endif /* CONFIG_AP_SUPPORT */
				arch_ops->archGetTsfTime =
					MtAsicGetTsfTimeByDriver;
				arch_ops->archSetPreTbtt = MtAsicSetPreTbtt;
#ifdef APCLI_SUPPORT
#ifdef MAC_REPEATER_SUPPORT
				arch_ops->archSetReptFuncEnable =
					MtAsicSetReptFuncEnableByDriver;
				arch_ops->archInsertRepeaterEntry =
					MtAsicInsertRepeaterEntry;
				arch_ops->archRemoveRepeaterEntry =
					MtAsicRemoveRepeaterEntry;
				arch_ops->archInsertRepeaterRootEntry =
					MtAsicInsertRepeaterRootEntry;
#endif /* MAC_REPEATER_SUPPORT */
#endif /* APCLI_SUPPORT */
				arch_ops->archDisableSync =
					MtAsicDisableSyncByDriver;
				arch_ops->archSetSyncModeAndEnable =
					MtAsicEnableBssSyncByDriver;
