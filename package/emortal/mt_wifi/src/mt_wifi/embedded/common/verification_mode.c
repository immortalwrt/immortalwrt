...
static VOID show_set_pkt_head_format(VOID)
{
	...
	MTWF_LOG(DBG_CAT_CFG,
		 DBG_SUBCAT_ALL,
		 DBG_LVL_OFF,
		 ("iwpriv ra0 set veri_pkt_head='type'-'subtype'-'addr1'-'addr2'-'addr3'\n"));
	MTWF_LOG(DBG_CAT_CFG,
		 DBG_SUBCAT_ALL,
		 DBG_LVL_OFF,
		 ("\nDescription:\n"));
	...
	MTWF_LOG(DBG_CAT_CFG,
		 DBG_SUBCAT_ALL,
		 DBG_LVL_OFF,
		 ("\tType: 1: CTRL, acceptable subtype: 8:BAR, PS-Poll:10, RTS:11\n"));
	MTWF_LOG(DBG_CAT_CFG,
		 DBG_SUBCAT_ALL,
		 DBG_LVL_OFF,
	...
	MTWF_LOG(DBG_CAT_CFG,
		 DBG_SUBCAT_ALL,
		 DBG_LVL_OFF,
		 ("\tType: 3: 802.3 format pacekt\n"));

	MTWF_LOG(DBG_CAT_CFG,
		 DBG_SUBCAT_ALL,
		 DBG_LVL_OFF,
		 ("\t\t- addr1 & addr2 is for a1 and a3 of 802.11 data and mgmt pkt\n"));
	MTWF_LOG(DBG_CAT_CFG,
		 DBG_SUBCAT_ALL,
		 DBG_LVL_OFF,
		 ("\t\t- also could be da & sa of 802.3 packet, ra & ta of CTRL pkt\n"));
	MTWF_LOG(DBG_CAT_CFG,
		 DBG_SUBCAT_ALL,
		 DBG_LVL_OFF,
		 ("\t\t- addr3 is a must for 802.11 data and mgmt packet\n"));
}
...
static VOID show_set_pkt_ctnt_format(VOID)
{
	...
	MTWF_LOG(DBG_CAT_CFG,
		 DBG_SUBCAT_ALL,
		 DBG_LVL_OFF,
		 ("iwpriv ra0 set veri_pkt_ctnt=len:'len'-ctnt:'ctnt'\n"));
	MTWF_LOG(DBG_CAT_CFG,
		 DBG_SUBCAT_ALL,
		 DBG_LVL_OFF,
		 ("\t- len: it will auto repeat the content to the length\n"));
	MTWF_LOG(DBG_CAT_CFG,
		 DBG_SUBCAT_ALL,
		 DBG_LVL_OFF,
		 ("\t\tif you don't assign length or it small than content, it will honor the len of ctnt\n"));
	MTWF_LOG(DBG_CAT_CFG,
		 DBG_SUBCAT_ALL,
		 DBG_LVL_OFF,
...
static VOID show_set_pkt_ctrl_en_format(VOID)
{
	MTWF_LOG(DBG_CAT_CFG,
		 DBG_SUBCAT_ALL,
		 DBG_LVL_OFF,
		 ("(Optional) Step 3: you can just type to enable some of them.\n"));
	MTWF_LOG(DBG_CAT_CFG,
		 DBG_SUBCAT_ALL,
		 DBG_LVL_OFF,
	...
	MTWF_LOG(DBG_CAT_CFG,
		 DBG_SUBCAT_ALL,
		 DBG_LVL_OFF,
		 ("\tiwpriv ra0 set veri_pkt_ctrl_en=du:'x'-na:'x'-tm:'x'"));
	MTWF_LOG(DBG_CAT_CFG,
		 DBG_SUBCAT_ALL,
		 DBG_LVL_OFF,
		 ("-sn:'x'-txs2m:'x'-txs2h:'x'-pm:'x'-life:'x'\n"));
	MTWF_LOG(DBG_CAT_CFG,
		 DBG_SUBCAT_ALL,
		 DBG_LVL_OFF,
		 ("\t\t- DU: 1:enable the duration is controlled by SW. need to assign dur in wifi hdr\n"));
	MTWF_LOG(DBG_CAT_CFG,
		 DBG_SUBCAT_ALL,
		 DBG_LVL_OFF,
		 ("\t\t- NA: 1:enable NO ACK packet in this packet\n"));
	MTWF_LOG(DBG_CAT_CFG,
		 DBG_SUBCAT_ALL,
		 DBG_LVL_OFF,
		 ("\t\t- TM: 1:enable timing measurement in this packet\n"));
	MTWF_LOG(DBG_CAT_CFG,
		 DBG_SUBCAT_ALL,
		 DBG_LVL_OFF,
		 ("\t\t- SN: 1:enable the seq is controlled by SW. need to assign seq in wifi hdr\n"));
	MTWF_LOG(DBG_CAT_CFG,
		 DBG_SUBCAT_ALL,
		 DBG_LVL_OFF,
		 ("\t\t- txs2m: 1:enable TXS2MCU in this packet. need to assign PacketID\n"));
	MTWF_LOG(DBG_CAT_CFG,
		 DBG_SUBCAT_ALL,
		 DBG_LVL_OFF,
		 ("\t\t- txs2h: 1:enable TXS2HOST in this packet. need to assign PacketID\n"));
	MTWF_LOG(DBG_CAT_CFG,
		 DBG_SUBCAT_ALL,
		 DBG_LVL_OFF,
	...
	MTWF_LOG(DBG_CAT_CFG,
		 DBG_SUBCAT_ALL,
		 DBG_LVL_OFF,
		 ("\t\t- life: 1:enable pkt lifetime is filled by SW. need to assign lifetime\n\n"));
}
...
static VOID show_set_pkt_ctrl_assign_format(VOID)
{
	...
	MTWF_LOG(DBG_CAT_CFG,
		 DBG_SUBCAT_ALL,
		 DBG_LVL_OFF,
		 ("\t\t- DU: the duration in wifi_hdr\n"));
	MTWF_LOG(DBG_CAT_CFG,
		 DBG_SUBCAT_ALL,
		 DBG_LVL_OFF,
		 ("\t\t- SN: the SEQ in wifi_hdr, it cannot exceed 4095\n"));
	MTWF_LOG(DBG_CAT_CFG,
		 DBG_SUBCAT_ALL,
		 DBG_LVL_OFF,
		 ("\t\t- PM: the PM in wifi_hdr, it follows the WIFI spec\n"));
	MTWF_LOG(DBG_CAT_CFG,
		 DBG_SUBCAT_ALL,
		 DBG_LVL_OFF,
		 ("\t\t- PID: the PID assigned for TXS, it cannot exceed 255\n\n"));
	MTWF_LOG(DBG_CAT_CFG,
		 DBG_SUBCAT_ALL,
		 DBG_LVL_OFF,
		 ("\t\t- life: the lifetime assigned, it cannot exceed 255\n\n"));
}
...
static VOID show_set_pkt_ctrl_send_format(VOID)
{
	...
	MTWF_LOG(DBG_CAT_CFG,
		 DBG_SUBCAT_ALL,
		 DBG_LVL_OFF,
		 ("\tiwpriv ra0 set send_veri_pkt=1\n"));
}
...
INT set_dump_rx_debug(struct _RTMP_ADAPTER *ad, char *arg)
{
	...
	MTWF_LOG(DBG_CAT_CFG,
		 DBG_SUBCAT_ALL,
		 DBG_LVL_OFF,
		 ("set verification mode rx dump = %d\n", ad->veri_ctrl.dump_rx_debug));
	return TRUE;
}
...
INT set_skip_ageout(struct _RTMP_ADAPTER *ad, char *arg)
{
	...
	MTWF_LOG(DBG_CAT_CFG,
		 DBG_SUBCAT_ALL,
		 DBG_LVL_OFF,
		 ("set verification mode skip_ageout = %d\n", ad->veri_ctrl.skip_ageout));
	return TRUE;
}
				  ...
				  UCHAR *buf)
{
	...
	default:
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			("%s: try to contruct wrong type:%d\n",
				__func__,
...
static BOOLEAN veri_pkt_ctnt_len_sanity_check(struct veri_ctrl *veri_ctrl, UCHAR tx_hw_hdr_len)
{
	...
	if ((veri_ctrl->veri_pkt_length + tx_hw_hdr_len) > MAX_LEN_OF_VERI_BUF) {
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			("%s: txd_size:%d pkt length:%d exceed:%d bytes\n",
				__func__,
				tx_hw_hdr_len,
...
INT send_veri_pkt(struct _RTMP_ADAPTER *ad, char *arg)
{
	...
	if (CHECK_VERI_PKT_STATE(VERI_PKT_UPDATE_CTNT)) {
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			("%s: state error:%d\n", __func__, veri_ctrl->veri_pkt_state));
		ret = NDIS_STATUS_FAILURE;
		goto done;
	}
	...
	if (ret != NDIS_STATUS_SUCCESS) {
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: AllocateNdisPacket fail\n", __func__));
		goto done;
	}
...
static BOOLEAN ctnt_length_sanity_check(char *arg, UINT32 ctnt_length, UINT32 padding_to_length)
{
	...
	if (ret == FALSE) {
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			("%s: ctnt format error: padding_to_length:%d, ctnt_length:%d, strlen(ctnt):%d\n",
				__func__, padding_to_length, ctnt_length, (int)strlen(arg)));
	}
...
INT prepare_veri_pkt_ctnt(struct _RTMP_ADAPTER *ad, UCHAR *padded_ctnt, UINT32 ctnt_length)
{
	...
	if (CHECK_VERI_PKT_STATE(VERI_PKT_UPDATE_HEAD)) {
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			("%s: state error:%d update pkt head first\n", __func__, veri_ctrl->veri_pkt_state));
		ret = FALSE;
		return ret;
	}
...
#define MAC_INPUT_FORMAT_LEN 18
	...
	if (arg) {
		...
		if (5 != input_argument) {
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s():input argument format error\n", __func__));
			ret = FALSE;
			goto print_format;
		}
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			("%s(): pkt_type:%d, pkt_subtype:0x%x, addr1_str:%17s, addr2_str:%17s, addr3_str:%17s\n",
				__func__, pkt_type, pkt_subtype, addr1_str, addr2_str, addr3_str));
...
static VOID print_veri_ctrl_en_status(struct _RTMP_ADAPTER *ad)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			("verify_pkt ctrl_en status:\n"));
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			("\tdu:\tno_ack:\ttm:\tsn:\ttxs2m:\ttxs2h:\tpm:\thtc:\n"));
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			("\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n",
