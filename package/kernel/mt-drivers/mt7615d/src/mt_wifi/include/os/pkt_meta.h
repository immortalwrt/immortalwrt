...
#define NET_DEVICE_REAL_IDX_MASK                                               \
	0x0f /* for each operation mode, we maximum support 15 entities. */
...
#define RTMP_GET_PACKET_DHCP(_p) (PACKET_CB(_p, 6) & RTMP_PACKET_SPECIFIC_DHCP)
...
#define RTMP_GET_PACKET_WAI(_p) (PACKET_CB(_p, 6) & RTMP_PACKET_SPECIFIC_WAI)

#define RTMP_GET_PACKET_FORCE_TX(_p)                                           \
	(PACKET_CB(_p, 6) &                                                    \
	 (RTMP_PACKET_SPECIFIC_EAPOL | RTMP_PACKET_SPECIFIC_WAI))

#define RTMP_GET_PACKET_LOWRATE(_p)                                            \
	((PACKET_CB(_p, 6) &                                                   \
	  (RTMP_PACKET_SPECIFIC_EAPOL | RTMP_PACKET_SPECIFIC_DHCP |            \
	   RTMP_PACKET_SPECIFIC_WAI | RTMP_PACKET_SPECIFIC_TDLS)) ||           \
...
#define RTMP_GET_PACKET_VLAN(_p) (PACKET_CB(_p, 6) & RTMP_PACKET_SPECIFIC_VLAN)
...
#define RTMP_GET_PACKET_IPV4(_p) (PACKET_CB(_p, 6) & RTMP_PACKET_SPECIFIC_IPV4)
...
#define RTMP_SET_PACKET_DISASSOC(_p, _flg)                                     \
	do {                                                                   \
		if (_flg)                                                      \
			(RTPKT_TO_OSPKT(_p)->cb[CB_OFF + 11]) |=               \
				(RTMP_PACKET_SPECIFIC_DISASSOC);               \
		else                                                           \
			(RTPKT_TO_OSPKT(_p)->cb[CB_OFF + 11]) &=               \
				(~RTMP_PACKET_SPECIFIC_DISASSOC);              \
	} while (0)
...
#define RTMP_GET_PACKET_PROTOCOL(_p)                                           \
	((((UINT16)PACKET_CB(_p, 11)) << 8) | ((UINT16)PACKET_CB(_p, 10)))
...
#define RTMP_GET_PACKET_MGMT_PKT(_p) ((PACKET_CB(_p, 16) & 0x08) >> 3)

/* use bit0 of cb[CB_OFF+20] */
#define RTMP_SET_PACKET_MGMT_PKT_DATA_QUE(_p, _flg)                            \
	(PACKET_CB(_p, 20) = (PACKET_CB(_p, 20) & 0xFE) | (_flg & 0x01))
#define RTMP_GET_PACKET_MGMT_PKT_DATA_QUE(_p) (PACKET_CB(_p, 20) & 0x01)
...
#define RTMP_GET_PACKET_TDLS_WAIT_ACK(_p)                                      \
	(PACKET_CB(_p, 23) & RTMP_TDLS_SPECIFIC_WAIT_ACK)
...
#define RTMP_GET_PACKET_TDLS_NO_ACK(_p)                                        \
	(PACKET_CB(_p, 23) & RTMP_TDLS_SPECIFIC_NOACK)
...
#define RTMP_CLEAN_PACKET_RECV_FROM(_p)                                        \
	(RTPKT_TO_OSPKT(_p)->cb[CB_OFF + 37] = 0)
...
#define RTMP_IS_PACKET_APCLI(_p)                                               \
	((RTPKT_TO_OSPKT(_p)->cb[CB_OFF + 37]) &                               \
	 (RTMP_PACKET_RECV_FROM_2G_CLIENT | RTMP_PACKET_RECV_FROM_5G_CLIENT |  \
	  RTMP_PACKET_RECV_FROM_5G_H_CLIENT))
...
#define RTMP_IS_PACKET_DIRECT_TX(_p)                                           \
	((RTPKT_TO_OSPKT(_p)->cb[CB_OFF + 37]) & (RTMP_PACKET_DIRECT_TX))

#endif /* CONFIG_HOTSPOT_R2 */

#define RTMP_SET_PACKET_QUEIDX(_p, _idx)                                       \
	(RTPKT_TO_OSPKT(_p)->cb[CB_OFF + 35] = (_idx))
...
#define RTMP_GET_PACKET_PING(_p) (PACKET_CB(_p, 33) & RTMP_PACKET_SPECIFIC_PING)
...
#define RTMP_GET_PACKET_ARP(_p) (PACKET_CB(_p, 33) & RTMP_PACKET_SPECIFIC_ARP)
...
#define RTMP_SET_PACKET_HIGH_PRIO(_p, _flg)                                    \
	do {                                                                   \
		if (_flg)                                                      \
			...
			PACKET_CB(_p, 33) &=                                   \
				(~RTMP_PACKET_SPECIFIC_HIGH_PRIO);             \
	} while (0)
#define RTMP_GET_PACKET_HIGH_PRIO(_p)                                          \
	(PACKET_CB(_p, 33) & RTMP_PACKET_SPECIFIC_HIGH_PRIO)

/* Packet type */
#define RTMP_SET_PACKET_TYPE(_p, _idx)                                         \
	(RTPKT_TO_OSPKT(_p)->cb[CB_OFF + 34] = (_idx))
