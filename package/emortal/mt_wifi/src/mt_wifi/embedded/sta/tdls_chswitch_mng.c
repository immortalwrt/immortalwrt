	...
	if (StatusCode == MLME_SUCCESS) {
		if (pTDLS->ChannelSwitchCurrentState == TDLS_CHANNEL_SWITCH_NONE) {
			if (pTdlsControl->bChannelSwitchInitiator) {
				MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s() - i am Initiator !!!\n", __func__));
			} else
				MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s() - i am responder!!!\n",  __func__));
	...
	IN PVOID SystemSpecific3)
{
	...
	if ((pAd != NULL) && INFRA_ON(pAd)) {
		...
		if (pAd->StaCfg[0].TdlsInfo.TdlsAsicOperateChannel != wdev->channel) {
			...
			TDLS_EnablePktChannel(pAd, FIFO_HCCA | FIFO_EDCA);
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("I am lready on Base Channel :(%d) !!!\n", wdev->channel));
		}
	}
