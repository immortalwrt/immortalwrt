...
#if defined(RLM_CAL_CACHE_SUPPORT) || defined(PRE_CAL_TRX_SET2_SUPPORT) || defined(PRE_CAL_MT7622_SUPPORT) || defined(PRE_CAL_MT7626_SUPPORT) || defined(PRE_CAL_MT7915_SUPPORT)
...
INT32 MtCmdEfuseFreeBlockCount(struct _RTMP_ADAPTER *pAd, PVOID GetFreeBlock, PVOID Result)
{
	...
	if (IS_MT7663(pAd) || IS_MT7915(pAd))
 ...
 *    ExT_CID = 0xAE
 *****************************************/
INT32 MtCmdSmeshConfigSet(RTMP_ADAPTER *pAd, UCHAR *pdata,
	P_EXT_EVENT_SMESH_T prSmeshResult)
{
	struct cmd_msg *msg;
	INT32 ret = 0, size = 0;
	...
	IN UINT8 *pData)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s :\n"
			...
			__func__, prICapInfo->fgTrigger, prICapInfo->fgRingCapEn, prICapInfo->u4TriggerEvent,
...
#if defined(MT7615) ||  defined(MT7663) ||  defined(MT7626) || defined(AXE) || defined(MT7915)

	if (IS_MT7615(pAd) || IS_MT7663(pAd) || IS_MT7626(pAd) || IS_AXE(pAd) || IS_MT7915(pAd)) {
...
INT32 MtCmdExtPmStateCtrl(RTMP_ADAPTER *pAd, MT_PMSTAT_CTRL_T PmStatCtrl)
{
	...
	MTWF_LOG(DBG_CAT_PS, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("%s::Dbdc(%d),Wcid(%d),OwnMac(%d),BSSID(%x,%x,%x,%x,%x,%x),BcnPrd(%d),DtimPrd(%d),AID(%d),Feature(0x%x)\n",
			  __func__,
...
INT32 MtCmdChannelSwitch(RTMP_ADAPTER *pAd, MT_SWITCH_CHANNEL_CFG SwChCfg)
{
	...
	if (ATE_ON(pAd)) {
		...
		for (ant_seq = 0 ; ant_seq < chip_cap->mcs_nss.max_nss ; ant_seq++)
	...
	if (SwChCfg.bScan) {
		...
		if (IS_MT7615(pAd) || IS_MT7915(pAd))
	...
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_INFO, (" TxPowerDrop = 0x%x, ucTxPath:%u\n", CmdChanSwitch.cTxPowerDrop, ucTxPath));
	...
	if (IS_MT7615(pAd) || IS_MT7622(pAd)) {
		/* Check G-Band/A-Band */
		u1SkuChannel = (fg5Gband) ? (SwChCfg.ControlChannel) : (SwChCfg.CentralChannel);

		/* Fill Sku parameter by channel */
		MtFillSkuParam(pAd, u1SkuChannel, fg5Gband, ucTxPath, CmdChanSwitch.acTxPowerSKU);
	}
...
#if defined(MT7915)
	if (IS_MT7915(pAd) && !ATE_ON(pAd)) {
		if (tx_ant_num < rx_ant_num) {
			MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
					 ("\x1b[41m%s: tx num %d < rx num %d, T/RX minimum value will be sent to fw\x1b[m\n",
					  __func__, tx_ant_num, rx_ant_num));
			rx_ant_num = tx_ant_num;
		} else if (tx_ant_num > rx_ant_num) {
			MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
					 ("\x1b[41m%s: rx num %d < tx num %d, T/RX minimum value will be sent to fw\x1b[m\n",
					  __func__, tx_ant_num, rx_ant_num));
			tx_ant_num = rx_ant_num;
		}
	}

	/* BW80+80 and BW160 ant config */
	if (IS_MT7915(pAd) &&
		(SwChCfg.Bw == BW_160 || SwChCfg.Bw == BW_8080)) {
		if (tx_ant_num >= 3) {
			MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
					 ("\x1b[41m%s: T/RX stream (%d) should not be larger than 2 when BW160 is set\x1b[m\n",
					  __func__, tx_ant_num));
			tx_ant_num = 4;
			rx_ant_num = 4;
		} else {
			tx_ant_num = (tx_ant_num << 1);
			rx_ant_num = (rx_ant_num << 1);
		}
	}
#endif
		...
		char *data, UINT16 len)
{
	...
	EXT_EVENT_MULTI_MIB_ACCESS_RD_T *EventMultiMibAccessRD =
		(EXT_EVENT_MULTI_MIB_ACCESS_RD_T *)(data + 20);
...
static INT32 MtCmdFillTxPowerInfo(RTMP_ADAPTER *pAd, EXT_CMD_TX_POWER_CTRL_T *CmdTxPwrCtrl, ATE_TXPOWER TxPower)
{
	...
	for (i = cap->efuse_content_start; i <= cap->efuse_content_end; i++) {
		...
		CmdTxPwrCtrl->aucBinContent[i - cap->efuse_content_start] = data;
	}

	/* Debug print */
	for (i = cap->efuse_content_start; i <= cap->efuse_content_end; i++) {
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_INFO,
				 ("[0x%x]%x ", i, CmdTxPwrCtrl->aucBinContent[i - cap->efuse_content_start]));
	}
...
static VOID MtCmdGetTxPowerRsp(struct cmd_msg *msg, char *Data, UINT16 Len)
{
	...
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s, Type: %d Addr: 0x%x Power: 0x%x Band: %d\n", __func__,
	...
	UINT_8       ucCmmWlanId)
{
	...
	struct _CMD_ATTRIBUTE attr = {0};
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("%s: WlanIdx = %d, ucPfmuIdx = %d\n", __func__, u2WlanIdx, ucCmmWlanId));
	...
	UINT_8       ucCliIdx)
{
	...
	struct _CMD_ATTRIBUTE attr = {0};
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("%s: WlanIdx = %d, ucCliIdx = %d\n", __func__, u2WlanIdx, ucCliIdx));
	...
	PUINT8 pucData)
{
	...
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			("%s: Action:%u, BandIdx:%u, PollPFMUIntrStatTimeOut:%u, FbRptDeQInterval:%u, WlanIdx:%u, PFMUUpdateEn:%u\n", __func__
	...
	PUINT8 pucData)
{
	...
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			("%s: ucAction=0x%02X, ucReadClr=0x%02X, ucVhtOpt=0x%02X, ucHeOpt=0x%02X,\n",
			__func__,
			prTxSndCmd->ucAction,
	...
	PUINT8 pucData)
{
	...
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			("%s: ucAction=0x%02X, ucGloOpt=0x%02X, ucGrpIBfOpt=0x%02X, ucGrpEBfOpt=0x%02X,\n",
			__func__,
			prTxSndCmd->ucAction,
			prTxSndCmd->ucGloOpt,
	...
	PUINT8 pucData)
{
	...
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			("%s: ucAction=0x%02X, fgTxCmdBfManual=0x%02X, ucTxCmdBfBit=0x%02X\n",
			__func__,
	...
	PUINT8 pucData)
{
	...
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			("%s: ucAction=0x%02X, ucBandIdx=0x%02X, ucSmthIntlBypass=0x%02X\n",
			__func__,
	...
	PUINT8 pucData)
{
	...
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			("%s: u1ReadClr=0x%02X, u1Band=0x%02X, u1NUser=0x%02X, u1DBW=0x%02X,\n",
			__func__,
	...
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			("%s: u1NStsUser0=0x%02X, u1NStsUser1=0x%02X, u1NStsUser2=0x%02X, u1NStsUser3=0x%02X,\n",
			__func__,
	...
	MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			("%s: u2PfidUser0=0x%04X, u2PfidUser1=0x%04X, u2PfidUser2=0x%04X, u2PfidUser3=0x%04X,\n",
			__func__,
	...
	EXT_CMD_ETXBF_PFMU_SW_TAG_T rEBfPfmuSwTag)
{
	...
	ret = chip_cmd_tx(pAd, msg);
error:
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s:(ret = %d)\n", __FUNCTION__, ret));
	return ret;
}
	...
	PUINT8 pucData)
{
	...
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s: u2Action=0x%02X\n",
			__func__, prMecCtrlCmd->u2Action));
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s: u2WlanIdx=0x%02X, u1AmsduAlgoEn=0x%02X\n",
			__func__,
			prMecCtrlCmd->mecCmdPara.mec_algo_en_sta_t.u2WlanIdx,
			prMecCtrlCmd->mecCmdPara.mec_algo_en_sta_t.u1AmsduAlgoEn));
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_INFO,
			("%s: u2WlanIdx=0x%02X, u1AmsduEn=0x%02X, u1AmsduNum=0x%02X, u2AmsduLen=0x%02X\n",
			__func__,
	...
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s: u1BaNum=0x%02X, u1AmsduNum=0x%02X, u2AmsduRateThr=0x%02X\n",
			__func__,
	...
	for (u1Idx = 0 ; u1Idx < sizeof(CMD_MEC_CTRL_CMD_T) ; u1Idx++) {
		MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("[%u]: %u\n", u1Idx, pu1Ptr[u1Idx]));
	}
...
VOID MtCmdEfusBufferModeSet(RTMP_ADAPTER *pAd, UINT8 EepromType)
{
	...
	for (cmd_seq = 0; cmd_seq < total_seq; cmd_seq++) {
		...
		if (ops->ee_gen_cmd)
			ret = ops->ee_gen_cmd(pAd, CmdEfuseBufferMode, cmd_seq, total_seq);
		else {
			MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				 ("%s:invalid apply ops, dismissed!\n", __func__));
			goto error;
		}
	...
	UINT16 recheck_time)
{
	...
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		("%s: trigger_type: %d, trigger_temp: %d\n", __func__,
		trigger_type, trigger_temp));

	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		("%s: restore_temp: %d, recheck_time: %d\n", __func__,
	...
	UINT16 u2Wcid, P_EXT_EVENT_TX_STATISTIC_RESULT_T prTxStatResult)
{
	...
	WCID_SET_H_L(rTxStatCmd.ucWlanIdxHnVer, rTxStatCmd.ucWlanIdxL, u2Wcid);
...
INT32 mt_cmd_get_sta_tx_statistic(
	struct _RTMP_ADAPTER *ad, UINT16 wcid, UINT8 dbdcidx, UINT32 field)
{
	struct cmd_msg *msg;
	...
	WCID_SET_H_L(cmd.ucWlanIdxHnVer, cmd.ucWlanIdxL, wcid);
...
INT32 MtCmdBcnOffloadSet(RTMP_ADAPTER *pAd, CMD_BCN_OFFLOAD_T bcn_offload)
{
	...
	MTWF_LOG(DBG_CAT_AP, CATAP_BCN, DBG_LVL_OFF,
			("%s(): Enable=%d, OM=%d, WlanIdx=%d, Band=%d, Len=%d, TimOffset=%d\n",
			__func__,
...
error:
	...
	if (ret == NDIS_STATUS_FAILURE) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s:(rty = %u)\n", __func__, retry_times));
		retry_times++;
		if (retry_times <= 2)
			goto retry;
	}
...
{
	struct cmd_msg *msg;
	EXT_CMD_THERMAL_MODE_CTRL_T ThermalModeCtrl;
	INT32 ret = 0;
...
)
{
	...
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		("%s: tx_mode: %d, tx_nss: %d, tx_bw: %d, mcs_cap: %d, set: %d\n",
		__func__, tx_mode, tx_nss, tx_bw, *mcs_cap, set));
...
)
{
	...
	struct _RTMP_CHIP_CAP *cap = hc_get_chip_cap(pAd->hdev_ctrl);
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			 ("%s: tx_pwr_sku_en: %d, BandIdx: %d\n", __FUNCTION__, tx_pwr_sku_en, BandIdx));
...
)
{
	...
	INT32 ret = 0;
	struct _CMD_ATTRIBUTE attr = {0};
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: fgTpcUtGo: %d \n", __func__, fgTpcUtGo));
	...
	PCHAR        pacTxPwrFccBfOffCase)
{
	...
	rTxPwrBackOff.ucCmdCategoryID = BF_TX_POWER_BACK_OFF_V0;
	...
	IN UINT8 u1ThermalAdc)
{
	...
	rThermalManCtrl.u1PowerCtrlFormatId = TXPOWER_EVENT_THERMAL_SENSOR_SHOW_INFO;
...
INT ShowHeraRuRaInfoProc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("WCID: %d RuIdx: %d Direction: %d DumpGroup: %d\n",
...
INT ShowHeraMuRaInfoProc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s\n", __func__));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("MuGroupIdx: %d UserIdx: %d Direction: %d DumpGroup: %d\n",
	...
	PRTMP_ADAPTER pAd)
{
	...
	MTWF_LOG(DBG_CAT_RA, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			("%s: Vht1024QamSupport=%u\n",
			__func__, rHeraSetVht1024Qam.fgVht1024QamSupport));
	...
	IN	RTMP_STRING *arg)
{
	...
	if (pch != NULL) {
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				("%s: Empty ProtectionPerPpduDis !!\n", __func__));
		Ret = 0;
		goto error;
	}

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			("%s: BandIdx:%d ProtectionPerPpduDis:%d !!\n",
			__func__, param.u1BandIdx, param.fgPtecPerPpduDis));
...
#endif
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s, ipi_hist_idx: 0x%x\n",
		__func__,
...
INT32 mt_cmd_set_ed_cca(
	RTMP_ADAPTER *pAd,
	P_EXT_CMD_ED_CCA_CTRL p_cmd_set_edcca)
{
	struct cmd_msg *msg;
	INT32 ret = 0;
	struct _CMD_ATTRIBUTE attr = {0};

	msg = MtAndesAllocCmdMsg(pAd, sizeof(EXT_CMD_ED_CCA_CTRL));
	...
	SET_CMD_ATTR_MCU_DEST(attr, HOST2N9);
	SET_CMD_ATTR_TYPE(attr, EXT_CID);
	SET_CMD_ATTR_EXT_TYPE(attr, EXT_CMD_ID_SET_EDCCA);
	SET_CMD_ATTR_CTRL_FLAGS(attr, INIT_CMD_SET);
	SET_CMD_ATTR_RSP_WAIT_MS_TIME(attr, 0);
	SET_CMD_ATTR_RSP_EXPECT_SIZE(attr, MT_IGNORE_PAYLOAD_LEN_CHECK);
	SET_CMD_ATTR_RSP_WB_BUF_IN_CALBK(attr, NULL);
	...
	MtAndesInitCmdMsg(msg, attr);
	MtAndesAppendCmdMsg(msg, (char *)p_cmd_set_edcca,
						sizeof(EXT_CMD_ED_CCA_CTRL));
	ret = chip_cmd_tx(pAd, msg);
error:
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("%s:(ret = %d)\n", __func__, ret));
...
INT32 cmd_vlan_update(struct _RTMP_ADAPTER *ad, UCHAR band_idx, UINT8 omac_idx, UINT8 op_code, UINT16 value)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
		("%s(): band_idx = %d, omac_idx = %d, op_code = %d, value = %d\n",
		__func__, band_idx, omac_idx, op_code, value));
...
INT ShowHeraRelatedInfoProc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s\n", __func__));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("Para1: %d Para2: %d Para3: %d Para4: %d\n",
	...
	UINT16 Len)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s, pAd:%p, Data:%p, Len:%d\n", __func__, pAd, Data, Len));
