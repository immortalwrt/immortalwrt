...
INT Set_PSMode_Proc(RTMP_ADAPTER *pAdapter, RTMP_STRING *arg)
{
	...
	if (pObj->ioctl_if_type == INT_APCLI) {
		struct wifi_dev *wdev = &pStaCfg->wdev;
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s:wdev(type=%d,if=%s)\n",
			__func__, wdev->wdev_type, wdev->if_dev->name));
	}
...
INT Set_Wpa_Support(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_Wpa_Support::(WpaSupplicantUP=%d)\n",
...
INT	 Set_WscConfMode_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (pStaCfg->BssType == BSS_ADHOC) {
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IF(ra0) Set_WscConfMode_Proc::(IWscConfMode=%d)\n",
...
INT	Set_WscGetConf_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (pStaCfg->BssType == BSS_ADHOC) {
		...
		if (STA_STATUS_TEST_FLAG(pStaCfg, fSTA_STATUS_MEDIA_STATE_CONNECTED) &&
			...
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Empty Available IPv4 Submask List. Cannot be Registrar\n"));
...
INT	Set_IWscLimitedUI_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_IWscLimitedUI_Proc::(bLimitedUI=%d)\n",
...
INT	Set_IWscDefaultSecurity_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if ((TmpValue != 0) && (TmpValue <= 3)) {
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::(IWscDefaultSecurity=%d)\n", __func__,
				 pStaCfg->IWscInfo.IWscDefaultSecurity));
		return TRUE;
	} else {
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::(Value=%d)\n", __func__, TmpValue));
		return FALSE;
	}
}
...
INT	Set_IWscOOB_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MlmeEnqueue(pAd,
				...
				(VOID *)&Ssid, 0);
	RTMP_MLME_HANDLER(pAd);
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_IWscOOB_Proc\n"));
	return TRUE;
}
...
INT	Set_IWscSinglePIN_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_IWscSinglePIN_Proc::(bSinglePIN=%d)\n",
...
INT Set_WiDiEnable_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IF Set_WiDiEnable_Proc::(bWIDI=%d)\n", pStaCfg->bWIDI));
	return TRUE;
}
#endif
...
#ifdef TXBF_SUPPORT
{
	...
	case RT_OID_802_11_SET_TDLS:
		if (wrq->u.data.length != sizeof(ULONG))
			...
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set::RT_OID_802_11_SET_DLS (=%d)\n",
	...
	case RT_OID_802_11_SET_TDLS_PARAM:
		if (wrq->u.data.length	!= sizeof(RT_802_11_TDLS_UI))
			...
			MlmeEnqueue(pAd,
						...
						&TDLS, 0);
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set::RT_OID_802_11_SET_TDLS_PARAM\n"));
		}
	...
	case RT_OID_802_11_SET_IMME_BA_CAP:
		...
		if (wrq->u.data.length != sizeof(OID_BACAP_STRUC))
			...
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
					 ("Set::(Orde.AutoBA = %d)(ReBAWinLimit=%d)(TxBAWinLimit=%d)\n",
					  Orde.AutoBA,
	...
	case OID_SET_COUNTERMEASURES:
		if (wrq->u.data.length != sizeof(int))
			...
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set::OID_SET_COUNTERMEASURES bBlockAssoc=%s\n",
	...
	case RT_OID_WPA_SUPPLICANT_SUPPORT:
		if (wrq->u.data.length != sizeof(UCHAR))
			...
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set::RT_OID_WPA_SUPPLICANT_SUPPORT (=0x%02X)\n",
					 pStaCfg->wpa_supplicant_info.WpaSupplicantUP));
		}
	...
	case OID_802_11_DROP_UNENCRYPTED:
		if (wrq->u.data.length != sizeof(int))
			...
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set::OID_802_11_DROP_UNENCRYPTED (=%d)\n", enabled));
		}
	...
	case OID_802_11_SET_IEEE8021X:
		if (wrq->u.data.length != sizeof(BOOLEAN))
			...
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set::OID_802_11_SET_IEEE8021X (=%d)\n", IEEE8021xState));
		}
	...
	case OID_802_11_SET_IEEE8021X_REQUIRE_KEY:
		if (wrq->u.data.length != sizeof(BOOLEAN))
			...
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set::OID_802_11_SET_IEEE8021X_REQUIRE_KEY (%d)\n",
	...
	case RT_OID_WPS_PROBE_REQ_IE:
		...
		if (pStaCfg->wpa_supplicant_info.pWpsProbeReqIe) {
			...
			if (Status) {
				...
				MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set::RT_OID_WPS_PROBE_REQ_IE, WpsProbeReqIeLen = %d!!\n",
						 pStaCfg->wpa_supplicant_info.WpsProbeReqIeLen));
			}
		} else
			Status = -ENOMEM;
	...
	case RT_OID_INTEL_WIDI:
		if (pStaCfg->bWIDI) {
			if ((wrq->u.data.length < 1) || (wrq->u.data.length > sizeof(WIDI_SERVICE_MSG))) {
				MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set::RT_OID_INTEL_WIDI : len=(%d) not correct!\n",
						 wrq->u.data.length));
				Status = -EINVAL;
				break;
			} else {
				...
				if (msg.type != WIDI_MSG_TYPE_SERVICE) {
					MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set::RT_OID_INTEL_WIDI : Invalid msg_type(%d) received!\n",
							 msg.type));
					Status = -EINVAL;
					break;
				}
	...
	case RT_OID_WSC_GEN_PIN_CODE:
		if (wrq->u.data.length != 1) {
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set::RT_OID_WSC_GEN_PIN_CODE : len=(%d) not correct!\n",
					 wrq->u.data.length));
			Status = -EINVAL;
			break;
		} else {
			...
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set::RT_OID_WSC_GEN_PIN_CODE (=%d)\n", Enable));
		}
	...
	IN struct wifi_dev *wdev)
{
	...
	case RT_OID_802_11_TX_POWER_LEVEL_1:
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
				("Query::RT_OID_802_11_TX_POWER_LEVEL_1 (BAND0) (=%d) STATUS[%d]\n",
	...
	case RT_OID_NEW_DRIVER: {
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::RT_OID_NEW_DRIVER (=%d)\n", enabled));
	}
	break;
	...
	case RT_OID_WPA_SUPPLICANT_SUPPORT:
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::RT_OID_WPA_SUPPLICANT_SUPPORT (=%d)\n",
				 pStaCfg->wpa_supplicant_info.WpaSupplicantUP));
		break;
#endif /* WPA_SUPPLICANT_SUPPORT */
#ifdef WSC_STA_SUPPORT
	...
	case OID_802_11_P2P_SCAN_LIST:
		...
		if (pUI_table) {
			...
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::OID_802_11_P2P_SCAN_LIST\n"));
	...
	case RT_OID_802_11_QUERY_WFD_TDLS_PEER_IP_ADDR: {
		...
		for (i = 0; i < MAX_NUM_OF_TDLS_ENTRY; i++) {
			if ((pStaCfg->TdlsInfo.TDLSEntry[i].Valid) && (pStaCfg->TdlsInfo.TDLSEntry[i].Status == TDLS_MODE_CONNECTED)) {
				MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Peer IP Addr = %d.%d.%d.%d\n",
	...
	case RT_OID_IWSC_SELF_IPV4:
		if (wrq->u.data.length != sizeof(UINT32)) {
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query Failed::RT_OID_IWSC_SELF_IPV4(=%d)\n",
					 wrq->u.data.length));
			Status  = -EINVAL;
		} else {
			Status = copy_to_user(wrq->u.data.pointer, &pStaCfg->IWscInfo.SelfIpv4Addr, sizeof(UINT32));
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::RT_OID_IWSC_SELF_IPV4(=0x%08x), Status = %d\n",
	...
	case RT_OID_IWSC_REGISTRAR_IPV4:
		if (wrq->u.data.length != sizeof(UINT32)) {
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query Failed::RT_OID_IWSC_REGISTRAR_IPV4(=%d)\n",
					 wrq->u.data.length));
			Status  = -EINVAL;
		} else {
			Status = copy_to_user(wrq->u.data.pointer, &pStaCfg->IWscInfo.RegIpv4Addr, sizeof(UINT32));
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::RT_OID_IWSC_REGISTRAR_IPV4(=0x%08x), Status = %d\n",
	...
	case RT_OID_IWSC_SMPBC_ENROLLEE_COUNT:
		if (wrq->u.data.length != sizeof(UINT8)) {
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query Failed::RT_OID_IWSC_SMPBC_ENROLLEE_COUNT(=%d)\n",
					 wrq->u.data.length));
			Status  = -EINVAL;
		} else {
			Status = copy_to_user(wrq->u.data.pointer, &pStaCfg->IWscInfo.SmpbcEnrolleeCount, sizeof(UINT8));
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("Query::RT_OID_IWSC_SMPBC_ENROLLEE_COUNT(=0x%08x), Status = %d\n",
...
range:
	...
	if (bIsRange) {
		...
		if (strlen(value) <= sizeof(temp2))
			...
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				("%s the length of value is too long\n", __func__));
	...
	IN	ULONG					Data)
{
	...
	if (pStaCfg == NULL) {
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: pStaCfg is NULL!!!\n", __func__));
		return NDIS_STATUS_FAILURE;
	}
#ifdef WPA_SUPPLICANT_SUPPORT
...
INT RtmpIoctl_rt_ioctl_siwscan(RTMP_ADAPTER *pAd, VOID *pData, ULONG Data)
{
	...
	do {
		...
		if (pConfig->FlgScanThisSsid) {
			...
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("rt_ioctl_siwscan:: req.essid_len-%d, essid-%s\n",
	...
	IN	ULONG					Data)
{
	...
	if (supc_i && supc_i->WpaSupplicantUP != WPA_SUPPLICANT_DISABLE) {
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("===> rt_ioctl_siwgenie\n"));
		...
		IN UINT32 BTMRspLen)
{
	...
	if (!IsFound) {
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, (" %s BTMPeerEntry not found\n", __func__));
		goto error0;
	}
