...
UCHAR cfg_tdls_GetRegulatoryClass(IN PRTMP_ADAPTER pAd, IN UCHAR ChannelWidth,
				  IN UCHAR TargetChannel)
{
	...
	do {
		if (cfg_reg_class[i].spacing == ChannelWidth) {
			...
			do {
				if (cfg_reg_class[i].channelset[j] ==
				    TargetChannel) {
...
VOID InitPeerEntryRateCapability(IN PRTMP_ADAPTER pAd,
				 ...
				 IN HT_CAPABILITY_IE *pHtCapability)
{
	...
	pEntry->MaxSupportedRate =
		min(pAd->CommonCfg.MaxDesiredRate, MaxSupportedRate);
	...
	if ((HtCapabilityLen != 0) && (wdev->PhyMode >= PHY_11ABGN_MIXED)) {
		...
		MTWF_LOG(
			DBG_CAT_PROTO, CATPROTO_TDLS, DBG_LVL_TRACE,
		...
		for (ii = 31; ii >= 0; ii--) { /* 3*3 */
			...
			if ((pAd->StaCfg[0].wdev.DesiredHtPhyInfo.MCSSet[j] &
			     bitmask) &&
		...
		if (pAd->StaCfg[0].wdev.DesiredTransmitSetting.field.MCS !=
		    MCS_AUTO) {
			if (pAd->StaCfg[0]
				...
				pEntry->MaxHTPhyMode.field.MCS = 32;
				   ...
				   pAd->StaCfg[0].wdev.HTPhyMode.field.MCS) {
		...
		if (pHtCapability->ExtHtCapInfo.MCSFeedback == 0x03)
			CLIENT_STATUS_SET_FLAG(
				pEntry, fCLIENT_STATUS_MCSFEEDBACK_CAPABLE);

		os_move_mem(&pEntry->HTCapability, pHtCapability,
	...
	if (pAd->StaCfg[0].wdev.bAutoTxRateSwitch == TRUE) {
		...
		pEntry->HTPhyMode.field.MODE =
			pAd->StaCfg[0].wdev.HTPhyMode.field.MODE;
		pEntry->HTPhyMode.field.MCS =
			pAd->StaCfg[0].wdev.HTPhyMode.field.MCS;
...
BOOLEAN CFG80211DRV_StaTdlsSetKeyCopyFlag(VOID *pAdOrg)
{
	...
	MTWF_LOG(DBG_CAT_PROTO, CATPROTO_TDLS, DBG_LVL_INFO,
		 ("======>  %s()\n", __func__));
	pAd->StaCfg[0].wpa_supplicant_info.CFG_Tdls_info.IneedKey = 1;
	return TRUE;
}

BOOLEAN CFG80211DRV_StaTdlsInsertDeletepEntry(VOID *pAdOrg, VOID *pData,
					      UINT Data)
{
	PRTMP_ADAPTER pAd = (PRTMP_ADAPTER)pAdOrg;
	UCHAR *peer;
	...
	if (Data == tdls_insert_entry) {
		...
		if (i == MAX_NUM_OF_CFG_TDLS_ENTRY) {
			MTWF_LOG(
				DBG_CAT_PROTO, CATPROTO_TDLS, DBG_LVL_ERROR,
				("%s() - EXCEED MAX_NUM_OF_CFG_TDLS_ENTRY %d. Abort this setup\n",
		...
		if (pMacEntry && IS_ENTRY_TDLS(pMacEntry))
			MTWF_LOG(DBG_CAT_PROTO, CATPROTO_TDLS, DBG_LVL_ERROR,
				 ("%s() - MacTable Entry exist !!!\n",
				  __func__));
		else
			pMacEntry = MacTableInsertEntry(
				pAd, peer, wdev, ENTRY_TDLS, OPMODE_STA, TRUE);
		...
		if (pMacEntry) {
			...
			pAd->StaCfg[0]
				.wpa_supplicant_info.CFG_Tdls_info
				.TDLSEntry[tdls_entry_index]
				.EntryValid = TRUE;
			pAd->StaCfg[0]
				.wpa_supplicant_info.CFG_Tdls_info
				.TDLSEntry[tdls_entry_index]
				.MacTabMatchWCID = pMacEntry->wcid;
			pAd->StaCfg[0]
				.wpa_supplicant_info.CFG_Tdls_info
				.TdlsLinkCount++;
			...
			if (WMODE_CAP_N(wdev->PhyMode)) {
				...
				os_zero_mem(&HtCapabilityTmp,
					    sizeof(HT_CAPABILITY_IE));
				os_move_mem(&HtCapabilityTmp,
				...
				*(USHORT *)(&HtCapabilityTmp.ExtHtCapInfo) =
					SWAP16(*(
						USHORT *)(&HtCapabilityTmp
								   .ExtHtCapInfo));
#endif
			}

			InitPeerEntryRateCapability(
				pAd, pMacEntry, &pAd->StaActive.CapabilityInfo,
			...
			if (IS_SECURITY(&wdev->SecConfig)) {
				...
				if (pMacEntry->Aid > 1) {
					...
					Info.BssIndex = BSS0;
					Info.Cipher = pMacEntry->SecConfig
							      .PairwiseCipher;
					Info.KeyIdx = 0;
					...
					pMacEntry->wdev->PortSecured =
						WPA_802_1X_PORT_SECURED;
					pMacEntry->PrivacyFilter =
		...
		for (i = 0; i < MAX_NUM_OF_CFG_TDLS_ENTRY; i++) {
			if (pAd->StaCfg[0].wpa_supplicant_info.CFG_Tdls_info
					    .TDLSEntry[i]
					    .EntryValid == TRUE &&
			    MAC_ADDR_EQUAL(
				    peer,
				...
				os_zero_mem(
					&(pAd->StaCfg[0]
						  .wpa_supplicant_info
						  .CFG_Tdls_info.TDLSEntry[i]),
					sizeof(CFG_TDLS_ENTRY));
				pAd->StaCfg[0]
					.wpa_supplicant_info.CFG_Tdls_info
					.TdlsLinkCount--;
#ifdef UAPSD_SUPPORT
...
BOOLEAN cfg_tdls_UAPSDP_AsicCanSleep(PRTMP_ADAPTER pAd)
{
	...
	for (i = 0; i < MAX_NUM_OF_CFG_TDLS_ENTRY; i++) {
		PCFG_TDLS_ENTRY pTDLS =
			&pAd->StaCfg[0]
				 .wpa_supplicant_info.CFG_Tdls_info.TDLSEntry[i];
...
VOID cfg_tdls_UAPSDP_PsmModeChange(IN PRTMP_ADAPTER pAd, IN USHORT PsmOld,
				   IN USHORT PsmNew)
{
	...
	for (IdTdls = 0; IdTdls < MAX_NUM_OF_CFG_TDLS_ENTRY; IdTdls++) {
		if (pAd->StaCfg[0]
			    .wpa_supplicant_info.CFG_Tdls_info.TDLSEntry[IdTdls]
			    .EntryValid == TRUE) {
			/* get MAC Entry */
			pMacEntry =
				&pAd->MacTab.Content[pAd->StaCfg[0]
							     .wpa_supplicant_info
							     .CFG_Tdls_info
							     .TDLSEntry[IdTdls]
							     .MacTabMatchWCID];
			 ...
			 */
			cfg_tdls_send_PeerTrafficIndication(pAd,
							    pMacEntry->Addr);
		}
	}
...
VOID cfg_tdls_rcv_PeerTrafficIndication(PRTMP_ADAPTER pAd, UINT8 dialog_token,
					UINT8 *peer)
{
	...
	if (tdls_entry_wcid == -1) {
		MTWF_LOG(
			DBG_CAT_PROTO, CATPROTO_TDLS, DBG_LVL_ERROR,
			("%s(): can't find valid link ID, tdls link to (%02X:%02X:%02X:%02X:%02X:%02X) doesn't exist.\n",
			 __func__, PRINT_MAC(peer)));
		return;
	}

	cfg_tdls_build_frame(pAd, peer, dialog_token,
			     TDLS_ACTION_CODE_PEER_TRAFFIC_RESPONSE, 0, NULL, 0,
			     FALSE, tdls_entry_wcid, 0);
	MTWF_LOG(DBG_CAT_PROTO, CATPROTO_TDLS, DBG_LVL_INFO,
		 ("<======  %s() out\n", __func__));
}
...
VOID cfg_tdls_send_PeerTrafficIndication(PRTMP_ADAPTER pAd, UINT8 *peer)
{
	...
	if (pAd->StaCfg[0].wpa_supplicant_info.CFG_Tdls_info.TdlsDialogToken ==
	    0)
		pAd->StaCfg[0]
			.wpa_supplicant_info.CFG_Tdls_info.TdlsDialogToken = 1;

	dialog_token =
		pAd->StaCfg[0]
			.wpa_supplicant_info.CFG_Tdls_info.TdlsDialogToken++;
	...
	if (pAd->StaCfg[0]
		    .wpa_supplicant_info.CFG_Tdls_info
		    .TDLSEntry[tdls_entry_link_index]
		    .FlgIsWaitingUapsdTraRsp == TRUE) {
		MTWF_LOG(
			DBG_CAT_PROTO, CATPROTO_TDLS, DBG_LVL_ERROR,
	...
	pAd->StaCfg[0]
		...
		.FlgIsWaitingUapsdTraRsp = TRUE;
	cfg_tdls_build_frame(pAd, peer, dialog_token,
			     TDLS_ACTION_CODE_PEER_TRAFFIC_INDICATION, 0, NULL,
			     0, FALSE, tdls_entry_link_index, 0);
	/*
...
VOID cfg_tdls_TimerInit(PRTMP_ADAPTER pAd)
{
	...
	for (i = 0; i < MAX_NUM_OF_CFG_TDLS_ENTRY; i++) {
		...
		pAd->StaCfg[0]
			.wpa_supplicant_info.CFG_Tdls_info.TDLSEntry[i]
			.pAd = pAd;
		RTMPInitTimer(pAd, &(pCfgTdls->Timer),
			      GET_TIMER_FUNCTION(cfg_tdls_PTITimeoutAction),
			      pCfgTdls, FALSE);
	}
}

VOID cfg_tdls_PTITimeoutAction(IN PVOID SystemSpecific1,
			       ...
			       IN PVOID SystemSpecific3)
{
	...
	if (pTDLS->FlgIsWaitingUapsdTraRsp == TRUE) {
		...
		MTWF_LOG(
			DBG_CAT_PROTO, CATPROTO_TDLS, DBG_LVL_OFF,
		...
		RTEnqueueInternalCmd(pAd, CMDTHREAD_TDLS_AUTO_TEARDOWN,
				     pMacEntry, sizeof(MAC_TABLE_ENTRY));
		return;
	}
}
...
INT cfg_tdls_search_wcid(PRTMP_ADAPTER pAd, UINT8 *peer)
{
	...
	for (i = 0; i < MAX_NUM_OF_CFG_TDLS_ENTRY; i++) {
		if (MAC_ADDR_EQUAL(pAd->StaCfg[0]
					   .wpa_supplicant_info.CFG_Tdls_info
					   .TDLSEntry[i]
					   .MacAddr,
				   peer) &&
...
INT cfg_tdls_build_frame(PRTMP_ADAPTER pAd, u8 *peer, u8 dialog_token,
			 u8 action_code, u16 status_code, const u8 *extra_ies,
			 size_t extra_ies_len, BOOLEAN send_by_tdls_link,
			 u8 tdls_entry_link_index, u8 reason_code)
{
	...
	if (NStatus != NDIS_STATUS_SUCCESS) {
		MTWF_LOG(
			DBG_CAT_PROTO, CATPROTO_TDLS, DBG_LVL_ERROR,
			("ACT - TDLS_SetupRequestAction() allocate memory failed\n"));
		return NStatus;
	}

	if (action_code != WLAN_PUB_ACTION_TDLS_DISCOVER_RES)
		MAKE_802_3_HEADER(Header802_3, peer, pAd->CurrentAddress,
				  TDLS_ETHERTYPE);
	...
	if (action_code != WLAN_PUB_ACTION_TDLS_DISCOVER_RES) {
		...
		MakeOutgoingFrame(pOutBuffer, &TempLen, 1, &RemoteFrameType,
				  END_OF_ARGS);
		FrameLen = FrameLen + TempLen;
		MakeOutgoingFrame(pOutBuffer + FrameLen, &TempLen, 1, &category,
				  1, &tmp_action_code, END_OF_ARGS);
		FrameLen = FrameLen + TempLen;
	} else {
		category = CATEGORY_PUBLIC;
		MakeOutgoingFrame(pOutBuffer + FrameLen, &TempLen, 1, &category,
				  1, &action_code, END_OF_ARGS);
		FrameLen = FrameLen + TempLen;
	}
	...
	if (action_code == WLAN_TDLS_SETUP_REQUEST ||
		...
		/* fill support rate */
		MakeOutgoingFrame(pOutBuffer + FrameLen, &TempLen, 1,
				  &SupRateIe, 1, &pAd->StaActive.SupRateLen,
				  pAd->StaActive.SupRateLen,
		...
		if (pAd->CommonCfg.bCountryFlag) {
			...
			/* prepare channel information */
			{
				...
				regclass = cfg_tdls_GetRegulatoryClass(
					pAd, op_ht_bw, wdev->channel);
				MakeOutgoingFrame(TmpFrame + TmpLen2, &TmpLen,
						  1, &RegluatoryRxtIdent, 1,
						  &regclass, 1, &CoverageClass,
						  END_OF_ARGS);
				TmpLen2 += TmpLen;
			}
		...
		if (pAd->StaCfg[0]
			...
			if (list_len > 0) {
				MakeOutgoingFrame(pOutBuffer + FrameLen,
						  &TempLen, 1, &SupportChIe, 1,
						  &list_len, list_len,
						  &ch_set[0], END_OF_ARGS);
				FrameLen = FrameLen + TempLen;
			}
		}
		...
		if (pAd->StaCfg[0]
			    .wpa_supplicant_info.CFG_Tdls_info.TdlsChSwitchSupp)
		...
		MakeOutgoingFrame(pOutBuffer + FrameLen, &TempLen, 1, &ExtCapIe,
				  1, &Length, sizeof(EXT_CAP_INFO_ELEMENT),
				  &extCapInfo, END_OF_ARGS);
		FrameLen = FrameLen + TempLen;
		...
		if (action_code == WLAN_PUB_ACTION_TDLS_DISCOVER_RES &&
			...
			do {
				...
				MakeOutgoingFrame(
					pOutBuffer + FrameLen, &TempLen, 1,
					&TimeOutIntervalIE, 1, &Length, 1,
					(PUCHAR)&TimeoutType, 4,
					(PUCHAR)&TimeoutValueBuf, END_OF_ARGS);
				FrameLen = FrameLen + TempLen;
			} while (0);
		}
		...
		do {
			...
			UCHAR Length = 16;
			UCHAR SuppClassesList[] = { 1,	2,  3,	4,  12,
						    22, 23, 24, 25, 27,
						    28, 29, 30, 32, 33 };
			UCHAR regclass;

			regclass = cfg_tdls_GetRegulatoryClass(pAd, op_ht_bw,
							       wdev->channel);
			MakeOutgoingFrame(pOutBuffer + FrameLen, &TempLen, 1,
					  &TDLS_IE, 1, &Length, 1, &regclass,
					  15, SuppClassesList, END_OF_ARGS);
			FrameLen = FrameLen + TempLen;
		} while (0);
		...
		if (WMODE_CAP_N(wdev->PhyMode)) {
			...
			*(USHORT *)(&HtCapabilityTmp.ExtHtCapInfo) = SWAP16(
				*(USHORT *)(&HtCapabilityTmp.ExtHtCapInfo));
			MakeOutgoingFrame(pOutBuffer + FrameLen, &TempLen, 1,
					  &HtCapIe, 1, &HtLen, HtLen,
					  &HtCapabilityTmp, END_OF_ARGS);
		...
		MakeOutgoingFrame(pOutBuffer + FrameLen, &TempLen, 1,
				  &BssCoexistIe, 1, &Length, 1,
				  &BssCoexistence.word, END_OF_ARGS);
		FrameLen = FrameLen + TempLen;
#endif /* DOT11N_DRAFT3 // */
#endif /* DOT11_N_SUPPORT // */
	}
	...
	if (action_code == WLAN_TDLS_SETUP_REQUEST ||
	    ...
	    action_code == TDLS_ACTION_CODE_AUTO_TEARDOWN ||
	    (action_code == TDLS_ACTION_CODE_PEER_TRAFFIC_INDICATION &&
	     pAd->StaCfg[0]
		     .wpa_supplicant_info.CFG_Tdls_info
		     .TDLSEntry[tdls_entry_link_index]
		     .bInitiator) ||
	    (action_code == TDLS_ACTION_CODE_PEER_TRAFFIC_RESPONSE &&
	     pAd->StaCfg[0]
		     .wpa_supplicant_info.CFG_Tdls_info
		     .TDLSEntry[tdls_entry_link_index]
		     .bInitiator)) {
		/* We are initiator. */
		MakeOutgoingFrame(pOutBuffer + FrameLen, &TempLen, 1, &TDLS_IE,
				  1, &TDLS_IE_LEN, 6, pAd->CommonCfg.Bssid, 6,
				  pAd->CurrentAddress, 6, peer, END_OF_ARGS);
		FrameLen = FrameLen + TempLen;
	} else {
		/* We are repsonder. */
		MakeOutgoingFrame(pOutBuffer + FrameLen, &TempLen, 1, &TDLS_IE,
				  1, &TDLS_IE_LEN, 6, pAd->CommonCfg.Bssid, 6,
				  peer, 6, pAd->CurrentAddress, END_OF_ARGS);
		FrameLen = FrameLen + TempLen;
	}
	...
	if (action_code == WLAN_TDLS_SETUP_REQUEST ||
		...
		if (pAd->CommonCfg.bWmmCapable) {
			...
			UCHAR WmeParmIe[8] = { IE_VENDOR_SPECIFIC,
					       7,
					       0x00,
					       0x50,
					       0xf2,
			...
			if (wdev->UapsdInfo.bAPSDCapable) {
				QosInfo.UAPSD_AC_BE =
					1; /* pAd->CommonCfg.TDLS_bAPSDAC_BE; */
				QosInfo.UAPSD_AC_BK =
					1; /* pAd->CommonCfg.TDLS_bAPSDAC_BK; */
				QosInfo.UAPSD_AC_VI =
					1; /* pAd->CommonCfg.TDLS_bAPSDAC_VI; */
				QosInfo.UAPSD_AC_VO =
					1; /* pAd->CommonCfg.TDLS_bAPSDAC_VO; */
				QosInfo.MaxSPLength =
					0; /* pAd->CommonCfg.TDLS_MaxSPLength; */
				MTWF_LOG(
					DBG_CAT_PROTO, CATPROTO_TDLS,
					DBG_LVL_ERROR,
					("tdls our uapsd> UAPSD %d %d %d %d %d!\n",
					 QosInfo.UAPSD_AC_BE,
					 QosInfo.UAPSD_AC_BK,
					 QosInfo.UAPSD_AC_VI,
					 QosInfo.UAPSD_AC_VO,
			...
			MakeOutgoingFrame(pOutBuffer + FrameLen, &TempLen, 8,
					  WmeParmIe, 1, &QosInfo, END_OF_ARGS);
			FrameLen = FrameLen + TempLen;
		}
		 ...
		 */
		UCHAR WmeParmIe[26] = { IE_VENDOR_SPECIFIC,
					24,
					0x00,
					0x50,
					0xf2,
		...
		if (wdev->UapsdInfo.bAPSDCapable) {
			...
			MTWF_LOG(DBG_CAT_PROTO, CATPROTO_TDLS, DBG_LVL_ERROR,
				 ("tdls our uapsd> UAPSD %d %d %d %d %d!\n",
				  QosInfo.UAPSD_AC_BE, QosInfo.UAPSD_AC_BK,
				  QosInfo.UAPSD_AC_VI, QosInfo.UAPSD_AC_VO,
				  QosInfo.MaxSPLength));
		}
		...
		for (idx = QID_AC_BK; idx <= QID_AC_VO; idx++) {
			WmeParmIe[10 + (idx * 4)] =
				(idx << 5) + /* b5-6 is ACI */
				((UCHAR)pAd->CommonCfg.APEdcaParm[0].bACM[idx]
				 << 4) + /* b4 is ACM */
				(pAd->CommonCfg.APEdcaParm[0].Aifsn[idx] &
				 0x0f); /* b0-3 is AIFSN */
			WmeParmIe[11 + (idx * 4)] =
				(pAd->CommonCfg.APEdcaParm[0].Cwmax[idx]
				 << 4) + /* b5-8 is CWMAX */
				(pAd->CommonCfg.APEdcaParm[0].Cwmin[idx] &
				 0x0f); /* b0-3 is CWMIN */
			WmeParmIe[12 + (idx * 4)] =
				(UCHAR)(pAd->CommonCfg.APEdcaParm[0].Txop[idx] &
		...
		MakeOutgoingFrame(pOutBuffer + FrameLen, &TempLen, 26,
				  WmeParmIe, END_OF_ARGS);
		FrameLen = FrameLen + TempLen;
	}

	if (action_code == TDLS_ACTION_CODE_PEER_TRAFFIC_RESPONSE ||
	    (action_code == WLAN_TDLS_TEARDOWN && reason_code != 25)) {
		pMacEntry = MacTableLookup(pAd, peer);
	...
	if (action_code == TDLS_ACTION_CODE_PEER_TRAFFIC_INDICATION) {
...
#ifdef UAPSD_SUPPORT
		/* get queue status */
		UAPSD_QueueStatusGet(pAd, pEntry, &FlgIsAnyPktForBK,
				     &FlgIsAnyPktForBE, &FlgIsAnyPktForVI,
				     &FlgIsAnyPktForVO);
		...
		/* init element */
		MakeOutgoingFrame(pOutBuffer + FrameLen, &TempLen, 1,
				  &IeIdPuBufferStatus, 1, &Length, 1,
				  &PuBufferStatus, END_OF_ARGS);
		FrameLen = FrameLen + TempLen;
	}
	...
	if (extra_ies_len != 0) {
		...
		MakeOutgoingFrame(pOutBuffer + FrameLen, &TempLen,
				  extra_ies_len, extra_ies, END_OF_ARGS);
		FrameLen = FrameLen + TempLen;
	}
	...
	case WLAN_TDLS_SETUP_RESPONSE:
		CFG80211DBG(DBG_LVL_ERROR,
			    ("80211> WLAN_TDLS_SETUP_RESPONSE ==>\n"));
		break;

	case WLAN_TDLS_SETUP_CONFIRM:
		CFG80211DBG(DBG_LVL_ERROR,
			    ("80211> WLAN_TDLS_SETUP_CONFIRM ==>\n"));
		break;
	...
	case WLAN_TDLS_DISCOVERY_REQUEST:
		CFG80211DBG(DBG_LVL_ERROR,
			    ("80211> WLAN_TDLS_DISCOVERY_REQUEST ==>\n"));
		break;

	case WLAN_PUB_ACTION_TDLS_DISCOVER_RES:
		CFG80211DBG(
			DBG_LVL_ERROR,
			("80211> WLAN_PUB_ACTION_TDLS_DISCOVERY_RESPONSE ==>\n"));
		break;

	case TDLS_ACTION_CODE_PEER_TRAFFIC_INDICATION:
		CFG80211DBG(
			DBG_LVL_ERROR,
			("80211> TDLS_ACTION_CODE_PEER_TRAFFIC_INDICATION ==>\n"));
		break;

	case TDLS_ACTION_CODE_WFD_TUNNELED_PROBE_REQ:
		CFG80211DBG(
			DBG_LVL_ERROR,
			("80211> TDLS_ACTION_CODE_WFD_TUNNELED_PROBE_REQ ==>\n"));
		break;

	case TDLS_ACTION_CODE_WFD_TUNNELED_PROBE_RSP:
		CFG80211DBG(
			DBG_LVL_ERROR,
			("80211> TDLS_ACTION_CODE_WFD_TUNNELED_PROBE_RSP ==>\n"));
		break;

	default:
		CFG80211DBG(DBG_LVL_ERROR,
			    ("80211> UNKNOWN TDLS PACKET ==>\n"));
		break;
	}
	...
	if (send_by_tdls_link == TRUE ||
	    (action_code == WLAN_TDLS_TEARDOWN &&
	     reason_code != 25)) { /* only uapsd PTR, normal teardown */
#ifdef UAPSD_SUPPORT
		if (action_code == TDLS_ACTION_CODE_PEER_TRAFFIC_RESPONSE)
			RTMP_PS_VIRTUAL_WAKEUP_PEER(
				pMacEntry); /* peer can not sleep for a while */
		...
		Status = RTMPAllocateNdisPacket(pAd, &pPacket, Header802_3,
						LENGTH_802_3, pOutBuffer,
						(UINT)FrameLen);

		if (Status != NDIS_STATUS_SUCCESS)
			return -1;

		Status = ge_enq_req(pAd, pPacket, QID_AC_VI,
				    &pAd->MacTab.tr_entry[BSSID_WCID], TRUE,
				    NULL);
		...
		if (Status != NDIS_STATUS_SUCCESS)
		   ...
		   WLAN_PUB_ACTION_TDLS_DISCOVER_RES) { /* discover_req,setup,teardown,uapsd PTI */
#ifdef UAPSD_SUPPORT
		if (action_code == TDLS_ACTION_CODE_PEER_TRAFFIC_INDICATION)
...
INT cfg_tdls_EntryInfo_Display_Proc(IN PRTMP_ADAPTER pAd, IN PUCHAR arg)
{
	INT i;
	...
	for (i = 0; i < MAX_NUM_OF_CFG_TDLS_ENTRY; i++) {
		MTWF_LOG(DBG_CAT_PROTO, CATPROTO_TDLS, DBG_LVL_OFF,
			 ("TDLS Entry %d MAC %02x:%02x:%02x:%02x:%02x:%02x\n",
			  i,
			  PRINT_MAC(pAd->StaCfg[0]
					    .wpa_supplicant_info.CFG_Tdls_info
					    .TDLSEntry[i]
					    .MacAddr)));
		...
		if (pAd->StaCfg[0]
			...
			/* MTWF_LOG(DBG_CAT_PROTO, CATPROTO_TDLS, DBG_LVL_OFF, ("\n")); */
			MTWF_LOG(DBG_CAT_PROTO, CATPROTO_TDLS, DBG_LVL_OFF,
				 ("TDLS Entry %d is valid, bInitiator = %d\n",
				  i,
				  pAd->StaCfg[0]
					  .wpa_supplicant_info.CFG_Tdls_info
					  .TDLSEntry[i]
...
VOID cfg_tdls_rx_parsing(PRTMP_ADAPTER pAd, RX_BLK *pRxBlk)
{
	UCHAR TDLS_LLC_SNAP_WITH_CATEGORY[] = {
		0xaa,	      0xaa, 0x03,
		0x00,	      0x00, 0x00,
		0x89,	      0x0d, PROTO_NAME_TDLS,
		CATEGORY_TDLS
	};

	if ((pRxBlk->DataSize >= (LENGTH_802_1_H + LENGTH_TDLS_H)) &&
	    os_equal_mem(TDLS_LLC_SNAP_WITH_CATEGORY, pRxBlk->pData,
			 LENGTH_802_1_H + 2)) {
		...
		if (TDLSType == TDLS_ACTION_CODE_SETUP_REQUEST ||
			...
			for (i = 0; i < MAX_NUM_OF_CFG_TDLS_ENTRY; i++) {
				if (pCfgTdls->TDLSEntry[i].EntryValid ==
					    FALSE &&
				    (MAC_ADDR_EQUAL(
					     pCfgTdls->TDLSEntry[i].MacAddr,
					     nullAddr) ||
				     MAC_ADDR_EQUAL(
					     pCfgTdls->TDLSEntry[i].MacAddr,
					     peerMAC))) {
					tdls_entry_index =
						i; /* found an empty entry */
					break;
				}
			}
			...
			if (i == MAX_NUM_OF_CFG_TDLS_ENTRY) {
				MTWF_LOG(
					DBG_CAT_PROTO, CATPROTO_TDLS,
					DBG_LVL_OFF,
					("%s() - TDLS LINK NUMBER EXCEED, WE ONLY SUPPORT CONCURRENT %d LINKs, Discard this setup\n",
					 __func__, MAX_NUM_OF_CFG_TDLS_ENTRY));
			...
			COPY_MAC_ADDR(
				pCfgTdls->TDLSEntry[tdls_entry_index].MacAddr,
				peerMAC);

			if (TDLSType == TDLS_ACTION_CODE_SETUP_REQUEST) {
				Ptr += LENGTH_802_1_H + LENGTH_TDLS_PAYLOAD_H +
				       1 + 2; /* token 1 + capa 2 */
				RemainLen -= LENGTH_802_1_H +
					     LENGTH_TDLS_PAYLOAD_H + 1 +
					     2; /* token 1 + capa 2 */
			} else {
				pCfgTdls->TDLSEntry[tdls_entry_index]
					.bInitiator = TRUE;
				MTWF_LOG(
					DBG_CAT_PROTO, CATPROTO_TDLS,
					DBG_LVL_OFF,
					("%s() - TDLSEntry[%d] Initiator = %d\n",
					 __func__, tdls_entry_index,
					 pAd->StaCfg[0]
						 .wpa_supplicant_info
						 .CFG_Tdls_info
						 .TDLSEntry[tdls_entry_index]
				...
				Ptr += LENGTH_802_1_H + LENGTH_TDLS_PAYLOAD_H +
				       2 + 1 +
				       2; /* status 2 + token 1 + capa 2 */
				RemainLen -=
					LENGTH_802_1_H + LENGTH_TDLS_PAYLOAD_H +
					2 + 1 +
					2; /* status 2 + token 1 + capa 2 */
			}
			...
			while ((Length + 2 + pEid->Len) <= RemainLen) {
				...
				case IE_VENDOR_SPECIFIC:
					...
					MTWF_LOG(
						DBG_CAT_PROTO, CATPROTO_TDLS,
						DBG_LVL_ERROR,
						("%s : RCV QosCapa 0x[%x] from peer MAC: (%02X:%02X:%02X:%02X:%02X:%02X)\n",
						 __func__,
						 pAd->StaCfg[0]
							 .wpa_supplicant_info
							 .CFG_Tdls_info
					...
					if (wdev->PhyMode >= PHY_11ABGN_MIXED) {
						if (pEid->Len >=
							...
							pCfgTdls->TDLSEntry
								[tdls_entry_index]
									.HtCapabilityLen =
								SIZE_HT_CAP_IE; /* Nnow we only support 26 bytes. */
						}
					}
				...
				pEid = (PEID_STRUCT)((UCHAR *)pEid + 2 +
						     pEid->Len);
			}

#endif /*UAPSD_SUPPORT*/
		} else if (TDLSType ==
			   TDLS_ACTION_CODE_CHANNEL_SWITCH_REQUEST) {
			/* DBGPRINT(RT_DEBUG_ERROR, ("\nPeerTdlsChannelSwitchReq!!\n")); */
			CFG_TDLS_CHSW_PARAM TdlsCHSWParam;

			os_zero_mem(&TdlsCHSWParam,
				    sizeof(CFG_TDLS_CHSW_PARAM));
			MTWF_LOG(DBG_CAT_PROTO, CATPROTO_TDLS, DBG_LVL_ERROR,
				 ("GOT CHSW REQ! %ld !!!\n",
				  (jiffies * 1000) / OS_HZ));
			...
			if (pCfgTdls->IamInOffChannel) {
				...
				if (pCfgTdls->TargetOffChannel ==
				    pCfgTdls->BaseChannel) { /* STOPCHSW */
					MTWF_LOG(
						DBG_CAT_PROTO, CATPROTO_TDLS,
						DBG_LVL_OFF,
						("%s() - GOT SW Req in off channel with target = base %d, STOP CH SWITCH!!\n",
						 __func__,
						 pCfgTdls->BaseChannel));
					pCfgTdls->bDoingPeriodChannelSwitch =
						FALSE;
				}

				/* gen chsw_resp */
				cfg_tdls_chsw_resp(pAd, peerMAC, 11000, 19000,
						   0);
			...
			if (pAd->StaActive.ExtCapInfo.TDLSChSwitchProhibited ==
				...
				cfg_tdls_chsw_resp(pAd, peerMAC,
						   pCfgTdls->ChSwitchTime,
						   pCfgTdls->ChSwitchTimeout,
						   37);
				MTWF_LOG(
					...
					("%s() - AP prohibit TDLS CH SWITH (%d), STOP!!\n",
					 __func__,
					 pAd->StaActive.ExtCapInfo
						 .TDLSChSwitchProhibited));
				return;
			}
			...
			pCfgTdls->OrigTargetOffChannel =
				pCfgTdls->TargetOffChannel;
			...
			RTEnqueueInternalCmd(pAd,
					     ...
					     sizeof(CFG_TDLS_CHSW_PARAM));
			mdelay(1);
			cfg_tdls_chsw_resp(pAd, peerMAC, pCfgTdls->ChSwitchTime,
					   pCfgTdls->ChSwitchTimeout, 0);
		} else if (TDLSType ==
			   TDLS_ACTION_CODE_CHANNEL_SWITCH_RESPONSE) {
			CFG_TDLS_CHSW_PARAM TdlsCHSWParam;
			USHORT StatusCode = 0;
			...
			if (pCfgTdls->IamInOffChannel) {
				...
				/* TdlsCHSWParam.cmd = 1; */
				/* RTEnqueueInternalCmd(pAd, CMDTHREAD_TDLS_SEND_CH_SW_SETUP, &TdlsCHSWParam, sizeof(CFG_TDLS_CHSW_PARAM)); */
			} else if (pAd->StaCfg[0].wpa_supplicant_info
						   .CFG_Tdls_info.IsentCHSW ==
					   TRUE &&
				   pAd->StaCfg[0].wpa_supplicant_info
						   .CFG_Tdls_info
						   .bChannelSwitchInitiator ==
					   TRUE) {
				pAd->StaCfg[0]
					.wpa_supplicant_info.CFG_Tdls_info
					.IsentCHSW =
					FALSE; /* to prevent receiving off channel CHSW resp in base channel */
				...
				if (RemainLen < 2) {
					MTWF_LOG(
						DBG_CAT_PROTO, CATPROTO_TDLS,
						DBG_LVL_ERROR,
						("PeerTdlsChannelSwitchRspSanity --> Invaild packet length - (status code)\n"));
					return;
				}
				...
				while ((Length + 2 + pEid->Len) <= RemainLen) {
					...
					case IE_TDLS_LINK_IDENTIFIER:
						if (pEid->Len !=
							...
							MTWF_LOG(
								...
								CATPROTO_TDLS,
								DBG_LVL_WARN,
								("PeerTdlsChannelSwitchRspSanity - wrong IE_TDLS_LINK_IDENTIFIER.\n"));
					...
					case IE_TDLS_CHANNEL_SWITCH_TIMING:
						if (pEid->Len == 4) {
							...
							MTWF_LOG(
								...
								CATPROTO_TDLS,
								DBG_LVL_ERROR,
								("PeerTdlsChannelSwitchRspSanity - wrong IE_TDLS_CHANNEL_SWITCH_TIMING.\n"));
					...
					default:
						...
						MTWF_LOG(
							...
							CATPROTO_TDLS,
							DBG_LVL_ERROR,
							("PeerTdlsChannelSwitchRspSanity - unrecognized EID = %d\n",
							 pEid->Eid));
						break;
					}
				...
				pCfgTdls->OffChannelStayTime =
					pAd->StaCfg[0].DtimPeriod * 100 -
					pCfgTdls->BaseChannelStayTime;
				pCfgTdls->BaseChannel = wdev->channel;
				pCfgTdls->bDoingPeriodChannelSwitch = TRUE;
				/* RTMP_OS_NETDEV_STOP_QUEUE(pAd->net_dev); */
				/* RTMP_SET_MORE_FLAG(pAd, fRTMP_ADAPTER_DISABLE_DEQUEUE); */
				/* CFG_TDLS_KYLE Send TDLS_CH_SW_START */
				COPY_MAC_ADDR(pCfgTdls->CHSWPeerMacAddr,
					      peerMAC);
				...
				MTWF_LOG(
					...
					("TdlsCHSWParam.offch_center : %d TdlsCHSWParam.offch_prim: %d pCfgTdls->TargetOffChannelExt: %d\n",
					 ...
					 TdlsCHSWParam.offch_prim,
					 pCfgTdls->TargetOffChannelExt));
				TdlsCHSWParam.bw_off =
					pCfgTdls->TargetOffChannelBW;
				TdlsCHSWParam.role =
					pCfgTdls->bChannelSwitchInitiator;
				/* TdlsCHSWParam.stay_time = pCfgTdls->OffChannelStayTime; */
				TdlsCHSWParam.switch_time =
					pCfgTdls->ChSwitchTime;
				TdlsCHSWParam.switch_timeout =
					pCfgTdls->ChSwitchTimeout;
				TdlsCHSWParam.start_time_tsf =
					pRxBlk->TimeStamp;
				RTEnqueueInternalCmd(
					pAd, CMDTHREAD_TDLS_SEND_CH_SW_SETUP,
					&TdlsCHSWParam,
					sizeof(CFG_TDLS_CHSW_PARAM));
			}
		}
			 ...
			 TDLS_ACTION_CODE_PEER_TRAFFIC_INDICATION) { /* TDLS_ACTION_CODE_PEER_TRAFFIC_INDICATION */
			peerMAC = pRxBlk->pHeader->Addr3;
			dialog_token = *(pRxBlk->pData + LENGTH_802_1_H + 3);
			cfg_tdls_rcv_PeerTrafficIndication(pAd, dialog_token,
							   peerMAC);
...
VOID cfg_tdls_BaseChannelTimeoutAction(IN PVOID SystemSpecific1,
				       IN PVOID FunctionContext,
				       IN PVOID SystemSpecific2,
				       IN PVOID SystemSpecific3)
{
	...
	MTWF_LOG(DBG_CAT_PROTO, CATPROTO_TDLS, DBG_LVL_ERROR,
		 ("tdls CHSW base channel timeout!!!\n"));

	if (pCfgTdls->bChannelSwitchInitiator &&
	    pCfgTdls->bDoingPeriodChannelSwitch)
		cfg_tdls_chsw_req(pAd, pCfgTdls->CHSWPeerMacAddr,
				  pCfgTdls->OrigTargetOffChannel,
				  pCfgTdls->TargetOffChannelBW);
}

INT cfg_tdls_chsw_req(IN PRTMP_ADAPTER pAd, IN UINT8 *peer,
		      IN UINT8 target_channel, IN UINT8 target_bw)
{
	UCHAR TDLS_ETHERTYPE[] = { 0x89, 0x0d };
	UCHAR Header802_3[14];
	PUCHAR pOutBuffer = NULL;
	...
	MakeOutgoingFrame(pOutBuffer, &TempLen, 1, &RemoteFrameType,
			  END_OF_ARGS);
	FrameLen = FrameLen + TempLen;
	MakeOutgoingFrame(pOutBuffer + FrameLen, &TempLen, 1, &category, 1,
			  &action_code, END_OF_ARGS);
	FrameLen = FrameLen + TempLen;
	/* fill target channel */
	MakeOutgoingFrame(pOutBuffer + FrameLen, &TempLen, 1, &target_channel,
			  END_OF_ARGS);
	FrameLen = FrameLen + TempLen;
	...
	regclass = 22;
	MakeOutgoingFrame(pOutBuffer + FrameLen, &TempLen, 1, &regclass,
			  END_OF_ARGS);
	FrameLen = FrameLen + TempLen;
	...
	if (target_bw != 0) {
		...
		MakeOutgoingFrame(pOutBuffer + FrameLen, &TempLen, 1,
				  &NewExtChanIe, 1, &length, 1,
				  &target_ext_ch_offset, END_OF_ARGS);
		FrameLen = FrameLen + TempLen;
	} else {
	...
	do {
		...
		MakeOutgoingFrame(pOutBuffer + FrameLen, &TempLen, 1, &TDLS_IE,
				  1, &TDLS_IE_LEN, 2, &SwitchTimeBuf, 2,
				  &SwitchTimeOutBuf, END_OF_ARGS);
		FrameLen = FrameLen + TempLen;
	} while (0);
...
INT cfg_tdls_chsw_resp(IN PRTMP_ADAPTER pAd, IN UINT8 *peer,
		       IN UINT32 ch_sw_time, IN UINT32 ch_sw_timeout,
		       IN UINT16 reason_code)
{
	...
	MakeOutgoingFrame(pOutBuffer + FrameLen, &TempLen, 2, &reason_code,
			  END_OF_ARGS);
	FrameLen = FrameLen + TempLen;
	/* fill link identifier */
	tdls_entry_link_index = cfg_tdls_search_ValidLinkIndex(pAd, peer);
	...
	MTWF_LOG(
		DBG_CAT_PROTO, CATPROTO_TDLS, DBG_LVL_ERROR,
		("%s(): CHSW Resp Status:%d , SwitchTime:%d , SwitchTimeout:%d\n",
	...
	Status =
		RTMPAllocateNdisPacket(pAd, &pPacket, Header802_3, LENGTH_802_3,
				       pOutBuffer, (UINT)FrameLen);
	...
	if (!pAd->StaCfg[0].wpa_supplicant_info.CFG_Tdls_info.IamInOffChannel)
		RTMP_SET_PACKET_TDLS_WAIT_ACK(pPacket,
					      1); /* SET TDLS WAIT ACK */
	...
	if (wdev && wdev->tx_pkt_handle)
		...
		MTWF_LOG(DBG_CAT_PROTO, CATPROTO_TDLS, DBG_LVL_ERROR,
			 ("%s():Invalid wdev(%p) or tx_pkt_handle(%p)!\n",
			  __func__, wdev, (wdev ? wdev->tx_pkt_handle : NULL)));
		RELEASE_NDIS_PACKET(pAd, pPacket, NDIS_STATUS_FAILURE);
	}

	if (!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_BSS_SCAN_IN_PROGRESS))
		RTMPDeQueuePacket(pAd, FALSE, WMM_NUM_OF_AC, WCID_ALL,
				  MAX_TX_PROCESS);
...
VOID cfg_tdls_prepare_null_frame(PRTMP_ADAPTER pAd, BOOLEAN powersave,
				 UCHAR dir, UCHAR *peerAddr)
{
#ifdef MT_MAC
	return;
#else
	...
	RtmpPrepareHwNullFrame(pAd, &pAd->MacTab.Content[BSSID_WCID], FALSE,
			       FALSE, 0, OPMODE_STA, PWR_SAVE, 0, 0);
	RtmpPrepareHwNullFrame(pAd, pEntry_TDLS, TRUE, FALSE, 0, OPMODE_STA,
			       PWR_ACTIVE, TRUE, 1);
#endif
}
...
VOID cfg_tdls_auto_teardown(PRTMP_ADAPTER pAd, PMAC_TABLE_ENTRY pEntry)
{
	...
	cfg80211_tdls_oper_request(pAd->net_dev, pEntry->Addr,
				   NL80211_TDLS_TEARDOWN, 25, GFP_KERNEL);
	mdelay(1);
	cfg80211_tdls_oper_request(pAd->net_dev, pEntry->Addr,
				   NL80211_TDLS_DISABLE_LINK, 0, GFP_KERNEL);
#else
	cfg_tdls_build_frame(pAd, pEntry->Addr, 1, TDLS_ACTION_CODE_TEARDOWN, 0,
			     NULL, 0, FALSE, 0, 25);
	mdelay(1);
	CFG80211DRV_StaTdlsInsertDeletepEntry(pAd, pEntry->Addr,
					      tdls_delete_entry);
 ...
 * ==========================================================================
 */
VOID cfg_tdls_TunneledProbeRequest(PRTMP_ADAPTER pAd, PUCHAR pMacAddr,
				   const u8 *extra_ies, size_t extra_ies_len)
{
	...
	MAKE_802_3_HEADER(Header802_3, pMacAddr, pAd->CurrentAddress,
			  TDLS_ETHERTYPE);
	/* Allocate buffer for transmitting message */
	NStatus = MlmeAllocateMemory(pAd, &pOutBuffer);
	...
	MakeOutgoingFrame(pOutBuffer, &TempLen, 1, &RemoteFrameType, 1,
			  &Category, 3, &OUI, 1, &FrameBodyType, END_OF_ARGS);
	FrameLen = FrameLen + TempLen;

	/* add extra_ies */
	if (extra_ies_len != 0) {
		hex_dump("extra_ies from supplicant ", extra_ies,
			 extra_ies_len);
		MakeOutgoingFrame(pOutBuffer + FrameLen, &TempLen,
				  extra_ies_len, extra_ies, END_OF_ARGS);
		FrameLen = FrameLen + TempLen;
 ...
 * ==========================================================================
 */
VOID cfg_tdls_TunneledProbeResponse(PRTMP_ADAPTER pAd, PUCHAR pMacAddr,
				    const u8 *extra_ies, size_t extra_ies_len)
{
	UCHAR TDLS_ETHERTYPE[] = { 0x89, 0x0d };
	UCHAR Header802_3[14];
