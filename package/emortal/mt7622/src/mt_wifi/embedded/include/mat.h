...
#define MAT_TB_ENTRY_AGEOUT_TIME                                               \
	(5 * 60 *                                                              \
	 OS_HZ) /* 30000, 5min. MAT convert table entry age-out time interval. now set it as 5min. */

/* 802.3 Ethernet related definition */
#define MAT_ETHER_HDR_LEN 14 /* dstMac(6) + srcMac(6) + protoType(2) */
#define MAT_VLAN_ETH_HDR_LEN                                                   \
	(MAT_ETHER_HDR_LEN +                                                   \
	 4) /* 4 for h_vlan_TCI and h_vlan_encapsulated_proto */

#define MAT_MAC_ADDR_HASH(Addr)                                                \
	(Addr[0] ^ Addr[1] ^ Addr[2] ^ Addr[3] ^ Addr[4] ^ Addr[5])
#define MAT_MAC_ADDR_HASH_INDEX(Addr)                                          \
	(MAT_MAC_ADDR_HASH(Addr) % MAT_MAX_HASH_ENTRY_SUPPORT)

#define isMcastEtherAddr(addr) (addr[0] & 0x1)
#define isBcastEtherAddr(addr)                                                 \
	((addr[0] & addr[1] & addr[2] & addr[3] & addr[4] & addr[5]) == 0xff)
#define isZeroEtherAddr(addr)                                                  \
	(!(addr[0] | addr[1] | addr[2] | addr[3] | addr[4] | addr[5]))

#define IS_GROUP_MAC(Addr) ((Addr[0]) & 0x01)
#define IS_UCAST_MAC(addr)                                                     \
	(!(isMcastEtherAddr(addr) ||                                           \
	   isZeroEtherAddr(                                                    \
		   addr))) /* isUcastMac = !(00:00:00:00:00:00 || mcastMac); */
#define IS_EQUAL_MAC(a, b)                                                     \
...
#define IPMAC_TB_HASH_ENTRY_NUM                                                \
	(MAT_MAX_HASH_ENTRY_SUPPORT + 1) /* One entry for broadcast address */
#define IPMAC_TB_HASH_INDEX_OF_BCAST                                           \
	MAT_MAX_HASH_ENTRY_SUPPORT /* cause hash index start from 0. */

#define MAT_IP_ADDR_HASH(Addr)                                                 \
	(((Addr >> 24) & 0xff) ^ ((Addr >> 16) & 0xff) ^                       \
	 ((Addr >> 8) & 0xff) ^ (Addr & 0xff))
#define MAT_IP_ADDR_HASH_INDEX(Addr)                                           \
	(MAT_IP_ADDR_HASH(Addr) % MAT_MAX_HASH_ENTRY_SUPPORT)

#define IS_GOOD_IP(IP) (IP != 0)
#define IS_MULTICAST_IP(IP) (((UINT32)(IP)&0xf0000000) == 0xe0000000)

/* IPv6 related definition */
#define IPV6MAC_TB_HASH_ENTRY_NUM                                              \
	(MAT_MAX_HASH_ENTRY_SUPPORT + 1) /* One entry for broadcast address */
#define IPV6MAC_TB_HASH_INDEX_OF_BCAST                                         \
	MAT_MAX_HASH_ENTRY_SUPPORT /* cause hash index start from 0. */
...
#define MAT_IPV6_ADDR_HASH(Addr)                                               \
	((Addr[10] & 0xff) ^ (Addr[13] & 0xff) ^ (Addr[14] & 0xff) ^           \
	 (Addr[15] & 0xff))
#define MAT_IPV6_ADDR_HASH_INDEX(Addr)                                         \
	(MAT_IPV6_ADDR_HASH(Addr) % MAT_MAX_HASH_ENTRY_SUPPORT)
...
#define IS_LOOPBACK_IPV6_ADDR(_addr)                                           \
	(NdisEqualMemory((UCHAR *)(&((_addr).ipv6_addr[0])),                   \
			 &IPV6_LOOPBACKADDR[0], IPV6_ADDR_LEN))
#define IS_MULTICAST_IPV6_ADDR(_addr) (((_addr).ipv6_addr[0] & 0xff) == 0xff)
...
typedef struct _MATProtoOps {
	...
	PUCHAR(*tx)
	(MAT_STRUCT *pMatCfg, PNDIS_PACKET pSkb, PUCHAR pLayerHdr,
	 PUCHAR pMacAddr);
	PUCHAR(*rx)
	(MAT_STRUCT *pMatCfg, PNDIS_PACKET pSkb, PUCHAR pLayerHdr,
	 PUCHAR pMacAddr);
