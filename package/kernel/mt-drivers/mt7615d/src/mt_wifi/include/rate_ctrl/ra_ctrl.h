...
#if (PRODUCT_VERSION == 7615 || PRODUCT_VERSION == 7637 ||                     \
     PRODUCT_VERSION == 7622)
...
} EXT_EVENT_G_BAND_256QAM_PROBE_RESULT_T,
	*P_EXT_EVENT_G_BAND_256QAM_PROBE_RESULT_T;
...
typedef enum { RAL_OLD_DRS, RAL_NEW_DRS, RAL_QUICK_DRS } RA_LOG_TYPE;
...
********************************************************************************
*/
#define PTX_RA_LEGACY_ENTRY(pTable, idx)                                       \
	((RTMP_RA_LEGACY_TB *)&(pTable[(idx + 1) * 5]))
#define RATE_TABLE_SIZE(pTable)                                                \
	((pTable)[0]) /* Byte 0 is number of rate indices */
#define RATE_TABLE_INIT_INDEX(pTable)                                          \
	((pTable)[1]) /* Byte 1 is initial rate index */
#define PTX_RA_GRP_ENTRY(pTable, idx)                                          \
	((RTMP_RA_GRP_TB *)&(pTable[(idx + 1) * 10]))

#define RA_AGBS_ENTRY(pTable, idx)                                             \
	((idx < pTable[0] ? (RA_AGBS_TABLE_ENTRY *)&(pTable[(idx + 1) * 14]) : \
				  (RA_AGBS_TABLE_ENTRY *)&(pTable[(1) * 14])))
...
#ifdef DOT11_VHT_AC
/* VHT */
#define ADAPT_RATE_TABLE(pTable)                                               \
	((pTable) == RateSwitchTableAdapt11B ||                                \
	 ...
	 (pTable) == RateTableVht1S || (pTable) == RateTableVht1S_MCS9 ||      \
	 (pTable) == RateTableVht2S || (pTable) == RateTableVht2S_MCS7 ||      \
	 (pTable) == RateTableVht2S_BW20 || (pTable) == RateTableVht2S_BW40)
#else
/* 11n */
#define ADAPT_RATE_TABLE(pTable)                                               \
	((pTable) == RateSwitchTableAdapt11B ||                                \
...
#define RATE_TABLE_AGBS(pTable)                                                \
	((pTable) == RateSwitchTableAGBS11B ||                                 \
...
#define RA_SAVE_LAST_TX_CFG(__pRaEntry)                                        \
	{                                                                      \
		...
		g_rRaCfg.rLastTxCfg.ShortGI =                                  \
			__pRaEntry->TxPhyCfg.ShortGI ? 1 : 0;                  \
...
*/
#ifdef MT_MAC
CHAR raMaxRssi(IN struct _RA_COMMON_INFO_T *pRaCfg, IN CHAR Rssi0,
	       IN CHAR Rssi1, IN CHAR Rssi2);

CHAR raMinRssi(IN struct _RA_COMMON_INFO_T *pRaCfg, IN CHAR Rssi0,
	       IN CHAR Rssi1, IN CHAR Rssi2);

VOID raWrapperEntrySet(IN struct _RTMP_ADAPTER *pAd,
		       IN struct _MAC_TABLE_ENTRY *pEntry,
		       OUT struct _RA_ENTRY_INFO_T *pRaEntry);

VOID raWrapperEntryRestore(IN struct _RTMP_ADAPTER *pAd,
			   IN struct _MAC_TABLE_ENTRY *pEntry,
			   IN struct _RA_ENTRY_INFO_T *pRaEntry);

VOID raWrapperConfigSet(IN struct _RTMP_ADAPTER *pAd, IN struct wifi_dev *wdev,
			OUT struct _RA_COMMON_INFO_T *pRaCfg);

VOID QuickResponeForRateAdaptMT(IN struct _RTMP_ADAPTER *pAd, UINT_8 idx);

VOID DynamicTxRateSwitchingAdaptMT(IN struct _RTMP_ADAPTER *pAd, UINT_8 idx);

VOID raSelectTxRateTable(IN struct _RA_ENTRY_INFO_T *pRaEntry,
...
VOID RAInit(IN struct _RTMP_ADAPTER *pAd, IN struct _MAC_TABLE_ENTRY *pEntry);

VOID RAParamUpdate(IN struct _RTMP_ADAPTER *pAd,
		   IN struct _MAC_TABLE_ENTRY *pEntry,
		   IN struct _STAREC_AUTO_RATE_UPDATE_T *prParam);

VOID RATriggerQuickResponeTimer(IN struct _RTMP_ADAPTER *pAd,
				IN struct _RA_ENTRY_INFO_T *pRaEntry);

#ifdef NEW_RATE_ADAPT_SUPPORT
VOID raClearTxQuality(IN struct _RA_INTERNAL_INFO_T *pRaInternal);

VOID raDecTxQuality(IN struct _RA_INTERNAL_INFO_T *pRaInternal,
		    IN UCHAR ucRateIndex);

VOID raSetTxQuality(IN struct _RA_INTERNAL_INFO_T *pRaInternal,
		    IN UINT_8 ucRateIndex, IN UINT_8 ucQuality);

UINT_8
raGetTxQuality(IN struct _RA_INTERNAL_INFO_T *pRaInternal,
	       IN UINT_8 ucRateIndex);

VOID raRestoreLastRate(IN struct _RA_INTERNAL_INFO_T *pRaInternal);

VOID raSetMcsGroup(IN struct _RA_ENTRY_INFO_T *pRaEntry,
		   IN struct _RA_COMMON_INFO_T *pRaCfg,
		   IN struct _RA_INTERNAL_INFO_T *pRaInternal);

UCHAR
raSelectUpRate(IN struct _RA_ENTRY_INFO_T *pRaEntry,
	       IN struct _RA_COMMON_INFO_T *pRaCfg,
	       IN struct _RA_INTERNAL_INFO_T *pRaInternal,
	       IN struct _RTMP_RA_GRP_TB *pCurrTxRate);

UCHAR
raSelectDownRate(IN struct _RA_ENTRY_INFO_T *pRaEntry,
		 IN struct _RA_COMMON_INFO_T *pRaCfg,
		 IN struct _RA_INTERNAL_INFO_T *pRaInternal,
		 IN UCHAR CurrRateIdx);

UCHAR *raSelectTxRateTableGRP(IN struct _RA_ENTRY_INFO_T *pRaEntry,
			      IN struct _RA_COMMON_INFO_T *pRaCfg,
			      IN struct _RA_INTERNAL_INFO_T *pRaInternal);

VOID QuickResponeForRateAdaptMTCore(IN struct _RTMP_ADAPTER *pAd,
				    IN struct _RA_ENTRY_INFO_T *pRaEntry,
				    IN struct _RA_COMMON_INFO_T *pRaCfg,
				    IN struct _RA_INTERNAL_INFO_T *pRaInternal);

VOID DynamicTxRateSwitchingAdaptMtCore(
	IN struct _RTMP_ADAPTER *pAd, IN struct _RA_ENTRY_INFO_T *pRaEntry,
	IN struct _RA_COMMON_INFO_T *pRaCfg,
	IN struct _RA_INTERNAL_INFO_T *pRaInternal);

VOID NewTxRateMtCore(IN struct _RTMP_ADAPTER *pAd,
		     IN struct _RA_ENTRY_INFO_T *pRaEntry,
		     IN struct _RA_COMMON_INFO_T *pRaCfg,
		     IN struct _RA_INTERNAL_INFO_T *pRaInternal);
#endif /* NEW_RATE_ADAPT_SUPPORT */
...
#ifdef RATE_ADAPT_AGBS_SUPPORT
VOID raClearTxQualityAGBS(IN struct _RA_INTERNAL_INFO_T *pRaInternal);

VOID raPrintRateTableNameAGBS(IN UCHAR *pTable);

UINT_16
raGetPhyRate(IN UINT_8 ucMode, IN UINT_8 ucMcs, IN UINT_8 ucVhtNss,
	     IN UINT_8 ucBw, IN UINT_8 ucSgi);

VOID raSetMcsGroupAGBS(IN struct _RA_ENTRY_INFO_T *pRaEntry,
		       IN struct _RA_COMMON_INFO_T *pRaCfg,
		       IN struct _RA_INTERNAL_INFO_T *pRaInternal);

VOID QuickResponeForRateAdaptAGBSMTCore(
	IN struct _RTMP_ADAPTER *pAd, IN struct _RA_ENTRY_INFO_T *pRaEntry,
	IN struct _RA_COMMON_INFO_T *pRaCfg,
	IN struct _RA_INTERNAL_INFO_T *pRaInternal);

VOID DynamicTxRateSwitchingAGBSMtCore(
	IN struct _RTMP_ADAPTER *pAd, IN struct _RA_ENTRY_INFO_T *pRaEntry,
	IN struct _RA_COMMON_INFO_T *pRaCfg,
	IN struct _RA_INTERNAL_INFO_T *pRaInternal);

UCHAR *raSelectVHTTxRateTableAGBS(IN struct _RA_ENTRY_INFO_T *pRaEntry,
				  IN struct _RA_COMMON_INFO_T *pRaCfg,
				  IN struct _RA_INTERNAL_INFO_T *pRaInternal);

UCHAR *raSelectTxRateTableAGBS(IN struct _RA_ENTRY_INFO_T *pRaEntry,
			       IN struct _RA_COMMON_INFO_T *pRaCfg,
			       IN struct _RA_INTERNAL_INFO_T *pRaInternal);

VOID SetTxRateMtCoreAGBS(IN struct _RTMP_ADAPTER *pAd,
			 IN struct _RA_ENTRY_INFO_T *pRaEntry,
			 IN struct _RA_COMMON_INFO_T *pRaCfg,
			 IN struct _RA_INTERNAL_INFO_T *pRaInternal);

VOID raMaxAmsduLenNotifyAGBS(IN struct _RA_ENTRY_INFO_T *pRaEntry,
			     IN struct _RA_INTERNAL_INFO_T *pRaInternal);
#endif /* RATE_ADAPT_AGBS_SUPPORT */
...
#ifdef NEW_RATE_ADAPT_SUPPORT
VOID MtAsicMcsLutUpdateCore(IN struct _RTMP_ADAPTER *pAd,
			    IN struct _RA_ENTRY_INFO_T *pRaEntry,
			    IN struct _RA_COMMON_INFO_T *pRaCfg,
			    IN struct _RA_INTERNAL_INFO_T *pRaInternal);
#endif /* NEW_RATE_ADAPT_SUPPORT */
...
#ifdef RATE_ADAPT_AGBS_SUPPORT
VOID MtAsicMcsLutUpdateCoreAGBS(IN struct _RTMP_ADAPTER *pAd,
				IN struct _RA_ENTRY_INFO_T *pRaEntry,
				IN struct _RA_COMMON_INFO_T *pRaCfg,
				IN struct _RA_INTERNAL_INFO_T *pRaInternal);
#endif /* RATE_ADAPT_AGBS_SUPPORT */
...
#ifdef DBG
INT Set_Fixed_Rate_Proc(IN struct _RTMP_ADAPTER *pAd, IN RTMP_STRING *arg);

INT Set_Fixed_Rate_With_FallBack_Proc(IN struct _RTMP_ADAPTER *pAd,
				      IN RTMP_STRING *arg);

INT Set_RA_Debug_Proc(IN struct _RTMP_ADAPTER *pAd, IN RTMP_STRING *arg);
#endif /* DBG */

#ifdef CONFIG_RA_PHY_RATE_SUPPORT
INT Set_SupRateSet_Proc(IN struct _RTMP_ADAPTER *pAd, IN RTMP_STRING *arg);

INT Set_HtSupRateSet_Proc(IN struct _RTMP_ADAPTER *pAd, IN RTMP_STRING *arg);

INT Set_VhtSupRateSet_Proc(IN struct _RTMP_ADAPTER *pAd, IN RTMP_STRING *arg);
#endif /* CONFIG_RA_PHY_RATE_SUPPORT */

#if defined(MT7615) || defined(MT7622) || defined(P18) || defined(MT7663)
INT32
BssInfoRACommCfgSet(IN P_RA_COMMON_INFO_T pRaCfg,
		    OUT P_CMD_BSSINFO_AUTO_RATE_CFG_T pCmdBssInfoAutoRateCfg);
#endif /* defined(MT7615) || defined(MT7622) || defined(P18) || defined(MT7663) */

#if defined(MT7636) || defined(MT7615) || defined(MT7637) ||                   \
	defined(MT7622) || defined(P18) || defined(MT7663)
INT32
StaRecAutoRateParamSet(IN struct _RA_ENTRY_INFO_T *pRaEntry,
		       OUT struct _STAREC_AUTO_RATE_T *pCmdStaRecAutoRate);

INT32
StaRecAutoRateUpdate(IN struct _RA_ENTRY_INFO_T *pRaEntry,
		     IN struct _RA_INTERNAL_INFO_T *pRaInternal,
...
*/
#if !defined(COMPOS_WIN) && !defined(WIFI_BUILD_RAM)
VOID RTMPSetSupportMCS(IN struct _RTMP_ADAPTER *pAd, IN UCHAR OpMode,
		       IN struct _MAC_TABLE_ENTRY *pEntry, IN UCHAR SupRate[],
		       IN UCHAR SupRateLen, IN UCHAR ExtRate[],
		       IN UCHAR ExtRateLen,
#ifdef DOT11_VHT_AC
		       IN UCHAR vht_cap_len, IN struct _VHT_CAP_IE *vht_cap,
#endif /* DOT11_VHT_AC */
