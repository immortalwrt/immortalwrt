...
UINT8 ECC_COST_TIME_DBG_LVL = DBG_LVL_INFO;
	...
	OUT BIG_INTEGER_EC_POINT **ec_point_res)
{
	...
	MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_NOISY,
			 ("==> %s()\n", __func__));
		...
		INOUT VOID **pub_key)
{
	...
	MTWF_LOG(DBG_CAT_SEC, CATSEC_ECC, DBG_LVL_ERROR, ("private key\n"));
	SAE_BN_PRINT(*priv_key);
	MTWF_LOG(DBG_CAT_SEC, CATSEC_ECC, DBG_LVL_ERROR, ("public key x\n"));
	SAE_BN_PRINT(pub->x);
	MTWF_LOG(DBG_CAT_SEC, CATSEC_ECC, DBG_LVL_ERROR, ("public key y\n"));
	SAE_BN_PRINT(pub->y);
	...
	OUT UINT32 *asn1_len)
{
	...
	*asn1_len = len;
	...
	hex_dump_with_cat_and_lvl("asn1 of sub_pub_key_info", asn1_out, len,
		DBG_CAT_SEC, CATSEC_ECC, DBG_LVL_OFF);
	...
	OUT BIG_INTEGER_EC_POINT **pub_key)
{
	...
	*group_bi = ec_group_bi;
	...
	if (ec_group_bi->ec_group->prime_len != len) {
		MTWF_LOG(DBG_CAT_SEC, CATSEC_ECC, DBG_LVL_ERROR,
				("%s, expected prime len = %d, peer prime_len = %d\n",
				__func__, ec_group_bi->ec_group->prime_len, len / 2));
		fail_reason = 8;
		goto Fail;
	}
	...
	OUT UINT32 *asn1_len)
{
	...
	*asn1_len = len;
	...
	hex_dump_with_cat_and_lvl("asn1 of ecdsa_sig_value", asn1_out, len,
		DBG_CAT_SEC, CATSEC_ECC, DBG_LVL_TRACE);
