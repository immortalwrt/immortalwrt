...
#define MLME_TASK_EXEC_MULTIPLE                                                \
	10 /*5*/ /* MLME_TASK_EXEC_MULTIPLE * MLME_TASK_EXEC_INTV = 1 sec */
...
#define BEACON_LOST_TIME (12 * OS_HZ) /* 2048 msec = 2 sec */
	...
	defined(MT7637_FPGA) || defined(MT7615_FPGA) ||                        \
...
#define FAST_ACTIVE_SCAN_TIME                                                  \
	30 /* Active scan waiting for probe response time */
...
#define RSSI_FOR_LOW_TX_POWER                                                  \
	-45 /* -45 db is considered very short distance and */
/* eligible to use a lower TX power */
#define RSSI_FOR_LOWEST_TX_POWER -30
/*#define MID_TX_POWER_DELTA          0   // 0 db from full TX power upon mid-distance to AP */
#define LOW_TX_POWER_DELTA                                                     \
	6 /* -3 db from full TX power upon very short distance. 1 grade is 0.5 db */
#define LOWEST_TX_POWER_DELTA                                                  \
	16 /* -8 db from full TX power upon shortest distance. 1 grade is 0.5 db */
...
enum SCAN_MODE {
	...
	SCAN_CISCO_NOISE =
		0x82, /* single channel only, for noise histogram collection */
	SCAN_CISCO_CHANNEL_LOAD =
		0x83, /* single channel only, for channel load collection */
	SCAN_PASSIVE_MAX,
...
#define SCAN_MODE_VALID(_x)                                                    \
	((SCAN_MODE_ACT(_x) && ((_x) < SCAN_ACTIVE_MAX)) ||                    \
...
#define TID_MAC_HASH_INDEX(Addr, TID)                                          \
	(TID_MAC_HASH(Addr, TID) & (HASH_TABLE_SIZE - 1))
...
#define SEQ_SMALLER(_SEQ1, _SEQ2, _Limit)                                      \
	(((_SEQ1 - _SEQ2) & ((_Limit + 1) >> 1)))
#define SEQ_LARGER(_SEQ1, _SEQ2, _Limit)                                       \
	((_SEQ1 != _SEQ2) && !(((_SEQ1 - _SEQ2) & ((_Limit + 1) >> 1))))
#define SEQ_WITHIN_WIN(_SEQ1, _SEQ2, _WIN, _Limit)                             \
	(SEQ_LARGER(_SEQ1, _SEQ2, _Limit) &&                                   \
...
typedef struct _BSS_ENTRY {
	...
	ExtCapInfo; /* this is the extened capibility IE appreed in MGMT frames. Doesn't need to update once set in Init. */
	UCHAR NewExtChanOffset;
	CHAR Rssi;
	...
	AuthModeAux; /* Addition mode for WPA2 / WPA capable AP */
	NDIS_802_11_AUTHENTICATION_MODE AuthMode;
	NDIS_802_11_WEP_STATUS
	WepStatus; /* Unicast Encryption Algorithm extract from VAR_IE */
...
#ifdef APCLI_SUPPORT
typedef VOID (*APCLI_STATE_MACHINE_FUNC)(VOID *pAd, MLME_QUEUE_ELEM *Elem,
					 PULONG pCurrState, USHORT ifIndex);
...
#define COPY_HTSETTINGS_FROM_MLME_AUX_TO_ACTIVE_CFG(_pAd, _pEntry, _pStaCfg)   \
	{                                                                      \
		_pStaCfg->StaActive.SupportedHtPhy.MimoPs =                    \
			_pStaCfg->MlmeAux.HtCapability.HtCapInfo.MimoPs;       \
		_pStaCfg->StaActive.SupportedHtPhy.GF =                        \
			_pStaCfg->MlmeAux.HtCapability.HtCapInfo.GF;           \
		_pStaCfg->StaActive.SupportedHtPhy.ShortGIfor20 =              \
			_pStaCfg->MlmeAux.HtCapability.HtCapInfo.ShortGIfor20; \
		_pStaCfg->StaActive.SupportedHtPhy.ShortGIfor40 =              \
			_pStaCfg->MlmeAux.HtCapability.HtCapInfo.ShortGIfor40; \
		_pStaCfg->StaActive.SupportedHtPhy.TxSTBC =                    \
			_pStaCfg->MlmeAux.HtCapability.HtCapInfo.TxSTBC;       \
		_pStaCfg->StaActive.SupportedHtPhy.RxSTBC =                    \
			_pStaCfg->MlmeAux.HtCapability.HtCapInfo.RxSTBC;       \
		_pStaCfg->StaActive.SupportedHtPhy.ExtChanOffset =             \
			_pStaCfg->MlmeAux.AddHtInfo.AddHtInfo.ExtChanOffset;   \
		_pStaCfg->StaActive.SupportedHtPhy.OperaionMode =              \
			_pStaCfg->MlmeAux.AddHtInfo.AddHtInfo2.OperaionMode;   \
		_pStaCfg->StaActive.SupportedHtPhy.NonGfPresent =              \
			_pStaCfg->MlmeAux.AddHtInfo.AddHtInfo2.NonGfPresent;   \
		NdisMoveMemory((_pEntry)->HTCapability.MCSSet,                 \
			       (_pStaCfg)->StaActive.SupportedPhyInfo.MCSSet,  \
			       sizeof(UCHAR) * 16);                            \
	}
...
#define COPY_AP_HTSETTINGS_FROM_BEACON(_pAd, _pHtCapability, _pEntry)          \
	{                                                                      \
		_pEntry->AMsduSize =                                           \
			(UCHAR)(_pHtCapability->HtCapInfo.AMsduSize);          \
		_pEntry->MmpsMode = (UCHAR)(_pHtCapability->HtCapInfo.MimoPs); \
		_pEntry->MaxRAmpduFactor =                                     \
			(UCHAR)(_pHtCapability->HtCapParm.MaxRAmpduFactor);    \
	}
...
typedef struct _QOSMAP_SET {
	...
	USHORT DSCP_Field
		[29]; /* Use to time out while slience, unit: second , set by UI */
