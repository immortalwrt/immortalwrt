	...
	IN UINT ifIndex)
{
	...
	if ((pAd->StaCfg[ifIndex].ReqVarIELen + 17) <= IW_CUSTOM_MAX) {
		...
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_TRACE, ("pAd->StaCfg[%d].ReqVarIELen + 17 > MAX_CUSTOM_LEN\n", ifIndex));
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (wdev->wdev_type == WDEV_TYPE_REPEATER) {
		if (!pAPEntry) {
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				("%s:ACPLI is not connected, return repeater connection\n", __func__));
			return;
		}
	...
	if (wdev->wdev_type == WDEV_TYPE_REPEATER) {
		...
		if (pAPEntry) {
			...
			if (IS_AKM_WPA3PSK(pStaCfg->wdev.SecConfig.AKMMap)) {
				...
				hex_dump_with_lvl("repeater mode: SecConfig.PMK:", (char *)pAPEntry->SecConfig.PMK, LEN_PMK, DBG_LVL_TRACE);
			}
#endif
	...
	if ((IS_AKM_OWE(wdev->SecConfig.AKMMap)) && (IS_AKM_OPEN_ONLY(pAPEntry->SecConfig.AKMMap))) {
		...
		MlmeFreeMemory(pOutBuffer);
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s:Set assoc Timeout(%ld)ms\n", __func__, Timeout));
