...
static int diag_statistic_proc_show(struct seq_file *m, void *v)
{
	...
	if (pCtrl == NULL) {
		DIAG_PRINT(DBG_LVL_ERROR, ("%s pCtrl is NULL\n", __func__));
		return -EFAULT;
	}
	pAd = pCtrl->pAd;
	if (pAd == NULL) {
		DIAG_PRINT(DBG_LVL_ERROR, ("%s pAd is NULL\n", __func__));
		return -EFAULT;
	}
	...
	DIAG_PRINT(DBG_LVL_TRACE, ("%s dbdc_idx=%d\n", __func__, dbdc_idx));
	...
	size_t count, loff_t *pos)
{
	...
	if ((val <= 300) && (val >= 60)) {
		...
		DIAG_PRINT(DBG_LVL_TRACE, ("%s set duraion %d, band=%d.\n", __func__, val, pCtrl->diag_band));
	} else {
...
static int diag_sta_proc_show(struct seq_file *m, void *v)
{
	...
	for (i = 0; i < wtbl_max_num; i++) {
		...
		if ((pCtrl->diag_band == DIAG_BAND_2G && pAd->ApCfg.MBSSID[apidx].wdev.channel > 14) ||
		...
		if (cap->fgRateAdaptFWOffload == TRUE) {
			...
			DIAG_PRINT(DBG_LVL_INFO, ("%s, tx_succ=%d, tx_total=%d\n",
				__func__, tx_suc, rTxStatResult.u4TotalTxCount));
...
void diag_get_snr(RTMP_ADAPTER *pAd, UINT16 wcid, UCHAR *pData)
{
	...
	MTWF_LOG(DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_INFO,
		("%s DiagAvgSnr:%d\n", __func__, pEntry->RssiSample.AvgSnr[0]));
}
#endif
...
void diag_proc_entry_init(ENUM_DIAG_BAND diag_band, DIAG_CTRL *pCtrl)
{
	...
	DIAG_PRINT(DBG_LVL_TRACE, ("%s diag_band:%d\n", __func__, diag_band));
...
void diag_ctrl_alloc(PRTMP_ADAPTER pAd)
{
	...
	for (dbdc_idx = 0; dbdc_idx < DBDC_BAND_NUM; dbdc_idx++) {
		...
		if (pCtrl == NULL) {
			DIAG_PRINT(DBG_LVL_ERROR,
				("%s fail to allocate pDiagCtrl memory dbdc_idx=%d\n", __func__, dbdc_idx));
			return;
		}
...
void diag_add_pid(OS_TASK *pTask)
{
	...
	if (!pTask) {
		DIAG_PRINT(DBG_LVL_ERROR, ("%s pTask is NULL\n", __func__));
		return;
	}
...
#endif
	if ((!pCtrl) || (!pCtrl->init_flag)) {
		DIAG_PRINT(DBG_LVL_ERROR, ("%s, pCtrl not init\n", __func__));
		return;
	}
...
void diag_conn_error_write(PRTMP_ADAPTER pAd)
{
	...
	for (dbdc_idx = 0; dbdc_idx < DBDC_BAND_NUM; dbdc_idx++) {
		...
		if ((!pCtrl) || (!pCtrl->init_flag)) {
			DIAG_PRINT(DBG_LVL_INFO, ("%s, diag not init dbdc_idx=%d\n", __func__, dbdc_idx));
			continue;
		}
		...
		if (IS_FILE_OPEN_ERR(fd)) {
			...
			if (IS_FILE_OPEN_ERR(fd)) {
				RtmpOSFSInfoChange(&osFSInfo, FALSE);
				DIAG_PRINT(DBG_LVL_ERROR, ("%s, open conn_error_file fail dbdc_idx=%d\n",
				__func__, dbdc_idx));
				continue;
			}
...
void diag_log_file_write(PRTMP_ADAPTER pAd)
{
	...
	for (dbdc_idx = 0; dbdc_idx < DBDC_BAND_NUM; dbdc_idx++) {
		...
		if (pCtrl->diag_enable == 0) {
			/*DIAG_PRINT(DBG_LVL_ERROR, ("%s, diag_log not enable dbdc_idx=%d\n", __func__, dbdc_idx)); */
			continue;
		}
		...
		if (IS_FILE_OPEN_ERR(fd)) {
			...
			if (IS_FILE_OPEN_ERR(fd)) {
				RtmpOSFSInfoChange(&osFSInfo, FALSE);
				DIAG_PRINT(DBG_LVL_ERROR, ("%s, open diag_log_file fail dbdc_idx=%d\n",
				__func__, dbdc_idx));
				continue;
			}
			pCtrl->diag_log_file_offset = 0;
...
void diag_miniport_mm_request(PRTMP_ADAPTER pAd, UCHAR *pData, UINT Length)
{
	...
	if (pMbssEntry) {
		...
		pCtrl = pAd->pDiagCtrl[dbdc_idx];
		if ((!pCtrl) || (!pCtrl->init_flag)) {
			DIAG_PRINT(DBG_LVL_ERROR, ("%s, pCtrl not init, dbdc_idx=%d\n", __func__, dbdc_idx));
			return;
		}
		if (pCtrl->diag_enable == 0) {
			/*DIAG_PRINT(DBG_LVL_ERROR, ("%s, diag_enable is 0, dbdc_idx=%d\n", __func__, dbdc_idx));*/
			return;
		}
...
void diag_dev_rx_mgmt_frm(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk)
{
	...
	if (pMbss) {
		...
		pCtrl = pAd->pDiagCtrl[dbdc_idx];
		if ((!pCtrl) || (!pCtrl->init_flag)) {
			DIAG_PRINT(DBG_LVL_TRACE, ("%s, pCtrl not init, dbdc_idx=%d\n", __func__, dbdc_idx));
			return;
		}
		if (pCtrl->diag_enable == 0) {
			/* DIAG_PRINT(DBG_LVL_ERROR, ("%s, diag_enable is 0, dbdc_idx=%d\n", __func__, dbdc_idx)); */
		...
		DIAG_PRINT(DBG_LVL_TRACE, ("%s, Error, no UP ap interface!!!\n", __func__));
}
...
BOOLEAN diag_proc_init(PRTMP_ADAPTER pAd, struct wifi_dev *wdev)
{
	...
	if (!pAd || !wdev) {
		DIAG_PRINT(DBG_LVL_ERROR, ("%s NULL parameters\n", __func__));
		return FALSE;
	}
	...
	pCtrl = pAd->pDiagCtrl[dbdc_idx];
	if (!pCtrl) {
		DIAG_PRINT(DBG_LVL_ERROR, ("%s pDiagCtrl not allocate dbdc_idx=%d\n", __func__, dbdc_idx));
		return FALSE;
	}
	if (pCtrl->init_flag == TRUE) {
		DIAG_PRINT(DBG_LVL_ERROR, ("%s pDiagCtrl already init dbdc_idx=%d\n", __func__, dbdc_idx));
		return FALSE;
	}
...
BOOLEAN diag_proc_exit(PRTMP_ADAPTER pAd, struct wifi_dev *wdev)
{
	...
	if (pAd->CommonCfg.dbdc_mode == TRUE)
		...
		dbdc_idx = 0;
#else
	dbdc_idx = 0;	  /* non-DBDC mode, just 1 band */
#endif

	pCtrl = pAd->pDiagCtrl[dbdc_idx];
	if (!pCtrl) {
		DIAG_PRINT(DBG_LVL_ERROR, ("%s pDiagCtrl is NULL dbdc_idx=%d\n", __func__, dbdc_idx));
		return FALSE;
	}
	if (pCtrl->init_flag == FALSE) {
		DIAG_PRINT(DBG_LVL_ERROR, ("%s pDiagCtrl already deinit dbdc_idx=%d\n", __func__, dbdc_idx));
		return FALSE;
	}
