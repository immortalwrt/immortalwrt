...
#define RTMP_BUILD_DRV_OPS_FUNCTION(_func)                                     \
	void Rtmp_Drv_Ops_##_func(VOID *__pDrvOps, VOID *__pNetOps,            \
				  VOID *__pPciConfig, VOID *__pUsbConfig)      \
	{                                                                      \
		RtmpDrvOpsInit(__pDrvOps, __pNetOps, __pPciConfig,             \
			       __pUsbConfig);                                  \
	}

/* #define RTMP_GET_DRV_OPS_FUNCTION(_func)	(PVOID)Rtmp_Drv_Ops_##_func */

#define RTMP_DRV_OPS_FUNCTION_BODY(_func) Rtmp_Drv_Ops_##_func
...
#define RTMP_OS_TASK_LEGALITY(__pTask) ((__pTask)->kthread_task != NULL)
...
#define RTMP_OS_TASK_GET(__pTask) ((OS_TASK *)((__pTask)->pContent))

#define RTMP_OS_TASK_DATA_GET(__pTask) RtmpOsTaskDataGet(__pTask)

#define RTMP_OS_TASK_IS_KILLED(__pTask) RtmpOsTaskIsKilled(__pTask)

#define RTMP_OS_TASK_WAKE_UP(__pTask) RtmpOsTaskWakeUp(pTask)

#define RTMP_OS_TASK_LEGALITY(__pTask) RtmpOsCheckTaskLegality(__pTask)
...
#define RTMP_SEM_EVENT_WAIT(_pSema, _status)                                   \
	((_status) = RtmpOsSemaWaitInterruptible((_pSema)))
...
/* */
#define NdisAllocateSpinLock(__pAd, __pLock)                                   \
	RtmpOsAllocateLock(__pLock, &(__pAd)->RscLockMemList)
#define NdisFreeSpinLock RtmpOsFreeSpinLock

#define RTMP_SEM_LOCK(__lock) RtmpOsSpinLockBh(__lock);

#define RTMP_SEM_UNLOCK(__lock) RtmpOsSpinUnLockBh(__lock);
...
#define RTMP_OS_INIT_COMPLETION(__pCompletion) init_completion(__pCompletion)
...
#else
#define RTMP_OS_REINIT_COMPLETION(__pCompletion) INIT_COMPLETION(*__pCompletion)
#endif
#define RTMP_OS_EXIT_COMPLETION(__pCompletion) complete(__pCompletion)

#define RTMP_OS_COMPLETE(__pCompletion) complete(__pCompletion)

#define RTMP_OS_WAIT_FOR_COMPLETION_TIMEOUT(__pCompletion, __Timeout)          \
	wait_for_completion_timeout(__pCompletion, __Timeout)

#ifdef WORKQUEUE_BH
#define RTMP_OS_TASKLET_SCHE(__pTasklet) schedule_work(__pTasklet)
#define RTMP_OS_TASKLET_INIT(__pAd, __pTasklet, __pFunc, __Data)               \
	INIT_WORK((struct work_struct *)__pTasklet, (work_func_t)__pFunc)
#define RTMP_OS_TASKLET_KILL(__pTasklet) cancel_work_sync(__pTasklet)
#else
#define RTMP_OS_TASKLET_SCHE(__pTasklet) tasklet_hi_schedule(__pTasklet)
#define RTMP_OS_TASKLET_INIT(__pAd, __pTasklet, __pFunc, __Data)               \
	tasklet_init(__pTasklet, __pFunc, __Data)
#define RTMP_OS_TASKLET_KILL(__pTasklet) tasklet_kill(__pTasklet)
#endif /* WORKQUEUE_BH */
...
#define RTMP_OS_COMPLETE(__pCompletion) RtmpOsComplete(__pCompletion)

#define RTMP_OS_WAIT_FOR_COMPLETION_TIMEOUT(__pCompletion, __Timeout)          \
	RtmpOsWaitForCompletionTimeout(__pCompletion, __Timeout)

#define RTMP_OS_TASKLET_SCHE(__pTasklet) RtmpOsTaskletSche(__pTasklet)

#define RTMP_OS_TASKLET_INIT(__pAd, __pTasklet, __pFunc, __Data)               \
	RtmpOsTaskletInit(__pTasklet, __pFunc, __Data,                         \
			  &(__pAd)->RscTaskletMemList)

#define RTMP_OS_TASKLET_KILL(__pTasklet) RtmpOsTaskletKill(__pTasklet)
...
#define APCLI_IF_UP_CHECK(pAd, ifidx)                                          \
	(RtmpOSNetDevIsUp((pAd)->ApCfg.ApCliTab[(ifidx)].wdev.if_dev) == TRUE)
