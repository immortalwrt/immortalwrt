...
static UINT16 fq_del_list_v2(RTMP_ADAPTER *pAd, struct dequeue_info *info,
			     CHAR deq_qid, UINT32 *tx_quota);

extern INT32 fp_fair_enq_dataq_pkt(RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
				   PNDIS_PACKET pkt, UCHAR q_idx);
extern INT32 fp_enq_dataq_pkt(RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
			      PNDIS_PACKET pkt, UCHAR q_idx);
...
INT fq_deinit(RTMP_ADAPTER *pAd)
{
	...
	pAd->fq_ctrl.enable =
		prev_enable | FQ_NO_PKT_STA_KEEP_IN_LIST | FQ_ARRAY_SCH;
...
static INT fq_add_list(RTMP_ADAPTER *pAd, UCHAR qidx, STA_TR_ENTRY *tr_entry)
{
	...
	if (pfq_sta && ((IsHit == 0) || (pAd->fq_ctrl.fq[qidx].Head == NULL))) {
		...
		InsertTailQueue(&pAd->fq_ctrl.fq[qidx],
				&(pfq_sta->Entry[qidx]));
...
UINT16 fq_del_list(RTMP_ADAPTER *pAd, struct dequeue_info *info, CHAR deq_qid,
		   UINT32 *tx_quota)
{
	INT tot_sta;
	...
	if (pAd->fq_ctrl.frm_max_cnt[deq_qid] < pAd->fq_ctrl.frm_cnt[deq_qid])
		pAd->fq_ctrl.frm_max_cnt[deq_qid] =
			pAd->fq_ctrl.frm_cnt[deq_qid];
#endif
	...
	while (tot_sta > 0) {
		tot_sta--;
		if (pEntry) {
			pfq_sta = container_of(pEntry, struct fq_stainfo_type,
					       Entry[deq_qid]);
			wcid = pfq_sta->wcid;
			tr_entry = &pAd->MacTab.tr_entry[wcid];
			list_bitmap[tr_entry->wcid >> FQ_BITMAP_SHIFT] |=
				1 << (tr_entry->wcid & FQ_BITMAP_MASK);
			if (IS_ENTRY_NONE(tr_entry)) {
				if (pEntry == pAd->fq_ctrl.fq[deq_qid].Head) {
					pTmpEntry = RemoveHeadQueue(
						&pAd->fq_ctrl.fq[deq_qid]);
					pPrevEntry =
						pAd->fq_ctrl.fq[deq_qid].Head;
					pEntry = pPrevEntry;
				} else {
					pTmpEntry = pEntry;
					PickFromQueue(&pAd->fq_ctrl.fq[deq_qid],
						      pPrevEntry, pTmpEntry);
					pEntry = pPrevEntry->Next;
				}

				pAd->fq_ctrl.list_map[deq_qid]
						     [wcid >> FQ_BITMAP_SHIFT] &=
					~(1 << (wcid & FQ_BITMAP_MASK));
				pAd->fq_ctrl.no_packet_chk_map
					[deq_qid][wcid >> FQ_BITMAP_SHIFT] &=
					~(1 << (wcid & FQ_BITMAP_MASK));

				fq_reset_list_entry(pAd, deq_qid,
						    pfq_sta->wcid);
				continue;
			}
			...
			if ((!(pAd->fq_ctrl.enable & FQ_SKIP_SINGLE_STA_CASE)) &&
			    (pAd->fq_ctrl.nactive == 1)) {
				*tx_quota =
					(tr_entry->tx_queue[deq_qid].Number <
					 MAX_TX_PROCESS) ?
						tr_entry->tx_queue[deq_qid]
							.Number :
						      MAX_TX_PROCESS;
			} else {
				quota = (pfq_sta->thMax[deq_qid]
					 << pAd->fq_ctrl.factor) -
					(INT16)(pfq_sta->macInQLen[deq_qid] -
						pfq_sta->macOutQLen[deq_qid]);
				if (quota <= 0)
			...
			if (pfq_sta->qlen_max_cnt[deq_qid] <
			    tr_entry->tx_queue[deq_qid].Number)
				pfq_sta->qlen_max_cnt[deq_qid] =
					tr_entry->tx_queue[deq_qid].Number;
#endif
			break;
		}
	...
	if (deq_wcid == 0) {
		for (j = 0; j < MAX_LEN_OF_MAC_TABLE; j++) {
			if ((pAd->fq_ctrl
				     .list_map[deq_qid][j >> FQ_BITMAP_SHIFT] &
			     (1 << (j & FQ_BITMAP_MASK))) &&
			    (!(list_bitmap[j >> FQ_BITMAP_SHIFT] &
			       (1 << (j & FQ_BITMAP_MASK))))) {
				/* add to list */
				tr_entry = &pAd->MacTab.tr_entry[j];
				pAd->fq_ctrl.list_map[deq_qid]
...
INT fq_del_report(RTMP_ADAPTER *pAd, struct dequeue_info *info)
{
	...
	if (pAd->fq_ctrl.dbg_en & FQ_DBG_DUMP_STA_LOG) {
		if (pAd->fq_ctrl.prev_qidx >= 0) {
			...
			if (the_prev_qidx == qidx)
				...
				prev_wcid_tmp =
					pAd->fq_ctrl.prev_wcid[the_prev_qidx];

			if ((prev_wcid_tmp > 0) &&
			    (prev_wcid_tmp < MAX_LEN_OF_MAC_TABLE)) {
				if ((pAd->fq_ctrl.prev_wcid[qidx] == wcid) &&
				    ((pAd->fq_ctrl.fq[qidx].Number >= 1) ||
				     (!pAd->fq_ctrl.enable) ||
				     (pAd->fq_ctrl.nactive == 1))) {
					tr_entry = &pAd->MacTab.tr_entry[wcid];
					pfq_sta = &tr_entry->fq_sta_rec;
					pAd->fq_ctrl.prev_kick_cnt[qidx] +=
						info->deq_pkt_cnt;
					RTMP_SEM_LOCK(&pfq_sta->lock[qidx]);
					if (pfq_sta->KMAX <
					    pAd->fq_ctrl.prev_kick_cnt[qidx])
					...
					tr_entry =
						&pAd->MacTab.tr_entry
							 [pAd->fq_ctrl.prev_wcid
								  [qidx]];
					pfq_sta = &tr_entry->fq_sta_rec;
					RTMP_SEM_LOCK(&pfq_sta->lock[qidx]);
					if (pfq_sta->KMAX <
					    pAd->fq_ctrl
						    .prev_kick_cnt[the_prev_qidx])
						pfq_sta->KMAX =
							pAd->fq_ctrl.prev_kick_cnt
								[the_prev_qidx];
					pAd->fq_ctrl.prev_kick_cnt[qidx] =
						info->deq_pkt_cnt;
					RTMP_SEM_UNLOCK(&pfq_sta->lock[qidx]);
	...
	if (tx_flow_check_state(pAd, NO_ENOUGH_SWQ_SPACE, qidx) &&
		...
		tx_flow_set_state_block(pAd, NULL, NO_ENOUGH_SWQ_SPACE, FALSE,
					qidx);
	...
	if (tr_entry->tx_queue[info->cur_q].Number > 0) {
		if ((pAd->fq_ctrl.enable & FQ_SKIP_SINGLE_STA_CASE) ||
			...
			if ((pfq_sta->thMax[qidx] << pAd->fq_ctrl.factor) <=
			    (INT16)(pfq_sta->macInQLen[qidx] -
				    pfq_sta->macOutQLen[qidx])) {
				pfq_sta->kickPktCnt[qidx] = 0;
				if (pAd->fq_ctrl.fq[qidx].Number > 1) {
					if (pEntry ==
					    pAd->fq_ctrl.fq[qidx].Head) {
						pTmpEntry = RemoveHeadQueue(
							&pAd->fq_ctrl.fq[qidx]);
					} else {
						if (pAd->fq_ctrl
		...
		if (pAd->fq_ctrl.enable & FQ_NO_PKT_STA_KEEP_IN_LIST) {
			if (pAd->fq_ctrl.fq[qidx].Number > 1) {
				if (pEntry == pAd->fq_ctrl.fq[qidx].Head) {
					...
					if (pAd->fq_ctrl.pPrevEntry[qidx] ==
					    NULL) {
						MTWF_LOG(
							DBG_CAT_TX,
							DBG_SUBCAT_ALL,
			...
			if (pEntry == pAd->fq_ctrl.fq[qidx].Head) {
				pTmpEntry =
					RemoveHeadQueue(&pAd->fq_ctrl.fq[qidx]);
			} else {
		...
		while (pEntry) {
			...
			if ((pfq_sta->status[qidx_c] == FQ_UN_CLEAN_STA) &&
				...
				if (pEntry == pAd->fq_ctrl.fq[qidx_c].Head) {
					pTmpEntry = RemoveHeadQueue(
						&pAd->fq_ctrl.fq[qidx_c]);
					pPrevTmpEntry =
						pAd->fq_ctrl.fq[qidx_c].Head;
					if (pPrevTmpEntry)
						pTmpEntry = pPrevTmpEntry;
				} else {
					PickFromQueue(&pAd->fq_ctrl.fq[qidx_c],
						      pPrevEntry, pEntry);
					pTmpEntry = pPrevEntry->Next;
					pPrevTmpEntry = pPrevEntry;
				}
				pAd->fq_ctrl.list_map[qidx_c][pfq_sta->wcid >>
							      FQ_BITMAP_SHIFT] &=
					~(1
					  << (pfq_sta->wcid & FQ_BITMAP_MASK));
				pAd->fq_ctrl
					.no_packet_chk_map[qidx_c]
							  [pfq_sta->wcid >>
							   FQ_BITMAP_SHIFT] &=
					~(1
					  << (pfq_sta->wcid & FQ_BITMAP_MASK));
				if (fq_reset_list_entry(pAd, qidx_c,
							pfq_sta->wcid)) {
					RTMP_SPIN_UNLOCK(
						&pfq_sta->lock[qidx_c]);
					break;
		...
		for (qidx_c = qidx_s; qidx_c <= qidx_e; qidx_c++) {
			...
			for (wcid = 0; wcid < MAX_LEN_OF_MAC_TABLE; wcid++) {
				if (pAd->fq_ctrl
					...
					pfq_sta = &tr_entry->fq_sta_rec;
					RTMP_SPIN_LOCK(&pfq_sta->lock[qidx_c]);
					if ((pfq_sta->status[qidx_c] ==
					     FQ_UN_CLEAN_STA) &&
					    (tr_entry->tx_queue[qidx_c].Number ==
					     0)) {
						pAd->fq_ctrl.list_map
							[qidx_c]
							[wcid >>
							 FQ_BITMAP_SHIFT] &=
							~(1
						...
						pAd->fq_ctrl.no_packet_chk_map
							...
							~(1
							  << (wcid &
							      FQ_BITMAP_MASK));
						fq_reset_list_entry(pAd, qidx_c,
								    wcid);
					} else {
						if ((pfq_sta->status[qidx_c] !=
						     FQ_IN_LIST_STA) &&
						    (tr_entry->tx_queue[qidx_c]
							     .Number > 0))
							MTWF_LOG(
								DBG_CAT_TX,
								DBG_SUBCAT_ALL,
		    ...
		    INT32 mpduTime, UINT32 dwrr_quantum, UINT32 *Value)
{
	...
	if (pAd->fq_ctrl.enable & FQ_READY) {
		...
		for (i = 0; i < WMM_NUM_OF_AC; i++) {
			...
			if ((pfq_sta->macInQLen[i] - pfq_sta->macOutQLen[i]) <
			    0) {
				pfq_sta->macInQLen[i] = 0;
				pfq_sta->macOutQLen[i] = 0;
			}
			if ((tr_entry->tx_queue[i].Number == 0) &&
			    ((pfq_sta->macInQLen[i] - pfq_sta->macOutQLen[i] <=
			      0) ||
			     (mpduTime < 0))) {
				not_active[i] = 1;
				if (pAd->fq_ctrl.enable &
				    FQ_NO_PKT_STA_KEEP_IN_LIST) {
					if ((pAd->fq_ctrl.no_packet_chk_map
						     [i]
						     [wcid >> FQ_BITMAP_SHIFT] &
					     (1 << (wcid & FQ_BITMAP_MASK))) &&
					    (pAd->fq_ctrl.list_map
			...
			if ((pAd->vow_watf_en) || (txop_usec == 0)) {
				dwrr_idx =
					pAd->vow_sta_cfg[wcid].dwrr_quantum[i];
				dwrr_time = ((dwrr_quantum >>
					      (dwrr_idx *
					       UMAX_AIRTIME_QUANTUM_OFFSET)) &
					     UMAX_AIRTIME_QUANTUM_MASK)
					    << 8;
				thMax = (dwrr_time > 0) ? (dwrr_time /
							   pfq_sta->mpduTime) :
								(MAX_FQ_PPDU_TIME /
							   pfq_sta->mpduTime);
			} else
			...
			} else if ((pEntry->HTPhyMode.field.MODE ==
				    MODE_HTMIX) ||
			...
			if (pfq_sta->macQPktLen[i] > 0)
				max_thMax =
					max_amptu_len / pfq_sta->macQPktLen[i];
			else
				max_thMax = pfq_sta->thMax[i];
			max_thMax = (max_thMax > max_ampdu_num) ?
					    max_ampdu_num :
						  max_thMax;
			if ((mpduTime < 0) &&
			    (tr_entry->tx_queue[i].Number > 0))
		...
		if ((not_active[0] == 0) || (not_active[1] == 0) ||
		    (not_active[2] == 0) || (not_active[3] == 0))
			return NDIS_STATUS_SUCCESS;
		else {
			return NDIS_STATUS_FAILURE;
		}
...
BOOLEAN fq_queue_limit_check(RTMP_ADAPTER *pAd, NDIS_PACKET *pkt, UCHAR qidx,
			     STA_TR_ENTRY *tr_entry)
{
	int i;
	UCHAR wcid;
	...
	if ((pAd->fq_ctrl.frm_cnt[qidx] >= (FQ_PER_AC_LIMIT)) &&
	    !(pAd->fq_ctrl.enable & FQ_LONGEST_DROP)) {
	...
	if (pAd->fq_ctrl.frm_cnt[qidx] >= (FQ_PER_AC_LIMIT)) {
		...
		for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++) {
			if ((pAd->fq_ctrl
				     .list_map[qidx_tmp][i >> FQ_BITMAP_SHIFT] &
			     (1 << (i & FQ_BITMAP_MASK)))) {
				tr_entry_tmp = &pAd->MacTab.tr_entry[i];
				if (tr_entry_tmp->tx_queue[qidx_tmp].Number >=
				    qmax) {
					qmax = tr_entry_tmp->tx_queue[qidx_tmp]
						       .Number;
	       ...
	       STA_TR_ENTRY *tr_entry, QUEUE_HEADER *pPktQueue)
{
	...
	if (tr_entry->enq_cap) {
		...
		pAd->fq_ctrl.no_packet_chk_map[qidx][tr_entry->wcid >>
						     FQ_BITMAP_SHIFT] &=
			~(1 << (tr_entry->wcid & FQ_BITMAP_MASK));
		...
		if (!(pAd->fq_ctrl.enable & FQ_ARRAY_SCH)) {
			...
			pAd->fq_ctrl.list_map[qidx][tr_entry->wcid >>
						    FQ_BITMAP_SHIFT] |=
				(1 << (tr_entry->wcid & FQ_BITMAP_MASK));
		pfq_sta = &tr_entry->fq_sta_rec;
		pfq_sta->macQPktLen[qidx] =
			(pfq_sta->macQPktLen[qidx] <= 0) ?
				GET_OS_PKT_LEN(pkt) :
				      ((((pfq_sta->macQPktLen[qidx] << 6) -
				   pfq_sta->macQPktLen[qidx]) >>
				  6) +
	...
	if (pPktQueue && pPktQueue->Number <= capCount) {
		while (pPktQueue->Head) {
			...
			if ((pAd->fq_ctrl.enable & FQ_READY) &&
				...
				RTMP_SPIN_LOCK(&tr_entry->txq_lock[qidx]);
				pAd->fq_ctrl
					.no_packet_chk_map[qidx]
							  [tr_entry->wcid >>
							   FQ_BITMAP_SHIFT] &=
					~(1
					  << (tr_entry->wcid & FQ_BITMAP_MASK));
				if ((tr_entry->tx_queue[qidx].Number == 0) ||
				    (pAd->fq_ctrl.fq[qidx].Number == 0) ||
				    (pAd->fq_ctrl.fq[qidx].Head == NULL))
					fq_add_list(pAd, qidx, tr_entry);
				...
				enq_done = fq_queue_limit_check(pAd, tmpPkt,
								qidx, tr_entry);
...
enq_end:
	...
	if (enq_done == FALSE) {
		 ...
		 */
		MTAdaptiveResourceCheckFastAdjustment(pAd, tr_entry->wcid,
						      qidx);
		MTAdaptiveResourceAllocation(pAd, WCID_ALL,
					     GET_NUM_OF_TX_RING(pAd->chipCap));
	}
...
void fq_tx_free_per_packet(RTMP_ADAPTER *pAd, UINT8 ucAC, UINT8 ucWlanIdx,
			   NDIS_PACKET *pkt)
{
	struct fq_stainfo_type *pfq_sta = NULL;
	STA_TR_ENTRY *tr_entry = NULL;
...
INT set_fq_enable(PRTMP_ADAPTER pAd, RTMP_STRING *arg)
{
	...
	if (arg) {
		...
		if ((rv > 0) && (en <= FQ_EN_MASK)) {
			if (((pAd->fq_ctrl.enable & FQ_READY) == 0) &&
			    (en & FQ_EN)) {
				tm_exit(pAd);
				qm_exit(pAd);
				...
				if (cap->qm == FAST_PATH_QM) {
					...
					*qm_ops = &fp_fair_qm_ops;
					os_move_mem(*qm_ops, &fp_qm_ops,
						    sizeof(struct qm_ops));
				} else {
					cap->qm = GENERIC_FAIR_QM;
					*qm_ops = &ge_fair_qm_ops;
					os_move_mem(*qm_ops, &ge_qm_ops,
						    sizeof(struct qm_ops));
				}
				qm_init(pAd);
				tm_init(pAd);
				pAd->fq_ctrl.prev_qm = prev_qm;
			} else if ((pAd->fq_ctrl.enable & FQ_READY) &&
				   ((en & FQ_EN) == 0)) {
				...
				pAd->fq_ctrl.enable =
					en | ((pAd->fq_ctrl.enable & FQ_READY) ?
						      FQ_READY :
							    0);
			...
			MTWF_LOG(
				...
				("%s: set Host FQ Enable to %x (%d).(factor to %d)\n",
				 __func__, pAd->fq_ctrl.enable, en,
				 pAd->fq_ctrl.factor));
		} else
			return FALSE;
	} else
...
INT show_fq_info(PRTMP_ADAPTER pAd, RTMP_STRING *arg)
{
	...
	for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++) {
		...
		for (qidx = 0; qidx < WMM_NUM_OF_AC; qidx++) {
			...
			pos += snprintf(
				buf + pos, s_size - pos,
				"[AC%d :macQL:%d;qmax:%d,dp:%d,txqn:%d,st:%d]",
				qidx,
				(INT16)(pfq_sta->macInQLen[qidx] -
					pfq_sta->macOutQLen[qidx]),
				pfq_sta->qlen_max_cnt[qidx],
				pfq_sta->drop_cnt[qidx],
				tr_entry->tx_queue[qidx].Number,
				pfq_sta->status[qidx]);
	...
	MTWF_LOG(
		...
		("FQ was Enabled [0x%x] qm:%d (nSTA:%d,%d[RED]) bcmc:%d ps:%d \n",
		 ...
		 pAd->red_in_use_sta, pAd->fq_ctrl.nbcmc_active,
		 pAd->fq_ctrl.npow_save));

	for (i = 0; i < WMM_NUM_OF_AC; i++) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			 ("FQAC%d's frm_cnt=%d.\n", i,
			  pAd->fq_ctrl.frm_cnt[i]));
	}
	for (i = 0; i < WMM_NUM_OF_AC; i++) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			 ("FQAC%d's frm_max_cnt=%d.\n", i,
			  pAd->fq_ctrl.frm_max_cnt[i]));
		pAd->fq_ctrl.frm_max_cnt[i] = 0;
	}
	for (i = 0; i < WMM_NUM_OF_AC; i++) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			 ("FQAC%d's drop_cnt=%d.\n", i,
			  pAd->fq_ctrl.drop_cnt[i]));
		pAd->fq_ctrl.drop_cnt[i] = 0;
	}
...
INT set_fq_dbg_listmap(PRTMP_ADAPTER pAd, RTMP_STRING *arg)
{
	...
	if (arg) {
		...
		if ((rv > 0) && (wcid <= MAX_LEN_OF_MAC_TABLE) &&
		    (qidx <= WMM_AC_VO)) {
			if (en == 0)
				pAd->fq_ctrl.list_map[qidx][wcid >>
							    FQ_BITMAP_SHIFT] &=
					~(1 << (wcid & FQ_BITMAP_MASK));
			else
				pAd->fq_ctrl.list_map[qidx][wcid >>
	...
	if (arg) {
		...
		if ((rv > 0) && (wcid < MAX_LEN_OF_MAC_TABLE) &&
			...
			if (en == 0) {
				...
				pPrevEntry = pEntry;
				while (pEntry) {
					pfq_sta = container_of(
						pEntry, struct fq_stainfo_type,
						Entry[qidx]);
			     ...
			     CHAR deq_qid, UINT32 *tx_quota)
{
	...
	do {
		...
		if (srch_pos == MAX_LEN_OF_MAC_TABLE)
			srch_pos = 1;
		wcid = srch_pos;
		if ((pAd->fq_ctrl.list_map[deq_qid][wcid >> FQ_BITMAP_SHIFT] &
		     (1 << (wcid & FQ_BITMAP_MASK))) == 0)
			goto LOOP_END;
		...
		if ((!(pAd->fq_ctrl.enable & FQ_SKIP_SINGLE_STA_CASE)) &&
			...
			*tx_quota = (tr_entry->tx_queue[deq_qid].Number <
			...
			if ((INT16)(pAd->fq_ctrl.msdu_in_hw -
				    pAd->fq_ctrl.msdu_out_hw) <
			    (pAd->fq_ctrl.msdu_threshold -
			     pAd->fq_ctrl.nactive * MAX_FQ_VHT_AMPDU_NUM)) {
				*tx_quota =
					(tr_entry->tx_queue[deq_qid].Number <
...
VOID app_show_fq_dbgmsg(RTMP_ADAPTER *pAd)
{
			...
			if ((pAd->fq_ctrl.dbg_en & FQ_DBG_DUMP_STA_LOG) &&
				...
				if (pAd->fq_ctrl.enable & FQ_READY) {
					os_zero_mem(buf,
						    MAX_LEN_OF_MAC_TABLE * 2);
					sprintf(buf,
						"STA%d[AC%d]th:%d macQLen:%d(%d,%d),PktL:%d,kPkt:%d,"
						"KMax:%d,tqn:%d,mT:%d,S=%d,"
						"[0x%08X] msdu_cnt=%d,avg=%d tr_st=%d",
						wcid, ac_idx,
						pfq_sta->thMax[ac_idx],
						(INT16)(pfq_sta->macInQLen
								[ac_idx] -
							pfq_sta->macOutQLen
								[ac_idx]),
						pfq_sta->macInQLen[ac_idx],
						pfq_sta->macOutQLen[ac_idx],
						pfq_sta->macQPktLen[ac_idx],
						pfq_sta->kickPktCnt[ac_idx],
						pfq_sta->KMAX,
						tr_entry->tx_queue[ac_idx]
							.Number,
						pfq_sta->mpduTime,
						pfq_sta->status[ac_idx],
						pAd->fq_ctrl.list_map[ac_idx]
								     [wcid >> 5],
						pfq_sta->tx_msdu_cnt,
	...
	MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("frmcnt:[%d, %d, %d, %d]\n", pAd->fq_ctrl.frm_cnt[0],
		  pAd->fq_ctrl.frm_cnt[1], pAd->fq_ctrl.frm_cnt[2],
		  pAd->fq_ctrl.frm_cnt[3]));
}
