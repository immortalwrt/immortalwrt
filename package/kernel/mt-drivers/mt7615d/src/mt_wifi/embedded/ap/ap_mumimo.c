...
INT SetGroupTblEntryProc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(
		...
		("Values %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d\n",
		 param.index, param.numUser, param.BW, param.NS0, param.NS1,
		 param.NS2, param.NS3, param.PFIDUser0, param.PFIDUser1,
		 param.PFIDUser2, param.PFIDUser3, param.fgIsShortGI,
		 param.fgIsUsed, param.fgIsDisable, param.initMcsUser0,
		 param.initMcsUser1, param.initMcsUser2, param.initMcsUser3,
		 param.dMcsUser0, param.dMcsUser1, param.dMcsUser2,
		 param.dMcsUser3));
	/* Allocate memory for msg */
	msg = AndesAllocCmdMsg(pAd, sizeof(cmd) +
					    sizeof(CMD_MU_SET_GROUP_TBL_ENTRY));
...
#endif
	AndesAppendCmdMsg(msg, (char *)&cmd, sizeof(cmd));
	AndesAppendCmdMsg(msg, (char *)&param,
			  sizeof(CMD_MU_SET_GROUP_TBL_ENTRY));
	AndesSendCmdMsg(pAd, msg);
error:
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO,
...
INT SetClusterTblEntryProc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	param.gidUserMemberStatus[0] =
		os_str_tol(pLowGidUserMemberStatus, 0, 16);
	param.gidUserMemberStatus[1] =
		os_str_tol(pHighGidUserMemberStatus, 0, 16);
	...
	AndesAppendCmdMsg(msg, (char *)&cmd, sizeof(cmd));
	AndesAppendCmdMsg(msg, (char *)&param,
			  sizeof(CMD_MU_SET_CLUSTER_TBL_ENTRY));
	AndesSendCmdMsg(pAd, msg);
...
INT SetTriggerMuTxProc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	for (index = 0; index < param.u4NumOfSTAs; index++) {
		...
		param.au4PayloadLength[index] =
			os_str_tol(pPayloadLength, 0, 10);
		param.au4PayloadLength[index] =
...
static VOID ShowGroupTblEntryCallback(char *rsp_payload, UINT16 rsp_payload_len)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
		 ("dMCS3  %d\n", pGentry->dMcsUser3));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("END"));
}
/* | Valid |BA | WLAN INDEX| */
...
{
	P_EVENT_GET_TXOP_DEFAULT pGentry =
		(P_EVENT_GET_TXOP_DEFAULT)rsp_payload;
...
bool hqa_wifi_test_mu_cal_lq(PRTMP_ADAPTER pAd, P_MU_STRUCT_SET_CALC_LQ pParams)
{
	INT Ret = 0;
	struct cmd_msg *msg = NULL;
...
static VOID hqa_wifi_test_mu_get_su_lq_callback(struct cmd_msg *msg,
						...
						UINT16 rsp_payload_len)
{
	P_EVENT_HQA_GET_SU_CALC_LQ pEntry =
		(P_EVENT_HQA_GET_SU_CALC_LQ)rsp_payload;
...
static VOID hqa_wifi_test_mu_get_lq_callback(struct cmd_msg *msg,
					     ...
					     UINT16 rsp_payload_len)
{
	P_EVENT_HQA_GET_MU_CALC_LQ pEntry =
		(P_EVENT_HQA_GET_MU_CALC_LQ)rsp_payload;
	...
	if (msg->attr.rsp.wb_buf_in_calbk == NULL) {
		...
		pOutput->lq_report[0][0] =
			cpu2le32(pEntry->rEntry.lq_report[0][0]);
		pOutput->lq_report[0][1] =
			cpu2le32(pEntry->rEntry.lq_report[0][1]);
		pOutput->lq_report[0][2] =
			cpu2le32(pEntry->rEntry.lq_report[0][2]);
		pOutput->lq_report[0][3] =
			cpu2le32(pEntry->rEntry.lq_report[0][3]);
		pOutput->lq_report[0][4] =
			cpu2le32(pEntry->rEntry.lq_report[0][4]);
		pOutput->lq_report[1][0] =
			cpu2le32(pEntry->rEntry.lq_report[1][0]);
		pOutput->lq_report[1][1] =
			cpu2le32(pEntry->rEntry.lq_report[1][1]);
		pOutput->lq_report[1][2] =
			cpu2le32(pEntry->rEntry.lq_report[1][2]);
		pOutput->lq_report[1][3] =
			cpu2le32(pEntry->rEntry.lq_report[1][3]);
		pOutput->lq_report[1][4] =
			cpu2le32(pEntry->rEntry.lq_report[1][4]);
		pOutput->lq_report[2][0] =
			cpu2le32(pEntry->rEntry.lq_report[2][0]);
		pOutput->lq_report[2][1] =
			cpu2le32(pEntry->rEntry.lq_report[2][1]);
		pOutput->lq_report[2][2] =
			cpu2le32(pEntry->rEntry.lq_report[2][2]);
		pOutput->lq_report[2][3] =
			cpu2le32(pEntry->rEntry.lq_report[2][3]);
		pOutput->lq_report[2][4] =
			cpu2le32(pEntry->rEntry.lq_report[2][4]);
		pOutput->lq_report[3][0] =
			cpu2le32(pEntry->rEntry.lq_report[3][0]);
		pOutput->lq_report[3][1] =
			cpu2le32(pEntry->rEntry.lq_report[3][1]);
		pOutput->lq_report[3][2] =
			cpu2le32(pEntry->rEntry.lq_report[3][2]);
		pOutput->lq_report[3][3] =
			cpu2le32(pEntry->rEntry.lq_report[3][3]);
		pOutput->lq_report[3][4] =
			cpu2le32(pEntry->rEntry.lq_report[3][4]);
	}
}
...
bool hqa_wifi_test_mu_table_set(PRTMP_ADAPTER pAd, P_MU_STRUCT_MU_TABLE ptr)
{
	...
	if (ptr->type == SU) {
		if (ptr->length <= sizeof(pSuParam->metric_table))
			NdisCopyMemory(pSuParam->metric_table, ptr->prTable,
				       ptr->length);
		else {
			MTWF_LOG(
		...
		if (ptr->length <= sizeof(pMuParam->metric_table))
			NdisCopyMemory(pMuParam->metric_table, ptr->prTable,
				       ptr->length);
		else {
			MTWF_LOG(
				DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
	...
	msg = AndesAllocCmdMsg(
		pAd, (sizeof(cmd) + ((ptr->type == SU) ? (sizeof(*pSuParam)) :
							       (sizeof(*pMuParam)))));
	...
	AndesAppendCmdMsg(msg, (char *)&cmd, sizeof(cmd));
	AndesAppendCmdMsg(
		msg, (ptr->type == SU) ? (char *)pSuParam : (char *)pMuParam,
...
bool hqa_wifi_test_mu_group_set(PRTMP_ADAPTER pAd,
	...
	if (Ret == 0) {
		...
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			 ("%s, init_mcs.user1InitMCS:%u\n", __func__,
			  init_mcs.user0InitMCS));
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			 ("%s, init_mcs.user1InitMCS:%u\n", __func__,
			  init_mcs.user1InitMCS));
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			 ("%s, init_mcs.user1InitMCS:%u\n", __func__,
			  init_mcs.user2InitMCS));
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			 ("%s, init_mcs.user1InitMCS:%u\n", __func__,
			  init_mcs.user3InitMCS));
	}
...
INT32 hqa_mu_cal_init_mcs(PRTMP_ADAPTER pAd, RTMP_STRING *arg)
{
	...
	if (pnum_of_user == NULL || pbandwidth == NULL ||
	    pnss_of_user0 == NULL || pnss_of_user1 == NULL ||
	    pnss_of_user2 == NULL || pnss_of_user3 == NULL ||
	    ppf_mu_id_of_user0 == NULL || ppf_mu_id_of_user1 == NULL ||
	    ppf_mu_id_of_user2 == NULL || ppf_mu_id_of_user3 == NULL ||
	    pnum_of_txer == NULL || arg == NULL) {
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
...
INT32 hqa_su_cal_lq(PRTMP_ADAPTER pAd, RTMP_STRING *arg)
{
	...
	if (pnum_of_user == NULL || pbandwidth == NULL ||
	    pnss_of_user0 == NULL || ppf_mu_id_of_user0 == NULL ||
	    pnum_of_txer == NULL || arg == NULL) {
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			 ("%s: Invalid parameters\n", __func__));
		return -1;
...
INT32 hqa_mu_set_mu_table(PRTMP_ADAPTER pAd, RTMP_STRING *arg)
{
	...
	if (type == MU) {
		...
		NdisFillMemory(&mu_metric_table, length,
			       specific_metric_content);
	} else if (type == SU) {
		length = sizeof(struct _CMD_HQA_SET_SU_METRIC_TABLE);
		NdisFillMemory(&su_metric_table, length,
			       specific_metric_content);
	} else {
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("%s: type:%u, length:%u, data = %zu\n", __func__, type,
		  length,
		  type == MU ? sizeof(mu_metric_table) :
				     sizeof(su_metric_table)));
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("%s\n",
	...
	info.type = type;
	info.length = length;
	info.prTable = (type == MU) ? (char *)&mu_metric_table :
					    (char *)&su_metric_table;
...
INT32 hqa_mu_get_qd(PRTMP_ADAPTER pAd, RTMP_STRING *arg)
{
	...
	Ret = hqa_wifi_test_mu_get_qd(pAd, subcarrier_idx,
...
INT32 hqa_mu_set_trigger_mu_tx(PRTMP_ADAPTER pAd, RTMP_STRING *arg)
{
	...
	UINT32 msduPayloadLength0 =
		0; /* payload length of the MSDU for user 0 */
	UINT32 msduPayloadLength1 =
		0; /* payload length of the MSDU for user 1 */
	UINT32 msduPayloadLength2 =
		0; /* payload length of the MSDU for user 2 */
	UINT32 msduPayloadLength3 =
		0; /* payload length of the MSDU for user 3 */
