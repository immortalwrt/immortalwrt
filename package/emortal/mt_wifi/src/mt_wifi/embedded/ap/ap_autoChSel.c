				...
				if (pAutoChCtrl->pChannelInfo->IsABand) {
					if (((channel_idx + GetABandChOffset(ch)) >= 0)
						...
						if ((ChOffsetIdx >= 0) && (ChOffsetIdx <= MAX_NUM_OF_CHANNELS))
							...
							MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Get ABand channel offset fail.\n"));
							return FirstChannel(pAd, wdev);
						}
					}
				} else {
	...
	IN struct wifi_dev *wdev)
{
	...
	for (i = 0; i < auto_ch_ctrl->pChannelInfo->group_ch_list_num; i++) {
		...
		if (auto_ch_ctrl->pChannelInfo->group_ch_list[i].skip_group == FALSE) {
			...
			if ((busy <= min_busy) &&
				((acs_grp_ch_list->bw80_not_allowed == FALSE) && (auto_ch_ctrl->pChannelInfo->group_ch_list[i].bw80_grp_ch_member_idx == 4)) &&
	...
	IN BOOLEAN IsABand)
{
...
#endif
	{
		...
		Reg[0].Counter = MIB_CNT_P_CCA_TIME;/* M0SDR16 Primary Channel Busy Time */
		MtCmdMultipleMibRegAccessRead(pAd, BandIdx, Reg, 1);
		pAutoChCtrl->AutoChSelCtrl.pre_pcca_time = Reg[0].Value;
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
				("%s: BandIdx = %d, pre_pcca_time = %u\n", __func__, BandIdx, pAutoChCtrl->AutoChSelCtrl.pre_pcca_time));
	...
	IN struct wifi_dev *pwdev)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
	("%s(): Update channel for wdev for this band PhyMode = %d, Channel = %d\n",
	...
	IN UCHAR band_idx)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			("%s IS_DFS_V10_ACS_VALID(pAd) = %d\n", __func__, IS_DFS_V10_ACS_VALID(pAd)));
	...
	IN struct wifi_dev *pwdev)
{
	...
	if (pAutoChCtrl->AutoChSelCtrl.ScanChIdx == -1) {
		...
		/* Read Channel busy time counter */
		Reg[0].Counter = MIB_CNT_P_CCA_TIME;/* M0SDR16 Primary Channel Busy Time */
		MtCmdMultipleMibRegAccessRead(pAd, BandIdx, Reg, 1);
		pAutoChCtrl->AutoChSelCtrl.pre_pcca_time = Reg[0].Value;
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("[%s] BandIdx %d, pre_pcca_time %u\n",
			__func__, BandIdx, pAutoChCtrl->AutoChSelCtrl.pre_pcca_time));
...
NDIS_STATUS set_idle_pwr_test(IN PRTMP_ADAPTER pAd, IN RTMP_STRING * arg)
{
	...
	if (arg) {
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("%s: pwr_thres %d, time %d, cnt = %d, cmd_type = %d, band_idx = %d\n",
			 __func__, pwr_thres, time, cnt, cmd_type, u1DbdcIdx));
