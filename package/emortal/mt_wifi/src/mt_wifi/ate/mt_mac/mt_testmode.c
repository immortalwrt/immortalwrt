...
#if defined(MT7615) || defined(MT7622) || defined(P18) || defined(MT7663) || defined(AXE) || defined(MT7915)
						  ...
						  struct _MAC_TABLE_ENTRY *entry)
{
	...
	if (stack->index < stack_boundary) {
		...
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("(%s)Stack for MAC_TABL_ENRTY is full!\n", __func__));
	}
							...
							struct _MAC_TABLE_ENTRY **entry)
{
	...
	if (stack->index > 0) {
		...
		*entry = NULL;
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("(%s)Stack for MAC_TABL_ENRTY is empty!\n", __func__));
	}
...
INT MtATESetRxMUAid(RTMP_ADAPTER *ad, UCHAR band_idx, UINT16 mu_rx_aid)
{
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		("%s: Set to decode MU accodring to AID:%d (61696 means disable)\n",
...
static INT MtCmdSetPhyManualTx(RTMP_ADAPTER *ad, BOOLEAN on, UINT32 apep_length, struct phy_params *phy_info, struct _ATE_RU_STA *ru_sta)
{
	...
	if (on) {
		...
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				("%s: [band:%d][TXV Common1 DDW1] total_pwr_dbm:0x%x pwr_dbm:%d user_cnt:%d, tx_mode:0x%x, bw:%d, stbc:%d, ru_mu:%d, spatial_ext:%d\n",
				__func__, ate_ctrl->control_band_idx, param.Data.set_phy_manual_tx.cg1_ddw1.total_pwr_dbm,
		...
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				("%s: [band:%d][TXV Common1 DDW2] trigger_frame_ind:%d format:%d ltf:%d, gi:%d, sig_a_rsvd:0x%x, total_pwr_ind:%d, tf_rsp_ind:%d, cfo_ind:%d, precomp_cfo_idx:%d\n",
				__func__, ate_ctrl->control_band_idx, param.Data.set_phy_manual_tx.cg1_ddw2.trigger_frame_ind,
		...
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				("%s: [band:%d][TXV Common2 DDW1] mimo_ltf:%d afactor:%d txop:%d, pe_disamb:%d, ltf_symbol:%d ppdu_sym_cnt:0x%x, ldpc_extra_symbol:%d, lg_txlen:0x%x\n",
				__func__, ate_ctrl->control_band_idx, param.Data.set_phy_manual_tx.cg2_ddw1.mimo_ltf,
		...
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				("%s: [band:%d][TXV Common2 DDW1] start_spatial_stream:%d ru_size:%d fec_coding:%d, nsts:%d, rate:%d\n",
				__func__, ate_ctrl->control_band_idx, param.Data.set_phy_manual_tx.user_g1.start_spatial_stream,
		...
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				("%s: [band:%d][TXV User2 DDW1] apep_length:0x%x\n", __func__, ate_ctrl->control_band_idx, param.Data.set_phy_manual_tx.user_g2.length));
...
static NDIS_STATUS mt_ate_calc_symbol_by_bytes(struct _ATE_RU_STA *ru_sta, BOOLEAN stbc, UCHAR rate_den, UINT32 apep_length)
{
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: RU index[%d], apep length:%d symbol_init:%d, \n",
							__func__, ru_sta->ru_index >> 1, apep_length, ru_sta->symbol_init));
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("\t%s: R[%d/%d], cbps:%d, dbps:%d, cbps_s:%d, dbps_s:%d excess:%d\n",
							__func__, rate_den-1, rate_den, ru_sta->cbps, ru_sta->dbps,
...
static UINT32 mt_ate_calc_bytes_by_time(UCHAR tx_mode, UCHAR nss, UCHAR t_pe, UCHAR ltf, UCHAR gi, UINT32 dbps, UINT32 tx_time)
{
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		("\t%s: \tsymbol=%d, PSDU length:%d (0x3fff is H/W limiation)\n", __func__, symbol_cnt, psdu_length));
...
static INT32 mt_ate_calc_afactor(struct _ATE_RU_STA *ru_sta)
{
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\t%s: \tafactor=%d, symbol cnt=%d\n",
															__func__, ru_sta->afactor_init, ru_sta->symbol_init));
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("\t%s: cbps_l:%d, dbps_l:%d, pld:%d, avbits:%d\n",
															__func__, ru_sta->cbps_last, ru_sta->dbps_last, ru_sta->pld, ru_sta->avbits));
...
static BOOLEAN mt_ate_calc_extr_sym(struct _ATE_RU_STA *ru_sta, BOOLEAN stbc, UCHAR rate_den)
{
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			("\t%s: cw:%d, avbits:%d, punc:%d, l_ldpc:%d, shrt:%d\n", __func__, cw, ru_sta->avbits, ru_sta->punc, l_ldpc, shrt));
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\t%s: LDPC extra symbol:%d\n", __func__, ret));
...
static UINT32 mt_ate_calc_pe_disamb(struct _ATE_RU_STA *ru_sta, UCHAR ltf_gi, UCHAR max_pe)
{
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\t%s: L-Len=%d, PE Disambiguilty=%d\n",
															__func__, ru_sta->l_len, ru_sta->pe_disamb));
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\t%s: tx_time(x5)=%d, tx_ltf_sym(x5):%d, tx_sym(x5):%d, tx_pe(x5):%d\n",
															__func__, ru_sta->tx_time_x5, ltf_time, he_t_sym_x5[gi], he_t_pe_x5[t_pe]));
...
static NDIS_STATUS mt_ate_recalc_phy_info(struct _ATE_RU_STA *ru_sta, UINT8 stbc, UINT8 ltf_gi, UINT8 max_pe)
{
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			("\t\t%s: (re)afactor:%d, (re)cw:%d, (re)avbits:%d, \n\t\t\t(re)punc:%d, (re)l_ldpc:%d, (re)shrt:%d\n",
				__func__, ru_sta->afactor_init, cw, ru_sta->avbits, ru_sta->punc, l_ldpc, shrt));
...
static struct _ATE_RU_STA *mt_ate_dominate_ru(struct _RTMP_ADAPTER *ad, UINT8 band_idx)
{
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: dominated by user[%d], RU index:%d\n",
								__func__, dominate_user_idx, ru_sta->ru_index >> 1));
...
ate_thread_dequeue:
		...
		if (free_num < stack->index) {
			MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
					("%s: [band%d] insificient ring buffer(resource=%ld:request=%d), thread dissmised!\n", __func__, band_idx, free_num, stack->index));
			break;
		}
...
round_tx:
		if (((pkt_tx_time > 0) || (ipg > 0)) &&
			...
			MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
					("%s: [band%d] mgmt queue is full, thread dissmised!\n", __func__, band_idx));
			break;
		}
		...
		if (tx_cnt <= txed_cnt) {
			MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
					("%s: [band%d] reach counter(%d), thread dissmised!\n", __func__, band_idx, txed_cnt*stack->index));
			break;
		}
...
static INT TestMode_PeriodicThread(ULONG Context)
{
	...
	if (!pTask) {
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			("%s: abnormal leave, pTask is NULL\n", __func__));
		return -1;
	}
...
static INT32 MT_ATESetRxUserIdx(RTMP_ADAPTER *pAd, UCHAR band_idx, UINT16 user_idx)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): band_idx: %d, user_idx: 0x%x\n", __func__, band_idx, user_idx));
...
static INT32 MT_ATESetForceTxPower(RTMP_ADAPTER *pAd, INT8 cTxPower, UINT8 uctx_mode, UINT8 ucTxRate, UINT8 ucBW)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: Band(%d), PhyMode(%d), MCS(%d), BW(%d), TxPower(%d)\n",
		__FUNCTION__, control_band_idx, uctx_mode, ucTxRate, ucBW, cTxPower));
...
err0:
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("[WARN]%s: idx:%x, overwritable:%x, log_type:%08x\n",
			  __func__, idx, (log_cb) ? log_cb->overwritable:0xff,
			  log_type));
err1:
...
INT ate_inf_open(struct wifi_dev *wdev)
{
	...
	if (wifi_sys_open(wdev) != TRUE) {
		MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s() open fail!!!\n", __func__));
		return FALSE;
	}

	MTWF_LOG(DBG_CAT_INIT, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("ATE inf up for ra_%x(func_idx) OmacIdx=%d\n",
...
INT ate_conn_act(struct wifi_dev *wdev, struct _MAC_TABLE_ENTRY *entry)
{
	...
	if (entry) {
		...
		if (IS_MT7615(ad) || IS_MT7637(ad) || IS_MT7622(ad) || IS_P18(ad) || IS_MT7663(ad) || IS_AXE(ad) || IS_MT7915(ad))
			RAInit(ad, entry);
#endif
		...
		{
			MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: fgRateAdaptFWOffload Not supported\n", __func__));
			goto error_out;
		}
	} else {
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: MacTableInsertEntry failed\n", __func__));
		goto error_out;
	}
...

	{
		...
		if (band_idx > -1) {
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				...
				__func__, pRxBlk->channel_freq, chfreq0, chfreq1));
	}
...
INT8 mt_ate_init_wdev(RTMP_ADAPTER *pAd, UINT32 band_idx)
{
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				 ("%s: wdev_idx=%d, channel=%d\n", __func__, wdev->wdev_idx, channel));
...
static INT32 MT_ATEInitBandInfo(RTMP_ADAPTER *pAd, UINT32 band_idx)
{
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: idx:%u, pkt:%p\n",
		__func__, idx, ATECtrl->band_ext[idx].test_pkt));
...
INT mt_ate_set_tmac_info(RTMP_ADAPTER *pAd, TMAC_INFO *tmac_info, UINT32 band_idx)
{
	...
	return Ret;
err0:
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s:: Cannot get Wdev by band:%d\n", __func__, band_idx));
	return NDIS_STATUS_FAILURE;
}
...
UINT32 mt_ate_get_txlen_by_pkt_tx_time(struct _RTMP_ADAPTER *ad, UINT32 band_idx)
{
	...
	if (tx_mode == MODE_CCK) { /* Legacy CCK mode */
		...
		default:
			if (gi > 0)
				MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: invalid gi=%d, ignored as 0.\n", __func__, gi));
			break;
		}
		/* Transfer from bit to byte with expected tx time */
		txlen = pkt_tx_time * tx_data_rate / 10 / 8;
		...
		if (data_subcarriers) {
			...
			MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
					("%s: unknown RU Index:[%d], forced transmit l024 bytes MPDU!\n", __func__, (ru_sta->ru_index >> 1)));
	...
	if (tx_mode > MODE_VHT) {
		if (txlen >= (MAX_VHT_MPDU_LEN * 256)) {
			...
			MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				("%s: Expected txlen > HE mode PPDU max length, reduce the txlen=%d\n",
				__func__, txlen));
		}
	} else
#endif /* DOT11_HE_AX */
	...
	BOOLEAN *need_ampdu)
{
	...
	*/
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: original header len=%d\n", __func__, hlen));
...
INT mt_ate_wtbl_cfg(RTMP_ADAPTER *pAd, UINT32 band_idx)
{
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: phy_mode=%d, mcs=%d, nss=%d, stbc=%d, wtbl_rate=0x%x\n",
			  __func__, tx_mode, mcs, nss, stbc, wtbl_rate));
...
INT32 mt_ate_enq_pkt(RTMP_ADAPTER *pAd, UINT32 band_idx, UINT16 sta_idx)
{
	...
	if (sta_idx < stack->index)
		...
		ret = -1;
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: invalid sta_idx=%d and stack depth:%d, ignored\n", __func__, sta_idx, stack->index));
		goto done;
	}

	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_INFO,
		("%s: band_idx:%u, ring Idx:%u\n", __func__, band_idx, qid));
	...
	if (mac_tbl_entry) {
		if (stack->pkt_skb[sta_idx]) {
			...
			if (pkt == NULL) {
				MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: clone pakcet fail, ignored\n", __func__));
				goto done;
			}
			...
			MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: packet for index[%d] is invalid, ignored\n", __func__, sta_idx));
			ret = -1;
		}
	} else {
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: mac_tbl_entry for index[%d] is invalid, ignored\n", __func__, sta_idx));
		ret = -1;
	}
done:
	return ret;
...
static INT32 mt_ate_set_wmm_param_by_qid(RTMP_ADAPTER *pAd, struct wifi_dev *wdev)
{
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("%s: qid=%d, slot_time=%d, sifs_time=%d, ac_num=%d, aifsn=%d, cw=%d\n",
			  __func__, ac_num, slot_time, sifs_time, ac_num, aifsn, cw));
	return ret;
}
...
static INT32 mt_ate_apply_spe_antid(RTMP_ADAPTER *pAd, UINT32 band_idx)
{
	...
	if (IS_HIF_TYPE(pAd, HIF_MT)) {
		...
		if (g_EBF_certification) {
			if (BF_ON_certification) {
				ant_pri = 0;
				MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("Info.AntPri = 0\n"));
			} else {
				ant_pri = 24;  /* 24 */
				MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("Info.AntPri = 24\n"));
			}
		}
#endif /* defined(TXBF_SUPPORT) && defined(MT_MAC) */
	} else
...
static INT32 mt_ate_apply_pwr_offset(RTMP_ADAPTER *pAd, UINT32 band_idx)
{
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: Info.PowerOffset = 0x%x (%d)\n",
		__func__, pwr_offset, pwr_offset));
	...
	if (is_mt_ate_mac_tbl_stack_full(ad, band_idx) == FALSE) {
		if (ate_tx_info) {
			if (mac_tbl_entry) {
				...
				MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
						("%s: Insert mac_table_entry failed\n", __func__));
			}
		}
		...
		if (sta_idx > -1) {
			...
			MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
					("%s: mac_table_entry stored failed\n", __func__));
		}
	} else {
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
					("%s: Stack is full!\n", __func__));
...
INT32 mt_ate_tx_v2(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, TX_BLK *tx_blk)
{
	...
	if (mt_ate_search_mac_tbl_entry(pAd, HcGetBandByWdev(wdev), RTMP_GET_PACKET_WCID(tx_blk->pPacket), &tx_blk->pMacEntry) < 0) {
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				("%s: wcid:%d is in-valid in stack!\n", __func__, RTMP_GET_PACKET_WCID(tx_blk->pPacket)));
...
static INT32 mt_ate_tx_subscribe(RTMP_ADAPTER *ad)
{
	...
	if (!is_mt_ate_mac_tbl_stack_empty(ad, control_band_idx)) {
		...
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_WARN,
				 ("%s: Preiously stored TX information for band[%d] will be flushed!\n", __func__, control_band_idx));
	}
...
static INT32 MT_ATEStartTx(RTMP_ADAPTER *pAd)
{
	...
	if (!wdev) {
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				 ("%s: wdev for band_idx[%d] is not initialized!\n", __func__, control_band_idx));
		goto err1;
	}
	...
	if (is_mt_ate_mac_tbl_stack_empty(pAd, control_band_idx)) {
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				 ("%s: TX information is not commit, dismissed!\n", __func__));
		...
		if (ru_sta) {
			...
			if (IS_AXE(pAd)) {
				...
				if (chip_dbg->ctrl_manual_hetb_tx)
					...
					MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: ctrl_manual_hetb_tx is not registered\n", __func__));
					goto done;
				}
			}
		} else {
			MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: Invalid RU info\n", __func__));
			goto err1;
		}
	}
#endif	/* DOT11_HE_AX*/
	...
	if (if_ops->test_frame_tx)
		...
		if ((pkt_tx_time > 0) || (ipg > 0)) {
			...
			MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				("%s: repeat=0x%x, ampdu_cnt=%d, token_limit=%d, pkt_tx_time=%d, ipg=%d\n",
				__func__, round, ampdu_cnt, token_limit, pkt_tx_time, ipg));
...
#endif /* DOT11_HE_AX */
			MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: txed=%d\n", __func__, rounded_cnt));

			TESTMODE_SET_PARAM(pAd, control_band_idx, ATE_TXED_CNT, rounded_cnt);
#if defined(DOT11_HE_AX)
			if (tx_mode == MODE_HE_TRIG && IS_MT7915(pAd)) {
				struct _RTMP_CHIP_DBG *chip_dbg = hc_get_chip_dbg(pAd->hdev_ctrl);
					...
					UINT32 Ch_Band)
{
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		...
		__func__, tx_sel, rx_sel, tx_stream_num, target_phymode));
	/* set antenna settings to pAd for MtCmdSetTxRxPath() */
#if defined(DBDC_MODE) && defined(MT7915)
	if (IS_MT7915(pAd) && IS_ATE_DBDC(pAd)) {
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		...
		__func__, control_band_idx, bw, ch, ctrl_ch, ch2, pri_sel));
...
err:
	if (!wdev || !wdev_txd)
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: invalid wdev\n", __func__));
#endif /* !defined(COMPOS_TESTMODE_WIN) */
	return -1;
}
...
INT32 MT_ATERFTestCB(RTMP_ADAPTER *pAd, UINT8 *Data, UINT32 Length)
{
	...
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("\x1b[42m%s(): Length[%d], Mode[0x%x]\x1b[m\n", __func__, Length, pAd->ATECtrl.op_mode));
	...
	if (test_done)
		goto done;
#if defined(PRE_CAL_MT7622_SUPPORT) || defined(PRE_CAL_MT7626_SUPPORT) || defined(PRE_CAL_MT7915_SUPPORT)
	...
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("\x1b[43m%s(): EventType[0x%x]\x1b[m\n", __func__, EventType));
...
#endif
#if defined(MT7915)
	AteOp->DnlK7915 = MtATE_DNL_Cal_Store_Proc_7915;
#endif
...
VOID MtATE_Dump_Group_PreCal_7915(RTMP_ADAPTER *pAd)
{
	...
	for (i = 0; i < (CAL_RESULT_SIZE + CAL_RESULT_SIZE_5G + CAL_RESULT_SIZE_6G) / 4; i += 4) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
...
VOID MtATE_Dump_DPD_PreCal_7915(RTMP_ADAPTER *pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
		("%s: DPD/Flatness Pre-Cal: \n", __func__));

	for (i = 0; i < DPD_CAL_TOTAL_SIZE / 4; i += 4)	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			("[0x%08x] 0x%8x 0x%8x 0x%8x 0x%8x\n",
			i * 4, ptr[i], ptr[i+1], ptr[i+2], ptr[i+3]));
	}
...
#if defined(MT7915)
INT MtATE_DNL_Cal_Store_Proc_7915(
	RTMP_ADAPTER *pAd,
	RTMP_STRING *arg)
{
	UINT8                 i = 0;
...
static INT32 mt_ate_show_ru_info(struct _RTMP_ADAPTER *ad, UINT8 band_idx)
{
	...
	for (sta_idx = 0 ; sta_idx < MAX_MULTI_TX_STA ; sta_idx++) {
		...
		if (ru_sta[sta_idx].valid) {
			...
			MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("(%s) RU index[%d] in Segment[%d]: WCID[%d](0: N/A) TX mcs[%d],nss[%d], ldpc[%d], mpdu length:%d, RU MU Nss:%d\n",
	...
	if (strlen(str) > 0) {
		...
		for (i = 0, value = (CHAR *)rstrtok((char *)str, ":"); value; value = (CHAR *)rstrtok((char *)NULL, ":"), i++) {
			if (strlen(value) > 0) {
				...
				MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
					 ("%s: ru_segment[%d]: allocation:%04x, ru_idx:%d, mpdu length:%d, alpha:%d\n\t\t\trate:0x%x, ldpc:%d, nss:%d, ru_mu_nss:%d\n",
					  __func__, (ru_sta[i].ru_index & 0x1), allocation, (ru_sta[i].ru_index >> 1), ru_sta[i].mpdu_length, ru_sta[i].alpha,
					  ru_sta[i].rate, ru_sta[i].ldpc, ru_sta[i].nss, ru_sta[i].ru_mu_nss));
			} else {
				MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Invalid format, %s ignored\n", value));
				MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("<1.allocation>-<2.aid>-<3.ru_index>-<4.rate>-<5.lepc>-<6.nss>-"
		...
		for (tone_idx = 0 ; tone_idx < sizeof(*ru_allocation) ; tone_idx++)
			MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: allocation[%d] = 0x%x\n", __func__, tone_idx, ru_allocation->allocation[tone_idx]));
...
out:
	if (ret == NDIS_STATUS_FAILURE)
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Invalid input string\n"));
