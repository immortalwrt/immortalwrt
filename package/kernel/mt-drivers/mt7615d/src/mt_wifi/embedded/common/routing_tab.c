...
VOID RoutingTabInit(IN PRTMP_ADAPTER pAd, IN UCHAR ifIndex, IN UINT32 Flag)
{
	UINT32 i;
	...
	if (pMbss->pRoutingEntryPool == NULL)
		os_alloc_mem(pAd, (UCHAR **)&pMbss->pRoutingEntryPool,
			     sizeof(ROUTING_ENTRY) * ROUTING_POOL_SIZE);

	if (pMbss->pRoutingEntryPool) {
		NdisZeroMemory(pMbss->pRoutingEntryPool,
			       sizeof(ROUTING_ENTRY) * ROUTING_POOL_SIZE);
		initList(&pMbss->RoutingEntryFreeList);
		for (i = 0; i < ROUTING_POOL_SIZE; i++)
			insertTailList(
				&pMbss->RoutingEntryFreeList,
				(RT_LIST_ENTRY *)(pMbss->pRoutingEntryPool +
						  i));
...
VOID RoutingTabDestory(IN PRTMP_ADAPTER pAd, IN UCHAR ifIndex, IN UINT32 Flag)
{
	BSS_STRUCT *pMbss = NULL;
...
VOID RoutingTabClear(IN PRTMP_ADAPTER pAd, IN UCHAR ifIndex, IN UINT32 Flag)
{
	INT32 i;
	BSS_STRUCT *pMbss = NULL;
		...
		while (pRoutingEntry) {
			...
			if (pRoutingEntry->Valid &&
				...
				if (!IS_ROUTING_ENTRY(pRoutingEntry)) {
					delEntryList(
						&pMbss->RoutingTab[i],
						(RT_LIST_ENTRY *)pRoutingEntry);
					NdisZeroMemory(pRoutingEntry,
...
PROUTING_ENTRY RoutingTabGetFree(IN PRTMP_ADAPTER pAd, IN UCHAR ifIndex)
{
	BSS_STRUCT *pMbss = NULL;
	...
	RTMP_SEM_LOCK(&pMbss->RoutingTabLock);
	pRoutingEntry =
		(PROUTING_ENTRY)removeHeadList(&pMbss->RoutingEntryFreeList);
	RTMP_SEM_UNLOCK(&pMbss->RoutingTabLock);

	return pRoutingEntry;
}

VOID RoutingTabSetAllFree(IN PRTMP_ADAPTER pAd, IN UCHAR ifIndex, IN UCHAR Wcid,
			  IN UINT32 Flag)
{
	INT32 i;
	BSS_STRUCT *pMbss = NULL;
...
VOID RoutingTabSetOneFree(IN PRTMP_ADAPTER pAd, IN UCHAR ifIndex,
			  IN PUCHAR pMac, IN UINT32 Flag)
{
	...
	while (pRoutingEntry) {
		if (pRoutingEntry->Valid &&
		    MAC_ADDR_EQUAL(pMac, pRoutingEntry->Mac)) {
			CLEAR_ROUTING_ENTRY(pRoutingEntry, Flag);
			/* If not any entry type, then recycle to free list. */
			if (!IS_ROUTING_ENTRY(pRoutingEntry)) {
				delEntryList(&pMbss->RoutingTab[HashId],
					     (RT_LIST_ENTRY *)pRoutingEntry);
				NdisZeroMemory(pRoutingEntry,
					       sizeof(ROUTING_ENTRY));
				insertTailList(&pMbss->RoutingEntryFreeList,
					       (RT_LIST_ENTRY *)pRoutingEntry);
			}
			break;
		}
...
VOID RoutingEntryRefresh(IN PRTMP_ADAPTER pAd, IN UCHAR ifIndex,
...
VOID RoutingEntrySet(IN PRTMP_ADAPTER pAd, IN UCHAR ifIndex, IN UCHAR Wcid,
		     IN PUCHAR pMac, IN PROUTING_ENTRY pRoutingEntry)
{
	INT32 HashId;
	BSS_STRUCT *pMbss = NULL;
	...
	RTMP_SEM_LOCK(&pMbss->RoutingTabLock);
	insertTailList(&pMbss->RoutingTab[HashId],
		       (RT_LIST_ENTRY *)pRoutingEntry);
	RTMP_SEM_UNLOCK(&pMbss->RoutingTabLock);
}
...
PROUTING_ENTRY GetRoutingTabHead(IN PRTMP_ADAPTER pAd, IN UCHAR ifIndex,
				 IN INT32 Index)
{
	BSS_STRUCT *pMbss = NULL;
...
BOOLEAN GetRoutingEntryAll(IN PRTMP_ADAPTER pAd, IN UCHAR ifIndex,
...
INT RoutingTabGetEntryCount(IN PRTMP_ADAPTER pAd, IN UCHAR ifIndex)
{
	int count = 0;
	BSS_STRUCT *pMbss = NULL;
...
PROUTING_ENTRY RoutingTabLookup(IN PRTMP_ADAPTER pAd, IN UCHAR ifIndex,
	...
	while (pRoutingEntry) {
		if (pRoutingEntry->Valid &&
			...
			if (VALID_WCID(pRoutingEntry->Wcid)) {
				if (bUpdateAliveTime) {
					RoutingEntryRefresh(pAd, ifIndex,
							    pRoutingEntry);
...
VOID RoutingTabARPLookupUpdate(IN PRTMP_ADAPTER pAd, IN UCHAR ifIndex,
			       IN PROUTING_ENTRY pRoutingEntry,
...
INT RoutingHardTransmit(IN PRTMP_ADAPTER pAd, IN struct wifi_dev *wdev,
			IN PNDIS_PACKET pkt)
{
...
INT RoutingEntrySendAliveCheck(PRTMP_ADAPTER pAd, UCHAR ifIndex,
			       PROUTING_ENTRY pRoutingEntry, UCHAR *pSrcMAC,
			       UINT32 SrcIP)
{
	...
	if (!pRoutingEntry || !pSrcMAC || !VALID_WCID(pRoutingEntry->Wcid) ||
	...
	skb = arp_create(ARPOP_REQUEST, ETH_P_ARP, pRoutingEntry->IPAddr,
			 wdev->if_dev, SrcIP, BROADCAST_ADDR, pSrcMAC,
			 ZERO_MAC_ADDR);
	if (!skb) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
...
VOID RoutingTabMaintain(IN PRTMP_ADAPTER pAd, IN UCHAR ifIndex)
{
...
#endif /* MAC_REPEATER_SUPPORT */
	{
		...
		for_each_netdev (net, pBridgeNetDev) {
			if (pBridgeNetDev->priv_flags & IFF_EBRIDGE) {
				const struct in_device *pBridgeInDev =
					pBridgeNetDev->ip_ptr;
				if (pBridgeInDev) {
					const struct in_ifaddr *ifa =
						pBridgeInDev->ifa_list;
					if (ifa) {
						COPY_MAC_ADDR(
							BridgeMAC,
							pBridgeNetDev->dev_addr);
						NdisCopyMemory(
							&BridgeIP,
							&ifa->ifa_address, 4);
						bBridgeFound = TRUE;
	...
	for (i = 0; i < ROUTING_HASH_TAB_SIZE; i++) {
		...
		while (pRoutingEntry) {
			...
			if ((pRoutingEntry->Valid &&
				...
				if (bBridgeFound) {
					/* Stage 2 Check*/
					if (pRoutingEntry->Retry == 0 ||
					    RTMP_TIME_AFTER(
						    Now,
						    pRoutingEntry->RetryTime)
#ifdef A4_CONN
					    || pRoutingEntry->NeedRefresh
#endif
					) {
						if (pRoutingEntry->Retry >=
							...
							pRoutingEntry
								->RetryTime =
								Now +
								ROUTING_ENTRY_RETRY_TIME;
						}
					}

					if (bCreateARP) {
						if (RoutingEntrySendAliveCheck(
							    pAd, ifIndex,
							    pRoutingEntry,
							    BridgeMAC,
							    BridgeIP))
