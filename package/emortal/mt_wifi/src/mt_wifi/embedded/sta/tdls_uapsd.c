	...
	IN	STATE_MACHINE * pFSM)
{
	...
	/* init lock */
	NdisAllocateSpinLock(pAd, &pAd->StaCfg[0].TdlsInfo.TDLSUapsdLock);
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("tdls uapsd> initialization ok!\n"));
	return TRUE;
}
...
{
	/* free lock */
	NdisFreeSpinLock(&pAd->StaCfg[0].TdlsInfo.TDLSUapsdLock);
	return TRUE;
}
	...
	IN	PRTMP_ADAPTER				pAd)
{
	...
	for (IdEntry = 0; IdEntry < MAX_NUM_OF_TDLS_ENTRY; IdEntry++) {
		pTDLS = (PRT_802_11_TDLS)&pAd->StaCfg[0].TdlsInfo.TDLSEntry[IdEntry];
	...
	IN	USHORT						PsmNew)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("tdls uapsd> our PSM mode change!\n"));
	/* indicate the peer */
	TDLS_SEMLOCK(pAd);

	for (IdTdls = 0; IdTdls < MAX_NUM_OF_TDLS_ENTRY; IdTdls++) {
		pTDLS = (RT_802_11_TDLS *) &pAd->StaCfg[0].TdlsInfo.TDLSEntry[IdTdls];
		...
		if ((pTDLS->Valid) && (pTDLS->Status == TDLS_MODE_CONNECTED)) {
			...
			if (TDLS_UAPSD_ARE_PEER_IN_ACTIVE(pMacEntry))
			{
				...
				MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("tdls uapsd> send a NULL frame!\n"));
	...
	IN	CHAR * pArgv)
{
	...
	if (pMacEntry == NULL) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("tdls_cmd> ERROR! No such peer!\n"));
		return;
	}
	...
	if (pMacEntry->MaxSPLength != 0) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("    Max SP Length: %d (%d frames)\n",
				 pMacEntry->MaxSPLength, pMacEntry->MaxSPLength << 1));
	} else
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("    Max SP Length: 0 (all frames)\n"));

	/* End of if */
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("    UAPSD/AC   AC0    AC1    AC2    AC3"));
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("\n    Tr/De      "));

	for (IdAcNum = 0; IdAcNum < 4; IdAcNum++) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%d/%d    ",
	...
	IN	CHAR * pArgv)
{
	...
	/* End of if */
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("      AP UAPSD/AC   AC0    AC1    AC2    AC3"));
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("    TDLS UAPSD/AC   AC0    AC1    AC2    AC3"));
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("\n         Tr/De      %d/%d    %d/%d    %d/%d    %d/%d\n",
	...
	IN	RTMP_STRING * pArgvIn)
{
	...
	Command = os_str_tol((RTMP_STRING *)BufCmd, 0, 10);
	...
	case TDLS_UAPSD_SIM_BEACON_LOST: /* tdls beacon lost */
		...
		if (pAd->StaCfg[0].bAutoConnectByBssid)
	...
	default: /* error command type */
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("tdls_cmd> ERROR! No such command!\n"));
	...
	OUT UCHAR * pHeader802_3)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("tdls uapsd> LinkId = %d\n", LinkId));
	pTDLS = TDLS_UAPSD_ENTRY_GET(pAd, LinkId);

	/* sanity check */
	if (TDLS_UAPSD_IS_CONN_NOT_BUILT(pTDLS)) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("tdls uapsd> link is not yet built so we can not send a traffic ind to the peer!!!"
																));
		goto LabelExit;
	}

	if (pTDLS->FlgIsWaitingUapsdTraRsp == TRUE) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("tdls uapsd> traffic ind was sent before!\n"));
	...
	IN	MLME_QUEUE_ELEM * pElem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("tdls uapsd> ====> %s\n", __func__));
	...
	if (LinkId == -1 || LinkId == MAX_NUM_OF_TDLS_ENTRY) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("TDLS - TDLS_UAPSD_PeerTrafficIndAction() can not find the LinkId!\n"));
		return;
	}
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("tdls uapsd> PU Buffer Status = 0x%x\n",
	...
	IN	CHAR						**ppArgv)
{
	...
	Buf[ID] = 0x00;
	*ppArgv += ID + 1; /* skip _ */
	return os_str_tol((RTMP_STRING *)Buf, 0, 10);
	...
	IN	CHAR						*pArgv)
{
	...
	for (IdTdls = 0; IdTdls < MAX_NUM_OF_TDLS_ENTRY; IdTdls++) {
		if (!pAd->StaCfg[0].TdlsInfo.TDLSEntry[IdTdls].Valid) {
			NdisMoveMemory(&pAd->StaCfg[0].TdlsInfo.TDLSEntry[IdTdls],
	...
	IN	CHAR						*pArgv)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("tdls uapsd> LinkId = %d\n", LinkId));
	/* cancel waiting flag to avoid tear down the link */
	pTDLS = TDLS_UAPSD_ENTRY_GET(pAd, LinkId);
	pTDLS->FlgIsWaitingUapsdTraRsp = FALSE;
	/* handle UAPSD SP */
	...
	IN	CHAR						*pArgv)
{
	...
	if (PeerPsMode != 0) {
		...
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("tdls_cmd> Change to ACTIVE!\n"));
	} else {
		pMacEntry->PsMode = PWR_SAVE;
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("tdls_cmd> Change to POWER SAVE!\n"));
	}
	...
	IN	CHAR						*pArgv)
{
	...
	if (PeerPsMode != 0) {
		pAd->StaCfg[0].PwrMgmt.Psm = PWR_ACTIVE;
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("tdls_cmd> Change to ACTIVE!\n"));
	} else {
		pAd->StaCfg[0].PwrMgmt.Psm = PWR_SAVE;
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("tdls_cmd> Change to POWER SAVE!\n"));
	}
}
