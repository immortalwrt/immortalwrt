...
char const *pEventText[EVENT_MAX_EVENT_TYPE] = {
	"restart access point",	  "successfully associated",
	"has disassociated",	  "has been aged-out and disassociated",
	"active countermeasures", "has disassociated with invalid PSK password"
};
UCHAR get_apidx_by_addr(RTMP_ADAPTER *pAd, UCHAR *addr)
{
	...
	for (apidx = 0; apidx < pAd->ApCfg.BssidNum; apidx++) {
		if (RTMPEqualMemory(addr, pAd->ApCfg.MBSSID[apidx].wdev.bssid,
				    MAC_ADDR_LEN))
			break;
	}
...
static INT ap_mlme_set_capability(RTMP_ADAPTER *pAd, BSS_STRUCT *pMbss)
{
	...
	pMbss->CapabilityInfo = CAP_GENERATE(
		1, 0, IS_SECURITY_Entry(wdev),
		(pAd->CommonCfg.TxPreamble == Rt802_11PreambleLong ? 0 : 1),
		wdev->bUseShortSlotTime, SpectrumMgmt);
	...
	if (pAd->ApCfg.bAutoChannelAtBootup[BandIdx]) {
		...
		if (wdev->channel != 0) {
			...
			if (wlan_config_get_vht_bw(wdev) == VHT_BW_80) {
				if (pAd->CommonCfg.bCh144Enabled == FALSE) {
					AutoChannelSkipChannels(
						pAd, V10_W56_VHT20_SIZE,
						GROUP5_LOWER);
					AutoChannelSkipChannels(pAd, 1, 144);
				}
				AutoChannelSkipChannels(pAd, V10_LAST_SIZE,
							GROUP6_LOWER);
				status = TRUE;
			} else if (wlan_config_get_vht_bw(wdev) ==
				   VHT_BW_2040) {
				if (pAd->CommonCfg.bCh144Enabled == FALSE)
					AutoChannelSkipChannels(pAd, 1, 144);
				AutoChannelSkipChannels(pAd, V10_LAST_SIZE,
		...
		if (wdev->channel == 0) {
			...
			if (chGrp == NA_GRP) {
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL,
					 DBG_LVL_OFF, ("[%s] W52\n", __func__));
				AutoChannelSkipChannels(pAd, V10_W53_SIZE,
							GROUP2_LOWER);
				AutoChannelSkipChannels(pAd, V10_W56_SIZE,
							GROUP3_LOWER);
				AutoChannelSkipChannels(pAd, V10_LAST_SIZE,
							GROUP6_LOWER);
				status = TRUE;
				...
				if (DfsV10CheckGrpChnlLeft(pAd, W53,
							   V10_W53_SIZE))
					AutoChannelSkipChannels(pAd,
								V10_W52_SIZE,
								GROUP1_LOWER);
				else {
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL,
				...
				status = TRUE;
			} else if (chGrp == W56_UA || chGrp == W56_UB ||
				   chGrp == W56_UC) {
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL,
					 DBG_LVL_OFF, ("[%s] W56\n", __func__));
...
#define DUAL_BAND 1
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("%s: AutoChannelBootup[%d] = %d\n", __func__, BandIdx,
		  pAd->ApCfg.bAutoChannelAtBootup[BandIdx]));
	...
	RetriggerACS:
		...
		if (pAutoChCtrl->AutoChSelCtrl.ACSChStat !=
			...
			NewChannel = APAutoSelectChannel(
				pAd, wdev, pAd->ApCfg.AutoChannelAlg[BandIdx],
				IsAband);
			MTWF_LOG(
			...
			if (IS_SUPPORT_ONDEMAND_ZEROWAIT_DFS(pAd))
				NewChannel =
					pAd->CommonCfg.DfsParameter.Band0Ch;
			else
#endif
		...
		if ((IS_SUPPORT_ONDEMAND_ZEROWAIT_DFS(pAd)) &&
			...
			pAutoChCtrl->AutoChSelCtrl.ACSChStat =
				ACS_CH_STATE_NONE;
			pAd->BgndScanCtrl.SkipDfsChannel = TRUE;
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				 ("%s(): Retrigger ACS\n", __func__));
		...
		if ((pAd->ApCfg.bAutoChannelAtBootup[BandIdx] == TRUE) &&
		    (vht_bw == VHT_BW_8080)) {
			wlan_config_set_cen_ch_2(wdev,
						 wdev->auto_channel_cen_ch_2);
			wlan_operate_set_cen_ch_2(wdev,
						  wdev->auto_channel_cen_ch_2);
		}
...
NDIS_STATUS APOneShotSettingInitialize(RTMP_ADAPTER *pAd)
{
	...
	MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		 ("---> APOneShotSettingInitialize\n"));
	RTMPInitTimer(pAd, &pAd->ApCfg.CounterMeasureTimer,
		      GET_TIMER_FUNCTION(CMTimerExec), pAd, FALSE);
#ifdef IDS_SUPPORT
	/* Init intrusion detection timer */
	RTMPInitTimer(pAd, &pAd->ApCfg.IDSTimer,
		      GET_TIMER_FUNCTION(RTMPIdsPeriodicExec), pAd, FALSE);
...
VOID ApUpdateCapabilityAndErpIe(RTMP_ADAPTER *pAd, struct _BSS_STRUCT *mbss)
{
	...
	bUseBGProtection =
		(pAd->CommonCfg.UseBGProtection == 1) || /* always use */
		((pAd->CommonCfg.UseBGProtection == 0) &&
		 ERP_IS_USE_PROTECTION(ErpIeContent));
#ifdef A_BAND_SUPPORT
...
VOID APStartUpForMbss(RTMP_ADAPTER *pAd, BSS_STRUCT *pMbss)
{
	...
	UINT32 phy_reg_value_table[MAX_TXTD_CCK_CR] = {
		0x3FEFF7FD, 0x3FDFF3FC, 0x3FBFEBFA, 0x3F9FE3F8, 0x3F7FDBF6,
		0x3F5FD7F5, 0x3F5FD7F6, 0x3F6FE3F9, 0x3FBFF801, 0x0050240E,
		0x0130641F, 0x0260B836, 0x03F12051, 0x05B1946F, 0x07A2108F,
		0x09928CAD, 0x0B62FCC8, 0x0D035CDE, 0x0E33A0EC, 0x0EF3C4F2
	};
#endif /* SPECIAL_11B_OBW_FEATURE */
...
#if defined(MT_DFS_SUPPORT) && defined(BACKGROUND_SCAN_SUPPORT)
	bZeroWaitStop = MbssZeroWaitStopValidate(pAd, pMbss->wdev.channel,
						 pMbss->mbss_idx);
#endif /* defined(MT_DFS_SUPPORT) && defined(BACKGROUND_SCAN_SUPPORT) */
	MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		 ("===> %s(), CfgMode:%d\n", __func__, phy_mode));
...
#endif
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		 ("[APStartUpForMbss] BuildChannelList, #711\n"));
	...
	if (wf_drv_tbl.wf_fwd_check_device_hook)
		wf_drv_tbl.wf_fwd_check_device_hook(wdev->if_dev, INT_MBSSID,
						    pMbss->mbss_idx,
						    wdev->channel, 1);
	...
	if (IS_OCE_ENABLE(wdev)) {
		...
		if (oceCtrl->ShortSSIDEnabled)
			oceCtrl->ShortSSID =
				Crcbitbybitfast(pMbss->Ssid, pMbss->SsidLen);

		if (IS_OCE_FD_FRAME_ENABLE(wdev) && IS_FD_FRAME_FW_MODE(cap)) {
			OceSendFilsDiscoveryAction(
				pAd, &pAd->ApCfg.MBSSID[MAIN_MBSSID].wdev);
		}
...
VOID APStartUpByBss(RTMP_ADAPTER *pAd, BSS_STRUCT *pMbss)
{
	...
	*/
	...
	AsicSetSyncModeAndEnable(pAd, pAd->CommonCfg.BeaconPeriod, HW_BSSID_0,
				 OPMODE_AP);
	RTMP_IRQ_ENABLE(pAd);
	RTMPEnableRxTx(pAd);
...
#ifdef VENDOR_FEATURE7_SUPPORT
	{
		...
		if (WMODE_CAP_5G(pAd->ApCfg.MBSSID[BSS0].wdev.PhyMode))
			sprintf(event_msg,
				"%sGHz AP start/restart successful\n", "5");
		else
			sprintf(event_msg,
				"%sGHz AP start/restart successful\n", "2.4");
...
VOID APStartUp(RTMP_ADAPTER *pAd, BSS_STRUCT *pMbss, ENUM_AP_BSS_OPER oper)
{
	...
	if (pMbss)
		MTWF_LOG(
			DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			("%s(), oper(%d) bssid(%d)=%02x:%02x:%02x:%02x:%02x:%02x\n",
			 __func__, oper, pMbss->mbss_idx,
			 PRINT_MAC(pMbss->wdev.bssid)));
	else
		MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			 ("%s(), oper(%d)\n", __func__, oper));
	...
	case AP_BSS_OPER_ALL:
		for (idx = 0; idx < pAd->ApCfg.BssidNum; idx++) {
			...
			if (pCurMbss->wdev.channel &&
			    (pCurMbss->wdev.channel < 14))
	...
	case AP_BSS_OPER_BY_RF:
		...
		for (idx = 0; idx < pAd->ApCfg.BssidNum; idx++) {
			...
			if (pCurMbss->wdev.open_state == FALSE) {
				RTMP_OS_COMPLETE(
					&pCurMbss->wdev.start_stop_complete);
			}
		...
		for (i = 0; i < pAd->ApCfg.BssidNum; i++) {
			...
			if (pDot11h->RDMode == RD_SILENCE_MODE) {
				if (wdev_temp->cac_not_required == TRUE)
					wapp_send_cac_period_event(
						pAd,
						RtmpOsGetNetIfIndex(
							wdev_temp->if_dev),
						wdev_temp->channel, 2, 0);
				else
					wapp_send_cac_period_event(
						pAd,
						RtmpOsGetNetIfIndex(
...
VOID APInitForMain(RTMP_ADAPTER *pAd)
{
	...
	AsicDisableSync(
		pAd,
		HW_BSSID_0); /* don't gen beacon, reset tsf timer, don't gen interrupt. */
	/*reset hw wtbl*/
	ap_hw_tb_init(pAd);
	...
	*/
	...
	for (apidx = 0; apidx < pAd->ApCfg.BssidNum; apidx++) {
		...
		OPSTATUS_SET_FLAG_WDEV(&pMbss->wdev,
...
VOID APStopByBss(RTMP_ADAPTER *pAd, BSS_STRUCT *pMbss)
{
	...
	if (wf_drv_tbl.wf_fwd_entry_delete_hook)
		wf_drv_tbl.wf_fwd_entry_delete_hook(pAd->net_dev,
						    wdev_bss->if_dev, 0);

	if (wf_drv_tbl.wf_fwd_check_device_hook)
		wf_drv_tbl.wf_fwd_check_device_hook(wdev_bss->if_dev,
						    INT_MBSSID, pMbss->mbss_idx,
						    wdev_bss->channel, 0);
...
VOID APStop(RTMP_ADAPTER *pAd, BSS_STRUCT *pMbss, ENUM_AP_BSS_OPER oper)
{
	...
	case AP_BSS_OPER_BY_RF:
		...
		for (idx = 0; idx < pAd->ApCfg.BssidNum; idx++) {
			...
			/* check MBSS status is up */
			if (!pCurMbss->wdev.if_up_down_state &&
			    !pCurMbss->wdev.bcn_buf.bBcnSntReq)
				continue;
			if (pCurMbss->wdev.open_state == FALSE) {
				MTWF_LOG(
					DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_OFF,
...
static VOID CalFarClientNum(struct _RTMP_ADAPTER *ad,
			    struct _MAC_TABLE_ENTRY *entry)
{
	...
	for (idx = 0; idx < 4; idx++)
		avg_rssi[idx] = ad->ApCfg.RssiSample.LastRssi[idx] -
				ad->BbpRssiToDbmDelta;

	for (idx = 0; idx < 4; idx++) {
		if ((entry->RssiSample.AvgRssi[idx] < FAR_CLIENT_RSSI) &&
		    (entry->RssiSample.AvgRssi[idx] != -127)) {
			if ((avg_rssi[idx] - entry->RssiSample.AvgRssi[idx]) >=
			    FAR_CLIENT_DELTA_RSSI) {
...
static VOID dynamic_amsdu_protect_adjust(struct _RTMP_ADAPTER *ad)
{
	...
	if (ad->mcli_ctl.amsdu_cnt != amsdu_cnt) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("%s(): change amsdu %d to %d\n", __func__,
			  ad->mcli_ctl.amsdu_cnt, amsdu_cnt));
		ad->mcli_ctl.amsdu_cnt = amsdu_cnt;
		if (IS_ASIC_CAP(ad, fASIC_CAP_MCU_OFFLOAD))
			MtCmdCr4Set(ad, CR4_SET_ID_CONFIG_STA_AMSDU_MAX_NUM,
				    255, amsdu_cnt);
		else
	...
	if (!mc_flg) {
		...
		ad->mcli_ctl.last_tx_fail_cnt =
			wlan_ct->AmpduFailCount.u.LowPart;
	...
	tx_cnt =
		wlan_ct->AmpduSuccessCount.u.LowPart - ad->mcli_ctl.last_tx_cnt;
	tx_fail_cnt = wlan_ct->AmpduFailCount.u.LowPart -
		      ad->mcli_ctl.last_tx_fail_cnt;
	...
	if (tx_cnt > 0) {
		...
		if ((ad->mcli_ctl.c2s_only == FALSE) &&
		    (per < VERIWAVE_PER_RTS_DIS_TH_LOW_MARK)) {
			asic_rts_on_off(wdev, FALSE);
			ad->mcli_ctl.c2s_only = TRUE;
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,
				 ("%s(): disable RTS, per=%lu\n", __func__,
				  per));
		} else if ((ad->mcli_ctl.c2s_only == TRUE) &&
			   (per >= VERIWAVE_PER_RTS_DIS_TH_HIGH_MARK)) {
			asic_rts_on_off(wdev, TRUE);
			ad->mcli_ctl.c2s_only = FALSE;
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,
				 ("%s(): enable RTS, per=%lu\n", __func__,
...
static VOID dynamic_airtime_fairness_adjust(struct _RTMP_ADAPTER *ad)
{
	...
	if (ad->txop_ctl[band_idx].multi_client_nums == multi_client_num_th) {
		...
		if (ad->txop_ctl[band_idx].multi_client_nums > 1) {
			if (ad->vow_mcli_ctl.pkt_avg_len >
			    veriwave_tp_amsdu_dis_th)
				shrink_flag = TRUE;
			else if (ad->vow_mcli_ctl.pkt_avg_len >
				 VERIWAVE_PKT_LEN_LOW)
				shrink_flag = TRUE;
		}
	}
	...
	if (ad->vow_sta_frr_flag != shrink_flag) {
		...
		if (ad->vow_cfg.en_airtime_fairness &&
		    ad->vow_sta_frr_quantum && !vow_watf_is_enabled(ad)) {
			if ((shrink_flag == TRUE) &
				    (ad->nearfar_far_client_num <= 1) &&
			    mc_flg)
				RTMP_SET_STA_DWRR_QUANTUM(
					ad, FALSE,
					ad->vow_sta_frr_quantum); /* fast round robin */
			else
				RTMP_SET_STA_DWRR_QUANTUM(ad, TRUE,
							  0); /* restore */
		}
	}
	return;
...
#ifdef WHNAT_SUPPORT
static VOID tx_offload_counter_update(struct _RTMP_ADAPTER *ad,
				      struct _MAC_TABLE_ENTRY *entry)
{
#ifdef CONFIG_AP_SUPPORT
	...
	if (IS_ENTRY_WDS(entry)) {
		ad->WdsTab.WdsEntry[entry->func_tb_idx]
			.WdsCounter.TransmittedFragmentCount.QuadPart +=
...
static VOID do_sta_keep_action(struct _RTMP_ADAPTER *ad,
			       struct _MAC_TABLE_ENTRY *entry, BOOLEAN is_no_rx)
{
#ifdef IAPP_SUPPORT
	if (entry->wdev && is_no_rx &&
	    (ad->Mlme.PeriodicRound % STA_KEEP_ALIVE_NOTIFY_L2) == 0) {
		IAPP_L2_Update_Frame_Send(ad, entry->Addr,
					  entry->wdev->wdev_idx);
	}
#endif /*IAPP_SUPPORT*/
}
...
VOID MacTableMaintenance(RTMP_ADAPTER *pAd)
{
	...
	for (BandIdx = 0; BandIdx < DBDC_BAND_NUM; BandIdx++) {
		pAd->SCSCtrl.SCSMinRssi[BandIdx] =
			0; /* (Reset)The minimum RSSI of STA */
	...
	for (wcid = startWcid; VALID_UCAST_ENTRY_WCID(pAd, wcid); wcid++) {
		...
		if (pEntry &&
			...
			if (pEntry->HTC_AAD_OM_CountDown >
			    0) /* count down to start all new pEntry->HTC_ICVErrCnt */
		...
		if (IS_ENTRY_CLIENT(pEntry) || IS_ENTRY_APCLI(pEntry) ||
			...
			pAd->SCSCtrl.OneSecRxByteCount[BandIdx] +=
				pEntry->OneSecRxBytes;
			pAd->SCSCtrl.OneSecTxByteCount[BandIdx] +=
				pEntry->OneSecTxBytes;

			if (pAd->SCSCtrl.SCSEnable[BandIdx] == SCS_ENABLE) {
				tmpRssi = RTMPMinRssi(
					pAd, pEntry->RssiSample.AvgRssi[0],
		...
		if (pAd->MacTab.Size == 0)
		...
		else if ((pAd->fgRaLimitPhyRate == FALSE) &&
			...
			if (pEntry->SupportRateMode & SUPPORT_VHT_MODE) {
				...
				if (RACTRL_LIMIT_MAX_PHY_RATE >=
				    MAX_PHY_RATE_3SS) {
					if (pEntry->SupportVHTMCS4SS)
						fgPhyModeCheck = TRUE;
				} else if (RACTRL_LIMIT_MAX_PHY_RATE >=
					   MAX_PHY_RATE_2SS) {
		...
		if ((pAd->CommonCfg.whnat_en) &&
			...
			MtCmdCr4Query(pAd, CR4_QUERY_OPTION_GET_TX_STATISTICS,
				      wcid, 0);
		...
		pEntry->AvgRxBytes =
			(pEntry->AvgRxBytes == 0) ?
				pEntry->OneSecRxBytes :
				      ((pEntry->AvgRxBytes + pEntry->OneSecRxBytes) >>
				 1);
		...
		if (pEntry->wdev) {
			UINT32 tx_tp =
				(pEntry->AvgTxBytes >> BYTES_PER_SEC_TO_MBPS);
			UINT32 rx_tp =
				(pEntry->AvgRxBytes >> BYTES_PER_SEC_TO_MBPS);
			...
			struct tx_delay_control *tx_delay_ctl =
				&pAd->tr_ctl.tx_delay_ctl;
			RTMP_CHIP_CAP *cap = hc_get_chip_cap(pAd->hdev_ctrl);

			if ((rx_tp >= MIN_AGG_EN_TP) &&
			    (rx_tp <= MAX_AGG_EN_TP) && cap->tx_delay_support) {
				if (!tx_delay_ctl->que_agg_en) {
					...
					if (IS_ASIC_CAP(pAd,
						...
						MtCmdCr4Set(
							pAd,
							CR4_SET_ID_CONFIG_TX_DELAY_MODE,
							TX_DELAY_MODE_ARG1_TX_BATCH_CNT,
							tx_delay_ctl
								->tx_process_batch_cnt);
					}
				}
			} else {
				if (tx_delay_ctl->que_agg_en) {
			...
			else if (((tx_tp * 100) / (tx_tp + rx_tp)) >
				 TX_MODE_RATIO_THRESHOLD)
				traffc_mode = TRAFFIC_DL_MODE;
			else if (((rx_tp * 100) / (tx_tp + rx_tp)) >
				...
				if (((pEntry->avg_tx_pkts >
				    ...
				    ((pEntry->avg_rx_pkts >
				     ...
				     (((avg_rx_b * 100) /
				       (avg_tx_b + avg_rx_b)) >
				      RX_MODE_RATIO_THRESHOLD)))
					pAd->txop_ctl[BandIdx]
						.multi_client_nums++;
			}

			if (WMODE_CAP_2G(pEntry->wdev->PhyMode) &&
			    (pAd->txop_ctl[BandIdx].multi_client_nums <
			     MULTI_CLIENT_2G_NUMS_TH)) {
				pAd->txop_ctl[BandIdx].cur_wdev = pEntry->wdev;

				if (((pEntry->avg_tx_pkts >
				      VERIWAVE_2G_PKT_CNT_TH) &&
				     ((avg_tx_b + avg_rx_b) > 0) &&
				     (((avg_tx_b * 100) /
				       (avg_tx_b + avg_rx_b)) >
				      TX_MODE_RATIO_THRESHOLD)) ||
			...
			if (WMODE_CAP_5G(pEntry->wdev->PhyMode)) {
				if (pAd->peak_tp_ctl[BandIdx].cur_wdev == NULL)
					pAd->peak_tp_ctl[BandIdx].cur_wdev =
						pEntry->wdev;
				pAd->peak_tp_ctl[BandIdx].client_nums++;
				if (traffc_mode == TRAFFIC_DL_MODE) {
					/* the entry is in tx mode */
					if (tx_tp > pAd->peak_tp_ctl[BandIdx]
							    .max_tx_tp) {
						pAd->peak_tp_ctl[BandIdx]
							.max_tx_tp = tx_tp;
						pAd->peak_tp_ctl[BandIdx]
							.cur_wdev =
							pEntry->wdev;
			...
			if (pEntry->one_sec_tx_succ_pkts >
				    INFRA_KEEP_STA_PKT_TH &&
			...
			if (IS_MAP_ENABLE(pAd)) {
				...
				if (tp_ratio > STA_TP_IDLE_THRESHOLD)
					bidir_traffc_mode =
						TRAFFIC_BIDIR_ACTIVE_MODE;
				else
					bidir_traffc_mode =
						TRAFFIC_BIDIR_IDLE_MODE;

				if ((pEntry->pre_traffic_mode ==
				     TRAFFIC_BIDIR_ACTIVE_MODE) &&
				    (tp_ratio <= STA_TP_IDLE_THRESHOLD))
					wapp_send_cli_active_change(pAd, pEntry,
								    INACTIVE);
				else if ((pEntry->pre_traffic_mode ==
					  TRAFFIC_BIDIR_IDLE_MODE) &&
					 (tp_ratio > STA_TP_IDLE_THRESHOLD))
					wapp_send_cli_active_change(pAd, pEntry,
								    ACTIVE);
		...
		if (pAd->StaFastIdleCheckEnable) {
			...
			} else if (pEntry->KeepAliveSendError >=
				   pAd->StaIdleCheck_At_NullFrameTxFailCnt) {
				bDisconnectSta = TRUE;
				pEntry->KeepAliveSendError = 0;
				goto DeauthSta;
			} else if ((pEntry->KeepAliveSend) &&
				   (pEntry->KeepAliveSendError == 0)) {
			...
			if (pEntry->wdev &&
				...
				if ((pEntry->NoDataIdleCount >
				     pEntry->NoDataLastIdleCount) &&
				    ((pEntry->NoDataIdleCount -
				      pEntry->NoDataLastIdleCount) >=
				     pEntry->Sta_Idle_Check_At_Sec)) {
					MTWF_LOG(
		...
		if ((pEntry->Sst != SST_ASSOC) &&
			...
			MTWF_LOG(
				...
				("%02x:%02x:%02x:%02x:%02x:%02x fail to complete ASSOC in %lu sec\n",
				 ...
				 pEntry->AssocDeadLine));
#ifdef WSC_AP_SUPPORT

			if (NdisEqualMemory(pEntry->Addr,
					    pMbss->wdev.WscControl.EntryAddr,
					    MAC_ADDR_LEN))
				NdisZeroMemory(pMbss->wdev.WscControl.EntryAddr,
					       MAC_ADDR_LEN);
		...
		*/
		if (pEntry->PsMode == PWR_SAVE) {
			...
			if (pEntry->wdev &&
				...
				pAd->MacTab.tr_entry[pEntry->wdev->tr_tb_idx]
					.PsMode = PWR_SAVE;
		...
		if (pEntry->NoDataIdleCount >= pEntry->StaIdleTimeout) {
			...
			if (pEntry && IS_ENTRY_CLIENT(pEntry))
				DiagConnError(pAd, pEntry->func_tb_idx,
					      pEntry->Addr,
					      DIAG_CONN_FRAME_LOST,
					      REASON_AGING_TIME_OUT);
#endif
		...
		if (pMbss->RssiLowForStaKickOut &&
			...
			MTWF_LOG(
				...
				("Disassoc STA %02x:%02x:%02x:%02x:%02x:%02x , RSSI Kickout Thres[%d]-[%d]\n",
				 PRINT_MAC(pEntry->Addr),
				 pMbss->RssiLowForStaKickOut, avgRssi));

#ifdef WIFI_DIAG
			if (pEntry && IS_ENTRY_CLIENT(pEntry))
				DiagConnError(pAd, pEntry->func_tb_idx,
					      pEntry->Addr, DIAG_CONN_DEAUTH,
					      REASON_RSSI_TOO_LOW);
#endif
		}
	...
	DeauthSta:
		...
		if (pEntry->BTMDisassocCount == 1) {
			...
			MiniportMMRequest(pAd, MGMT_USE_PS_FLAG, pOutBuffer,
					  FrameLen);
			MlmeFreeMemory(pOutBuffer);
		...
		if ((pEntry->BSS2040CoexistenceMgmtSupport) &&
			...
			SendNotifyBWActionFrame(pAd, pEntry->wcid,
						pEntry->func_tb_idx);
		...
		if ((pEntry->HTPhyMode.field.MCS != 32) &&
		    ((pEntry->HTPhyMode.field.MCS % 8 == 0) ||
		     (pEntry->HTPhyMode.field.MCS % 8 == 1)))
			pMacTable->fAllStationGainGoodMCS = FALSE;

		/* Check Current STA's Operation Mode is BW20 or BW40 */
		pMacTable->fCurrentStaBw40 =
			(pEntry->HTPhyMode.field.BW == BW_40) ? TRUE : FALSE;
#ifdef WFA_VHT_PF
...
#endif /* WFA_VHT_PF */
#if (defined(WH_EVENT_NOTIFIER) ||                                             \
     (defined(A4_CONN) && defined(IGMP_SNOOP_SUPPORT)))
		if (pEntry && IS_ENTRY_CLIENT(pEntry)
#ifdef A4_CONN
		    && !IS_ENTRY_A4(pEntry)
#endif /* A4_CONN */
			...
			if (pMbss->a4_init && pMbss->wdev.IgmpSnoopEnable) {
				...
				if ((pAd->Mlme.OneSecPeriodicRound % 10) == 0) {
					...
					if (pAd->bIGMPperiodicQuery == TRUE) {
						...
						if ((pMbss->IgmpQueryHoldTick >
						     0) &&
						    (pMbss->IgmpQueryHoldTickChanged ==
						     FALSE)) {
						...
						if ((pAd->IgmpQuerySendTick >
						     0) &&
						    (IgmpQuerySendTickChanged ==
						     FALSE)) {
						...
						if ((pMbss->IGMPPeriodicQuerySent ==
						     FALSE) &&
						    ((pMbss->IgmpQueryHoldTick ==
						      0) &&
						     (pAd->IgmpQuerySendTick ==
							...
							pMbss->IGMPPeriodicQuerySent =
								TRUE;
						}
					}

					if (pAd->bMLDperiodicQuery ==
					    TRUE) { /*If Periodic MLD query feature enabled*/

						if ((pMbss->MldQueryHoldTick >
						     0) &&
						    (pMbss->MldQueryHoldTickChanged ==
						     FALSE)) {
						...
						if ((pAd->MldQuerySendTick >
						     0) &&
						    (MldQuerySendTickChanged ==
						     FALSE)) {
						...
						if ((pMbss->MLDPeriodicQuerySent ==
							...
							pMbss->MLDPeriodicQuerySent =
								TRUE;
						}
					}
				} else if ((pAd->Mlme.OneSecPeriodicRound %
					    10) == 1) { /* Per 11 sec check */

					if (pAd->IgmpQuerySendTick ==
					    0) /* Set the period for IGMP query again */
						pAd->IgmpQuerySendTick =
							QUERY_SEND_PERIOD;

					if (pAd->MldQuerySendTick ==
					    0) /* Set the period for MLD query again */
						pAd->MldQuerySendTick =
							QUERY_SEND_PERIOD;
					...
					if (pMbss->IGMPPeriodicQuerySent ==
					    TRUE)
						pMbss->IGMPPeriodicQuerySent =
							FALSE; /* Reset flag for next period query */
					...
					pMbss->IgmpQueryHoldTickChanged =
						FALSE; /* Reset flag for next 10th second counter edit */
					pMbss->MldQueryHoldTickChanged =
						FALSE; /* Reset flag for next 10th second counter edit */
				}
			}
	...
	if (worst_rssi != NULL && ((pAd->Mlme.OneSecPeriodicRound % 10) == 5) &&
		...
		MTWF_LOG(
			...
			("%s():WorstRSSI for STA(%02x:%02x:%02x:%02x:%02x:%02x):%d,%d,%d, Set Gain as %s\n",
			 ...
			 worst_rssi->AvgRssi[0], worst_rssi->AvgRssi[1],
			 worst_rssi->AvgRssi[2], (gain == 2 ? "Mid" : "Low")));
	}
	...
	if (sta_hit_2g_infra_case_number == STA_NUMBER_FOR_TRIGGER) {
		if (pAd->G_MODE_INFRA_TXOP_RUNNING == FALSE) {
			...
			enable_tx_burst(pAd, txop_wdev, AC_BE, PRIO_2G_INFRA,
					TXOP_FE);
		} else if (pAd->g_mode_txop_wdev != txop_wdev) {
			disable_tx_burst(pAd, pAd->g_mode_txop_wdev, AC_BE,
					 PRIO_2G_INFRA, TXOP_0);
			enable_tx_burst(pAd, txop_wdev, AC_BE, PRIO_2G_INFRA,
					TXOP_FE);
			pAd->g_mode_txop_wdev = txop_wdev;
		}
...
#ifndef DISABLE_MULTICLIENT_DYNAMIC_TXOP
	for (BandIdx = 0; BandIdx < DBDC_BAND_NUM; BandIdx++) {
		if (pAd->txop_ctl[BandIdx].multi_client_nums ==
		    MULTI_CLIENT_NUMS_TH) {
			if (pAd->txop_ctl[BandIdx].multi_cli_txop_running ==
			    FALSE) {
				pAd->txop_ctl[BandIdx].multi_cli_txop_running =
					TRUE;
				enable_tx_burst(pAd,
						pAd->txop_ctl[BandIdx].cur_wdev,
						AC_BE, PRIO_MULTI_CLIENT,
						TXOP_0);
			}
		} else {
			if (pAd->txop_ctl[BandIdx].multi_cli_txop_running ==
				...
				pAd->txop_ctl[BandIdx].multi_cli_txop_running =
					FALSE;
				disable_tx_burst(
					pAd, pAd->txop_ctl[BandIdx].cur_wdev,
...
*/
	...
	if (pAd->CommonCfg.bRalinkBurstMode &&
	    pMacTable->fAllStationGainGoodMCS)
	...
	if (bRalinkBurstMode !=
	    RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_RALINK_BURST_MODE))
...
/*
	...
	==========================================================================
*/
MAC_TABLE_ENTRY *APSsPsInquiry(IN RTMP_ADAPTER *pAd, IN UCHAR *pAddr,
			       OUT SST *Sst, OUT USHORT *Aid, OUT UCHAR *PsMode,
			       OUT UCHAR *Rate)
{
	MAC_TABLE_ENTRY *pEntry = NULL;
...
VOID ApLogEvent(RTMP_ADAPTER *pAd, UCHAR *pAddr, USHORT Event)
{
	if (pAd->EventTab.Num < MAX_NUM_OF_EVENT) {
		RT_802_11_EVENT_LOG *pLog =
			&pAd->EventTab.Log[pAd->EventTab.Num];

		RTMP_GetCurrentSystemTime(&pLog->SystemTime);
		COPY_MAC_ADDR(pLog->Addr, pAddr);
		pLog->Event = Event;
		MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("LOG#%ld %02x:%02x:%02x:%02x:%02x:%02x %s\n",
...
VOID APUpdateOperationMode(RTMP_ADAPTER *pAd, struct wifi_dev *wdev)
{
	...
	if (ht_protect_en) {
		...
		if ((pAd->ApCfg.LastNoneHTOLBCDetectTime + (5 * OS_HZ)) >
			    pAd->Mlme.Now32 &&
		    pAd->Mlme.Now32 != 0) {
		...
		if (pAd->MacTab.fAnyStationIsLegacy ||
		    pAd->MacTab.Size >= multi_client_num_th) {
		...
		MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_INFO,
			 (" --%s:\n OperationMode: %d, bNonGFExist: %d\n",
			  __func__, addht->AddHtInfo2.OperaionMode,
			  bNonGFExist));

		if ((op_mode != addht->AddHtInfo2.OperaionMode) ||
		    (pAd->MacTab.fAnyStationNonGF !=
		     addht->AddHtInfo2.NonGfPresent)) {
...
BOOLEAN ApCheckAccessControlList(RTMP_ADAPTER *pAd, UCHAR *pAddr, UCHAR Apidx)
{
	...
	if (pAd->ApDisableSTAConnectFlag == TRUE) {
		...
		for (i = 0; i < count; i++) {
			if (NdisEqualMemory(&(pAd->AllowedStaList.AllowedSta[i]
						      .MacAddr[0]),
					    pAddr, MAC_ADDR_LEN)) {
				if ((time - pAd->AllowedStaList.AllowedSta[i]
						    .DissocTime) < 30000) {
					Result = TRUE;
				}
				break;
	...
	if (pAd->ApCfg.MBSSID[Apidx].AccessControlList.Policy ==
	    0) /* ACL is disabled */
		Result = TRUE;
	else {
		ULONG i;
		...
		if (pAd->ApCfg.MBSSID[Apidx].AccessControlList.Policy ==
		    1) /* ACL is a positive list */
			Result = FALSE;
		else /* ACL is a negative list */
			Result = TRUE;

		for (i = 0; i < pAd->ApCfg.MBSSID[Apidx].AccessControlList.Num;
		     i++) {
	...
	if (pAd->ApCfg.MBSSID[Apidx].AccessControlList.Policy != 2) {
		for (idx = 0;
		     idx < pAd->ApCfg.MBSSID[Apidx].AccessControlList.Num;
		     idx++)
			(pAd->ApCfg.MBSSID[Apidx]
				 .AccessControlList.Entry[idx]
				 .Reject_Count) = 0;
	}
#endif
	...
	if (Result == FALSE) {
		MTWF_LOG(
			DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			("%02x:%02x:%02x:%02x:%02x:%02x failed in ACL checking\n",
			 PRINT_MAC(pAddr)));
	}
...
VOID ApUpdateAccessControlList(RTMP_ADAPTER *pAd, UCHAR Apidx)
{
	...
	MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		 ("ApUpdateAccessControlList : Apidx = %d\n", Apidx));
	...
	for (MacIdx = 0; VALID_UCAST_ENTRY_WCID(pAd, MacIdx); MacIdx++) {
		...
		for (AclIdx = 0; AclIdx < pMbss->AccessControlList.Num;
		     AclIdx++) {
			if (MAC_ADDR_EQUAL(&pEntry->Addr[0],
					   pMbss->AccessControlList
						   .Entry[AclIdx]
						   .Addr)) {
				Matched = TRUE;
				break;
		...
		if ((Matched == FALSE) &&
			...
			MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			   ...
			   (pMbss->AccessControlList.Policy == 2)) {
			drop = TRUE;
			MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		...
		if (drop == TRUE) {
			...
			MTWF_LOG(
				...
				("pAd->ApCfg.MBSSID[%d].AccessControlList.Policy = %ld\n",
				 Apidx, pMbss->AccessControlList.Policy));
			...
			if (pEntry && IS_ENTRY_CLIENT(pEntry))
				DiagConnError(pAd, pEntry->func_tb_idx,
					      pEntry->Addr, DIAG_CONN_ACL_BLK,
					      0);
#endif
#ifdef MAP_R2
			if (IS_ENTRY_CLIENT(pEntry) && IS_MAP_ENABLE(pAd) &&
			    IS_MAP_R2_ENABLE(pAd))
				wapp_handle_sta_disassoc(pAd, pEntry->wcid,
							 Reason);
#endif
			mac_entry_delete(pAd, pEntry);
		}
		...
		where
			...
			"==" means that the values on either side of the "==" are to be tested for equaliy with a resulting
				...
				=>When the value of OSi is the empty set, then the expression (S == OSi for all values of i)
					is defined to be TRUE
*/
INT GetBssCoexEffectedChRange(IN RTMP_ADAPTER *pAd, IN struct wifi_dev *wdev,
			      ...
			      IN UCHAR Channel)
{
	...
	if (Channel > 14) {
		...
		if (index < pChCtrl->ChListNum) {
			...
			pCoexChRange->primaryCh =
				pChCtrl->ChList[index].Channel;

			/* Now check about the secondary and central channel */
			if (op_ext_cha == EXTCHA_ABOVE) {
				pCoexChRange->effectChStart =
					pCoexChRange->primaryCh;
				pCoexChRange->effectChEnd =
					pCoexChRange->primaryCh + 4;
			...
			MTWF_LOG(
				...
				("5.0GHz: Found CtrlCh idx(%d) from the ChList, ExtCh=%s, PriCh=[Idx:%d, CH:%d], SecCh=[Idx:%d, CH:%d], effected Ch=[CH:%d~CH:%d]!\n",
				 index,
				 ((op_ext_cha == EXTCHA_ABOVE) ? "ABOVE" :
								       "BELOW"),
				 pCoexChRange->primaryCh,
				 pChCtrl->ChList[pCoexChRange->primaryCh]
					 .Channel,
				 pCoexChRange->secondaryCh,
				 pChCtrl->ChList[pCoexChRange->secondaryCh]
					 .Channel,
				 pChCtrl->ChList[pCoexChRange->effectChStart]
					 .Channel,
				 pChCtrl->ChList[pCoexChRange->effectChEnd]
					 .Channel));
			return TRUE;
		}
	...
	{ /* For 2.4GHz band */
		...
		if (index < pChCtrl->ChListNum) {
			...
			if (cntrCh) {
				pCoexChRange->effectChStart =
					(cntrCh - 5) > 0 ? (cntrCh - 5) : 0;
				pCoexChRange->effectChEnd =
					(cntrCh + 5) < pChCtrl->ChListNum ?
						(cntrCh + 5) :
						      (pChCtrl->ChListNum - 1);
				MTWF_LOG(
					DBG_CAT_AP, DBG_SUBCAT_ALL,
...
static INT32 wlan_update_rf_ht_bw(RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
				  UCHAR ht_bw, UCHAR ext_cha)
{
	UINT32 idx;
	BSS_STRUCT *pCurMbss = NULL;
	...
	for (idx = 0; idx < pAd->ApCfg.BssidNum; idx++) {
		...
		if (pCurMbss->wdev.channel == wdev->channel) {
			wlan_operate_set_prim_ch(&pCurMbss->wdev,
...
VOID APOverlappingBSSScan(RTMP_ADAPTER *pAd, struct wifi_dev *wdev)
{
	...
	if ((!WMODE_CAP_N(PhyMode)) || (ht_bw == BW_20)) {
		MTWF_LOG(
			DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
	...
	if (index < pChCtrl->ChListNum) {
		if (ext_cha == EXTCHA_ABOVE) {
			curPriChIdx = index;
			curSecChIdx = ((index + 4) < pChCtrl->ChListNum) ?
					      (index + 4) :
						    (pChCtrl->ChListNum - 1);
			chStartIdx = (curPriChIdx >= 3) ? (curPriChIdx - 3) : 0;
			chEndIdx = ((curSecChIdx + 3) < pChCtrl->ChListNum) ?
					   (curSecChIdx + 3) :
						 (pChCtrl->ChListNum - 1);
		} else {
			curPriChIdx = index;
			...
			chEndIdx = ((curPriChIdx + 3) < pChCtrl->ChListNum) ?
					   (curPriChIdx + 3) :
						 (pChCtrl->ChListNum - 1);
		}
	} else {
	...
	MTWF_LOG(
		...
		("Ready to do passive scanning for Channel[%d] to Channel[%d]!\n",
		 pChCtrl->ChList[chStartIdx].Channel,
		 pChCtrl->ChList[chEndIdx].Channel));
	...
	for (index = chStartIdx; index <= chEndIdx; index++) {
		MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			 ("Channel[Idx=%d, Ch=%d].bEffectedChannel=0x%x!\n",
			  index, pChCtrl->ChList[index].Channel,
			  pChCtrl->ChList[index].bEffectedChannel));

		if ((pChCtrl->ChList[index].bEffectedChannel &
		     (EFFECTED_CH_PRIMARY | EFFECTED_CH_LEGACY)) &&
		    (index != curPriChIdx)) {
			needFallBack = TRUE;
			MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
...
static VOID dynamic_ampdu_efficiency_adjust_all(struct _RTMP_ADAPTER *ad)
{
	...
	for (band_idx = 0; band_idx < DBDC_BAND_NUM; band_idx++) {
		...
		if (ad->txop_ctl[band_idx].multi_client_nums ==
		    multi_client_num_th) {
		...
		if (tx_cnt > 0)
			per = 100 * (wlan_ct->AmpduFailCount.u.LowPart) /
			      (wlan_ct->AmpduFailCount.u.LowPart + tx_cnt);
		...
		if ((ad->CommonCfg.dbdc_mode) &&
			...
			if (level !=
			    ad->peak_tp_ctl[band_idx].cli_peak_tp_txop_level)
				ad->peak_tp_ctl[band_idx]
					.cli_peak_tp_txop_enable = FALSE;
		} else if ((ad->peak_tp_ctl[band_idx].max_tx_tp >
			    TP_100M_LOWER_BOUND) &&
			   (ad->peak_tp_ctl[band_idx].max_tx_tp <
			    TP_100M_UPPER_BOUND)) {
			ad->peak_tp_ctl[band_idx].cli_peak_tp_running = 1;
			level = TXOP_60;
		...
		MTWF_LOG(
			DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_INFO,
			("%s[%d]per=%lu, tx=%d M, (%d, %d, %d)\n\r", __func__,
			 __LINE__, per, ad->peak_tp_ctl[band_idx].max_tx_tp,
		...
		if (ad->peak_tp_ctl[band_idx].cli_peak_tp_running) {
			if (!ad->peak_tp_ctl[band_idx]
				     .cli_ampdu_efficiency_running) {
				if (query_tx_burst_prio(ad, wdev) <=
				    PRIO_PEAK_TP) {
			...
			if (!ad->peak_tp_ctl[band_idx].cli_peak_tp_txop_enable) {
				enable_tx_burst(ad, wdev, AC_BE, PRIO_PEAK_TP,
						level);
				ad->peak_tp_ctl[band_idx]
					.cli_peak_tp_txop_level = level;
				ad->peak_tp_ctl[band_idx]
					.cli_peak_tp_txop_enable = TRUE;
			}
		} else {
			/* restore to original */
			if (ad->peak_tp_ctl[band_idx]
				...
				if (edcaparam)
					AsicAmpduEfficiencyAdjust(
						wdev, edcaparam->Aifsn[0]);

				ad->peak_tp_ctl[band_idx]
					.cli_ampdu_efficiency_running = FALSE;
			}

			if (ad->peak_tp_ctl[band_idx].cli_peak_tp_txop_enable) {
				disable_tx_burst(ad, wdev, AC_BE, PRIO_PEAK_TP,
						 level);
				ad->peak_tp_ctl[band_idx]
					.cli_peak_tp_txop_enable = FALSE;
			}
...
#define FRAME_BUF_LEN                                                          \
	(LENGTH_802_3 + sizeof(RalinkIe) + VLAN_HDR_LEN +                      \
	 sizeof(DOT1X_QUERY_STA_DATA) + OCE_FILS_CMD_PAYLOAD_LEN)
#else
#define FRAME_BUF_LEN                                                          \
	(LENGTH_802_3 + sizeof(RalinkIe) + VLAN_HDR_LEN +                      \
	 OCE_FILS_CMD_PAYLOAD_LEN)
	...
	MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		 ("%s::rx_byte:%lu  tx_byte:%lu rx_pkt:%lu tx_pkt:%lu\n",
		  __func__, data.rx_bytes, data.tx_bytes, data.rx_packets,
		  data.rx_packets));
#endif /* RADIUS_ACCOUNTING_SUPPORT */
	...
	if (pEntry) {
		...
		NdisMoveMemory(s_addr, pAd->ApCfg.MBSSID[apidx].wdev.bssid,
			       MAC_ADDR_LEN);
		s_addr[0] |= 0x80;
	}
	...
	if (pAd->ApCfg.MBSSID[apidx].wdev.VLAN_VID) {
		...
		MAKE_802_3_HEADER(FrameBuf, pAd->ApCfg.MBSSID[apidx].wdev.bssid,
				  s_addr, EAPOL_IE);
	...
	/*add accounting info*/
	NdisMoveMemory(&FrameBuf[offset], (unsigned char *)&data,
		       sizeof(DOT1X_QUERY_STA_DATA));
#endif /*RADIUS_ACCOUNTING_SUPPORT*/
#ifdef OCE_FILS_SUPPORT
	if ((cmd == DOT1X_MLME_EVENT) || (cmd == DOT1X_AEAD_ENCR_EVENT)) {
#ifdef RADIUS_ACCOUNTING_SUPPORT
		...
		if (frame_len > FRAME_BUF_LEN) {
			MTWF_LOG(
				DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				("%s frame_len(%u) > FRAME_BUF_LEN(%u) (cmd=%d)\n",
				 __func__, frame_len, FRAME_BUF_LEN, cmd));
			return FALSE;
		}

		NdisMoveMemory(&FrameBuf[offset], pEntry->filsInfo.pending_ie,
			       pEntry->filsInfo.pending_ie_len);
		...
		NdisMoveMemory(&FrameBuf[offset], pEntry->filsInfo.PTK,
			       pEntry->filsInfo.PTK_len);
		...
		frame_len += (pEntry->filsInfo.pending_ie_len - hdr_len -
			      LENGTH_802_1_H);
		if (frame_len > FRAME_BUF_LEN) {
			MTWF_LOG(
				DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
		...
		NdisMoveMemory(
			&FrameBuf[offset],
			&pEntry->filsInfo.pending_ie[hdr_len + LENGTH_802_1_H],
			(pEntry->filsInfo.pending_ie_len - hdr_len -
			 LENGTH_802_1_H));
	}
#endif /* OCE_FILS_SUPPORT */
...
BOOLEAN DOT1X_EapTriggerAction(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry)
{
	...
	enum { FRAME_LEN = LENGTH_802_3 + sizeof(eapol_start_1x_hdr) };
	UINT8 frame_len = FRAME_LEN;
	...
	if (IS_AKM_1X_Entry(pEntry) ||
	    IS_IEEE8021X_Entry(&pAd->ApCfg.MBSSID[apidx].wdev)) {
		/* Init the frame buffer */
		...
		if (pAd->ApCfg.MBSSID[apidx].wdev.VLAN_VID) {
			...
			bss_Vlan = pAd->ApCfg.MBSSID[apidx].wdev.VLAN_VID;
			bss_Vlan_Priority =
				pAd->ApCfg.MBSSID[apidx].wdev.VLAN_Priority;
		...
		NdisMoveMemory(&FrameBuf[offset], eapol_start_1x_hdr,
			       sizeof(eapol_start_1x_hdr));
#ifdef CONFIG_HOTSPOT_R2

		if (pEntry) {
			BSS_STRUCT *pMbss = pEntry->pMbss;

			if ((pMbss->HotSpotCtrl.HotSpotEnable == 1) &&
			    (IS_AKM_WPA2_Entry(&pMbss->wdev)) &&
			    (pEntry->hs_info.ppsmo_exist == 1)) {
				UCHAR HS2_Header[4] = { 0x50, 0x6f, 0x9a,
							0x12 };

				memcpy(&FrameBuf[offset +
						 sizeof(eapol_start_1x_hdr)],
				       HS2_Header, 4);
				memcpy(&FrameBuf[offset +
						 sizeof(eapol_start_1x_hdr) + 4],
				       &pEntry->hs_info,
				       sizeof(struct _sta_hs_info));
				frame_len += 4 + sizeof(struct _sta_hs_info);
				MTWF_LOG(
					DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_OFF,
					("DOT1X_EapTriggerAction: event eapol start, %x:%x:%x:%x\n",
					 FrameBuf[offset +
						  sizeof(eapol_start_1x_hdr) +
						  4],
					 FrameBuf[offset +
		...
		/* Report to upper layer */
		if (RTMP_L2_FRAME_TX_ACTION(pAd, apidx, FrameBuf, frame_len) ==
		    FALSE)
			return FALSE;
...
INT rtmp_ap_init(RTMP_ADAPTER *pAd)
{
	...
	for (j = BSS0; j < pAd->ApCfg.BssidNum; j++) {
		...
		wdev = &pAd->ApCfg.MBSSID[j].wdev;
		{
			...
			os_alloc_mem(pAd, &pWscControl->pWscRxBuf,
				     MAX_MGMT_PKT_LEN);
			...
			os_alloc_mem(pAd, &pWscControl->pWscTxBuf,
				     MAX_MGMT_PKT_LEN);
...
*/
UINT32 starec_ap_feature_decision(struct wifi_dev *wdev,
				  struct _MAC_TABLE_ENTRY *entry,
				  UINT32 *feature)
{
...
INT ap_link_down(struct wifi_dev *wdev)
{
	...
	if (WDEV_BSS_STATE(wdev) >= BSS_INITED) {
		...
		MbssKickOutStas(wdev->sys_handle, wdev->func_idx,
				REASON_DISASSOC_INACTIVE);
		UpdateBeaconHandler(wdev->sys_handle, wdev,
...
INT ap_conn_act(struct wifi_dev *wdev, struct _MAC_TABLE_ENTRY *entry)
{
	...
	if (IS_MT7615(ad) || IS_MT7637(ad) || IS_MT7622(ad) || IS_P18(ad) ||
	    IS_MT7663(ad))
		RAInit(ad, entry);

#endif /* defined(MT7615) || defined(MT7637) || defined(MT7622) */
#if defined(MT_DFS_SUPPORT) && defined(BACKGROUND_SCAN_SUPPORT) &&             \
	defined(TXBF_SUPPORT) && defined(MT7615)

	/* Disable TXBF apply for DFS zero wait start */
	if ((ad->BgndScanCtrl.BgndScanStatMachine.CurrState ==
	     BGND_RDD_DETEC) &&
...
INT ap_inf_open(struct wifi_dev *wdev)
{
...
#ifdef CONVERTER_MODE_SWITCH_SUPPORT
	if (pAd->ApCfg.MBSSID[wdev->func_idx].APStartPseduState !=
	...
	*/
		if (pMbss && (pMbss->wdev.channel < 14)) {
			if (!(pAd->ApCfg.ObssGBandChanBitMap &
			      (1 << pMbss->wdev.channel))) {
			 ...
			 wdev.if_up_down_state:%d GBandChanBitMap:0x%x\n",
					 ...
					 wdev->channel, pAd->ApCfg.BssidNum,
					 pMbss->wdev.if_up_down_state,
					 pAd->ApCfg.ObssGBandChanBitMap));
				...
				if (pAd->CommonCfg.need_fallback == 1) {
					MTWF_LOG(
						...
						("[%s] Need fallback to 20 MHz: pMbss->wdev.channel:%d\n",
						 __func__,
						 pMbss->wdev.channel));
					wlan_operate_set_ht_bw(&pMbss->wdev,
							       HT_BW_20,
							       EXTCHA_NONE);
				}
			}
