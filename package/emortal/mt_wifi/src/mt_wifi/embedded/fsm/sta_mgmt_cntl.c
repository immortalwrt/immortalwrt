	...
	struct wifi_dev *wdev)
{
	...
	if ((JoinReq.SsidLen == 0) && MAC_ADDR_EQUAL(JoinReq.Bssid, ZERO_MAC_ADDR)) {
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_WARN, ("(%s) SsidLen=0 & zero mac bssid, return\n",
			__func__));
		return;
	}
		...
		if (IS_AKM_AUTOSWITCH(pStaCfg->wdev.SecConfig.AKMMap)
			...
			if (wdev->wdev_type == WDEV_TYPE_REPEATER) {
				...
				MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
					("%s:wdev(type=%d,func_idx=%d)auth fail reason(%d) and free rept_entry\n",
					__func__, wdev->wdev_type, wdev->func_idx, Reason));
			}
	...
	VOID *elem_obj)
{
	...
	if (pEntry && (Reason == MLME_FINITE_CYCLIC_GROUP_NOT_SUPPORTED) &&
		...
		if (wdev->wdev_type == WDEV_TYPE_REPEATER) {
			HW_REMOVE_REPT_ENTRY(pAd, wdev->func_idx);
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				("%s:wdev(type=%d,func_idx=%d)assoc fail reason(%d) and free rept_entry\n",
				__func__, wdev->wdev_type, wdev->func_idx, Reason));
		}
#endif /* MAC_REPEATER_SUPPORT */
...
VOID LinkUp_Infra(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, MAC_TABLE_ENTRY *pEntry, UCHAR *tmpWscSsid, UCHAR tmpWscSsidLen, UINT link_up_type, MLME_QUEUE_ELEM *Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s:BeaconPeriod(%d),DtimPeriod(%d)\n",
		__func__, pStaCfg->MlmeAux.BeaconPeriod, pStaCfg->MlmeAux.DtimPeriod));
...
#ifdef DOT11_N_SUPPORT
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Old ExtCh:%d new ExtCh:%d !!\n"
	...
	*/
	if (wdev->WscControl.bWscTrigger)
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("WPS is in Prgress; Not sending this LINK UP\n"));
	else {
		if ((wdev->AuthMode != Ndis802_11AuthModeWPA2PSK)
			...
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Auth Mode is WPA2PSK or WPAPSK; Not sending this LINK UP\n"));
	}
...
VOID LinkUp(RTMP_ADAPTER *pAd, UCHAR BssType, struct wifi_dev *wdev, UINT link_up_type, MLME_QUEUE_ELEM *Elem)
{
	...
	if (pEntry == NULL) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			("%s:wdev(type=%d,func_idx=%d),pEntry=NULL,return!\n",
			__func__, wdev->wdev_type, wdev->func_idx));
		return;
	}
	...
	*/
	...
	if (IS_HIF_TYPE(pAd, HIF_MT)) {
		...
		MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s():Send HW_STA_BF_SOUNDING_ADJUST Cmd\n", __func__));
	...
	if (pStaCfg->ApCliMode == APCLI_MODE_START_AP_AFTER_APCLI_CONNECTION) {
			...
			wapp_send_apcli_association_change_vendor10(WAPP_APCLI_DISASSOCIATED, pAd, pStaCfg);
				...
				if (IS_REPT_IN_DISCONNECTING(rept)) {
					MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
						("%s:wdev(type=%d,fun_idx=%d) in disconnecting, return\n",
						__func__, wdev->wdev_type, wdev->func_idx));
...
VOID LinkDown(RTMP_ADAPTER *pAd, UINT linkdown_type, struct wifi_dev *wdev, MLME_QUEUE_ELEM *Elem)
{
	...
	if (wdev) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			("%s,wdev(type=%d,func_idx=%d)(caller:%pS)\n",
			__func__, wdev->wdev_type, wdev->func_idx, OS_TRACE));
	} else {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_OFF,
	...
	/* Reset WPA-PSK state. Only reset when supplicant enabled */
		...
		if (pStaCfg->WscControl.bWscTrigger)
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("WPS is in Prgress; Not sending this LINK DOWN\n"));
		else {
			...
			if (!MAC_ADDR_EQUAL(pAd->CommonCfg.LastBssid, ZERO_MAC_ADDR)) {
				...
				MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("LastBssid is zero; Not sending this LINK DOWN\n"));
		}
#endif /* WIDI_SUPPORT */
