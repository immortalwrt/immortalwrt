...
typedef VOID (*MSG_RSP_HANDLER)(struct cmd_msg *msg, char *payload,
				UINT16 payload_len);
...
typedef enum _ENUM_EXT_CMD_CR4_SET_ID_T {
	...
	CR4_SET_ID_NUM
} ENUM_EXT_CMD_CR4_SET_ID_T,
	*P_ENUM_EXT_CMD_CR4_SET_ID_T;
...
#define SET_CMD_ATTR_MCU_DEST(attr, cmd_dest) ((attr).mcu_dest = cmd_dest)
#define SET_CMD_ATTR_TYPE(attr, cmd_type) ((attr).type = cmd_type)
#define SET_CMD_ATTR_EXT_TYPE(attr, ext_cmd_type)                              \
	((attr).ext_type = ext_cmd_type)
#define SET_CMD_ATTR_CTRL_FLAGS(attr, cmd_flags) ((attr).ctrl.flags = cmd_flags)
...
#define SET_CMD_ATTR_RSP_HANDLER(attr, callback) ((attr).rsp.handler = callback)

/* CMD ATTRIBUTE ctrl flags related Macro */
#define GET_CMD_ATTR_CTRL_FLAGS(attr) (attr.ctrl.flags & CMD_FLAGS_MASK)
...
#define SET_CMD_MSG_SEQUENCE(msg, msg_sequence) ((msg)->seq = msg_sequence)
...
#define SET_CMD_MSG_TYPE(msg, cmd_type) SET_CMD_ATTR_TYPE((msg)->attr, cmd_type)
...
#define SET_CMD_MSG_RSP_WB_BUF_IN_CALBK(msg, write_back_buffer_in_callback)    \
	SET_CMD_ATTR_RSP_WB_BUF_IN_CALBK((msg)->attr,                          \
					 write_back_buffer_in_callback)
...
#define GET_MSG_NEED_SYNC_WITH_FW_FLAG(msg)                                    \
	((((msg)->attr.ctrl.flags & CMD_FLAG_NEED_SYNC_WITH_FW_MASK)) ? TRUE : \
									      FALSE)
...
#define GET_CMD_MSG_CTRL_FLAGS(msg) GET_CMD_ATTR_CTRL_FLAGS((msg)->attr)

#define IS_CMD_MSG_LEN_VAR_FLAG_SET(msg)                                       \
	IS_CMD_ATTR_LEN_VAR_FLAG_SET((msg)->attr)
#define IS_CMD_MSG_NA_FLAG_SET(msg) IS_CMD_ATTR_NA_FLAG_SET((msg)->attr)
...
#define IS_EXT_CMD_AND_SET_NEED_RSP(m)                                         \
	((((((struct cmd_msg *)(m))->attr.type) == EXT_CID) &&                 \
	  (IS_CMD_MSG_NEED_FW_RSP_FLAG_SET(m))) ?                              \
		 TRUE :                                                        \
		       FALSE)
...
} LINK_TEST_ACTION_CATEGORY,
	*P_LINK_TEST_ACTION_CATEGORY;
...
typedef struct GNU_PACKED _EXT_CMD_ETXBf_PFMU_PROFILE_DATA_W_20M_ALL_T {
	...
	UINT8 aucReserved[2];
	UINT8 ucBuf[512];
} EXT_CMD_ETXBf_PFMU_PROFILE_DATA_W_20M_ALL_T,
	*P_EXT_CMD_ETXBf_PFMU_PROFILE_DATA_W_20M_ALL_T;
...
/* EXT_CMD_RF_TEST */
/* ACTION */
#define ACTION_SWITCH_TO_RFTEST                                                \
	0 /* to switch firmware mode between normal mode or rf test mode */
...
/*u4CalDump*/
typedef enum { DISABLE_DUMP = 0x0, ENABLE_DUMP = 0x1 } TEST_CAL_DUMP;
...
#define MAX_BUF_SIZE_OF_DEVICEINFO                                             \
	(sizeof(CMD_DEVINFO_UPDATE_T) + sizeof(CMD_DEVINFO_ACTIVE_T))
...
typedef struct _STAREC_HANDLE_T {
	UINT32 StaRecTag;
	UINT32 StaRecTagLen;
	INT32(*StaRecTagHandler)
	(struct _RTMP_ADAPTER *pAd, struct cmd_msg *msg, VOID *args);
...
#define MAX_BUF_SIZE_OF_BSS_INFO                                               \
	(sizeof(CMD_BSSINFO_UPDATE_T) +                                        \
	 sizeof(CMD_BSSINFO_CONNECT_OWN_DEV_T) + sizeof(CMD_BSSINFO_BASIC_T) + \
	 sizeof(CMD_BSSINFO_RF_CH_T) + sizeof(CMD_BSSINFO_PM_T) +              \
	 sizeof(CMD_BSSINFO_UAPSD_T) + sizeof(CMD_BSSINFO_RSSI_RM_DET_T) +     \
...
typedef struct _BSS_INFO_HANDLE_T {
	...
	(struct _RTMP_ADAPTER *pAd,
	 struct _BSS_INFO_ARGUMENT_T bss_info_argument, struct cmd_msg *msg);
...
#define MAX_BUF_SIZE_OF_WTBL_INFO                                              \
	(sizeof(CMD_WTBL_UPDATE_T) + sizeof(CMD_WTBL_GENERIC_T) +              \
	 sizeof(CMD_WTBL_RX_T) + sizeof(CMD_WTBL_HT_T) +                       \
	 sizeof(CMD_WTBL_VHT_T) + sizeof(CMD_WTBL_PEER_PS_T) +                 \
	 sizeof(CMD_WTBL_TX_PS_T) + sizeof(CMD_WTBL_HDR_TRANS_T) +             \
	 sizeof(CMD_WTBL_SECURITY_KEY_T) + sizeof(CMD_WTBL_BA_T) +             \
	 sizeof(CMD_WTBL_RDG_T) + sizeof(CMD_WTBL_PROTECTION_T) +              \
	 sizeof(CMD_WTBL_CLEAR_T) + sizeof(CMD_WTBL_BF_T) +                    \
	 sizeof(CMD_WTBL_SMPS_T) + sizeof(CMD_WTBL_RAW_DATA_RW_T))
...
} ENUM_THERMAL_PROTECTION_REASON_T,
	*P_ENUM_THERMAL_PROTECTION_REASON_T;
...
typedef union GNU_PACKED _VOW_DRR_CTRL_VALUE_T {
	UINT32 u4ComValue;
	AIRTIME_QUA_ALL_FIELD_T
	rAirTimeQuantumAllField; /* used for ENUM_VOW_DRR_CTRL_FIELD_AIRTIME_QUANTUM_ALL ctrl only */
...
} ENUM_EXT_CMD_VOW_DRR_CTRL_ID_T,
	P_ENUM__EXT_CMD_VOW_DRR_CTRL_ID_T;
...
} ENUM_EXT_CMD_BSSGROUP_CTRL_ID_T,
	P_ENUM_EXT_CMD_BSSGROUP_CTRL_ID_T;
...
} ENUM_UMAC_VOW_REFILL_PERIOD_T,
	*P_ENUM_UMAC_VOW_REFILL_PERIOD_T;
...
} ENUM_RX_AT_AC_Q_MASK_T,
	*P_ENUM_RX_AT_AC_Q_MASK_T;
...
} ENUM_RX_AT_WMM_GROUP_IDX_T,
	*P_ENUM_RX_AT_WMM_GROUP_IDX_T;
...
} ENUM_RX_AT_SW_COMPENSATE_MODE_T,
	*P_ENUM_RX_AT_SW_COMPENSATE_MODE_T;
...
} ENUM_RX_AT_CTRL_FIELD_T,
	*P_ENUM_RX_AT_CTRL_FIELD_T;
...
} ENUM_RX_AT_FEATURE_SUB_FIELD_T,
	*P_ENUM_RX_AT_FEATURE_SUB_FIELD_T;
...
} ENUM_RX_AT_BITWISE_SUB_FIELD_T,
	*_ENUM_RX_AT_BITWISE_SUB_FIELD_T;
...
} ENUM_RX_AT_TIME_VALUE_SUB_FIELD_T,
	*P_ENUM_RX_AT_TIME_VALUE_SUB_FIELD_T;
...
} ENUM_RX_AT_TIME_REPORT_SUB_FIELD_T,
	*P_ENUM_RX_AT_TIME_REPORT_SUB_FIELD_T;
...
} ENUM_AT_ESTIMATE_SUB_FIELD_T,
	*P_ENUM_AT_ESTIMATE_SUB_FIELD_T;
...
} ENUM_AT_BAD_NODE_SUB_FIELD_T,
	*P_ENUM_AT_BAD_NODE_SUB_FIELD_T;
...
} ENUM_AT_RPOCESS_FIELD_T,
	*P_ENUM_AT_RPOCESS_FIELD_T;
...
#define CMD_EDCA_ALL_BITS                                                      \
	(CMD_EDCA_AIFS_BIT | CMD_EDCA_WIN_MIN_BIT | CMD_EDCA_WIN_MAX_BIT |     \
	 CMD_EDCA_TXOP_BIT)
...
enum { EDCA_ACT_SET, EDCA_ACT_GET, EDCA_ACT_MAX };
...
typedef struct GNU_PACKED _EXT_CMD_BCN_OFFLOAD_T {
	...
	UINT8 acPktContent
		[1520]; /* whole pkt template length which include TXD, max shall not exceed 1520 bytes. */
#else
	UINT8 acPktContent
		[512]; /* whole pkt template length which include TXD, max shall not exceed 512 bytes. */
#endif
...
enum _TMR_CTRL_TYPE_T { SET_TMR_ENABLE = 0x00, TMR_CALIBRATION = 0x01 };
...
typedef struct GNU_PACKED _EXT_CMD_GENERAL_TEST_T {
	...
	union {
		 ...
		 */
		GENERAL_TEST_SIM_ERR_SWITCH_ON_OFF_T
		rGeneralTestSimErrorSwitchOnOff;

		/* for category = GENERAL_TEST_CATEGORY_SIM_ERROR_DETECTION
		 * and ucAction = GENERAL_TEST_ACTION_RECOVERY
		 */
		GENERAL_TEST_SIM_ERR_DET_RECOVERY_T
		rGeneralTestSimErrDetRecovery;
	} Data;
...
} EXT_CMD_DBDC_CTRL_T, *P_EXT_CMD_DBDC_CTRL_T, EXT_EVENT_DBDC_CTRL_T,
	*P_EXT_EVENT_DBDC_CTRL_T;
...
} ENUM_MAC_ENABLE_CTRL_T,
	*P_ENUM_MAC_ENABLE_CTRL_T;
...
#if defined(PRE_CAL_TRX_SET1_SUPPORT) || defined(RLM_CAL_CACHE_SUPPORT) ||     \
	defined(PRE_CAL_TRX_SET2_SUPPORT)
...
typedef enum { CAL_RESTORE_PA_TRIM = 0x00 } CAL_RESTORE_FUNC_IDX;
#endif /* CAL_BIN_FILE_SUPPORT */
...
#define CR4_GET_BSS_ACQ_PKT_NUM_CMD_DEFAULT                                    \
	(CR4_GET_BSS_ACQ_PKT_NUM_READ_CLEAR_EN |                               \
	 CR4_GET_BSS_ACQ_PKT_NUM_BSS_GROUP_DEFAULT)
...
BOOLEAN isIgmpMldFloodingPkt(IN struct _RTMP_ADAPTER *pAd,
			     IN PUCHAR pGroupIpAddr, IN UINT16 ProtoType);
...
typedef enum _EAP_FEATURE_CATEGORY {
	...
	GET_RATE_INFO = 0x6,
	EAP_FEATURE_NUM
} EAP_FEATURE_CATEGORY,
	*P_EAP_FEATURE_CATEGORY;
...
typedef enum _ENUM_RA_SWITCH_TABLE {
	...
	eRateSwTblhe2ss,
	eRateSwTblMax = 0xff
} ENUM_RA_SWITCH_TABLE,
	*P_ENUM_RA_SWITCH_TABLE;
...
typedef struct _EXT_EVENT_TXPOWER_INFO_T {
	...
	UINT32 u4RatePowerCRValue
		[8]; /* (TMAC) Band0: 0x820F4020~0x820F403C, Band1: 0x820F4040~0x820F405C */
	INT8 cTxPwrBFBackoffValue[BF_BACKOFF_MODE][BF_BACKOFF_CASE];
	UINT32 u4BackoffCRValue
		[6]; /* (BBP) Band0: 0x8207067C~82070690, Band1: 0x8207087C~82070890 */
...
} EXT_EVENT_TX_POWER_BACKUP_TABLE_INFO_T,
	*P_EXT_EVENT_TX_POWER_BACKUP_TABLE_INFO_T;
...
typedef struct _EXT_EVENT_THERMAL_COMPENSATION_TABLE_INFO_T {
	...
	UINT8 aucReserve2;
} EXT_EVENT_THERMAL_COMPENSATION_TABLE_INFO_T,
	*P_EXT_EVENT_THERMAL_COMPENSATION_TABLE_INFO_T;
...
} INIT_CMD_WIFI_START_WITH_DECOMPRESSION,
	*P_INIT_CMD_WIFI_START_WITH_DECOMPRESSION;
...
BOOLEAN MtUpdateBcnAndTimToMcu(IN struct _RTMP_ADAPTER *pAd, VOID *wdev_void,
			       IN UINT16 FrameLen, IN UCHAR UpdatePktType);

INT32 MtCmdBcnOffloadSet(struct _RTMP_ADAPTER *pAd,
			 CMD_BCN_OFFLOAD_T *bcn_offload);
#endif

INT32 MtCmdFdFrameOffloadSet(struct _RTMP_ADAPTER *pAd,
			     P_CMD_FD_FRAME_OFFLOAD_T fdFrame_offload);
INT32 MtCmdMuarConfigSet(struct _RTMP_ADAPTER *pAd, UCHAR *pdata);

INT32 MtCmdExtPwrMgtBitWifi(struct _RTMP_ADAPTER *pAd,
...
#ifdef GREENAP_SUPPORT
INT32 MtCmdExtGreenAPOnOffCtrl(struct _RTMP_ADAPTER *pAd,
			       MT_GREENAP_CTRL_T GreenAPCtrl);
#endif /* GREENAP_SUPPORT */

#ifdef PCIE_ASPM_DYM_CTRL_SUPPORT
INT32 mt_cmd_ext_pcie_aspm_dym_ctrl(struct _RTMP_ADAPTER *pAd,
				    MT_PCIE_ASPM_DYM_CTRL_T PcieAspmDymCtrl);
#endif /* PCIE_ASPM_DYM_CTRL_SUPPORT */

INT32 MtCmdExtPmStateCtrl(struct _RTMP_ADAPTER *pAd,
			  MT_PMSTAT_CTRL_T PmStatCtrl);

UCHAR GetCfgBw2RawBw(UCHAR CfgBw);

INT32 CmdExtPwrMgtBitWifi(struct _RTMP_ADAPTER *pAd, UINT8 ucWlanIdx,
			  UINT8 ucPwrMgtBit);

INT32 MtCmdRestartDLReq(struct _RTMP_ADAPTER *pAd);

INT32 MtCmdHIFLoopBackTest(struct _RTMP_ADAPTER *pAdapter, BOOLEAN IsEnable,
			   UINT8 RxQ);
...
INT32 MtCmdAddressLenReq(struct _RTMP_ADAPTER *pAd, UINT32 address, UINT32 len,
			 UINT32 data_mode);

INT32 MtCmdFwScatter(struct _RTMP_ADAPTER *pAd, UINT8 *dl_payload,
		     UINT32 dl_len, UINT32 count);

INT32 MtCmdPatchFinishReq(struct _RTMP_ADAPTER *pAd);

INT32 MtCmdFwStartReq(struct _RTMP_ADAPTER *pAd, UINT32 override,
		      UINT32 address);

INT32 MtCmdWifiHifCtrl(struct _RTMP_ADAPTER *pAd, UINT8 ucDbdcIdx,
		       UINT8 ucHifCtrlId, VOID *pRsult);

INT32 CmdInitAccessRegWrite(struct _RTMP_ADAPTER *pAd, UINT32 address,
			    UINT32 data);

INT32 CmdInitAccessRegRead(struct _RTMP_ADAPTER *pAd, UINT32 address,
			   UINT32 *data);

INT32 CmdChPrivilege(struct _RTMP_ADAPTER *pAd, UINT8 Action, UINT8 control_chl,
		     UINT8 central_chl, UINT8 BW, UINT8 TXStream,
		     UINT8 RXStream);
...
INT32 MtCmdRFRegAccessWrite(struct _RTMP_ADAPTER *pAd, UINT32 RFIdx,
			    UINT32 Offset, UINT32 Value);

INT32 MtCmdRFRegAccessRead(struct _RTMP_ADAPTER *pAd, UINT32 RFIdx,
			   UINT32 Offset, UINT32 *Value);
INT32 MtCmdRadioOnOffCtrl(struct _RTMP_ADAPTER *pAd, UINT8 On);

INT32 MtCmdWiFiRxDisable(struct _RTMP_ADAPTER *pAd);

INT32 MtCmdChannelSwitch(struct _RTMP_ADAPTER *pAd,
...
INT32 MtCmdSecKeyReq(struct _RTMP_ADAPTER *pAd, UINT8 AddRemove, UINT8 Keytype,
		     UINT8 *pAddr, UINT8 Alg, UINT8 KeyID, UINT8 KeyLen,
		     UINT8 WlanIdx, UINT8 *KeyMaterial);

INT32 MtCmdRfTestSwitchMode(struct _RTMP_ADAPTER *pAd, UINT32 OpMode,
			    UINT8 IcapLen, UINT16 rsp_len);

#ifdef WIFI_SPECTRUM_SUPPORT
INT32 MtCmdWifiSpectrumStart(IN struct _RTMP_ADAPTER *pAd, IN UINT8 *pData);

INT32 MtCmdWifiSpectrumGen1Status(IN struct _RTMP_ADAPTER *pAd);

INT32 MtCmdWifiSpectrumGen2Status(IN struct _RTMP_ADAPTER *pAd);

VOID MtCmdWifiSpectrumStatusCb(IN struct cmd_msg *msg, IN INT8 *pData,
			       IN UINT16 Length);

INT32 MtCmdWifiSpectrumRawDataProc(IN struct _RTMP_ADAPTER *pAd);
#endif /* WIFI_SPECTRUM_SUPPORT */

#ifdef INTERNAL_CAPTURE_SUPPORT
INT32 MtCmdRfTestICapStart(IN struct _RTMP_ADAPTER *pAd, IN UINT8 *pData);

INT32 MtCmdRfTestGen1ICapStatus(IN struct _RTMP_ADAPTER *pAd);

INT32 MtCmdRfTestGen2ICapStatus(IN struct _RTMP_ADAPTER *pAd);

VOID MtCmdRfTestICapStatusCb(IN struct cmd_msg *msg, IN INT8 *pData,
			     IN UINT16 Length);

INT32 MtCmdRfTestICapRawDataProc(IN struct _RTMP_ADAPTER *pAd);
#endif /* INTERNAL_CAPTURE_SUPPORT */

INT32 MtCmdRfTestSetADC(struct _RTMP_ADAPTER *pAd, UINT32 ChannelFreq,
			UINT8 AntIndex, UINT8 BW, UINT8 SX, UINT8 DbdcIdx,
			UINT8 RunType, UINT8 FType);

INT32 MtCmdRfTestSetRxGain(struct _RTMP_ADAPTER *pAd, UINT8 LPFG, UINT8 LNA,
			   UINT8 DbdcIdx, UINT8 AntIndex);

INT32 MtCmdRfTestSetTTG(struct _RTMP_ADAPTER *pAd, UINT32 ChannelFreq,
			UINT32 ToneFreq, UINT8 TTGPwrIdx, UINT8 XtalFreq,
			UINT8 DbdcIdx);

INT32 MtCmdRfTestSetTTGOnOff(struct _RTMP_ADAPTER *pAd, UINT8 TTGEnable,
			     UINT8 DbdcIdx, UINT8 AntIndex);

INT32 MtCmdDoCalibration(struct _RTMP_ADAPTER *pAd, UINT32 func_idx,
			 UINT32 CalItem, UINT32 band_idx);

INT32 MtCmdTxContinous(struct _RTMP_ADAPTER *pAd, UINT32 PhyMode, UINT32 BW,
		       UINT32 PriCh, UINT32 Central_Ch, UINT32 Mcs,
		       UINT32 WFSel, UINT32 Txfd, UINT8 Band, UINT8 onoff);

INT32 MtCmdTxTone(struct _RTMP_ADAPTER *pAd, UINT8 BandIdx, UINT8 Control,
		  UINT8 AntIndex, UINT8 ToneType, UINT8 ToneFreq,
		  INT32 DcOffset_I, INT32 DcOffset_Q, UINT32 Band);

INT32 MtCmdTxTonePower(struct _RTMP_ADAPTER *pAd, INT32 type, INT32 dec,
		       UINT8 TxAntennaSel, UINT8 Band);

INT32 MtCmdSetRDDTestExt(struct _RTMP_ADAPTER *pAd, UINT32 rdd_idx,
			 UINT32 rdd_in_sel, UINT32 IsStart);
...
INT32 MtCmdMultipleMacRegAccessWrite(struct _RTMP_ADAPTER *pAd,
				     struct _RTMP_REG_PAIR *RegPair,
				     UINT32 Num);

INT32 MtCmdMultipleMacRegAccessRead(struct _RTMP_ADAPTER *pAd,
				    struct _RTMP_REG_PAIR *RegPair, UINT32 Num);

INT32 MtCmdMultipleRfRegAccessWrite(struct _RTMP_ADAPTER *pAd,
				    struct _MT_RF_REG_PAIR *RegPair,
				    UINT32 Num);

INT32 MtCmdMultipleRfRegAccessRead(struct _RTMP_ADAPTER *pAd,
				   struct _MT_RF_REG_PAIR *RegPair, UINT32 Num);

INT32 MtCmdThermoCal(struct _RTMP_ADAPTER *pAd, UINT8 IsEnable,
		     UINT8 SourceMode, UINT8 RFDiffTemp, UINT8 HiBBPHT,
		     UINT8 HiBBPNT, INT8 LoBBPLT, INT8 LoBBPNT);

INT32 MtCmdFwLog2Host(struct _RTMP_ADAPTER *pAd, UINT8 McuDest,
		      UINT8 FWLog2HostCtrl);
...
INT32 MtCmdSetRxvFilter(struct _RTMP_ADAPTER *pAd, UCHAR BandIdx,
			BOOLEAN bEnable);
...
INT32 MtCmdSendRaw(struct _RTMP_ADAPTER *pAd, UCHAR ExtendID, UCHAR *Input,
		   INT len, UCHAR SetQuery);

#if defined(TXRX_STAT_SUPPORT) || defined(EAP_STATS_SUPPORT)
INT32 MtCmdGetPerStaTxStat(struct _RTMP_ADAPTER *pAd, UINT8 *ucEntryBitmap,
			   UINT8 ucEntryCount);
#endif

#if defined(CUSTOMER_RSG_FEATURE) || defined(CUSTOMER_DCC_FEATURE)
INT32 MtCmdGetWtblTxStat(struct _RTMP_ADAPTER *pAd, UINT32 u4Field,
			 UINT8 ucWcid);
#endif

#ifdef CONFIG_ATE
INT32 MtCmdGetTxPower(struct _RTMP_ADAPTER *pAd, UINT8 pwrType, UINT8 centerCh,
		      UINT8 dbdc_idx, UINT8 Ch_Band,
		      P_EXT_EVENT_ID_GET_TX_POWER_T prTxPwrResult);

INT32 MtCmdSetTxPowerCtrl(struct _RTMP_ADAPTER *pAd,
			  struct _ATE_TXPOWER TxPower);
INT32 MtCmdSetForceTxPowerCtrl(struct _RTMP_ADAPTER *pAd, UINT8 ucBandIdx,
			       INT8 cTxPower, UINT8 ucPhyMode, UINT8 ucTxRate,
			       UINT8 ucBW);
#endif /* CONFIG_ATE */

#ifdef MT_MAC
INT32 CmdETxBfAidSetting(struct _RTMP_ADAPTER *pAd, UINT16 Aid);

INT32 CmdTxBfApClientCluster(struct _RTMP_ADAPTER *pAd, UCHAR ucWlanIdx,
			     UCHAR ucCmmWlanId);

INT32 CmdTxBfReptClonedStaToNormalSta(struct _RTMP_ADAPTER *pAd,
				      UCHAR ucWlanIdx, UCHAR ucCliIdx);

INT32 CmdTxBfTxApplyCtrl(struct _RTMP_ADAPTER *pAd, UCHAR ucWlanId,
			 BOOLEAN fgETxBf, BOOLEAN fgITxBf, BOOLEAN fgMuTxBf,
			 BOOLEAN fgPhaseCali);

INT32 CmdITxBfPhaseCal(struct _RTMP_ADAPTER *pAd, UCHAR ucGroup,
		       UCHAR ucGroupL_M_H, BOOLEAN fgSX2, BOOLEAN ucPhaseCal,
		       UCHAR ucPhaseVerifyLnaGainLevel);

INT32 CmdITxBfPhaseComp(struct _RTMP_ADAPTER *pAd, UCHAR ucBW, UCHAR ucBand,
			UCHAR ucDbdcBandIdx, UCHAR ucGroup, BOOLEAN fgRdFromE2p,
			BOOLEAN fgDisComp);

INT32 CmdTxBfLnaGain(struct _RTMP_ADAPTER *pAd, UCHAR ucLnaGain);

INT32 CmdETxBfSoundingPeriodicTriggerCtrl(struct _RTMP_ADAPTER *pAd,
					  UCHAR SndgEn, UINT32 u4SNDPeriod,
					  UCHAR ucSu_Mu, UCHAR ucMuNum,
					  PUCHAR pwlanidx);

INT32 CmdPfmuMemAlloc(struct _RTMP_ADAPTER *pAd, UCHAR ucSu_Mu,
		      UCHAR ucWlanIdx);

INT32 CmdPfmuMemRelease(struct _RTMP_ADAPTER *pAd, UCHAR ucWlanIdx);

INT32 CmdPfmuMemAllocMapRead(struct _RTMP_ADAPTER *pAd);

INT32 CmdETxBfPfmuProfileTagRead(struct _RTMP_ADAPTER *pAd, UCHAR PfmuIdx,
				 BOOLEAN fgBFer);

INT32 CmdETxBfPfmuProfileTagWrite(struct _RTMP_ADAPTER *pAd, PUCHAR prPfmuTag1,
				  PUCHAR prPfmuTag2, UCHAR PfmuIdx);

INT32 CmdETxBfPfmuProfileDataRead(struct _RTMP_ADAPTER *pAd, UCHAR PfmuIdx,
				  BOOLEAN fgBFer, USHORT SubCarrIdx);

INT32 CmdETxBfPfmuProfileDataWrite(struct _RTMP_ADAPTER *pAd, UCHAR PfmuIdx,
				   USHORT SubCarrIdx, PUCHAR pProfileData);

INT32 CmdETxBfPfmuProfileDataWrite20MAll(struct _RTMP_ADAPTER *pAd,
					 UCHAR PfmuIdx, PUCHAR pProfileData);

INT32 CmdETxBfPfmuProfilePnRead(struct _RTMP_ADAPTER *pAd, UCHAR PfmuIdx);

INT32 CmdETxBfPfmuProfilePnWrite(struct _RTMP_ADAPTER *pAd, UCHAR PfmuIdx,
				 UCHAR ucBw, PUCHAR pProfileData);

INT32 CmdETxBfQdRead(struct _RTMP_ADAPTER *pAd, INT8 subCarrIdx);
#endif /* MT_MAC */
...
INT AndesCoexProtectionFrameOP(struct _RTMP_ADAPTER *pAd, UCHAR Mode,
			       UCHAR Rate);
...
typedef struct _IGMP_MULTICAST_TABLE_ENTRY {
	...
	UINT8 Rsvd2[2];
	IGMP_MULTICAST_TABLE_MEMBER IgmpMcastMember
		[1]; /* This member will be multiple of NumOfMember, shows variable structure */
...
typedef struct _IGMP_MULTICAST_TABLE {
	...
	UINT16 TotalSize;
	IGMP_MULTICAST_TABLE_ENTRY
	*pNxtFreeGroupLocation; /* Used only in driver */
	IGMP_MULTICAST_TABLE_ENTRY IgmpMcastTableEntry
		[1]; /* This member will be multiple of NumOfGroup, shows variable structure */
...
INT32 MtCmdEfuseAccessRead(struct _RTMP_ADAPTER *pAd, USHORT offset,
			   PUCHAR pData, PUINT isVaild);

VOID MtCmdEfuseAccessWrite(struct _RTMP_ADAPTER *pAd, USHORT offset,
			   PUCHAR pData);

INT32 MtCmdEfuseFreeBlockCount(struct _RTMP_ADAPTER *pAd, UINT32 GetFreeBlock,
			       UINT32 *Value);
INT32 MtCmdEfuseAccessCheck(struct _RTMP_ADAPTER *pAd, UINT32 offset,
			    PUCHAR pData);

#endif /* RTMP_EFUSE_SUPPORT */

INT32 MtCmdThermalProtect(struct _RTMP_ADAPTER *pAd, UINT8 HighEn,
			  CHAR HighTempTh, UINT8 LowEn, CHAR LowTempTh,
			  UINT32 RechkTimer, UINT8 RFOffEn, CHAR RFOffTh,
			  UINT8 ucType);

INT32
MtCmdThermalProtectAdmitDuty(struct _RTMP_ADAPTER *pAd, UINT32 u4Lv0Duty,
			     UINT32 u4Lv1Duty, UINT32 u4Lv2Duty,
			     UINT32 u4Lv3Duty);

#ifdef PRETBTT_INT_EVENT_SUPPORT
VOID MtSetTriggerPretbttIntEvent(struct _RTMP_ADAPTER *ad, INT apidx,
				 UCHAR HWBssidIdx, BOOLEAN Enable,
				 UINT16 BeaconPeriod);
...
VOID *pTlvAppend(VOID *pTlvBuffer, UINT16 u2Type, UINT16 u2Length,
		 VOID *pNextTlvBuffer, UINT32 *pu4TotalTlvLen,
		 UCHAR *pucTotalTlvNumber);
INT32 CmdExtTlvBufferSend(struct _RTMP_ADAPTER *pAd, UINT8 ExtCmdType,
			  VOID *pTlvBuffer, UINT32 u4TlvLength);
INT32 CmdExtWtblUpdate(struct _RTMP_ADAPTER *pAd, UINT8 ucWlanIdx,
		       UINT8 ucOperation, VOID *pBuffer, UINT32 u4BufferLen);
UINT32 WtblDwQuery(struct _RTMP_ADAPTER *pAd, UINT8 ucWlanIdx, UINT8 ucWtbl1234,
		   UINT8 ucWhichDW);
INT32 WtblDwSet(struct _RTMP_ADAPTER *pAd, UINT8 ucWlanIdx, UINT8 ucWtbl1234,
...
INT32 WtblResetAndDWsSet(struct _RTMP_ADAPTER *pAd, UINT8 ucWlanIdx,
			 UINT8 ucWtbl1234, INT dw_cnt,
			 struct cmd_wtbl_dw_mask_set *dw_set);

INT32 CmdExtDevInfoUpdate(struct _RTMP_ADAPTER *pAd, UINT8 OwnMacIdx,
			  UINT8 *OwnMacAddr, UINT8 BandIdx, UINT8 Active,
			  UINT32 EnableFeature);

INT32 CmdExtSetTmrCR(struct _RTMP_ADAPTER *pAd, UCHAR enable, UCHAR BandIdx);

INT32 CmdExtStaRecUpdate(struct _RTMP_ADAPTER *pAd, STA_REC_CFG_T StaRecCfg);

INT32 CmdETxBfStaRecRead(struct _RTMP_ADAPTER *pAd, UCHAR ucWlanID);

INT32 CmdTxBfTxPwrBackOff(struct _RTMP_ADAPTER *pAd, UCHAR ucBandIdx,
			  PCHAR pacTxPwrFccBfOnCase,
			  PCHAR pacTxPwrFccBfOffCase);

INT32 CmdTxBfAwareCtrl(struct _RTMP_ADAPTER *pAd, BOOLEAN fgBfAwareCtrl);

INT32 CmdTxBfeeHwCtrl(struct _RTMP_ADAPTER *pAd, BOOLEAN fgBfeeHwEn);

INT32 CmdTxBfHwEnableStatusUpdate(struct _RTMP_ADAPTER *pAd, BOOLEAN fgEBf,
				  BOOLEAN fgIBf);

INT32 CmdTxBfModuleEnCtrl(struct _RTMP_ADAPTER *pAd, UINT8 u1BfNum,
			  UINT8 u1BfBitmap, UINT8 u1BfSelBand[]);

INT32 CmdExtStaRecBaUpdate(struct _RTMP_ADAPTER *pAd,
...
INT32 CmdExtBssInfoUpdate(struct _RTMP_ADAPTER *pAd,
			  struct _BSS_INFO_ARGUMENT_T bss_info_argument);

#ifdef CONFIG_HW_HAL_OFFLOAD
INT32 MtCmdATETest(struct _RTMP_ADAPTER *pAd,
		   struct _EXT_CMD_ATE_TEST_MODE_T *param);
INT32 MtCmdCfgOnOff(struct _RTMP_ADAPTER *pAd, UINT8 Type, UINT8 Enable,
		    UINT8 Band);
INT32 MtCmdSetAntennaPort(struct _RTMP_ADAPTER *pAd, UINT8 RfModeMask,
			  UINT8 RfPortMask, UINT8 AntPortMask);
INT32 MtCmdATESetSlotTime(struct _RTMP_ADAPTER *pAd, UINT8 SlotTime,
			  UINT8 SifsTime, UINT8 RifsTime, UINT16 EifsTime,
			  UCHAR BandIdx);
INT32 MtCmdATESetPowerDropLevel(struct _RTMP_ADAPTER *pAd, UINT8 PowerDropLevel,
				UCHAR BandIdx);
INT32 MtCmdRxFilterPktLen(struct _RTMP_ADAPTER *pAd, UINT8 Enable, UINT8 Band,
			  UINT32 RxPktLen);
...
INT32 MtCmdGetCfgOnOff(struct _RTMP_ADAPTER *pAd, UINT32 Type, UINT8 Band,
		       UINT32 *Status);
INT32 MtCmdSetPhyCounter(struct _RTMP_ADAPTER *pAd, UINT32 Control,
			 UINT8 band_idx);
INT32 MtCmdSetRxvIndex(struct _RTMP_ADAPTER *pAd, UINT8 Group_1, UINT8 Group_2,
		       UINT8 band_idx);
...
#ifdef CONFIG_ATE
INT32 CmdTxContinous(struct _RTMP_ADAPTER *pAd, UINT32 PhyMode, UINT32 BW,
		     UINT32 PriCh, UINT32 Mcs, UINT32 WFSel, UCHAR onoff);
INT32 CmdTxTonePower(struct _RTMP_ADAPTER *pAd, INT32 type, INT32 dec);
#endif
INT32 MtCmdGetThermalSensorResult(struct _RTMP_ADAPTER *pAd, UINT8 ActionIdx,
				  UINT32 *SensorResult);
...
#define SET_STAREC_BW_STATS_SUPPORT(enable) ((Is2040StatFeatureEnbl) = enable)
#define IF_STAREC_BW_STATS_SUPPORT() ((Is2040StatFeatureEnbl) == TRUE)
...
INT32 MtCmdGetTxStatistic(
	struct _RTMP_ADAPTER *pAd, UINT32 u4Field, UINT8 ucBand, UINT8 ucWcid,
...
INT32 MtCmdTmrCal(struct _RTMP_ADAPTER *pAd, UINT8 enable, UINT8 band, UINT8 bw,
		  UINT8 ant, UINT8 role);

INT32 MtCmdEdcaParameterSet(struct _RTMP_ADAPTER *pAd,
			    MT_EDCA_CTRL_T EdcaParam);

INT32 MtCmdSlotTimeSet(struct _RTMP_ADAPTER *pAd, UINT8 SlotTime,
		       UINT8 SifsTime, UINT8 RifsTime, UINT16 EifsTime,
		       UCHAR BandIdx);
...
INT32 MtCmdLEDCtrl(struct _RTMP_ADAPTER *pAd, UINT32 LEDNumber,
		   UINT32 LEDBehavior);

#ifdef ERR_RECOVERY
INT32 CmdExtGeneralTestOn(struct _RTMP_ADAPTER *pAd, BOOLEAN enable);
INT32 CmdExtGeneralTestMode(struct _RTMP_ADAPTER *pAd, UINT8 mode,
			    UINT8 submode);
#endif /* ERR_RECOVERY */

#ifdef DBDC_MODE
INT32 MtCmdGetDbdcCtrl(struct _RTMP_ADAPTER *pAd,
		       struct _BCTRL_INFO_T *pBandInfo);
INT32 MtCmdSetDbdcCtrl(struct _RTMP_ADAPTER *pAd,
		       struct _BCTRL_INFO_T *pBandInfo);
#endif

#ifdef MT_DFS_SUPPORT /* Jelly20150123 */
INT32 MtCmdRddCtrl(IN struct _RTMP_ADAPTER *pAd, IN UCHAR ucRddCtrl,
		   IN UCHAR ucRddIdex, IN UCHAR ucRddInSel, IN UCHAR ucSetVal);
...
INT32 MtCmdSetRadarThresholdParam(
	struct _RTMP_ADAPTER *pAd,
	P_CMD_RDM_RADAR_THRESHOLD_UPDATE_T pRadarThreshold);
INT32 MtCmdSetPulseThresholdParam(
	struct _RTMP_ADAPTER *pAd,
	P_CMD_RDM_PULSE_THRESHOLD_UPDATE_T pPulseThreshold);
INT32 MtCmdSetRddLogConfigUpdate(struct _RTMP_ADAPTER *pAd,
				 UINT8 ucHwRDDLogEnable, UINT8 ucSwRDDLogEnable,
				 UINT8 ucSwRDDLogCond);
#endif /* CUSTOMISE_RDD_THRESHOLD_SUPPORT */

#ifdef RDM_FALSE_ALARM_DEBUG_SUPPORT
INT32 MtCmdSetTestRadarPattern(struct _RTMP_ADAPTER *pAd,
			       P_CMD_RDM_TEST_RADAR_PATTERN_T pTestPulsePattern);
#endif

INT32 MtCmdGetEdca(struct _RTMP_ADAPTER *pAd, MT_EDCA_CTRL_T *pEdcaCtrl);
INT32 MtCmdGetTsfTime(struct _RTMP_ADAPTER *pAd, UCHAR HwBssidIdx,
		      TSF_RESULT_T *pTsfResult);
INT32 MtCmdGetChBusyCnt(struct _RTMP_ADAPTER *pAd, UCHAR ChIdx,
			UINT32 *pChBusyCnt);
INT32 MtCmdGetWifiInterruptCnt(struct _RTMP_ADAPTER *pAd, UCHAR ChIdx,
			       UCHAR WifiIntNum, UINT32 WifiIntMask,
			       UINT32 *pWifiInterruptCnt);
INT32 MtCmdSetMacTxRx(struct _RTMP_ADAPTER *pAd, UCHAR BandIdx,
		      BOOLEAN bEnable);
...
#endif /* PRE_CAL_TRX_SET1_SUPPORT */
#ifdef PRE_CAL_MT7622_SUPPORT
INT32 MtCmdRfTestRecal(struct _RTMP_ADAPTER *pAd, UINT32 u4CalId,
		       UINT16 rsp_len);
#endif /*PRE_CAL_MT7622_SUPPORT*/
...
#ifdef PRE_CAL_TRX_SET2_SUPPORT
INT32 MtCmdGetPreCalResult(struct _RTMP_ADAPTER *pAd, UINT8 CalId,
			   UINT16 PreCalBitMap);
...
#if defined(CAL_BIN_FILE_SUPPORT) && defined(MT7615)
INT32 MtCmdCalReStoreFromFileProc(struct _RTMP_ADAPTER *pAd,
				  CAL_RESTORE_FUNC_IDX FuncIdx);
INT32 MtCmdPATrimReStoreProc(struct _RTMP_ADAPTER *pAd);
#endif /* CAL_BIN_FILE_SUPPORT */

INT32 CmdRxHdrTransUpdate(struct _RTMP_ADAPTER *pAd, BOOLEAN En,
			  BOOLEAN ChkBssid, BOOLEAN InSVlan, BOOLEAN RmVlan,
			  BOOLEAN SwPcP);
INT32 CmdRxHdrTransBLUpdate(struct _RTMP_ADAPTER *pAd, UINT8 Index, UINT8 En,
			    UINT16 EthType);
#ifdef VOW_SUPPORT
...
INT32 MtCmdSetEmptyThreshold(struct _RTMP_ADAPTER *pAd, UINT8 McuDest,
			     UINT32 threshold);
...
INT32 MtCmdSetRedTargetDelay(struct _RTMP_ADAPTER *pAd, UINT8 McuDest,
			     UINT32 Num);
#endif /* RED_SUPPORT */
INT32 MtCmdSetCPSEnable(struct _RTMP_ADAPTER *pAd, UINT8 McuDest, UINT32 en);
INT32 CmdAutoBATrigger(struct _RTMP_ADAPTER *pAd, BOOLEAN Enable,
		       UINT32 Timeout);

INT32 MtCmdCr4Query(struct _RTMP_ADAPTER *pAd, UINT32 arg0, UINT32 arg1,
		    UINT32 arg2);
INT32 MtCmdCr4Set(struct _RTMP_ADAPTER *pAd, UINT32 arg0, UINT32 arg1,
		  UINT32 arg2);
...
INT MtCmdSetTxRxPath(struct _RTMP_ADAPTER *pAd,
		     struct _MT_SWITCH_CHANNEL_CFG SwChCfg);
INT32 MtCmdCr4QueryBssAcQPktNum(struct _RTMP_ADAPTER *pAd, UINT32 u4bssbitmap);

#ifdef TXPWRMANUAL
INT32 MtCmdTxPowerManualCtrl(struct _RTMP_ADAPTER *pAd, BOOLEAN fgPwrManCtrl,
...
#ifdef BACKGROUND_SCAN_SUPPORT
INT32 MtCmdBgndScan(struct _RTMP_ADAPTER *pAd,
		    struct _MT_BGND_SCAN_CFG BgScCfg);
INT32 MtCmdBgndScanNotify(struct _RTMP_ADAPTER *pAd,
			  struct _MT_BGND_SCAN_NOTIFY BgScNotify);
#endif /* BACKGROUND_SCAN_SUPPORT */

INT32 CmdExtGeneralTestAPPWS(struct _RTMP_ADAPTER *pAd, UINT action);
#ifdef IGMP_SNOOP_SUPPORT
INT32 CmdMcastCloneEnable(struct _RTMP_ADAPTER *pAd, UINT Enable,
			  UINT8 omac_idx);
BOOLEAN CmdMcastEntryInsert(struct _RTMP_ADAPTER *pAd, PUCHAR GrpAddr,
			    UINT8 BssIdx, UINT8 Type, PUCHAR MemberAddr,
			    PNET_DEV dev, UINT8 WlanIndex);
BOOLEAN CmdMcastEntryDelete(struct _RTMP_ADAPTER *pAd, PUCHAR GrpAddr,
			    UINT8 BssIdx, PUCHAR MemberAddr, PNET_DEV dev,
			    UINT8 WlanIndex);
#ifdef IGMP_TVM_SUPPORT
BOOLEAN CmdSetMcastEntryAgeOut(struct _RTMP_ADAPTER *pAd, UINT8 AgeOutTime,
			       UINT8 omac_idx);
BOOLEAN CmdGetMcastEntryTable(struct _RTMP_ADAPTER *pAd, UINT8 omac_idx,
...
INT32 MtCmdTxPowerSKUCtrl(struct _RTMP_ADAPTER *pAd, BOOLEAN fgTxPowerSKUEn,
			  UCHAR BandIdx);
INT32 MtCmdTxCCKStream(struct _RTMP_ADAPTER *pAd, UINT8 u1CCKTxStream,
		       UCHAR BandIdx);
INT32 MtCmdTxPowerPercentCtrl(struct _RTMP_ADAPTER *pAd,
			      BOOLEAN fgTxPowerPercentEn, UCHAR BandIdx);
INT32 MtCmdTxPowerDropCtrl(struct _RTMP_ADAPTER *pAd, UINT8 ucPowerDrop,
			   UCHAR BandIdx);
INT32 MtCmdTxBfBackoffCtrl(struct _RTMP_ADAPTER *pAd, BOOLEAN fgTxBFBackoffEn,
			   UCHAR BandIdx);
INT32 MtCmdThermoCompCtrl(struct _RTMP_ADAPTER *pAd, BOOLEAN fgThermoCompEn,
			  UCHAR BandIdx);
INT32 MtCmdTxPwrRfTxAntCtrl(struct _RTMP_ADAPTER *pAd, UINT8 ucTxAntIdx);
INT32 MtCmdTxPwrShowInfo(struct _RTMP_ADAPTER *pAd, UCHAR ucTxPowerInfoCatg,
			 UINT8 ucBandIdx);
#ifdef WIFI_EAP_FEATURE
...
INT32 MtCmdGetRaTblInfo(struct _RTMP_ADAPTER *pAd, UINT8 BandIdx, UINT8 TblType,
...
INT32 MtCmdMUPowerCtrl(struct _RTMP_ADAPTER *pAd, BOOLEAN MUPowerForce,
		       UCHAR MUPowerCtrl);
INT32 MtCmdBFNDPATxDCtrl(struct _RTMP_ADAPTER *pAd, BOOLEAN fgNDPA_ManualMode,
...
INT32 MtCmdTemperatureCtrl(struct _RTMP_ADAPTER *pAd, BOOLEAN fgManualMode,
			   CHAR cTemperature);
...
INT32 MtCmdLinkTestTxCsdCtrl(struct _RTMP_ADAPTER *pAd, BOOLEAN fgTxCsdConfigEn,
...
INT32 MtCmdLinkTestRxCtrl(struct _RTMP_ADAPTER *pAd, UINT8 ucRxAntIdx,
			  UINT8 ucBandIdx);
INT32 MtCmdLinkTestTxPwrCtrl(struct _RTMP_ADAPTER *pAd, BOOLEAN fgTxPwrConfigEn,
			     UINT8 ucDbdcBandIdx, UINT8 ucBandIdx);
INT32 MtCmdLinkTestTxPwrUpTblCtrl(struct _RTMP_ADAPTER *pAd, UINT8 ucTxPwrUpCat,
				  PUINT8 pucTxPwrUpValue);
INT32 MtCmdLinkTestACRCtrl(struct _RTMP_ADAPTER *pAd, BOOLEAN fgACRConfigEn,
			   UINT8 ucDbdcBandIdx, UINT8 ucReserved);
INT32 MtCmdLinkTestRcpiCtrl(struct _RTMP_ADAPTER *pAd, BOOLEAN fgRCPIConfigEn);
INT32 MtCmdLinkTestSeIdxCtrl(struct _RTMP_ADAPTER *pAd,
			     BOOLEAN fgSeIdxConfigEn);
INT32 MtCmdLinkTestRcpiMACtrl(struct _RTMP_ADAPTER *pAd, UINT8 ucMAParameter);

#ifdef TPC_SUPPORT
INT32 MtCmdTpcFeatureCtrl(struct _RTMP_ADAPTER *pAd, INT8 TpcPowerValue,
			  UINT8 BandIdx, UINT8 CentralChannel);
#endif /* TPC_SUPPORT */
...
#if defined(CONFIG_HOTSPOT_R2) || defined(DSCP_QOS_MAP_SUPPORT)
INT32 MtCmdHotspotInfoUpdate(struct _RTMP_ADAPTER *pAd,
...
#endif /* PRE_CAL_MT7622_SUPPORT */
#ifdef AMPDU_CONF_SUPPORT
INT32 Set_CR4_retry_counter(struct _RTMP_ADAPTER *pAd, UINT8 band_idx,
			    UINT8 count);
#endif /* AMPDU_CONF_SUPPORT*/
...
#endif
#endif
INT32 MtCmdSendNullFrame(struct _RTMP_ADAPTER *pAd, UINT8 PID,
			 UINT8 No_Of_Null_Frames, UINT8 WlanIdx);
#endif /* __MT_CMD_H__ */
