...
{
	MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO, "ap_ftrc> Initialize FT RIC Module...\n");
#ifdef CONFIG_STA_SUPPORT
...
VOID TYPE_FUNC FT_RIC_Release(
	IN	PRTMP_ADAPTER		pAd)
{
	MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO, "ap_ftrc> Release FT RIC Module...\n");
#ifdef CONFIG_STA_SUPPORT
		...
		*/
				...
				*/
				...
				case IE_FT_RIC_DESCRIPTOR:
					...
					if (BA_ResourceAllocate(pAd,
											...
											pBufRspElm,
											&RspLen) == 0) {
						MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO, "ft_ric> Accept BA.\n");
		...
		if (FlgIsTspecAccepted == TRUE) {
			...
			*/
			...
			MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_INFO, "ft_ric> Resource request fails!\n");
		} /* End of if */
	...
	OUT ULONG *pBufRspLen)
{
	...
	if (!ba_decline && IS_HT_STA(pMacEntry)) {
		...
		if (ba_resrc_rec_add(pAd, pMacEntry->wcid, AddreqFrame.BaParm.TID, AddreqFrame.TimeOutValue, 64))
			Status = 0;
		else
