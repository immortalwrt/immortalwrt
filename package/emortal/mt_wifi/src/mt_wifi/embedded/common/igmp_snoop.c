...
)
{
	...
	if (isIgmpPkt(pDstMacAddr, pIpHeader)) {
		...
		case IGMP_V2_MEMBERSHIP_REPORT: /* IGMP version 2 membership report. */
			...
			MTWF_LOG(DBG_CAT_PROTO, CATPROTO_IGMP, DBG_LVL_TRACE,
				("%s: EntryInsert IGMP Group=%pI4\n", __func__, pGroupIpAddr));
#ifdef IGMP_TVM_SUPPORT
...
INT Set_IgmpSn_Allow_Non_Memb_Enable_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_PROTO, CATPROTO_IGMP, DBG_LVL_TRACE,
		("%s:: %s\n", __func__, Enable == TRUE ? "Enable non-member igmp":"Disable non-member igmp"));
...
INT Set_IgmpSn_AddEntry_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	while ((thisChar = strsep((char **)&arg, "-")) != NULL) {
		...
		if (bGroupId == 1) {
			...
			MTWF_LOG(DBG_CAT_PROTO, CATPROTO_IGMP, DBG_LVL_TRACE, ("%s group ip (%pI4)\n",
					 __func__, IpAddr));
		}
		if (bGroupId == 0) {
...
INT Set_IgmpSn_DelEntry_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	while ((thisChar = strsep((char **)&arg, "-")) != NULL) {
		...
		if (bGroupId == 1) {
			RTMPZeroMemory(GroupId, IPV6_ADDR_LEN);
			CVT_IPV4_IPV6(GroupId, IpAddr);
			MTWF_LOG(DBG_CAT_PROTO, CATPROTO_IGMP, DBG_LVL_TRACE, ("%s group ip (%d.%d.%d.%d)\n",
					 __func__, IpAddr[0], IpAddr[1], IpAddr[2], IpAddr[3]));
		} else {
			pEntry = MacTableLookup(pAd, Addr);
			memberCnt++;
		}
...
{
	RTMP_STRING IgmpIn[4] = {"1-7"};
	MTWF_LOG(DBG_CAT_PROTO, CATPROTO_IGMP, DBG_LVL_OFF,("=================	Dump IgmpInfo Table  =================\n"));
	if (IS_ASIC_CAP(pAd, fASIC_CAP_MCU_OFFLOAD))
...
INT Set_Igmp_Show_FW_Flooding_CIDR_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_PROTO, CATPROTO_IGMP, DBG_LVL_OFF,("=================  Dump FW Table  =================\n\n"));
...
/*IgmpSnEnable */
	if (RTMPGetKeyParameter("IgmpSnEnable", tmpbuf, 128, buffer, FALSE)) {
		for (i = 0, macptr = rstrtok(tmpbuf, ";"); (macptr && i < pAd->ApCfg.BssidNum); macptr = rstrtok(NULL, ";"), i++) {
			...
			MTWF_LOG(DBG_CAT_PROTO, CATPROTO_IGMP, DBG_LVL_TRACE, ("I/F(%s%d) ==> ",
