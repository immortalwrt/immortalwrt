...
static WSC_TLV_0B wsc_tlv_0b[] = {
	...
	*/
	...
	{ /*0x1053,*/ 2 }, /* WSC_ID_SEL_REG_CFG_METHODS */
...
int AppendWSCTLV(USHORT index, OUT UCHAR *obuf, IN UCHAR *ibuf,
		 IN USHORT varlen)
{
	...
	*/
	if (varlen != (USHORT)0)
		...
		if (WSC_TLV_ENT(index) <=
		    ((sizeof(wsc_tlv_0b) / sizeof(WSC_TLV_0B)) - 1))
...
*	========================================================================
*/
static VOID WscParseEncrSettings(IN PRTMP_ADAPTER pAdapter,
				 IN PUCHAR pPlainData, IN INT PlainLength,
				 IN PWSC_CTRL pWscControl)
{
	...
	RT_HMAC_SHA256(pReg->AuthKey, 32, pPlainData, HmacLen, Temp,
		       SHA256_DIGEST_SIZE);
	...
	if (RTMPEqualMemory(Hmac, Temp, 8) != 1) {
		...
		MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
			 ("MD --> 0x%08x-%08x\n",
			  (UINT)cpu2be32(*((PUINT)&Temp[0])),
			  (UINT)cpu2be32(*((PUINT)&Temp[4]))));
		MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
			 ("calculated --> 0x%08x-%08x\n",
			  (UINT)cpu2be32(*((PUINT)&Hmac[0])),
			  (UINT)cpu2be32(*((PUINT)&Hmac[4]))));
	}
}
...
BOOLEAN WscProcessCredential(IN PRTMP_ADAPTER pAdapter, IN PUCHAR pPlainData,
			     IN INT PlainLength, IN PWSC_CTRL pWscControl)
{
	...
	while (PlainLength > 4) {
		...
		case WSC_ID_NW_INDEX:
			...
			CurrentIdx =
				Cnt; /* since the index start from 0, we have to minus 1 */
			Cnt++;
			break;
		...
		case WSC_ID_SSID:
			...
			if (RTMPCheckStrPrintAble((CHAR *)pData, Idx) ||
			    (pWscControl->bCheckMultiByte == FALSE))
				NdisMoveMemory(
					pProfile->Profile[CurrentIdx].SSID.Ssid,
					pData,
		...
		case WSC_ID_AUTH_TYPE:
			...
			pProfile->Profile[CurrentIdx].AuthType =
				cpu2be16(tmpVal);
			break;

		case WSC_ID_ENCR_TYPE:
			tmpVal = get_unaligned((PUSHORT)pData);
			pProfile->Profile[CurrentIdx].EncrType =
				cpu2be16(tmpVal);
			break;
		...
		case WSC_ID_NW_KEY:
			...
			if (RTMPCheckStrPrintAble((CHAR *)pData, WscLen) ||
			    (pWscControl->bCheckMultiByte == FALSE)) {
				pProfile->Profile[CurrentIdx].KeyLength =
					WscLen;
				NdisMoveMemory(
					pProfile->Profile[CurrentIdx].Key,
					pData,
					pProfile->Profile[CurrentIdx].KeyLength);
			} else
				return FALSE;
		...
		case WSC_ID_MAC_ADDR:
			...
			*/
			RTMPMoveMemory(pProfile->Profile[CurrentIdx].MacAddr,
				       pData, MAC_ADDR_LEN);
			break;
		...
		*/
		for (Idx = 0; Idx < pProfile->ProfileCnt; Idx++) {
			...
			if (pCredential->EncrType == WSC_ENCRTYPE_WEP) {
				bReject = TRUE;
				/* Cleanup contents */
				NdisZeroMemory(&pWscControl->WscProfile,
					       sizeof(WSC_PROFILE));
			}
		}
...
int WscDeriveKey(unsigned char *kdk, unsigned int kdk_len,
		 ...
		 unsigned char *key, unsigned int keyBits)
{
	...
	for (i = 0; i < iterations; i++) {
		...
		memcpy(input, &temp, 4);
		RT_HMAC_SHA256(kdk, kdk_len, input, 4 + str_len + 4, hmac,
			       SHA256_DIGEST_SIZE);
...
VOID WscGenPSK1(PRTMP_ADAPTER pAd, PWSC_CTRL pWscCtrl, UCHAR *pPSK1)
{
	...
	if (pTempPsk == NULL) {
		MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_ERROR,
			 ("%s - pTempPsk alloc failed.", __func__));
		return;
	}

	{
		...
		if (pReg->PinCodeLen == 4)
			RT_HMAC_SHA256(pReg->AuthKey, 32, &pReg->PIN[0], 2,
				       pTempPsk, SHA256_DIGEST_SIZE);
		else
			RT_HMAC_SHA256(pReg->AuthKey, 32, &pReg->PIN[0], 4,
				       pTempPsk, SHA256_DIGEST_SIZE);
	}
...
VOID WscGenPSK2(PRTMP_ADAPTER pAd, PWSC_CTRL pWscCtrl, UCHAR *pPSK2)
{
	PWSC_REG_DATA pReg = (PWSC_REG_DATA)&pWscCtrl->RegData;
	PUCHAR pTempPsk = NULL;
	os_alloc_mem(NULL, &pTempPsk, SHA256_DIGEST_SIZE);
...

	{
		/* Generate PSK2 */
		if (pReg->PinCodeLen == 4)
			RT_HMAC_SHA256(pReg->AuthKey, 32, &pReg->PIN[2], 2,
				       pTempPsk, SHA256_DIGEST_SIZE);
		else
			RT_HMAC_SHA256(pReg->AuthKey, 32, &pReg->PIN[4], 4,
				       pTempPsk, SHA256_DIGEST_SIZE);
	}
...
*	========================================================================
*/
int BuildMessageM1(IN PRTMP_ADAPTER pAdapter, IN PWSC_CTRL pWscControl,
		   OUT VOID *pbuf)
{
	UCHAR TB[1];
	INT Len = 0, templen = 0;
	...
	templen = AppendWSCTLV(WSC_ID_UUID_E, pData,
			       &pWscControl->Wsc_Uuid_E[0], 0);
	pData += templen;
	Len += templen;
	/* 4. MAC address */
	if (wdev)
		templen =
			AppendWSCTLV(WSC_ID_MAC_ADDR, pData, wdev->if_addr, 0);
	else
		templen = AppendWSCTLV(WSC_ID_MAC_ADDR, pData,
				       pReg->SelfInfo.MacAddr, 0);
	...
	templen =
		AppendWSCTLV(WSC_ID_ENROLLEE_NONCE, pData, pReg->SelfNonce, 0);
	pData += templen;
	Len += templen;
	...
	templen = AppendWSCTLV(WSC_ID_AUTH_TYPE_FLAGS, pData,
			       (UINT8 *)&pReg->SelfInfo.AuthTypeFlags, 0);
	pData += templen;
	Len += templen;
	/* 8. Encryption Type Flags */
	templen = AppendWSCTLV(WSC_ID_ENCR_TYPE_FLAGS, pData,
			       (UINT8 *)&pReg->SelfInfo.EncrTypeFlags, 0);
	pData += templen;
	Len += templen;
	/* 9. Connection Type Flag ESS */
	templen = AppendWSCTLV(WSC_ID_CONN_TYPE_FLAGS, pData,
			       (UINT8 *)&pReg->SelfInfo.ConnTypeFlags, 0);
	pData += templen;
	Len += templen;
	/*10. Config Method */
	...
	templen = AppendWSCTLV(WSC_ID_CONFIG_METHODS, pData,
			       (UINT8 *)&ConfigMethods, 0);
	pData += templen;
	Len += templen;
	...
	templen = AppendWSCTLV(WSC_ID_SC_STATE, pData,
			       (UINT8 *)&pReg->SelfInfo.ScState, 0);
	pData += templen;
	Len += templen;
	/*12. Manufacture */
	...
	Len += templen;
	/*18. RF Band */
	templen = AppendWSCTLV(WSC_ID_RF_BAND, pData,
			       (UINT8 *)&pReg->SelfInfo.RfBand, 0);
	pData += templen;
	Len += templen;
	/*19. Associate state (Not associated) */
	templen = AppendWSCTLV(WSC_ID_ASSOC_STATE, pData,
			       (UINT8 *)&pReg->SelfInfo.AssocState, 0);
	pData += templen;
	Len += templen;
	/*20. Device Password ID */
	templen = AppendWSCTLV(WSC_ID_DEVICE_PWD_ID, pData,
			       (UINT8 *)&pReg->SelfInfo.DevPwdId, 0);

	pData += templen;
	Len += templen;
	/*21. Configure Error */
	templen = AppendWSCTLV(WSC_ID_CONFIG_ERROR, pData,
			       (UINT8 *)&ConfigError, 0);
	pData += templen;
	Len += templen;
	/*22. OS Version Not associated) */
	templen = AppendWSCTLV(WSC_ID_OS_VERSION, pData,
			       (UINT8 *)&pReg->SelfInfo.OsVersion, 0);
	pData += templen;
	Len += templen;
#ifdef WSC_V2_SUPPORT
	...
	if (IS_MAP_ENABLE(pAdapter) &&
	    wdev->MAPCfg.DevOwnRole == WDEV_TYPE_APCLI) {
	...
	/* Extra attribute that is not defined in WSC Sepc. */
	if (pWscControl->WscV2Info.bEnableWpsV2 && pWscTLV->pTlvData &&
	    pWscTLV->TlvLen) {
		templen = AppendWSCTLV(pWscTLV->TlvTag, pData,
				       (UINT8 *)pWscTLV->pTlvData,
				       pWscTLV->TlvLen);
	...
	MTWF_LOG(
		DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
		("BuildMessageM1 - bUPnPMsgTimerRunning = %d, pWscControl->WscUseUPnP = %d, pWscControl->EapMsgRunning = %d\n",
...
*	========================================================================
*/
int BuildMessageM2(IN PRTMP_ADAPTER pAdapter, IN PWSC_CTRL pWscControl,
		   OUT VOID *pbuf)
{
	UCHAR TB[1];
	INT Len = 0, templen = 0;
	...
	RT_DH_SecretKey_Generate(pReg->Pke, sizeof(pReg->Pke), WPS_DH_P_VALUE,
				 sizeof(WPS_DH_P_VALUE), pReg->EnrolleeRandom,
				 sizeof(pReg->EnrolleeRandom), pReg->SecretKey,
				 (UINT *)&DH_Len);
	...
	templen = AppendWSCTLV(WSC_ID_ENROLLEE_NONCE, pData,
			       pReg->EnrolleeNonce, 0);
	pData += templen;
	Len += templen;
	/* 4. Registrar Nonce, 16 bytes */
	templen = AppendWSCTLV(WSC_ID_REGISTRAR_NONCE, pData,
			       pReg->RegistrarNonce, 0);
	pData += templen;
	Len += templen;
	/* UUID, 16 bytes */
	...
	/* Config Error */
	templen = AppendWSCTLV(WSC_ID_CONFIG_ERROR, pData,
			       (UINT8 *)&pReg->SelfInfo.ConfigError, 0);
	pData += templen;
	Len += templen;
	/* Device Password ID */
	templen = AppendWSCTLV(WSC_ID_DEVICE_PWD_ID, pData,
	...
	/* KDF */
	WscDeriveKey(KDK, 32, Wsc_Personal_String,
		     (sizeof(Wsc_Personal_String) - 1), KdfKey, 640);
	/* Assign Key from KDF */
	...
	if (pAdapter->pHmacData) {
		...
		/* Validate HMAC, reuse KDK buffer */
		RT_HMAC_SHA256(pReg->AuthKey, 32, pAdapter->pHmacData, HmacLen,
			       KDK, SHA256_DIGEST_SIZE);
	}
...
*	========================================================================
*/
int BuildMessageM2D(IN PRTMP_ADAPTER pAdapter, IN PWSC_CTRL pWscControl,
		    OUT VOID *pbuf)
{
	UCHAR TB[1];
	INT Len = 0, templen = 0;
	...
	/* 4. Registrar Nonce, 16 bytes */
	templen =
		AppendWSCTLV(WSC_ID_REGISTRAR_NONCE, pData, pReg->SelfNonce, 0);
	...
	/* Config Methods */
	templen = AppendWSCTLV(WSC_ID_CONFIG_METHODS, pData,
			       (UINT8 *)&pReg->SelfInfo.ConfigMethods, 0);
	pData += templen;
	Len += templen;
	/* Manufacturer Name */
	templen = AppendWSCTLV(
...
*	========================================================================
*/
int BuildMessageM3(IN PRTMP_ADAPTER pAdapter, IN PWSC_CTRL pWscControl,
		   OUT VOID *pbuf)
{
	UCHAR TB[32];
	INT Len = 0, templen = 0;
	...
	/* Generate E-Hash1 */
	RT_HMAC_SHA256(pReg->AuthKey, 32, pHash, 416, pReg->EHash1,
		       SHA256_DIGEST_SIZE);
	...
	NdisMoveMemory(pHash + 224, pReg->Pkr, 192);
	/* Generate E-Hash2 */
	RT_HMAC_SHA256(pReg->AuthKey, 32, pHash, 416, pReg->EHash2,
		       SHA256_DIGEST_SIZE);
...
*	========================================================================
*/
int BuildMessageM4(IN PRTMP_ADAPTER pAdapter, IN PWSC_CTRL pWscControl,
		   OUT VOID *pbuf)
{
	UCHAR TB[32];
	INT Len = 0, templen = 0;
	...
	NdisMoveMemory(pHash + 224, pReg->Pkr, 192);
	/* Generate R-Hash1 */
	RT_HMAC_SHA256(pReg->AuthKey, 32, pHash, 416, pReg->RHash1,
		       SHA256_DIGEST_SIZE);
	...
	NdisMoveMemory(pHash + 224, pReg->Pkr, 192);
	/* Generate R-Hash2 */
	RT_HMAC_SHA256(pReg->AuthKey, 32, pHash, 416, pReg->RHash2,
		       SHA256_DIGEST_SIZE);
	...
	/* Generate HMAC */
	RT_HMAC_SHA256(pReg->AuthKey, 32, &Plain[0], PlainLen, TB,
		       SHA256_DIGEST_SIZE);

	if (PlainLen >= 0 && PlainLen <= 123)
		PlainLen += AppendWSCTLV(WSC_ID_KEY_WRAP_AUTH, &Plain[PlainLen],
					 TB, 0);

	/* 6b. Encrypted Settings */
	/* Encrypt data */
	EncrLen = IV_ENCR_DATA_LEN_144 - 16;
	AES_CBC_Encrypt(Plain, PlainLen, pReg->KeyWrapKey,
			sizeof(pReg->KeyWrapKey), &IV_EncrData[0], 16,
			(UINT8 *)&IV_EncrData[16], (UINT *)&EncrLen);
	templen = AppendWSCTLV(WSC_ID_ENCR_SETTINGS, pData, &IV_EncrData[0],
			       16 + EncrLen); /*IVLen + EncrLen */
	pData += templen;
	Len += templen;
#ifdef WSC_V2_SUPPORT
...
*	========================================================================
*/
int BuildMessageM5(IN PRTMP_ADAPTER pAdapter, IN PWSC_CTRL pWscControl,
		   OUT VOID *pbuf)
{
	UCHAR TB[32];
	INT Len = 0, templen = 0;
...
*	========================================================================
*/
int BuildMessageM6(IN PRTMP_ADAPTER pAdapter, IN PWSC_CTRL pWscControl,
		   OUT VOID *pbuf)
{
	UCHAR TB[32];
	INT Len = 0, templen = 0;
...
*	========================================================================
*/
int BuildMessageM7(IN PRTMP_ADAPTER pAdapter, IN PWSC_CTRL pWscControl,
		   OUT VOID *pbuf)
{
	UCHAR TB[32];
	INT Len = 0, templen = 0;
	...
	if ((CurOpMode == AP_MODE) &&
		...
		WscCreateProfileFromCfg(pAdapter, ENROLLEE_ACTION | AP_MODE,
		...
		*/
		...
		if (PlainLen >= 0 && PlainLen <= 251)
			PlainLen += AppendWSCTLV(WSC_ID_SSID, &Plain[PlainLen],
						 pCredential->SSID.Ssid,
						 pCredential->SSID.SsidLength);

		if (PlainLen >= 0 && PlainLen <= 247)
			PlainLen +=
				AppendWSCTLV(WSC_ID_MAC_ADDR, &Plain[PlainLen],
					     pCredential->MacAddr, 0);

		if (PlainLen >= 0 && PlainLen <= 243)
			PlainLen +=
				AppendWSCTLV(WSC_ID_AUTH_TYPE, &Plain[PlainLen],
					     (UINT8 *)&authType, 0);

		if (PlainLen >= 0 && PlainLen <= 239)
			PlainLen +=
				AppendWSCTLV(WSC_ID_ENCR_TYPE, &Plain[PlainLen],
					     (UINT8 *)&encyType, 0);

		if (PlainLen >= 0 && PlainLen <= 235)
			PlainLen += AppendWSCTLV(WSC_ID_NW_KEY_INDEX,
						 &Plain[PlainLen],
						 &pCredential->KeyIndex, 0);

		if (PlainLen >= 0 && PlainLen <= 231)
			PlainLen +=
				AppendWSCTLV(WSC_ID_NW_KEY, &Plain[PlainLen],
					     pCredential->Key,
	...
	AES_CBC_Encrypt(Plain, PlainLen, pReg->KeyWrapKey,
			sizeof(pReg->KeyWrapKey), &IV_EncrData[0], 16,
			(UINT8 *)&IV_EncrData[16], (UINT *)&EncrLen);
	templen = AppendWSCTLV(WSC_ID_ENCR_SETTINGS, pData, IV_EncrData,
			       16 + EncrLen); /*IVLen + EncrLen */
	pData += templen;
	Len += templen;
#ifdef WSC_V2_SUPPORT
...
unsigned char same_band_profile(PWSC_CTRL pWscControl,
...
*	========================================================================
*/
int BuildMessageM8(IN PRTMP_ADAPTER pAdapter, IN PWSC_CTRL pWscControl,
		   OUT VOID *pbuf)
{
	UCHAR *TB = NULL;
	INT Len = 0, templen = 0;
	...
	if (CurOpMode == AP_MODE) {
		...
		if (pReg->PeerInfo.map_DevPeerRole &
			...
			if (!IS_MAP_TURNKEY_ENABLE(pAdapter)) {
				...
				UCHAR apidx = (pWscControl->EntryIfIdx & 0x0F);
				UCHAR band_idx = HcGetBandByWdev(
				...
				if (bh_wdev) {
					...
					MTWF_LOG(
						...
						("band_idx %d  bh_bss_wdev [%s]\n",
						 band_idx,
						 bh_wdev->if_dev->name));
					pBhWscControl->WscConfStatus =
						WSC_SCSTATE_CONFIGURED;
					COPY_MAC_ADDR(pBhWscControl->EntryAddr,
					...
					pCredential = &pBhWscControl->WscProfile
							       .Profile[0];
				} else
					goto LabelErr;
			}
	...
	APPEND_CONFIGS:
		for (i = 0; i < pWscControl->WscBhProfiles.ProfileCnt; i++) {
			...
			if (pWscControl->WscBhProfiles.Profile[i].bss_role &
			    BIT(MAP_ROLE_BACKHAUL_BSS)) {
			...
			if (profile_count == 1)
				CerLen += AppendWSCTLV(WSC_ID_NW_INDEX, &TB[0],
						       (PUCHAR) "1", 0);
			else if (profile_count == 2)
				CerLen += AppendWSCTLV(WSC_ID_NW_INDEX,
						       &TB[CerLen],
						       (PUCHAR) "2", 0);
			...
			CerLen += AppendWSCTLV(WSC_ID_NW_KEY, &TB[CerLen],
					       pCredential->Key,
					       pCredential->KeyLength);
			CerLen += AppendWSCTLV(WSC_ID_MAC_ADDR, &TB[CerLen],
					       pCredential->MacAddr, 0);
			/*	  Prepare plain text */
		}
		if (profile_count == 0 && same_band_profile_missing == FALSE) {
...
#endif
	{
		/* 4a. Encrypted R-S1 */
		CerLen +=
			AppendWSCTLV(WSC_ID_NW_INDEX, &TB[0], (PUCHAR) "1", 0);
	...
	*/
		...
		CerLen += AppendWSCTLV(WSC_ID_NW_KEY_INDEX, &TB[CerLen],
				       &pCredential->KeyIndex, 0);
		CerLen +=
			AppendWSCTLV(WSC_ID_NW_KEY, &TB[CerLen],
	...
	if ((CurOpMode == AP_MODE)) {
		...
		PlainLen +=
			AppendWSCTLV(WSC_ID_CREDENTIAL, &Plain[0], TB, CerLen);
...
int BuildMessageDONE(IN PRTMP_ADAPTER pAdapter, IN PWSC_CTRL pWscControl,
		     OUT VOID *pbuf)
{
	UCHAR TB[1];
	INT Len = 0, templen = 0;
...
int BuildMessageACK(IN PRTMP_ADAPTER pAdapter, IN PWSC_CTRL pWscControl,
		    OUT VOID *pbuf)
{
	UCHAR TB[1];
	INT Len = 0, templen = 0;
...
int BuildMessageNACK(IN PRTMP_ADAPTER pAdapter, IN PWSC_CTRL pWscControl,
		     OUT VOID *pbuf)
{
	UCHAR TB[2];
	INT Len = 0, templen = 0;
...
*	========================================================================
*/
int ProcessMessageM1(IN PRTMP_ADAPTER pAdapter, IN PWSC_CTRL pWscControl,
		     IN VOID *precv, IN INT Length, OUT PWSC_REG_DATA pReg)
{
	...
	pReg->PeerInfo.Version2 = 0;
	{
		...
		RT_DH_PublicKey_Generate(
			WPS_DH_G_VALUE, sizeof(WPS_DH_G_VALUE), WPS_DH_P_VALUE,
			sizeof(WPS_DH_P_VALUE),
			pWscControl->RegData.EnrolleeRandom,
			sizeof(pWscControl->RegData.EnrolleeRandom), pReg->Pkr,
			(UINT *)&DH_Len);
	...
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_VERSION))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_VERSION));
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_MSG_TYPE))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_MSG_TYPE));
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_UUID_E))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_UUID_E));
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_MAC_ADDR))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_MAC_ADDR));
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_ENROLLEE_NONCE))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_ENROLLEE_NONCE));
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_PUBLIC_KEY))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_PUBLIC_KEY));
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_AUTH_TYPE_FLAGS))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_AUTH_TYPE_FLAGS));
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_ENCR_TYPE_FLAGS))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_ENCR_TYPE_FLAGS));
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_CONN_TYPE_FLAGS))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_CONN_TYPE_FLAGS));
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_CONFIG_METHODS))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_CONFIG_METHODS));
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_SC_STATE))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_SC_STATE));
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_MANUFACTURER))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_MANUFACTURER));
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_MODEL_NAME))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_MODEL_NAME));
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_MODEL_NUMBER))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_MODEL_NUMBER));
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_SERIAL_NUM))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_SERIAL_NUM));
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_PRIM_DEV_TYPE))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_PRIM_DEV_TYPE));
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_DEVICE_NAME))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_DEVICE_NAME));
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_RF_BAND))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_RF_BAND));
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_ASSOC_STATE))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_ASSOC_STATE));
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_CONFIG_ERROR))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_CONFIG_ERROR));
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_DEVICE_PWD_ID))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_DEVICE_PWD_ID));
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_OS_VERSION))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_OS_VERSION));
	...
	while (Length > 4) {
		...
		case WSC_ID_MAC_ADDR:
			...
			NdisMoveMemory(pWscControl->WscPeerInfo.WscPeerMAC,
				       pData, WscLen);
			FieldCheck[(WSC_TLV_BYTE2(WSC_ID_MAC_ADDR))] ^=
				(1 << WSC_TLV_BYTE1(WSC_ID_MAC_ADDR));
			break;
		...
		case WSC_ID_CONFIG_METHODS:
			pReg->PeerInfo.ConfigMethods = get_unaligned(
				(PUSHORT)pData); /**((PUSHORT) pData); */
			FieldCheck[(WSC_TLV_BYTE2(WSC_ID_CONFIG_METHODS))] ^=
				(1 << WSC_TLV_BYTE1(WSC_ID_CONFIG_METHODS));
			break;

		case WSC_ID_SC_STATE:
			pReg->PeerInfo.ScState = get_unaligned(
				(PUSHORT)pData); /**((PUSHORT) pData); */
			FieldCheck[(WSC_TLV_BYTE2(WSC_ID_SC_STATE))] ^=
				(1 << WSC_TLV_BYTE1(WSC_ID_SC_STATE));
			break;
		...
		case WSC_ID_MANUFACTURER:
			NdisMoveMemory(&pReg->PeerInfo.Manufacturer, pData,
				       WscLen);
			NdisMoveMemory(
				&pWscControl->WscPeerInfo.WscPeerManufacturer,
				pData, WscLen);
			FieldCheck[(WSC_TLV_BYTE2(WSC_ID_MANUFACTURER))] ^=
				(1 << WSC_TLV_BYTE1(WSC_ID_MANUFACTURER));
			break;
		...
		case WSC_ID_MODEL_NAME:
			NdisMoveMemory(&pReg->PeerInfo.ModelName, pData,
				       WscLen);
			NdisMoveMemory(
				&pWscControl->WscPeerInfo.WscPeerModelName,
				pData, WscLen);
			FieldCheck[(WSC_TLV_BYTE2(WSC_ID_MODEL_NAME))] ^=
				(1 << WSC_TLV_BYTE1(WSC_ID_MODEL_NAME));
			break;
		...
		case WSC_ID_MODEL_NUMBER:
			NdisMoveMemory(&pReg->PeerInfo.ModelNumber, pData,
				       WscLen);
			NdisMoveMemory(
				&pWscControl->WscPeerInfo.WscPeerModelNumber,
				pData, WscLen);
			FieldCheck[(WSC_TLV_BYTE2(WSC_ID_MODEL_NUMBER))] ^=
				(1 << WSC_TLV_BYTE1(WSC_ID_MODEL_NUMBER));
			break;
		...
		case WSC_ID_SERIAL_NUM:
			NdisMoveMemory(&pReg->PeerInfo.SerialNumber, pData,
				       WscLen);
			NdisMoveMemory(
				&pWscControl->WscPeerInfo.WscPeerSerialNumber,
				pData, WscLen);
			FieldCheck[(WSC_TLV_BYTE2(WSC_ID_SERIAL_NUM))] ^=
				(1 << WSC_TLV_BYTE1(WSC_ID_SERIAL_NUM));
			break;
		...
		case WSC_ID_PRIM_DEV_TYPE:
			NdisMoveMemory(&pReg->PeerInfo.PriDeviceType, pData,
				       WscLen);
			FieldCheck[(WSC_TLV_BYTE2(WSC_ID_PRIM_DEV_TYPE))] ^=
				(1 << WSC_TLV_BYTE1(WSC_ID_PRIM_DEV_TYPE));
			break;
		...
		case WSC_ID_DEVICE_NAME:
			NdisMoveMemory(&pReg->PeerInfo.DeviceName, pData,
				       WscLen);
			NdisMoveMemory(
				pWscControl->WscPeerInfo.WscPeerDeviceName,
				pData, WscLen);
			FieldCheck[(WSC_TLV_BYTE2(WSC_ID_DEVICE_NAME))] ^=
				(1 << WSC_TLV_BYTE1(WSC_ID_DEVICE_NAME));
			break;
		...
		case WSC_ID_ASSOC_STATE:
			pReg->PeerInfo.AssocState = get_unaligned(
				(PUSHORT)pData); /**((PUSHORT) pData); */
			FieldCheck[(WSC_TLV_BYTE2(WSC_ID_ASSOC_STATE))] ^=
				(1 << WSC_TLV_BYTE1(WSC_ID_ASSOC_STATE));
			break;
		...
		case WSC_ID_CONFIG_ERROR:
			pReg->PeerInfo.ConfigError = get_unaligned(
				(PUSHORT)pData); /**((PUSHORT) pData); */
			FieldCheck[(WSC_TLV_BYTE2(WSC_ID_CONFIG_ERROR))] ^=
				(1 << WSC_TLV_BYTE1(WSC_ID_CONFIG_ERROR));
			break;
		...
		case WSC_ID_DEVICE_PWD_ID:
			...
			if (get_unaligned((PUSHORT)pData) ==
			    WSC_DEVICEPWDID_DEFAULT) { /**(PUSHORT) pData) */
				MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
					 ("Rx WPS           DPID PIN\n"));
				pWscControl->RegData.SelfInfo.DevPwdId =
					cpu2be16(DEV_PASS_ID_PIN);
			} else if (get_unaligned((PUSHORT)pData) ==
				   WSC_DEVICEPWDID_PUSH_BTN) { /**(PUSHORT) pData) */
				MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
					 ("Rx WPS           DPID PBC\n"));
				pWscControl->RegData.SelfInfo.DevPwdId =
					cpu2be16(DEV_PASS_ID_PBC);
			} else
				MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
		...
		case WSC_ID_OS_VERSION:
			pReg->PeerInfo.OsVersion = get_unalignedlong(
		...
		case WSC_ID_VENDOR_EXT:
			...
			if (pWscControl->WscV2Info.bEnableWpsV2) {
				...
				WscParseV2SubItem(WFA_EXT_ID_VERSION2, pData,
						  WscLen,
			...
			if (IS_MAP_ENABLE(pAdapter)) {
				...
				WscParseV2SubItem(
					WFA_EXT_ID_MAP_EXT_ATTRIBUTE, pData,
					WscLen, &pReg->PeerInfo.map_DevPeerRole,
					&tmp_data_len);
				MTWF_LOG(
	...
	if (FieldCheck[0] || FieldCheck[1] || FieldCheck[2] || FieldCheck[3] ||
	    FieldCheck[4] || FieldCheck[5] || FieldCheck[6])
...
*	========================================================================
*/
int ProcessMessageM2(IN PRTMP_ADAPTER pAdapter, IN PWSC_CTRL pWscControl,
		     IN VOID *precv, IN INT Length, IN UCHAR apidx,
		     OUT PWSC_REG_DATA pReg)
{
	int ret = WSC_ERROR_NO_ERROR;
	INT HmacLen;
	UCHAR Hmac[8] = { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff },
	      KDK[32];
	...
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_ENROLLEE_NONCE))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_ENROLLEE_NONCE));
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_REGISTRAR_NONCE))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_REGISTRAR_NONCE));
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_UUID_R))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_UUID_R));
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_PUBLIC_KEY))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_PUBLIC_KEY));
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_AUTH_TYPE_FLAGS))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_AUTH_TYPE_FLAGS));
	...
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_OS_VERSION))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_OS_VERSION));
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_AUTHENTICATOR))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_AUTHENTICATOR));
	/* Copy the content to Regdata for lastRx information */
	/* Length must include authenticator IE size */
	pReg->LastRx.Length = Length;
	NdisMoveMemory(pReg->LastRx.Data, precv, Length);
	...
	while (Length > 4) {
		...
		case WSC_ID_ENROLLEE_NONCE:
			...
			/* for verification with our enrollee nonce */
			if (RTMPCompareMemory(pReg->SelfNonce, pData, WscLen) !=
			    0)
				MTWF_LOG(
					DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
		...
		case WSC_ID_AUTH_TYPE_FLAGS:
			pReg->PeerInfo.AuthTypeFlags =
				get_unaligned((PUSHORT)pData);
			FieldCheck[(WSC_TLV_BYTE2(WSC_ID_AUTH_TYPE_FLAGS))] ^=
				(1 << WSC_TLV_BYTE1(WSC_ID_AUTH_TYPE_FLAGS));
			break;

		case WSC_ID_ENCR_TYPE_FLAGS:
			pReg->PeerInfo.EncrTypeFlags =
				get_unaligned((PUSHORT)pData);
			FieldCheck[(WSC_TLV_BYTE2(WSC_ID_ENCR_TYPE_FLAGS))] ^=
				(1 << WSC_TLV_BYTE1(WSC_ID_ENCR_TYPE_FLAGS));
			break;
		...
		case WSC_ID_OS_VERSION:
			pReg->PeerInfo.OsVersion =
				get_unalignedlong((PULONG)pData);
			FieldCheck[(WSC_TLV_BYTE2(WSC_ID_OS_VERSION))] ^=
				(1 << WSC_TLV_BYTE1(WSC_ID_OS_VERSION));
			break;
		...
		NdisMoveMemory(&pWscControl->WscPeerInfo.WscPeerMAC,
			       &pWscControl->RegData.PeerInfo.MacAddr, 6);
		/* Offset to net WSC Ie */
		pData += WscLen;
		Length -= WscLen;
	}
	...
	DH_Len = sizeof(pReg->SecretKey);
	NdisZeroMemory(pReg->SecretKey, sizeof(pReg->SecretKey));
	RT_DH_SecretKey_Generate(pReg->Pkr, sizeof(pReg->Pkr), WPS_DH_P_VALUE,
				 sizeof(WPS_DH_P_VALUE), pReg->EnrolleeRandom,
				 sizeof(pReg->EnrolleeRandom), pReg->SecretKey,
				 (UINT *)&DH_Len);
	...
	if (pAdapter->pHmacData) {
		NdisMoveMemory(pAdapter->pHmacData, pReg->LastTx.Data,
			       pReg->LastTx.Length);
		NdisMoveMemory(pAdapter->pHmacData + pReg->LastTx.Length,
			       pReg->LastRx.Data, pReg->LastRx.Length - 12);
		/* Validate HMAC, reuse KDK buffer */
		RT_HMAC_SHA256(pReg->AuthKey, 32, pAdapter->pHmacData, HmacLen,
			       KDK, SHA256_DIGEST_SIZE);
	}
	...
	if (RTMPEqualMemory(Hmac, KDK, 8) != 1) {
		...
		MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
			 ("MD --> 0x%08x-%08x\n", (UINT)cpu2be32(*((PUINT)KDK)),
			  (UINT)cpu2be32(*((PUINT)(KDK + 4)))));
		MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
			 ("calculated --> 0x%08x-%08x\n",
			  (UINT)cpu2be32(*((PUINT)&Hmac[0])),
			  (UINT)cpu2be32(*((PUINT)&Hmac[4]))));
...
*	========================================================================
*/
int ProcessMessageM2D(IN PRTMP_ADAPTER pAdapter, IN VOID *precv, IN INT Length,
		      OUT PWSC_REG_DATA pReg)
{
	int ret = WSC_ERROR_NO_ERROR;
	PUCHAR pData = NULL;
	...
	while (Length > 4) {
		...
		case WSC_ID_ENROLLEE_NONCE:
			...
			/* for verification with our enrollee nonce */
			if (RTMPCompareMemory(pReg->EnrolleeNonce, pData,
					      WscLen) != 0)
				MTWF_LOG(
					DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
					("Rx M2 Compare enrollee nonce mismatched\n"));
		...
		case WSC_ID_AUTH_TYPE_FLAGS:
			pReg->PeerInfo.AuthTypeFlags = get_unaligned(
				(PUSHORT)pData); /**((PUSHORT) pData); */
			break;

		case WSC_ID_ENCR_TYPE_FLAGS:
			pReg->PeerInfo.EncrTypeFlags = get_unaligned(
				(PUSHORT)pData); /**((PUSHORT) pData); */
			break;
		...
		case WSC_ID_CONFIG_METHODS:
			pReg->PeerInfo.ConfigMethods = be2cpu16(get_unaligned((
				PUSHORT)pData)); /*be2cpu16(*((PUSHORT) pData)); */
			break;
...
*	========================================================================
*/
int ProcessMessageM3(IN PRTMP_ADAPTER pAdapter, IN VOID *precv, IN INT Length,
		     OUT PWSC_REG_DATA pReg)
{
	int ret = WSC_ERROR_NO_ERROR;
	INT HmacLen;
	...
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_REGISTRAR_NONCE))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_REGISTRAR_NONCE));
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_E_HASH1))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_E_HASH1));
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_E_HASH2))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_E_HASH2));
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_AUTHENTICATOR))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_AUTHENTICATOR));
	/* Copy the content to Regdata for lastRx information */
	/* Length must include authenticator IE size */
	...
	while (Length > 4) {
		...
		case WSC_ID_REGISTRAR_NONCE:
			...
			if (RTMPCompareMemory(pReg->RegistrarNonce, pData,
					      WscLen) != 0)
				MTWF_LOG(
					DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
...
*	========================================================================
*/
int ProcessMessageM4(IN PRTMP_ADAPTER pAdapter, IN PWSC_CTRL pWscControl,
		     IN VOID *precv, IN INT Length, OUT PWSC_REG_DATA pReg)
{
	int ret = WSC_ERROR_NO_ERROR;
	INT HmacLen;
	...
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_ENROLLEE_NONCE))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_ENROLLEE_NONCE));
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_R_HASH1))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_R_HASH1));
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_R_HASH2))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_R_HASH2));
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_ENCR_SETTINGS))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_ENCR_SETTINGS));
	FieldCheck[(WSC_TLV_BYTE2(WSC_ID_AUTHENTICATOR))] |=
		(1 << WSC_TLV_BYTE1(WSC_ID_AUTHENTICATOR));
	...
	while (Length > 4) {
		...
		case WSC_ID_ENCR_SETTINGS:
			...
			AES_CBC_Decrypt(IV_DecrData + 16, (WscLen - 16),
					pReg->KeyWrapKey,
					sizeof(pReg->KeyWrapKey), IV_DecrData,
					16, (UINT8 *)pReg->ApEncrSettings,
					(UINT *)&EncrLen);
			MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE,
				 ("M4 ApEncrSettings len = %d\n", EncrLen));
			/* Parse encryption settings */
			WscParseEncrSettings(pAdapter, pReg->ApEncrSettings,
					     EncrLen, pWscControl);
			FieldCheck[(WSC_TLV_BYTE2(WSC_ID_ENCR_SETTINGS))] ^=
				(1 << WSC_TLV_BYTE1(WSC_ID_ENCR_SETTINGS));
			break;
	...
	NdisMoveMemory(pHash + 224, pReg->Pkr, 192);
	/* Generate R-Hash1 */
	RT_HMAC_SHA256(pReg->AuthKey, 32, pHash, 416, RHash,
		       SHA256_DIGEST_SIZE);
...
*	========================================================================
*/
int ProcessMessageM5(IN PRTMP_ADAPTER pAdapter, IN PWSC_CTRL pWscControl,
		     IN VOID *precv, IN INT Length, OUT PWSC_REG_DATA pReg)
{
	int ret = WSC_ERROR_NO_ERROR;
	INT HmacLen;
	...
	NdisMoveMemory(pHash + 224, pReg->Pkr, 192);
	/* Generate E-Hash1 */
	RT_HMAC_SHA256(pReg->AuthKey, 32, pHash, 416, EHash,
		       SHA256_DIGEST_SIZE);
...
*	========================================================================
*/
int ProcessMessageM6(IN PRTMP_ADAPTER pAdapter, IN PWSC_CTRL pWscControl,
		     IN VOID *precv, IN INT Length, OUT PWSC_REG_DATA pReg)
{
	int ret = WSC_ERROR_NO_ERROR;
	INT HmacLen;
...
*	========================================================================
*/
int ProcessMessageM7(IN PRTMP_ADAPTER pAdapter, IN PWSC_CTRL pWscControl,
		     IN VOID *precv, IN INT Length, OUT PWSC_REG_DATA pReg)
{
	int ret = WSC_ERROR_NO_ERROR;
	INT HmacLen;
...
*	========================================================================
*/
int ProcessMessageM8(IN PRTMP_ADAPTER pAdapter, IN VOID *precv, IN INT Length,
		     IN PWSC_CTRL pWscControl)
{
	int ret = WSC_ERROR_NO_ERROR;
	INT HmacLen;
	...
	while (Length > 4) {
		...
		case WSC_ID_ENCR_SETTINGS:
			...
			/* Parse encryption settings */
			if (WscProcessCredential(pAdapter, pReg->ApEncrSettings,
						 EncrLen,
						 pWscControl) == FALSE) {
				if (IV_DecrData)
					os_free_mem(IV_DecrData);
