...
#define CFG80211DBG(__Flg, __pMsg)                                             \
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, __Flg, __pMsg)
#else
#define CFG80211DBG(__Flg, __pMsg)
...
#define RT_CFG80211_CRDA_REG_RULE_APPLY(__pAd)                                 \
	CFG80211_RegRuleApply((VOID *)__pAd, NULL,                             \
			      __pAd->cfg80211_ctrl.Cfg80211_Alpha2);

#define RT_CFG80211_CONN_RESULT_INFORM(__pAd, __pBSSID, __pReqIe, __ReqIeLen,  \
				       __pRspIe, __RspIeLen, __FlgIsSuccess)   \
	CFG80211_ConnectResultInform((VOID *)__pAd, __pBSSID, __pReqIe,        \
				     __ReqIeLen, __pRspIe, __RspIeLen,         \
				     __FlgIsSuccess);
...
#define RT_CFG80211_P2P_CLI_CONN_RESULT_INFORM(__pAd, __pBSSID, __pReqIe,      \
					       __ReqIeLen, __pRspIe,           \
					       __RspIeLen, __FlgIsSuccess)     \
	CFG80211_ApClientConnectResultInform(__pAd, __pBSSID, __pReqIe,        \
					     __ReqIeLen, __pRspIe, __RspIeLen, \
					     __FlgIsSuccess);
#else
#define RT_CFG80211_P2P_CLI_CONN_RESULT_INFORM(__pAd, __pBSSID, __pReqIe,      \
					       __ReqIeLen, __pRspIe,           \
					       __RspIeLen, __FlgIsSuccess)     \
	CFG80211_P2pClientConnectResultInform(__pAd, __pBSSID, __pReqIe,       \
					      __ReqIeLen, __pRspIe,            \
					      __RspIeLen, __FlgIsSuccess);
...
#define RT_CFG80211_JOIN_IBSS(_pAd, _pBssid) CFG80211_JoinIBSS(_pAd, _pBssid);

#define CFG80211_BANDINFO_FILL(__pAd, __wdev, __pBandInfo)                     \
	{                                                                      \
		do {                                                           \
			(__pBandInfo)->RFICType = HcGetRadioRfIC(__pAd);       \
			(__pBandInfo)->MpduDensity =                           \
				__pAd->CommonCfg.BACapability.field            \
					.MpduDensity;                          \
			(__pBandInfo)->TxStream =                              \
				((__wdev == NULL) ||                           \
				 (__wdev->wpf_op == NULL)) ?                   \
					1 :                                    \
					      wlan_operate_get_tx_stream(__wdev);    \
			(__pBandInfo)->RxStream =                              \
				((__wdev == NULL) ||                           \
				 (__wdev->wpf_op == NULL)) ?                   \
					1 :                                    \
					      wlan_operate_get_rx_stream(__wdev);    \
			...
			(__pBandInfo)->RtsThreshold =                          \
				(__wdev == NULL) ?                             \
					DEFAULT_RTS_LEN_THLD :                 \
					      wlan_operate_get_rts_len_thld(__wdev); \
			(__pBandInfo)->FragmentThreshold =                     \
				(__wdev == NULL) ?                             \
					DEFAULT_FRAG_THLD :                    \
					      wlan_operate_get_frag_thld(__wdev);    \
...
BOOLEAN CFG80211DRV_OpsScanSetSpecifyChannel(VOID *pAdOrg, VOID *pData,
					     UINT8 dataLen);
...
VOID CFG80211_P2pClientConnectResultInform(IN VOID *pAdCB, IN UCHAR *pBSSID,
					   IN UCHAR *pReqIe, IN UINT32 ReqIeLen,
					   IN UCHAR *pRspIe, IN UINT32 RspIeLen,
					   IN UCHAR FlgIsSuccess);

#ifdef APCLI_CFG80211_SUPPORT
VOID CFG80211_ApClientConnectResultInform(IN VOID *pAdCB, IN UCHAR *pBSSID,
					  IN UCHAR *pReqIe, IN UINT32 ReqIeLen,
					  IN UCHAR *pRspIe, IN UINT32 RspIeLen,
					  IN UCHAR FlgIsSuccess);
#endif /* APCLI_CFG80211_SUPPORT */
...
BOOLEAN CFG80211DRV_OpsRemainOnChannel(VOID *pAdOrg, VOID *pData,
				       UINT32 duration);

BOOLEAN CFG80211DRV_OpsCancelRemainOnChannel(VOID *pAdOrg, UINT32 cookie);

VOID CFG80211DRV_OpsMgmtFrameProbeRegister(VOID *pAdOrg, VOID *pData,
					   BOOLEAN isReg);

VOID CFG80211DRV_OpsMgmtFrameActionRegister(VOID *pAdOrg, VOID *pData,
					    BOOLEAN isReg);

BOOLEAN CFG80211_CheckActionFrameType(IN RTMP_ADAPTER *pAd, IN PUCHAR preStr,
				      IN PUCHAR pData, IN UINT32 length);

BOOLEAN CFG80211_SyncPacketWmmIe(RTMP_ADAPTER *pAd, VOID *pData, ULONG dataLen);
BOOLEAN CFG80211_HandleP2pMgmtFrame(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk,
				    UCHAR OpMode);
...
VOID CFG80211_Announce802_3Packet(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk,
...
VOID CFG80211_ParseBeaconIE(RTMP_ADAPTER *pAd, BSS_STRUCT *pMbss,
			    struct wifi_dev *wdev, UCHAR *wpa_ie,
			    UCHAR *rsn_ie);
...
BOOLEAN CFG80211_HandleTdlsDiscoverRespFrame(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk,
					     UCHAR OpMode);

VOID cfg_tdls_send_PeerTrafficIndication(PRTMP_ADAPTER pAd, u8 *peer);
VOID cfg_tdls_rcv_PeerTrafficIndication(PRTMP_ADAPTER pAd, u8 dialog_token,
					u8 *peer);
...
VOID cfg_tdls_UAPSDP_PsmModeChange(PRTMP_ADAPTER pAd, USHORT PsmOld,
				   USHORT PsmNew);
...
INT cfg_tdls_chsw_req(PRTMP_ADAPTER pAd, u8 *peer, u8 target_channel,
		      u8 target_bw);
INT cfg_tdls_chsw_resp(PRTMP_ADAPTER pAd, u8 *peer, UINT32 ch_sw_time,
		       UINT32 ch_sw_timeout, u16 reason_code);
...
VOID CFG80211_JoinIBSS(IN VOID *pAdCB, IN UCHAR *pBSSID);
