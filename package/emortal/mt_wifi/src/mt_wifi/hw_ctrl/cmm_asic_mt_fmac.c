...
VOID asic_wrap_protinfo_in_bssinfo(struct _RTMP_ADAPTER *ad, VOID *cookie)
{
	...
	MTWF_LOG(DBG_CAT_MISC, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			("%s: prepare to deliver setting to firmware\n", __func__));
	...
	if (arch_ops->archSetBssid)
		...
		MTWF_LOG(DBG_CAT_MISC, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			("%s: there is no lower layer implementation.\n", __func__));
	}
