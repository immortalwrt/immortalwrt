	...
	OUT	PULONG	pFrameLen)
{
	...
	if (pAd->CommonCfg.bCountryFlag) {
		...
		NdisZeroMemory(TmpFrame, sizeof(TmpFrame));
		{
			...
			regclass = TDLS_GetRegulatoryClass(pAd, op_ht_bw, wdev->channel);
...
TDLS_InsertSupportChannelIE(
	...
	OUT	PULONG	pFrameLen)
{
	if (pAd->StaCfg[0].TdlsInfo.TdlsChSwitchSupp) {
		...
		if (WMODE_CAP_2G(pAd->StaCfg[0].wdev.PhyMode)) {
			ch_set[list_len] = 1; /* first channel */
			ch_set[list_len + 1] = 11; /* channel number */
			list_len += 2;
		}

		if (WMODE_CAP_5G(pAd->StaCfg[0].wdev.PhyMode)) {
	...
	OUT	PULONG	pFrameLen)
{
	...
	/* if (WMODE_CAP_N(pAd->StaCfg[0].wdev.PhyMode) || (pAd->CommonCfg.bWmmCapable)) */
	...
	IN	BOOLEAN	bEnable)
{
	...
	if (pAd->CommonCfg.bWmmCapable) {
		...
		if (pAd->StaCfg[0].wdev.UapsdInfo.bAPSDCapable) {
			...
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("tdls uapsd> UAPSD %d %d %d %d %d!\n",
	...
	IN PRT_802_11_TDLS	pTDLS)
{
	...
	if ((WMODE_CAP_N(pAd->StaCfg[0].wdev.PhyMode) || (pAd->CommonCfg.bWmmCapable)) && (pTDLS->bWmmCapable)) {
		USHORT	idx;
	...
	OUT	PULONG	pFrameLen)
{
	...
	if (WMODE_CAP_N(pAd->StaCfg[0].wdev.PhyMode)) {
		UCHAR HtLen;
		HT_CAPABILITY_IE HtCapabilityTmp;
		UCHAR op_ht_bw = wlan_operate_get_ht_bw(&pAd->StaCfg[0].wdev);
	...
	OUT	PULONG	pFrameLen)
{
	...
	if ((pAd->CommonCfg.bBssCoexEnable == TRUE) &&
		(WMODE_CAP_N(pAd->StaCfg[0].wdev.PhyMode)) &&
		WMODE_CAP_2G(&pAd->StaCfg[0].wdev.PhyMode))
	...
	IN	UCHAR						*pPeerMac)
{
	...
	if (pMacEntry == NULL) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("tdls_cmd> ERROR! No such peer in %s!\n",
				 __func__));
		return;
	}
