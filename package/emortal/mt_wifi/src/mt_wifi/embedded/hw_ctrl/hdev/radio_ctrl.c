...
static INT32 rcUpdateBandForBFMU(struct hdev_ctrl *ctrl, BCTRL_INFO_T *pBInfo)
{
	...
	for (i = 0; i < pHwResource->concurrent_bands; i++) {
	   MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s	PhyCtrl[%d].RadioCtrl.IsBfBand = %d\n", __func__, i, pHwResource->PhyCtrl[i].RadioCtrl.IsBfBand));
	}
#ifdef TXBF_SUPPORT
...
*/
static struct radio_dev *rcGetHdevByPhyMode(struct hdev_ctrl *ctrl, UCHAR phymode, UCHAR channel)
{
	UCHAR i;
	...
	for (i = 0 ; i < hw_res->concurrent_bands; i++) {
		...
		if (WMODE_CAP_2G(phymode) && (phy_ctrl->rf_band_cap & RFIC_24GHZ) &&
			(phy_ctrl->RadioCtrl.BandIdx == BAND0))
			rdev = &ctrl->rdev[i];
		else if (WMODE_CAP_5G(phymode) && (phy_ctrl->rf_band_cap & RFIC_5GHZ) &&
			(phy_ctrl->RadioCtrl.BandIdx == BAND1))
			rdev = &ctrl->rdev[i];
		if (rdev) {
	...
	if (!rdev) {
		...
		("%s(): can't find radio for phymode:%u, channel: %d !\n",
		__func__, phymode, channel));
...
VOID rc_radio_init(struct hdev_ctrl *ctrl, UCHAR rfic, UCHAR dbdc_mode)
{
	...
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			("%s(): DBDC MODE=%d, ConcurrentBand=%d\n",
			 __func__, dbdc_mode, pHwResource->concurrent_bands));
	...
	for (i = 0; i < pHwResource->concurrent_bands; i++) {
		...
		if ((phy_ctrl->rf_band_cap) & RFIC_24GHZ)
			radio_ctrl->Channel = rcGetDefaultChannel(WMODE_B);
		else
			radio_ctrl->Channel = rcGetDefaultChannel(WMODE_A);
#ifdef DOT11_HE_AX
		max_nss = ctrl->chip_cap.mcs_nss.max_nss;
		max_ru_num = ctrl->chip_cap.mcs_nss.max_24g_ru_num;
		if ((phy_ctrl->rf_band_cap) & RFIC_5GHZ)
			max_ru_num = ctrl->chip_cap.mcs_nss.max_5g_ru_num;
		init_default_ppe(&phy_ctrl->pe_ctrl.pe_info, max_nss, max_ru_num);
#endif
		radio_ctrl->PhyMode = rcGetDefaultPhyMode(radio_ctrl->Channel);
		...
		MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				("%s(): radio_ctrl=%p,Band=%d,rfcap=%d,channel=%d,PhyMode=%d extCha=0x%x\n",
				 __func__, radio_ctrl, i, phy_ctrl->rf_band_cap,
...
VOID RcReleaseBandForObj(struct hdev_ctrl *ctrl, struct hdev_obj *obj)
{
	...
	rdev = obj->rdev;
	if (!rdev) {
		MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s():rdev is null!!!\n", __func__));
		return;
	}
