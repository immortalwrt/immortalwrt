			     ...
			     struct _HQA_CMD_FRAME *HqaCmdFrame)
{
		 ...
		 ("%s\n", __func__));
	{
		...
		CmdTxContinous(pAd, ATECtrl->PhyMode, ATECtrl->BW,
			       ATECtrl->ControlChl, ATECtrl->Mcs,
			       ATECtrl->TxAntennaSel, 0);
	}
	ResponseToQA(HqaCmdFrame, WRQ, 2, Ret);
	return Ret;
}
				...
				struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	UINT16 Offset;
	UINT16 Len;
	UINT16 Buffer[EEPROM_SIZE >>
		      1]; /* TODO Build warning frame size > 1024 for stack */
				 ...
				 struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	Ret = RTMP_OS_WAIT_FOR_COMPLETION_TIMEOUT(&ATECtrl->cmd_done,
						  ATECtrl->cmd_expire);
...
static INT32 HQA_GetFWInfo(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *WRQ,
			   struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	INT32 Ret = 0;
	struct _ATE_CTRL *ATECtrl = &(pAd->ATECtrl);
		     ...
		     struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	UINT32 Value = 0;
	INT32 Ret = 0;
	static UINT32 RFCR, OMA0R0, OMA0R1, OMA1R0, OMA1R1, OMA2R0, OMA2R1,
		OMA3R0, OMA3R1, OMA4R0, OMA4R1;
...
static INT32 HQA_CMDHandler(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *WRQ,
			    struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	while (TableIndex <
		...
		if ((CmdIndex >= 0) &&
		    (CmdIndex < HQA_CMD_TABLES[TableIndex].CmdSetSize)) {
...
INT32 RtmpDoAte(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *WRQ,
		RTMP_STRING *wrq_name)
{
	...
	default:
		...
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			 ("Unknown magic number of HQA command = %x\n",
			  ATEMagicNum));
		break;
	}
