...
#ifdef FTM_SUPPORT
inline VOID FillTodToa(IN UINT32 tod_high, IN UINT32 tod_low,
		       IN UINT32 toa_high, IN UINT32 toa_low, OUT UINT64 *p_tod,
		       OUT UINT64 *p_toa)
{
		     ...
		     BOOLEAN fgFinalResult, UINT32 TOAECalibrationResult)
{
	...
	if (ptmr_d0->IR == TMR_IR0_TX)
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("TMR(%d%d)PID:0x%02X\n", ptmr_d0->ToaVld,
			  ptmr_d0->TodVld, tmr_d1->field_init.Pid));
	else
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("TMR: TYPE=%x, SUB_TYPE=%x, SN=0x%04X\n",
			  ptmr_d0->Type, ptmr_d0->SubType,
			  ptmr_d2->field.SnField));
...
#endif /* FTM_SUPPORT */
		tmr_raw_dump("TMR RAW data: ", (UCHAR *)tmr,
			     sizeof(TMR_FRM_STRUC));
	...
	if (fgFinalResult == FALSE) {
		toa_low = toa_low +
			  LtfSyncAddr * 50; /* for now, always use CBW 80. */
	...
	MTWF_LOG(
		DBG_CAT_ALL, DBG_SUBCAT_ALL, dbg_lvl,
		("DWORD_0: ByteCnt=%d, NC=%d, TMF=%d, ToaVld=%d, TodVld=%d, tod_fine=%x\n",
		 ptmr_d0->RxByteCnt, ptmr_d0->Nc, ptmr_d0->Tmf, ptmr_d0->ToaVld,
		 ptmr_d0->TodVld, tod_fine));
	...
	if (ptmr_d0->IR == TMR_IR1_RX) {
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, dbg_lvl,
			 ("TYPE=%x, SUB_TYPE=%x, SN=%x\n", ptmr_d0->Type,
			  ptmr_d0->SubType, ptmr_d2->field.SnField));
...
#ifdef FTM_SUPPORT
	{
		...
		if (ptmr_d0->IR == TMR_IR1_RX)
			...
			pFtmEntry = FtmGetPidPendingNode(
				pAd, tmr_d1->field_init.Pid);

		if (!pFtmEntry) {
			MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FTM, DBG_LVL_INFO,
				 ("%s()#%d: invalid FTM entry !!\n", __func__,
				  __LINE__));
			return;
		}
		...
		if (ptmr_d0->IR == TMR_IR0_TX) {
			...
			NdisCopyMemory(&pFtmEntry->Tmr.HwReport, tmr,
				       sizeof(TMR_FRM_STRUC));
			...
			if (os_alloc_mem(NULL, (UCHAR **)&pNode,
					 sizeof(FTM_MAPPING)) ==
			    NDIS_STATUS_FAILURE)
				return;
			...
			NdisCopyMemory(&pNode->tmr.HwReport, tmr,
				       sizeof(TMR_FRM_STRUC));
			pNode->tmr.toa = toa;
			pNode->tmr.tod = tod;
...
INT TmrCtrlInit(RTMP_ADAPTER *pAd, UCHAR TmrType, UCHAR Ver)
{
	...
	if (pAd->pTmrCtrlStruct != NULL) {
		...
		Ret = os_alloc_mem(pAd, (PUCHAR *)&TmrCtrlStructMem,
				   sizeof(TMR_CTRL_STRUCT));
