...
#define TDLS_CHANGE_TO_ACTIVE(__pAd)										\
	{																			\
		BOOLEAN __FlgOld = (__pAd)->StaCfg[0].TdlsInfo.TdlsFlgIsKeepingActiveCountDown;		\
		(__pAd)->StaCfg[0].TdlsInfo.TdlsFlgIsKeepingActiveCountDown = TRUE;					\
		(__pAd)->StaCfg[0].TdlsInfo.TdlsPowerSaveActiveCountDown =							\
				TDLS_POWER_SAVE_ACTIVE_COUNT_DOWN_NUM;		\
		if (__FlgOld == FALSE)	/* send a null frame to trigger asic wake up */	\
			RTMPSendNullFrame(__pAd, &(__pAd)->StaCfg[MAIN_MSTA_ID], __pAd->CommonCfg.TxRate, TRUE, FALSE);	\
	}

/* recover PS mode when TdlsPowerSaveActiveCountDown = 0 */
#define TDLS_RECOVER_POWER_SAVE(__pAd)										\
	{																			\
		(__pAd)->StaCfg[0].TdlsInfo.TdlsFlgIsKeepingActiveCountDown = FALSE;				\
		(__pAd)->StaCfg[0].TdlsInfo.TdlsPowerSaveActiveCountDown = 0;						\
		/* send a null frame to trigger asic sleep */							\
		RTMPSendNullFrame(__pAd, &(__pAd)->StaCfg[MAIN_MSTA_ID], __pAd->CommonCfg.TxRate, TRUE, __pAd->StaCfg[0].PwrMgmt.Psm);				\
	}
#endif /* __TDLS_CMM_H */
