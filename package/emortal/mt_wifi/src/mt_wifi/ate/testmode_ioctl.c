	...
	struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("%s: Band = %d, Control = %d, AntIndex = %d, Phymode = %d, BW = %d, CH = %d, Rate = %d, Central_Ch = %d, TxfdMode = %d\n",
			  __func__, Band, Control, AntMask, tx_mode, BW, Pri_Ch, Rate, Central_Ch, TxfdMode));
	...
	struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	if (ATEOp->GetCfgOnOff) {
		...
		if ((IS_MT7615(pAd) || IS_MT7622(pAd) || IS_P18(pAd) || IS_MT7663(pAd) || IS_AXE(pAd) || IS_MT7626(pAd) || IS_MT7915(pAd)) && Type != 2)
#endif
		{
...
static INT32 HQA_CapWiFiSpectrum(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *WRQ, struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	UINT32 StopCycle = 0, BW = 0, MACTriggerEvent = 0, Band = 0;
	UINT32 CapSource = 0, PhyIdx = 0, SourceAddrLSB = 0, SourceAddrMSB = 0;
		...
		if (ATEOp->SetICapStart) {
			{
				...
				MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("CapSource:%d\n", CapSource));
		...
		if (ATECtrl->reCalInDumpSts == DISABLE_RECAL_IN_DUMP) {
			...
			MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
					 ("%s:LogOnOff, log_type:%d, log_ctrl:%d, log_size:%d\n",
					  __func__, ATECtrl->logStsInDump[0],
...
static INT32 HQA_SetMaxPacExt(PRTMP_ADAPTER ad, RTMP_IOCTL_INPUT_STRUCT *wrq, struct _HQA_CMD_FRAME *hqa_cmd_frame)
{
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("%s: Max Packet Extension:%d (%d us)\n", __func__, TESTMODE_GET_PARAM(ad, TESTMODE_GET_BAND_IDX(ad), max_pkt_ext),
...
static INT32 HQA_SetStaRUSetting(PRTMP_ADAPTER ad, RTMP_IOCTL_INPUT_STRUCT *wrq, struct _HQA_CMD_FRAME *hqa_cmd_frame)
{
	...
	for (segment_idx = 0; segment_idx < 2 ; segment_idx++) {
		for (sta_seq = 0 ; sta_seq < seg_sta_cnt[segment_idx] ; sta_seq++) {
			...
			MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
					 ("%s: ru_segment[%d][0x%x]: ru_idx:%d, rate:%x, ldpc:%d, nss:%d,\n\t\t\t\t start spatial stream:%d, mpdu length=%d, alpha:%d, ru_mu_nss=%d\n",
					  __func__, segment_idx, allocation, ru_sta[sta_seq].ru_index >> 1, ru_sta[sta_seq].rate, ru_sta[sta_seq].ldpc,
					...
					struct _HQA_CMD_FRAME *cmd_frame)
{
	...
	default:
		bw = BAND_WIDTH_20;
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_WARN,
				 ("%s: Cannot find BW with param.sys_bw:%x, forced as 0x%x\n",
				  __func__, param.sys_bw, bw));
		break;
	}
	...
	default:
		...
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_WARN,
				 ("%s: Cannot find BW with param.sys_bw:%x\n, forced as 0x%x",
				  __func__, param.sys_bw, per_pkt_bw));
		break;
	}
...
static INT32 hqa_start_rx_ext(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *wrq, struct _HQA_CMD_FRAME *cmd_frame)
{
	...
	if (param_num > 3) {
		...
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				("%s: param num:%d, tx_mode:%d, ltf_gi:%d, user_idx:%d\n", __func__, param_num, tx_mode, ltf_gi, user_idx));
	}
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			 ("%s: param num:%u, band_sel:%u, rx_path:%x, mac:%02x:%02x:%02x:%02x:%02x:%02x, MU Aid:%d\n",
			  __func__, param_num, band_idx, rx_path, PRINT_MAC(own_mac), TESTMODE_GET_PARAM(pAd, band_idx, mu_rx_aid)));
