	...
	if (pSecConfig->RSNE_Type[ie_idx] == SEC_RSNIE_WPA2_IE) {
		...
		if (pSecConfig->PmfCfg.MFPC == TRUE) {
			...
			if (pSecConfig->bIsWPA2EntOSEN) {
				MTWF_LOG(DBG_CAT_SEC, CATSEC_PMF, DBG_LVL_ERROR,
					("[PMF]%s: Return as RSN OSEN is enabled \n", __func__));
				return TRUE;
			}
#endif
...
BOOLEAN PMF_PerformRxFrameAction(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk)
{
...
#endif /* APCLI_SUPPORT */
			MTWF_LOG(DBG_CAT_SEC, CATSEC_PMF, DBG_LVL_ERROR,
					 ("[PMF]%s: NOT_ROBUST_UNICAST_FRAME, FC->SubType=%d (wcid=%d)\n",
					 __func__, FC->SubType, pRxBlk->wcid));
			return FALSE;
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
		...
		case NOT_ROBUST_UNICAST_FRAME:
			...
			MTWF_LOG(DBG_CAT_SEC, CATSEC_PMF, DBG_LVL_ERROR, ("[PMF]%s: NOT_ROBUST_UNICAST_FRAME, FC->SubType=%d\n", __func__, FC->SubType));
			return FALSE;
	...
	IN RTMP_STRING *arg)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
		...
		MTWF_LOG(DBG_CAT_SEC, CATSEC_PMF, DBG_LVL_ERROR, ("[PMF]%s:: staidx=%d, Desired MFPC=%d\n", __func__
	...
	IN RTMP_STRING *arg)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
		...
		MTWF_LOG(DBG_CAT_SEC, CATSEC_PMF, DBG_LVL_ERROR, ("[PMF]%s:: staidx=%d, Desired MFPR=%d\n", __func__
	...
	IN RTMP_STRING *arg)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
		...
		MTWF_LOG(DBG_CAT_SEC, CATSEC_PMF, DBG_LVL_ERROR, ("[PMF]%s:: staidx=%d, Desired PMFSHA256=%d\n", __func__
	...
	IN RTMP_STRING *arg)
{
	...
	if ((pEntry->SecConfig.PmfCfg.UsePMFConnect == TRUE) &&
		(tr_entry->PortSecured != WPA_802_1X_PORT_SECURED)) {
		MTWF_LOG(DBG_CAT_SEC, CATSEC_PMF, DBG_LVL_ERROR,
				 ("%s: PMF Connection IGNORE THIS PKT DUE TO NOT IN PORTSECURED(wcid = %d)\n", __func__, pEntry->wcid));
		return FALSE;
	}

	if (pEntry->SecConfig.PmfCfg.UsePMFConnect == TRUE) {
		MTWF_LOG(DBG_CAT_SEC, CATSEC_PMF, DBG_LVL_ERROR,
				 ("%s: PMF CONNECTION BUT RECV WEP=0 ACTION, ACTIVE THE SA QUERY(wcid = %d)\n", __func__, pEntry->wcid));
		PMF_MlmeSAQueryReq(pAd, pEntry);
		return TRUE;
	}
#endif /* CONFIG_AP_SUPPORT */
	...
	IN UCHAR *buf)
{
	...
	hex_dump_with_cat_and_lvl("bcn_mmie", buf, LEN_PMF_MMIE + 2, DBG_CAT_SEC, CATSEC_BCNPROT, DBG_LVL_TRACE);
	...
	OUT PULONG pFrameLen)
{
	...
	hex_dump_with_cat_and_lvl("bigtk kde", pFrameBuf,
		LEN_KDE_HDR + 8 + LEN_BIP128_IGTK, DBG_CAT_SEC, CATSEC_BCNPROT, DBG_LVL_OFF); /* todo: change to trace*/
}
