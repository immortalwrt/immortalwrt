			...
			do {
				if (BW_VALUE[reg_class_he[i].spacing] >= BW_VALUE[bw]) {
					...
					if (is_channel_in_channelset(reg_class_he[i].channel_set, Channel) ||
						(is_channel_in_channelset(reg_class_he[i].center_freq, vht_cent_ch_freq(Channel, cfg_he_bw, ch_band)))) {
...
{
	REG_CLASS *reg_class = get_reg_table_by_country(pAd->CommonCfg.CountryCode, PhyMode);
	UINT i = 0, j = 0;

	if (reg_class == NULL) {
		MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				("%s: reg_class is NULL for 6G!!!\n", __func__));
		return;
	}

	for (i = 1; reg_class[i].reg_class != 0 && j < 5; i++) {
		if ((reg_class[i].reg_class > 130 &&
					reg_class[i].reg_class <= 135)) {
