	...
	PRT_802_11_STA_MLME_EVENT mlmeEvent)
{
	...
	if (filsInfo->pending_action) {
		...
		if (elem == NULL) {
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : os_alloc_mem fail.\n", __func__));
			return;
			}
		else
	...
	IN      RTMP_IOCTL_INPUT_STRUCT * wrq)
{
	...
	if (pSyncEvent->apidx >= MAX_BEACON_NUM) {
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			("%s: pSyncEvent->apidx:%d error\n", __func__, pSyncEvent->apidx));
		os_free_mem(pSyncEvent);
		return;
	}
	...
	for (rsne_idx = 0; rsne_idx < SEC_RSNIE_NUM; rsne_idx++) {
		...
		if (pSyncEvent->rsne_len + 3 < MAX_OPT_IE) {
			...
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: syncEvent.rsne_len:%d\n", __func__, pSyncEvent->rsne_len));
		}
	...
	IN      RTMP_IOCTL_INPUT_STRUCT * wrq)
{
	...
	if (KeyEvent->action == FILS_KEY_INSTALL_PTK) {
		...
		if (info) {
			...
			if (keyInfo->KeyLength <= LEN_MAX_PTK)
				os_move_mem(pSecConfig->PTK, &keyInfo->KeyMaterial,
						keyInfo->KeyLength);
			else
				MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
						("%s: keylen:%d\n", __func__, keyInfo->KeyLength));
