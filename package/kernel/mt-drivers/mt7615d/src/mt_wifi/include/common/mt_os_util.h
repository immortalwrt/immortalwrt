...
INT os_usb_vendor_req(struct _RTMP_ADAPTER *pAd, UINT32 TransferFlags,
		      UCHAR RequestType, UCHAR Request, USHORT Value,
		      USHORT Index, PVOID TransferBuffer,
		      UINT32 TransferBufferLength);

#if !defined(COMPOS_WIN) && !defined(COMPOS_TESTMODE_WIN)
VOID os_alloc_dma_buf(IN VOID *pHandler, IN ULONG Length, IN BOOLEAN Cached,
		      OUT VOID **VirtualAddress,
		      OUT OS_PHYSICAL_ADDRESS *phy_addr);

VOID os_free_dma_buf(IN VOID *pHandler, IN ULONG Length,
		     IN PVOID VirtualAddress,
		     IN OS_PHYSICAL_ADDRESS PhysicalAddress);

OS_PACKET
os_alloc_packet(IN VOID *pHandler, IN ULONG Length);

VOID os_free_packet(IN VOID *pHandler, IN OS_PACKET *pPacket);
