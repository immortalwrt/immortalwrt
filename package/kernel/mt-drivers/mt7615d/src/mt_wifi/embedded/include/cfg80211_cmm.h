...
#define RTMP_CFG80211_HOSTAPD_ON(__pAd)                                        \
	(__pAd->net_dev->ieee80211_ptr->iftype == RT_CMD_80211_IFTYPE_AP)
...
#define LENGTH_TDLS_PAYLOAD_H                                                  \
	3 /* payload type(1 byte) + category(1 byte) + action(1 byte) */
...
#define IS_TDLS_SUPPORT(_P)                                                    \
	((_P)->StaCfg[0].wpa_supplicant_info.CFG_Tdls_info.bCfgTDLSCapable ==  \
	 TRUE)
...
enum tdls_entry_op { tdls_insert_entry = 0, tdls_delete_entry };
