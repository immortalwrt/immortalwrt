...
#define MlmeAllocateMemory(_pAd, _ppVA)                                        \
	NdisAllocateMemoryWithTag(_ppVA, MGMT_DMA_BUFFER_SIZE, NIC_TAG)
...
#define COMMON_CFG_OFFSET(field)                                               \
	((UINT)FIELD_OFFSET(RTMP_ADAPTER, CommonCfg) +                         \
	 (UINT)FIELD_OFFSET(COMMON_CONFIG, field))
#define STA_CFG_OFFSET(field)                                                  \
	((UINT)FIELD_OFFSET(RTMP_ADAPTER, StaCfg) +                            \
	 (UINT)FIELD_OFFSET(STA_ADMIN_CONFIG, field))
#define AP_CFG_OFFSET(field)                                                   \
	((UINT)FIELD_OFFSET(RTMP_ADAPTER, ApCfg) +                             \
	 (UINT)FIELD_OFFSET(AP_ADMIN_CONFIG, field))
...
#define NdisInitializeString(Destination, Source)                              \
	{                                                                      \
		...
		NdisAllocateMemory((PVOID *)&(_D->Buffer), _D->MaximumLength,  \
				   0, phyaddr);                                \
		_P = _D->Buffer;                                               \
		while (*_S != '\0') {                                          \
			*_P = (WCHAR)(*_S);                                    \
...
#define RTMP_IO_READ32(_A, _R, _pV)                                            \
	{                                                                      \
		NdisReadRegisterUlong((PULONG)((_A)->CSRBaseAddress + (_R)),   \
				      (_pV));                                  \
	}
#define RTMP_IO_WRITE32(_A, _R, _V)                                            \
	{                                                                      \
		NdisWriteRegisterUlong((PULONG)((_A)->CSRBaseAddress + (_R)),  \
				       (_V));                                  \
	}
#define RTMP_IO_WRITE8(_A, _R, _V)                                             \
	{                                                                      \
		NdisWriteRegisterUchar((PUCHAR)((_A)->CSRBaseAddress + (_R)),  \
				       (_V));                                  \
	}
#define RTMP_IO_WRITE16(_A, _R, _V)                                            \
	{                                                                      \
		ULONG Val;                                                     \
		NdisWriteRegisterUshort((PSHORT)((_A)->CSRBaseAddress + (_R)), \
					(_V));                                 \
	}
...
#define RTMP_SET_PACKET_UP(_p, _prio)                                          \
	((_p)->MiniportReservedEx[sizeof(PVOID)] =                             \
		 ((_p)->MiniportReservedEx[sizeof(PVOID)] & 0xf0) | (_prio))
#define RTMP_GET_PACKET_UP(_p) ((_p)->MiniportReservedEx[sizeof(PVOID)] & 0x0f)

/* b4-b7 as fragment # */
#define RTMP_SET_PACKET_FRAGMENTS(_p, _num)                                    \
	((_p)->MiniportReservedEx[sizeof(PVOID)] =                             \
		 ((_p)->MiniportReservedEx[sizeof(PVOID)] & 0x0f) |            \
		 (_num << 4))
#define RTMP_GET_PACKET_FRAGMENTS(_p)                                          \
	(((_p)->MiniportReservedEx[sizeof(PVOID)] & 0xf0) >> 4)

/* 0x0 ~0x7f: TX to AP's own BSS which has the specified AID. if AID>127, set bit 7 in RTMP_SET_PACKET_EMACTAB too. */
/*(this value also as MAC(on-chip WCID) table index) */
/* 0x80~0xff: TX to a WDS link. b0~6: WDS index */
#define RTMP_SET_PACKET_WCID(_p, _wcid)                                        \
	((_p)->MiniportReservedEx[sizeof(PVOID) + 1] = _wcid)
#define RTMP_GET_PACKET_WCID(_p) ((_p)->MiniportReservedEx[sizeof(PVOID) + 1])

/* 0xff: PKTSRC_NDIS, others: local TX buffer index. This value affects how to a packet */
#define RTMP_SET_PACKET_SOURCE(_p, _pktsrc)                                    \
	((_p)->MiniportReservedEx[sizeof(PVOID) + 2] = _pktsrc)
#define RTMP_GET_PACKET_SOURCE(_p) ((_p)->MiniportReservedEx[sizeof(PVOID) + 2])

/* b0~2: RTS/CTS-to-self protection method */
#define RTMP_SET_PACKET_RTS(_p, _num)                                          \
	((_p)->MiniportReservedEx[sizeof(PVOID) + 3] =                         \
		 ((_p)->MiniportReservedEx[sizeof(PVOID) + 3] & 0xf8) |        \
		 (_num))
#define RTMP_GET_PACKET_RTS(_p)                                                \
	((_p)->MiniportReservedEx[sizeof(PVOID) + 3] & 0x07)
/* b7: see RTMP_S(G)ET_PACKET_EMACTAB */

/* b3~7: TX rate index */
#define RTMP_SET_PACKET_TXRATE(_p, _rate)                                      \
	((_p)->MiniportReservedEx[sizeof(PVOID) + 3] =                         \
		 ((_p)->MiniportReservedEx[sizeof(PVOID) + 3] & 0x07) |        \
		 (_rate << 3))
#define RTMP_GET_PACKET_TXRATE(_p)                                             \
	(((_p)->MiniportReservedEx[sizeof(PVOID) + 3] & 0xf8) >> 3)
...
#define RELEASE_NDIS_PACKET(_pAd, _pPacket, _Status)                           \
	{                                                                      \
		if (RTMP_GET_PACKET_SOURCE(_pPacket) == PKTSRC_NDIS) {         \
			NdisMSendComplete(_pAd->AdapterHandle, _pPacket,       \
					  _Status);                            \
...
_inline VOID RtmpusecDelay(IN ULONG usec)
{
	ULONG i;
...
typedef void (*TIMER_FUNCTION)(IN PVOID SystemSpecific1,
...
/* */
NDIS_STATUS DriverEntry(IN PDRIVER_OBJECT pDriverObject,
