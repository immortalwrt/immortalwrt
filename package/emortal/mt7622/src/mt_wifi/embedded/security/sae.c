...
static UCHAR delete_all_removable_sae_instance(IN SAE_CFG *pSaeCfg)
{
	UINT32 i;
	...
	for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++) {
		...
		MTWF_LOG(
			...
			("idx:%d, v/r:%d/%d, OM=%02x:%02x:%02x:%02x:%02x:%02x, PM=%02x:%02x:%02x:%02x:%02x:%02x\n",
			 ...
			 PRINT_MAC(pSaeIns->own_mac),
			 PRINT_MAC(pSaeIns->peer_mac)));
		...
		MTWF_LOG(
			...
			("\th2e_connect=%d, is_pwd_id_only = %d, removable = %d\n",
			 pSaeIns->h2e_connect, pSaeIns->is_pwd_id_only,
			 pSaeIns->removable));
...
static VOID hkdf_extract(IN const UINT8 key[], IN UINT key_len,
			 IN const UINT8 msg[], IN UINT msg_len, OUT UINT8 mac[],
			 IN UINT hash_len)
{
...
static VOID hkdf_expand(IN UCHAR *hash, IN INT hash_len, IN UCHAR *info,
			IN INT info_len, OUT UCHAR *output, INT output_Len)
{
	if (hash_len == SHA256_DIGEST_SIZE)
		HKDF_expand_sha256(hash, hash_len, info, info_len, output,
				   output_Len);
	else if (hash_len == SHA384_DIGEST_SIZE)
		HKDF_expand_sha384(hash, hash_len, info, info_len, output,
				   output_Len);
	else if (hash_len == SHA512_DIGEST_SIZE)
		MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_ERROR,
			 ("%s(): not support group 21 now\n",
...
static VOID sae_kdf_hash(IN PUINT8 hash, IN INT hash_len, IN PUINT8 label,
			 IN INT label_len, IN PUINT8 data, IN INT data_len,
			 OUT PUINT8 output, IN USHORT len)
{
	if (hash_len == SHA256_DIGEST_SIZE)
		KDF(hash, hash_len, label, label_len, data, data_len, output,
		    len);
	else if (hash_len == SHA384_DIGEST_SIZE)
		KDF_384(hash, hash_len, label, label_len, data, data_len,
			output, len);
	else if (hash_len == SHA512_DIGEST_SIZE)
		MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_ERROR,
			 ("%s(): not support group 21 now\n",
...
VOID sae_cfg_init(IN RTMP_ADAPTER *pAd, IN SAE_CFG *pSaeCfg)
{
	UINT32 i = 0;
...
VOID sae_cfg_deinit(IN RTMP_ADAPTER *pAd, IN SAE_CFG *pSaeCfg)
{
	UINT32 i;
...
SAE_INSTANCE *search_sae_instance(IN SAE_CFG *pSaeCfg, IN UCHAR *own_mac,
				  IN UCHAR *peer_mac)
{
	UINT32 i;
	SAE_INSTANCE *pSaeIns = NULL;
	...
	for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++) {
		...
		if (RTMPEqualMemory(pSaeCfg->sae_ins[i].own_mac, own_mac,
				    MAC_ADDR_LEN) /* ellis */
		    && RTMPEqualMemory(pSaeCfg->sae_ins[i].peer_mac, peer_mac,
				       MAC_ADDR_LEN)) {
			pSaeIns = &pSaeCfg->sae_ins[i];
			break;
		}
	...
	if (pSaeIns && pSaeIns->state == SAE_ACCEPTED && pSaeIns->same_mac_ins)
		pSaeIns = pSaeIns->same_mac_ins;

	return pSaeIns;
}

SAE_INSTANCE *create_sae_instance(IN RTMP_ADAPTER *pAd, IN SAE_CFG *pSaeCfg,
				  IN UCHAR *own_mac, IN UCHAR *peer_mac,
				  IN UCHAR *bssid, IN UCHAR *psk,
				  ...
				  IN UCHAR is_pwd_id_only)
{
	...
	if (pSaeCfg == NULL || own_mac == NULL || peer_mac == NULL ||
	    bssid == NULL || (!is_pwd_id_only && psk == NULL)) {
		MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_ERROR,
			 ("%s():input should not be null\n", __func__));
		return NULL;
	}

	if (is_pwd_id_only && (pwd_id_list_head == NULL ||
			       DlListEmpty(&pwd_id_list_head->list))) {
		MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_ERROR,
	...
	for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++) {
		if (pSaeCfg->sae_ins[i].valid == FALSE) {
			pSaeIns = &pSaeCfg->sae_ins[i];
			sae_ins_init(pAd, pSaeCfg, pSaeIns, own_mac, peer_mac,
				     bssid, psk, pwd_id_list_head,
				     is_pwd_id_only);
			pSaeIns->valid = TRUE;
			pSaeCfg->total_ins++;
			break;
...
VOID delete_sae_instance(IN SAE_INSTANCE *pSaeIns)
{
...
UCHAR set_sae_instance_removable(IN SAE_CFG *pSaeCfg, IN UCHAR *own_mac,
				 IN UCHAR *peer_mac)
{
	SAE_INSTANCE *pSaeIns = search_sae_instance(pSaeCfg, own_mac, peer_mac);

	MTWF_LOG(
		DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_LOUD,
...
VOID sae_ins_init(IN RTMP_ADAPTER *pAd, IN SAE_CFG *pSaeCfg,
		  IN SAE_INSTANCE *pSaeIns, IN UCHAR *own_mac,
		  IN UCHAR *peer_mac, IN UCHAR *bssid, IN UCHAR *psk,
		  IN struct pwd_id_list *pwd_id_list_head,
		  IN UCHAR is_pwd_id_only)
{
	MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_LOUD,
	...
	RTMPInitTimer(pAd, &pSaeIns->sae_retry_timer,
		      GET_TIMER_FUNCTION(sae_auth_retransmit), pSaeIns, FALSE);
...
/* partial */
VOID sae_clear_data(IN SAE_INSTANCE *pSaeIns)
{
	BOOLEAN Cancelled;
	MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_LOUD,
		 ("==> %s()\n", __func__));
...
static VOID sae_record_time_begin(INOUT ULONG *time_interval)
{
	NdisGetSystemUpTime(time_interval);
}

static VOID sae_record_time_end(IN UCHAR *str, INOUT ULONG *time_interval)
{
	ULONG temp;
...
VOID sae_dump_time(IN SAE_TIME_INTERVAL *time_cost)
{
...
#ifdef LINUX
	MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, SAE_COST_TIME_DBG_LVL,
		 (", %u msec", jiffies_to_msecs(time_cost->derive_pwe_time)));
#endif
	MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, SAE_COST_TIME_DBG_LVL,
		 ("\nparse scalar: %lu jiffies",
		  time_cost->parse_commit_scalar_time));
#ifdef LINUX
	MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, SAE_COST_TIME_DBG_LVL,
		 (", %u msec",
		  jiffies_to_msecs(time_cost->parse_commit_scalar_time)));
#endif
	MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, SAE_COST_TIME_DBG_LVL,
		 ("\nparse element: %lu jiffies",
		  time_cost->parse_commit_element_time));
#ifdef LINUX
	MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, SAE_COST_TIME_DBG_LVL,
		 (", %u msec",
		  jiffies_to_msecs(time_cost->parse_commit_element_time)));
#endif
	MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, SAE_COST_TIME_DBG_LVL,
		 ("\nderive scalar: %lu jiffies",
		  time_cost->derive_commit_scalar_time));
#ifdef LINUX
	MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, SAE_COST_TIME_DBG_LVL,
		 (", %u msec",
		  jiffies_to_msecs(time_cost->derive_commit_scalar_time)));
#endif
	MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, SAE_COST_TIME_DBG_LVL,
		 ("\nderive element: %lu jiffies",
		  time_cost->derive_commit_element_time));
#ifdef LINUX
	MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, SAE_COST_TIME_DBG_LVL,
		 (", %u msec",
		  jiffies_to_msecs(time_cost->derive_commit_element_time)));
#endif
	MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, SAE_COST_TIME_DBG_LVL,
		 ("\nk: %lu jiffies", time_cost->derive_k_time));
#ifdef LINUX
	MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, SAE_COST_TIME_DBG_LVL,
		 (", %u msec", jiffies_to_msecs(time_cost->derive_k_time)));
#endif
	MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, SAE_COST_TIME_DBG_LVL,
		 ("\npmk: %lu jiffies", time_cost->derive_pmk_time));
#ifdef LINUX
	MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, SAE_COST_TIME_DBG_LVL,
		 (", %u msec", jiffies_to_msecs(time_cost->derive_pmk_time)));
#endif
	MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, SAE_COST_TIME_DBG_LVL, ("\n"));
...
UCHAR sae_using_anti_clogging(IN SAE_CFG *pSaeCfg)
{
	UINT32 i;
	UINT32 ins_cnt = 0;
	UINT32 open = 0;
...
VOID sae_set_retransmit_timer(IN SAE_INSTANCE *pSaeIns)
{
	MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_LOUD,
		 ("==> %s()\n", __func__));
	RTMPSetTimer(&pSaeIns->sae_retry_timer,
		     pSaeIns->pParentSaeCfg->dot11RSNASAERetransPeriod * 1000);
}

VOID sae_clear_retransmit_timer(IN SAE_INSTANCE *pSaeIns)
{
	BOOLEAN Cancelled;
	MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_LOUD,
		 ("==> %s()\n", __func__));
...
VOID sae_auth_retransmit(IN VOID *SystemSpecific1, IN VOID *FunctionContext,
			 IN VOID *SystemSpecific2, IN VOID *SystemSpecific3)
{
	...
	for (idx = 0; idx < pAd->ApCfg.ApCliNum; idx++) {
		...
		if (MAC_ADDR_EQUAL(pSaeIns->own_mac,
				   apcli_entry->wdev.if_addr) &&
	...
	if ((rept_entry != NULL) && pSaeIns && sae_check_big_sync(pSaeIns)) {
...
static VOID *sae_search_pt_by_group(IN struct sae_pt *pt_list, IN USHORT group)
{
...
UCHAR sae_auth_init(IN RTMP_ADAPTER *pAd, IN SAE_CFG *pSaeCfg,
		    IN UCHAR *own_mac, IN UCHAR *peer_mac, IN UCHAR *bssid,
		    IN UCHAR *psk, IN struct sae_pt *pt_list, IN INT32 group)
{
	SAE_INSTANCE *pSaeIns = search_sae_instance(pSaeCfg, own_mac, peer_mac);
	...
	/* 12.4.8.6.1 Upon receipt of an Initiate event, the parent process shall check whether there exists a protocol instance for
	  ...
	  * protocol instance.
	  */
	if (pSaeIns && ((pSaeIns->state == SAE_COMMITTED) ||
			(pSaeIns->state == SAE_CONFIRMED)))
		return FALSE;

	pSaeIns = create_sae_instance(pAd, pSaeCfg, own_mac, peer_mac, bssid,
				      psk, NULL, FALSE);
	...
	if (sae_group_allowed(pSaeIns, pSaeCfg->support_group, group) !=
	    MLME_SUCCESS)
		goto FAIL;
...
UCHAR sae_handle_auth(IN RTMP_ADAPTER *pAd, IN SAE_CFG *pSaeCfg, IN VOID *msg,
		      IN UINT32 msg_len, IN UCHAR *psk,
		      IN struct sae_pt *pt_list,
		      IN struct sae_capability *sae_cap,
#ifdef DOT11_SAE_PWD_ID_SUPPORT
		      IN struct pwd_id_list *pwd_id_list_head,
#endif
		      IN USHORT auth_seq, IN USHORT auth_status,
		      OUT UCHAR **pmk, OUT UCHAR *is_h2e_connect)
{
#define DATA_SIZE 80
	USHORT res = MLME_SUCCESS;
	FRAME_802_11 *Fr = (PFRAME_802_11)msg;
	SAE_INSTANCE *pSaeIns =
		search_sae_instance(pSaeCfg, Fr->Hdr.Addr1, Fr->Hdr.Addr2);
	...
	case SAE_COMMIT_SEQ:
		if (auth_status != MLME_SUCCESS &&
			...
			if (auth_status == MLME_ANTI_CLOGGING_TOKEN_REQ) {
				...
				is_token_req = TRUE;
				res = sae_parse_commit(pSaeCfg, pSaeIns, msg,
						       msg_len, &token,
						       &token_len,
						       is_token_req);
				...
				if (sae_group != pSaeIns->group) {
					...
					if (pSaeIns->support_group_idx <
					    MAX_SIZE_OF_ALLOWED_GROUP)
						new_sae_group =
							pSaeCfg->support_group
								[pSaeIns->support_group_idx];

					if (new_sae_group != 0) {
						if (sae_group_allowed(
							    pSaeIns,
							    pSaeCfg->support_group,
							    new_sae_group) !=
						    MLME_SUCCESS)
				   ...
				   MLME_UNKNOWN_PASSWORD_IDENTIFIER) {
		...
		if (!pSaeIns || pSaeIns->state == SAE_ACCEPTED) {
			/* 12.4.8.6, the parent process checks the value of Open first.
			  ...
			  */
			SAE_INSTANCE *pPreSaeIns = pSaeIns;
			pSaeIns = create_sae_instance(
				pAd, pSaeCfg, Fr->Hdr.Addr1, Fr->Hdr.Addr2,
		...
		res = sae_parse_commit(pSaeCfg, pSaeIns, msg, msg_len, &token,
				       &token_len, is_token_req);
		...
		if (auth_status == MLME_SAE_HASH_TO_ELEMENT) {
			...
			/* 12.4.4.2.3 Hash-to-curve generation of the password element with ECC groups
			  ...
			  */
			if (pSaeIns->pt == NULL) {
				pSaeIns->pt = sae_search_pt_by_group(
					pt_list, pSaeIns->group);
				if (pSaeIns->pt == NULL) {
		...
		if (token &&
		    sae_check_token(pSaeIns, token, token_len) == FALSE) {
			MTWF_LOG(
	...
	if (res != MLME_SUCCESS && res != SAE_SILENTLY_DISCARDED)
		sae_send_auth(pAd, Fr->Hdr.Addr1, Fr->Hdr.Addr2, Fr->Hdr.Addr3,
			      AUTH_MODE_SAE, auth_seq, res, data, data_len);
	...
	if (pSaeIns && pSaeIns->state == SAE_ACCEPTED) {
		...
		hex_dump_with_lvl("sae success, pmk:", (char *)*pmk, LEN_PMK,
				  DBG_LVL_TRACE);
		return TRUE;
	}
unfinished:
...
USHORT sae_sm_step(IN RTMP_ADAPTER *pAd, IN SAE_INSTANCE *pSaeIns,
		   IN USHORT auth_seq)
{
#define F(a, b) (a << 2 | b)
	USHORT res = MLME_SUCCESS;
...
UCHAR sae_check_big_sync(IN SAE_INSTANCE *pSaeIns)
{
	MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_LOUD,
		 ("==> %s()\n", __func__));
...
UCHAR sae_get_pmk_cache(IN SAE_CFG *pSaeCfg, IN UCHAR *own_mac,
			IN UCHAR *peer_mac, OUT UCHAR *pmkid, OUT UCHAR *pmk)
{
	...
	if (pSaeIns->state != SAE_ACCEPTED || !pSaeIns->own_commit_scalar ||
	...
	if (pmkid) {
		...
		SAE_BN_MOD_ADD_QUICK(pSaeIns->own_commit_scalar,
				     pSaeIns->peer_commit_scalar,
				     pSaeIns->order, &tmp);
...
static VOID sae_renew_token_key(IN SAE_CFG *pSaeCfg)
{
...
UCHAR sae_build_token_req(IN RTMP_ADAPTER *pAd, IN SAE_INSTANCE *pSaeIns,
			  OUT UCHAR *token_req, OUT UINT32 *token_req_len)
{
	...
	RT_HMAC_SHA256(sae_cfg->token_key, SAE_TOKEN_KEY_LEN, pSaeIns->peer_mac,
		       MAC_ADDR_LEN, token_req + 2, SHA256_DIGEST_SIZE);
	*token_req_len = SHA256_DIGEST_SIZE + 2;

	return TRUE;
}

UCHAR sae_check_token(IN SAE_INSTANCE *pSaeIns, IN UCHAR *peer_token,
		      IN UINT32 peer_token_len)
{
	...
	RT_HMAC_SHA256(sae_cfg->token_key, SAE_TOKEN_KEY_LEN, pSaeIns->peer_mac,
		       MAC_ADDR_LEN, token, SHA256_DIGEST_SIZE);
...
VOID sae_parse_commit_token_req(IN SAE_INSTANCE *pSaeIns, IN UCHAR **pos,
				IN UCHAR *end, IN UCHAR **token,
				IN UINT32 *token_len)
{
	MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_LOUD,
		 ("==> %s()\n", __func__));
...
USHORT sae_parse_commit(IN SAE_CFG *pSaeCfg, IN SAE_INSTANCE *pSaeIns,
			IN UCHAR *msg, IN UINT32 msg_len, IN UCHAR **token,
			IN UINT32 *token_len, IN UCHAR is_token_req)
{
	USHORT sae_group;
	USHORT res;
	FRAME_802_11 *Fr = (PFRAME_802_11)msg;
	...
	if (is_token_req == TRUE) {
		...
		sae_parse_commit_token_req(pSaeIns, &pos, end, token,
					   token_len);
		return MLME_SUCCESS;
	}
	...
	sae_record_time_end("parse_commit_scalar_time",
			    &pSaeIns->sae_cost_time.parse_commit_scalar_time);
	/* commit-element */
	...
	res = sae_parse_password_identifier(pSaeIns, &pos, end,
					    pSaeIns->is_pwd_id_only);
...
VOID sae_parse_commit_token(IN SAE_INSTANCE *pSaeIns, IN UCHAR **pos,
			    IN UCHAR *end, IN UCHAR **token,
			    IN UINT32 *token_len)
{
	UINT32 non_token_len = (is_sae_group_ecc(pSaeIns->group) ? 3 : 2) *
			       pSaeIns->prime_len; /* ellis */
	MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_LOUD,
		 ("==> %s()\n", __func__));
	...
	if (*pos + non_token_len < end) {
		if (is_sae_pwd_id_element(*pos + non_token_len, end, NULL) ||
		    (pSaeIns->h2e_connect &&
		     is_sae_rejected_group_element(*pos + non_token_len, end,
						   NULL))) {
			if (token)
				*token = NULL;
...
USHORT sae_parse_commit_scalar(IN SAE_INSTANCE *pSaeIns, IN UCHAR **pos,
			       IN UCHAR *end)
{
	...
	hex_dump_with_lvl("peer scalar:", (char *)*pos, pSaeIns->prime_len,
			  SAE_DEBUG_LEVEL);
	...
	if (pPreSaeIns && (pPreSaeIns->state == SAE_ACCEPTED) &&
	...
	if (pSaeIns->state == SAE_CONFIRMED && (pSaeIns->peer_commit_scalar) &&
	...
	/* 1 < scalar < r */
	if (SAE_BN_IS_ZERO(peer_scalar) || SAE_BN_IS_ONE(peer_scalar) ||
...
USHORT sae_parse_commit_element(IN SAE_INSTANCE *pSaeIns, IN UCHAR **pos,
				IN UCHAR *end)
{
	USHORT res = MLME_UNSPECIFY_FAIL;
	MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_LOUD,
		 ("==> %s()\n", __func__));
	sae_record_time_begin(
		&pSaeIns->sae_cost_time.parse_commit_element_time);

	if (pSaeIns->group_op)
		res = pSaeIns->group_op->sae_parse_commit_element(pSaeIns, pos,
								  end);

	sae_record_time_end("parse_commit_element_time",
			    &pSaeIns->sae_cost_time.parse_commit_element_time);
	return res;
}

UCHAR is_sae_pwd_id_element(IN UCHAR *pos, IN UCHAR *end, OUT UINT32 *len)
{
	if (!(end - pos < 3) && pos[0] == IE_WLAN_EXTENSION && pos[1] >= 1 &&
	    end - pos - 2 >= pos[1] && pos[2] == EID_EXT_PASSWORD_IDENTIFIER) {
		if (len)
			*len = pos[1] - 1;
		return TRUE;
	} else
...
UCHAR is_sae_rejected_group_element(IN UCHAR *pos, IN UCHAR *end,
				    OUT UINT32 *len)
{
	if (!(end - pos < 3) && pos[0] == IE_WLAN_EXTENSION && pos[1] >= 1 &&
	    end - pos - 2 >= pos[1] && pos[2] == EID_EXT_REJECTED_GROUP) {
		if (len)
			*len = pos[1] - 1;
		return TRUE;
	} else
...
USHORT sae_parse_password_identifier(IN SAE_INSTANCE *pSaeIns, IN UCHAR **pos,
				     IN UCHAR *end, IN UCHAR is_pwd_id_only)
{
	...
	if (pSaeIns->pwd_id_list_head != NULL &&
		...
		DlListForEach(list, &pSaeIns->pwd_id_list_head->list,
			      struct pwd_id_list, list)
		{
			if (RTMPEqualMemory(list->pwd_id, pSaeIns->peer_pwd_id,
					    pSaeIns->peer_pwd_id_len)) {
...
USHORT sae_parse_rejected_groups(IN SAE_INSTANCE *pSaeIns, IN UCHAR *pos,
				 IN UCHAR *end)
{
	UINT32 len;
	...
	hex_dump_with_lvl("peer_rejected_group", pSaeIns->peer_rejected_group,
			  len, DBG_LVL_OFF);

	return MLME_SUCCESS;
}

USHORT sae_check_rejected_group(IN SAE_INSTANCE *pSaeIns)
{
	UCHAR i;
	USHORT group;
	...
	if (!pSaeIns->h2e_connect)
		...
		MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_ERROR,
			 ("rejected_group = %d, peer_rejected_group_len = %d\n",
			  pSaeIns->rejected_group,
			  pSaeIns->peer_rejected_group_len));
		return MLME_UNSPECIFY_FAIL;
	}
...
USHORT sae_prepare_commit(IN SAE_INSTANCE *pSaeIns)
{
	USHORT res;
	MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_LOUD,
		 ("==> %s()\n", __func__));
	...
	sae_record_time_end("derive_pwe_time",
			    &pSaeIns->sae_cost_time.derive_pwe_time);
...
USHORT sae_derive_commit(IN SAE_INSTANCE *pSaeIns)
{
	SAE_BN *mask = NULL;
	USHORT res = MLME_SUCCESS;
	UINT32 counter = 0;
	MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_LOUD,
		 ("==> %s()\n", __func__));
	sae_record_time_begin(
		&pSaeIns->sae_cost_time.derive_commit_scalar_time);
	...
	do {
		...
		SAE_BN_MOD_ADD(pSaeIns->sae_rand, mask, pSaeIns->order,
			       &pSaeIns->own_commit_scalar);
	} while (SAE_BN_IS_ZERO(pSaeIns->own_commit_scalar) ||
		 SAE_BN_IS_ONE(pSaeIns->own_commit_scalar));

	sae_record_time_end("derive_commit_scalar_time",
			    &pSaeIns->sae_cost_time.derive_commit_scalar_time);
	sae_record_time_begin(
		&pSaeIns->sae_cost_time.derive_commit_element_time);

	if (pSaeIns->group_info == NULL || pSaeIns->group_op == NULL ||
	    pSaeIns->group_op->sae_derive_commit_element(pSaeIns, mask) ==
		    FALSE)
		res = MLME_UNSPECIFY_FAIL;

	sae_record_time_end("derive_commit_element_time",
			    &pSaeIns->sae_cost_time.derive_commit_element_time);
end:
	SAE_BN_FREE(&mask);
...
USHORT sae_process_commit(IN SAE_INSTANCE *pSaeIns)
{
	UCHAR *k = NULL;
	USHORT res = MLME_SUCCESS;
	MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_LOUD,
	...
	if (pSaeIns->group_op &&
		...
		sae_record_time_end("derive_k_time",
				    &pSaeIns->sae_cost_time.derive_k_time);
		...
		sae_record_time_end("derive_pmk_time",
				    &pSaeIns->sae_cost_time.derive_pmk_time);
	} else
		res = MLME_UNSPECIFY_FAIL;
...
UCHAR sae_derive_key(IN SAE_INSTANCE *pSaeIns, IN UCHAR *k)
{
	...
	if (pSaeIns->h2e_connect && pSaeIns->peer_rejected_group_len != 0) {
		NdisCopyMemory(salt, pSaeIns->peer_rejected_group,
			       pSaeIns->peer_rejected_group_len);
	...
	hkdf_extract(salt, salt_len, k, pSaeIns->prime_len, keyseed, hash_len);

	hex_dump_with_lvl("keyseed:", (char *)keyseed, hash_len,
			  SAE_DEBUG_LEVEL);
	...
	SAE_BN_INIT(&tmp);
	SAE_BN_MOD_ADD(pSaeIns->own_commit_scalar, pSaeIns->peer_commit_scalar,
		       pSaeIns->order, &tmp);
	SAE_BN_BI2BIN_WITH_PAD(tmp, val, &val_len, pSaeIns->prime_len);
	hex_dump_with_lvl("(commit-scalar + peer-commit-scalar) modulo r:",
			  (char *)val, val_len, SAE_DEBUG_LEVEL);
	...
	sae_kdf_hash(keyseed, hash_len, (UINT8 *)"SAE KCK and PMK", 15, val,
		     val_len, keys, hash_len + LEN_PMK);
	NdisCopyMemory(pSaeIns->kck, keys, hash_len);
	NdisCopyMemory(pSaeIns->pmk, keys + hash_len, LEN_PMK);
	hex_dump_with_lvl("kck:", (char *)pSaeIns->kck, hash_len,
			  SAE_DEBUG_LEVEL);
	hex_dump_with_lvl("pmk:", (char *)pSaeIns->pmk, LEN_PMK,
			  SAE_DEBUG_LEVEL);
...
VOID sae_send_auth(IN RTMP_ADAPTER *pAd, IN UCHAR *own_mac, IN UCHAR *peer_mac,
		   IN UCHAR *bssid, IN USHORT alg, IN USHORT seq,
		   IN USHORT status_code, IN UCHAR *buf, IN UINT32 buf_len)
{
	...
	MTWF_LOG(
		...
		("==> %s(), seq = %d, statuscode = %d, own mac addr = %02x:%02x:%02x:%02x:%02x:%02x, peer mac addr = %02x:%02x:%02x:%02x:%02x:%02x\n",
		 __func__, seq, status_code, PRINT_MAC(own_mac),
		 PRINT_MAC(peer_mac)));
	...
	MgtMacHeaderInitExt(pAd, &AuthHdr, SUBTYPE_AUTH, 0, peer_mac, own_mac,
			    bssid);

	MakeOutgoingFrame(pOutBuffer, &FrameLen, sizeof(HEADER_802_11),
			  &AuthHdr, 2, &alg, 2, &seq, 2, &status_code, buf_len,
			  buf, END_OF_ARGS);
	MiniportMMRequest(pAd, 0, pOutBuffer, FrameLen);
	MlmeFreeMemory(pOutBuffer);
}

UCHAR sae_send_auth_commit(IN RTMP_ADAPTER *pAd, IN SAE_INSTANCE *pSaeIns)
{
	UCHAR *buf = NULL;
	UCHAR *pos;
	UCHAR *end;
	...
	SAE_BN_BI2BIN_WITH_PAD(pSaeIns->own_commit_scalar, pos, &len,
			       pSaeIns->prime_len);
	pos += len;

	if (is_sae_group_ecc(pSaeIns->group)) {
		BIG_INTEGER_EC_POINT *element =
			(BIG_INTEGER_EC_POINT *)pSaeIns->own_commit_element;
		len = pSaeIns->prime_len;
		SAE_BN_BI2BIN_WITH_PAD(element->x, pos, &len,
				       pSaeIns->prime_len);
		pos += len;
		len = pSaeIns->prime_len;
		SAE_BN_BI2BIN_WITH_PAD(element->y, pos, &len,
				       pSaeIns->prime_len);
		pos += len;
	} else {
	...
	if (pSaeIns->pwd_id_ptr) {
		...
		NdisMoveMemory(pos + 3, pSaeIns->pwd_id_ptr->pwd_id,
			       strlen(pSaeIns->pwd_id_ptr->pwd_id));
	...
	sae_send_auth(pAd, pSaeIns->own_mac, pSaeIns->peer_mac, pSaeIns->bssid,
		      AUTH_MODE_SAE, SAE_COMMIT_SEQ, status_code, buf,
		      pos - buf);
	os_free_mem(buf);
	return TRUE;
}

UCHAR sae_send_auth_confirm(IN RTMP_ADAPTER *pAd, IN SAE_INSTANCE *pSaeIns)
{
	UCHAR *buf = NULL;
	UCHAR *pos;
	UCHAR *end;
	...
	hex_dump_with_lvl("confirm(pos):", (char *)pos, pSaeIns->kck_len,
			  SAE_DEBUG_LEVEL);
	pos += pSaeIns->kck_len;
	sae_send_auth(pAd, pSaeIns->own_mac, pSaeIns->peer_mac, pSaeIns->bssid,
		      AUTH_MODE_SAE, SAE_CONFIRM_SEQ, MLME_SUCCESS, buf,
		      pos - buf);
	os_free_mem(buf);
	return TRUE;
}

USHORT sae_parse_confirm(IN SAE_INSTANCE *pSaeIns, IN UCHAR *msg,
			 IN UINT32 msg_len)
{
	...
	if (pSaeIns->state == SAE_ACCEPTED &&
		...
		MTWF_LOG(
			DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_WARN,
...
USHORT sae_check_confirm(IN SAE_INSTANCE *pSaeIns, IN UCHAR *peer_confirm)
{
	...
	if (RTMPEqualMemory(peer_confirm, verifier, pSaeIns->kck_len))
		...
		hex_dump_with_lvl("peer_confirm:", (char *)peer_confirm,
				  pSaeIns->kck_len, SAE_DEBUG_LEVEL2);
		hex_dump_with_lvl("verifier:", (char *)verifier,
				  pSaeIns->kck_len, SAE_DEBUG_LEVEL2);
		return MLME_UNSPECIFY_FAIL;
	}
}

SAE_BN *sae_gen_rand(IN SAE_INSTANCE *pSaeIns)
{
	UINT8 *rand = NULL;
	UINT32 i;
	SAE_BN *rand_bi = NULL;
	...
	for (iter = 0; iter < 100; iter++) {
		...
		hex_dump_with_lvl("rand:", (char *)rand, pSaeIns->order_len,
				  SAE_DEBUG_LEVEL);
		SAE_BN_BIN2BI(rand, pSaeIns->order_len, &rand_bi);

		if (SAE_BN_IS_ZERO(rand_bi) || SAE_BN_IS_ONE(rand_bi) ||
...
INT sae_set_k_iteration(IN struct _RTMP_ADAPTER *ad, IN RTMP_STRING *arg)
{
	if (arg == NULL)
		return FALSE;
...
INT sae_set_anti_clogging_th(IN struct _RTMP_ADAPTER *ad, IN RTMP_STRING *arg)
{
	if (arg == NULL)
		return FALSE;
...
USHORT sae_group_allowed(IN SAE_INSTANCE *pSaeIns, IN UCHAR *allowed_groups,
			 IN INT32 group)
{
	MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_LOUD,
		 ("==> %s():\n", __func__));
	...
	if (pSaeIns->group != group) {
		...
		pSaeIns->group_op->sae_group_init(pSaeIns,
						  group); /* ellis exception */
...
static BIG_INTEGER_EC_POINT *sswu(IN EC_GROUP_INFO_BI *ec_group_bi,
				  IN SAE_BN *u)
{
	SAE_BN *m = NULL;
	SAE_BN *u2 = NULL;
	...
	SAE_BN_MOD_MUL(ec_group_bi->z, ec_group_bi->a, ec_group_bi->prime,
		       &tmp);
...
static BIG_INTEGER_EC_POINT *sae_derive_pt_ecc(IN USHORT group, IN UCHAR *psk,
					       IN CHAR *ssid, IN UCHAR ssid_len)
{
	...
	hkdf_expand(pwd_seed, hash_len, "SAE Hash to Element u1 P1", 25,
	...
	hkdf_expand(pwd_seed, hash_len, "SAE Hash to Element u2 P2", 25,
		    pwd_value, pwd_value_len);
...
static VOID *sae_derive_pt_group(IN USHORT group, IN UCHAR *psk, IN CHAR *ssid,
				 IN UCHAR ssid_len)
{
	if (is_sae_group_ecc(group))
...
VOID sae_derive_pt(IN SAE_CFG *pSaeCfg, IN UCHAR *psk, IN CHAR *ssid,
		   IN UCHAR ssid_len, OUT struct sae_pt **pt_list)
{
	...
	for (i = 0; allowed_groups[i] > 0; i++) {
		if (tmp2 == NULL) {
			os_alloc_mem(NULL, (UCHAR **)&tmp2,
				     sizeof(struct sae_pt));
			tmp2->next = NULL;
			tmp->next = tmp2;
			tmp = tmp2;
		}
		tmp->group = allowed_groups[i];
		tmp->pt = (VOID *)sae_derive_pt_group(allowed_groups[i], psk,
						      ssid, ssid_len);
		tmp2 = tmp->next;
	}
}

VOID sae_pt_list_deinit(INOUT struct sae_pt **pt_list)
{
	struct sae_pt *tmp;
...
UCHAR is_sae_group_ecc(IN INT32 group)
{
	switch (group) {
	case 19:
	case 20:
...
UCHAR is_sae_group_ffc(IN INT32 group)
{
	switch (group) {
	case 1:
	case 2:
...
VOID sae_group_init_ecc(IN SAE_INSTANCE *pSaeIns, IN INT32 group)
{
	EC_GROUP_INFO *ec_group = NULL;
	EC_GROUP_INFO_BI *ec_group_bi = NULL;
	MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_LOUD,
...
VOID sae_group_init_ffc(IN SAE_INSTANCE *pSaeIns, IN INT32 group)
{
	UINT32 i;
	...
	if (dh_group == NULL || dh_group_bi == NULL)
		return;
	...
	if (dh_group_bi->is_init == FALSE) {
		...
		SAE_BN_BIN2BI((UINT8 *)dh_group->prime, dh_group->prime_len,
			      &dh_group_bi->prime);
		SAE_BN_BIN2BI((UINT8 *)dh_group->order, dh_group->order_len,
			      &dh_group_bi->order);
		SAE_BN_BIN2BI((UINT8 *)dh_group->generator,
			      dh_group->generator_len, &dh_group_bi->generator);
	}
...
VOID sae_group_deinit_ecc(IN SAE_INSTANCE *pSaeIns)
{
...
VOID sae_group_deinit_ffc(IN SAE_INSTANCE *pSaeIns)
{
...
VOID sae_cn_confirm_ecc(IN SAE_INSTANCE *pSaeIns,
			...
			OUT UCHAR *confirm)
{
	...
	UINT32 prime_len;
	BIG_INTEGER_EC_POINT *own_element =
		(BIG_INTEGER_EC_POINT *)pSaeIns->own_commit_element; /* ellis */
	BIG_INTEGER_EC_POINT *peer_element =
		(BIG_INTEGER_EC_POINT *)pSaeIns->peer_commit_element;
	MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_LOUD,
		 ("==> %s()\n", __func__));
	prime_len = pSaeIns->prime_len; /* ellis */
	SAE_BN_BI2BIN_WITH_PAD(own_element->x, own_element_bin, &prime_len,
			       pSaeIns->prime_len);
	prime_len = pSaeIns->prime_len;
	SAE_BN_BI2BIN_WITH_PAD(own_element->y,
			       own_element_bin + pSaeIns->prime_len, &prime_len,
			       pSaeIns->prime_len);
	prime_len = pSaeIns->prime_len;
	SAE_BN_BI2BIN_WITH_PAD(peer_element->x, peer_element_bin, &prime_len,
			       pSaeIns->prime_len);
	prime_len = pSaeIns->prime_len;
	SAE_BN_BI2BIN_WITH_PAD(peer_element->y,
			       peer_element_bin + pSaeIns->prime_len,
			       &prime_len, pSaeIns->prime_len);

	if (is_send)
		sae_cn_confirm_cmm(pSaeIns, pSaeIns->own_commit_scalar,
				   pSaeIns->peer_commit_scalar, own_element_bin,
				   peer_element_bin, 2 * pSaeIns->prime_len,
				   pSaeIns->send_confirm, confirm);
	else
		sae_cn_confirm_cmm(pSaeIns, pSaeIns->peer_commit_scalar,
				   pSaeIns->own_commit_scalar, peer_element_bin,
				   own_element_bin, 2 * pSaeIns->prime_len,
				   pSaeIns->peer_send_confirm, confirm);
}

VOID sae_cn_confirm_ffc(IN SAE_INSTANCE *pSaeIns,
			IN UCHAR is_send, /* otherwise, is verfication */
			OUT UCHAR *confirm)
{
	...
	prime_len = pSaeIns->prime_len; /* ellis */
	SAE_BN_BI2BIN_WITH_PAD(own_element, own_element_bin, &prime_len,
			       pSaeIns->prime_len);
	prime_len = pSaeIns->prime_len; /* ellis */
	SAE_BN_BI2BIN_WITH_PAD(peer_element, peer_element_bin, &prime_len,
			       pSaeIns->prime_len);
	...
	if (is_send)
		...
		sae_cn_confirm_cmm(pSaeIns, pSaeIns->peer_commit_scalar,
				   pSaeIns->own_commit_scalar, peer_element_bin,
				   own_element_bin, pSaeIns->prime_len,
				   pSaeIns->peer_send_confirm, confirm);
...
VOID sae_cn_confirm_cmm(IN SAE_INSTANCE *pSaeIns, IN SAE_BN *scalar1,
			IN SAE_BN *scalar2, IN UCHAR *element_bin1,
			IN UCHAR *element_bin2, IN UINT32 element_len,
			IN USHORT send_confirm, OUT UCHAR *confirm)
{
	UCHAR *msg;
	UINT32 msg_len =
		sizeof(send_confirm) + 2 * element_len + 2 * pSaeIns->prime_len;
	UINT32 offset = 0;
	UINT32 prime_len;
	MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_LOUD,
	...
	prime_len = pSaeIns->prime_len;
	SAE_BN_BI2BIN_WITH_PAD(scalar1, msg + offset, &prime_len,
			       pSaeIns->prime_len);
	offset += pSaeIns->prime_len;
	NdisMoveMemory(msg + offset, element_bin1, element_len);
	offset += element_len;
	prime_len = pSaeIns->prime_len;
	SAE_BN_BI2BIN_WITH_PAD(scalar2, msg + offset, &prime_len,
			       pSaeIns->prime_len);
	offset += pSaeIns->prime_len;
	NdisMoveMemory(msg + offset, element_bin2, element_len);
	offset += element_len;

	hex_dump_with_lvl("element_bin1:", (char *)element_bin1, element_len,
			  SAE_DEBUG_LEVEL);
	hex_dump_with_lvl("element_bin2:", (char *)element_bin2, element_len,
			  SAE_DEBUG_LEVEL);
	hkdf_extract(pSaeIns->kck, pSaeIns->kck_len, msg, msg_len, confirm,
		     pSaeIns->kck_len);
	hex_dump_with_lvl("confirm:", (char *)confirm, pSaeIns->kck_len,
			  SAE_DEBUG_LEVEL);

	os_free_mem(msg);
}

USHORT sae_parse_commit_element_ecc(IN SAE_INSTANCE *pSaeIns, IN UCHAR **pos,
				    IN UCHAR *end)
{
	...
	EC_GROUP_INFO_BI *ec_group_bi =
		(EC_GROUP_INFO_BI *)pSaeIns->group_info_bi;
	USHORT res = MLME_SUCCESS;
	...
	SAE_BN_BIN2BI(*pos + pSaeIns->prime_len, pSaeIns->prime_len,
		      &peer_element_y);
	hex_dump_with_lvl("peer element x:", (char *)pos, pSaeIns->prime_len,
			  SAE_DEBUG_LEVEL2);
	hex_dump_with_lvl("peer element y:", (char *)pos + pSaeIns->prime_len,
			  pSaeIns->prime_len, SAE_DEBUG_LEVEL2);
	/*
...
USHORT sae_parse_commit_element_ffc(IN SAE_INSTANCE *pSaeIns, IN UCHAR **pos,
				    IN UCHAR *end)
{
	...
	hex_dump_with_lvl("peer element:", (char *)*pos, pSaeIns->prime_len,
			  SAE_DEBUG_LEVEL2);
	...
	SAE_BN_MOD_EXP_MONT(peer_commit_element, pSaeIns->order, pSaeIns->prime,
			    &scalar_op_res);
...
UCHAR sae_derive_commit_element_ecc(IN SAE_INSTANCE *pSaeIns, IN SAE_BN *mask)
{
...
UCHAR sae_derive_commit_element_ffc(IN SAE_INSTANCE *pSaeIns, IN SAE_BN *mask)
{
...
USHORT sae_derive_pwe_ecc(IN SAE_INSTANCE *pSaeIns)
{
	UCHAR counter = 0;
	...
	if (RTMPCompareMemory(pSaeIns->own_mac, pSaeIns->peer_mac,
			      MAC_ADDR_LEN) == 1) {
	...
	if (pSaeIns->pwd_id_ptr) {
		NdisMoveMemory(base + base_len, pSaeIns->pwd_id_ptr->pwd_id,
			       strlen(pSaeIns->pwd_id_ptr->pwd_id));
	...
	for (counter = 1; counter <= k || !res; counter++) {
		...
		hkdf_extract(addrs, sizeof(addrs), msg, msg_len, pwd_seed,
			     sizeof(pwd_seed));
		lsb_pwd_seed = pwd_seed[SHA256_DIGEST_SIZE - 1] & BIT0;

		hex_dump_with_lvl("pwd_seed:", (char *)pwd_seed,
				  sizeof(pwd_seed), SAE_DEBUG_LEVEL);
		/*  z = len(p)
		     pwd-value = KDF-z(pwd-seed, ¡§SAE Hunting and Pecking¡¨, p) */
		KDF(pwd_seed, sizeof(pwd_seed),
		    (UINT8 *)"SAE Hunting and Pecking", 23,
		    (UINT8 *)ec_group->prime, ec_group->prime_len, pwd_value,
		    pSaeIns->prime_len);

		hex_dump_with_lvl("pwd_value:", (char *)pwd_value,
				  pSaeIns->prime_len, SAE_DEBUG_LEVEL);
		...
		if (!res) {
			...
			for (i = 0; i < base_len; i++)
				base[i] =
					RandomByte(pSaeIns->pParentSaeCfg->pAd);
		}
	}
...
USHORT sae_derive_pwe_pt_ecc(IN SAE_INSTANCE *pSaeIns)
{
	UINT32 hash_len;
	...
	ECC_POINT_MUL((BIG_INTEGER_EC_POINT *)pSaeIns->pt, val, ec_group_bi,
		      &res);
...
USHORT sae_derive_pwe_ffc(IN SAE_INSTANCE *pSaeIns)
{
	UCHAR counter = 0;
	UCHAR found = FALSE;
	UCHAR addrs[2 * MAC_ADDR_LEN];
	...
	hex_dump_with_lvl("psk:", (char *)pSaeIns->psk, strlen(pSaeIns->psk),
			  SAE_DEBUG_LEVEL);
	...
	for (counter = 1; counter <= 200; counter++) {
		...
		hex_dump_with_lvl("msg:", (char *)msg, strlen(pSaeIns->psk),
				  SAE_DEBUG_LEVEL);
		if (pSaeIns->pwd_id_ptr) {
			NdisMoveMemory(msg + len, pSaeIns->pwd_id_ptr->pwd_id,
				       strlen(pSaeIns->pwd_id_ptr->pwd_id));
			len += strlen(pSaeIns->pwd_id_ptr->pwd_id);
		}
		NdisMoveMemory(msg + len, &counter, sizeof(counter));
		msg_len = len + sizeof(counter);
		hex_dump_with_lvl("addr:", (char *)addrs, 2 * MAC_ADDR_LEN,
				  SAE_DEBUG_LEVEL);
		hex_dump_with_lvl("msg:", (char *)msg, msg_len,
				  SAE_DEBUG_LEVEL);
		RT_HMAC_SHA256(addrs, sizeof(addrs), msg, msg_len, pwd_seed,
			       sizeof(pwd_seed));
		MTWF_LOG(DBG_CAT_SEC, CATSEC_SAE, DBG_LVL_TRACE,
			 ("%s(): z = len(p) = %d\n", __func__,
		...
		hex_dump_with_lvl("pwd_seed:", (char *)pwd_seed,
				  SHA256_DIGEST_SIZE, SAE_DEBUG_LEVEL);
		hex_dump_with_lvl("prime:", (char *)dh_group->prime,
				  dh_group->prime_len, SAE_DEBUG_LEVEL);
		/*  z = len(p)
		     pwd-value = KDF-z(pwd-seed, ¡§SAE Hunting and Pecking¡¨, p) */
		KDF(pwd_seed, sizeof(pwd_seed),
		    (UINT8 *)"SAE Hunting and Pecking", 23,
		    (UINT8 *)dh_group->prime, dh_group->prime_len, pwd_value,
		    pSaeIns->prime_len);
		hex_dump_with_lvl("pwd_value:", (char *)pwd_value,
				  pSaeIns->prime_len, SAE_DEBUG_LEVEL);
		/* pwd-value should be less than prime */
		...
		if (dh_group->safe_prime) {
			...
			SAE_BN_BIN2BI(tmp, sizeof(tmp), &exp);
			SAE_BN_SUB(
				dh_group_bi->prime, exp,
				&tmp_bi); /* ellis: SAE_BN_SUB(A, B, A) need to be fix */
			SAE_BN_MOD(tmp_bi, dh_group_bi->order, &exp);
		}

		SAE_BN_BIN2BI(pwd_value, dh_group->prime_len, &pwd_value_bi);
		SAE_BN_MOD_EXP_MONT(pwd_value_bi, exp, dh_group_bi->prime,
				    &pwe);
...
UCHAR sae_derive_k_ecc(IN SAE_INSTANCE *pSaeIns, OUT UCHAR *k)
{
...
UCHAR sae_derive_k_ffc(IN SAE_INSTANCE *pSaeIns, OUT UCHAR *k)
{
	SAE_BN *K = NULL;
	SAE_BN *tmp = NULL;
	SAE_BN *tmp2 = NULL;
	...
	SAE_BN_INIT(&tmp);
	SAE_BN_INIT(&tmp2);
	SAE_BN_MOD_EXP_MONT(pwe, pSaeIns->peer_commit_scalar, pSaeIns->prime,
			    &tmp);
	SAE_BN_MOD_MUL(tmp, (SAE_BN *)pSaeIns->peer_commit_element,
		       pSaeIns->prime, &tmp2);
...
USHORT sae_reflection_check_ecc(IN SAE_INSTANCE *pSaeIns)
{
	BIG_INTEGER_EC_POINT *own_commit_element =
		(BIG_INTEGER_EC_POINT *)pSaeIns->own_commit_element;
	BIG_INTEGER_EC_POINT *peer_commit_element =
		(BIG_INTEGER_EC_POINT *)pSaeIns->peer_commit_element;

	if (!pSaeIns->own_commit_scalar ||
	    (SAE_BN_UCMP(pSaeIns->own_commit_scalar,
			 pSaeIns->peer_commit_scalar) != 0) ||
	    !own_commit_element ||
...
USHORT sae_reflection_check_ffc(IN SAE_INSTANCE *pSaeIns)
{
	if (!pSaeIns->own_commit_scalar ||
	    (SAE_BN_UCMP(pSaeIns->own_commit_scalar,
			 pSaeIns->peer_commit_scalar) != 0) ||
	    !pSaeIns->own_commit_element ||
	    (SAE_BN_UCMP(pSaeIns->own_commit_element,
			 pSaeIns->peer_commit_element) != 0))
		return MLME_SUCCESS;
	else
		return SAE_SILENTLY_DISCARDED;
}
