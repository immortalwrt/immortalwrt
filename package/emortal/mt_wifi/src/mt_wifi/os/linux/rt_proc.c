	...
	char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if (!rTmpCSIData) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		("%s:rTmpCSIData fail to alloc mem!\n", __func__));
		return -1;
	}
	...
	if (copy_to_user(buf, &temp[u4StartIdx], u4CopySize)) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("copy to user failed\n"));
		if (rTmpCSIData)
...
int csi_proc_init(RTMP_ADAPTER *pAd)
{
	...
	if (!csi_proc_dir) {
		...
		if (!csi_proc_dir) {
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s:csi_proc_dir get error!\n", __func__));
			return -1;
		}
	}
	...
	if (!prEntry) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s:prEntry is NULL!\n", __func__));
		return -1;
	}
...
int csi_proc_deinit(RTMP_ADAPTER *pAd)
{
	...
	if (!csi_proc_dir) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s:csi_proc_dir is NULL!\n", __func__));
		return -1;
	}
	...
	if (csi_if_num == 0) {
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		("%s:csi_if_num is error!(%d)\n", __func__, (INT32)csi_if_num));
