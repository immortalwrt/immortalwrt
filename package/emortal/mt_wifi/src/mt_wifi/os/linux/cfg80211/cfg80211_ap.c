...
static INT CFG80211DRV_UpdateApSettingFromBeacon(PRTMP_ADAPTER pAd, UINT mbss_idx, CMD_RTPRIV_IOCTL_80211_BEACON *pBeacon)
{
	...
	if (ext_cap_ie != NULL) {
		...
		if (ext_cap.qosmap) {
			...
			hotspot_update_bss_info_to_cr4(pAd, mbss_idx);
	...
	if (md_ie != NULL) {
		...
		MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				("%s::MDID::%x %x FToverDS:%d RsrCap:%d\n", __func__,
...
#define MCAST_WCID_TO_REMOVE 0
	...
	if ((pAd->cfg80211_ctrl.Cfg80211VifDevSet.vifDevList.size > 0) &&
		...
		if (!Ret) {
			MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(): register wdev fail\n", __func__));
...
#endif /* RT_CFG80211_P2P_CONCURRENT_DEVICE */
	{
		...
		if (IS_MT7615(pAd) || IS_MT7622(pAd) || IS_MT7626(pAd) || IS_MT7915(pAd)) {
	...
	UINT threshold)
{
	...
	if ((threshold > 0) && (threshold <= MAX_ACK_THRESHOLD))
		...
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			("[%s](%d): NOT support this funcation.\n", __func__, __LINE__));
		return FALSE;
}
...
#if defined(MT7615) || defined(MT7626) || defined(MT7622) || defined(MT7915)
				if (IS_MT7615(pAd) || IS_MT7626(pAd) || IS_MT7622(pAd) || IS_MT7915(pAd)) {
						struct _ASIC_SEC_INFO *info = NULL;
	...
	IN VOID 								*pData)
{
	...
	if (pEntry) {
		...
		MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				("%s:Can't find pEntry in MacTable to AddRemove:%d\n",
				__func__, pApPmkidEntry->AddRemove));
	}
