	...
	if ((*Ptr == WFD_TUNNELED_PROBE_REQ) ||
		...
		for (idx = MAX_NUM_OF_TDLS_ENTRY - 1; idx >= 0; idx--) {
			...
			if (pTDLS->Valid && MAC_ADDR_EQUAL(PeerAddr, pTDLS->MacAddr)) {
				MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::find the same entry and discard it\n", __func__));
				break;
			}
		}
		...
		if (idx < 0) {
			for (idx = (MAX_NUM_OF_TDLS_ENTRY - 1); idx >= 0; idx--) {
				...
				if (!pTDLS->Valid) {
					...
					pTDLS->Valid = TRUE;
					NdisMoveMemory(pTDLS->MacAddr, PeerAddr, MAC_ADDR_LEN);
					MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s() -create a new entry\n", __func__));
					break;
				}
			}
		}
		...
		if (idx < 0) {
			...
			if ((pAd->StaCfg[0].WfdCfg.bWfdEnable) &&
				...
				if (StatusCode == MLME_REQUEST_WITH_INVALID_PARAM)
					...
					MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s:: My WFD Device Type = %d, Peer WFD Device Type = %d\n", __func__,
	...
	if (StatusCode == MLME_SUCCESS) {
		...
		if (pMacEntry) {
			...
			if ((pAd->StaCfg[0].wdev.PhyMode >= PHY_11ABGN_MIXED) || (pAd->CommonCfg.bWmmCapable)) {
				...
				MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("TDLS -  WMM Capable\n"));
			} else
