...
VOID next_key(IN PUCHAR key, IN INT round)
{
	UCHAR rcon;
	UCHAR sbox_key[4];
	UCHAR rcon_table[12] = { 0x01, 0x02, 0x04, 0x08, 0x10, 0x20,
...
void construct_mic_header1(unsigned char *mic_header1, int header_length,
			   unsigned char *mpdu)
{
	...
	mic_header1[3] =
		mpdu[1] & 0xc7; /* Mute retry, more data and pwr mgt bits */
...
void construct_mic_header2(unsigned char *mic_header2, unsigned char *mpdu,
...
void construct_mic_iv(unsigned char *mic_iv, int qc_exists, int a4_exists,
		      unsigned char *mpdu, unsigned int payload_length,
...
void construct_ctr_preload(unsigned char *ctr_preload, int a4_exists,
			   int qc_exists, unsigned char *mpdu,
			   unsigned char *pn_vector, int c)
{
	int i = 0;
	...
	for (i = 8; i < 14; i++)
		ctr_preload[i] =
			pn_vector[i - 8]; /* ctr_preload[8:13] = PN[0:5] */
			   ...
			   IN ULONG DataByteCnt, IN PCIPHER_KEY pWpaKey)
{
	...
	payload_len = DataByteCnt - HeaderLen - 8 -
		      8; /* 8 bytes for CCMP header , 8 bytes for MIC*/
	...
	for (i = 0; i < num_blocks; i++) {
		construct_ctr_preload(ctr_preload, a4_exists, qc_exists, pData,
				      PN, i + 1);
...
/*
	...
	========================================================================
*/
VOID RTMPConstructCCMPAAD(IN PUCHAR pHdr, IN BOOLEAN isDataFrame,
			  IN UINT8 a4_exists, IN UINT8 qc_exists,
...
/*
	...
	========================================================================
*/
VOID RTMPConstructCCMPNonce(IN PUCHAR pHdr, IN UINT8 a4_exists,
			    IN UINT8 qc_exists, IN BOOLEAN isMgmtFrame,
			    IN UCHAR *pn, OUT UCHAR *nonce_hdr,
...
VOID CCMP_test_vector(IN PRTMP_ADAPTER pAd, IN INT input)
{
	...
	UINT8 CCM_NONCE[13] = { 0x00, 0x50, 0x30, 0xf1, 0x84, 0x44, 0x08,
				0xb5, 0x03, 0x97, 0x76, 0xe7, 0x0c };
	UINT8 P_TEXT_DATA[20] = { 0xf8, 0xba, 0x1a, 0x55, 0xd0, 0x2f, 0x85,
				  0xae, 0x96, 0x7b, 0xb6, 0x2f, 0xb6, 0xcd,
				  0xa8, 0xeb, 0x7e, 0x78, 0xa0, 0x50 };
	UINT8 C_TEXT_DATA[28] = { 0xf3, 0xd0, 0xa2, 0xfe, 0x9a, 0x3d, 0xbf,
				  0x23, 0x42, 0xa6, 0x43, 0xe4, 0x32, 0x46,
				  0xe8, 0x0c, 0x3c, 0x04, 0xd0, 0x19, 0x78,
				  0x45, 0xce, 0x0b, 0x16, 0xf9, 0x76, 0x23 };
	UINT8 res_buf[100];
	UINT res_len = 0;
	...
	if (AES_CCM_Encrypt(res_buf, sizeof(P_TEXT_DATA), TK, sizeof(TK),
			    CCM_NONCE, sizeof(CCM_NONCE), AAD, sizeof(AAD), 8,
			    res_buf, &res_len) == 0) {
	...
	if (AES_CCM_Decrypt(res_buf, sizeof(C_TEXT_DATA), TK, 16, CCM_NONCE,
			    sizeof(CCM_NONCE), AAD, sizeof(AAD), 8, res_buf,
			    &res_len) == 0) {
