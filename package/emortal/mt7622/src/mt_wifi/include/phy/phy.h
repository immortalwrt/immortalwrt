...
#define RFIC_3322                                                              \
	12 /* 2.4G 2T2R with PA (RT3352/RT3371/RT3372/RT3391/RT3392) */
...
#define RFIC_IS_5G_BAND(__pAd)                                                 \
	((__pAd->RfIcType == RFIC_2850) || (__pAd->RfIcType == RFIC_2750) ||   \
	 (__pAd->RfIcType == RFIC_3052) || (__pAd->RfIcType == RFIC_2853) ||   \
	 (__pAd->RfIcType == RFIC_3053) || (__pAd->RfIcType == RFIC_3853) ||   \
	 (__pAd->RfIcType == RFIC_5592) || (__pAd->RfIcType == RFIC_7650) ||   \
	 (__pAd->RfIcType == RFIC_7610E) || (__pAd->RfIcType == RFIC_7610U) || \
	 (__pAd->RfIcType == RFIC_7662) || (__pAd->RfIcType == RFIC_7612) ||   \
	 (__pAd->RfIcType == RFIC_7636) || (__pAd->RfIcType == RFIC_7615) ||   \
	 (__pAd->RfIcType == RFIC_7636) || (__pAd->RfIcType == RFIC_7637) ||   \
	 (__pAd->RfIcType == RFIC_P18) || (__pAd->RfIcType == RFIC_7663) ||    \
	 (__pAd->RfIcType == RFIC_UNKNOWN))

#define BOARD_IS_2G_ONLY(__pAd) 0
#define BOARD_IS_5G_ONLY(__pAd)                                                \
	((__pAd->RfIcType == RFIC_7611) || (__pAd->RfIcType == RFIC_7615A))
...
#define MAX_BUSY_COUNT                                                         \
	20 /* Number of retry before failing access BBP & RF indirect register */
#else
#define MAX_BUSY_COUNT                                                         \
	100 /* Number of retry before failing access BBP & RF indirect register */
#define MAX_BUSY_COUNT_US                                                      \
	2000 /* Number of retry before failing access BBP & RF indirect register */
#endif
...
typedef struct phy_ops {
	INT (*bbp_is_ready)(struct _RTMP_ADAPTER *pAd);
	UCHAR (*get_random_seed_by_phy)(struct _RTMP_ADAPTER *pAd);
	INT(*filter_coefficient_ctrl)
	(struct _RTMP_ADAPTER *pAd, UCHAR Channel);
	INT(*bbp_set_agc)
	(struct _RTMP_ADAPTER *pAd, UCHAR agc, RX_CHAIN_IDX chain);
	INT(*bbp_get_agc)
	(struct _RTMP_ADAPTER *pAd, CHAR *agc, RX_CHAIN_IDX chain);
	INT (*bbp_set_mmps)(struct _RTMP_ADAPTER *pAd, BOOLEAN ReduceCorePower);
	INT (*bbp_set_bw)(struct _RTMP_ADAPTER *pAd, UINT8 bw);
	INT (*bbp_set_ctrlch)(struct _RTMP_ADAPTER *pAd, UINT8 ext_ch);
	INT (*bbp_set_rxpath)(struct _RTMP_ADAPTER *pAd, INT rxpath);
	INT (*bbp_set_txdac)(struct _RTMP_ADAPTER *pAd, INT tx_dac);
	INT(*bbp_tx_comp_init)
	(struct _RTMP_ADAPTER *pAd, INT adc_insel, INT tssi_mode);
	INT (*bbp_get_temp)(struct _RTMP_ADAPTER *pAd, CHAR *temp_val);
	...
	INT (*dynamic_vga_adjust)(struct _RTMP_ADAPTER *pAd);
#endif /* DYNAMIC_VGA_SUPPORT */
	INT32(*ShowPartialBBP)
	(struct _RTMP_ADAPTER *pAd, UINT32 Start, UINT32 End);
	INT32 (*ShowAllBBP)(struct _RTMP_ADAPTER *pAd);
	INT32(*ShowPartialRF)
	(struct _RTMP_ADAPTER *pAd, UINT32 Start, UINT32 End);
	INT32 (*ShowAllRF)(struct _RTMP_ADAPTER *pAd);
#ifdef CONFIG_AP_SUPPORT
	UCHAR(*AutoCh)
	(struct _RTMP_ADAPTER *pAd, struct wifi_dev *pwdev, ChannelSel_Alg Alg,
	 BOOLEAN IsABand);
#endif /*CONFIG_AP_SUPPORT*/
#ifdef SMART_CARRIER_SENSE_SUPPORT
	INT (*Smart_Carrier_Sense)(struct _RTMP_ADAPTER *pAd);
