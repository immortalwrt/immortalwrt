...
INT Show_ReptTable_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	for (band_idx = 0; band_idx < DBDC_BAND_NUM; band_idx++) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			("Band_%d_RpEn(%d),RpEnByAnyBand(%d),RpEn(%d)\n",
			band_idx,
	...
	for (CliIdx = 0; CliIdx < GET_MAX_REPEATER_ENTRY_NUM(cap); CliIdx++) {
		...
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_ERROR,
				("%-3d", wdev->func_idx));
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_ERROR,
				("%-5d", HcGetBandByWdev(wdev)));
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_ERROR,
				("%-4d", pReptCliEntry->MatchLinkIdx));
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_ERROR,
				("%-5d", pReptCliEntry->pMacEntry ?
				pReptCliEntry->pMacEntry->wcid : 0));
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_ERROR,
				("%-4d", pReptCliEntry->CliEnable));
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_ERROR,
				("%-4d", pReptCliEntry->CliValid));
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_ERROR,
				("%-8d", pReptCliEntry->Cli_Type));
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_ERROR,
				("%-6d", pReptCliEntry->bBlockAssoc));
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_ERROR,
				("%-5d", pReptCliEntry->CliConnectState));
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_ERROR,
				("%-5lu", wdev->cntl_machine.CurrState));
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_ERROR,
				("%-5lu", wdev->auth_machine.CurrState));
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_ERROR,
...
VOID RepeaterCtrlInit(RTMP_ADAPTER *pAd)
{
	...
	if (pAd->ApCfg.bMACRepeaterEn == TRUE) {
		...
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_ERROR,
				 ("%s, repeater feature already init. by one of bands repeater\n", __func__));
		return;
	}
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_OFF, ("%s() is done\n", __func__));
	NdisReleaseSpinLock(&pAd->ApCfg.ReptCliEntryLock);
}
...
VOID RepeaterCliReset(RTMP_ADAPTER *pAd)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_TRACE, ("%s() is done\n", __func__));
	...
	PUCHAR pAddr)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_ERROR,
		("%s:repeater_entry_time(%ld),CliIdx=%d\n",
		__func__, pReptCliEntry->CliTriggerTime, CliIdx));
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	if (pAd->ApCfg.bMACRepeaterEn &&
		...
		for (i = 0; i < MAX_APCLI_NUM; i++) {
			...
			MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_INFO, (" %s(): i=%d,%d,%d,%d,%d,%d\n",
					 __func__, (int)i,
			...
			if (pReptCliEntry->CliEnable && pReptCliEntry->pMacEntry) {
				...
				if (tr_entry != NULL) {
					...
					if (!IS_REPT_LINK_UP(pReptCliEntry)) {
						MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_ERROR,
							("%s:repeater_entry(%d),time(%ld),allocated->not linkup->overtime->del rp_entry\n",
							__func__, pReptCliEntry->CliIdx, pReptCliEntry->CliTriggerTime));
...
#endif
						MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_ERROR,
							("%s:repeater_entry(%d),time(%ld),allocated->linkup->overtime->disconnect\n",
							__func__, pReptCliEntry->CliIdx, pReptCliEntry->CliTriggerTime));
				...
				if (RTMP_TIME_AFTER(pAd->Mlme.Now32, (pReptCliEntry->CliTriggerTime + (10 * OS_HZ)))) {
					if (!pReptCliEntry->CliValid) {
						MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_ERROR,
							("%s:repeater_entry(%d),pMacEntry(NULL),En(%d),Vld(%d)\n",
							__func__, pReptCliEntry->CliIdx, pReptCliEntry->CliEnable, pReptCliEntry->CliValid));
						HW_REMOVE_REPT_ENTRY(pAd, CliIdx);
					}
				}
			}
